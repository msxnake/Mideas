...
import { Boss, BossAttack, BossCrushMovement, BossNeckChain, BossPhase, Tile, Sprite } from '../../../types';
...
function renderPhaseRecord(
  ...
  return [
    ...
    ], 'tileMatrix,collision,neck,crush,attacks')
  ].join('\n');
}
 ...
 * - update_boss_system
 *   input: none
 *   output: boss_runtime_tick increments once per call
 *   clobbers: A
 */
...
`;
  ...
  lines.push(`; Register Contract:`);
  lines.push(`; input: none`);
  lines.push(`; output: boss_runtime_tick increments once per call`);
  lines.push(`; clobbers: A`);
