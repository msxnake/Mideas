...
import { getSpriteDrawableLayerIndexes } from '../../../components/utils/spriteUtils';
...
const BOSS_ATTACK_DRAW_LABELS: Record<string, string> = {
...
interface RuntimeBossEntry {
  boss: Boss;
  originalIndex: number;
}

export interface BossDataPointerConfig {
  windowMaskExpr: string;
  windowBaseExpr: string;
  bankDivisorExpr: string;
}

interface BossDataRenderResult {
  bossTableRows: string[];
  detailBlocks: string[];
}

export interface BossDataBankSectionResult {
  asm: string;
  bankCount: number;
  banks: Array<{
    bank: number;
...
function collectPlacedBossAssetIds(analysis: ProjectAnalysis): Set<string> {
  const bossIds = new Set<string>();
  (analysis.screenMaps || []).forEach(screen => {
    ((screen as any).bossInstances || []).forEach((instance: { bossAssetId?: string }) => {
...
function selectRuntimeBossEntries(analysis: ProjectAnalysis, bosses: Boss[]): RuntimeBossEntry[] {
...
function collectUsedBossAttackIds(boss: Boss): Set<string> {
...
function stripUnusedBossBehaviorRuntime(asm: string, features: BossFeatureSet): string {
  let optimizedAsm = asm;
...
function stripUnusedBossGeneralRuntime(asm: string): string {
...
function word(value: unknown, fallback = 0): number {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return fallback & 0xffff;
...
function chunkedDb(bytes: number[], perLine = 16): string {
  if (bytes.length === 0) return '    db #FF';
  const lines: string[] = [];
...
function bossMatrixKey(width: number, height: number, bytes: number[]): string {
  return `${width}x${height}:${bytes.join(',')}`;
}

function directionId(direction: BossAttack['projectileDirection'] | BossCrushMovement['direction'] | undefined): number {
  switch (direction) {
    case 'right': return 1;
    case 'up': return 2;
    case 'down': return 3;
...
function buildIndexById<T extends { id: string }>(items: T[] | undefined): Map<string, number> {
  return new Map((items || []).map((item, index) => [item.id, index]));
}

function getTileIndex(tileId: string | null | undefined, tileIndexById: Map<string, number>): number {
  if (!tileId) return 255;
  const index = tileIndexById.get(tileId);
  return index === undefined ? 255 : clampByte(index, 255);
}

function getSpriteIndex(spriteId: string | undefined, spriteIndexById: Map<string, number>): number {
  if (!spriteId) return 255;
  const index = spriteIndexById.get(spriteId);
  return index === undefined ? 255 : clampByte(index, 255);
}

function getAttackTypeId(type: BossAttack['type']): number {
  switch (type) {
    case 'Melee': return 1;
...
function hexToMSX1Index(hex: string): number {
  ...
  for (const color of MSX1_PALETTE) {
...
function getProjectileSpriteMeta(attack: BossAttack, analysis: ProjectAnalysis): { layerCount: number; frameCount: number; color0: number; color1: number } {
  const sprite = attack.spriteAssetId
    ? (analysis.sprites || []).find(candidate => candidate.id === attack.spriteAssetId)
    : undefined;
  const drawableLayers = sprite ? getSpriteDrawableLayerIndexes(sprite) : [];
  const palette = (sprite as any)?.spritePalette || [];
  const backgroundColor = (sprite as any)?.backgroundColor;
  const colors = drawableLayers
    .map(layerIndex => {
      const hex = palette[layerIndex];
      if (!hex || (backgroundColor && hex === backgroundColor)) return 0;
  ...
  return {
    ...
    frameCount: clampByte(Math.max(1, (sprite as any)?.frames?.length || 1)),
    color0,
...
function getBehaviorActionTypeId(type: BossBehaviorAction['type'] | undefined): number {
  switch (type) {
...
function getBehaviorTargetTypeId(type: BossBehaviorTarget['type'] | undefined): number {
  switch (type) {
...
function behaviorActionDuration(action: BossBehaviorAction): number {
  switch (action.type) {
    case 'wait':
...
function getBehaviorTarget(action: BossBehaviorAction): BossBehaviorTarget | undefined {
  return 'target' in action ? action.target : undefined;
}

const CURRENT_FORM_ID = '__phase_current_form';

function buildFormIndexById(phase: BossPhase): Map<string, number> {
...
function getFormIndex(formId: string | undefined, formIndexById: Map<string, number>): number {
  if (!formId) return 0;
  return formIndexById.get(formId) ?? 0;
}

function renderBehaviorLoop(
  label: string,
  phase: BossPhase,
  attackIndexById: Map<string, number>,
  formIndexById: Map<string, number>
): string {
  const loop = phase.behaviorLoop || [];
  const expandedActions: Array<{
    sourceIndex: number;
  ...
  const sourceIndexToExpandedIndex = new Map<number, number>();
  ...
  expandedActions.forEach(item => {
    ...
    actionBytes.push(
      item.typeId ?? getBehaviorActionTypeId(action.type),
      clampByte(item.duration ?? behaviorActionDuration(action), 1),
      getBehaviorTargetTypeId(target?.type),
      target?.type === 'bossRelative' ? signedByte(target.dxChar || 0) : clampByte(target?.xChar || 0),
      target?.type === 'bossRelative' ? signedByte(target.dyChar || 0) : clampByte(target?.yChar || 0),
      aux0,
      clampByte(target?.framesAhead || 0),
      0
    );
  });
...
function buildTileMatrixBytes(phase: BossPhase, tileIndexById: Map<string, number>, analysis: ProjectAnalysis, bossStartYChar = 0): number[] {
...
interface BossMatrixRenderResult {
  label: string;
  block?: string;
}

interface BossMatrixDeduper {
  readonly zeroWeakLabel: string;
  readonly zeroWeakCellCount: number;
  renderTile(label: string, phase: BossPhase, tileIndexById: Map<string, number>, analysis: ProjectAnalysis, bossStartYChar?: number): BossMatrixRenderResult;
  renderWeak(label: string, phase: BossPhase): BossMatrixRenderResult;
}

function getBossPhaseCellCount(phase: BossPhase): number {
  const width = phase.dimensions?.width || 0;
  const height = phase.dimensions?.height || 0;
  return Math.max(0, width * height);
}

function collectMaxBossWeakMatrixCellCount(bosses: Boss[]): number {
  let maxCells = 0;
  bosses.forEach(boss => {
    (boss.phases || []).forEach(phase => {
...
function createBossMatrixDeduper(zeroWeakCellCount: number): BossMatrixDeduper {
  const tileLabelsByKey = new Map<string, string>();
  const weakLabelsByKey = new Map<string, string>();
...
function renderFormTable(
  ...
  analysis: ProjectAnalysis,
  bossStartYChar = 0,
  currentTileMatrixLabel?: string,
  currentWeakMatrixLabel?: string,
  matrixDeduper?: BossMatrixDeduper,
  pointerConfig?: BossDataPointerConfig
): string[] {
...
function renderCollisionMatrix(label: string, phase: BossPhase): string {
  const width = phase.dimensions?.width || 0;
  const height = phase.dimensions?.height || 0;
  const bytes: number[] = [];
  for (let y = 0; y < height; y++) {
...
function buildWeakMatrixBytes(phase: BossPhase): number[] {
  const width = phase.dimensions?.width || 0;
  const height = phase.dimensions?.height || 0;
...
function renderNeckChain(label: string, neckChain: BossNeckChain | undefined): string {
...
function renderCrushMovement(label: string, movement: BossCrushMovement | undefined): string {
...
function renderAttackSequence(label: string, phase: BossPhase, attackIndexById: Map<string, number>): string {
...
`;
  return wrapMideasAsmBlock(
    stripUnusedBossGeneralRuntime(stripUnusedBossBehaviorRuntime(stripUnusedBossAttackRuntime(asm, features), features)),
    {
...
function renderBossDataSections(
  analysis: ProjectAnalysis,
  runtimeBossEntries: RuntimeBossEntry[],
  runtimeFeatures: BossFeatureSet,
  tileIndexById: Map<string, number>,
  spriteIndexById: Map<string, number>,
  matrixDeduperFactory: () => BossMatrixDeduper,
  pointerConfig?: BossDataPointerConfig
): BossDataRenderResult {
  ...
  runtimeBossEntries.forEach(({ boss, originalIndex }) => {
    ...
    const phaseRecordLabels: string[] = [];
    const phaseDataBlocks: string[] = [];
...
function estimateBossDataAsmBytes(asm: string): number {
  return asm.split(/\r?\n/).reduce((total, line) => {
...
export function generateBossDataBankSections(
  analysis: ProjectAnalysis,
  firstPhysicalBank: number,
  dataZoneSize: number,
  pointerConfig: BossDataPointerConfig
): BossDataBankSectionResult {
  const allBosses = (analysis.bosses || []) as Boss[];
  ...
  const bankSections: string[] = [];
  const bankMetadata: BossDataBankSectionResult['banks'] = [];

  runtimeBossEntries.forEach((entry, bankOffset) => {
    const { detailBlocks } = renderBossDataSections(
      analysis,
      [entry],
      runtimeFeatures,
      tileIndexById,
      spriteIndexById,
      () => createBossMatrixDeduper(collectMaxBossWeakMatrixCellCount([entry.boss])),
      pointerConfig
    );
 ...
 *   preserves: IX
 */
export function generateBossesFile(analysis: ProjectAnalysis, options: { includeBossData?: boolean } = {}): string {
...
`;
  ...
  const { bossTableRows, detailBlocks } = includeBossData
    ? renderBossDataSections(
        analysis,
        runtimeBossEntries,
        runtimeFeatures,
        tileIndexById,
        spriteIndexById,
        () => createBossMatrixDeduper(collectMaxBossWeakMatrixCellCount(bosses))
      )
