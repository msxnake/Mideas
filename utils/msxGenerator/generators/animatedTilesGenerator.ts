...
interface PreparedAnimationGroup {
  label: string;
  groupId: string;
  speed: number;
  targetTileId: string;
...
interface PreparedTransformGroup {
  label: string;
  groupId: string;
  speed: number;
  targetTileId: string;
...
function parseFrameSuffix(name: string): { groupId: string; frameOrder: number } | null {
  ...
  if (!groupId || Number.isNaN(frameOrder)) return null;
  return { groupId, frameOrder };
}

const TRANSFORM_OPERATION_CODES: Record<string, number> = {
  rotate_left: 1,
  rotate_right: 2,
  shift_left: 3,
  shift_right: 4,
...
function normalizeAnimationMode(value: any): 'frames' | 'transform' {
  const text = String(value || '').trim().toLowerCase();
  return text === 'transform' ? 'transform' : 'frames';
}

function normalizeTransformEffect(value: any): string | null {
  const text = String(value || '').trim().toLowerCase();
  if (!text) return null;
  return Object.prototype.hasOwnProperty.call(TRANSFORM_OPERATION_CODES, text) ? text : null;
}

function readAnimationMeta(tile: any): AnimationMeta {
  ...
  const transformEffect = normalizeTransformEffect(
...
function buildTileCharMap(analysis: ProjectAnalysis): Map<string, TileCharInfo> {
  const map = new Map<string, TileCharInfo>();
...
function chunkBytesForDb(bytes: number[], perLine = 16): string {
  if (!bytes.length) return '    db #00';
  const lines: string[] = [];
...
function prepareAnimationGroups(analysis: ProjectAnalysis): {
  ...
  const frameGroups: PreparedAnimationGroup[] = [];

  for (const entries of candidatesByGroup.values()) {
    if (entries.length < 2) continue;

    const sorted = [...entries].sort((a, b) => {
      if (a.frameOrder !== b.frameOrder) return a.frameOrder - b.frameOrder;
    ...
    if (compatibleFrames.length < 2) continue;
    ...
    for (const frame of compatibleFrames) {
      const patternBytes = Array.from(generateTilePatternBytes(frame.tile, SCREEN2_MODE) || []);
      if (patternBytes.length !== expectedPatternBytes) continue;

      const rawColorBytes = generateTileColorBytes(frame.tile);
      const defaultColor = 0xF1;
      const colorBytes = rawColorBytes
        ? Array.from(rawColorBytes).slice(0, expectedPatternBytes)
        : new Array(expectedPatternBytes).fill(defaultColor);
...
export function generateAnimatedTilesFile(
  ...
  const defaultGlobalSpeed = clampByte(
    frameGroups[0]?.speed ?? transformGroups[0]?.speed ?? 8,
    8,
    1,
    255
  );
