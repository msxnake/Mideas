...
const formatHexWord = (value: number): string =>
  `#${Math.max(0, Math.min(0xFFFF, Math.floor(value) || 0)).toString(16).toUpperCase().padStart(4, '0')}`;

export interface Msx2PlayerBulletCharSettings {
  charCode: number;
  pattern: number[];
  color: number[];
}

export function getPlayerBulletCharSettings(
  analysis: ProjectAnalysis,
  getProjectileEntity: (analysis: ProjectAnalysis) => any | undefined,
  getTileBytes: (screen: any, entity: any) => { pattern: number[]; color: number[] } | undefined,
  getPrimaryScreen: (analysis: ProjectAnalysis) => any | undefined
): Msx2PlayerBulletCharSettings {
  const screen = getPrimaryScreen(analysis);
  const projectile = getProjectileEntity(analysis);
  const charCode = Math.max(1, Math.min(255, Math.floor(Number(
    projectile?.components?.msx2_char_render?.charCode
  ...
  while (pattern.length < 8) pattern.push(0xFF);
  while (color.length < 8) color.push(0x6E);
  return { charCode, pattern: pattern.slice(0, 8), color: color.slice(0, 8) };
}

export function buildMsx2ReadCurrentScreenNameByteAsm(
  tileScreenLoadLabels: string[],
  useKonamiDataBank: boolean,
  payloadDataBankConstants?: Map<string, string>
): string {
...
export function buildMsx2PlayerBulletCharCoreAsm(
  settings: Msx2PlayerBulletCharSettings,
  tileScreenLoadLabels: string[],
  useKonamiDataBank: boolean,
  payloadDataBankConstants?: Map<string, string>
): string {
  const charCode = settings.charCode;
  const runtimeCharCopy = (label: string, tableBase: number): string =>
...
export function buildPlayerBulletCharSlotUpdateAsm(options: {
  slot: 0 | 1;
  horizontal: boolean;
  speedX: number;
  speedY: number;
...
export function buildPlayerBulletCharSpawnAsm(options: {
  slot: 0 | 1;
  horizontal: boolean;
