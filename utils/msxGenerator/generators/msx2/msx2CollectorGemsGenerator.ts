...
import { Msx2CollectorGemsConfig } from '../../../msx2PlatformPhysics';

/** collector_gems owns no skill RAM: score/latch live in the base runtime. */
export const MSX2_COLLECTOR_GEMS_RAM_BYTES = 0;
 ...
 */
export function buildMsx2CollectorGemsCollectHookAsm(config: Msx2CollectorGemsConfig): string {
 ...
 * do not write.
 */
export function buildMsx2CollectorGemsSfxAsm(config: Msx2CollectorGemsConfig): string {
  if (!config.enabled || !config.collectSound) return '';
  return `msx2_sfx_gem:
