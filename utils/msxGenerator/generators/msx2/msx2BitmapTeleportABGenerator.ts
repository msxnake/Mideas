...
export const MSX2_BITMAP_TELEPORT_AB_RAM_BYTES = 8;
...
export function bitmapTeleportABEnabled(config: Msx2TeleportABConfig | undefined): boolean {
  return Boolean(config?.enabled);
}

/** RAM EQU block (empty when disabled). */
export function buildBitmapTeleportABEquates(
  config: Msx2TeleportABConfig | undefined,
  ramBase: number,
): string {
  if (!bitmapTeleportABEnabled(config)) return '';
...
export function buildBitmapTeleportABInitClearAsm(config: Msx2TeleportABConfig | undefined): string {
  if (!bitmapTeleportABEnabled(config)) return '';
  return `    xor a
...
export function buildBitmapTeleportABGateAsm(config: Msx2TeleportABConfig | undefined): string {
  if (!bitmapTeleportABEnabled(config)) return '';
  return '    call bitmap_try_teleport_ab\n';
}

/** The full teleport runtime ASM. Empty when disabled. */
export function buildBitmapTeleportABRuntimeAsm(
  config: Msx2TeleportABConfig | undefined,
): string {
  if (!config || !bitmapTeleportABEnabled(config)) return '';
