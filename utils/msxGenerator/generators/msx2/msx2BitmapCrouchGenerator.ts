...
import type { Msx2CrouchConfig } from '../../../msx2PlatformPhysics';

// bitmap_crouch_active / bitmap_crouch_slide_t / bitmap_crouch_slide_dir
export const MSX2_BITMAP_CROUCH_RAM_BYTES = 3;
...
export function bitmapCrouchEnabled(config: Msx2CrouchConfig): boolean {
  return !!config.enabled;
}

export function buildBitmapCrouchEquates(config: Msx2CrouchConfig, ramBase: number): string {
  if (!bitmapCrouchEnabled(config)) return '';
...
export function buildBitmapCrouchInitClearAsm(config: Msx2CrouchConfig): string {
  if (!bitmapCrouchEnabled(config)) return '';
  return `    xor a
...
export function buildBitmapCrouchHorizontalHookAsm(config: Msx2CrouchConfig): string {
  if (!bitmapCrouchEnabled(config)) return '';
  return `    push bc
...
export function buildBitmapCrouchRuntimeAsm(
  config: Msx2CrouchConfig,
  stateAnimIds: { crouching?: number; sliding?: number } = {},
): string {
  if (!bitmapCrouchEnabled(config)) return '';
