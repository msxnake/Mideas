...
export interface BitmapWallClimbConfig {
  enabled: boolean;
  /** Vertical climb speed in px/frame (clamped 1..8). */
  climbSpeed: number;
}
...
export function resolveBitmapWallClimbConfig(player: any | undefined): BitmapWallClimbConfig {
  const activeSkills = Array.isArray(player?.activeSkills)
    ? player.activeSkills.map((id: unknown) => String(id || '').trim())
    : [];
  const enabled = activeSkills.includes('climb');
  const params = (player?.skillParameters?.climb || {}) as Record<string, unknown>;
  const climbSpeed = Math.max(1, Math.min(8, Math.floor(Number(params.climbSpeed)) || 2));
  return { enabled, climbSpeed };
}

export function bitmapWallClimbEnabled(config: BitmapWallClimbConfig | undefined): boolean {
  return Boolean(config?.enabled);
}

/** Gravity-hook call, injected after gravity and before the vertical move loop. */
export function buildBitmapWallClimbGravityHookAsm(config: BitmapWallClimbConfig | undefined): string {
  if (!bitmapWallClimbEnabled(config)) return '';
  return '    call bitmap_wall_climb_step\n';
}

/** The wall-climb routine (empty when disabled). */
export function buildBitmapWallClimbRuntimeAsm(config: BitmapWallClimbConfig | undefined): string {
  if (!bitmapWallClimbEnabled(config)) return '';
