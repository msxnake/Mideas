import { Msx2HighJumpConfig } from '../../../msx2PlatformPhysics';
...
export const MSX2_BITMAP_HIGH_JUMP_RAM_BYTES = 1;
...
export function bitmapHighJumpEnabled(c: Msx2HighJumpConfig | undefined): boolean { return Boolean(c?.enabled); }

export function buildBitmapHighJumpEquates(c: Msx2HighJumpConfig | undefined, ramBase: number): string {
  if (!bitmapHighJumpEnabled(c)) return '';
  return `; --- HIGH JUMP skill runtime state (1 byte) ---
bitmap_highjump_hold EQU ${asmWord(ramBase)}
`;
}

export function buildBitmapHighJumpInitClearAsm(c: Msx2HighJumpConfig | undefined): string {
  if (!bitmapHighJumpEnabled(c)) return '';
  return `    xor a
...
export function buildBitmapHighJumpLandClearAsm(c: Msx2HighJumpConfig | undefined): string {
  if (!bitmapHighJumpEnabled(c)) return '';
  return `    xor a
    ld (bitmap_highjump_hold), a
`;
...
export function buildBitmapHighJumpGravityHookAsm(c: Msx2HighJumpConfig | undefined): string {
  if (!bitmapHighJumpEnabled(c)) return '';
...
export function buildBitmapHighJumpRuntimeAsm(c: Msx2HighJumpConfig | undefined): string {
  if (!c || !bitmapHighJumpEnabled(c)) return '';
...
export function buildBitmapHighJumpInputHookAsm(c: Msx2HighJumpConfig | undefined): string {
  if (!bitmapHighJumpEnabled(c)) return '';
