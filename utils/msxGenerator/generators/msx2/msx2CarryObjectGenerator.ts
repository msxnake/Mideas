...
import { Msx2CarryObjectConfig } from '../../../msx2PlatformPhysics';
...
export const MSX2_CARRY_DEFAULT_COLOR = 14;

export interface Msx2CarryableRuntimeSlot {
  /** Pixel X (tile * 16). */
  x: number;
  /** Pixel Y (tile * 16). */
  y: number;
  /** Referenced msx2sprite asset id, or undefined for the default pattern. */
...
export function isMsx2CarryableEntity(entity: any): boolean {
  if (!entity) return false;
  const component = entity.components?.msx2_carryable;
  if (component) {
    return component.enabled !== false && component.enabled !== 'false';
  }
  return entity.params?.carryable === true
    || entity.params?.carryable === 'true'
    || entity.kind === 'carryable';
}

function getCarryableRenderSpriteId(entity: any): string | undefined {
  // Mirrors getEntityRenderSpriteId (msx2Screen4Generator) so the catalog
  // Render component (msx2_hardware_sprite) binds the carryable sprite,
  // plus param-level fallbacks for hand-written JSON entities.
  const id = entity?.components?.msx2_hardware_sprite?.msx2SpriteAssetId
    ?? entity?.components?.msx2_render?.msx2SpriteAssetId
    ?? entity?.components?.msx2_render?.spriteAssetId
    ?? entity?.components?.msx2_render?.sprite
...
export function usesMsx2CarryObject(
  config: Msx2CarryObjectConfig,
...
export function resolveMsx2CarryObjectRamBase(options: Msx2SkillRamOptions): number {
  return resolveMsx2SkillExtensionRamBase(options);
}

export function buildMsx2CarryObjectEquates(ramBase: number): string {
  const slots = MSX2_MAX_CARRYABLES_PER_SCREEN;
...
export function buildMsx2CarryObjectInitClearAsm(): string {
  // init_hardware_sprites runs AFTER msx2_reset_enemy_runtime_for_current_screen
  // at boot/restart, so a plain clear here would wipe the slots the reset just
  // populated (boot bug found in OpenMSX smoke 2026-06-12). Re-running the
  // reset is idempotent: it re-pulls the slots from ROM for the current screen
  // and clears every carry scalar.
  return `    call msx2_reset_carry_runtime_for_current_screen
`;
}
 ...
 */
export function buildMsx2CarryObjectAttrWritesAsm(attrBase: number, patternBase: number): string {
  return Array.from({ length: MSX2_MAX_CARRYABLES_PER_SCREEN }, (_unused, slot) => {
...
function buildCarrySlotColorUploadAsm(slot: number, satSlotBase: number): string {
 ...
 */
function buildCarrySharedSlotRoutinesAsm(throwPower: number): string {
  const power = formatHexByte(throwPower);
  const stride = MSX2_MAX_CARRYABLES_PER_SCREEN;
  const yOff = stride;
...
export function buildMsx2CarryObjectRuntimeAsm(
  config: Msx2CarryObjectConfig,
  options: { satSlotBase: number }
): string {
...
${Array.from({ length: MSX2_MAX_CARRYABLES_PER_SCREEN }, (_unused, slot) =>
    buildCarrySlotColorUploadAsm(slot, options.satSlotBase)).join('')}    ret
...
.carry_try_pickup:
    ...
    or a
    jp nz, .carry_update_phases
${Array.from({ length: MSX2_MAX_CARRYABLES_PER_SCREEN }, (_unused, slot) => `    ld ix, msx2_carry_runtime_x + ${slot}
 ...
 */
export function buildMsx2CarryObjectDataTablesAsm(tables: {
  countByScreen: number[];
  xBytes: number[];
  yBytes: number[];
  spriteIdxBytes: number[];
  colorRows: number[];
}): string {
