import { Msx2SlashConfig } from '../../../msx2PlatformPhysics';
...
export const MSX2_BITMAP_SLASH_RAM_BYTES = 3;
...
export function bitmapSlashEnabled(c: Msx2SlashConfig | undefined): boolean { return Boolean(c?.enabled); }

export function buildBitmapSlashEquates(c: Msx2SlashConfig | undefined, ramBase: number): string {
  if (!bitmapSlashEnabled(c)) return '';
...
export function buildBitmapSlashInitClearAsm(c: Msx2SlashConfig | undefined): string {
  if (!bitmapSlashEnabled(c)) return '';
  return `    xor a
...
export function buildBitmapSlashGateAsm(c: Msx2SlashConfig | undefined): string {
  if (!bitmapSlashEnabled(c)) return '';
  return '    call bitmap_try_slash\n';
}

export function buildBitmapSlashRuntimeAsm(c: Msx2SlashConfig | undefined): string {
  if (!c || !bitmapSlashEnabled(c)) return '';
