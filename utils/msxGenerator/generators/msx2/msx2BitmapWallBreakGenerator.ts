import { Msx2WallBreakConfig } from '../../../msx2PlatformPhysics';
...
export const MSX2_BITMAP_WALL_BREAK_RAM_BYTES = 2;
...
export function bitmapWallBreakEnabled(c: Msx2WallBreakConfig | undefined): boolean { return Boolean(c?.enabled); }

export function buildBitmapWallBreakEquates(c: Msx2WallBreakConfig | undefined, ramBase: number): string {
  if (!bitmapWallBreakEnabled(c)) return '';
...
export function buildBitmapWallBreakInitClearAsm(c: Msx2WallBreakConfig | undefined): string {
  if (!bitmapWallBreakEnabled(c)) return '';
  return `    xor a
...
export function buildBitmapWallBreakGateAsm(c: Msx2WallBreakConfig | undefined): string {
  if (!bitmapWallBreakEnabled(c)) return '';
  return '    call bitmap_try_wall_break\n';
}

export function buildBitmapWallBreakRuntimeAsm(c: Msx2WallBreakConfig | undefined): string {
  if (!c || !bitmapWallBreakEnabled(c)) return '';
