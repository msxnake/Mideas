import { Msx2SpinAttackConfig } from '../../../msx2PlatformPhysics';
...
export const MSX2_BITMAP_SPIN_ATTACK_RAM_BYTES = 3;
...
export function bitmapSpinAttackEnabled(c: Msx2SpinAttackConfig | undefined): boolean { return Boolean(c?.enabled); }

export function buildBitmapSpinAttackEquates(c: Msx2SpinAttackConfig | undefined, ramBase: number): string {
  if (!bitmapSpinAttackEnabled(c)) return '';
...
export function buildBitmapSpinAttackInitClearAsm(c: Msx2SpinAttackConfig | undefined): string {
  if (!bitmapSpinAttackEnabled(c)) return '';
  return `    xor a
...
export function buildBitmapSpinAttackGateAsm(c: Msx2SpinAttackConfig | undefined): string {
  if (!bitmapSpinAttackEnabled(c)) return '';
  return '    call bitmap_try_spin_attack\n';
}

export function buildBitmapSpinAttackRuntimeAsm(c: Msx2SpinAttackConfig | undefined): string {
  if (!c || !bitmapSpinAttackEnabled(c)) return '';
