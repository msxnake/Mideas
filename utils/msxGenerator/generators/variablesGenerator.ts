...
/** Allocate the exact RAM ABI required by PT3-ROM-alltables-glass.asm at a
 * caller-selected base. Both the modular and bitmap ROM routes use this. */
export function buildExternalPT3WorkspaceAsm(pt3Base: number): { asm: string; nextFree: number } {
  const hex = (offset: number) => (pt3Base + offset).toString(16).toUpperCase().padStart(4, '0');
  return {
    asm: `
; ==================================================================
...
`;
  ...
  currentAddress++;
  code += `music_pt3_data_bank_0 EQU #${currentAddress.toString(16).toUpperCase().padStart(4, '0')}   ; Source-faithful PT3 mapper bank for #8000 window\n`;
  currentAddress++;
  code += `music_pt3_data_bank_1 EQU #${currentAddress.toString(16).toUpperCase().padStart(4, '0')}   ; Source-faithful PT3 mapper bank for #A000 window\n`;
  currentAddress++;
  ...
  const pt3ScalarDefs = [
    ...
    ['music_pt3_used_noise', 'Nonzero when a PT3 voice owns the global noise register this frame'],
    ['music_pt3_used_env', 'Nonzero when a PT3 voice owns the global hardware envelope this frame'],
  ...
  const hasPT3 = Array.isArray(analysis.tracks) &&
    analysis.tracks.some((t: any) => t?.playbackBackend === 'external-pt3');
  if (hasPT3) {
    const workspace = buildExternalPT3WorkspaceAsm(currentAddress);
    code += workspace.asm;
    currentAddress = workspace.nextFree;
  }
