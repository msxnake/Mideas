...
import { normalizeGlobalVariableName } from '../../globalVariablesUtils';
...
interface HudDynamicFieldBinding {
    index: number;
    asmName: string;
    digits: number;
    fieldOffset: number;
...
function findGlobalVariable(analysis: ProjectAnalysis, rawName: string): any | null {
    const globals = Array.isArray(analysis.globalVariables) ? analysis.globalVariables : [];
    const normalizedName = normalizeGlobalVariableName(rawName || '').trim().toLowerCase();
    if (!normalizedName) return null;

    return globals.find(variable => {
        const variableName = normalizeGlobalVariableName(String((variable as any)?.name || '')).trim().toLowerCase();
        const asmName = String((variable as any)?.asmName || '').trim().toLowerCase();
        return variableName === normalizedName || asmName === normalizedName;
    }) || null;
}

function resolveHudDynamicFieldBinding(
    element: HUDElement,
    index: number,
    analysis: ProjectAnalysis
): HudDynamicFieldBinding | null {
    if (element.type === HUDElementType.Score || element.type === HUDElementType.Lives) {
        return null;
    }

    const rawText = String(element.text || element.name || '');
    const details = (element as any).details || {};
...
function getHudDynamicFieldBindings(hudElements: HUDElement[], analysis: ProjectAnalysis): HudDynamicFieldBinding[] {
    return hudElements
        .map((element, index) => resolveHudDynamicFieldBinding(element, index, analysis))
        .filter((binding): binding is HudDynamicFieldBinding => binding !== null);
}
 ...
 * Generate HUD data structures
 */
function generateHudDataStructures(hudElements: HUDElement[], analysis: ProjectAnalysis): string {
    let asm = `; ------------------------------------------------------------------
; HUD DATA STRUCTURES
; ------------------------------------------------------------------
...
`;
    ...
    const dynamicFieldBindingMap = new Map<number, HudDynamicFieldBinding>(
        dynamicFieldBindings.map(binding => [binding.index, binding])
    );
 ...
 */
function generateHudHelperFunctions(hudElements: HUDElement[], analysis: ProjectAnalysis): string {
    const scoreIndex = hudElements.findIndex(el => el.type === HUDElementType.Score);
    const livesIndex = hudElements.findIndex(el => el.type === HUDElementType.Lives);
    ...
    const getNumericFieldInfo = (text: string, fallbackDigits: number) => {
        const safeText = text || '';
    ...
    const getFieldVramAddress = (element: HUDElement | null, fieldOffset: number): number | null => {
    ...
    const buildDecimalWriteCode = (digits: number, scoreMode = false): string => {
        ...
        const divisors = availableDivisors.slice(
            availableDivisors.length - Math.max(0, supportedDigits - 1)
        );

        const leadingZeroCode = Array.from(
            { length: leadingZeroCount },
            (_, index) => `    ; Leading digit ${index}: forced zero (${scoreMode ? 'Score' : '16-bit value'} max 65535)
    ld a, '0'
    push hl
    ld h, d
    ld l, e
