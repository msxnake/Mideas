...
import {
  buildSerializedTrackerEffectTable,
  flattenSerializedTrackerEffectTable,
} from './trackerEffectGenerator';
...
function buildTrackData(
  ...
  const effectTableLabel = useRelativePointers ? '.effect_table' : `${labelBase}_effect_table`;
  ...
  const effectTable = buildSerializedTrackerEffectTable(
    patterns.flatMap((pattern, patternIndex) => {
      ...
      return Array.from({ length: rowCount }, (_, rowIndex) => {
        ...
        return TRACK_CHANNELS.map(channelId => ({
          cell: getCellValue(row, channelId),
          context: `${song.name}/pattern${patternIndex}/row${rowIndex}/${channelId}`,
        }));
      }).flat();
    }),
    warnings,
  ...
  lines.push(`    DW ${useRelativePointers ? `${effectTableLabel} - ${dataLabel}` : effectTableLabel}`);
  lines.push('');
  ...
  lines.push('');
  lines.push(buildDbLines(effectTableLabel, flattenSerializedTrackerEffectTable(effectTable)));
  lines.push('');
  ...
  patterns.forEach((pattern, patternIndex) => {
    ...
    for (let rowIndex = 0; rowIndex < rowCount; rowIndex++) {
      ...
      TRACK_CHANNELS.forEach((channelId) => {
        ...
        rowBytes.push(effectTable.indexForCell(cell));
      });
...
/**
 * Native PT3 step runtime, shared by the PSG tracker block and by the SCC/bitmap
 * MegaROM music block.
 *
 * `trackerFx` gates the Vortex native-effect hooks (tone slide delta, vibrato
 * gate, envelope slide offset). Those hooks need both the `music_fx_*` RAM map
 * and `music_fx_apply_period_delta`, which only exist inside
 * `buildTrackerMusicBlock`. The SCC/bitmap path builds its own music RAM map and
 * never decodes Vortex commands, so it must ask for `trackerFx: false` — with
 * the hooks left in, glass.jar fails on the undefined labels.
 */
export function buildNativePT3SampleRuntimeAsm(options: { trackerFx?: boolean } = {}): string {
  const trackerFx = options.trackerFx !== false;
  // Each snippet collapses to nothing when the FX engine is not part of the build.
  const fxPeriodDelta = trackerFx ? '    call music_fx_apply_period_delta\n' : '';
  const fxVolumeGate = trackerFx
    ? `    ld hl, music_fx_gate_enabled_base
    call music_load_channel_byte
    or a
    jr nz, .pt3_native_gate_ready
    ld b, 0
.pt3_native_gate_ready:
`
    : '';
  const fxMixerGate = trackerFx
    ? `    ld hl, music_fx_gate_enabled_base
    call music_load_channel_byte
    or a
    jr nz, .pt3_native_mixer_gate_ready
    ld d, 0
    ld e, 0
.pt3_native_mixer_gate_ready:
`
    : '';
  const fxEnvelopeOffset = trackerFx
    ? `    ld a, (music_fx_env_offset_lo)
    ld e, a
    ld a, (music_fx_env_offset_hi)
    ld d, a
    add hl, de
`
    : '';
...
.pt3_tone_acc_ready:
    ...
    ld a, (music_pt3_channel_work)
    ld c, a
${fxPeriodDelta}    ld a, (music_pt3_channel_work)
    call psg_set_tone
...
.pt3_global_done:
    ...
    ld a, (music_pt3_channel_work)
    ld c, a
${fxVolumeGate}    ld a, (music_pt3_step_flags)
    bit 3, a
...
.pt3_noise_gate_ready:
    ld a, (music_pt3_channel_work)
    ld c, a
${fxMixerGate}    call music_pt3_apply_mixer_bits
    ret
...
.pt3_env_sum_ready:
    add hl, de
${fxEnvelopeOffset}    ld a, PSG_ENV_LO
    ld e, l
    call psg_write
    ld a, PSG_ENV_HI
    ld e, h
...
function buildTrackerMusicBlock(tracks: TrackerSongData[], bankedTrackData: boolean = false): string {
  ...
  const trackTableLines: string[] = [
    ...
    'MUSIC_TRACK_EFFECT_TABLE    EQU 16',
    'MUSIC_TRACK_ROW_SIZE        EQU 15',
    ...
    '    call music_reset_native_fx_state',
    ...
    '',
    '; ------------------------------------------------------------------',
    '; music_reset_native_fx_state',
    '; Reset the Vortex-compatible native pattern FX engine.',
    '; Input: None',
    '; Output: Slides/gates cleared; every channel gate enabled',
    '; Destroys: AF',
    '; Preserves: BC, DE, HL, IX, IY',
    '; ------------------------------------------------------------------',
    'music_reset_native_fx_state:',
    ...
    '    push hl',
    '    xor a',
    '    ld hl, music_fx_speed_override',
    '    ld (hl), a',
    '    ld de, music_fx_speed_override + 1',
    '    ld bc, music_fx_ch_c_gate_off - music_fx_speed_override',
    '    ldir',
    '    ld a, 1',
    '    ld (music_fx_ch_a_gate_enabled), a',
    '    ld (music_fx_ch_b_gate_enabled), a',
    '    ld (music_fx_ch_c_gate_enabled), a',
    '    pop hl',
    '    pop de',
    '    pop bc',
    '    ret',
    ...
    '',
    '; ------------------------------------------------------------------',
    '; music_fx_load_channel_pair / music_fx_store_channel_pair',
    '; Access a little-endian per-channel word whose high-byte array follows',
    '; the low-byte array (three bytes per array).',
    '; Input: C = channel, HL = low-byte array; store also takes DE = value',
    '; Output: load returns DE; store updates RAM',
    '; Destroys: AF, HL (load), AF, HL (store)',
    '; Preserves: BC, IX, IY',
    '; ------------------------------------------------------------------',
    'music_fx_load_channel_pair:',
    '    call music_load_channel_byte',
    '    push af',
    '    ld de, 3',
    '    add hl, de',
    ...
    '',
    '; ------------------------------------------------------------------',
    '; music_fx_get_channel_note_period',
    '; Input: C = channel index',
    '; Output: DE = base note period, or 0 when the channel is silent',
    '; Destroys: AF, DE, HL',
    '; Preserves: BC, IX, IY',
    '; ------------------------------------------------------------------',
    'music_fx_get_channel_note_period:',
    ...
    '',
    '; ------------------------------------------------------------------',
    '; music_fx_initialize_note_state',
    '; Reset one channel FX state on note-on/cut. Portamento preserves the',
    '; previous current period and targets the newly decoded note.',
    '; Input: scratch channel/new-note/command already populated',
    '; Output: channel FX state initialized',
    '; Destroys: AF, BC, DE, HL',
    '; Preserves: IX, IY',
    '; ------------------------------------------------------------------',
    'music_fx_initialize_note_state:',
    '    ld a, (music_fx_new_note_work)',
    '    or a',
    '    ret z',
    '    ld b, a',
    '    ld a, (music_fx_channel_work)',
    '    ld c, a',
    '    xor a',
    '    ld hl, music_fx_tone_mode_base',
    '    call music_store_channel_byte',
    '    ld hl, music_fx_tone_delay_base',
    '    call music_store_channel_byte',
    '    ld hl, music_fx_tone_counter_base',
    '    call music_store_channel_byte',
    '    ld hl, music_fx_tone_step_lo_base',
    '    call music_store_channel_byte',
    '    ld hl, music_fx_tone_step_hi_base',
    '    call music_store_channel_byte',
    '    ld hl, music_fx_gate_counter_base',
    '    call music_store_channel_byte',
    '    ld hl, music_fx_gate_on_base',
    '    call music_store_channel_byte',
    '    ld hl, music_fx_gate_off_base',
    '    call music_store_channel_byte',
    '    ld a, 1',
    '    ld hl, music_fx_gate_enabled_base',
    '    call music_store_channel_byte',
    ...
    '    cp 2',
    '    jr z, .fx_note_cut',
    '    call music_fx_get_channel_note_period',
    '    ld hl, music_fx_target_lo_base',
    '    call music_fx_store_channel_pair',
    '    ld a, (music_fx_command_work)',
    '    cp 2',
    '    jr nz, .fx_note_store_current',
    '    ld hl, music_fx_period_lo_base',
    '    call music_fx_load_channel_pair',
    '    ld a, d',
    '    or e',
    '    ret nz',
    '    call music_fx_get_channel_note_period',
    '.fx_note_store_current:',
    '    ld hl, music_fx_period_lo_base',
    '    call music_fx_store_channel_pair',
    '    ret',
    '.fx_note_cut:',
    ...
    '',
    '; ------------------------------------------------------------------',
    '; music_apply_native_effect',
    '; Decode one compact Vortex FX record referenced by the row cell.',
    '; Input: A = effect-table index, C = channel, D = 0 hold/1 note/2 cut',
    '; Output: channel/global FX state updated',
    '; Destroys: AF, BC, DE, HL',
    '; Preserves: IX, IY; caller preserves its row pointer',
    '; ------------------------------------------------------------------',
    'music_apply_native_effect:',
    '    ld (music_fx_command_work), a',
    '    ld a, c',
    '    ld (music_fx_channel_work), a',
    '    ld a, d',
    '    ld (music_fx_new_note_work), a',
    '    ld a, MUSIC_TRACK_EFFECT_TABLE',
    '    call music_read_track_word',
    '    ld a, (music_fx_command_work)',
    '    ld e, a',
    '    ld d, 0',
    ...
    '    inc hl',
    '    ld a, l',
    '    ld (music_fx_param_ptr_l), a',
    '    ld a, h',
    '    ld (music_fx_param_ptr_h), a',
    '    call music_fx_initialize_note_state',
    '    ld a, (music_fx_new_note_work)',
    '    cp 2',
    '    jr nz, .fx_dispatch',
    '    ld a, (music_fx_command_work)',
    ...
    '    ld a, (music_fx_command_work)',
    '    cp 1',
    '    jp z, music_fx_apply_gliss',
    '    cp 2',
    '    jp z, music_fx_apply_porta',
    '    cp 3',
    '    jp z, music_fx_apply_sample_pos',
    '    cp 4',
    '    jp z, music_fx_apply_ornament_pos',
    '    cp 5',
    '    jp z, music_fx_apply_gate',
    '    cp 8',
    '    jp z, music_fx_apply_envelope_slide',
    '    cp 9',
    '    jp z, music_fx_apply_speed',
    '    ret',
    '',
    'music_fx_load_param_ptr:',
    '    ld a, (music_fx_param_ptr_l)',
    '    ld l, a',
    '    ld a, (music_fx_param_ptr_h)',
    '    ld h, a',
    '    ret',
    '',
    'music_fx_apply_gliss:',
    '    call music_fx_load_param_ptr',
    '    ld a, (hl)',
    '    or a',
    '    jr nz, .fx_gliss_delay_ready',
    '    inc a',
    '.fx_gliss_delay_ready:',
    '    ld b, a',
    '    ld a, (music_fx_channel_work)',
    '    ld c, a',
    '    ld a, b',
    ...
    '    ret',
    '',
    'music_fx_apply_porta:',
    '    call music_fx_load_param_ptr',
    '    ld a, (hl)',
    '    or a',
    '    jr nz, .fx_porta_delay_ready',
    '    inc a',
    '.fx_porta_delay_ready:',
    '    ld b, a',
    '    ld a, (music_fx_channel_work)',
    '    ld c, a',
    '    ld a, b',
    ...
    '    sbc a, d',
    '    ld d, a',
    '.fx_porta_step_positive:',
    '    ld a, d',
    '    or e',
    '    jr nz, .fx_porta_step_ready',
    '    inc e',
    '.fx_porta_step_ready:',
    '    ld hl, music_fx_tone_step_lo_base',
    '    call music_fx_store_channel_pair',
    ...
    '    ret',
    '',
    'music_fx_apply_envelope_slide:',
    '    call music_fx_load_param_ptr',
    '    ld a, (hl)',
    '    or a',
    '    jr nz, .fx_env_delay_ready',
    '    inc a',
    '.fx_env_delay_ready:',
    '    ld (music_fx_env_delay), a',
    '    ld (music_fx_env_counter), a',
    '    inc hl',
    '    ld a, (hl)',
    '    ld (music_fx_env_step_lo), a',
    '    inc hl',
    '    ld a, (hl)',
    '    ld (music_fx_env_step_hi), a',
    '    ret',
    '',
    'music_fx_apply_speed:',
    '    call music_fx_load_param_ptr',
    '    ld a, (hl)',
    '    or a',
    '    jr nz, .fx_speed_nonzero',
    '    inc a',
    '.fx_speed_nonzero:',
    '    cp 32',
    '    jr c, .fx_speed_ready',
    '    ld a, 31',
    '.fx_speed_ready:',
    '    ld (music_fx_speed_override), a',
    '    ret',
    '',
    '; ------------------------------------------------------------------',
    '; music_fx_step_all',
    '; Advance native Vortex slides and gates exactly once per 50 Hz frame.',
    '; Input: None',
    '; Output: Global envelope offset and A/B/C channel FX states advanced',
    '; Destroys: AF, BC, DE, HL',
    '; Preserves: IX, IY',
    '; ------------------------------------------------------------------',
    'music_fx_step_all:',
    '    ld a, (music_fx_env_delay)',
    '    or a',
    '    jr z, .fx_step_channels',
    '    ld a, (music_fx_env_counter)',
    '    or a',
    '    jr z, .fx_env_tick',
    '    dec a',
    '    ld (music_fx_env_counter), a',
    '    jr nz, .fx_step_channels',
    '.fx_env_tick:',
    '    ld a, (music_fx_env_delay)',
    '    ld (music_fx_env_counter), a',
    '    ld a, (music_fx_env_offset_lo)',
    ...
    '    ld a, (music_fx_env_offset_hi)',
    '    ld h, a',
    '    ld a, (music_fx_env_step_lo)',
    '    ld e, a',
    '    ld a, (music_fx_env_step_hi)',
    '    ld d, a',
    '    add hl, de',
    '    ld a, l',
    '    ld (music_fx_env_offset_lo), a',
    '    ld a, h',
    '    ld (music_fx_env_offset_hi), a',
    '.fx_step_channels:',
    '    ld c, 0',
    '    call music_fx_step_channel',
    '    ld c, 1',
    '    call music_fx_step_channel',
    '    ld c, 2',
    '    jp music_fx_step_channel',
    '',
    '; ------------------------------------------------------------------',
    '; music_fx_step_channel',
    '; Input: C = channel index',
    '; Output: Tone slide and vibrato gate advanced for that channel',
    '; Destroys: AF, B, DE, HL',
    '; Preserves: C, IX, IY',
    '; ------------------------------------------------------------------',
    'music_fx_step_channel:',
    ...
    '    call music_load_channel_byte',
    '    or a',
    '    jr nz, .fx_tone_reload_ready',
    '    inc a',
    '.fx_tone_reload_ready:',
    '    ld hl, music_fx_tone_counter_base',
    '    call music_store_channel_byte',
    '    ld a, b',
    '    cp 1',
    ...
    '    ld a, h',
    '    cp #10',
    '    jr c, .fx_gliss_nonzero',
    '    ld hl, #0FFF',
    '    jr .fx_gliss_store',
    '.fx_gliss_nonzero:',
    '    ld a, h',
    '    or l',
    '    jr nz, .fx_gliss_store',
    '    inc hl',
    '.fx_gliss_store:',
    '    ex de, hl',
    '    ld hl, music_fx_period_lo_base',
    ...
    '    ld a, h',
    '    or l',
    '    jr z, .fx_porta_reached',
    '    jr c, .fx_porta_current_above',
    '    jr .fx_porta_current_below',
    '.fx_porta_current_below:',
    '    ld hl, music_fx_period_lo_base',
    '    call music_fx_load_channel_pair',
    '    push de',
    '    ld hl, music_fx_tone_step_lo_base',
    ...
    '    or a',
    '    sbc hl, de',
    '    jr nc, .fx_porta_use_target',
    '    add hl, de',
    '    ex de, hl',
    '    jr .fx_porta_store_period',
    '.fx_porta_current_above:',
    '    ld hl, music_fx_period_lo_base',
    '    call music_fx_load_channel_pair',
    '    push de',
    '    ld hl, music_fx_tone_step_lo_base',
    ...
    '',
    '; ------------------------------------------------------------------',
    '; music_fx_apply_period_delta',
    '; Add the Vortex raw-period slide delta to an already ornamented period.',
    '; Input: C = channel, HL = resolved tone period',
    '; Output: HL = clamped 1..#0FFF period',
    '; Destroys: AF, DE',
    '; Preserves: BC, IX, IY',
    '; ------------------------------------------------------------------',
    'music_fx_apply_period_delta:',
    ...
    '    ld a, h',
    '    bit 7, a',
    '    jr z, .fx_period_check_high',
    '    ld hl, 1',
    '    jr .fx_period_done',
    '.fx_period_check_high:',
    '    cp #10',
    '    jr c, .fx_period_check_zero',
    '    ld hl, #0FFF',
    '    jr .fx_period_done',
    '.fx_period_check_zero:',
    '    ld a, h',
    '    or l',
    '    jr nz, .fx_period_done',
    '    inc hl',
    '    jr .fx_period_done',
    '.fx_period_no_state:',
    '    pop hl',
    ...
    '    push hl',
    '    push de',
    '    call music_apply_native_effect',
    '    pop de',
    '    pop hl',
    ...
    '    call music_read_track_byte',
    '    ld b, a',
    '    ld a, (music_fx_speed_override)',
    '    or a',
    '    jr nz, .row_speed_ready',
    '    ld a, b',
    '.row_speed_ready:',
    '    ld (music_row_frames), a',
    '    ld (music_row_countdown), a',
    '    call music_fx_step_all',
    '    call music_update_channel_effects',
    '    ret',
    '',
    '; ------------------------------------------------------------------',
    ...
    '    ld h, d',
    '    ld l, e',
    '    call music_fx_apply_period_delta',
    '    ld a, c',
    '    push bc',
    '    call psg_set_tone',
    '    pop bc',
