...
${hasMenus ? `    call update_menu_system
...
execute_gameflow_node:
    ...
    ; We are generating code labels in gameFlowGenerator that jump to execute_gameflow_node
    ; But actually, the state machine generated by generateGameFlowStateMachine
    ; creates LABELS (gameflow_node_ID) that do the work and then jump to the next one.
    ; So execute_gameflow_node is really a jump trampoline or dispatcher.

    ; If HL contains the address of the next node ROUTINE (label), we just jump to it.
    jp (hl)
...
${hasMenus ? `    ; Menu system detected - render menu
    ; Menu rendering is handled by show_menu routines
    ; Here we could update animations if needed
