; ==================================================================
; FAKEPLAYER_DIALOG_MORE_TEXT_UPPER - UNIFIED FILE
...
; BANK 02 @#0C88 : sprites.asm (4984 bytes)
; BANK 03 @#0000 : sprites.asm (3036 bytes)
; BANK 03 @#0BDC : font.asm (3456 bytes)
; BANK 03 @#195C : hud.asm (1700 bytes)
; BANK 04 @#0000 : hud.asm (2247 bytes)
; BANK 04 @#08C7 : menus.asm (168 bytes)
; BANK 04 @#096F : sound.asm part 1/2 (5777 bytes)
; BANK 05 @#0000 : sound.asm part 2/2 (3771 bytes)
; BANK 05 @#0EBB : scroll.asm (2353 bytes)
; BANK 05 @#17EC : animtiles.asm (2068 bytes)
; BANK 06 @#0000 : animtiles.asm (3325 bytes)
; BANK 06 @#0CFD : statemachine.asm part 1/3 (4867 bytes)
; BANK 07 @#0000 : statemachine.asm part 2/3 (8192 bytes)
; BANK 08 @#0000 : statemachine.asm part 3/3 (6876 bytes)
; BANK 08 @#1ADC : gameflow.asm (1316 bytes)
; BANK 09 @#0000 : gameflow.asm (5818 bytes)
...
sprite_asset_base_pattern_slot_runtime EQU #E38E   ; Runtime base 16x16 slot per sprite asset (11 bytes)
sprite_placeholder_base_pattern_num EQU #E399   ; Runtime placeholder pattern number (base slot * 4)
current_sprite_pattern_pack_id EQU #E39A   ; Active runtime sprite pattern pack id (#FF=none loaded)
sprite_attributes   EQU #E39B   ; Interleaved sprite attributes (32 * 4 bytes)

; ==================================================================
; SCREEN SYSTEM VARIABLES (1 screens detected)
; ==================================================================
current_screen_id   EQU #E41B   ; Currently displayed screen ID
current_screen_engine EQU #E41C   ; Runtime engine: 0=Player, 1=FakePlayer
screen_dirty_flag   EQU #E41D   ; Screen needs redraw flag
screen_transition_cooldown EQU #E41E   ; Cooldown frames after screen transition
current_world_id    EQU #E41F   ; Current world ID (for multi-world support)
current_screen_index EQU #E420   ; Current screen index within world
current_screen_anim_group_count EQU #E421   ; Animated tile groups visible in current screen
current_screen_entity_count EQU #E422   ; Entity instances assigned to current screen
current_screen_sprite_pattern_slots EQU #E423   ; Sprite pattern slots needed by current screen
current_screen_summary_flags EQU #E424   ; Runtime screen summary flags (music/hud/effects/anim)

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #E425   ; Player X position (16-bit)
player_y            EQU #E427   ; Player Y position (16-bit)
player_runtime_enabled EQU #E429   ; 1=player fast runtime bound to hero entity
player_entity_index EQU #E42A   ; Entity index used by player fast runtime (#FF=none)
player_vx_runtime   EQU #E42B   ; Cached player X velocity (signed 8-bit)
player_vy_runtime   EQU #E42C   ; Cached player Y velocity (signed 8-bit)
player_health       EQU #E42D   ; Player health points
player_score        EQU #E42E   ; Player score (16-bit)
gem_count           EQU #E430   ; Collectible tile counter (8-bit)
last_interaction_char EQU #E431   ; Char code of last interacted tile (for SM VARIABLE_COMPARE)
last_gem_char       EQU last_interaction_char   ; Backwards-compatible alias for collectible SM checks
last_interaction_pending EQU #E432   ; 1 when a new tile interaction is pending for State Machine logic
last_interaction_type EQU #E433   ; Interaction type id of last interacted tile
last_interaction_value EQU #E434   ; Interaction value byte of last interacted tile
last_interaction_target EQU #E435   ; Interaction target id of last interacted tile
last_interaction_x  EQU #E436   ; Tile X coordinate of last interaction
last_interaction_y  EQU #E437   ; Tile Y coordinate of last interaction
last_interaction_entity EQU #E438   ; Entity index that triggered the last interaction

; Persistent collectibles list (survives screen re-entry)
MAX_COLLECTIBLES     EQU 64              ; Max persistent collectible records
collected_count      EQU #E439   ; Number of collected tiles recorded (8-bit)
collected_world      EQU #E43A   ; World IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_screen     EQU #E47A   ; Screen IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_idx_l      EQU #E4BA   ; Tile name-table index low byte (MAX_COLLECTIBLES bytes)
collected_idx_h      EQU #E4FA   ; Tile name-table index high byte (MAX_COLLECTIBLES bytes)

; Timed bonus tile respawn slots (bonus gem regeneration)
MAX_BONUS_RESPAWNS   EQU 16              ; Max timed bonus tiles waiting to respawn
bonus_respawn_world  EQU #E53A   ; World IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_screen EQU #E54A   ; Screen IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_l  EQU #E55A   ; Tile index low byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_h  EQU #E56A   ; Tile index high byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_secs   EQU #E57A   ; Remaining seconds per timed respawn slot (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_frames EQU #E58A   ; Frame countdown (60..1) per timed respawn slot (MAX_BONUS_RESPAWNS bytes)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #E59A   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #E59B   ; Temporary 16-bit storage
temp_word_2         EQU #E59D   ; Temporary 16-bit storage
temp_byte_1         EQU #E59F   ; Temporary 8-bit storage
temp_byte_2         EQU #E5A0   ; Temporary 8-bit storage
temp_byte_3         EQU #E5A1   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #E5C1   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #E5E1   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #E601   ; Temporary 8-bit storage (32 bytes)

; ==================================================================
; SOUND SYSTEM VARIABLES
; ==================================================================
sfx_active          EQU #E621   ; 0=no SFX active, 1=playing
sfx_timer           EQU #E622   ; Frames remaining for current SFX
sfx_fadeout         EQU #E623   ; Reserved fadeout flag/state
temp_byte_7         EQU #E624   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #E644   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #E664   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #E684   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #E6A4   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #E6C4   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #E6E4   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #E704   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #E724   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #E744   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #E764   ; Temporary 8-bit storage (32 bytes)
temp_byte_18        EQU #E784   ; Temporary 8-bit storage (32 bytes)
temp_byte_19        EQU #E7A4   ; Temporary 8-bit storage (32 bytes)
temp_byte_20        EQU #E7C4   ; Temporary 8-bit storage (32 bytes)
temp_byte_21        EQU #E7E4   ; Temporary 8-bit storage (32 bytes)
temp_byte_22        EQU #E804   ; Temporary 8-bit storage (32 bytes)
temp_byte_23        EQU #E824   ; Temporary 8-bit storage (32 bytes)
temp_byte_24        EQU #E844   ; Temporary 8-bit storage (32 bytes)
temp_byte_25        EQU #E864   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #E884   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #E8C4   ; Temporary 16-bit storage (64 bytes)
temp_byte_26        EQU #E904   ; Temporary 8-bit storage (32 bytes)
temp_byte_27        EQU #E924   ; Temporary 8-bit storage (32 bytes)
temp_byte_28        EQU #E944   ; Temporary 8-bit storage (32 bytes)
tileDead_dbg        EQU #E964   ; Debug byte: current hero deadly contact
tileDead_latched_dbg EQU #E965   ; Debug byte: latched hero deadly contact
tileDead_x_dbg      EQU #E966   ; Debug byte: last sampled deadly tile X
tileDead_y_dbg      EQU #E967   ; Debug byte: last sampled deadly tile Y
tileDead_value_dbg  EQU #E968   ; Debug byte: last raw deadly behavior value

; Wall collision temporary variables
wall_temp_x         EQU #E969   ; Cached entity X for wall checks
wall_temp_y         EQU #E96A   ; Cached entity Y for wall checks
wall_hit_left       EQU #E96B   ; Hitbox left edge cache
wall_hit_top        EQU #E96C   ; Hitbox top edge cache
wall_hit_right      EQU #E96D   ; Hitbox right edge cache
wall_hit_bottom     EQU #E96E   ; Hitbox bottom edge cache
wall_hit_w          EQU #E96F   ; Hitbox width cache (min 1)
wall_hit_h          EQU #E970   ; Hitbox height cache (min 1)
wall_probe_left     EQU #E971   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #E972   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #E973   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #E974   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #E975   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #E995   ; Number of entries in active_entity_list
hero_entity_id      EQU #E996   ; First current-screen entity flagged as player (#FF = none)
active_entity_list_dirty EQU #E997   ; 1=rebuild active_entity_list required
input_entity_list   EQU #E998   ; Active current-screen entities with Input component (MAX_ENTITIES bytes)
input_entity_count  EQU #E9B8   ; Number of entries in input_entity_list
render_entity_list  EQU #E9B9   ; Active current-screen entities with Sprite component (MAX_ENTITIES bytes)
render_entity_count EQU #E9D9   ; Number of entries in render_entity_list
collision_entity_list EQU #E9DA   ; Active current-screen entities with Collision component (MAX_ENTITIES bytes)
collision_entity_count EQU #E9FA   ; Number of entries in collision_entity_list
ground_entity_list  EQU #E9FB   ; Active current-screen entities with Collision or Gravity (MAX_ENTITIES bytes)
ground_entity_count EQU #EA1B   ; Number of entries in ground_entity_list
anim_entity_list    EQU #EA1C   ; Active current-screen entities with Animation+Sprite (MAX_ENTITIES bytes)
anim_entity_count   EQU #EA3C   ; Number of entries in anim_entity_list

; Entity-entity collision optimized variables
coll_list           EQU #EA3D   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #EA5D   ; Number of entities in coll_list
coll_src_left       EQU #EA5E   ; Source AABB left edge (scratch)
coll_src_right      EQU #EA5F   ; Source AABB right edge (scratch)
coll_src_top        EQU #EA60   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #EA61   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #EA62   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #EA62   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #EA64   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #EA66   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #EA68   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #EA6A   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #EA6C   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #EA6E   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #EA70   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #EA72   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #EA73   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #EA78   ; Frame counter (16-bit)
task_exec_time          EQU #EA7A   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #EA7C   ; Set to 1 on each VBlank (1 byte)
RAM_INTERRUPT_END       EQU #EA7D   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #EA7D   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #EA7E   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #EA7F   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #EA80   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #EA81   ; 0=stopped, 1=track active
music_muted          EQU #EA82   ; 0=audible, 1=muted/pause
music_loop           EQU #EA83   ; 0=no loop, 1=loop enabled
music_track_index    EQU #EA84   ; Current ROM track index
music_row_frames     EQU #EA85   ; Frames per tracker row
music_row_countdown  EQU #EA86   ; Countdown to next row
music_order_pos      EQU #EA87   ; Current order position
music_pattern_index  EQU #EA88   ; Current pattern index
music_pattern_row    EQU #EA89   ; Current row inside pattern
music_pattern_rows   EQU #EA8A   ; Cached rows in current pattern
music_track_ptr_l    EQU #EA8B   ; Current track pointer low byte
music_track_ptr_h    EQU #EA8C   ; Current track pointer high byte
music_pattern_ptr_l  EQU #EA8D   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #EA8E   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #EA8F   ; PSG mixer shadow for music runtime
music_ch_note_base EQU #EA90   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #EA90   ; Channel A
music_ch_b_note EQU #EA91   ; Channel B
music_ch_c_note EQU #EA92   ; Channel C
music_ch_instrument_base EQU #EA93   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #EA93   ; Channel A
music_ch_b_instrument EQU #EA94   ; Channel B
music_ch_c_instrument EQU #EA95   ; Channel C
music_ch_ornament_base EQU #EA96   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #EA96   ; Channel A
music_ch_b_ornament EQU #EA97   ; Channel B
music_ch_c_ornament EQU #EA98   ; Channel C
music_ch_volume_base EQU #EA99   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #EA99   ; Channel A
music_ch_b_volume EQU #EA9A   ; Channel B
music_ch_c_volume EQU #EA9B   ; Channel C
music_ch_vol_step_base EQU #EA9C   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #EA9C   ; Channel A
music_ch_b_vol_step EQU #EA9D   ; Channel B
music_ch_c_vol_step EQU #EA9E   ; Channel C
music_ch_tone_step_base EQU #EA9F   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #EA9F   ; Channel A
music_ch_b_tone_step EQU #EAA0   ; Channel B
music_ch_c_tone_step EQU #EAA1   ; Channel C
music_ch_noise_step_base EQU #EAA2   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #EAA2   ; Channel A
music_ch_b_noise_step EQU #EAA3   ; Channel B
music_ch_c_noise_step EQU #EAA4   ; Channel C
music_ch_orn_step_base EQU #EAA5   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #EAA5   ; Channel A
music_ch_b_orn_step EQU #EAA6   ; Channel B
music_ch_c_orn_step EQU #EAA7   ; Channel C
...
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #EAA8   ; End of project variables (10920 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#EAA8: Project variables (10920 bytes)
;   #EAA8-#F37F: Free RAM (~2264 bytes available)
...
sprite_asset_base_pattern_slot_worldmap_1776512078647:
    ...
    db 4 ; Sprite 6: nina_walk_left
    db 0 ; Sprite 7: nina_grub_left
    db 0 ; Sprite 8: nina_jump_left
    db 0 ; Sprite 9: nina_idle_left
...
SCREEN_PAN1_0_SPRITE_PATTERN_SLOTS EQU 9
