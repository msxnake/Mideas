...
show_sprite:
    ...
    push hl
    ld de, sprite_y_pos
    add hl, de                    ; HL points to sprite_y_pos[A]
    pop de                        ; DE = sprite offset
    push de                       ; Save sprite offset again
    ld a, c                       ; A = Y parameter (from C)
    ld (hl), a                    ; sprite_y_pos[A] = Y
    pop hl                        ; HL = sprite offset

    ; Set X position
    push hl
    ld de, sprite_x_pos
    add hl, de                    ; HL points to sprite_x_pos[A]
    ld (hl), b                    ; sprite_x_pos[A] = X (B still has X)
    pop hl

    ; Set pattern
    push hl
    ld de, sprite_pattern
    add hl, de                    ; HL points to sprite_pattern[A]
    pop de                        ; DE = sprite offset
    ld a, (sp)                    ; A = original D (pattern) from stack (low byte of DE)
    ld (hl), a                    ; sprite_pattern[A] = pattern
    ld l, e                       ; HL = sprite offset (restore from DE)
    ld h, d

    ; Set color
    ld de, sprite_color
    add hl, de                    ; HL points to sprite_color[A]
    pop de                        ; DE = original D=pattern, E=color
    ld (hl), e                    ; sprite_color[A] = color
...
load_world_worldmap_1759564718811:
...
; Entity: Key Item 1
ENTITY_KEY_ITEM_1_ID EQU 0
ENTITY_KEY_ITEM_1_TEMPLATE EQU "tpl_item_key"
ENTITY_KEY_ITEM_1_X EQU 15
...
; Pure game - no text strings needed for SPRITE1(2)(1)
