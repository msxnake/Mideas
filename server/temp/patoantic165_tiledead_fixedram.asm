...
tileDead_dbg        EQU #D0DC   ; Debug byte: current hero deadly contact
tileDead_latched_dbg EQU #D0DD   ; Debug byte: latched hero deadly contact
tileDead_x_dbg      EQU #D0DE   ; Debug byte: last sampled deadly tile X
tileDead_y_dbg      EQU #D0DF   ; Debug byte: last sampled deadly tile Y
tileDead_value_dbg  EQU #D0E0   ; Debug byte: last raw deadly behavior value

; Wall collision temporary variables
wall_temp_x         EQU #D0E1   ; Cached entity X for wall checks
wall_temp_y         EQU #D0E2   ; Cached entity Y for wall checks
wall_hit_left       EQU #D0E3   ; Hitbox left edge cache
wall_hit_top        EQU #D0E4   ; Hitbox top edge cache
wall_hit_right      EQU #D0E5   ; Hitbox right edge cache
wall_hit_bottom     EQU #D0E6   ; Hitbox bottom edge cache
wall_hit_w          EQU #D0E7   ; Hitbox width cache (min 1)
wall_hit_h          EQU #D0E8   ; Hitbox height cache (min 1)
wall_probe_left     EQU #D0E9   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #D0EA   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #D0EB   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #D0EC   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #D0ED   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #D10D   ; Number of entries in active_entity_list
active_entity_list_dirty EQU #D10E   ; 1=rebuild active_entity_list required

; Entity-entity collision optimized variables
coll_list           EQU #D10F   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #D12F   ; Number of entities in coll_list
coll_src_left       EQU #D130   ; Source AABB left edge (scratch)
coll_src_right      EQU #D131   ; Source AABB right edge (scratch)
coll_src_top        EQU #D132   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #D133   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #D134   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #D134   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #D136   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #D138   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #D13A   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #D13C   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #D13E   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #D140   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #D142   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #D144   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #D145   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #D14A   ; Frame counter (16-bit)
task_exec_time          EQU #D14C   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #D14E   ; Set to 1 on each VBlank (1 byte)
RAM_INTERRUPT_END       EQU #D14F   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #D14F   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #D150   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #D151   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #D152   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #D153   ; 0=stopped, 1=track active
music_muted          EQU #D154   ; 0=audible, 1=muted/pause
music_loop           EQU #D155   ; 0=no loop, 1=loop enabled
music_track_index    EQU #D156   ; Current ROM track index
music_row_frames     EQU #D157   ; Frames per tracker row
music_row_countdown  EQU #D158   ; Countdown to next row
music_order_pos      EQU #D159   ; Current order position
music_pattern_index  EQU #D15A   ; Current pattern index
music_pattern_row    EQU #D15B   ; Current row inside pattern
music_pattern_rows   EQU #D15C   ; Cached rows in current pattern
music_track_ptr_l    EQU #D15D   ; Current track pointer low byte
music_track_ptr_h    EQU #D15E   ; Current track pointer high byte
music_pattern_ptr_l  EQU #D15F   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #D160   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #D161   ; PSG mixer shadow for music runtime
music_ch_note_base EQU #D162   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #D162   ; Channel A
music_ch_b_note EQU #D163   ; Channel B
music_ch_c_note EQU #D164   ; Channel C
music_ch_instrument_base EQU #D165   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #D165   ; Channel A
music_ch_b_instrument EQU #D166   ; Channel B
music_ch_c_instrument EQU #D167   ; Channel C
music_ch_ornament_base EQU #D168   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #D168   ; Channel A
music_ch_b_ornament EQU #D169   ; Channel B
music_ch_c_ornament EQU #D16A   ; Channel C
music_ch_volume_base EQU #D16B   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #D16B   ; Channel A
music_ch_b_volume EQU #D16C   ; Channel B
music_ch_c_volume EQU #D16D   ; Channel C
music_ch_vol_step_base EQU #D16E   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #D16E   ; Channel A
music_ch_b_vol_step EQU #D16F   ; Channel B
music_ch_c_vol_step EQU #D170   ; Channel C
music_ch_tone_step_base EQU #D171   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #D171   ; Channel A
music_ch_b_tone_step EQU #D172   ; Channel B
music_ch_c_tone_step EQU #D173   ; Channel C
music_ch_noise_step_base EQU #D174   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #D174   ; Channel A
music_ch_b_noise_step EQU #D175   ; Channel B
music_ch_c_noise_step EQU #D176   ; Channel C
music_ch_orn_step_base EQU #D177   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #D177   ; Channel A
music_ch_b_orn_step EQU #D178   ; Channel B
music_ch_c_orn_step EQU #D179   ; Channel C

; ==================================================================
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #D17A   ; End of project variables (4474 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#D17A: Project variables (4474 bytes)
;   #D17A-#F37F: Free RAM (~8710 bytes available)
