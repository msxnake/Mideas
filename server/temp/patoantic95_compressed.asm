...
; Estimated payload bytes: 110941
; Estimated banks used: 14
; ------------------------------------------------------------------
; BANK 00 @#0000 : patterns.asm (1075 bytes)
; BANK 00 @#0433 : colors.asm (969 bytes)
; BANK 00 @#07FC : components.asm (22 bytes)
; BANK 00 @#0812 : entities.asm (5330 bytes)
; BANK 00 @#1CE4 : worlds.asm (796 bytes)
; BANK 01 @#0000 : worlds.asm (3204 bytes)
; BANK 01 @#0C84 : screens.asm part 1/3 (4988 bytes)
; BANK 02 @#0000 : screens.asm part 2/3 (8192 bytes)
; BANK 03 @#0000 : screens.asm part 3/3 (4346 bytes)
; BANK 03 @#10FA : sprites.asm part 1/2 (3846 bytes)
; BANK 04 @#0000 : sprites.asm part 2/2 (5919 bytes)
; BANK 04 @#171F : font.asm (2273 bytes)
; BANK 05 @#0000 : font.asm (1274 bytes)
; BANK 05 @#04FA : hud.asm (3997 bytes)
; BANK 05 @#1497 : menus.asm (444 bytes)
; BANK 05 @#1653 : sound.asm (2477 bytes)
; BANK 06 @#0000 : sound.asm (993 bytes)
; BANK 06 @#03E1 : scroll.asm (2353 bytes)
; BANK 06 @#0D12 : animtiles.asm (4846 bytes)
; BANK 07 @#0000 : animtiles.asm (1070 bytes)
; BANK 07 @#042E : particles.asm (4963 bytes)
; BANK 07 @#1791 : statemachine.asm part 1/4 (2159 bytes)
...
; BANK 11 @#0000 : statemachine.asm part 5/4 (3852 bytes)
; BANK 11 @#0F0C : gameflow.asm part 1/3 (4340 bytes)
; BANK 12 @#0000 : gameflow.asm part 2/3 (8192 bytes)
; BANK 13 @#0000 : gameflow.asm part 3/3 (4445 bytes)
...
wall_temp_x         EQU #CFFA   ; Cached entity X for wall checks
wall_temp_y         EQU #CFFB   ; Cached entity Y for wall checks
wall_hit_left       EQU #CFFC   ; Hitbox left edge cache
wall_hit_top        EQU #CFFD   ; Hitbox top edge cache
wall_hit_right      EQU #CFFE   ; Hitbox right edge cache
wall_hit_bottom     EQU #CFFF   ; Hitbox bottom edge cache
wall_hit_w          EQU #D000   ; Hitbox width cache (min 1)
wall_hit_h          EQU #D001   ; Hitbox height cache (min 1)
wall_probe_left     EQU #D002   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #D003   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #D004   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #D005   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #D006   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #D026   ; Number of entries in active_entity_list
active_entity_list_dirty EQU #D027   ; 1=rebuild active_entity_list required

; Entity-entity collision optimized variables
coll_list           EQU #D028   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #D048   ; Number of entities in coll_list
coll_src_left       EQU #D049   ; Source AABB left edge (scratch)
coll_src_right      EQU #D04A   ; Source AABB right edge (scratch)
coll_src_top        EQU #D04B   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #D04C   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #D04D   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #D04D   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #D04F   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #D051   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #D053   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #D055   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #D057   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #D059   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #D05B   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #D05D   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #D05E   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #D063   ; Frame counter (16-bit)
task_exec_time          EQU #D065   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #D067   ; Set to 1 on each VBlank (1 byte)
RAM_INTERRUPT_END       EQU #D068   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #D068   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #D069   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #D06A   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #D06B   ; Next sound frame pointer high byte

; ==================================================================
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #D06C   ; End of project variables (4204 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#D06C: Project variables (4204 bytes)
;   #D06C-#F37F: Free RAM (~8980 bytes available)
...
load_pattern_bank0:
    ...
    ld bc, 104    ; Total bytes for all tile characters (16x16 tiles = 4 chars each)
...
load_pattern_bank1:
    ...
    ld bc, 104    ; Total bytes for all tile characters
    call FAST_LDIRVM              ; Fast VRAM write (direct port access)
    call mapper_pop_p2
    ret
...
; ==================================================================
tile_color_bank0:
    ; ZX0 compressed tile_color (104 -> 29 bytes)
    DB #8A,#41,#51,#FF,#FA,#F0,#F7,#FE,#E0,#E9,#F0,#BA,#C1,#FF,#31,#A2
    DB #C1,#8A,#81,#91,#A0,#A1,#4F,#9A,#7D,#70,#5D,#55,#56
load_color_bank0:
    ...
    ld bc, 104     ; Total color bytes for all tile characters
    call FAST_LDIRVM              ; Fast VRAM write (direct port access)
    call mapper_pop_p2
    ret
...
SCREEN_PAN1_0_LAYOUT:
    ...
    DB #95,#84,#69,#00,#56,#95,#84,#6A,#80,#E3,#81,#FC,#E4,#FB,#FC,#6E
    DB #82,#50,#52,#39,#82,#83,#FC,#BD,#80,#02,#1F,#B1,#D0,#C4,#AF,#86
    DB #80,#03,#D5,#00,#37,#7E,#79,#80,#5F,#8B,#9E,#FC,#80,#57,#88,#CD
    DB #00,#85,#32,#C6,#57,#E7,#FB,#C0,#F9,#76,#38,#1F,#80,#76,#3E,#57
    DB #DE,#80,#F6,#F4,#80,#4F,#C0,#76,#BE,#03,#BF,#C0,#F0,#A8,#40,#86
    DB #42,#9D,#82,#F9,#88,#A8,#E0,#FE,#78,#80,#7B,#FC,#D4,#80,#FF,#FC
    DB #80,#4A,#86,#13,#94,#FC,#FC,#CB,#80,#E5,#FC,#3E,#CB,#80,#35,#55
    DB #58
...
BEHAVIOR_PAN1_0_DATA:
    ; ZX0 compressed behavior (768 -> 56 bytes)
    DB #85,#00,#56,#80,#10,#0F,#90,#57,#D4,#40,#F4,#C0,#69,#08,#13,#D5
    DB #00,#F0,#C0,#03,#34,#C0,#7C,#C0,#3F,#EE,#C5,#70,#F0,#C0,#13,#61
    DB #C2,#7C,#EE,#B9,#08,#90,#0E,#98,#5F,#C0,#55,#F5,#BA,#E8,#C1,#08
    DB #7D,#FE,#1E,#C0,#10,#D5,#55,#60
SCREEN_PAN2_1_WIDTH     EQU 32
SCREEN_PAN2_1_HEIGHT    EQU 24
SCREEN_PAN2_1_SIZE      EQU 768

SCREEN_PAN2_1_LAYOUT:
    ; ZX0 compressed layout (768 -> 50 bytes)
    DB #85,#00,#56,#3D,#80,#81,#FC,#1F,#BA,#F5,#3A,#2A,#80,#94,#86,#4D
    DB #95,#46,#0F,#00,#55,#77,#46,#41,#7C,#C4,#23,#82,#83,#80,#FC,#E0
    DB #84,#7D,#40,#0F,#9C,#F5,#C0,#6D,#86,#34,#40,#4F,#81,#D1,#FC,#D5
    DB #55,#60
...
BEHAVIOR_PAN2_1_DATA:
    ...
    DB #3D,#00,#55,#DD,#46,#05,#F4,#C4,#E0,#C0,#4F,#40,#43,#81,#C0,#69
    DB #08,#43,#D5,#FE,#35,#55,#58
...
SCREEN_PAN3_3_LAYOUT:
    ; ZX0 compressed layout (768 -> 78 bytes)
    DB #85,#00,#56,#3D,#80,#81,#FC,#1F,#82,#57,#8A,#C0,#86,#41,#B9,#82
    DB #F4,#BC,#83,#FC,#38,#80,#0A,#00,#54,#F5,#FE,#E0,#80,#41,#AC,#86
    DB #D4,#54,#E5,#80,#32,#EB,#93,#88,#D0,#80,#A4,#88,#5A,#82,#5E,#F4
    DB #35,#C3,#9A,#C7,#00,#9D,#2C,#C1,#00,#D7,#34,#53,#A9,#80,#82,#A4
    DB #86,#5F,#FC,#13,#E7,#C1,#00,#95,#FC,#E0,#80,#4D,#55,#56
...
BEHAVIOR_PAN3_3_DATA:
    ...
    DB #39,#C0,#10,#6B,#08,#35,#54,#39,#C0,#14,#D7,#9C,#57,#59,#74,#03
    DB #85,#C0,#2D,#08,#31,#82,#1E,#C0,#01,#35,#55,#58
...
ZX0_SPRITE_FRAME_NINA_JUMP_RIGHT_4_F0_DATA:
    ; ZX0 compressed sprite frame NINA_JUMP_RIGHT_4_F0 (64 -> 54 bytes)
    DB #68,#00,#03,#07,#29,#09,#14,#20,#40,#69,#0B,#02,#27,#0F,#AB,#80
    DB #00,#86,#C0,#F8,#00,#49,#E8,#40,#C0,#E1,#AA,#00,#26,#02,#03,#01
    DB #87,#00,#10,#40,#00,#54,#88,#CA,#08,#F0,#D0,#D8,#F8,#F0,#80,#14
    DB #80,#A3,#55,#55,#55,#80
...
ZX0_SPRITE_FRAME_NINA_JUMP_LEFT_11_F0_DATA:
    ; ZX0 compressed sprite frame NINA_JUMP_LEFT_11_F0 (64 -> 54 bytes)
    DB #08,#00,#03,#1F,#00,#64,#88,#17,#02,#03,#87,#55,#00,#A0,#C0,#E0
    DB #A5,#90,#28,#04,#02,#99,#D0,#40,#E4,#F0,#D5,#01,#00,#7A,#0F,#0B
    DB #1B,#1F,#0F,#01,#28,#E8,#B9,#AA,#A8,#96,#40,#C0,#80,#80,#00,#08
    DB #02,#BD,#2A,#EB,#55,#56
...
; Free RAM buffer for tile pattern data decompression (104 bytes)
; ==================================================================
ZX0_TILE_PATTERN_BUFFER EQU #D700

; ==================================================================
; ZX0 TILE COLOR BUFFER (AUTO-INJECTED)
; Free RAM buffer for tile color data decompression (104 bytes)
; ==================================================================
