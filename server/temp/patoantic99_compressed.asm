...
; Estimated payload bytes: 111419
...
; BANK 00 @#0812 : entities.asm (5372 bytes)
; BANK 00 @#1D0E : worlds.asm (754 bytes)
; BANK 01 @#0000 : worlds.asm (3246 bytes)
; BANK 01 @#0CAE : screens.asm part 1/3 (4946 bytes)
; BANK 02 @#0000 : screens.asm part 2/3 (8192 bytes)
; BANK 03 @#0000 : screens.asm part 3/3 (4388 bytes)
; BANK 03 @#1124 : sprites.asm part 1/2 (3804 bytes)
; BANK 04 @#0000 : sprites.asm part 2/2 (5961 bytes)
; BANK 04 @#1749 : font.asm (2231 bytes)
; BANK 05 @#0000 : font.asm (1316 bytes)
; BANK 05 @#0524 : hud.asm (4339 bytes)
; BANK 05 @#1617 : menus.asm (444 bytes)
; BANK 05 @#17D3 : sound.asm (2093 bytes)
; BANK 06 @#0000 : sound.asm (1471 bytes)
; BANK 06 @#05BF : scroll.asm (2353 bytes)
; BANK 06 @#0EF0 : animtiles.asm (4368 bytes)
; BANK 07 @#0000 : animtiles.asm (1548 bytes)
; BANK 07 @#060C : particles.asm (4963 bytes)
; BANK 07 @#196F : statemachine.asm part 1/4 (1681 bytes)
...
; BANK 11 @#0000 : statemachine.asm part 5/4 (4330 bytes)
; BANK 11 @#10EA : gameflow.asm part 1/3 (3862 bytes)
; BANK 12 @#0000 : gameflow.asm part 2/3 (8192 bytes)
; BANK 13 @#0000 : gameflow.asm part 3/3 (4923 bytes)
...
wall_temp_x         EQU #D01A   ; Cached entity X for wall checks
wall_temp_y         EQU #D01B   ; Cached entity Y for wall checks
wall_hit_left       EQU #D01C   ; Hitbox left edge cache
wall_hit_top        EQU #D01D   ; Hitbox top edge cache
wall_hit_right      EQU #D01E   ; Hitbox right edge cache
wall_hit_bottom     EQU #D01F   ; Hitbox bottom edge cache
wall_hit_w          EQU #D020   ; Hitbox width cache (min 1)
wall_hit_h          EQU #D021   ; Hitbox height cache (min 1)
wall_probe_left     EQU #D022   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #D023   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #D024   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #D025   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #D026   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #D046   ; Number of entries in active_entity_list
active_entity_list_dirty EQU #D047   ; 1=rebuild active_entity_list required

; Entity-entity collision optimized variables
coll_list           EQU #D048   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #D068   ; Number of entities in coll_list
coll_src_left       EQU #D069   ; Source AABB left edge (scratch)
coll_src_right      EQU #D06A   ; Source AABB right edge (scratch)
coll_src_top        EQU #D06B   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #D06C   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #D06D   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #D06D   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #D06F   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #D071   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #D073   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #D075   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #D077   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #D079   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #D07B   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #D07D   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #D07E   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #D083   ; Frame counter (16-bit)
task_exec_time          EQU #D085   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #D087   ; Set to 1 on each VBlank (1 byte)
RAM_INTERRUPT_END       EQU #D088   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #D088   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #D089   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #D08A   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #D08B   ; Next sound frame pointer high byte

; ==================================================================
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #D08C   ; End of project variables (4236 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#D08C: Project variables (4236 bytes)
;   #D08C-#F37F: Free RAM (~8948 bytes available)
...
anim_done_entity:
    ...
    ; ==================================================================
        ...
        update_jump_component:
            ...
            ; Uses: entity_jump_count, entity_jump_max, entity_on_ground, entity_gravity_vel
