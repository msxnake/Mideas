...
; Warning: none
; ------------------------------------------------------------------
; DYNAMIC BANK PACKER (FFD) â€” Estimated layout for code banks
; ------------------------------------------------------------------
; Bank 1 [#6000-#8000]: components (58707/8192 bytes est.)
; Bank 2 [#8000-#A000]: statemachine, gameflow (28557/8192 bytes est.)
; Bank 5 [#6000-#8000]: bosses (11107/8192 bytes est.) [FAR â€” accessed via trampoline]
; Bank 6 [#6000-#8000]: screens_code (7509/8192 bytes est.) [FAR â€” accessed via trampoline]
; Bank 7 [#6000-#8000]: entities (6179/8192 bytes est.) [FAR â€” accessed via trampoline]
; Bank 8 [#6000-#8000]: sprites (5523/8192 bytes est.) [FAR â€” accessed via trampoline]
; Bank 9 [#6000-#8000]: sound (4521/8192 bytes est.) [FAR â€” accessed via trampoline]
; Bank 10 [#6000-#8000]: hud (3995/8192 bytes est.) [FAR â€” accessed via trampoline]
; Bank 11 [#6000-#8000]: font (3585/8192 bytes est.) [FAR â€” accessed via trampoline]
; Bank 12 [#6000-#8000]: animtiles (3489/8192 bytes est.) [FAR â€” accessed via trampoline]
; Bank 13 [#6000-#8000]: scroll (2418/8192 bytes est.) [FAR â€” accessed via trampoline]
; Bank 14 [#6000-#8000]: worlds (2147/8192 bytes est.) [FAR â€” accessed via trampoline]
; Bank 15 [#6000-#8000]: patterns_code (894/8192 bytes est.) [FAR â€” accessed via trampoline]
; Bank 16 [#6000-#8000]: colors_code (846/8192 bytes est.) [FAR â€” accessed via trampoline]
...
; ##################################################################
; BANK 0 â€” Bootstrap (#4000h-#5FFFh, FIXED window in Konami mapper)
...
; Flow: Start â†’ WorldLink (gfn_1776512097888)
; ==================================================================
...
FAST_LDIRVM:
    ...
    ; NOTE: The old LD A,I / PUSH AF / RET PO pattern is unreliable on Z80 â€”
...
; ==================================================================
; PAGE-0 STUBS â€” labels required by header.asm, no-ops in megarom
; ==================================================================
...
; ==================================================================
; FAR-CALL TRAMPOLINES â€” bank 0 (always accessible at #4000-#5FFF)
...
; ==================================================================
; RESIDENT CALL WRAPPERS â€” bank 0 stable entrypoints
...
; ==================================================================
; INIT_GAME_SYSTEMS â€” in bank 0 so it is reachable from any bank
...
; --- End of Bank 0 â€” pad to 8KB boundary ---
BANK_0_USED_END:
    ds #6000 - $, #FF

; ##################################################################
; BANK 1 â€” [#6000h-#8000h] PRIMARY: components
; (Always mapped at boot: bank1â†’P1, bank2â†’P2)
; ##################################################################
    org #6000
...
;   Inputs:
;     - B = tile row    (0..23, out-of-range â†’ A=0, passable)
;     - C = tile column (0..31, out-of-range â†’ A=0, passable)
...
get_behavior_tile:
    ...
    ; NOTE: jp nc (not jr nc) to gbt_oob â€” gbt_oob is a global label defined after
...
get_behavior_tile_nb:
    ; Entry point for callers that guarantee B âˆˆ 0..23 and C âˆˆ 0..31.
...
;            Used to apply entity_collision_offset_x/y to entity origin (entityâ†’hitbox).
;   Inputs:
...
;   Preserved: C, DE, HL
;   Notes:
;     - Negative offset: carry=0 after add â†’ underflow â†’ A clamped to 0.
;     - Positive offset: carry=1 after add â†’ overflow â†’ A clamped to 255.
...
;       Overflow (carry clear after add) â†’ A clamped to 255.
;     - If offset is positive: hitbox - offset computed directly.
;       Underflow (carry clear after sub) â†’ A clamped to 0.
...
aircontrol_should_lock_horizontal_c:
    xor a
    ret
    ; WallGrab system filtered out (requires WallCollision)
...
.ti_process_entity:
    ...
    ld a, (hl)
    and COMP_MASK_INPUT
    jp z, .ti_next                 ; No input component â†’ skip
    ...
    ; Convert pixel â†’ tile coords (div 8 via 3x rrca + and #1F)
    ld a, d
    rrca
    rrca
    rrca
...
; --- End of Bank 1 â€” pad to 8KB boundary ---
BANK_1_USED_END:
    ds #8000 - $, #FF

; ##################################################################
; BANK 2 â€” [#8000h-#A000h] PRIMARY: statemachine, gameflow
; (Always mapped at boot: bank1â†’P1, bank2â†’P2)
; ##################################################################
    org #8000
...
SM_FacingDirTablePtrs:
    DW sprite_dir_left_table    ; facing 1 (LEFT)  â†’ dec â†’ 0
    DW sprite_dir_right_table   ; facing 2 (RIGHT) â†’ dec â†’ 1
    DW sprite_dir_up_table      ; facing 3 (UP)    â†’ dec â†’ 2
    DW sprite_dir_down_table    ; facing 4 (DOWN)  â†’ dec â†’ 3
...
;   3. Reset de animaciÃ³n: pone entity_anim_frame y entity_anim_tick a 0.
;   4. Flags de animaciÃ³n: activa PLAYING, aplica el flag de LOOP del
;      sprite, borra ONLY_WHEN_MOVING y marca FORCE_UPLOAD para que el
;      prÃ³ximo update_animation_component sincronice el frame actual
...
;
; Input:
;   HL  = puntero al parÃ¡metro (sprite asset ID, 1 byte)
;   B   = entity index (convenciÃ³n SM_ExecuteActions)
;
; Output:
;   HL  = puntero al byte siguiente a los parÃ¡metros (para el caller)
;
; Destruye: AF, BC, DE, HL (todos restaurados al salir salvo HL=next param)
;
; Stack al entrar (top â†’ bottom):
;   [llamada desde SM_ExecuteActions]
; Stack al salir: igual que al entrar.
;
; Tablas ROM usadas:
;   SM_FacingDirTablePtrs      â€” punteros a las 4 tablas de redirect
;   sprite_loop_flags          â€” 1 byte/sprite: 0x02=loop, 0x00=one-shot
;   SM_SpritePatternPtrTable   â€” puntero al frame 0 de cada sprite
;   SM_SpriteLayerColorTable   â€” colores por sprite (SPRITE_MAX_ENTITY_LAYERS bytes/sprite)
;   SM_SpriteLayerYOffsetTable â€” offsets Y por sprite (SPRITE_MAX_ENTITY_LAYERS bytes/sprite)
;
; Variables RAM usadas:
;   entity_facing_dir          â€” direcciÃ³n actual de la entidad (0-4)
;   entity_sprite_asset_index  â€” Ã­ndice del sprite activo de la entidad
;   entity_anim_frame          â€” frame actual de la animaciÃ³n
;   entity_anim_tick           â€” contador de ticks entre frames
;   entity_anim_flags          â€” flags de animaciÃ³n (ver bits mÃ¡s abajo)
;   entity_sprite_config       â€” base HW sprite + layer count (2 bytes/entidad)
;   sprite_layer_colors        â€” colores actuales por slot HW sprite (RAM)
;   sprite_layer_y_offsets     â€” offsets Y actuales por slot HW sprite (RAM)
;
...
;   bit 3 = ANIM_FLAG_COMPLETED     (1 = one-shot llegÃ³ al Ãºltimo frame)
;   bit 4 = ANIM_FLAG_FORCE_UPLOAD  (1 = sincronizar frame actual en el prÃ³ximo update_animation_component)
;
...
; Se restaura explÃ­citamente con "ld b, 0" antes de los add hl, bc.
; ==================================================================
Action_ChangeSprite:
    ld a, (hl)              ; A = Sprite Asset ID pedido por la SM
    inc hl                  ; HL apunta al byte siguiente al parÃ¡metro
    push hl                 ; [stack] guarda puntero de parÃ¡metros para el ret final

    push af                 ; [stack] guarda Sprite Asset ID (se necesita tras setup)

    ; ------------------------------------------------------------------
    ; Setup: convertir B (entity index) a BC = (0, entity_index)
    ; ConvenciÃ³n de SM_ExecuteActions: B = entity index al entrar.
    ; ------------------------------------------------------------------
    ld c, b                 ; C = entity index
    ld b, 0                 ; B = 0  â†’  BC = (0, entity_index)
...
.acs_not_wall_grabbing:
    ...
    ; Se usarÃ¡ tras el bloque de redirect para escribir el sprite final.
    ld hl, entity_sprite_asset_index
    ...
    ; su variante para la direcciÃ³n actual.
    ;   facing 0 = sin direcciÃ³n â†’ usar sprite tal cual
    ;   facing 1 = izquierda  â†’ SM_FacingDirTablePtrs[0] â†’ sprite_dir_left_table
    ;   facing 2 = derecha     â†’ SM_FacingDirTablePtrs[1] â†’ sprite_dir_right_table
    ;   facing 3 = arriba      â†’ SM_FacingDirTablePtrs[2] â†’ sprite_dir_up_table
    ;   facing 4 = abajo       â†’ SM_FacingDirTablePtrs[3] â†’ sprite_dir_down_table
    ;
    ; Las tablas de direcciÃ³n son arrays de 1 byte por sprite asset:
    ...
    ; Al salir, B queda con el sprite ID (no con 0). Se corrige despuÃ©s.
    ; ------------------------------------------------------------------
...
.acs_input_facing_done:
    ...
    ld a, e                 ; A = facing dir
    or a
    jr z, .acs_dir_done     ; facing = 0 â†’ no hay redirect, usar sprite original
    cp 5
    jr nc, .acs_dir_done    ; facing invÃ¡lido â†’ ignorar redirect y usar sprite original

    ; Convertir facing (1-4) a Ã­ndice de tabla (0-3): dec a
    dec a                   ; A = Ã­ndice en SM_FacingDirTablePtrs (0=left, 1=right, 2=up, 3=down)
    ...
    ld e, (hl)
    inc hl
    ld d, (hl)              ; DE = puntero a la tabla de sprites para esta direcciÃ³n
...
.acs_dir_done:
    ...
    ; ------------------------------------------------------------------
    ; BLOQUE 2: Commit del sprite y reset de estado de animaciÃ³n
    ; ------------------------------------------------------------------

    ; D guardarÃ¡ el sprite ID para uso posterior (color update, loop flags).
    ...
    ; RESTAURAR B=0: el bloque de redirect dejÃ³ B=sprite_ID.
    ; Todos los "add hl, bc" siguientes necesitan BC = (0, entity_index).
    ld b, 0                 ; B = 0  â†’  BC = (0, entity_index)  [BUG FIX: corrupto por redirect]
    ...
    ;
    ; Cambios aplicados:
    ;   - bit 3 (COMPLETED)       â†’ 0  (el one-shot anterior ya no importa)
    ;   - bit 0 (PLAYING)         â†’ 1  (arrancar animaciÃ³n)
    ;   - bit 1 (LOOP)            â†’ segÃºn sprite_loop_flags del nuevo sprite
    ;   - bit 2 (ONLY_WHEN_MOVING)â†’ 0  SIEMPRE, para cualquier sprite
    ;   - bit 4 (FORCE_UPLOAD)    â†’ 1  pedir sincronizaciÃ³n del frame actual
    ;                                 en el prÃ³ximo update_animation_component
    ;
    ; RazÃ³n de limpiar ONLY_WHEN_MOVING siempre:
    ;   Cuando el SM llama ChangeSprite, lo hace porque quiere mostrar ese
    ;   sprite ahora. La animaciÃ³n debe avanzar siempre que PLAYING=1,
    ;   sin importar la velocidad. La lÃ³gica de "anima solo si se mueve"
    ;   es solo relevante para el sprite inicial de la entidad (config del
    ;   editor). Una vez en la SM, el estado controla quÃ© sprite se muestra.
    ;   Si se dejara ONLY_WHEN_MOVING=1 para sprites loop, la animaciÃ³n walk
    ;   no avanzarÃ­a: la fricciÃ³n del movement component puede dejar vel_x=0
    ...
    or e                    ; bit 1 = nuevo loop flag (E=0x02 o 0x00 segÃºn el sprite)
    ...
    ; para que render_sprites use los colores correctos en el prÃ³ximo frame.
    ;
    ...
    jp nc, .acs_skip_color_update  ; fuera de rango â†’ saltar
    ...
    ; mediante suma repetida (SPRITE_MAX_ENTITY_LAYERS es pequeÃ±o, tÃ­picamente 2-4)
...
.acs_mul_max_layers:
    ...
    djnz .acs_mul_max_layers ; repetir SPRITE_MAX_ENTITY_LAYERS veces â†’ HL = spriteId * maxLayers
    ...
    ; C = slot HW actual (se incrementa en cada iteraciÃ³n)
...
.acs_color_update_loop:
    ...
    ld h, 0
    ld l, c                 ; HL = slot HW actual (Ã­ndice en sprite_layer_colors)
    ld de, sprite_layer_colors
    ...
    ;
    ; Misma alineaciÃ³n que los colores: se indexa por slot HW sprite.
    ; ------------------------------------------------------------------
...
.acs_skip_color_update:
    ...
    ; ------------------------------------------------------------------
    ; Epilogue: restaurar puntero de parÃ¡metros y retornar al dispatcher
    ; ------------------------------------------------------------------
    pop hl                  ; HL = puntero al byte tras los parÃ¡metros  [del push inicial]
    ret
...
; --- End of Bank 2 â€” pad to 8KB boundary ---
BANK_2_USED_END:
    ds #A000 - $, #FF

; ==================================================================
; DATA BANKS â€” Zone-packed data (8192 bytes per zone)
; First data bank: 3
...
; ##################################################################
; FAR BANK 5 â€” [#6000h-#8000h] FAR CODE: bosses
...
; --- End of Far Bank 5 â€” pad to 8KB boundary ---
BANK_5_USED_END:
    ds #8000 - $, #FF
    org FAR_BANK_5_ROM_START + #2000

; ##################################################################
; FAR BANK 6 â€” [#6000h-#8000h] FAR CODE: screens_code
; Accessed ONLY via trampolines in bank 0 (entrypoint_far labels).
...
; --- End of Far Bank 6 â€” pad to 8KB boundary ---
BANK_6_USED_END:
    ds #8000 - $, #FF
    org FAR_BANK_6_ROM_START + #2000

; ##################################################################
; FAR BANK 7 â€” [#6000h-#8000h] FAR CODE: entities
; Accessed ONLY via trampolines in bank 0 (entrypoint_far labels).
...
; --- End of Far Bank 7 â€” pad to 8KB boundary ---
BANK_7_USED_END:
    ds #8000 - $, #FF
    org FAR_BANK_7_ROM_START + #2000

; ##################################################################
; FAR BANK 8 â€” [#6000h-#8000h] FAR CODE: sprites
; Accessed ONLY via trampolines in bank 0 (entrypoint_far labels).
...
; --- End of Far Bank 8 â€” pad to 8KB boundary ---
BANK_8_USED_END:
    ds #8000 - $, #FF
    org FAR_BANK_8_ROM_START + #2000

; ##################################################################
; FAR BANK 9 â€” [#6000h-#8000h] FAR CODE: sound
; Accessed ONLY via trampolines in bank 0 (entrypoint_far labels).
...
; --- End of Far Bank 9 â€” pad to 8KB boundary ---
BANK_9_USED_END:
    ds #8000 - $, #FF
    org FAR_BANK_9_ROM_START + #2000

; ##################################################################
; FAR BANK 10 â€” [#6000h-#8000h] FAR CODE: hud
; Accessed ONLY via trampolines in bank 0 (entrypoint_far labels).
...
; --- End of Far Bank 10 â€” pad to 8KB boundary ---
BANK_10_USED_END:
    ds #8000 - $, #FF
    org FAR_BANK_10_ROM_START + #2000

; ##################################################################
; FAR BANK 11 â€” [#6000h-#8000h] FAR CODE: font
; Accessed ONLY via trampolines in bank 0 (entrypoint_far labels).
...
; --- End of Far Bank 11 â€” pad to 8KB boundary ---
BANK_11_USED_END:
    ds #8000 - $, #FF
    org FAR_BANK_11_ROM_START + #2000

; ##################################################################
; FAR BANK 12 â€” [#6000h-#8000h] FAR CODE: animtiles
; Accessed ONLY via trampolines in bank 0 (entrypoint_far labels).
...
; --- End of Far Bank 12 â€” pad to 8KB boundary ---
BANK_12_USED_END:
    ds #8000 - $, #FF
    org FAR_BANK_12_ROM_START + #2000

; ##################################################################
; FAR BANK 13 â€” [#6000h-#8000h] FAR CODE: scroll
; Accessed ONLY via trampolines in bank 0 (entrypoint_far labels).
...
; --- End of Far Bank 13 â€” pad to 8KB boundary ---
BANK_13_USED_END:
    ds #8000 - $, #FF
    org FAR_BANK_13_ROM_START + #2000

; ##################################################################
; FAR BANK 14 â€” [#6000h-#8000h] FAR CODE: worlds
; Accessed ONLY via trampolines in bank 0 (entrypoint_far labels).
...
; --- End of Far Bank 14 â€” pad to 8KB boundary ---
BANK_14_USED_END:
    ds #8000 - $, #FF
    org FAR_BANK_14_ROM_START + #2000

; ##################################################################
; FAR BANK 15 â€” [#6000h-#8000h] FAR CODE: patterns_code
; Accessed ONLY via trampolines in bank 0 (entrypoint_far labels).
...
; --- End of Far Bank 15 â€” pad to 8KB boundary ---
BANK_15_USED_END:
    ds #8000 - $, #FF
    org FAR_BANK_15_ROM_START + #2000

; ##################################################################
; FAR BANK 16 â€” [#6000h-#8000h] FAR CODE: colors_code
; Accessed ONLY via trampolines in bank 0 (entrypoint_far labels).
...
; --- End of Far Bank 16 â€” pad to 8KB boundary ---
BANK_16_USED_END:
    ds #8000 - $, #FF
