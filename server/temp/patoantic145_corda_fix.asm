; ==================================================================
; PATOANTIC145_CORDA_FIX - UNIFIED FILE
...
.ground_check_done:
    ...
    ; IMPORTANT:
    ;   - Probe inside the hitbox bottom edge so non-solid deadly tiles
    ;     trigger on overlap.
    ;   - Probe one pixel BELOW the hitbox so solid deadly floors still
    ;     trigger after wall collision snaps the entity on top of them.
    ;
    ; Read from the behavior map generated from the collision layer, NOT from
    ; the visual screen layout. Deadly flag uses logicalProperties.causesDamage
    ; (mapId bit 2 = #04).
    ld e, c
    ld d, 0
    ...
    ; First pass: inside the hitbox bottom edge (overlap detection)
    ...
    ; Second pass: one pixel below the hitbox (standing on deadly floor)
    ld a, (wall_hit_bottom)
    cp 191
