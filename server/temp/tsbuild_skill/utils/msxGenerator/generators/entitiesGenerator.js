...
function generateEntitiesFile(analysis) {
    ...
    const parseByte = (value, defaultValue) => {
        const num = typeof value === 'number' ? value : parseInt(String(value ?? ''), 10);
        if (Number.isNaN(num))
            return defaultValue;
        return Math.max(0, Math.min(255, num | 0));
    };
    const parseWord = (value, defaultValue) => {
        const num = typeof value === 'number' ? value : parseInt(String(value ?? ''), 10);
        if (Number.isNaN(num))
            return Math.max(0, Math.min(65535, defaultValue | 0));
        return Math.max(0, Math.min(65535, num | 0));
    };
    const parseOffsetByte = (value, defaultValue) => {
        const num = typeof value === 'number' ? value : parseInt(String(value ?? ''), 10);
        if (Number.isNaN(num))
    ...
    const resolveBehaviorType = (value) => {
    ...
    const resolveBehaviorDirection = (value) => {
        const normalized = String(value ?? 'right').trim().toLowerCase();
        if (['left', 'l', '-1', 'west', 'w'].includes(normalized))
            return 1;
        return 2;
    };
    ...
    const parseActionTrigger = (value, defaultValue = 'fire') => {
    ...
    const isCharShootMode = (value) => {
        const normalized = String(value ?? '').trim().toLowerCase().replace(/[\s_-]+/g, '');
        return ['char', 'tile', 'screenchar'].includes(normalized);
    };
    const resolveSpriteAssetIndex = (spriteRef, spriteNameToIndex, spriteCount) => {
    ...
    const buildTileIdToBaseCharMap = (tiles) => {
        const map = {};
    ...
    const resolveTileCharCode = (value, tileIdToCharCode) => {
    ...
    const buildGlobalVariableInfoMap = (analysis) => {
        const variableMap = {};
    ...
    const getComponentDefinitionDefaults = (definitionId) => {
        const definition = (analysis.components || []).find((component) => component?.id === definitionId);
        const defaults = {};
    ...
    const resolveConfiguredVariableInfo = (variableRef, variableMap) => {
    ...
    const stateMachineControlsSprite = (stateMachine) => collectStateMachineActions(stateMachine).some((action) => action?.type === 'CHANGE_SPRITE');
    ...
    const entityLabelNames = activeEntities.map((entity, index) => {
    ...
    const retractableGateConfigs = [];
    const wallJumpConfigs = [];
    const wallGrabConfigs = [];
    const dashConfigs = [];
    const airControlConfigs = [];
    const inWaterConfigs = [];
...
`;
        ...
        activeEntities.forEach((entity, index) => {
            ...
            const limitOnTemplateComp = template?.components?.find((component) => component?.definitionId === 'comp_limit_on' || component?.definitionName === 'Limit_on');
            ...
            const mirrorTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_mirror' || c.definitionName === 'Mirror');
            ...
            const behaviorTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_behavior' || c.definitionName === 'Behavior');
            const aiBehaviorTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_ai_behavior' || c.definitionName === 'AIBehavior');
            ...
            if (componentMask & 0x01)
                usedComponentNames.push('Position');
            if (componentMask & 0x02)
                usedComponentNames.push('Sprite');
            if (componentMask & 0x04)
                usedComponentNames.push('Movement');
            if (componentMask & 0x08)
                usedComponentNames.push('Collision');
            if (componentMask & 0x10)
                usedComponentNames.push('Input');
            if (componentMask & 0x20)
                usedComponentNames.push('Behavior');
            if (componentMask & 0x40)
                usedComponentNames.push('Health');
            if (componentMask & 0x80)
                usedComponentNames.push('Animation');
            ...
            if (hasMirrorComponent)
                usedComponentNames.push('Mirror');
            ...
            if (componentMask & 0x10) { // Has Input component
                ...
                if (cursorsComp) {
                    ...
                    directionMask = 0;
                    if (finalValues.allowUp !== false)
                        directionMask |= 0x01; // Bit 0
                    if (finalValues.allowDown !== false)
                        directionMask |= 0x02; // Bit 1
                    if (finalValues.allowLeft !== false)
                        directionMask |= 0x04; // Bit 2
                    if (finalValues.allowRight !== false)
            ...
            if (directionMask & 0x01)
                directionInfo.push('UP');
            if (directionMask & 0x02)
                directionInfo.push('DOWN');
            if (directionMask & 0x04)
                directionInfo.push('LEFT');
            if (directionMask & 0x08)
                directionInfo.push('RIGHT');
            ...
            if (componentMask & 0x80) {
                const animComp = template?.components?.find((c) => c.definitionId === 'comp_animation' || c.definitionName === 'Animation');
                const defaultValues = animComp?.defaultValues || animComp?.values || {};
...
`;
            ...
            if (componentMask & 0x08) { // COMP_MASK_COLLISION
                const collisionTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_collision' || c.definitionName === 'Collision');
                const wallCollisionTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_wall_collision' || c.definitionName === 'Wall Collision');
...
`;
            ...
            if (componentMask & 0x0040) { // COMP_MASK_HEALTH
                const healthTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_health' || c.definitionName === 'Health');
...
`;
            ...
            if (componentMask & 0x0400) { // COMP_MASK_AUTO_DESTROY
                const lifetimeTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_lifetime' || c.definitionName === 'Lifetime');
...
`;
            ...
            if (componentMask & 0x0800) { // COMP_MASK_DAMAGE
                const damageTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_damage' || c.definitionName === 'Damage');
...
`;
            ...
            if (componentMask & 0x1000) { // COMP_MASK_SHOOT
                const shootTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_shoot' || c.definitionName === 'Shoot');
                const shootDefaults = {
                ...
                const shootTrigger = parseActionTrigger(hasExplicitTrigger ? (shootValues.trigger ?? shootValues.fireTrigger) : undefined, shootIsCharMode ? 'action2' : 'fire');
                const projectileSprite = resolveSpriteAssetIndex(shootValues.spriteAssetId ?? shootValues.spriteId ?? shootValues.sprite, spriteNameToIndex, spriteCount);
...
`;
            ...
            const collectibleTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_collectible' || c.definitionName === 'Collectible');
...
`;
            ...
            const smTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_statemachine');
            ...
            if (smAssetId && analysis.stateMachines) {
                const sm = analysis.stateMachines.find((s) => s.id === smAssetId);
                if (sm && sm.states && sm.states.length > 0) {
                    ...
                    if (sm.initialStateId) {
                        const found = sm.states.find((s) => s.id === sm.initialStateId);
                        if (found)
                            initialState = found;
                    }
...
${initialOnEnterAsm}`;
            ...
            const gateTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_retractable_gate');
            if (gateTemplateComp) {
                ...
                const gateDirectionIdMap = { up: 1, down: 2, left: 3, right: 4 };
                const gateOperatorName = String(gateValues.triggerOperator || '==').trim();
                const gateOperatorIdMap = { '==': 0, '!=': 1, '>': 2, '<': 3, '>=': 4, '<=': 5 };
            ...
            const wallJumpTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_wall_jump');
            if (wallJumpTemplateComp) {
                ...
                const wallJumpHorizontalPush = wallJumpOverrides.horizontalPush ?? wallJumpTemplateDefaults.horizontalPush ?? 7;
                const wallJumpVerticalImpulse = wallJumpOverrides.verticalImpulse ?? wallJumpTemplateDefaults.verticalImpulse ?? 1280;
                const wallJumpLockFrames = wallJumpOverrides.lockFrames ?? wallJumpTemplateDefaults.lockFrames ?? 16;
                const wallJumpVerticalMagnitude = Math.max(1, parseWord(wallJumpVerticalImpulse, 1280));
                const wallJumpAnimationSprite = resolveSpriteAssetIndex(wallJumpValues.animationSpriteAssetId ?? wallJumpValues.wallJumpAnimationSprite ?? wallJumpValues.animationSprite, spriteNameToIndex, spriteCount);
                if (wallJumpEnabled) {
                    wallJumpConfig = {
                        enabled: 1,
            ...
            const dashTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_dash');
            ...
            const airControlTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_air_control');
            ...
            const inWaterTemplateComp = template?.components?.find((c) => c.definitionId === 'comp_in_water');
