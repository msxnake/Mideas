...
exports.buildWorldSpritePatternPolicyManifest = buildWorldSpritePatternPolicyManifest;
exports.generateSpritesFile = generateSpritesFile;
exports.getSpritesBank4Data = getSpritesBank4Data;
...
const hasBossAttackSpriteSlots = (analysis) => {
    const bosses = analysis.bosses || [];
    return bosses.some((boss) => {
        const attacks = boss?.attacks || [];
        if (attacks.length === 0)
            return false;
        const attackIds = new Set(attacks.map((attack) => attack?.id).filter(Boolean));
        return (boss?.phases || []).some((phase) => {
            const behaviorLoop = phase?.behaviorLoop || [];
            return behaviorLoop.some((action) => (action?.type === 'attack' && attackIds.has(action?.attackId)));
        });
    });
};
const collectBossAttackSpriteRefs = (analysis) => {
    const refs = [];
    const bosses = analysis.bosses || [];
    bosses.forEach((boss) => {
        const attacks = boss?.attacks || [];
...
const getWallJumpAnimationSpriteRef = (values) => {
...
const resolveEntityWallJumpAnimationSpriteAssetId = (entity, analysis) => {
    const template = analysis.templates?.find((t) => t.id === entity.entityTemplateId);
...
function buildWorldSpritePatternPolicyManifest(analysis) {
    const spriteCatalog = (0, spriteUtils_1.buildMSXDirectionalSpriteCatalog)(analysis.sprites || []);
    ...
    if (worldMaps.length > 0) {
        ...
        worldMaps.forEach((world, worldIndex) => {
            ...
            lines.push(`${worldIndex.toString().padStart(2, '0')} ${worldName} (${worldId}) -> ` +
 ...
 */
function generateSpritesFile(analysis, romMode = 'simple32k', dataInBank4 = false, targetFormat = 'konami') {
    ...
    const playerEntityIndexes = new Set(activeEntities
    ...
    const highReservedSlot = Math.max(hasSubmenuCursorSprite ? SUBMENU_CURSOR_BASE + SUBMENU_CURSOR_MAX : 0, hasBossAttackSpriteSlots(analysis)
...
`;
    ...
    const maxEntityLayers = Math.max(1, ...entityAllocations.map(a => a.layerCount), ...sprites.map(sprite => getSpriteLayerColors(sprite).length));
...
`}`;
                        ...
                        drawableLayerIndexes.forEach((layerIndex, layerOffset) => {
                            ...
                            const frameLayerResourceId = (0, megaromResourceArtifacts_1.buildResourceIdLabelFromAsmLabel)(frameLayerLabel);
...
function getSpritesBank4Data(analysis) {
    const sourceSprites = analysis.sprites || [];
    const spriteCatalog = (0, spriteUtils_1.buildMSXDirectionalSpriteCatalog)(sourceSprites);
