...
exports.renderScreenToCanvas = exports.deepCompareTiles = exports.generateSuperRLEData = exports.generateOptimizedRLEData = exports.generateBehaviorMapData = exports.generateBehaviorMapASMCode = exports.generateScreenLayoutASMCode = exports.generateScreenMapLayoutBytes = void 0;
...
const generateScreenMapLayoutBytes = (screenMap, tileset, tileBanks, currentScreenMode) => {
    ...
    const activeX = (_a = screenMap.activeAreaX) !== null && _a !== void 0 ? _a : 0;
    const activeY = (_b = screenMap.activeAreaY) !== null && _b !== void 0 ? _b : 0;
    const activeW = (_c = screenMap.activeAreaWidth) !== null && _c !== void 0 ? _c : screenMap.width;
    const activeH = (_d = screenMap.activeAreaHeight) !== null && _d !== void 0 ? _d : screenMap.height;
    const mapIndices = [];
    ...
    for (let r = 0; r < activeH; r++) {
        ...
        for (let c = 0; c < activeW; c++) {
            ...
            if (mapY >= backgroundLayer.length || mapX >= ((_e = backgroundLayer[mapY]) === null || _e === void 0 ? void 0 : _e.length)) {
            ...
            if (!screenTile || !screenTile.tileId) {
                ...
                if (currentScreenMode === "SCREEN 2 (Graphics I)" && tileBanks && tileAsset) {
                    ...
                    for (const bank of tileBanks) {
                        ...
                        if (((_f = bank.enabled) !== null && _f !== void 0 ? _f : true) && bank.assignedTiles[screenTile.tileId]) {
                    ...
                    const partKey = `${screenTile.tileId}_${(_g = screenTile.subTileX) !== null && _g !== void 0 ? _g : 0}_${(_h = screenTile.subTileY) !== null && _h !== void 0 ? _h : 0}`;
...
const generateBehaviorMapData = (screenMapData, tileset) => {
    ...
    for (let r = 0; r < ((_a = screenMapData.activeAreaHeight) !== null && _a !== void 0 ? _a : screenMapData.height); r++) {
        const mapY = ((_b = screenMapData.activeAreaY) !== null && _b !== void 0 ? _b : 0) + r;
        for (let c = 0; c < ((_c = screenMapData.activeAreaWidth) !== null && _c !== void 0 ? _c : screenMapData.width); c++) {
            const mapX = ((_d = screenMapData.activeAreaX) !== null && _d !== void 0 ? _d : 0) + c;
            const screenTile = (_e = activeCollisionLayer[mapY]) === null || _e === void 0 ? void 0 : _e[mapX];
            if (screenTile === null || screenTile === void 0 ? void 0 : screenTile.tileId) {
                const tileAsset = tileset.find(t => t.id === screenTile.tileId);
                behaviorMapData.push((_g = (_f = tileAsset === null || tileAsset === void 0 ? void 0 : tileAsset.logicalProperties) === null || _f === void 0 ? void 0 : _f.mapId) !== null && _g !== void 0 ? _g : 0);
            }
            else {
...
const generateSuperRLEData = (backgroundLayer, tileset, baseTileDim, tileBanks, compressionType = 'superRLE') => {
    ...
    const mapWidth = ((_a = backgroundLayer[0]) === null || _a === void 0 ? void 0 : _a.length) || 0;
    ...
    const getByteForTilePart = (tileId, subTileX, subTileY) => {
        ...
        if (tileBanks && tileAsset && baseTileDim === constants_1.EDITOR_BASE_TILE_DIM_S2) {
            let foundInBank = false;
            for (const bank of tileBanks) {
                const isBankEffectivelyEnabled = (_a = bank.enabled) !== null && _a !== void 0 ? _a : true;
                if (isBankEffectivelyEnabled && bank.assignedTiles[tileId]) {
                    ...
                    if (calculatedCharCode >= bank.charsetRangeStart && calculatedCharCode <= bank.charsetRangeEnd) {
                        ...
                        if (!tilePartReferencesList.some(ref => ref.byteValue === charCodeForCell)) {
                            tilePartReferencesList.push({ byteValue: charCodeForCell, tileId, subTileX: sX, subTileY: sY, name: (tileAsset === null || tileAsset === void 0 ? void 0 : tileAsset.name) || 'Unknown Tile (S2)' });
                        }
                        break;
                    }
                }
            ...
            const partKey = `${tileId}_${subTileX !== null && subTileX !== void 0 ? subTileX : 0}_${subTileY !== null && subTileY !== void 0 ? subTileY : 0}`;
            if (!tilePartToByteMap.has(partKey)) {
                ...
                tilePartReferencesList.push({ byteValue: charCodeForCell, tileId, subTileX, subTileY, name: (tileAsset === null || tileAsset === void 0 ? void 0 : tileAsset.name) || 'Unknown Tile' });
                nextByteValueForRefs = valueToAssign + 1;
            }
            else {
...
function createTileDataURL(fullTileAsset, subTileXCoord, subTileYCoord, outputCellPixelWidth, outputCellPixelHeight, baseSliceDim, currentScreenMode) {
    ...
    const sX = (subTileXCoord !== null && subTileXCoord !== void 0 ? subTileXCoord : 0) * baseSliceDim;
    const sY = (subTileYCoord !== null && subTileYCoord !== void 0 ? subTileYCoord : 0) * baseSliceDim;
...
function createSpriteDataURL(spriteFrameData, spriteWidth, spriteHeight) {
    ...
    for (let y = 0; y < spriteHeight; y++) {
        for (let x = 0; x < spriteWidth; x++) {
            const color = (_a = spriteFrameData[y]) === null || _a === void 0 ? void 0 : _a[x];
...
const renderScreenToCanvas = (canvas, screenMap, tileset, currentScreenMode, baseSliceDim, tileBanks, allAssets) => {
    ...
    for (let y = 0; y < screenMap.height; y++) {
        for (let x = 0; x < screenMap.width; x++) {
            const screenTile = (_a = layer[y]) === null || _a === void 0 ? void 0 : _a[x];
            if (!(screenTile === null || screenTile === void 0 ? void 0 : screenTile.tileId))
                continue;
            const tileAsset = tileset.find(t => t.id === screenTile.tileId);
            if (!tileAsset)
            ...
            if (!fullPixelData)
                continue;
            const subTileXCoord = (_b = screenTile.subTileX) !== null && _b !== void 0 ? _b : 0;
            const subTileYCoord = (_c = screenTile.subTileY) !== null && _c !== void 0 ? _c : 0;
            ...
            for (let py = 0; py < baseSliceDim; py++) {
                for (let px = 0; px < baseSliceDim; px++) {
                    ...
                    if (fullDataY < fullAssetHeight && fullDataX < fullAssetWidth) {
                        let color = (_d = fullPixelData[fullDataY]) === null || _d === void 0 ? void 0 : _d[fullDataX];
                        if (color === undefined)
                            continue;
