            ...
            if (width === 16 && height === 16) {
                ...
                for (let y = 0; y < 8; y++) {
                    ...
                    for (let bit = 0; bit < 8; bit++) {
                        const pixelColorValue = (_a = frame.data[y]) === null || _a === void 0 ? void 0 : _a[bit];
                ...
                for (let y = 8; y < 16; y++) {
                    let byteValue = 0;
                    for (let bit = 0; bit < 8; bit++) {
                        const pixelColorValue = (_b = frame.data[y]) === null || _b === void 0 ? void 0 : _b[bit];
                        if (pixelColorValue === layerColor) {
                            byteValue |= (1 << (7 - bit));
                            colorUsedInFrameLayer = true;
                        }
                ...
                for (let y = 0; y < 8; y++) {
                    let byteValue = 0;
                    for (let bit = 0; bit < 8; bit++) {
                        const pixelColorValue = (_c = frame.data[y]) === null || _c === void 0 ? void 0 : _c[8 + bit];
                        if (pixelColorValue === layerColor) {
                            byteValue |= (1 << (7 - bit));
                            colorUsedInFrameLayer = true;
                        }
                ...
                for (let y = 8; y < 16; y++) {
                    let byteValue = 0;
                    for (let bit = 0; bit < 8; bit++) {
                        const pixelColorValue = (_d = frame.data[y]) === null || _d === void 0 ? void 0 : _d[8 + bit];
                        if (pixelColorValue === layerColor) {
                            byteValue |= (1 << (7 - bit));
                            colorUsedInFrameLayer = true;
                        }
                ...
                for (let y = 0; y < height; y++) {
                    for (let xByte = 0; xByte < Math.ceil(width / 8); xByte++) {
                        ...
                        for (let bit = 0; bit < 8; bit++) {
                            ...
                            if (px < width) {
                                const pixelColorValue = (_e = frame.data[y]) === null || _e === void 0 ? void 0 : _e[px];
                                if (pixelColorValue === layerColor) {
                                    byteValue |= (1 << (7 - bit));
                                    colorUsedInFrameLayer = true;
                                }
...
const createAutoDirectionalSprite = (source, targetName, targetDirection, transform) => {
    return {
        ...source,
...
const buildMSXDirectionalSpriteCatalog = (sourceSprites) => {
    ...
    directionalFamilies.forEach((family, baseName) => {
        const addGeneratedVariant = (targetDirection, sourceIndex, transform, transformName) => {
            ...
            const generatedSprite = createAutoDirectionalSprite(sourceEntry.sprite, targetName, targetDirection, transform);
        ...
        if (family.up !== undefined && family.down === undefined) {
            addGeneratedVariant('down', family.up, exports.mirrorPixelDataVertically, 'vertical mirror');
        }
        else if (family.down !== undefined && family.up === undefined) {
            addGeneratedVariant('up', family.down, exports.mirrorPixelDataVertically, 'vertical mirror');
        }
    });
    const nameToIndex = {};
        ...
        if (spriteWidth === 16 && spriteHeight === 16) {
            ...
            for (let y = 0; y < 8; y++) {
                ...
                for (let bit = 0; bit < 8; bit++) {
                    ...
                    const pixelColorValue = (_a = frameData[y]) === null || _a === void 0 ? void 0 : _a[px];
                    if (pixelColorValue === layerColor) {
                        byteValue |= (1 << (7 - bit));
                    }
                }
            ...
            for (let y = 8; y < 16; y++) {
                ...
                for (let bit = 0; bit < 8; bit++) {
                    const px = bit; // Left column (pixels 0-7)
                    const pixelColorValue = (_b = frameData[y]) === null || _b === void 0 ? void 0 : _b[px];
                    if (pixelColorValue === layerColor) {
                        byteValue |= (1 << (7 - bit));
                    }
                }
            ...
            for (let y = 0; y < 8; y++) {
                ...
                for (let bit = 0; bit < 8; bit++) {
                    ...
                    const pixelColorValue = (_c = frameData[y]) === null || _c === void 0 ? void 0 : _c[px];
                    if (pixelColorValue === layerColor) {
                        byteValue |= (1 << (7 - bit));
                    }
                }
            ...
            for (let y = 8; y < 16; y++) {
                ...
                for (let bit = 0; bit < 8; bit++) {
                    const px = 8 + bit; // Right column (pixels 8-15)
                    const pixelColorValue = (_d = frameData[y]) === null || _d === void 0 ? void 0 : _d[px];
                    if (pixelColorValue === layerColor) {
                        byteValue |= (1 << (7 - bit));
                    }
                }
            ...
            for (let y = 0; y < spriteHeight; y++) {
                for (let xByte = 0; xByte < Math.ceil(spriteWidth / 8); xByte++) {
                    ...
                    for (let bit = 0; bit < 8; bit++) {
                        const px = xByte * 8 + bit;
                        if (px < spriteWidth) {
                            const pixelColorValue = (_e = frameData[y]) === null || _e === void 0 ? void 0 : _e[px];
                            if (pixelColorValue === layerColor) {
                                byteValue |= (1 << (7 - bit));
                            }
                        }
...
const generateSpriteASMCode = (sprite, dataFormat = 'hex', uniqueIndex) => {
    ...
    const usedLayerIndexes = sprite.spritePalette
        ...
        if (!layerColor || layerColor === sprite.backgroundColor)
            return false;
        return sprite.frames.some((frame) => { var _a; return (_a = frame === null || frame === void 0 ? void 0 : frame.data) === null || _a === void 0 ? void 0 : _a.some((row) => row === null || row === void 0 ? void 0 : row.some((pixel) => pixel === layerColor)); });
    });
