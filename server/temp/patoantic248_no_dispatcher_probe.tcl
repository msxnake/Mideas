set log_path "C:/Users/salam/Documents/Programacion/Mideas/server/temp/patoantic248_no_dispatcher_probe.log"
...
proc regs_line {tag} {
    ...
    set enabled [mem8 0xEA82]
    set task0lo [mem8 0xEA72]
    set task0hi [mem8 0xEA73]
    set irqlo [mem8 0xEA88]
    set irqhi [mem8 0xEA89]
    logline [format "%s pc=%04X sp=%04X p=%02X/%02X/%02X flow=%02X exit=%02X lives=%d time=%02X%02X enabled=%02X task0=%02X%02X irq=%02X%02X" $tag $pc $sp $p1 $p2 $p3 $flow $exit $lives $timehi $timelo $enabled $task0hi $task0lo $irqhi $irqlo]
}
proc tap_space {tag} {
...
proc disable_dispatcher {} {
    poke8 0xEA82 0
    regs_line "dispatcher_disabled"
}
debug set_bp 0x0000 {} {
    regs_line "BP_0000"
    debug cont
