; ==================================================================
; SAILOR_SPINACH_MARCH - UNIFIED FILE
...
; BANK 02 @#0000 : sound.asm part 3/2 (2436 bytes)
; BANK 02 @#0984 : scroll.asm (2407 bytes)
; BANK 02 @#12EB : animtiles.asm (2549 bytes)
; BANK 02 @#1CE0 : bosses.asm (800 bytes)
; BANK 03 @#0000 : bosses.asm (4974 bytes)
; BANK 03 @#136E : statemachine.asm (5 bytes)
; BANK 03 @#1373 : gameflow.asm (3213 bytes)
; BANK 04 @#0000 : gameflow.asm (1700 bytes)
...
; Flow: Start → Music (gf_sailor_music)
; ==================================================================
...
sprite_asset_base_pattern_slot_runtime EQU #DE5A   ; Runtime base 16x16 slot per sprite asset (1 bytes)
sprite_placeholder_base_pattern_num EQU #DE5B   ; Runtime placeholder pattern number (base slot * 4)
current_sprite_pattern_pack_id EQU #DE5C   ; Active runtime sprite pattern pack id (#FF=none loaded)
sprite_attributes   EQU #DE5D   ; Interleaved sprite attributes (32 * 4 bytes)

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #DEDD   ; Player X position (16-bit)
player_y            EQU #DEDF   ; Player Y position (16-bit)
player_runtime_enabled EQU #DEE1   ; 1=player fast runtime bound to hero entity
player_entity_index EQU #DEE2   ; Entity index used by player fast runtime (#FF=none)
player_vx_runtime   EQU #DEE3   ; Cached player X velocity (signed 8-bit)
player_vy_runtime   EQU #DEE4   ; Cached player Y velocity (signed 8-bit)
player_dash_timer   EQU #DEE5   ; Frames remaining in current Player dash
player_dash_cooldown EQU #DEE6   ; Frames until Player can dash again
player_dash_dir     EQU #DEE7   ; Player dash direction (1=left,2=right,3=up,4=down)
player_dash_tile_x  EQU #DEE8   ; Dash front probe tile X scratch
player_dash_tile_y  EQU #DEE9   ; Dash front probe tile Y scratch
player_health       EQU #DEEA   ; Player health points
player_score        EQU #DEEB   ; Player score (16-bit)
gem_count           EQU #DEED   ; Collectible tile counter (8-bit)
last_interaction_char EQU #DEEE   ; Char code of last interacted tile (for SM VARIABLE_COMPARE)
last_gem_char       EQU last_interaction_char   ; Backwards-compatible alias for collectible SM checks
last_interaction_pending EQU #DEEF   ; 1 when a new tile interaction is pending for State Machine logic
last_interaction_type EQU #DEF0   ; Interaction type id of last interacted tile
last_interaction_value EQU #DEF1   ; Interaction value byte of last interacted tile
last_interaction_target EQU #DEF2   ; Interaction target id of last interacted tile
last_interaction_x  EQU #DEF3   ; Tile X coordinate of last interaction
last_interaction_y  EQU #DEF4   ; Tile Y coordinate of last interaction
last_interaction_entity EQU #DEF5   ; Entity index that triggered the last interaction

; Persistent collectibles list (survives screen re-entry)
MAX_COLLECTIBLES     EQU 64              ; Max persistent collectible records
collected_count      EQU #DEF6   ; Number of collected tiles recorded (8-bit)
collected_world      EQU #DEF7   ; World IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_screen     EQU #DF37   ; Screen IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_idx_l      EQU #DF77   ; Tile name-table index low byte (MAX_COLLECTIBLES bytes)
collected_idx_h      EQU #DFB7   ; Tile name-table index high byte (MAX_COLLECTIBLES bytes)

; Timed bonus tile respawn slots (bonus gem regeneration)
MAX_BONUS_RESPAWNS   EQU 16              ; Max timed bonus tiles waiting to respawn
bonus_respawn_world  EQU #DFF7   ; World IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_screen EQU #E007   ; Screen IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_l  EQU #E017   ; Tile index low byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_h  EQU #E027   ; Tile index high byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_secs   EQU #E037   ; Remaining seconds per timed respawn slot (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_frames EQU #E047   ; Frame countdown (60..1) per timed respawn slot (MAX_BONUS_RESPAWNS bytes)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #E057   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #E058   ; Temporary 16-bit storage
temp_word_2         EQU #E05A   ; Temporary 16-bit storage
temp_byte_1         EQU #E05C   ; Temporary 8-bit storage
temp_byte_2         EQU #E05D   ; Temporary 8-bit storage
temp_byte_3         EQU #E05E   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #E07E   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #E09E   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #E0BE   ; Temporary 8-bit storage (32 bytes)

; ==================================================================
; SOUND SYSTEM VARIABLES
; ==================================================================
sfx_active          EQU #E0DE   ; 0=no SFX active, 1=playing
sfx_timer           EQU #E0DF   ; Frames remaining for current SFX
sfx_fadeout         EQU #E0E0   ; Reserved fadeout flag/state
temp_byte_7         EQU #E0E1   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #E101   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #E121   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #E141   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #E161   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #E181   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #E1A1   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #E1C1   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #E1E1   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #E201   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #E221   ; Temporary 8-bit storage (32 bytes)
temp_byte_18        EQU #E241   ; Temporary 8-bit storage (32 bytes)
temp_byte_19        EQU #E261   ; Temporary 8-bit storage (32 bytes)
temp_byte_20        EQU #E281   ; Temporary 8-bit storage (32 bytes)
temp_byte_21        EQU #E2A1   ; Temporary 8-bit storage (32 bytes)
temp_byte_22        EQU #E2C1   ; Temporary 8-bit storage (32 bytes)
temp_byte_23        EQU #E2E1   ; Temporary 8-bit storage (32 bytes)
temp_byte_24        EQU #E301   ; Temporary 8-bit storage (32 bytes)
temp_byte_25        EQU #E321   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #E341   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #E381   ; Temporary 16-bit storage (64 bytes)
temp_byte_26        EQU #E3C1   ; Temporary 8-bit storage (32 bytes)
temp_byte_27        EQU #E3E1   ; Temporary 8-bit storage (32 bytes)
temp_byte_28        EQU #E401   ; Temporary 8-bit storage (32 bytes)
temp_byte_29        EQU #E421   ; Temporary 8-bit storage (32 bytes)
temp_byte_30        EQU #E441   ; Temporary 8-bit storage (32 bytes)
temp_byte_31        EQU #E461   ; Temporary 8-bit storage (32 bytes)
tileDead_dbg        EQU #E481   ; Debug byte: current hero deadly contact
tileDead_latched_dbg EQU #E482   ; Debug byte: latched hero deadly contact
tileDead_x_dbg      EQU #E483   ; Debug byte: last sampled deadly tile X
tileDead_y_dbg      EQU #E484   ; Debug byte: last sampled deadly tile Y
tileDead_value_dbg  EQU #E485   ; Debug byte: last raw deadly behavior value

; Wall collision temporary variables
wall_temp_x         EQU #E486   ; Cached entity X for wall checks
wall_temp_y         EQU #E487   ; Cached entity Y for wall checks
wall_hit_left       EQU #E488   ; Hitbox left edge cache
wall_hit_top        EQU #E489   ; Hitbox top edge cache
wall_hit_right      EQU #E48A   ; Hitbox right edge cache
wall_hit_bottom     EQU #E48B   ; Hitbox bottom edge cache
wall_hit_w          EQU #E48C   ; Hitbox width cache (min 1)
wall_hit_h          EQU #E48D   ; Hitbox height cache (min 1)
wall_probe_left     EQU #E48E   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #E48F   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #E490   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #E491   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #E492   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #E4B2   ; Number of entries in active_entity_list
hero_entity_id      EQU #E4B3   ; First current-screen entity flagged as player (#FF = none)
active_entity_list_dirty EQU #E4B4   ; 1=rebuild active_entity_list required
input_entity_list   EQU #E4B5   ; Active current-screen entities with Input component (MAX_ENTITIES bytes)
input_entity_count  EQU #E4D5   ; Number of entries in input_entity_list
render_entity_list  EQU #E4D6   ; Active current-screen entities with Sprite component (MAX_ENTITIES bytes)
render_entity_count EQU #E4F6   ; Number of entries in render_entity_list
collision_entity_list EQU #E4F7   ; Active current-screen entities with Collision component (MAX_ENTITIES bytes)
collision_entity_count EQU #E517   ; Number of entries in collision_entity_list
ground_entity_list  EQU #E518   ; Active current-screen entities with Collision or Gravity (MAX_ENTITIES bytes)
ground_entity_count EQU #E538   ; Number of entries in ground_entity_list
anim_entity_list    EQU #E539   ; Active current-screen entities with Animation+Sprite (MAX_ENTITIES bytes)
anim_entity_count   EQU #E559   ; Number of entries in anim_entity_list

; Entity-entity collision optimized variables
coll_list           EQU #E55A   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #E57A   ; Number of entities in coll_list
coll_src_left       EQU #E57B   ; Source AABB left edge (scratch)
coll_src_right      EQU #E57C   ; Source AABB right edge (scratch)
coll_src_top        EQU #E57D   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #E57E   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #E57F   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #E57F   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #E581   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #E583   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #E585   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #E587   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #E589   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #E58B   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #E58D   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #E58F   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #E590   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #E595   ; Frame counter (16-bit)
task_exec_time          EQU #E597   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #E599   ; Set to 1 on each VBlank (1 byte)
interrupt_in_progress   EQU #E59A   ; 1 while the H.TIMI dispatcher is running
RAM_INTERRUPT_END       EQU #E59B   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #E59B   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #E59C   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #E59D   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #E59E   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #E59F   ; 0=stopped, 1=track active
music_muted          EQU #E5A0   ; 0=audible, 1=muted/pause
music_loop           EQU #E5A1   ; 0=no loop, 1=loop enabled
music_track_index    EQU #E5A2   ; Current ROM track index
music_row_frames     EQU #E5A3   ; Frames per tracker row
music_row_countdown  EQU #E5A4   ; Countdown to next row
music_order_pos      EQU #E5A5   ; Current order position
music_pattern_index  EQU #E5A6   ; Current pattern index
music_pattern_row    EQU #E5A7   ; Current row inside pattern
music_pattern_rows   EQU #E5A8   ; Cached rows in current pattern
music_track_ptr_l    EQU #E5A9   ; Current track pointer low byte
music_track_ptr_h    EQU #E5AA   ; Current track pointer high byte
music_pattern_ptr_l  EQU #E5AB   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #E5AC   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #E5AD   ; PSG mixer shadow for music runtime
music_pitch_note_work EQU #E5AE   ; Scratch note index while resolving tone/ornament macros
music_pitch_step_work EQU #E5AF   ; Scratch macro step while resolving tone/ornament macros
music_pitch_len_work  EQU #E5B0   ; Scratch macro length while resolving tone/ornament macros
music_ch_note_base EQU #E5B1   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #E5B1   ; Channel A
music_ch_b_note EQU #E5B2   ; Channel B
music_ch_c_note EQU #E5B3   ; Channel C
music_ch_instrument_base EQU #E5B4   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #E5B4   ; Channel A
music_ch_b_instrument EQU #E5B5   ; Channel B
music_ch_c_instrument EQU #E5B6   ; Channel C
music_ch_ornament_base EQU #E5B7   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #E5B7   ; Channel A
music_ch_b_ornament EQU #E5B8   ; Channel B
music_ch_c_ornament EQU #E5B9   ; Channel C
music_ch_volume_base EQU #E5BA   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #E5BA   ; Channel A
music_ch_b_volume EQU #E5BB   ; Channel B
music_ch_c_volume EQU #E5BC   ; Channel C
music_ch_vol_step_base EQU #E5BD   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #E5BD   ; Channel A
music_ch_b_vol_step EQU #E5BE   ; Channel B
music_ch_c_vol_step EQU #E5BF   ; Channel C
music_ch_tone_step_base EQU #E5C0   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #E5C0   ; Channel A
music_ch_b_tone_step EQU #E5C1   ; Channel B
music_ch_c_tone_step EQU #E5C2   ; Channel C
music_ch_noise_step_base EQU #E5C3   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #E5C3   ; Channel A
music_ch_b_noise_step EQU #E5C4   ; Channel B
music_ch_c_noise_step EQU #E5C5   ; Channel C
music_ch_orn_step_base EQU #E5C6   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #E5C6   ; Channel A
music_ch_b_orn_step EQU #E5C7   ; Channel B
music_ch_c_orn_step EQU #E5C8   ; Channel C
music_ch_hw_env_step_base EQU #E5C9   ; Software hardware-envelope divider step (3 bytes)
music_ch_a_hw_env_step EQU #E5C9   ; Channel A
music_ch_b_hw_env_step EQU #E5CA   ; Channel B
music_ch_c_hw_env_step EQU #E5CB   ; Channel C
...
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #E5CC   ; End of project variables (9676 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#E5CC: Project variables (9676 bytes)
;   #E5CC-#E600: Alignment padding/free RAM (52 bytes)
...
music_track_ptr_table:
    DW music_track_0_Sailor_Spinach_March_data

; ------------------------------------------------------------------
; Tracker Song 0: Sailor Spinach March
; ------------------------------------------------------------------
music_track_0_Sailor_Spinach_March_data:
    DB #04
    DB #02
    DB #00
    DB #01
    DB #02
    DW music_track_0_Sailor_Spinach_March_order_table
    DW music_track_0_Sailor_Spinach_March_pattern_table
    DW music_track_0_Sailor_Spinach_March_instrument_ptr_table
    DW music_track_0_Sailor_Spinach_March_ornament_ptr_table
    DW #00D2
    DB #0C

music_track_0_Sailor_Spinach_March_order_table:
    DB #00,#01

music_track_0_Sailor_Spinach_March_pattern_table:
    DW music_track_0_Sailor_Spinach_March_pattern_0_rows
    DB #40
    DW music_track_0_Sailor_Spinach_March_pattern_1_rows
    DB #40

music_track_0_Sailor_Spinach_March_instrument_ptr_table:
    DW 0
    DW music_track_0_Sailor_Spinach_March_inst_1
    DW music_track_0_Sailor_Spinach_March_inst_2
    DW 0
    DW 0
    DW music_track_0_Sailor_Spinach_March_inst_5
    DW music_track_0_Sailor_Spinach_March_inst_6
    DW music_track_0_Sailor_Spinach_March_inst_7
    DW 0
    DW 0
    DW 0
    DW 0
...
music_track_0_Sailor_Spinach_March_ornament_ptr_table:
    DW 0
    DW 0
    DW 0
    DW 0
...
music_track_0_Sailor_Spinach_March_pattern_0_rows:
    DB #3C,#01,#FF,#0F,#18,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #40,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #43,#01,#FF,#0F,#1F,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #40,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #39,#01,#FF,#0F,#24,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #40,#01,#FF,#0F,#1F,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #37,#01,#FF,#0F,#1D,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3B,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3E,#01,#FF,#0F,#24,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3B,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#1D,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #37,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #34,#01,#FF,#0F,#24,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #37,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #35,#01,#FF,#0F,#1F,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #39,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#26,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #39,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3E,#01,#FF,#0F,#1F,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3B,#01,#FF,#0F,#26,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #37,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #40,#01,#FF,#0F,#18,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3E,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#1F,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #39,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #37,#01,#FF,#0F,#24,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3B,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#1F,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF

music_track_0_Sailor_Spinach_March_pattern_1_rows:
    DB #40,#01,#FF,#0F,#18,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #43,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #40,#01,#FF,#0F,#1C,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3E,#01,#FF,#0F,#1F,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #41,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3E,#01,#FF,#0F,#24,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3B,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#1F,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #40,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #43,#01,#FF,#0F,#26,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #40,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #45,#01,#FF,#0F,#1F,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #43,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #40,#01,#FF,#0F,#23,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #41,#01,#FF,#0F,#21,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #40,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3E,#01,#FF,#0F,#28,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3B,#01,#FF,#0F,#21,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3E,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #43,#01,#FF,#0F,#28,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3E,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#1D,#02,#FF,#0E,#18,#05,#FF,#0F
    ...
    DB #39,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #35,#01,#FF,#0F,#24,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #32,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #30,#01,#FF,#0F,#1F,#02,#FF,#0E,#18,#05,#FF,#0F
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #34,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #37,#01,#FF,#0F,#24,#02,#FF,#0E,#24,#06,#FF,#0E
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #3C,#01,#FF,#0F,#FF,#FF,#FF,#FF,#30,#07,#FF,#09
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF

music_track_0_Sailor_Spinach_March_inst_1:
    DB #01
    DB #0F
    DB #00
    DB #0C
    DW #00D2
    DW music_track_0_Sailor_Spinach_March_inst_1_vol_env
    DB #07
    DB #FF
    DW music_track_0_Sailor_Spinach_March_inst_1_tone_env
    DB #01
    DB #FF
    DW 0
    DB #00
    DB #FF
music_track_0_Sailor_Spinach_March_inst_1_vol_env:
    DB #0F,#0D,#0A,#07,#04,#02,#00
music_track_0_Sailor_Spinach_March_inst_1_tone_env:
    DB #00

music_track_0_Sailor_Spinach_March_inst_2:
    DB #01
    DB #0F
    DB #00
    DB #0C
    DW #00D2
    DW music_track_0_Sailor_Spinach_March_inst_2_vol_env
    DB #07
    DB #FF
    DW music_track_0_Sailor_Spinach_March_inst_2_tone_env
    DB #03
    DB #FF
    DW 0
    DB #00
    DB #FF
music_track_0_Sailor_Spinach_March_inst_2_vol_env:
    DB #0F,#0E,#0C,#09,#06,#03,#00
music_track_0_Sailor_Spinach_March_inst_2_tone_env:
    DB #00,#FF,#00

music_track_0_Sailor_Spinach_March_inst_5:
    DB #03
    DB #0F
    DB #00
    DB #02
    DW #00D2
    DW music_track_0_Sailor_Spinach_March_inst_5_vol_env
    DB #06
    DB #FF
    DW music_track_0_Sailor_Spinach_March_inst_5_tone_env
    DB #04
    DB #FF
    DW music_track_0_Sailor_Spinach_March_inst_5_noise_env
    DB #04
    DB #FF
music_track_0_Sailor_Spinach_March_inst_5_vol_env:
    DB #0F,#0D,#09,#05,#01,#00
music_track_0_Sailor_Spinach_March_inst_5_tone_env:
    DB #E4,#F0,#F9,#00
music_track_0_Sailor_Spinach_March_inst_5_noise_env:
    DB #02,#04,#08,#0E

music_track_0_Sailor_Spinach_March_inst_6:
    DB #02
    DB #0E
    DB #00
    DB #08
    DW #00D2
    DW music_track_0_Sailor_Spinach_March_inst_6_vol_env
    DB #06
    DB #FF
    DW music_track_0_Sailor_Spinach_March_inst_6_tone_env
    DB #01
    DB #FF
    DW music_track_0_Sailor_Spinach_March_inst_6_noise_env
    DB #04
    DB #FF
music_track_0_Sailor_Spinach_March_inst_6_vol_env:
    DB #0E,#0B,#08,#04,#01,#00
music_track_0_Sailor_Spinach_March_inst_6_tone_env:
    DB #00
music_track_0_Sailor_Spinach_March_inst_6_noise_env:
    DB #08,#0C,#12,#18

music_track_0_Sailor_Spinach_March_inst_7:
    DB #02
    DB #09
    DB #00
    DB #02
    DW #00D2
    DW music_track_0_Sailor_Spinach_March_inst_7_vol_env
    DB #05
    DB #FF
    DW music_track_0_Sailor_Spinach_March_inst_7_tone_env
    DB #01
    DB #FF
    DW music_track_0_Sailor_Spinach_March_inst_7_noise_env
    DB #04
    DB #FF
music_track_0_Sailor_Spinach_March_inst_7_vol_env:
    DB #09,#07,#04,#01,#00
music_track_0_Sailor_Spinach_March_inst_7_tone_env:
    DB #00
music_track_0_Sailor_Spinach_March_inst_7_noise_env:
    DB #02,#03,#04,#06
...
gameflow_start:
    ; Load the Start node
    ld hl, gameflow_node_gf_sailor_start
...
; Node: Start - "gf_sailor_start"
gameflow_node_gf_sailor_start:
    db NODE_TYPE_START
    dw gameflow_node_gf_sailor_start_data
    dw gameflow_node_gf_sailor_start_conn

gameflow_node_gf_sailor_start_data:
    dw gameflow_node_gf_sailor_start_init    ; Initialization routine address
    db ((gameflow_node_gf_sailor_start_init - #4000) / #2000)    ; Initialization routine bank

gameflow_node_gf_sailor_start_conn:
    db CONNECTION_DEFAULT
    dw gameflow_node_gf_sailor_music
    db CONNECTION_END

; ------------------------------------------------------------------
; gameflow_node_gf_sailor_start_init
...
; Node: Music - "gf_sailor_music"
gameflow_node_gf_sailor_music:
    db NODE_TYPE_MUSIC
    dw gameflow_node_gf_sailor_music_data
    dw gameflow_node_gf_sailor_music_conn

gameflow_node_gf_sailor_music_data:
    db 1, 0, 1    ; command, track index, loop flag

gameflow_node_gf_sailor_music_conn:
    db CONNECTION_DEFAULT
    dw gameflow_node_gf_sailor_end
    db CONNECTION_END

; Node: End - "gf_sailor_end"
gameflow_node_gf_sailor_end:
    db NODE_TYPE_END
    dw gameflow_no_data
    dw gameflow_node_gf_sailor_end_conn

gameflow_node_gf_sailor_end_conn:
    db CONNECTION_DEFAULT
    dw 0
    db CONNECTION_END
