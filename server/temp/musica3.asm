...
; BANK 00 @#0BD9 : sound.asm part 1/2 (5159 bytes)
; BANK 01 @#0000 : sound.asm part 2/2 (6114 bytes)
; BANK 01 @#17E2 : scroll.asm (2078 bytes)
; BANK 02 @#0000 : scroll.asm (264 bytes)
; BANK 02 @#0108 : animtiles.asm (3835 bytes)
; BANK 02 @#1003 : particles.asm (4093 bytes)
; BANK 03 @#0000 : particles.asm (870 bytes)
; BANK 03 @#0366 : statemachine.asm (5 bytes)
; BANK 03 @#036B : gameflow.asm (4903 bytes)
...
; ==================================================================
music_active         EQU #D106   ; 0=stopped, 1=track active
music_muted          EQU #D107   ; 0=audible, 1=muted/pause
music_loop           EQU #D108   ; 0=no loop, 1=loop enabled
music_track_index    EQU #D109   ; Current ROM track index
music_row_frames     EQU #D10A   ; Frames per tracker row
music_row_countdown  EQU #D10B   ; Countdown to next row
music_order_pos      EQU #D10C   ; Current order position
music_pattern_index  EQU #D10D   ; Current pattern index
music_pattern_row    EQU #D10E   ; Current row inside pattern
music_pattern_rows   EQU #D10F   ; Cached rows in current pattern
music_track_ptr_l    EQU #D110   ; Current track pointer low byte
music_track_ptr_h    EQU #D111   ; Current track pointer high byte
music_pattern_ptr_l  EQU #D112   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #D113   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #D114   ; PSG mixer shadow for music runtime
music_ch_note_base EQU #D115   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #D115   ; Channel A
music_ch_b_note EQU #D116   ; Channel B
music_ch_c_note EQU #D117   ; Channel C
music_ch_instrument_base EQU #D118   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #D118   ; Channel A
music_ch_b_instrument EQU #D119   ; Channel B
music_ch_c_instrument EQU #D11A   ; Channel C
music_ch_ornament_base EQU #D11B   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #D11B   ; Channel A
music_ch_b_ornament EQU #D11C   ; Channel B
music_ch_c_ornament EQU #D11D   ; Channel C
music_ch_volume_base EQU #D11E   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #D11E   ; Channel A
music_ch_b_volume EQU #D11F   ; Channel B
music_ch_c_volume EQU #D120   ; Channel C
music_ch_vol_step_base EQU #D121   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #D121   ; Channel A
music_ch_b_vol_step EQU #D122   ; Channel B
music_ch_c_vol_step EQU #D123   ; Channel C
music_ch_tone_step_base EQU #D124   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #D124   ; Channel A
music_ch_b_tone_step EQU #D125   ; Channel B
music_ch_c_tone_step EQU #D126   ; Channel C
music_ch_noise_step_base EQU #D127   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #D127   ; Channel A
music_ch_b_noise_step EQU #D128   ; Channel B
music_ch_c_noise_step EQU #D129   ; Channel C
music_ch_orn_step_base EQU #D12A   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #D12A   ; Channel A
music_ch_b_orn_step EQU #D12B   ; Channel B
music_ch_c_orn_step EQU #D12C   ; Channel C

; ==================================================================
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #D12D   ; End of project variables (4397 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#D12D: Project variables (4397 bytes)
;   #D12D-#F37F: Free RAM (~8787 bytes available)
...
music_track_0_New_Track_pattern_0_rows:
    ...
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #26,#03,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #25,#03,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
