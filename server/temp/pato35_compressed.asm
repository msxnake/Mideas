...
; Estimated payload bytes: 98449
; Estimated banks used: 13
; ------------------------------------------------------------------
; BANK 00 @#0000 : patterns.asm (865 bytes)
; BANK 00 @#0361 : colors.asm (810 bytes)
; BANK 00 @#068B : components.asm (22 bytes)
; BANK 00 @#06A1 : entities.asm (3910 bytes)
; BANK 00 @#15E7 : worlds.asm (2233 bytes)
; BANK 00 @#1EA0 : screens.asm part 1/2 (352 bytes)
; BANK 01 @#0000 : screens.asm part 2/2 (8192 bytes)
; BANK 02 @#0000 : screens.asm part 3/2 (1645 bytes)
; BANK 02 @#066D : sprites.asm (4558 bytes)
; BANK 02 @#183B : font.asm (1989 bytes)
; BANK 03 @#0000 : font.asm (1558 bytes)
; BANK 03 @#0616 : hud.asm (4339 bytes)
; BANK 03 @#1709 : menus.asm (444 bytes)
; BANK 03 @#18C5 : sound.asm part 1/2 (1851 bytes)
; BANK 04 @#0000 : sound.asm part 2/2 (7550 bytes)
; BANK 04 @#1D7E : scroll.asm (642 bytes)
; BANK 05 @#0000 : scroll.asm (1711 bytes)
; BANK 05 @#06AF : animtiles.asm (4555 bytes)
; BANK 05 @#187A : particles.asm (1926 bytes)
; BANK 06 @#0000 : particles.asm (3037 bytes)
; BANK 06 @#0BDD : statemachine.asm part 1/4 (5155 bytes)
; BANK 07 @#0000 : statemachine.asm part 2/4 (8192 bytes)
; BANK 08 @#0000 : statemachine.asm part 3/4 (8192 bytes)
; BANK 09 @#0000 : statemachine.asm part 4/4 (7583 bytes)
; BANK 09 @#1D9F : gameflow.asm part 1/3 (609 bytes)
; BANK 10 @#0000 : gameflow.asm part 2/3 (8192 bytes)
; BANK 11 @#0000 : gameflow.asm part 3/3 (8192 bytes)
; BANK 12 @#0000 : gameflow.asm part 4/3 (145 bytes)
...
; SYSTEM VARIABLES
; ==================================================================
ROM_slot            EQU #C012   ; ROM slot number (for SETPAGES32K)
mapper_bank_p1_current EQU #C013   ; Mapper current bank for page/window 1
mapper_bank_p2_current EQU #C014   ; Mapper current bank for page/window 2
mapper_bank_p3_current EQU #C015   ; Mapper current bank for page/window 3
mapper_bank_p4_current EQU #C016   ; Mapper current bank for page/window 4
mapper_saved_bank    EQU #C017   ; Saved mapper bank for push/pop helpers
mapper_saved_bank_p1 EQU #C018   ; Saved mapper bank for page/window 1 helpers
mapper_saved_bank_p3 EQU #C019   ; Saved mapper bank for page/window 3 helpers
mapper_saved_bank_p4 EQU #C01A   ; Saved mapper bank for page/window 4 helpers
frame_counter       EQU #C01B   ; Frame counter (16-bit)

; ==================================================================
; SCREEN MAP POINTERS (Current active screen)
; ==================================================================
current_screen_layout   EQU #C01D   ; Pointer to current screen layout data (16-bit)
current_screen_layout_bank EQU #C01F   ; Mapper bank for current screen layout data
current_behavior_map    EQU #C020   ; Pointer to current behavior map data (16-bit)
current_behavior_map_bank EQU #C022   ; Mapper bank for current behavior map data
behavior_cache_row     EQU #C023   ; Cached behavior row (255=invalid)
behavior_cache_map_l   EQU #C024   ; Cached behavior map pointer low byte
behavior_cache_map_h   EQU #C025   ; Cached behavior map pointer high byte
behavior_cache_row_base EQU #C026   ; Cached row base address in behavior map (16-bit)
RUNTIME_SCREEN_MAP_SIZE EQU 768
runtime_screen_layout  EQU #C028   ; Mutable copy of current screen layout (32x24)
runtime_behavior_map   EQU #C328   ; Mutable copy of current behavior map (32x24)

; ==================================================================
; VIEWPORT/CAMERA VARIABLES (for scroll system)
; ==================================================================
camera_x            EQU #C628   ; Camera X position in pixels (16-bit)
camera_y            EQU #C62A   ; Camera Y position in pixels (16-bit)
camera_tile_x       EQU #C62C   ; Camera tile X (column)
camera_tile_y       EQU #C62D   ; Camera tile Y (row)
world_width_tiles   EQU #C62E   ; World width in tiles
world_height_tiles  EQU #C62F   ; World height in tiles
scroll_dirty_flag   EQU #C630   ; 1=viewport changed, needs redraw
hud_dirty_flag      EQU #C631   ; 1=HUD needs redraw, 0=clean

; ==================================================================
; ANIMATED TILES VARIABLES
; ==================================================================
anim_tile_timer     EQU #C632   ; Animation frame timer
anim_tile_frame     EQU #C633   ; Current animation frame (0-3)
anim_tile_speed     EQU #C634   ; Frames between animation updates
anim_tile_transform_flags EQU #C635   ; Runtime flags for transform-mode tile animation (byte0=flags, byte1=opcode scratch)
anim_tile_row_buffer EQU #C637   ; Temp buffer (8 bytes) for row transforms

; ==================================================================
; PARTICLE SYSTEM VARIABLES
; ==================================================================
particle_pool       EQU #C63F   ; Particle pool (8 particles * 8 bytes = 64 bytes)

; ==================================================================
; ENTITY SYSTEM VARIABLES (Fixed 32 entities)
; ==================================================================
MAX_ENTITIES        EQU 32
entity_active       EQU #C67F   ; Entity active flags (32 bytes, 0=inactive, 1=active)
entity_x_pos        EQU #C69F   ; Entity X positions (32 bytes)
entity_y_pos        EQU #C6BF   ; Entity Y positions (32 bytes)
entity_vel_x        EQU #C6DF   ; Entity X velocity (32 bytes)
entity_vel_y        EQU #C6FF   ; Entity Y velocity (32 bytes)
entity_comp_masks   EQU #C71F   ; Entity component masks (32 bytes)
entity_comp_masks_hi EQU #C73F   ; Entity component masks high byte (32 bytes)
entity_screen_id    EQU #C75F   ; Entity screen ID (32 bytes)
entity_job_period   EQU #C77F   ; Entity job period in frames (32 bytes, 1=100%,2=50%,3=33%,4=25%)
entity_job_entry    EQU #C79F   ; Entity job entry slot within period window (32 bytes)
entity_dir_mask     EQU #C7BF   ; Entity direction mask (32 bytes)
entity_input_speed  EQU #C7DF   ; Entity input/cursor speed (32 bytes)
entity_health       EQU #C7FF   ; Entity health (32 bytes)
entity_anim_frame   EQU #C81F   ; Entity animation frame (32 bytes)
entity_anim_tick    EQU #C83F   ; Entity animation tick counter (32 bytes)
entity_anim_speed   EQU #C85F   ; Entity animation speed (ticks per frame) (32 bytes)
entity_anim_flags   EQU #C87F   ; Entity animation flags (32 bytes)
entity_sm_ptr_l     EQU #C89F   ; Entity State Pointer Low (32 bytes)
entity_sm_ptr_h     EQU #C8BF   ; Entity State Pointer High (32 bytes)
entity_sm_timer_l   EQU #C8DF   ; Entity State Timer Low (32 bytes)
entity_sm_timer_h   EQU #C8FF   ; Entity State Timer High (32 bytes)
entity_sm_wait_timer EQU #C91F   ; Entity State Wait Timer (32 bytes)
entity_lifetime     EQU #C93F   ; Entity lifetime for auto-destroy (32 bytes, 0=infinite)
entity_carried_by   EQU #C95F   ; Entity carrier ID (32 bytes, 255=not carried)
entity_template_token EQU #C97F   ; Entity template token (32 bytes, 0=unknown)
entity_facing_dir   EQU #C99F   ; Last facing direction (32 bytes, 0=none,1=left,2=right,3=up,4=down)
entity_sm_var_0     EQU #C9BF   ; Entity Variable 0 (32 bytes)
entity_sm_var_1     EQU #C9DF   ; Entity Variable 1 (32 bytes)
entity_sm_var_2     EQU #C9FF   ; Entity Variable 2 (32 bytes)
entity_sm_var_3     EQU #CA1F   ; Entity Variable 3 (32 bytes)
entity_sm_var_4     EQU #CA3F   ; Entity Variable 4 (32 bytes)
entity_sm_var_5     EQU #CA5F   ; Entity Variable 5 (32 bytes)
entity_sm_var_6     EQU #CA7F   ; Entity Variable 6 (32 bytes)
entity_sm_var_7     EQU #CA9F   ; Entity Variable 7 (32 bytes)

; ==================================================================
; SPRITE SYSTEM VARIABLES
; ==================================================================
entity_sprite_asset_index EQU #CABF   ; Entity sprite asset index - RAM copy (32 bytes)
active_sprite_count EQU #CADF   ; Number of sprites currently active
sprites_dirty      EQU #CAE0   ; 1=sprite_attributes changed, needs VRAM sync
sprite_pattern      EQU #CAE1   ; Sprite pattern IDs (32 bytes)
sprite_color        EQU #CB01   ; Sprite colors (32 bytes)
sprite_layer_colors EQU #CB21   ; HW sprite layer color cache - RAM copy (32 bytes, indexed by HW sprite index)
sprite_attributes   EQU #CB41   ; Interleaved sprite attributes (32 * 4 bytes)

; ==================================================================
; SCREEN SYSTEM VARIABLES (3 screens detected)
; ==================================================================
current_screen_id   EQU #CBC1   ; Currently displayed screen ID
screen_dirty_flag   EQU #CBC2   ; Screen needs redraw flag
screen_transition_cooldown EQU #CBC3   ; Cooldown frames after screen transition
current_world_id    EQU #CBC4   ; Current world ID (for multi-world support)
current_screen_index EQU #CBC5   ; Current screen index within world

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #CBC6   ; Player X position (16-bit)
player_y            EQU #CBC8   ; Player Y position (16-bit)
player_health       EQU #CBCA   ; Player health points
player_score        EQU #CBCB   ; Player score (16-bit)
gem_count           EQU #CBCD   ; Collectible tile counter (8-bit)
last_gem_char       EQU #CBCE   ; Char code of last collected gem tile (for SM VARIABLE_COMPARE)

; Persistent collectibles list (survives screen re-entry)
MAX_COLLECTIBLES     EQU 64              ; Max persistent collectible records
collected_count      EQU #CBCF   ; Number of collected tiles recorded (8-bit)
collected_world      EQU #CBD0   ; World IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_screen     EQU #CC10   ; Screen IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_idx_l      EQU #CC50   ; Tile name-table index low byte (MAX_COLLECTIBLES bytes)
collected_idx_h      EQU #CC90   ; Tile name-table index high byte (MAX_COLLECTIBLES bytes)

; Timed bonus tile respawn slots (bonus gem regeneration)
MAX_BONUS_RESPAWNS   EQU 16              ; Max timed bonus tiles waiting to respawn
bonus_respawn_world  EQU #CCD0   ; World IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_screen EQU #CCE0   ; Screen IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_l  EQU #CCF0   ; Tile index low byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_h  EQU #CD00   ; Tile index high byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_secs   EQU #CD10   ; Remaining seconds per timed respawn slot (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_frames EQU #CD20   ; Frame countdown (60..1) per timed respawn slot (MAX_BONUS_RESPAWNS bytes)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #CD30   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #CD31   ; Temporary 16-bit storage
temp_word_2         EQU #CD33   ; Temporary 16-bit storage
temp_byte_1         EQU #CD35   ; Temporary 8-bit storage
temp_byte_2         EQU #CD36   ; Temporary 8-bit storage
temp_byte_3         EQU #CD37   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #CD57   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #CD77   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #CD97   ; Temporary 8-bit storage (32 bytes)

; ==================================================================
; SOUND SYSTEM VARIABLES
; ==================================================================
sfx_active          EQU #CDB7   ; 0=no SFX active, 1=playing
sfx_timer           EQU #CDB8   ; Frames remaining for current SFX
sfx_fadeout         EQU #CDB9   ; Reserved fadeout flag/state
temp_byte_7         EQU #CDBA   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #CDDA   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #CDFA   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #CE1A   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #CE3A   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #CE5A   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #CE7A   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #CE9A   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #CEBA   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #CEDA   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #CEFA   ; Temporary 8-bit storage (32 bytes)
temp_byte_18        EQU #CF1A   ; Temporary 8-bit storage (32 bytes)
temp_byte_19        EQU #CF3A   ; Temporary 8-bit storage (32 bytes)
temp_byte_20        EQU #CF5A   ; Temporary 8-bit storage (32 bytes)
temp_byte_21        EQU #CF7A   ; Temporary 8-bit storage (32 bytes)
temp_byte_22        EQU #CF9A   ; Temporary 8-bit storage (32 bytes)
temp_byte_23        EQU #CFBA   ; Temporary 8-bit storage (32 bytes)
temp_byte_24        EQU #CFDA   ; Temporary 8-bit storage (32 bytes)
temp_byte_25        EQU #CFFA   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #D01A   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #D05A   ; Temporary 16-bit storage (64 bytes)
temp_byte_26        EQU #D09A   ; Temporary 8-bit storage (32 bytes)
temp_byte_27        EQU #D0BA   ; Temporary 8-bit storage (32 bytes)

; Wall collision temporary variables
wall_temp_x         EQU #D0DA   ; Cached entity X for wall checks
wall_temp_y         EQU #D0DB   ; Cached entity Y for wall checks
wall_hit_left       EQU #D0DC   ; Hitbox left edge cache
wall_hit_top        EQU #D0DD   ; Hitbox top edge cache
wall_hit_right      EQU #D0DE   ; Hitbox right edge cache
wall_hit_bottom     EQU #D0DF   ; Hitbox bottom edge cache
wall_hit_w          EQU #D0E0   ; Hitbox width cache (min 1)
wall_hit_h          EQU #D0E1   ; Hitbox height cache (min 1)
wall_probe_left     EQU #D0E2   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #D0E3   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #D0E4   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #D0E5   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #D0E6   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #D106   ; Number of entries in active_entity_list
active_entity_list_dirty EQU #D107   ; 1=rebuild active_entity_list required

; Entity-entity collision optimized variables
coll_list           EQU #D108   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #D128   ; Number of entities in coll_list
coll_src_left       EQU #D129   ; Source AABB left edge (scratch)
coll_src_right      EQU #D12A   ; Source AABB right edge (scratch)
coll_src_top        EQU #D12B   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #D12C   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #D12D   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #D12D   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #D12F   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #D131   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #D133   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #D135   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #D137   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #D139   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #D13B   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #D13D   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #D13E   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #D143   ; Frame counter (16-bit)
task_exec_time          EQU #D145   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #D147   ; Set to 1 on each VBlank (1 byte)
RAM_INTERRUPT_END       EQU #D148   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #D148   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #D149   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #D14A   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #D14B   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #D14C   ; 0=stopped, 1=track active
music_muted          EQU #D14D   ; 0=audible, 1=muted/pause
music_loop           EQU #D14E   ; 0=no loop, 1=loop enabled
music_track_index    EQU #D14F   ; Current ROM track index
music_row_frames     EQU #D150   ; Frames per tracker row
music_row_countdown  EQU #D151   ; Countdown to next row
music_order_pos      EQU #D152   ; Current order position
music_pattern_index  EQU #D153   ; Current pattern index
music_pattern_row    EQU #D154   ; Current row inside pattern
music_pattern_rows   EQU #D155   ; Cached rows in current pattern
music_track_ptr_l    EQU #D156   ; Current track pointer low byte
music_track_ptr_h    EQU #D157   ; Current track pointer high byte
music_pattern_ptr_l  EQU #D158   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #D159   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #D15A   ; PSG mixer shadow for music runtime
music_ch_note_base EQU #D15B   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #D15B   ; Channel A
music_ch_b_note EQU #D15C   ; Channel B
music_ch_c_note EQU #D15D   ; Channel C
music_ch_instrument_base EQU #D15E   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #D15E   ; Channel A
music_ch_b_instrument EQU #D15F   ; Channel B
music_ch_c_instrument EQU #D160   ; Channel C
music_ch_ornament_base EQU #D161   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #D161   ; Channel A
music_ch_b_ornament EQU #D162   ; Channel B
music_ch_c_ornament EQU #D163   ; Channel C
music_ch_volume_base EQU #D164   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #D164   ; Channel A
music_ch_b_volume EQU #D165   ; Channel B
music_ch_c_volume EQU #D166   ; Channel C
music_ch_vol_step_base EQU #D167   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #D167   ; Channel A
music_ch_b_vol_step EQU #D168   ; Channel B
music_ch_c_vol_step EQU #D169   ; Channel C
music_ch_tone_step_base EQU #D16A   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #D16A   ; Channel A
music_ch_b_tone_step EQU #D16B   ; Channel B
music_ch_c_tone_step EQU #D16C   ; Channel C
music_ch_noise_step_base EQU #D16D   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #D16D   ; Channel A
music_ch_b_noise_step EQU #D16E   ; Channel B
music_ch_c_noise_step EQU #D16F   ; Channel C
music_ch_orn_step_base EQU #D170   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #D170   ; Channel A
music_ch_b_orn_step EQU #D171   ; Channel B
music_ch_c_orn_step EQU #D172   ; Channel C

; ==================================================================
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #D173   ; End of project variables (4467 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#D173: Project variables (4467 bytes)
;   #D173-#F37F: Free RAM (~8717 bytes available)
...
;   Active entities: 2
;   Used components: Position, Sprite, Animation, Patrol, Input, StateMachine, Cursors, Gravity, WallCollision, Jump, Collision
...
; Initialize component systems(OPTIMIZED - only used components) 
    ; Used: Position, Sprite, Animation, Patrol, Input, StateMachine, Cursors, Gravity, WallCollision, Jump, Collision 
...
; SAT Upload Sprites per frame: 4
; ==================================================================
...
SPRITE_BOLA_DEAD_4_WIDTH     EQU 16
SPRITE_BOLA_DEAD_4_HEIGHT    EQU 16
SPRITE_BOLA_DEAD_4_FRAMES    EQU 5

;; ---- Sprite Frame: bola_dead_4_F0 ----
;; Size: 16x16
BOLA_DEAD_4_F0_LAYER1: ; Brush Color Index 1 (Actual Color: #FFFFFF)
    DB #00,#07,#05,#1D,#1B,#1E,#1F,#3D,#1F,#37,#39,#1F,#1F,#0F,#07,#00
    DB #00,#00,#E0,#60,#60,#B0,#70,#68,#C8,#58,#F0,#B0,#B0,#60,#C0,#00

;; ---- End of Frame: bola_dead_4_F0 ----

;; ---- Sprite Frame: bola_dead_4_F1 ----
;; Size: 16x16
BOLA_DEAD_4_F1_LAYER1: ; Brush Color Index 1 (Actual Color: #FFFFFF)
    DB #00,#05,#01,#05,#03,#36,#22,#17,#3D,#0F,#36,#1F,#0F,#08,#04,#00
    DB #00,#40,#E0,#40,#60,#80,#F8,#78,#80,#50,#D0,#70,#30,#20,#40,#00

;; ---- End of Frame: bola_dead_4_F1 ----

;; ---- Sprite Frame: bola_dead_4_F2 ----
;; Size: 16x16
BOLA_DEAD_4_F2_LAYER1: ; Brush Color Index 1 (Actual Color: #FFFFFF)
    DB #00,#05,#0D,#09,#00,#0B,#22,#30,#11,#1D,#2F,#0C,#10,#08,#01,#00
    DB #00,#40,#20,#40,#40,#00,#58,#50,#00,#40,#50,#10,#10,#A0,#40,#00

;; ---- End of Frame: bola_dead_4_F2 ----
...
SPRITE_ANEC_LEFT_5_WIDTH     EQU 16
SPRITE_ANEC_LEFT_5_HEIGHT    EQU 16
SPRITE_ANEC_LEFT_5_FRAMES    EQU 2
...
; Sprite 4: bola_dead frame pointers
SPRITE_4_FRAME_PTRS:
    dw BOLA_DEAD_4_F0_LAYER1
    dw BOLA_DEAD_4_F1_LAYER1
    dw BOLA_DEAD_4_F2_LAYER1
    dw BOLA_DEAD_4_F3_LAYER1
    dw BOLA_DEAD_4_F4_LAYER1

; Sprite 5: anec_left frame pointers
SPRITE_5_FRAME_PTRS:
    dw ANEC_LEFT_5_F0_LAYER1
    dw ANEC_LEFT_5_F1_LAYER1
...
SM_SpriteLayerColorTable:
    ...
    db 15, 0 ; Sprite 2: bossa
    db 15, 8 ; Sprite 3: panell
    db 15, 0 ; Sprite 4: bola_dead
    db 15, 7 ; Sprite 5: anec_left
...
update_sprites_to_vram:
    ...
    ld hl, sprite_attributes
    ld de, SPRATR
    ld bc, 16  ; Upload active sprite range + SAT end marker
    call FAST_LDIRVM
    ret
...
BEHAVIOR_PAN1_0_DATA:
    ...
    DB #85,#00,#55,#68,#10,#38,#C0,#51,#7C,#FE,#55,#35,#55,#58
...
BEHAVIOR_PAN2_1_DATA:
    ...
    DB #80,#00,#11,#0A,#10,#51,#13,#55,#55,#80
...
ENTITY_HERO_1_COMP_MASK EQU #393  ; Component mask: 1110010011b
...
init_hero_1:
    ...
    ; Direction mask: #0E (1110b) = DOWN+LEFT+RIGHT
    ...
    ld hl, entity_dir_mask
    add hl, de
    ld (hl), #0E            ; Direction restrictions: DOWN+LEFT+RIGHT
...
.skip_force_show_1:
    ...
    ld hl, SM_New_Statemachine_state_1771166973837 + 1      ; HL = &OnEnter Actions Ptr field
    ld e, (hl)
    inc hl
...
SM_New_Statemachine_state_1771166973837_Transitions: 
    ...
    DB 4; KEY_PRESSED 
    DB 5          ; Key: arrowdown
    DW SM_New_Statemachine_state_1771506162550 
    ...
    DW SM_New_Statemachine_state_1771166973837_Transitions_Actions_1 

SM_New_Statemachine_state_1771166973837_Transitions_Actions_0: 
    DB 5; CHANGE_SPRITE 
    DB 0; sprite: anec 
    DB 0xFF; END
SM_New_Statemachine_state_1771166973837_Transitions_Actions_1: 
    DB 5; CHANGE_SPRITE 
    DB 4; sprite: bola_dead 
    DB 0xFF; END

SM_New_Statemachine_state_1771506162550: 
    DB 0; ID(unused) 
    DW 0 
    DW 0 
    DW SM_New_Statemachine_state_1771506162550_Transitions 
SM_New_Statemachine_state_1771506162550_Transitions: 
    DB 2; Count
    DB 4; KEY_PRESSED 
    DB 5          ; Key: arrowdown
    DW SM_New_Statemachine_state_1771166973837 
    DW SM_New_Statemachine_state_1771506162550_Transitions_Actions_0 
    DB 8; HAS_COLLISION 
    DB 4          ; collisionType: entity
    DW SM_New_Statemachine_state_1771520893784 
    DW SM_New_Statemachine_state_1771506162550_Transitions_Actions_1 

SM_New_Statemachine_state_1771506162550_Transitions_Actions_0: 
    DB 5; CHANGE_SPRITE 
    DB 1; sprite: bola 
    DB 4; APPLY_FORCE 
    DB 100, 100 
    DB 0xFF; END
SM_New_Statemachine_state_1771506162550_Transitions_Actions_1: 
    DB 5; CHANGE_SPRITE 
    DB 4; sprite: bola_dead 
    DB 0xFF; END

SM_New_Statemachine_state_1771520893784: 
    DB 0; ID(unused) 
    DW 0 
    DW 0 
    DW 0 
...
SUBMENU_CURSOR_PATTERN_COUNT EQU 5
