...
.slash_loop:
    ...
    ; While a slash is active, override the normal per-frame velocity/gravity
    ; integration so update_position_component and wall collision keep the
    ; movement inside the engine's usual pipeline.
    ld hl, entity_vel_x
    add hl, de
    ld (hl), 0
    ld hl, entity_vel_y
...
.slash_move_right:
    ld hl, entity_vel_x
    add hl, de
    ld (hl), SLASH_STEP_PIXELS
    jp .slash_advance_state
...
.ti_bonus_no_input:
    ; If no cursor is held at pickup time, fall back to the current facing
    ; direction so the bonus still produces a visible dash.
