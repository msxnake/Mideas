...
; Estimated payload bytes: 107733
...
; BANK 05 @#0000 : screens.asm (3313 bytes)
; BANK 05 @#0CF1 : sprites.asm part 1/2 (4879 bytes)
; BANK 06 @#0000 : sprites.asm part 2/2 (3670 bytes)
; BANK 06 @#0E56 : font.asm (1872 bytes)
; BANK 06 @#15A6 : hud.asm (2650 bytes)
; BANK 07 @#0000 : hud.asm (1297 bytes)
; BANK 07 @#0511 : menus.asm (168 bytes)
; BANK 07 @#05B9 : sound.asm (4418 bytes)
; BANK 07 @#16FB : scroll.asm (2309 bytes)
; BANK 08 @#0000 : scroll.asm (44 bytes)
; BANK 08 @#002C : animtiles.asm (5524 bytes)
; BANK 08 @#15C0 : bosses.asm part 1/2 (2624 bytes)
; BANK 09 @#0000 : bosses.asm part 2/2 (6049 bytes)
; BANK 09 @#17A1 : statemachine.asm part 1/3 (2143 bytes)
; BANK 10 @#0000 : statemachine.asm part 2/3 (8192 bytes)
; BANK 11 @#0000 : statemachine.asm part 3/3 (8192 bytes)
; BANK 12 @#0000 : statemachine.asm part 4/3 (2833 bytes)
; BANK 12 @#0B11 : gameflow.asm (5359 bytes)
; BANK 13 @#0000 : gameflow.asm (1237 bytes)
...
current_screen2_tilebank_id EQU #C130   ; Current SCREEN 2 shared tilebank loaded in VRAM (#FF=none/unknown)
frame_counter       EQU #C131   ; Frame counter (16-bit)

; Profiling counters (16-bit, cumulative)
prof_update_all_entities_calls EQU #C133   ; Calls to update_all_entities
prof_execute_sm_calls EQU #C135   ; Calls to execute_all_state_machines
prof_sm_update_calls  EQU #C137   ; Calls to SM_Update
prof_collision_calls  EQU #C139   ; Calls to update_collision_component
prof_wall_calls       EQU #C13B   ; Calls to update_wallcollision_component
prof_deadly_calls     EQU #C13D   ; Calls to update_deadly_tiles_component
prof_tile_interaction_calls EQU #C13F   ; Calls to check_tile_interaction
prof_animation_calls  EQU #C141   ; Calls to update_animation_component
prof_sprite_calls     EQU #C143   ; Calls to update_sprite_component
prof_music_task_calls EQU #C145   ; Calls to task_update_music
prof_deadly_behavior_reads EQU #C147   ; Deadly helper behavior-map reads
...
runtime_behavior_map   EQU #C754   ; Mutable copy of current behavior map (32x24)
runtime_interaction_type_map EQU #CA54   ; Mutable copy of current interaction type map (32x24)
runtime_interaction_value_map EQU #CD54   ; Mutable copy of current interaction value map (32x24)
runtime_interaction_target_map EQU #D054   ; Mutable copy of current interaction target map (32x24)
runtime_char_behavior_table EQU #D354   ; Current screen char -> behavior lookup table (256 bytes)
runtime_effects_layout EQU #D454   ; Alternate effects layout copy for secret zones (32x24)
screen_block_catalog_ptr EQU #D754   ; Scratch pointer to current screen block catalog during layout expansion
screen_block_map_ptr EQU #D756   ; Scratch pointer to current screen block index map during layout expansion
runtime_effect_zone_table EQU #D758   ; Current screen effect zone table (0 bytes)
current_effect_zone_count EQU #D758   ; Number of effect zones copied into runtime_effect_zone_table
secret_zone_active EQU #D759   ; 1 if hero is currently inside an active secret zone
secret_zone_rect_x EQU #D75A   ; Active secret zone rect X in cells
secret_zone_rect_y EQU #D75B   ; Active secret zone rect Y in cells
secret_zone_rect_w EQU #D75C   ; Active secret zone rect width in cells
secret_zone_rect_h EQU #D75D   ; Active secret zone rect height in cells

; ==================================================================
; VIEWPORT/CAMERA VARIABLES (for scroll system)
; ==================================================================
camera_x            EQU #D75E   ; Camera X position in pixels (16-bit)
camera_y            EQU #D760   ; Camera Y position in pixels (16-bit)
camera_tile_x       EQU #D762   ; Camera tile X (column)
camera_tile_y       EQU #D763   ; Camera tile Y (row)
world_width_tiles   EQU #D764   ; World width in tiles
world_height_tiles  EQU #D765   ; World height in tiles
scroll_dirty_flag   EQU #D766   ; 1=viewport changed, needs redraw
hud_dirty_flag      EQU #D767   ; 1=HUD needs redraw, 0=clean
time_second_frame_counter EQU #D768   ; VBlank frames remaining until the next TimeRemaining decrement
time_last_interrupt_counter EQU #D769   ; Last interrupt_counter snapshot used by TimeRemaining sync (16-bit)

; ==================================================================
; ANIMATED TILES VARIABLES
; ==================================================================
anim_tile_timer     EQU #D76B   ; Animation frame timer
anim_tile_frame     EQU #D76C   ; Current animation frame (0-3)
anim_tile_speed     EQU #D76D   ; Frames between animation updates
anim_tile_transform_flags EQU #D76E   ; Runtime flags for transform-mode tile animation (byte0=flags, byte1=opcode scratch)
anim_tile_row_buffer EQU #D770   ; Temp buffer (8 bytes) for row transforms
...
entity_button_contact_active EQU #D7B8   ; 1 while entity stays on the same button tile (32 bytes)
entity_button_contact_x EQU #D7D8   ; Button tile X currently latched per entity (32 bytes)
entity_button_contact_y EQU #D7F8   ; Button tile Y currently latched per entity (32 bytes)
entity_on_ladder   EQU #D818   ; 1 while entity is centered on a ladder tile (32 bytes)
entity_gate_current_step EQU #D838   ; Current applied retract step (32 bytes)
entity_gate_step_timer EQU #D858   ; Countdown until next retract step (32 bytes)
entity_walljump_lock EQU #D878   ; Remaining horizontal lock frames after wall jump (32 bytes)
entity_walljump_locked_vx EQU #D898   ; Horizontal velocity preserved while wall jump lock is active (32 bytes)
entity_wallgrab_active EQU #D8B8   ; 1 if entity is currently grabbing a wall (32 bytes)
entity_wallgrab_grace EQU #D8D8   ; Frames to keep wall grab during transient wall flag gaps (32 bytes)
entity_wallgrab_timer EQU #D8F8   ; Remaining wall-grab frames until grounded reset (32 bytes)
entity_wallgrab_lockout EQU #D918   ; Wall grab disabled until grounded after timer is spent (32 bytes)
entity_walljump_anim_active EQU #D938   ; Wall jump one-shot animation is waiting to restore base sprite (32 bytes)
entity_x_pos        EQU #D958   ; Entity X positions (32 bytes)
entity_y_pos        EQU #D978   ; Entity Y positions (32 bytes)
entity_vel_x        EQU #D998   ; Entity X velocity (32 bytes)
entity_vel_y        EQU #D9B8   ; Entity Y velocity (32 bytes)
entity_comp_masks   EQU #D9D8   ; Entity component masks (32 bytes)
entity_comp_masks_hi EQU #D9F8   ; Entity component masks high byte (32 bytes)
entity_screen_id    EQU #DA18   ; Entity screen ID (32 bytes)
entity_job_period   EQU #DA38   ; Entity job period in frames (32 bytes, 1=100%,2=50%,3=33%,4=25%)
entity_job_entry    EQU #DA58   ; Entity job entry slot within period window (32 bytes)
entity_job_scheduler_active EQU #DA78   ; 1 when any entity uses non-default job cadence
entity_dir_mask     EQU #DA79   ; Entity direction mask (32 bytes)
entity_input_speed  EQU #DA99   ; Entity input/cursor speed (32 bytes)
entity_health       EQU #DAB9   ; Entity health (32 bytes)
entity_anim_frame   EQU #DAD9   ; Entity animation frame (32 bytes)
entity_anim_tick    EQU #DAF9   ; Entity animation tick counter (32 bytes)
entity_anim_speed   EQU #DB19   ; Entity animation speed (ticks per frame) (32 bytes)
entity_anim_flags   EQU #DB39   ; Entity animation flags (32 bytes)
entity_sm_ptr_l     EQU #DB59   ; Entity State Pointer Low (32 bytes)
entity_sm_ptr_h     EQU #DB79   ; Entity State Pointer High (32 bytes)
entity_sm_timer_l   EQU #DB99   ; Entity State Timer Low (32 bytes)
entity_sm_timer_h   EQU #DBB9   ; Entity State Timer High (32 bytes)
entity_sm_wait_timer EQU #DBD9   ; Entity State Wait Timer (32 bytes)
entity_sm_sprite_control EQU #DBF9   ; 1 when the assigned state machine explicitly drives sprite changes (32 bytes)
entity_lifetime     EQU #DC19   ; Entity lifetime for auto-destroy (32 bytes, 0=infinite)
entity_collectible_enabled EQU #DC39   ; 1 when entity has Collectible component (32 bytes)
entity_carried_by   EQU #DC59   ; Entity carrier ID (32 bytes, 255=not carried)
entity_template_token EQU #DC79   ; Entity template token (32 bytes, 0=unknown)
entity_facing_dir   EQU #DC99   ; Last facing direction (32 bytes, 0=none,1=left,2=right,3=up,4=down)
entity_sm_var_0     EQU #DCB9   ; Entity Variable 0 (32 bytes)
entity_sm_var_1     EQU #DCD9   ; Entity Variable 1 (32 bytes)
entity_sm_var_2     EQU #DCF9   ; Entity Variable 2 (32 bytes)
entity_sm_var_3     EQU #DD19   ; Entity Variable 3 (32 bytes)
entity_sm_var_4     EQU #DD39   ; Entity Variable 4 (32 bytes)
entity_sm_var_5     EQU #DD59   ; Entity Variable 5 (32 bytes)
entity_sm_var_6     EQU #DD79   ; Entity Variable 6 (32 bytes)
entity_sm_var_7     EQU #DD99   ; Entity Variable 7 (32 bytes)

; ==================================================================
; SPRITE SYSTEM VARIABLES
; ==================================================================
entity_sprite_asset_index EQU #DDB9   ; Entity sprite asset index - RAM copy (32 bytes)
active_sprite_count EQU #DDD9   ; Number of sprites currently active
sprites_dirty      EQU #DDDA   ; 1=sprite_attributes changed, needs VRAM sync
sprite_pattern      EQU #DDDB   ; Sprite pattern IDs (32 bytes)
sprite_color        EQU #DDFB   ; Sprite colors (32 bytes)
sprite_layer_colors EQU #DE1B   ; HW sprite layer color cache - RAM copy (32 bytes, indexed by HW sprite index)
sprite_layer_y_offsets EQU #DE3B   ; HW sprite layer signed Y offsets - RAM copy (32 bytes, indexed by HW sprite index)
sprite_asset_base_pattern_slot_runtime EQU #DE5B   ; Runtime base 16x16 slot per sprite asset (11 bytes)
...
tileDead_dbg        EQU #E436   ; Debug byte: current hero deadly contact
tileDead_latched_dbg EQU #E437   ; Debug byte: latched hero deadly contact
tileDead_x_dbg      EQU #E438   ; Debug byte: last sampled deadly tile X
tileDead_y_dbg      EQU #E439   ; Debug byte: last sampled deadly tile Y
tileDead_value_dbg  EQU #E43A   ; Debug byte: last raw deadly behavior value

; Wall collision temporary variables
wall_temp_x         EQU #E43B   ; Cached entity X for wall checks
wall_temp_y         EQU #E43C   ; Cached entity Y for wall checks
wall_hit_left       EQU #E43D   ; Hitbox left edge cache
wall_hit_top        EQU #E43E   ; Hitbox top edge cache
wall_hit_right      EQU #E43F   ; Hitbox right edge cache
wall_hit_bottom     EQU #E440   ; Hitbox bottom edge cache
wall_hit_w          EQU #E441   ; Hitbox width cache (min 1)
wall_hit_h          EQU #E442   ; Hitbox height cache (min 1)
wall_probe_left     EQU #E443   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #E444   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #E445   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #E446   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #E447   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #E467   ; Number of entries in active_entity_list
hero_entity_id      EQU #E468   ; First current-screen entity flagged as player (#FF = none)
active_entity_list_dirty EQU #E469   ; 1=rebuild active_entity_list required
input_entity_list   EQU #E46A   ; Active current-screen entities with Input component (MAX_ENTITIES bytes)
input_entity_count  EQU #E48A   ; Number of entries in input_entity_list
render_entity_list  EQU #E48B   ; Active current-screen entities with Sprite component (MAX_ENTITIES bytes)
render_entity_count EQU #E4AB   ; Number of entries in render_entity_list
collision_entity_list EQU #E4AC   ; Active current-screen entities with Collision component (MAX_ENTITIES bytes)
collision_entity_count EQU #E4CC   ; Number of entries in collision_entity_list
ground_entity_list  EQU #E4CD   ; Active current-screen entities with Collision or Gravity (MAX_ENTITIES bytes)
ground_entity_count EQU #E4ED   ; Number of entries in ground_entity_list
anim_entity_list    EQU #E4EE   ; Active current-screen entities with Animation+Sprite (MAX_ENTITIES bytes)
anim_entity_count   EQU #E50E   ; Number of entries in anim_entity_list

; Entity-entity collision optimized variables
coll_list           EQU #E50F   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #E52F   ; Number of entities in coll_list
coll_src_left       EQU #E530   ; Source AABB left edge (scratch)
coll_src_right      EQU #E531   ; Source AABB right edge (scratch)
coll_src_top        EQU #E532   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #E533   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #E534   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #E534   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #E536   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #E538   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #E53A   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #E53C   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #E53E   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #E540   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #E542   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #E544   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #E545   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #E54A   ; Frame counter (16-bit)
task_exec_time          EQU #E54C   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #E54E   ; Set to 1 on each VBlank (1 byte)
interrupt_in_progress   EQU #E54F   ; 1 while the H.TIMI dispatcher is running
RAM_INTERRUPT_END       EQU #E550   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #E550   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #E551   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #E552   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #E553   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #E554   ; 0=stopped, 1=track active
music_muted          EQU #E555   ; 0=audible, 1=muted/pause
music_loop           EQU #E556   ; 0=no loop, 1=loop enabled
music_track_index    EQU #E557   ; Current ROM track index
music_row_frames     EQU #E558   ; Frames per tracker row
music_row_countdown  EQU #E559   ; Countdown to next row
music_order_pos      EQU #E55A   ; Current order position
music_pattern_index  EQU #E55B   ; Current pattern index
music_pattern_row    EQU #E55C   ; Current row inside pattern
music_pattern_rows   EQU #E55D   ; Cached rows in current pattern
music_track_ptr_l    EQU #E55E   ; Current track pointer low byte
music_track_ptr_h    EQU #E55F   ; Current track pointer high byte
music_pattern_ptr_l  EQU #E560   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #E561   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #E562   ; PSG mixer shadow for music runtime
music_pitch_note_work EQU #E563   ; Scratch note index while resolving tone/ornament macros
music_pitch_step_work EQU #E564   ; Scratch macro step while resolving tone/ornament macros
music_pitch_len_work  EQU #E565   ; Scratch macro length while resolving tone/ornament macros
music_ch_note_base EQU #E566   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #E566   ; Channel A
music_ch_b_note EQU #E567   ; Channel B
music_ch_c_note EQU #E568   ; Channel C
music_ch_instrument_base EQU #E569   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #E569   ; Channel A
music_ch_b_instrument EQU #E56A   ; Channel B
music_ch_c_instrument EQU #E56B   ; Channel C
music_ch_ornament_base EQU #E56C   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #E56C   ; Channel A
music_ch_b_ornament EQU #E56D   ; Channel B
music_ch_c_ornament EQU #E56E   ; Channel C
music_ch_volume_base EQU #E56F   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #E56F   ; Channel A
music_ch_b_volume EQU #E570   ; Channel B
music_ch_c_volume EQU #E571   ; Channel C
music_ch_vol_step_base EQU #E572   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #E572   ; Channel A
music_ch_b_vol_step EQU #E573   ; Channel B
music_ch_c_vol_step EQU #E574   ; Channel C
music_ch_tone_step_base EQU #E575   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #E575   ; Channel A
music_ch_b_tone_step EQU #E576   ; Channel B
music_ch_c_tone_step EQU #E577   ; Channel C
music_ch_noise_step_base EQU #E578   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #E578   ; Channel A
music_ch_b_noise_step EQU #E579   ; Channel B
music_ch_c_noise_step EQU #E57A   ; Channel C
music_ch_orn_step_base EQU #E57B   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #E57B   ; Channel A
music_ch_b_orn_step EQU #E57C   ; Channel B
music_ch_c_orn_step EQU #E57D   ; Channel C
music_ch_hw_env_step_base EQU #E57E   ; Software hardware-envelope divider step (3 bytes)
music_ch_a_hw_env_step EQU #E57E   ; Channel A
music_ch_b_hw_env_step EQU #E57F   ; Channel B
music_ch_c_hw_env_step EQU #E580   ; Channel C
...
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #E581   ; End of project variables (9601 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#E581: Project variables (9601 bytes)
;   #E581-#E600: Alignment padding/free RAM (127 bytes)
...
; ==================================================================
tile_pattern_bank0:
    ; ZX0 compressed tile_pattern (1384 -> 1016 bytes)
...
; ==================================================================
tile_color_bank0:
    ; ZX0 compressed tile_color (1384 -> 503 bytes)
...
SCREEN_PAN1_0_BOSS_TABLE:
    ...
    dw boss_1_mico_phase_table, boss_1_mico_attack_table
    db 17, 4, 0, 1, 1, 100, 0    ; xChar,yChar,initialPhase,flags,updateEveryNFrames,healthLo,healthHi
...
boss_table:
    ...
    dw boss_1_mico_phase_table, boss_1_mico_attack_table    ; mico
    db #64, #00, #01, #00    ; healthLo,healthHi,phaseCount,attackCount
...
; Boss 1: mico
; Boss attack definitions: 0 defined, 0 referenced
boss_1_mico_phase_table:
    dw boss_1_mico_phase_0
boss_1_mico_attack_table:
    dw #FFFF
boss_1_mico_phase_0:
    db #00, #00, #10, #10    ; healthThreshold,buildType,width,height
    dw boss_1_mico_phase_0_tiles, #FFFF, #FFFF, #FFFF, #FFFF, boss_1_mico_phase_0_behavior, boss_1_mico_phase_0_forms, boss_1_mico_phase_0_weak    ; tileMatrix,collision,neck,crush,attacks,behavior,forms,weak
boss_1_mico_phase_0_tiles:
    db #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D
    db #9D, #9D, #9D, #9D, #FF, #FF, #FF, #FF, #FF, #FF, #FF, #FF, #9D, #9D, #9D, #9D
    db #9D, #9D, #9D, #9D, #FF, #FF, #FF, #9D, #9D, #9E, #9F, #A0, #A1, #9D, #9D, #FF
    db #9D, #9D, #9D, #9D, #FF, #FF, #FF, #9D, #9D, #9D, #A2, #A3, #A4, #9D, #9D, #FF
    db #9D, #9D, #9D, #9D, #FF, #FF, #FF, #9D, #9D, #A5, #A6, #A7, #A8, #A9, #9D, #FF
    db #9D, #9D, #9D, #9D, #FF, #FF, #FF, #9D, #AA, #AB, #AC, #AD, #AE, #AF, #9D, #FF
    db #9D, #9D, #9D, #9D, #FF, #FF, #FF, #9D, #B0, #B1, #B2, #B3, #B4, #B5, #B6, #FF
    db #9D, #9D, #9D, #9D, #FF, #FF, #FF, #9D, #B7, #B8, #B9, #BA, #BB, #BC, #BD, #FF
    db #C2, #C2, #C2, #C2, #FF, #FF, #FF, #9D, #9D, #9D, #BE, #BF, #C0, #C1, #9D, #FF
    db #C2, #C2, #C2, #C2, #FF, #FF, #FF, #C3, #C4, #C5, #C6, #C7, #C8, #C9, #C2, #FF
    db #C2, #C2, #C2, #C2, #FF, #FF, #FF, #CA, #CB, #CC, #CD, #CE, #CF, #D0, #D1, #FF
    db #C2, #C2, #C2, #C2, #FF, #FF, #FF, #D2, #D3, #D4, #D5, #D6, #D7, #D8, #D9, #FF
    db #C2, #C2, #C2, #C2, #FF, #FF, #FF, #C2, #DA, #DB, #DC, #DD, #DE, #DF, #E0, #FF
    db #C2, #C2, #C2, #C2, #FF, #FF, #FF, #C2, #C2, #E1, #E2, #E3, #E4, #E5, #C2, #FF
    db #C2, #C2, #C2, #C2, #FF, #FF, #FF, #C2, #C2, #E6, #E7, #E8, #E9, #EA, #C2, #FF
    db #C2, #C2, #C2, #C2, #FF, #FF, #FF, #C2, #C2, #EB, #EC, #C2, #ED, #EE, #EF, #FF
boss_1_mico_phase_0_weak:
    db #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00
    db #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00
    db #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00
    db #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00
    ...
    db #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00
    db #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00
boss_1_mico_phase_0_behavior:
    db #02, #1F, #00    ; count,totalLo,totalHi
    db #00, #1E, #00, #00, #00, #FF, #00, #00
    db #08, #01, #00, #00, #00, #00, #00, #00
boss_1_mico_phase_0_forms:
    db #02    ; count
    dw boss_1_mico_phase_0_tiles, boss_1_mico_phase_0_weak
    dw boss_1_mico_phase_0_forms_1, boss_1_mico_phase_0_forms_1_weak
boss_1_mico_phase_0_forms_1:
    db #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D, #9D
    db #9D, #9D, #9D, #9D, #9D, #9D, #9E, #9F, #A0, #A1, #9D, #9D, #9D, #9D, #9D, #9D
    db #9D, #9D, #9D, #9D, #9D, #9D, #9D, #A2, #A3, #A4, #9D, #9D, #9D, #9D, #9D, #9D
    db #9D, #9D, #9D, #9D, #9D, #9D, #A5, #A6, #A7, #A8, #A9, #9D, #9D, #9D, #9D, #9D
    db #9D, #9D, #9D, #9D, #9D, #AA, #AB, #AC, #AD, #AE, #AF, #9D, #9D, #9D, #9D, #9D
    db #9D, #9D, #9D, #9D, #9D, #B0, #B1, #B2, #B3, #B4, #B5, #B6, #9D, #9D, #9D, #9D
    db #9D, #9D, #9D, #9D, #9D, #B7, #B8, #B9, #BA, #BB, #BC, #BD, #9D, #9D, #9D, #9D
    db #9D, #9D, #9D, #9D, #9D, #9D, #9D, #BE, #BF, #C0, #C1, #9D, #9D, #9D, #9D, #9D
    db #C2, #C2, #C2, #C2, #C3, #C4, #C5, #C6, #C7, #C8, #C9, #C2, #C2, #C2, #C2, #C2
    db #C2, #C2, #C2, #C2, #CA, #CB, #CC, #CD, #CE, #CF, #D0, #D1, #C2, #C2, #C2, #C2
    db #C2, #C2, #C2, #C2, #D2, #D3, #D4, #D5, #D6, #D7, #D8, #D9, #C2, #C2, #C2, #C2
    db #C2, #C2, #C2, #C2, #C2, #DA, #DB, #DC, #DD, #DE, #DF, #E0, #C2, #C2, #C2, #C2
    db #C2, #C2, #C2, #C2, #C2, #C2, #E1, #E2, #E3, #E4, #E5, #C2, #C2, #C2, #C2, #C2
    db #C2, #C2, #C2, #C2, #C2, #C2, #E6, #E7, #E8, #E9, #EA, #C2, #C2, #C2, #C2, #C2
    db #C2, #C2, #C2, #C2, #C2, #C2, #EB, #EC, #C2, #ED, #EE, #EF, #C2, #C2, #C2, #C2
    db #C2, #C2, #C2, #C2, #C2, #C2, #C2, #C2, #C2, #C2, #C2, #C2, #C2, #C2, #C2, #C2
boss_1_mico_phase_0_forms_1_weak:
    db #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00
    db #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00
    db #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00
    db #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00, #00
...
ZX0_SPRITE_FRAME_NEW_SPRITE_LEFT_10_F0_DATA:
    ; ZX0 compressed sprite frame NEW_SPRITE_LEFT_10_F0 (64 -> 57 bytes)
