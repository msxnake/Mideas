...
; ==================================================================
active_sprite_count EQU #C3FA   ; Number of sprites currently active
sprite_pattern      EQU #C3FB   ; Sprite pattern IDs (32 bytes)
sprite_color        EQU #C41B   ; Sprite colors (32 bytes)
sprite_attributes   EQU #C43B   ; Interleaved sprite attributes (32 * 4 bytes)

; ==================================================================
; SCREEN SYSTEM VARIABLES (1 screens detected)
; ==================================================================
current_screen_id   EQU #C4BB   ; Currently displayed screen ID
screen_dirty_flag   EQU #C4BC   ; Screen needs redraw flag
current_world_id    EQU #C4BD   ; Current world ID (for multi-world support)
current_screen_index EQU #C4BE   ; Current screen index within world

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #C4BF   ; Player X position (16-bit)
player_y            EQU #C4C1   ; Player Y position (16-bit)
player_health       EQU #C4C3   ; Player health points
player_score        EQU #C4C4   ; Player score (16-bit)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #C4C6   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #C4C7   ; Temporary 16-bit storage
temp_word_2         EQU #C4C9   ; Temporary 16-bit storage
temp_byte_1         EQU #C4CB   ; Temporary 8-bit storage
temp_byte_2         EQU #C4CC   ; Temporary 8-bit storage
temp_byte_3         EQU #C4CD   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #C4ED   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #C50D   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #C52D   ; Temporary 8-bit storage (32 bytes)
temp_byte_7         EQU #C54D   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #C56D   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #C58D   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #C5AD   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #C5CD   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #C5ED   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #C60D   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #C62D   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #C64D   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #C66D   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #C68D   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #C6AD   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #C6ED   ; Temporary 16-bit storage (64 bytes)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #C72D   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #C72D   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #C72F   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #C731   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #C733   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #C735   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #C737   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #C739   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #C73B   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #C73D   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #C73E   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #C743   ; Frame counter (16-bit)
task_exec_time          EQU #C745   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #C747   ; Set to 1 on each VBlank (1 byte)
RAM_INTERRUPT_END       EQU #C748   ; End of interrupt system

; ==================================================================
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #C748   ; End of project variables (1864 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#C748: Project variables (1864 bytes)
;   #C748-#F37F: Free RAM (~11320 bytes available)
...
;   Active entities: 1
;   Used components: Position, Sprite, Animation, Patrol
...
; Initialize component systems(OPTIMIZED - only used components) 
    ; Used: Position, Sprite, Animation, Patrol 
...
tile_pattern_bank0:
    ...
    ; Row 0: Char0 Char1 
    ; Row 1: Char2 Char3 
    db #00, #7F, #7F, #00, #FB, #FB, #00, #7F, #00, #F7, #F7, #00, #FF, #FF, #00, #F7, #7F, #00, #FB, #FB, #00, #7F, #7F, #00, #F7, #00, #FF, #FF, #00, #F7, #F7, #00
...
SPRITE_ANEC_0_WIDTH     EQU 16
SPRITE_ANEC_0_HEIGHT    EQU 16
SPRITE_ANEC_0_FRAMES    EQU 2

;; ---- Sprite Frame: anec_0_F0 ----
;; Size: 16x16
ANEC_0_F0_LAYER1: ; Brush Color Index 1 (Actual Color: #FFFFFF)
    DB #00,#00,#01,#01,#03,#02,#03,#01,#00,#00,#00,#09,#0E,#07,#04,#0A
    DB #00,#C0,#B0,#D0,#58,#DC,#F7,#E0,#40,#00,#E0,#F8,#F8,#F0,#08,#14

ANEC_0_F0_LAYER2: ; Brush Color Index 2 (Actual Color: #42EBF5)
    DB #00,#00,#00,#00,#00,#01,#00,#40,#60,#30,#19,#16,#11,#08,#00,#04
    DB #00,#00,#00,#20,#A0,#20,#00,#00,#00,#E0,#18,#04,#04,#08,#00,#08

ANEC_0_F0_LAYER3: ; Brush Color Index 3 (Actual Color: #00FF00)
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00

;; ---- End of Frame: anec_0_F0 ----

;; ---- Sprite Frame: anec_0_F1 ----
;; Size: 16x16
ANEC_0_F1_LAYER1: ; Brush Color Index 1 (Actual Color: #FFFFFF)
    DB #00,#00,#01,#01,#03,#02,#03,#01,#00,#20,#10,#09,#0E,#03,#00,#00
    DB #00,#C0,#B0,#D0,#58,#DC,#F7,#E0,#40,#00,#E0,#78,#F8,#F0,#40,#A0

ANEC_0_F1_LAYER2: ; Brush Color Index 2 (Actual Color: #42EBF5)
    DB #00,#00,#00,#00,#00,#01,#00,#00,#E0,#D0,#29,#16,#01,#04,#00,#00
    DB #00,#00,#00,#20,#A0,#20,#00,#00,#00,#E0,#18,#84,#04,#08,#00,#40

ANEC_0_F1_LAYER3: ; Brush Color Index 3 (Actual Color: #00FF00)
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00

;; ---- End of Frame: anec_0_F1 ----


; Unified pattern label for sprite 0
SPRITE_0_PATTERN EQU ANEC_0_F0_LAYER1
...
load_screen_new_screenmap_770754008863:
    ; Load New Screenmap screen (fast direct port access)
    ; HUD occupies rows 0-1 (2 rows = 64 bytes)
    ; Game area: rows 2-23 (22 rows = 704 bytes)
    ...
    ld hl, SCREEN_NEW_SCREENMAP_0_LAYOUT + 64  ; Skip first 2 rows
    ld de, NAMETBL + 64            ; VRAM destination: start at row 2
    ld bc, 704                      ; Only 22 rows
    call FAST_LDIRVM           ; Fast VRAM write (direct port access)
    ret
...
init_pato_1:
    ...
    ; Waypoints: (48, 120) -> (208, 120)
    ; Override position with waypoint1
    ld hl, entity_x_pos
    add hl, de
    ld (hl), 48         ; Start X = waypoint1_x
...
load_world_worldmap_1770754170935:
    ...
    call load_screen_new_screenmap_770754008863
