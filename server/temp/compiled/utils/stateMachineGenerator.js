...
exports.DEFAULT_STATE_MACHINE_CONFIG = void 0;
exports.analyzeProjectForStateMachine = analyzeProjectForStateMachine;
exports.generateProjectStateMachine = generateProjectStateMachine;
const projectUtils_1 = require("./projectUtils");
const z80CodeGenerator_1 = require("./z80CodeGenerator");
 ...
 */
function analyzeProjectForStateMachine(assets) {
    const config = { ...exports.DEFAULT_STATE_MACHINE_CONFIG };
    ...
    const hasMenuScreen = screenMaps.some(screen => screen.name.toLowerCase().includes('menu') ||
        screen.name.toLowerCase().includes('title'));
    if (hasMenuScreen) {
        config.includeMenu = true;
    }
    // Check for boss or special areas
    const hasBossScreen = screenMaps.some(screen => screen.name.toLowerCase().includes('boss') ||
        screen.name.toLowerCase().includes('final'));
    if (hasBossScreen) {
        customStates.push('STATE_BOSS_BATTLE');
    }
    // Check for cutscenes or story elements
    const hasCutsceneElements = assets.some(asset => asset.name.toLowerCase().includes('cutscene') ||
 ...
 */
function generateProjectStateMachine(projectName, assets, config = exports.DEFAULT_STATE_MACHINE_CONFIG, options = z80CodeGenerator_1.DEFAULT_CODE_OPTIONS) {
    const formattedDate = (0, projectUtils_1.getFormattedDate)();
    const filename = `${projectName.toLowerCase().replace(/\s+/g, '_')}_state_machine.asm`;
    let code = (0, projectUtils_1.generateAsmFileHeader)(projectName, formattedDate, filename);
...
function generateIncludes(config) {
    let code = '';
    code += '; ==================================================\n';
    code += '; INCLUDES\n';
    code += '; ==================================================\n\n';
...
function generateStateDefinitions(config) {
    let code = '';
    code += '; ==================================================\n';
...
function generateMemoryLayout(config) {
    let code = '';
    code += '; ==================================================\n';
    code += '; MEMORY LAYOUT\n';
    code += '; ==================================================\n\n';
...
function generateStateMachineCore(config, assets) {
    let code = '';
    code += '; ==================================================\n';
...
function generateJumpTable(config) {
    let code = '';
    code += '; ==================================================\n';
    code += '; JUMP TABLE\n';
    code += '; ==================================================\n\n';
    ...
    const optionalStates = [];
    if (config.includeMenu) {
        optionalStates.push({ name: 'STATE_MENU', routine: 'logic_menu' }, { name: 'STATE_GAMEOVER', routine: 'logic_gameover' });
    }
    if (config.includeLoading) {
...
function generateStateLogicRoutines(config, assets, options) {
    let code = '';
    code += '; ==================================================\n';
...
function generatePlayingStateLogic(config, assets) {
    let code = '';
    code += 'logic_playing:\n';
    code += '    ; Read input\n';
...
function generateDyingStateLogic(config) {
    let code = '';
    code += 'logic_dying:\n';
...
function generatePausedStateLogic(config) {
    let code = '';
    code += 'logic_paused:\n';
...
function generateMenuStateLogic(config) {
    let code = '';
    code += 'logic_menu:\n';
...
function generateGameOverStateLogic(config) {
    let code = '';
    code += 'logic_gameover:\n';
...
function generateLoadingStateLogic(config, assets) {
    let code = '';
    code += 'logic_loading:\n';
...
function generateCustomStateLogic(stateName, routineName, config, assets) {
    let code = '';
    code += `${routineName}:\n`;
...
function generateProjectHelpers(config, assets) {
    let code = '';
    code += '; ==================================================\n';
...
function generateDataSection(config) {
    let code = '';
    code += '; ==================================================\n';
    code += '; DATA SECTION\n';
    code += '; ==================================================\n\n';
