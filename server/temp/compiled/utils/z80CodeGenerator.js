...
exports.DEFAULT_CODE_OPTIONS = void 0;
exports.colorToMSX1Index = colorToMSX1Index;
exports.pixelDataToPattern = pixelDataToPattern;
exports.generateTileAssembly = generateTileAssembly;
exports.generateSpriteAssembly = generateSpriteAssembly;
exports.generateScreenMapAssembly = generateScreenMapAssembly;
exports.generateEntityAssembly = generateEntityAssembly;
exports.generateCompleteGameAssembly = generateCompleteGameAssembly;
exports.generateStateMachineAssembly = generateStateMachineAssembly;
exports.generateCompleteGameWithStateMachine = generateCompleteGameWithStateMachine;
exports.createProjectStateMachineCopy = createProjectStateMachineCopy;
// Import state machine generator
const stateMachineGenerator_1 = require("./stateMachineGenerator");
/**
 * Default code generation options
 */
exports.DEFAULT_CODE_OPTIONS = {
    dataFormat: 'hex',
    ...
    stateMachineConfig: stateMachineGenerator_1.DEFAULT_STATE_MACHINE_CONFIG
};
/**
 * Debug function to log sprite data processing
 */
function debugSpriteData(pixelData, spriteName = 'Unknown') {
 ...
 */
function colorToMSX1Index(color) {
    if (!color)
        return 1; // Default to transparent/black
 ...
 */
function pixelDataToPattern(pixelData, isSprite = false) {
    const pattern = [];
    if (!pixelData || pixelData.length === 0) {
    ...
    if (isSprite) {
        ...
        for (let y = 0; y < Math.min(8, pixelData.length); y++) {
            ...
            for (let x = 0; x < Math.min(8, row.length); x++) {
                ...
                let colorValue = 0;
                if (colorIndex === 1)
                    colorValue = 0; // Transparent (00)
                else if (colorIndex <= 8)
                    colorValue = 1; // Color set 1 (01)
                else if (colorIndex <= 14)
                    colorValue = 2; // Color set 2 (10)  
                else
                    colorValue = 3; // Color set 3 (11)
 ...
 */
function generateTileAssembly(tiles, options = exports.DEFAULT_CODE_OPTIONS) {
    let code = '';
 ...
 */
function generateSpriteAssembly(sprites, options = exports.DEFAULT_CODE_OPTIONS) {
    let code = '';
    if (options.includeComments) {
        code += '; ==================================================\n';
    ...
    sprites.forEach((sprite, spriteIndex) => {
        ...
        sprite.spritePalette.forEach((color, colorIndex) => {
            ...
            if (colorIndex < sprite.spritePalette.length - 1)
                code += ', ';
        });
        code += '\n\n';
    });
 ...
 */
function generateScreenMapAssembly(screenMap, tiles, options = exports.DEFAULT_CODE_OPTIONS) {
    let code = '';
    if (options.includeComments) {
        code += '; ==================================================\n';
 ...
 */
function generateEntityAssembly(entities, components, templates, options = exports.DEFAULT_CODE_OPTIONS) {
    let code = '';
    if (options.includeComments) {
        code += '; ==================================================\n';
 ...
 */
function generateCompleteGameAssembly(assets, options = exports.DEFAULT_CODE_OPTIONS) {
 ...
 */
function generateStateMachineAssembly(projectName, assets, options = exports.DEFAULT_CODE_OPTIONS) {
    // Analyze project to determine optimal state machine configuration
    const config = (0, stateMachineGenerator_1.analyzeProjectForStateMachine)(assets);
    ...
    const finalConfig = options.stateMachineConfig ?
        ...
        config;
    return (0, stateMachineGenerator_1.generateProjectStateMachine)(projectName, assets, finalConfig, options);
}
/**
 * Generate complete game assembly with optional state machine integration
 */
function generateCompleteGameWithStateMachine(projectName, assets, options = exports.DEFAULT_CODE_OPTIONS) {
    let code = '';
 ...
 */
function createProjectStateMachineCopy(projectName, assets, options = exports.DEFAULT_CODE_OPTIONS) {
    // Analyze project for optimal configuration
    const config = (0, stateMachineGenerator_1.analyzeProjectForStateMachine)(assets);
    // Generate the state machine code
    const content = (0, stateMachineGenerator_1.generateProjectStateMachine)(projectName, assets, config, options);
