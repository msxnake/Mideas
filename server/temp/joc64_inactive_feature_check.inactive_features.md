...
- Metrics: findings=31, patchable=0, removed_lines=0, removed_source_bytes=0
...
- [report-only] `init_sound_system_far` lines 14728-14745: `init_sound_system_far` looks like audio runtime (18 lines, 350 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: boot_or_init (Boot/init routine). External references still exist (1): call_init_sound_system_resident@15297. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `task_audio_tick_far` lines 14746-14763: `task_audio_tick_far` looks like audio runtime (18 lines, 342 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). No external label references were found, making this a candidate for a future feature-specific patch rule once block ownership and invariants are added. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `sfx_update_far` lines 14764-14781: `sfx_update_far` looks like audio runtime (18 lines, 322 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). External references still exist (1): call_sfx_update_resident@15320. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `music_update_far` lines 14782-14799: `music_update_far` looks like audio runtime (18 lines, 330 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). External references still exist (1): call_music_update_resident@15317. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `music_stop_far` lines 14800-14817: `music_stop_far` looks like audio runtime (18 lines, 322 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). External references still exist (1): call_music_stop_resident@15323. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `music_play_track_far` lines 14818-14837: `music_play_track_far` looks like audio runtime (20 lines, 383 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). External references still exist (1): call_music_play_track_resident@15326. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `music_execute_command_far` lines 14838-14858: `music_execute_command_far` looks like audio runtime (21 lines, 439 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). External references still exist (1): call_music_execute_command_resident@15329. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `call_init_sound_system_resident` lines 15296-15298: `call_init_sound_system_resident` looks like audio runtime (3 lines, 63 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). External references still exist (1): init_rom@10137. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `call_task_audio_tick_resident` lines 15299-15315: `call_task_audio_tick_resident` looks like audio runtime (17 lines, 485 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). External references still exist (3): init_rom@10158, gameflow_handle_end@25480, gameflow_world_game_loop@25724. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `call_music_update_resident` lines 15316-15318: `call_music_update_resident` looks like audio runtime (3 lines, 53 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). External references still exist (2): call_task_audio_tick_resident@15308, task_audio_tick@31966. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `call_sfx_update_resident` lines 15319-15321: `call_sfx_update_resident` looks like audio runtime (3 lines, 49 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). External references still exist (1): gameflow_world_game_loop@25759. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `call_music_stop_resident` lines 15322-15324: `call_music_stop_resident` looks like audio runtime (3 lines, 49 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). External references still exist (1): update_scroll@34389. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `call_music_play_track_resident` lines 15325-15327: `call_music_play_track_resident` looks like audio runtime (3 lines, 61 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). External references still exist (1): update_scroll@34407. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `call_music_execute_command_resident` lines 15328-15330: `call_music_execute_command_resident` looks like audio runtime (3 lines, 71 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: far_trampoline (MegaROM far-call or resident-call trampoline). No external label references were found, making this a candidate for a future feature-specific patch rule once block ownership and invariants are added. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `init_sound_system` lines 31934-31959: `init_sound_system` looks like audio runtime (26 lines, 880 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: boot_or_init (Boot/init routine). External references still exist (1): init_sound_system_far@14735. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `task_audio_tick` lines 31960-31987: `task_audio_tick` looks like audio runtime (28 lines, 795 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: shared_runtime (Shared low-level runtime helper). External references still exist (1): task_audio_tick_far@14753. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `sfx_silence_all` lines 32106-32131: `sfx_silence_all` looks like audio runtime (26 lines, 757 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). External references still exist (2): init_sound_system@31951, sfx_update@32389. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `sfx_beep` lines 32132-32152: `sfx_beep` looks like audio runtime (21 lines, 492 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). External references still exist (1): sfx_damage@32294. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `sfx_jump` lines 32153-32174: `sfx_jump` looks like audio runtime (22 lines, 473 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). External references still exist (1): sfx_damage@32300. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `sfx_shoot` lines 32175-32199: `sfx_shoot` looks like audio runtime (25 lines, 565 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). External references still exist (1): sfx_damage@32306. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `sfx_explosion` lines 32200-32219: `sfx_explosion` looks like audio runtime (20 lines, 497 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). External references still exist (1): sfx_damage@32312. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `sfx_coin` lines 32220-32241: `sfx_coin` looks like audio runtime (22 lines, 536 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). External references still exist (1): sfx_damage@32318. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
...
- [report-only] `sfx_play` lines 32335-32363: `sfx_play` looks like audio runtime (29 lines, 600 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). External references still exist (6): sfx_damage@32296, sfx_damage@32302, sfx_damage@32308, sfx_damage@32314, sfx_damage@32320, ... (+1 more). Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `sfx_update` lines 32364-32401: `sfx_update` looks like audio runtime (38 lines, 939 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). External references still exist (1): sfx_update_far@14771. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `music_init_system` lines 32402-32422: `music_init_system` looks like audio runtime (21 lines, 566 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). External references still exist (2): init_sound_system@31948, music_stop@32442. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `music_reset_channel_state` lines 32423-32426: `music_reset_channel_state` looks like audio runtime (4 lines, 89 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). No external label references were found, making this a candidate for a future feature-specific patch rule once block ownership and invariants are added. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `music_silence_channels` lines 32427-32440: `music_silence_channels` looks like audio runtime (14 lines, 212 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). External references still exist (1): music_stop@32443. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
...
- [report-only] `music_play_track` lines 32450-32452: `music_play_track` looks like audio runtime (3 lines, 27 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). External references still exist (1): music_play_track_far@14826. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
- [report-only] `music_execute_command` lines 32453-32460: `music_execute_command` looks like audio runtime (8 lines, 98 source bytes), but project_usage marks feature `sounds` disabled (sounds=0, tracks=0). Category: runtime_code (Instruction-bearing runtime label). External references still exist (1): music_execute_command_far@14845. Deletion must stay blocked until callers are proven dead or rewired. This is report-only until the generator metadata and invariants prove deletion is safe.
