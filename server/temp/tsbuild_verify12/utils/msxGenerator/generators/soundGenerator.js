...
exports.getSerializedTrackerMusicBufferSize = getSerializedTrackerMusicBufferSize;
exports.getSoundBank4Data = getSoundBank4Data;
const megaromResourceArtifacts_1 = require("../utils/megaromResourceArtifacts");
const ASM_NOTE_KEEP = 0xFF;
const ASM_NOTE_CUT = 0xFE;
const TRACK_CHANNELS = ['A', 'B', 'C'];
function countAsmDataBytes(asm) {
 ...
 */
function generateSoundFile(analysis, executionPlan, romMode = 'simple32k') {
...
function clampByte(value, min = 0, max = 255) {
    const safeValue = Number.isFinite(value) ? Math.round(value) : min;
    return Math.max(min, Math.min(max, safeValue));
}
function clampWord(value, min = 0, max = 0xffff) {
    const safeValue = Number.isFinite(value) ? Math.round(value) : min;
    return Math.max(min, Math.min(max, safeValue));
}
...
function computeRowFrames(song) {
...
function isPT3Instrument(value) {
    return !!value && !Array.isArray(value.waveform);
}
function collectPsgTracks(analysis) {
...
function collectPT3Tracks(analysis) {
    const projectTracks = Array.isArray(analysis.tracks) ? analysis.tracks : [];
    return projectTracks.filter((track) => track?.playbackBackend === 'external-pt3');
}
function collectSerializedTrackerTracks(analysis) {
    const pt3Tracks = collectPT3Tracks(analysis);
    return pt3Tracks.length > 0 ? [] : collectPsgTracks(analysis);
}
function getSerializedTrackerMusicBufferSize(analysis) {
...
function getSoundBank4Data(analysis) {
    const tracks = collectSerializedTrackerTracks(analysis);
    if (tracks.length === 0) {
        return '';
    }
...
function getInstrumentMap(song) {
    const instrumentMap = new Map();
    for (const instrument of song.instruments || []) {
        if (!isPT3Instrument(instrument))
            continue;
...
function encodeInstrumentField(value, instrumentMap, warnings, context) {
...
function encodeOrnamentField(value, ornamentMap, warnings, context) {
    if (value === null || value === undefined)
        return 0xff;
    if (value === 0)
        return 0;
...
function encodeVolumeField(value, warnings, context) {
    if (value === null || value === undefined)
        return 0xff;
...
function buildDbLines(label, values) {
    const lines = [`${label}:`];
    if (values.length === 0) {
        lines.push('    DB #00');
        return lines.join('\n');
...
function buildDwLines(label, values) {
    const lines = [`${label}:`];
    if (values.length === 0) {
        lines.push('    DW #0000');
        return lines.join('\n');
...
function serializeSignedByteArray(values) {
    return values.map((value) => clampByte(value & 0xff));
}
function normalizeVolumeEnvelopeData(values) {
...
function buildTrackData(song, trackIndex, options = {}) {
    ...
    lines.push(`    DW ${useRelativePointers ? `${labelBase}_order_table - ${dataLabel}` : `${labelBase}_order_table`}`);
    lines.push(`    DW ${useRelativePointers ? `${labelBase}_pattern_table - ${dataLabel}` : `${labelBase}_pattern_table`}`);
    lines.push(`    DW ${useRelativePointers ? `${labelBase}_instrument_ptr_table - ${dataLabel}` : `${labelBase}_instrument_ptr_table`}`);
    lines.push(`    DW ${useRelativePointers ? `${labelBase}_ornament_ptr_table - ${dataLabel}` : `${labelBase}_ornament_ptr_table`}`);
    ...
    patterns.forEach((pattern, patternIndex) => {
        lines.push(`    DW ${useRelativePointers ? `${labelBase}_pattern_${patternIndex}_rows - ${dataLabel}` : `${labelBase}_pattern_${patternIndex}_rows`}`);
    ...
    for (let instrumentId = 0; instrumentId <= 31; instrumentId++) {
        lines.push(`    DW ${instrumentId > 0 && instrumentMap.has(instrumentId)
            ? (useRelativePointers ? `${labelBase}_inst_${instrumentId} - ${dataLabel}` : `${labelBase}_inst_${instrumentId}`)
            : '0'}`);
    }
    lines.push('');
    ...
    for (let ornamentId = 0; ornamentId <= 15; ornamentId++) {
        lines.push(`    DW ${ornamentId > 0 && ornamentMap.has(ornamentId)
            ? (useRelativePointers ? `${labelBase}_orn_${ornamentId} - ${dataLabel}` : `${labelBase}_orn_${ornamentId}`)
            : '0'}`);
    }
    lines.push('');
    patterns.forEach((pattern, patternIndex) => {
    ...
    Array.from(instrumentMap.entries()).sort((left, right) => left[0] - right[0]).forEach(([instrumentId, instrument]) => {
        ...
        lines.push(`    DW ${volumeEnvelope.length > 0
            ? (useRelativePointers ? `${labelBase}_inst_${instrumentId}_vol_env - ${dataLabel}` : `${labelBase}_inst_${instrumentId}_vol_env`)
            : '0'}`);
        lines.push(`    DB ${toAsmByte(volumeEnvelope.length)}`);
        lines.push(`    DB ${toAsmByte(volumeLoop)}`);
        lines.push(`    DW ${toneEnvelope.length > 0
            ? (useRelativePointers ? `${labelBase}_inst_${instrumentId}_tone_env - ${dataLabel}` : `${labelBase}_inst_${instrumentId}_tone_env`)
            : '0'}`);
        lines.push(`    DB ${toAsmByte(toneEnvelope.length)}`);
        lines.push(`    DB ${toAsmByte(toneLoop)}`);
        lines.push(`    DW ${noiseEnvelope.length > 0
            ? (useRelativePointers ? `${labelBase}_inst_${instrumentId}_noise_env - ${dataLabel}` : `${labelBase}_inst_${instrumentId}_noise_env`)
            : '0'}`);
    ...
    Array.from(ornamentMap.entries()).sort((left, right) => left[0] - right[0]).forEach(([ornamentId, ornament]) => {
        ...
        lines.push(`    DW ${ornamentData.length > 0
            ? (useRelativePointers ? `${labelBase}_orn_${ornamentId}_data - ${dataLabel}` : `${labelBase}_orn_${ornamentId}_data`)
            : '0'}`);
        lines.push(`    DB ${toAsmByte(ornamentData.length)}`);
        lines.push(`    DB ${toAsmByte(loopPosition)}`);
        if (ornamentData.length > 0)
            lines.push(buildDbLines(`${labelBase}_orn_${ornamentId}_data`, ornamentData));
        lines.push('');
    });
...
function buildPT3MusicBlock(tracks) {
    const lines = [
        '; ==================================================================',
        '; PT3 MUSIC BACKEND',
...
function buildTrackerMusicBlock(tracks, bankedTrackData = false) {
    ...
    if (serializedTracks.length === 0) {
        ...
        serializedTracks.forEach((track) => {
            if (bankedTrackData) {
                trackTableLines.push(`    DB ${(0, megaromResourceArtifacts_1.buildResourceIdLabelFromAsmLabel)(track.dataLabel)}`);
            }
            else {
