...
; BANK 00 @#0A6D : worlds.asm (1821 bytes)
; BANK 00 @#118A : screens.asm (3702 bytes)
; BANK 01 @#0000 : screens.asm (3768 bytes)
; BANK 01 @#0EB8 : sprites.asm (2569 bytes)
; BANK 01 @#18C1 : font.asm (1855 bytes)
; BANK 02 @#0000 : font.asm (4719 bytes)
; BANK 02 @#126F : hud.asm (3473 bytes)
; BANK 03 @#0000 : hud.asm (7 bytes)
; BANK 03 @#0007 : menus.asm (168 bytes)
; BANK 03 @#00AF : sound.asm part 1/2 (8017 bytes)
; BANK 04 @#0000 : sound.asm part 2/2 (1524 bytes)
; BANK 04 @#05F4 : scroll.asm (2353 bytes)
; BANK 04 @#0F25 : animtiles.asm (4315 bytes)
; BANK 05 @#0000 : animtiles.asm (298 bytes)
; BANK 05 @#012A : statemachine.asm (5 bytes)
; BANK 05 @#012F : gameflow.asm (6021 bytes)
...
; ==================================================================
tile_pattern_bank0:
    ; ZX0 compressed tile_pattern (40 -> 25 bytes)
    DB #26,#00,#7F,#28,#00,#FB,#98,#00,#BF,#BE,#00,#FD,#ED,#DC,#7E,#ED
    DB #DC,#63,#7E,#42,#FD,#FF,#F8,#55,#56
load_pattern_bank0:
...
; ==================================================================
tile_color_bank0:
    ; ZX0 compressed tile_color (40 -> 15 bytes)
    DB #86,#91,#B8,#81,#F8,#E5,#FE,#A6,#81,#8A,#91,#A1,#5D,#55,#56
load_color_bank0:
...
SCREEN_PAN1_0_BLOCK_CATALOG_COUNT EQU 14
SCREEN_PAN1_0_BLOCK_CATALOG_SIZE EQU 224
...
SCREEN_PAN1_0_BLOCK_TOTAL_SIZE EQU 272
...
SCREEN_PAN1_0_BLOCK_CATALOG:
    ...
    DB #FF,#FF,#FF,#80,#FF,#FF,#FF,#82,#FF,#FF,#FF,#80,#FF,#FF,#FF,#82
    DB #80,#81,#FF,#FF,#82,#83,#FF,#FF,#80,#81,#FF,#FF,#82,#83,#80,#81
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#80,#81,#80,#81
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#84,#FF,#80,#FF,#84,#FF
    DB #80,#81,#82,#83,#82,#83,#80,#81,#80,#81,#82,#83,#82,#83,#80,#81
    DB #82,#83,#82,#83,#80,#81,#80,#81,#82,#83,#82,#83,#80,#81,#80,#81
    DB #82,#FF,#84,#FF,#80,#FF,#84,#FF,#82,#FF,#84,#FF,#80,#FF,#84,#FF
    DB #FF,#FF,#84,#FF,#FF,#FF,#84,#FF,#FF,#FF,#84,#FF,#80,#81,#80,#81
    DB #80,#81,#82,#83,#82,#83,#81,#80,#80,#81,#83,#82,#82,#83,#80,#81
    DB #82,#83,#82,#83,#81,#80,#81,#80,#83,#82,#83,#82,#80,#81,#80,#81
    DB #FF,#FF,#FF,#FF,#80,#81,#80,#81,#82,#83,#82,#83,#80,#81,#80,#81
    DB #FF,#FF,#FF,#80,#80,#81,#80,#81,#82,#83,#82,#83,#80,#81,#80,#81
...
SCREEN_PAN1_0_BLOCK_MAP:
    ...
    DB #06,#07,#07,#07,#07,#08,#01,#02,#03,#04,#04,#04,#04,#09,#01,#02
    DB #0A,#0B,#0B,#0B,#0B,#0B,#01,#02,#06,#07,#07,#07,#07,#07,#0C,#0D
...
BEHAVIOR_PAN1_0_DATA:
    ; ZX0 compressed behavior (768 -> 26 bytes)
    DB #A9,#10,#00,#42,#E5,#10,#C0,#55,#E4,#FE,#3D,#C0,#55,#DD,#00,#54
    DB #FE,#FE,#C0,#41,#5E,#FE,#55,#75,#55,#58
...
hud_imported_frame_pan1_776511902784_data:
    ; Imported HUD frame snapshot for pan1 (128 cells)
    DB #00,#00,#80
    DB #01,#00,#81
    DB #02,#00,#00
    DB #03,#00,#00
...
hud_imported_frame_pan1_776511902784_draw:
    ; Draw imported HUD frame chars into Name Table
    ld hl, hud_imported_frame_pan1_776511902784_data
    ld bc, 128

hud_imported_frame_pan1_776511902784_draw_loop:
    ld a, b
    or c
    ret z
    ...
    dec bc
    jr hud_imported_frame_pan1_776511902784_draw_loop

load_screen_pan1_776511902784:
    ; Load pan1 screen (fast direct port access)
    ; Active Area: X=0, Y=4, W=32, H=20
...
.load_pan1_776511902784_zones_done:
    ...
    ld hl, runtime_screen_layout + 128
    ld de, NAMETBL + 128
    ld bc, 640
    call FAST_LDIRVM
    ld a, 0
...
FONT_PATTERN_DATA:
    ; ZX0 compressed font_pattern (1024 -> 655 bytes)
    DB #96,#00,#02,#7E,#81,#A5,#81,#BD,#99,#81,#7E,#81,#89,#FF,#DB,#FF
    DB #C3,#E7,#FF,#7E,#6C,#FE,#78,#7C,#38,#10,#00,#10,#38,#7C,#F0,#BF
    DB #38,#F7,#DE,#88,#7C,#10,#7E,#F9,#91,#08,#18,#3C,#3C,#18,#83,#FF
    DB #FF,#E7,#C3,#A0,#A0,#2E,#FF,#00,#3C,#66,#42,#42,#66,#3C,#87,#39
    DB #99,#BD,#71,#7B,#C3,#FF,#0F,#07,#0F,#7D,#CC,#FF,#78,#F9,#D3,#FF
    DB #B1,#0E,#7E,#18,#3F,#33,#3F,#30,#30,#70,#F0,#E0,#7F,#63,#FD,#00
    DB #A5,#63,#67,#E6,#C0,#99,#5A,#3C,#E7,#E7,#3C,#5A,#99,#80,#E0,#F8
    DB #FE,#A3,#E0,#80,#00,#02,#0E,#3E,#FE,#0E,#02,#BF,#54,#A9,#A3,#FA
    DB #9D,#95,#FC,#88,#00,#78,#7F,#DB,#DB,#7B,#1B,#FF,#6B,#00,#3E,#63
    DB #38,#6C,#38,#D8,#61,#A8,#FF,#FF,#B0,#EF,#B2,#FF,#F0,#EF,#FE,#E1
    DB #FD,#FF,#E0,#A7,#D2,#0C,#FE,#0C,#E2,#F3,#F1,#30,#60,#8B,#60,#30
    DB #A8,#FE,#C0,#BA,#FE,#F0,#19,#24,#66,#FF,#66,#24,#ED,#9E,#B7,#A4
    DB #8E,#FF,#A2,#FE,#79,#A3,#8E,#78,#78,#30,#FD,#F8,#6C,#FF,#E0,#EF
    DB #F3,#FE,#FD,#E1,#F7,#D1,#89,#7C,#C0,#78,#0C,#F8,#6A,#00,#C6,#CC
    DB #18,#30,#66,#C6,#38,#86,#38,#76,#DC,#CC,#76,#8B,#60,#60,#8F,#B0
    DB #D7,#FF,#ED,#31,#6F,#F9,#EE,#01,#E8,#4F,#2D,#66,#3C,#F8,#19,#5E
    DB #8F,#30,#FC,#46,#7E,#CF,#EC,#A7,#FC,#D9,#E0,#FD,#00,#06,#0C,#A5
    DB #7B,#96,#11,#7C,#C6,#CE,#DE,#F6,#E6,#7C,#CF,#C9,#D7,#E4,#FF,#BB
    DB #E6,#78,#CC,#0C,#38,#60,#CC,#F0,#AA,#0C,#78,#0D,#1C,#3C,#6C,#CC
    DB #B9,#5F,#1E,#87,#FF,#C0,#F8,#0C,#E0,#C7,#CB,#EF,#52,#FF,#E1,#C1
    DB #7F,#FF,#FF,#B1,#E7,#FA,#EE,#F0,#7C,#DD,#FE,#70,#48,#F8,#5B,#60
    DB #F7,#34,#C0,#FF,#02,#FA,#EF,#E8,#0C,#FF,#40,#F3,#E4,#BF,#01,#8E
    DB #82,#BA,#AA,#BA,#80,#78,#6E,#A3,#FC,#CA,#BB,#0F,#7C,#3D,#AF,#CB
    DB #79,#90,#5A,#60,#35,#F8,#6C,#A2,#F0,#6C,#F8,#97,#FE,#62,#68,#78
    DB #68,#62,#FE,#98,#F0,#E2,#60,#F0,#C0,#AB,#CE,#3E,#FF,#9B,#92,#91
    DB #3F,#85,#FE,#8F,#71,#1E,#0C,#FF,#7B,#E0,#E6,#B9,#97,#78,#8B,#CA
    DB #E6,#00,#F0,#C2,#72,#62,#66,#FE,#83,#0A,#EE,#FE,#FE,#D6,#E0,#C6
    DB #F1,#B3,#E6,#F6,#DE,#CE,#70,#BA,#C6,#F5,#6C,#E3,#20,#FE,#BD,#61
    DB #03,#01,#61,#DC,#78,#1C,#FE,#90,#E1,#77,#E0,#70,#1C,#30,#B9,#B4
    DB #50,#EF,#C2,#2A,#E7,#F0,#F9,#27,#71,#FF,#FF,#D6,#FE,#EE,#71,#76
    DB #FF,#69,#F1,#D4,#E1,#B0,#FB,#FE,#C6,#8C,#18,#32,#21,#81,#60,#FD
    DB #FE,#E1,#E7,#06,#0C,#06,#02,#BF,#E1,#18,#FE,#BF,#E1,#10,#A6,#86
    DB #FE,#F4,#FF,#99,#DD,#F6,#6D,#33,#7C,#6B,#50,#E0,#DA,#97,#00,#F8
    DB #DC,#E0,#CE,#CC,#C0,#00,#EF,#0C,#D2,#D1,#88,#E0,#8F,#FC,#C0,#83
    DB #77,#A7,#A0,#EC,#FF,#76,#EE,#1C,#F8,#A1,#22,#6C,#76,#AB,#E6,#30
    DB #5F,#7E,#1A,#C3,#0C,#C9,#FE,#F3,#D1,#FE,#FF,#D1,#D4,#FB,#D0,#FF
    DB #CC,#3E,#FE,#90,#AE,#F8,#61,#87,#CC,#FB,#FF,#D0,#DC,#6F,#1C,#5F
    DB #CB,#60,#1E,#BE,#E0,#61,#40,#34,#8D,#0E,#C7,#A1,#FF,#89,#81,#34
    DB #18,#F6,#C3,#F0,#76,#00,#62,#00,#00,#71,#FB,#BF,#F0,#6C,#67,#5A
    DB #DF,#D3,#E0,#61,#91,#8E,#FC,#98,#30,#64,#FC,#11,#8A,#E0,#1C,#C3
    DB #C8,#3D,#E1,#8F,#00,#E1,#1A,#43,#36,#FA,#FE,#7E,#5D,#55,#56
FONT_CHAR_INDEX:
...
FONT_COLOR_DATA:
    ; ZX0 compressed font_color (1024 -> 6 bytes)
    DB #95,#F0,#55,#5D,#55,#56
load_font_colors:
...
load_world_worldmap_1776512078647:
    ...
    call hud_imported_frame_pan1_776511902784_draw
