...
; BANK 05 @#146D : statemachine.asm part 1/2 (2963 bytes)
; BANK 06 @#0000 : statemachine.asm part 2/2 (8192 bytes)
; BANK 07 @#0000 : statemachine.asm part 3/2 (2236 bytes)
; BANK 07 @#08BC : gameflow.asm (5956 bytes)
; BANK 08 @#0000 : gameflow.asm (65 bytes)
...
sprite_asset_base_pattern_slot_runtime EQU #E1DB   ; Runtime base 16x16 slot per sprite asset (4 bytes)
sprite_placeholder_base_pattern_num EQU #E1DF   ; Runtime placeholder pattern number (base slot * 4)
current_sprite_pattern_pack_id EQU #E1E0   ; Active runtime sprite pattern pack id (#FF=none loaded)
sprite_attributes   EQU #E1E1   ; Interleaved sprite attributes (32 * 4 bytes)

; ==================================================================
; SCREEN SYSTEM VARIABLES (1 screens detected)
; ==================================================================
current_screen_id   EQU #E261   ; Currently displayed screen ID
screen_dirty_flag   EQU #E262   ; Screen needs redraw flag
screen_transition_cooldown EQU #E263   ; Cooldown frames after screen transition
current_world_id    EQU #E264   ; Current world ID (for multi-world support)
current_screen_index EQU #E265   ; Current screen index within world
current_screen_anim_group_count EQU #E266   ; Animated tile groups visible in current screen
current_screen_entity_count EQU #E267   ; Entity instances assigned to current screen
current_screen_sprite_pattern_slots EQU #E268   ; Sprite pattern slots needed by current screen
current_screen_summary_flags EQU #E269   ; Runtime screen summary flags (music/hud/effects/anim)

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #E26A   ; Player X position (16-bit)
player_y            EQU #E26C   ; Player Y position (16-bit)
player_runtime_enabled EQU #E26E   ; 1=player fast runtime bound to hero entity
player_entity_index EQU #E26F   ; Entity index used by player fast runtime (#FF=none)
player_vx_runtime   EQU #E270   ; Cached player X velocity (signed 8-bit)
player_vy_runtime   EQU #E271   ; Cached player Y velocity (signed 8-bit)
player_health       EQU #E272   ; Player health points
player_score        EQU #E273   ; Player score (16-bit)
gem_count           EQU #E275   ; Collectible tile counter (8-bit)
last_interaction_char EQU #E276   ; Char code of last interacted tile (for SM VARIABLE_COMPARE)
last_gem_char       EQU last_interaction_char   ; Backwards-compatible alias for collectible SM checks
last_interaction_pending EQU #E277   ; 1 when a new tile interaction is pending for State Machine logic
last_interaction_type EQU #E278   ; Interaction type id of last interacted tile
last_interaction_value EQU #E279   ; Interaction value byte of last interacted tile
last_interaction_target EQU #E27A   ; Interaction target id of last interacted tile
last_interaction_x  EQU #E27B   ; Tile X coordinate of last interaction
last_interaction_y  EQU #E27C   ; Tile Y coordinate of last interaction
last_interaction_entity EQU #E27D   ; Entity index that triggered the last interaction

; Persistent collectibles list (survives screen re-entry)
MAX_COLLECTIBLES     EQU 64              ; Max persistent collectible records
collected_count      EQU #E27E   ; Number of collected tiles recorded (8-bit)
collected_world      EQU #E27F   ; World IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_screen     EQU #E2BF   ; Screen IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_idx_l      EQU #E2FF   ; Tile name-table index low byte (MAX_COLLECTIBLES bytes)
collected_idx_h      EQU #E33F   ; Tile name-table index high byte (MAX_COLLECTIBLES bytes)

; Timed bonus tile respawn slots (bonus gem regeneration)
MAX_BONUS_RESPAWNS   EQU 16              ; Max timed bonus tiles waiting to respawn
bonus_respawn_world  EQU #E37F   ; World IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_screen EQU #E38F   ; Screen IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_l  EQU #E39F   ; Tile index low byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_h  EQU #E3AF   ; Tile index high byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_secs   EQU #E3BF   ; Remaining seconds per timed respawn slot (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_frames EQU #E3CF   ; Frame countdown (60..1) per timed respawn slot (MAX_BONUS_RESPAWNS bytes)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #E3DF   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #E3E0   ; Temporary 16-bit storage
temp_word_2         EQU #E3E2   ; Temporary 16-bit storage
temp_byte_1         EQU #E3E4   ; Temporary 8-bit storage
temp_byte_2         EQU #E3E5   ; Temporary 8-bit storage
temp_byte_3         EQU #E3E6   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #E406   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #E426   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #E446   ; Temporary 8-bit storage (32 bytes)

; ==================================================================
; SOUND SYSTEM VARIABLES
; ==================================================================
sfx_active          EQU #E466   ; 0=no SFX active, 1=playing
sfx_timer           EQU #E467   ; Frames remaining for current SFX
sfx_fadeout         EQU #E468   ; Reserved fadeout flag/state
temp_byte_7         EQU #E469   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #E489   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #E4A9   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #E4C9   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #E4E9   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #E509   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #E529   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #E549   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #E569   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #E589   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #E5A9   ; Temporary 8-bit storage (32 bytes)
temp_byte_18        EQU #E5C9   ; Temporary 8-bit storage (32 bytes)
temp_byte_19        EQU #E5E9   ; Temporary 8-bit storage (32 bytes)
temp_byte_20        EQU #E609   ; Temporary 8-bit storage (32 bytes)
temp_byte_21        EQU #E629   ; Temporary 8-bit storage (32 bytes)
temp_byte_22        EQU #E649   ; Temporary 8-bit storage (32 bytes)
temp_byte_23        EQU #E669   ; Temporary 8-bit storage (32 bytes)
temp_byte_24        EQU #E689   ; Temporary 8-bit storage (32 bytes)
temp_byte_25        EQU #E6A9   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #E6C9   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #E709   ; Temporary 16-bit storage (64 bytes)
temp_byte_26        EQU #E749   ; Temporary 8-bit storage (32 bytes)
temp_byte_27        EQU #E769   ; Temporary 8-bit storage (32 bytes)
temp_byte_28        EQU #E789   ; Temporary 8-bit storage (32 bytes)
tileDead_dbg        EQU #E7A9   ; Debug byte: current hero deadly contact
tileDead_latched_dbg EQU #E7AA   ; Debug byte: latched hero deadly contact
tileDead_x_dbg      EQU #E7AB   ; Debug byte: last sampled deadly tile X
tileDead_y_dbg      EQU #E7AC   ; Debug byte: last sampled deadly tile Y
tileDead_value_dbg  EQU #E7AD   ; Debug byte: last raw deadly behavior value

; Wall collision temporary variables
wall_temp_x         EQU #E7AE   ; Cached entity X for wall checks
wall_temp_y         EQU #E7AF   ; Cached entity Y for wall checks
wall_hit_left       EQU #E7B0   ; Hitbox left edge cache
wall_hit_top        EQU #E7B1   ; Hitbox top edge cache
wall_hit_right      EQU #E7B2   ; Hitbox right edge cache
wall_hit_bottom     EQU #E7B3   ; Hitbox bottom edge cache
wall_hit_w          EQU #E7B4   ; Hitbox width cache (min 1)
wall_hit_h          EQU #E7B5   ; Hitbox height cache (min 1)
wall_probe_left     EQU #E7B6   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #E7B7   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #E7B8   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #E7B9   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #E7BA   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #E7DA   ; Number of entries in active_entity_list
hero_entity_id      EQU #E7DB   ; First current-screen entity flagged as player (#FF = none)
active_entity_list_dirty EQU #E7DC   ; 1=rebuild active_entity_list required
input_entity_list   EQU #E7DD   ; Active current-screen entities with Input component (MAX_ENTITIES bytes)
input_entity_count  EQU #E7FD   ; Number of entries in input_entity_list
render_entity_list  EQU #E7FE   ; Active current-screen entities with Sprite component (MAX_ENTITIES bytes)
render_entity_count EQU #E81E   ; Number of entries in render_entity_list
collision_entity_list EQU #E81F   ; Active current-screen entities with Collision component (MAX_ENTITIES bytes)
collision_entity_count EQU #E83F   ; Number of entries in collision_entity_list
ground_entity_list  EQU #E840   ; Active current-screen entities with Collision or Gravity (MAX_ENTITIES bytes)
ground_entity_count EQU #E860   ; Number of entries in ground_entity_list
anim_entity_list    EQU #E861   ; Active current-screen entities with Animation+Sprite (MAX_ENTITIES bytes)
anim_entity_count   EQU #E881   ; Number of entries in anim_entity_list

; Entity-entity collision optimized variables
coll_list           EQU #E882   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #E8A2   ; Number of entities in coll_list
coll_src_left       EQU #E8A3   ; Source AABB left edge (scratch)
coll_src_right      EQU #E8A4   ; Source AABB right edge (scratch)
coll_src_top        EQU #E8A5   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #E8A6   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #E8A7   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #E8A7   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #E8A9   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #E8AB   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #E8AD   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #E8AF   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #E8B1   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #E8B3   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #E8B5   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #E8B7   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #E8B8   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #E8BD   ; Frame counter (16-bit)
task_exec_time          EQU #E8BF   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #E8C1   ; Set to 1 on each VBlank (1 byte)
RAM_INTERRUPT_END       EQU #E8C2   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #E8C2   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #E8C3   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #E8C4   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #E8C5   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #E8C6   ; 0=stopped, 1=track active
music_muted          EQU #E8C7   ; 0=audible, 1=muted/pause
music_loop           EQU #E8C8   ; 0=no loop, 1=loop enabled
music_track_index    EQU #E8C9   ; Current ROM track index
music_row_frames     EQU #E8CA   ; Frames per tracker row
music_row_countdown  EQU #E8CB   ; Countdown to next row
music_order_pos      EQU #E8CC   ; Current order position
music_pattern_index  EQU #E8CD   ; Current pattern index
music_pattern_row    EQU #E8CE   ; Current row inside pattern
music_pattern_rows   EQU #E8CF   ; Cached rows in current pattern
music_track_ptr_l    EQU #E8D0   ; Current track pointer low byte
music_track_ptr_h    EQU #E8D1   ; Current track pointer high byte
music_pattern_ptr_l  EQU #E8D2   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #E8D3   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #E8D4   ; PSG mixer shadow for music runtime
...
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #E8ED   ; End of project variables (10477 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#E8ED: Project variables (10477 bytes)
;   #E8ED-#F37F: Free RAM (~2707 bytes available)
