...
; BANK 02 @#0000 : screens.asm part 3/2 (448 bytes)
; BANK 02 @#01C0 : sprites.asm (2823 bytes)
; BANK 02 @#0CC7 : font.asm (177 bytes)
; BANK 02 @#0D78 : hud.asm (79 bytes)
; BANK 02 @#0DC7 : menus.asm (168 bytes)
; BANK 02 @#0E6F : sound.asm part 1/2 (4497 bytes)
; BANK 03 @#0000 : sound.asm part 2/2 (6787 bytes)
; BANK 03 @#1A83 : scroll.asm (1405 bytes)
; BANK 04 @#0000 : scroll.asm (937 bytes)
; BANK 04 @#03A9 : animtiles.asm (5448 bytes)
; BANK 04 @#18F1 : bosses.asm part 1/3 (1807 bytes)
; BANK 05 @#0000 : bosses.asm part 2/3 (8192 bytes)
; BANK 06 @#0000 : bosses.asm part 3/3 (8192 bytes)
; BANK 07 @#0000 : bosses.asm part 4/3 (4224 bytes)
; BANK 07 @#1080 : statemachine.asm (5 bytes)
; BANK 07 @#1085 : gameflow.asm (3963 bytes)
; BANK 08 @#0000 : gameflow.asm (2101 bytes)
...
mapper_bank_p1_current EQU #C115   ; Mapper current bank for page/window 1
mapper_bank_p2_current EQU #C116   ; Mapper current bank for page/window 2
mapper_bank_p3_current EQU #C117   ; Mapper current bank for page/window 3
mapper_bank_p4_current EQU #C118   ; Mapper current bank for page/window 4
mapper_saved_bank    EQU #C119   ; Saved mapper bank for push/pop helpers
mapper_saved_bank_p1 EQU #C11A   ; Saved mapper bank for page/window 1 helpers
mapper_saved_bank_p3 EQU #C11B   ; Saved mapper bank for page/window 3 helpers
mapper_saved_bank_p4 EQU #C11C   ; Saved mapper bank for page/window 4 helpers
resource_descriptor_ptr EQU #C11D   ; Pointer to cached resource descriptor entry (16-bit)
resource_descriptor_id EQU #C11F   ; Cached resource id
resource_descriptor_type EQU #C120   ; Cached resource type
resource_descriptor_group EQU #C121   ; Cached resource group
resource_descriptor_bank EQU #C122   ; Cached resource bank
resource_descriptor_addr EQU #C123   ; Cached resource visible address (16-bit)
resource_descriptor_size EQU #C125   ; Cached resource size (16-bit)
vram_cache_tile_patterns_ready EQU #C127   ; 1 when shared gameplay tile patterns are already resident in VRAM
vram_cache_tile_colors_ready EQU #C128   ; 1 when shared gameplay tile colors are already resident in VRAM
vram_cache_font_ready EQU #C129   ; 1 when shared font patterns/colors are already resident in VRAM
current_screen2_tilebank_id EQU #C12A   ; Current SCREEN 2 shared tilebank loaded in VRAM (#FF=none/unknown)
frame_counter       EQU #C12B   ; Frame counter (16-bit)

; Profiling counters (16-bit, cumulative)
prof_update_all_entities_calls EQU #C12D   ; Calls to update_all_entities
prof_execute_sm_calls EQU #C12F   ; Calls to execute_all_state_machines
prof_sm_update_calls  EQU #C131   ; Calls to SM_Update
prof_collision_calls  EQU #C133   ; Calls to update_collision_component
prof_wall_calls       EQU #C135   ; Calls to update_wallcollision_component
prof_deadly_calls     EQU #C137   ; Calls to update_deadly_tiles_component
prof_tile_interaction_calls EQU #C139   ; Calls to check_tile_interaction
prof_animation_calls  EQU #C13B   ; Calls to update_animation_component
prof_sprite_calls     EQU #C13D   ; Calls to update_sprite_component
prof_music_task_calls EQU #C13F   ; Calls to task_update_music
prof_deadly_behavior_reads EQU #C141   ; Deadly helper behavior-map reads
...
runtime_interaction_type_map EQU #CA4E   ; Mutable copy of current interaction type map (32x24)
runtime_interaction_value_map EQU #CD4E   ; Mutable copy of current interaction value map (32x24)
runtime_interaction_target_map EQU #D04E   ; Mutable copy of current interaction target map (32x24)
runtime_char_behavior_table EQU #D34E   ; Current screen char -> behavior lookup table (256 bytes)
runtime_effects_layout EQU #D44E   ; Alternate effects layout copy for secret zones (32x24)
screen_block_catalog_ptr EQU #D74E   ; Scratch pointer to current screen block catalog during layout expansion
screen_block_map_ptr EQU #D750   ; Scratch pointer to current screen block index map during layout expansion
runtime_effect_zone_table EQU #D752   ; Current screen effect zone table (0 bytes)
current_effect_zone_count EQU #D752   ; Number of effect zones copied into runtime_effect_zone_table
secret_zone_active EQU #D753   ; 1 if hero is currently inside an active secret zone
secret_zone_rect_x EQU #D754   ; Active secret zone rect X in cells
secret_zone_rect_y EQU #D755   ; Active secret zone rect Y in cells
secret_zone_rect_w EQU #D756   ; Active secret zone rect width in cells
secret_zone_rect_h EQU #D757   ; Active secret zone rect height in cells

; ==================================================================
; VIEWPORT/CAMERA VARIABLES (for scroll system)
; ==================================================================
camera_x            EQU #D758   ; Camera X position in pixels (16-bit)
camera_y            EQU #D75A   ; Camera Y position in pixels (16-bit)
camera_tile_x       EQU #D75C   ; Camera tile X (column)
camera_tile_y       EQU #D75D   ; Camera tile Y (row)
world_width_tiles   EQU #D75E   ; World width in tiles
world_height_tiles  EQU #D75F   ; World height in tiles
scroll_dirty_flag   EQU #D760   ; 1=viewport changed, needs redraw
hud_dirty_flag      EQU #D761   ; 1=HUD needs redraw, 0=clean
time_second_frame_counter EQU #D762   ; VBlank frames remaining until the next TimeRemaining decrement
time_last_interrupt_counter EQU #D763   ; Last interrupt_counter snapshot used by TimeRemaining sync (16-bit)

; ==================================================================
; ANIMATED TILES VARIABLES
; ==================================================================
anim_tile_timer     EQU #D765   ; Animation frame timer
anim_tile_frame     EQU #D766   ; Current animation frame (0-3)
anim_tile_speed     EQU #D767   ; Frames between animation updates
anim_tile_transform_flags EQU #D768   ; Runtime flags for transform-mode tile animation (byte0=flags, byte1=opcode scratch)
anim_tile_row_buffer EQU #D76A   ; Temp buffer (8 bytes) for row transforms

; ==================================================================
; ENTITY SYSTEM VARIABLES (Fixed 32 entities)
; ==================================================================
MAX_ENTITIES        EQU 32
entity_active       EQU #D772   ; Entity active flags (32 bytes, 0=inactive, 1=active)
entity_is_player    EQU #D792   ; Entity hero/player flag (32 bytes, 0=no, 1=yes)
entity_button_contact_active EQU #D7B2   ; 1 while entity stays on the same button tile (32 bytes)
entity_button_contact_x EQU #D7D2   ; Button tile X currently latched per entity (32 bytes)
entity_button_contact_y EQU #D7F2   ; Button tile Y currently latched per entity (32 bytes)
entity_on_ladder   EQU #D812   ; 1 while entity is centered on a ladder tile (32 bytes)
entity_gate_current_step EQU #D832   ; Current applied retract step (32 bytes)
entity_gate_step_timer EQU #D852   ; Countdown until next retract step (32 bytes)
entity_walljump_lock EQU #D872   ; Remaining horizontal lock frames after wall jump (32 bytes)
entity_walljump_locked_vx EQU #D892   ; Horizontal velocity preserved while wall jump lock is active (32 bytes)
entity_wallgrab_active EQU #D8B2   ; 1 if entity is currently grabbing a wall (32 bytes)
entity_wallgrab_grace EQU #D8D2   ; Frames to keep wall grab during transient wall flag gaps (32 bytes)
entity_wallgrab_timer EQU #D8F2   ; Remaining wall-grab frames until grounded reset (32 bytes)
entity_wallgrab_lockout EQU #D912   ; Wall grab disabled until grounded after timer is spent (32 bytes)
entity_walljump_anim_active EQU #D932   ; Wall jump one-shot animation is waiting to restore base sprite (32 bytes)
entity_x_pos        EQU #D952   ; Entity X positions (32 bytes)
entity_y_pos        EQU #D972   ; Entity Y positions (32 bytes)
entity_vel_x        EQU #D992   ; Entity X velocity (32 bytes)
entity_vel_y        EQU #D9B2   ; Entity Y velocity (32 bytes)
entity_comp_masks   EQU #D9D2   ; Entity component masks (32 bytes)
entity_comp_masks_hi EQU #D9F2   ; Entity component masks high byte (32 bytes)
entity_screen_id    EQU #DA12   ; Entity screen ID (32 bytes)
entity_job_period   EQU #DA32   ; Entity job period in frames (32 bytes, 1=100%,2=50%,3=33%,4=25%)
entity_job_entry    EQU #DA52   ; Entity job entry slot within period window (32 bytes)
entity_job_scheduler_active EQU #DA72   ; 1 when any entity uses non-default job cadence
entity_dir_mask     EQU #DA73   ; Entity direction mask (32 bytes)
entity_input_speed  EQU #DA93   ; Entity input/cursor speed (32 bytes)
entity_health       EQU #DAB3   ; Entity health (32 bytes)
entity_anim_frame   EQU #DAD3   ; Entity animation frame (32 bytes)
entity_anim_tick    EQU #DAF3   ; Entity animation tick counter (32 bytes)
entity_anim_speed   EQU #DB13   ; Entity animation speed (ticks per frame) (32 bytes)
entity_anim_flags   EQU #DB33   ; Entity animation flags (32 bytes)
entity_sm_ptr_l     EQU #DB53   ; Entity State Pointer Low (32 bytes)
entity_sm_ptr_h     EQU #DB73   ; Entity State Pointer High (32 bytes)
entity_sm_timer_l   EQU #DB93   ; Entity State Timer Low (32 bytes)
entity_sm_timer_h   EQU #DBB3   ; Entity State Timer High (32 bytes)
entity_sm_wait_timer EQU #DBD3   ; Entity State Wait Timer (32 bytes)
entity_sm_sprite_control EQU #DBF3   ; 1 when the assigned state machine explicitly drives sprite changes (32 bytes)
entity_lifetime     EQU #DC13   ; Entity lifetime for auto-destroy (32 bytes, 0=infinite)
entity_collectible_enabled EQU #DC33   ; 1 when entity has Collectible component (32 bytes)
entity_carried_by   EQU #DC53   ; Entity carrier ID (32 bytes, 255=not carried)
entity_template_token EQU #DC73   ; Entity template token (32 bytes, 0=unknown)
entity_facing_dir   EQU #DC93   ; Last facing direction (32 bytes, 0=none,1=left,2=right,3=up,4=down)
entity_sm_var_0     EQU #DCB3   ; Entity Variable 0 (32 bytes)
entity_sm_var_1     EQU #DCD3   ; Entity Variable 1 (32 bytes)
entity_sm_var_2     EQU #DCF3   ; Entity Variable 2 (32 bytes)
entity_sm_var_3     EQU #DD13   ; Entity Variable 3 (32 bytes)
entity_sm_var_4     EQU #DD33   ; Entity Variable 4 (32 bytes)
entity_sm_var_5     EQU #DD53   ; Entity Variable 5 (32 bytes)
entity_sm_var_6     EQU #DD73   ; Entity Variable 6 (32 bytes)
entity_sm_var_7     EQU #DD93   ; Entity Variable 7 (32 bytes)

; ==================================================================
; SPRITE SYSTEM VARIABLES
; ==================================================================
entity_sprite_asset_index EQU #DDB3   ; Entity sprite asset index - RAM copy (32 bytes)
active_sprite_count EQU #DDD3   ; Number of sprites currently active
sprites_dirty      EQU #DDD4   ; 1=sprite_attributes changed, needs VRAM sync
sprite_pattern      EQU #DDD5   ; Sprite pattern IDs (32 bytes)
sprite_color        EQU #DDF5   ; Sprite colors (32 bytes)
sprite_layer_colors EQU #DE15   ; HW sprite layer color cache - RAM copy (32 bytes, indexed by HW sprite index)
sprite_layer_y_offsets EQU #DE35   ; HW sprite layer signed Y offsets - RAM copy (32 bytes, indexed by HW sprite index)
sprite_asset_base_pattern_slot_runtime EQU #DE55   ; Runtime base 16x16 slot per sprite asset (1 bytes)
sprite_placeholder_base_pattern_num EQU #DE56   ; Runtime placeholder pattern number (base slot * 4)
current_sprite_pattern_pack_id EQU #DE57   ; Active runtime sprite pattern pack id (#FF=none loaded)
sprite_attributes   EQU #DE58   ; Interleaved sprite attributes (32 * 4 bytes)

; ==================================================================
; SCREEN SYSTEM VARIABLES (1 screens detected)
; ==================================================================
current_screen_id   EQU #DED8   ; Currently displayed screen ID
current_screen_engine EQU #DED9   ; Runtime engine: 0=Player, 1=FakePlayer
screen_dirty_flag   EQU #DEDA   ; Screen needs redraw flag
screen_transition_cooldown EQU #DEDB   ; Cooldown frames after screen transition
current_world_id    EQU #DEDC   ; Current world ID (for multi-world support)
current_screen_index EQU #DEDD   ; Current screen index within world
current_screen_anim_group_count EQU #DEDE   ; Animated tile groups visible in current screen
current_screen_entity_count EQU #DEDF   ; Entity instances assigned to current screen
current_screen_sprite_pattern_slots EQU #DEE0   ; Sprite pattern slots needed by current screen
current_screen_summary_flags EQU #DEE1   ; Runtime screen summary flags (music/hud/effects/anim)

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #DEE2   ; Player X position (16-bit)
player_y            EQU #DEE4   ; Player Y position (16-bit)
player_runtime_enabled EQU #DEE6   ; 1=player fast runtime bound to hero entity
player_entity_index EQU #DEE7   ; Entity index used by player fast runtime (#FF=none)
player_vx_runtime   EQU #DEE8   ; Cached player X velocity (signed 8-bit)
player_vy_runtime   EQU #DEE9   ; Cached player Y velocity (signed 8-bit)
player_dash_timer   EQU #DEEA   ; Frames remaining in current Player dash
player_dash_cooldown EQU #DEEB   ; Frames until Player can dash again
player_dash_dir     EQU #DEEC   ; Player dash direction (1=left,2=right,3=up,4=down)
player_dash_tile_x  EQU #DEED   ; Dash front probe tile X scratch
player_dash_tile_y  EQU #DEEE   ; Dash front probe tile Y scratch
player_health       EQU #DEEF   ; Player health points
player_score        EQU #DEF0   ; Player score (16-bit)
gem_count           EQU #DEF2   ; Collectible tile counter (8-bit)
last_interaction_char EQU #DEF3   ; Char code of last interacted tile (for SM VARIABLE_COMPARE)
last_gem_char       EQU last_interaction_char   ; Backwards-compatible alias for collectible SM checks
last_interaction_pending EQU #DEF4   ; 1 when a new tile interaction is pending for State Machine logic
last_interaction_type EQU #DEF5   ; Interaction type id of last interacted tile
last_interaction_value EQU #DEF6   ; Interaction value byte of last interacted tile
last_interaction_target EQU #DEF7   ; Interaction target id of last interacted tile
last_interaction_x  EQU #DEF8   ; Tile X coordinate of last interaction
last_interaction_y  EQU #DEF9   ; Tile Y coordinate of last interaction
last_interaction_entity EQU #DEFA   ; Entity index that triggered the last interaction

; Persistent collectibles list (survives screen re-entry)
MAX_COLLECTIBLES     EQU 64              ; Max persistent collectible records
collected_count      EQU #DEFB   ; Number of collected tiles recorded (8-bit)
collected_world      EQU #DEFC   ; World IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_screen     EQU #DF3C   ; Screen IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_idx_l      EQU #DF7C   ; Tile name-table index low byte (MAX_COLLECTIBLES bytes)
collected_idx_h      EQU #DFBC   ; Tile name-table index high byte (MAX_COLLECTIBLES bytes)

; Timed bonus tile respawn slots (bonus gem regeneration)
MAX_BONUS_RESPAWNS   EQU 16              ; Max timed bonus tiles waiting to respawn
bonus_respawn_world  EQU #DFFC   ; World IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_screen EQU #E00C   ; Screen IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_l  EQU #E01C   ; Tile index low byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_h  EQU #E02C   ; Tile index high byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_secs   EQU #E03C   ; Remaining seconds per timed respawn slot (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_frames EQU #E04C   ; Frame countdown (60..1) per timed respawn slot (MAX_BONUS_RESPAWNS bytes)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #E05C   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #E05D   ; Temporary 16-bit storage
temp_word_2         EQU #E05F   ; Temporary 16-bit storage
temp_byte_1         EQU #E061   ; Temporary 8-bit storage
temp_byte_2         EQU #E062   ; Temporary 8-bit storage
temp_byte_3         EQU #E063   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #E083   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #E0A3   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #E0C3   ; Temporary 8-bit storage (32 bytes)

; ==================================================================
; SOUND SYSTEM VARIABLES
; ==================================================================
sfx_active          EQU #E0E3   ; 0=no SFX active, 1=playing
sfx_timer           EQU #E0E4   ; Frames remaining for current SFX
sfx_fadeout         EQU #E0E5   ; Reserved fadeout flag/state
temp_byte_7         EQU #E0E6   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #E106   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #E126   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #E146   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #E166   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #E186   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #E1A6   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #E1C6   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #E1E6   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #E206   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #E226   ; Temporary 8-bit storage (32 bytes)
temp_byte_18        EQU #E246   ; Temporary 8-bit storage (32 bytes)
temp_byte_19        EQU #E266   ; Temporary 8-bit storage (32 bytes)
temp_byte_20        EQU #E286   ; Temporary 8-bit storage (32 bytes)
temp_byte_21        EQU #E2A6   ; Temporary 8-bit storage (32 bytes)
temp_byte_22        EQU #E2C6   ; Temporary 8-bit storage (32 bytes)
temp_byte_23        EQU #E2E6   ; Temporary 8-bit storage (32 bytes)
temp_byte_24        EQU #E306   ; Temporary 8-bit storage (32 bytes)
temp_byte_25        EQU #E326   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #E346   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #E386   ; Temporary 16-bit storage (64 bytes)
temp_byte_26        EQU #E3C6   ; Temporary 8-bit storage (32 bytes)
temp_byte_27        EQU #E3E6   ; Temporary 8-bit storage (32 bytes)
temp_byte_28        EQU #E406   ; Temporary 8-bit storage (32 bytes)
tileDead_dbg        EQU #E426   ; Debug byte: current hero deadly contact
tileDead_latched_dbg EQU #E427   ; Debug byte: latched hero deadly contact
tileDead_x_dbg      EQU #E428   ; Debug byte: last sampled deadly tile X
tileDead_y_dbg      EQU #E429   ; Debug byte: last sampled deadly tile Y
tileDead_value_dbg  EQU #E42A   ; Debug byte: last raw deadly behavior value

; Wall collision temporary variables
wall_temp_x         EQU #E42B   ; Cached entity X for wall checks
wall_temp_y         EQU #E42C   ; Cached entity Y for wall checks
wall_hit_left       EQU #E42D   ; Hitbox left edge cache
wall_hit_top        EQU #E42E   ; Hitbox top edge cache
wall_hit_right      EQU #E42F   ; Hitbox right edge cache
wall_hit_bottom     EQU #E430   ; Hitbox bottom edge cache
wall_hit_w          EQU #E431   ; Hitbox width cache (min 1)
wall_hit_h          EQU #E432   ; Hitbox height cache (min 1)
wall_probe_left     EQU #E433   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #E434   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #E435   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #E436   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #E437   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #E457   ; Number of entries in active_entity_list
hero_entity_id      EQU #E458   ; First current-screen entity flagged as player (#FF = none)
active_entity_list_dirty EQU #E459   ; 1=rebuild active_entity_list required
input_entity_list   EQU #E45A   ; Active current-screen entities with Input component (MAX_ENTITIES bytes)
input_entity_count  EQU #E47A   ; Number of entries in input_entity_list
render_entity_list  EQU #E47B   ; Active current-screen entities with Sprite component (MAX_ENTITIES bytes)
render_entity_count EQU #E49B   ; Number of entries in render_entity_list
collision_entity_list EQU #E49C   ; Active current-screen entities with Collision component (MAX_ENTITIES bytes)
collision_entity_count EQU #E4BC   ; Number of entries in collision_entity_list
ground_entity_list  EQU #E4BD   ; Active current-screen entities with Collision or Gravity (MAX_ENTITIES bytes)
ground_entity_count EQU #E4DD   ; Number of entries in ground_entity_list
anim_entity_list    EQU #E4DE   ; Active current-screen entities with Animation+Sprite (MAX_ENTITIES bytes)
anim_entity_count   EQU #E4FE   ; Number of entries in anim_entity_list

; Entity-entity collision optimized variables
coll_list           EQU #E4FF   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #E51F   ; Number of entities in coll_list
coll_src_left       EQU #E520   ; Source AABB left edge (scratch)
coll_src_right      EQU #E521   ; Source AABB right edge (scratch)
coll_src_top        EQU #E522   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #E523   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #E524   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #E524   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #E526   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #E528   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #E52A   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #E52C   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #E52E   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #E530   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #E532   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #E534   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #E535   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #E53A   ; Frame counter (16-bit)
task_exec_time          EQU #E53C   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #E53E   ; Set to 1 on each VBlank (1 byte)
interrupt_in_progress   EQU #E53F   ; 1 while the H.TIMI dispatcher is running
RAM_INTERRUPT_END       EQU #E540   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #E540   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #E541   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #E542   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #E543   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #E544   ; 0=stopped, 1=track active
music_muted          EQU #E545   ; 0=audible, 1=muted/pause
music_loop           EQU #E546   ; 0=no loop, 1=loop enabled
music_track_index    EQU #E547   ; Current ROM track index
music_row_frames     EQU #E548   ; Frames per tracker row
music_row_countdown  EQU #E549   ; Countdown to next row
music_order_pos      EQU #E54A   ; Current order position
music_pattern_index  EQU #E54B   ; Current pattern index
music_pattern_row    EQU #E54C   ; Current row inside pattern
music_pattern_rows   EQU #E54D   ; Cached rows in current pattern
music_track_ptr_l    EQU #E54E   ; Current track pointer low byte
music_track_ptr_h    EQU #E54F   ; Current track pointer high byte
music_pattern_ptr_l  EQU #E550   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #E551   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #E552   ; PSG mixer shadow for music runtime
music_pitch_note_work EQU #E553   ; Scratch note index while resolving tone/ornament macros
music_pitch_step_work EQU #E554   ; Scratch macro step while resolving tone/ornament macros
music_pitch_len_work  EQU #E555   ; Scratch macro length while resolving tone/ornament macros
music_ch_note_base EQU #E556   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #E556   ; Channel A
music_ch_b_note EQU #E557   ; Channel B
music_ch_c_note EQU #E558   ; Channel C
music_ch_instrument_base EQU #E559   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #E559   ; Channel A
music_ch_b_instrument EQU #E55A   ; Channel B
music_ch_c_instrument EQU #E55B   ; Channel C
music_ch_ornament_base EQU #E55C   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #E55C   ; Channel A
music_ch_b_ornament EQU #E55D   ; Channel B
music_ch_c_ornament EQU #E55E   ; Channel C
music_ch_volume_base EQU #E55F   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #E55F   ; Channel A
music_ch_b_volume EQU #E560   ; Channel B
music_ch_c_volume EQU #E561   ; Channel C
music_ch_vol_step_base EQU #E562   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #E562   ; Channel A
music_ch_b_vol_step EQU #E563   ; Channel B
music_ch_c_vol_step EQU #E564   ; Channel C
music_ch_tone_step_base EQU #E565   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #E565   ; Channel A
music_ch_b_tone_step EQU #E566   ; Channel B
music_ch_c_tone_step EQU #E567   ; Channel C
music_ch_noise_step_base EQU #E568   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #E568   ; Channel A
music_ch_b_noise_step EQU #E569   ; Channel B
music_ch_c_noise_step EQU #E56A   ; Channel C
music_ch_orn_step_base EQU #E56B   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #E56B   ; Channel A
music_ch_b_orn_step EQU #E56C   ; Channel B
music_ch_c_orn_step EQU #E56D   ; Channel C
music_ch_hw_env_step_base EQU #E56E   ; Software hardware-envelope divider step (3 bytes)
music_ch_a_hw_env_step EQU #E56E   ; Channel A
music_ch_b_hw_env_step EQU #E56F   ; Channel B
music_ch_c_hw_env_step EQU #E570   ; Channel C
...
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #E571   ; End of project variables (9585 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#E571: Project variables (9585 bytes)
;   #E571-#E600: Alignment padding/free RAM (143 bytes)
