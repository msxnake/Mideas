...
; BANK 00 @#0393 : sprites.asm (2584 bytes)
; BANK 00 @#0DAB : font.asm (177 bytes)
; BANK 00 @#0E5C : hud.asm (79 bytes)
; BANK 00 @#0EAB : menus.asm (168 bytes)
; BANK 00 @#0F53 : sound.asm part 1/2 (4269 bytes)
; BANK 01 @#0000 : sound.asm part 2/2 (5272 bytes)
; BANK 01 @#1498 : scroll.asm (2342 bytes)
; BANK 01 @#1DBE : animtiles.asm (578 bytes)
; BANK 02 @#0000 : animtiles.asm (3999 bytes)
; BANK 02 @#0F9F : statemachine.asm (5 bytes)
; BANK 02 @#0FA4 : gameflow.asm (4188 bytes)
; BANK 03 @#0000 : gameflow.asm (1187 bytes)
...
global_var_lives     EQU #C048   ; Remaining lives (0-255) (8-bit)

; ==================================================================
; SYSTEM VARIABLES
; ==================================================================
ROM_slot            EQU #C049   ; Expanded slot for normal page 1 ROM access
slot_primary_normal EQU #C04A   ; Primary slot register snapshot for BIOS-ROM-ROM-RAM layout
page0_bios_slot     EQU #C04B   ; Expanded slot for normal BIOS page 0
page2_normal_slot   EQU #C04C   ; Expanded slot for normal page 2 layout
page3_normal_slot   EQU #C04D   ; Expanded slot for normal RAM page 3
mapper_bank_p1_current EQU #C04E   ; Mapper current bank for page/window 1
mapper_bank_p2_current EQU #C04F   ; Mapper current bank for page/window 2
mapper_bank_p3_current EQU #C050   ; Mapper current bank for page/window 3
mapper_bank_p4_current EQU #C051   ; Mapper current bank for page/window 4
mapper_saved_bank    EQU #C052   ; Saved mapper bank for push/pop helpers
mapper_saved_bank_p1 EQU #C053   ; Saved mapper bank for page/window 1 helpers
mapper_saved_bank_p3 EQU #C054   ; Saved mapper bank for page/window 3 helpers
mapper_saved_bank_p4 EQU #C055   ; Saved mapper bank for page/window 4 helpers
resource_descriptor_ptr EQU #C056   ; Pointer to cached resource descriptor entry (16-bit)
resource_descriptor_id EQU #C058   ; Cached resource id
resource_descriptor_type EQU #C059   ; Cached resource type
resource_descriptor_group EQU #C05A   ; Cached resource group
resource_descriptor_bank EQU #C05B   ; Cached resource bank
resource_descriptor_addr EQU #C05C   ; Cached resource visible address (16-bit)
resource_descriptor_size EQU #C05E   ; Cached resource size (16-bit)
vram_cache_tile_patterns_ready EQU #C060   ; 1 when shared gameplay tile patterns are already resident in VRAM
vram_cache_tile_colors_ready EQU #C061   ; 1 when shared gameplay tile colors are already resident in VRAM
vram_cache_font_ready EQU #C062   ; 1 when shared font patterns/colors are already resident in VRAM
resource_ram_cache_screen_layout_id EQU #C063   ; Cached resource id for runtime_background_layout source
resource_ram_cache_effects_layout_id EQU #C064   ; Cached resource id for runtime_effects_layout source
resource_ram_cache_behavior_map_id EQU #C065   ; Cached resource id for pristine behavior-map RAM copy
resource_ram_cache_effect_zone_table_id EQU #C066   ; Cached resource id for runtime_effect_zone_table source
current_screen2_tilebank_id EQU #C067   ; Current SCREEN 2 shared tilebank loaded in VRAM (#FF=none/unknown)
frame_counter       EQU #C068   ; Frame counter (16-bit)

; Profiling counters (16-bit, cumulative)
prof_update_all_entities_calls EQU #C06A   ; Calls to update_all_entities
prof_execute_sm_calls EQU #C06C   ; Calls to execute_all_state_machines
prof_sm_update_calls  EQU #C06E   ; Calls to SM_Update
prof_collision_calls  EQU #C070   ; Calls to update_collision_component
prof_wall_calls       EQU #C072   ; Calls to update_wallcollision_component
prof_deadly_calls     EQU #C074   ; Calls to update_deadly_tiles_component
prof_tile_interaction_calls EQU #C076   ; Calls to check_tile_interaction
prof_animation_calls  EQU #C078   ; Calls to update_animation_component
prof_sprite_calls     EQU #C07A   ; Calls to update_sprite_component
prof_music_task_calls EQU #C07C   ; Calls to task_update_music
prof_deadly_behavior_reads EQU #C07E   ; Deadly helper behavior-map reads
page0_transfer_buffer EQU #C080   ; Temporary RAM buffer for page0 -> VRAM copies

; ==================================================================
; SCREEN MAP POINTERS (Current active screen)
; ==================================================================
current_screen_layout   EQU #C180   ; Pointer to current screen layout data (16-bit)
current_screen_layout_bank EQU #C182   ; Mapper bank for current screen layout data
current_behavior_map    EQU #C183   ; Pointer to current behavior map data (16-bit)
current_behavior_map_bank EQU #C185   ; Mapper bank for current behavior map data
behavior_cache_row     EQU #C186   ; Cached behavior row (255=invalid)
behavior_cache_map_l   EQU #C187   ; Cached behavior map pointer low byte
behavior_cache_map_h   EQU #C188   ; Cached behavior map pointer high byte
behavior_cache_row_base EQU #C189   ; Cached row base address in behavior map (16-bit)
RUNTIME_SCREEN_MAP_SIZE EQU 768
MAX_RUNTIME_EFFECT_ZONES EQU 64
runtime_background_layout EQU #C18B   ; Immutable copy of current background layout (32x24)
runtime_screen_layout  EQU #C48B   ; Mutable copy of current screen layout (32x24)
resource_ram_cache_behavior_map EQU #C78B   ; Pristine behavior map cache for current banked resource (32x24)
runtime_behavior_map   EQU #CA8B   ; Mutable copy of current behavior map (32x24)
runtime_interaction_type_map EQU #CD8B   ; Mutable copy of current interaction type map (32x24)
runtime_interaction_value_map EQU #D08B   ; Mutable copy of current interaction value map (32x24)
runtime_interaction_target_map EQU #D38B   ; Mutable copy of current interaction target map (32x24)
runtime_char_behavior_table EQU #D68B   ; Current screen char -> behavior lookup table (256 bytes)
runtime_effects_layout EQU #D78B   ; Alternate effects layout copy for secret zones (32x24)
screen_block_catalog_ptr EQU #DA8B   ; Scratch pointer to current screen block catalog during layout expansion
screen_block_map_ptr EQU #DA8D   ; Scratch pointer to current screen block index map during layout expansion
runtime_effect_zone_table EQU #DA8F   ; Current screen effect zone table (MAX_RUNTIME_EFFECT_ZONES * 8 bytes)
current_effect_zone_count EQU #DC8F   ; Number of effect zones copied into runtime_effect_zone_table
secret_zone_active EQU #DC90   ; 1 if hero is currently inside an active secret zone
secret_zone_rect_x EQU #DC91   ; Active secret zone rect X in cells
secret_zone_rect_y EQU #DC92   ; Active secret zone rect Y in cells
secret_zone_rect_w EQU #DC93   ; Active secret zone rect width in cells
secret_zone_rect_h EQU #DC94   ; Active secret zone rect height in cells

; ==================================================================
; VIEWPORT/CAMERA VARIABLES (for scroll system)
; ==================================================================
camera_x            EQU #DC95   ; Camera X position in pixels (16-bit)
camera_y            EQU #DC97   ; Camera Y position in pixels (16-bit)
camera_tile_x       EQU #DC99   ; Camera tile X (column)
camera_tile_y       EQU #DC9A   ; Camera tile Y (row)
world_width_tiles   EQU #DC9B   ; World width in tiles
world_height_tiles  EQU #DC9C   ; World height in tiles
scroll_dirty_flag   EQU #DC9D   ; 1=viewport changed, needs redraw
hud_dirty_flag      EQU #DC9E   ; 1=HUD needs redraw, 0=clean
time_second_frame_counter EQU #DC9F   ; VBlank frames remaining until the next TimeRemaining decrement
time_last_interrupt_counter EQU #DCA0   ; Last interrupt_counter snapshot used by TimeRemaining sync (16-bit)

; ==================================================================
; ANIMATED TILES VARIABLES
; ==================================================================
anim_tile_timer     EQU #DCA2   ; Animation frame timer
anim_tile_frame     EQU #DCA3   ; Current animation frame (0-3)
anim_tile_speed     EQU #DCA4   ; Frames between animation updates
anim_tile_transform_flags EQU #DCA5   ; Runtime flags for transform-mode tile animation (byte0=flags, byte1=opcode scratch)
anim_tile_row_buffer EQU #DCA7   ; Temp buffer (8 bytes) for row transforms

; ==================================================================
; ENTITY SYSTEM VARIABLES (Fixed 32 entities)
; ==================================================================
MAX_ENTITIES        EQU 32
entity_active       EQU #DCAF   ; Entity active flags (32 bytes, 0=inactive, 1=active)
entity_is_player    EQU #DCCF   ; Entity hero/player flag (32 bytes, 0=no, 1=yes)
entity_button_contact_active EQU #DCEF   ; 1 while entity stays on the same button tile (32 bytes)
entity_button_contact_x EQU #DD0F   ; Button tile X currently latched per entity (32 bytes)
entity_button_contact_y EQU #DD2F   ; Button tile Y currently latched per entity (32 bytes)
entity_on_ladder   EQU #DD4F   ; 1 while entity is centered on a ladder tile (32 bytes)
entity_gate_current_step EQU #DD6F   ; Current applied retract step (32 bytes)
entity_gate_step_timer EQU #DD8F   ; Countdown until next retract step (32 bytes)
entity_walljump_lock EQU #DDAF   ; Remaining horizontal lock frames after wall jump (32 bytes)
entity_walljump_locked_vx EQU #DDCF   ; Horizontal velocity preserved while wall jump lock is active (32 bytes)
entity_wallgrab_active EQU #DDEF   ; 1 if entity is currently grabbing a wall (32 bytes)
entity_wallgrab_grace EQU #DE0F   ; Frames to keep wall grab during transient wall flag gaps (32 bytes)
entity_wallgrab_timer EQU #DE2F   ; Remaining wall-grab frames until grounded reset (32 bytes)
entity_wallgrab_lockout EQU #DE4F   ; Wall grab disabled until grounded after timer is spent (32 bytes)
entity_walljump_anim_active EQU #DE6F   ; Wall jump one-shot animation is waiting to restore base sprite (32 bytes)
entity_x_pos        EQU #DE8F   ; Entity X positions (32 bytes)
entity_y_pos        EQU #DEAF   ; Entity Y positions (32 bytes)
entity_vel_x        EQU #DECF   ; Entity X velocity (32 bytes)
entity_vel_y        EQU #DEEF   ; Entity Y velocity (32 bytes)
entity_comp_masks   EQU #DF0F   ; Entity component masks (32 bytes)
entity_comp_masks_hi EQU #DF2F   ; Entity component masks high byte (32 bytes)
entity_screen_id    EQU #DF4F   ; Entity screen ID (32 bytes)
entity_job_period   EQU #DF6F   ; Entity job period in frames (32 bytes, 1=100%,2=50%,3=33%,4=25%)
entity_job_entry    EQU #DF8F   ; Entity job entry slot within period window (32 bytes)
entity_job_scheduler_active EQU #DFAF   ; 1 when any entity uses non-default job cadence
entity_dir_mask     EQU #DFB0   ; Entity direction mask (32 bytes)
entity_input_speed  EQU #DFD0   ; Entity input/cursor speed (32 bytes)
entity_health       EQU #DFF0   ; Entity health (32 bytes)
entity_anim_frame   EQU #E010   ; Entity animation frame (32 bytes)
entity_anim_tick    EQU #E030   ; Entity animation tick counter (32 bytes)
entity_anim_speed   EQU #E050   ; Entity animation speed (ticks per frame) (32 bytes)
entity_anim_flags   EQU #E070   ; Entity animation flags (32 bytes)
entity_sm_ptr_l     EQU #E090   ; Entity State Pointer Low (32 bytes)
entity_sm_ptr_h     EQU #E0B0   ; Entity State Pointer High (32 bytes)
entity_sm_timer_l   EQU #E0D0   ; Entity State Timer Low (32 bytes)
entity_sm_timer_h   EQU #E0F0   ; Entity State Timer High (32 bytes)
entity_sm_wait_timer EQU #E110   ; Entity State Wait Timer (32 bytes)
entity_sm_sprite_control EQU #E130   ; 1 when the assigned state machine explicitly drives sprite changes (32 bytes)
entity_lifetime     EQU #E150   ; Entity lifetime for auto-destroy (32 bytes, 0=infinite)
entity_collectible_enabled EQU #E170   ; 1 when entity has Collectible component (32 bytes)
entity_carried_by   EQU #E190   ; Entity carrier ID (32 bytes, 255=not carried)
entity_template_token EQU #E1B0   ; Entity template token (32 bytes, 0=unknown)
entity_facing_dir   EQU #E1D0   ; Last facing direction (32 bytes, 0=none,1=left,2=right,3=up,4=down)
entity_sm_var_0     EQU #E1F0   ; Entity Variable 0 (32 bytes)
entity_sm_var_1     EQU #E210   ; Entity Variable 1 (32 bytes)
entity_sm_var_2     EQU #E230   ; Entity Variable 2 (32 bytes)
entity_sm_var_3     EQU #E250   ; Entity Variable 3 (32 bytes)
entity_sm_var_4     EQU #E270   ; Entity Variable 4 (32 bytes)
entity_sm_var_5     EQU #E290   ; Entity Variable 5 (32 bytes)
entity_sm_var_6     EQU #E2B0   ; Entity Variable 6 (32 bytes)
entity_sm_var_7     EQU #E2D0   ; Entity Variable 7 (32 bytes)

; ==================================================================
; SPRITE SYSTEM VARIABLES
; ==================================================================
entity_sprite_asset_index EQU #E2F0   ; Entity sprite asset index - RAM copy (32 bytes)
active_sprite_count EQU #E310   ; Number of sprites currently active
sprites_dirty      EQU #E311   ; 1=sprite_attributes changed, needs VRAM sync
sprite_pattern      EQU #E312   ; Sprite pattern IDs (32 bytes)
sprite_color        EQU #E332   ; Sprite colors (32 bytes)
sprite_layer_colors EQU #E352   ; HW sprite layer color cache - RAM copy (32 bytes, indexed by HW sprite index)
sprite_layer_y_offsets EQU #E372   ; HW sprite layer signed Y offsets - RAM copy (32 bytes, indexed by HW sprite index)
sprite_asset_base_pattern_slot_runtime EQU #E392   ; Runtime base 16x16 slot per sprite asset (1 bytes)
sprite_placeholder_base_pattern_num EQU #E393   ; Runtime placeholder pattern number (base slot * 4)
current_sprite_pattern_pack_id EQU #E394   ; Active runtime sprite pattern pack id (#FF=none loaded)
sprite_attributes   EQU #E395   ; Interleaved sprite attributes (32 * 4 bytes)

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #E415   ; Player X position (16-bit)
player_y            EQU #E417   ; Player Y position (16-bit)
player_runtime_enabled EQU #E419   ; 1=player fast runtime bound to hero entity
player_entity_index EQU #E41A   ; Entity index used by player fast runtime (#FF=none)
player_vx_runtime   EQU #E41B   ; Cached player X velocity (signed 8-bit)
player_vy_runtime   EQU #E41C   ; Cached player Y velocity (signed 8-bit)
player_health       EQU #E41D   ; Player health points
player_score        EQU #E41E   ; Player score (16-bit)
gem_count           EQU #E420   ; Collectible tile counter (8-bit)
last_interaction_char EQU #E421   ; Char code of last interacted tile (for SM VARIABLE_COMPARE)
last_gem_char       EQU last_interaction_char   ; Backwards-compatible alias for collectible SM checks
last_interaction_pending EQU #E422   ; 1 when a new tile interaction is pending for State Machine logic
last_interaction_type EQU #E423   ; Interaction type id of last interacted tile
last_interaction_value EQU #E424   ; Interaction value byte of last interacted tile
last_interaction_target EQU #E425   ; Interaction target id of last interacted tile
last_interaction_x  EQU #E426   ; Tile X coordinate of last interaction
last_interaction_y  EQU #E427   ; Tile Y coordinate of last interaction
last_interaction_entity EQU #E428   ; Entity index that triggered the last interaction

; Persistent collectibles list (survives screen re-entry)
MAX_COLLECTIBLES     EQU 64              ; Max persistent collectible records
collected_count      EQU #E429   ; Number of collected tiles recorded (8-bit)
collected_world      EQU #E42A   ; World IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_screen     EQU #E46A   ; Screen IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_idx_l      EQU #E4AA   ; Tile name-table index low byte (MAX_COLLECTIBLES bytes)
collected_idx_h      EQU #E4EA   ; Tile name-table index high byte (MAX_COLLECTIBLES bytes)

; Timed bonus tile respawn slots (bonus gem regeneration)
MAX_BONUS_RESPAWNS   EQU 16              ; Max timed bonus tiles waiting to respawn
bonus_respawn_world  EQU #E52A   ; World IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_screen EQU #E53A   ; Screen IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_l  EQU #E54A   ; Tile index low byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_h  EQU #E55A   ; Tile index high byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_secs   EQU #E56A   ; Remaining seconds per timed respawn slot (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_frames EQU #E57A   ; Frame countdown (60..1) per timed respawn slot (MAX_BONUS_RESPAWNS bytes)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #E58A   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #E58B   ; Temporary 16-bit storage
temp_word_2         EQU #E58D   ; Temporary 16-bit storage
temp_byte_1         EQU #E58F   ; Temporary 8-bit storage
temp_byte_2         EQU #E590   ; Temporary 8-bit storage
temp_byte_3         EQU #E591   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #E5B1   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #E5D1   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #E5F1   ; Temporary 8-bit storage (32 bytes)

; ==================================================================
; SOUND SYSTEM VARIABLES
; ==================================================================
sfx_active          EQU #E611   ; 0=no SFX active, 1=playing
sfx_timer           EQU #E612   ; Frames remaining for current SFX
sfx_fadeout         EQU #E613   ; Reserved fadeout flag/state
temp_byte_7         EQU #E614   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #E634   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #E654   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #E674   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #E694   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #E6B4   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #E6D4   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #E6F4   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #E714   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #E734   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #E754   ; Temporary 8-bit storage (32 bytes)
temp_byte_18        EQU #E774   ; Temporary 8-bit storage (32 bytes)
temp_byte_19        EQU #E794   ; Temporary 8-bit storage (32 bytes)
temp_byte_20        EQU #E7B4   ; Temporary 8-bit storage (32 bytes)
temp_byte_21        EQU #E7D4   ; Temporary 8-bit storage (32 bytes)
temp_byte_22        EQU #E7F4   ; Temporary 8-bit storage (32 bytes)
temp_byte_23        EQU #E814   ; Temporary 8-bit storage (32 bytes)
temp_byte_24        EQU #E834   ; Temporary 8-bit storage (32 bytes)
temp_byte_25        EQU #E854   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #E874   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #E8B4   ; Temporary 16-bit storage (64 bytes)
temp_byte_26        EQU #E8F4   ; Temporary 8-bit storage (32 bytes)
temp_byte_27        EQU #E914   ; Temporary 8-bit storage (32 bytes)
temp_byte_28        EQU #E934   ; Temporary 8-bit storage (32 bytes)
tileDead_dbg        EQU #E954   ; Debug byte: current hero deadly contact
tileDead_latched_dbg EQU #E955   ; Debug byte: latched hero deadly contact
tileDead_x_dbg      EQU #E956   ; Debug byte: last sampled deadly tile X
tileDead_y_dbg      EQU #E957   ; Debug byte: last sampled deadly tile Y
tileDead_value_dbg  EQU #E958   ; Debug byte: last raw deadly behavior value

; Wall collision temporary variables
wall_temp_x         EQU #E959   ; Cached entity X for wall checks
wall_temp_y         EQU #E95A   ; Cached entity Y for wall checks
wall_hit_left       EQU #E95B   ; Hitbox left edge cache
wall_hit_top        EQU #E95C   ; Hitbox top edge cache
wall_hit_right      EQU #E95D   ; Hitbox right edge cache
wall_hit_bottom     EQU #E95E   ; Hitbox bottom edge cache
wall_hit_w          EQU #E95F   ; Hitbox width cache (min 1)
wall_hit_h          EQU #E960   ; Hitbox height cache (min 1)
wall_probe_left     EQU #E961   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #E962   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #E963   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #E964   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #E965   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #E985   ; Number of entries in active_entity_list
hero_entity_id      EQU #E986   ; First current-screen entity flagged as player (#FF = none)
active_entity_list_dirty EQU #E987   ; 1=rebuild active_entity_list required
input_entity_list   EQU #E988   ; Active current-screen entities with Input component (MAX_ENTITIES bytes)
input_entity_count  EQU #E9A8   ; Number of entries in input_entity_list
render_entity_list  EQU #E9A9   ; Active current-screen entities with Sprite component (MAX_ENTITIES bytes)
render_entity_count EQU #E9C9   ; Number of entries in render_entity_list
collision_entity_list EQU #E9CA   ; Active current-screen entities with Collision component (MAX_ENTITIES bytes)
collision_entity_count EQU #E9EA   ; Number of entries in collision_entity_list
ground_entity_list  EQU #E9EB   ; Active current-screen entities with Collision or Gravity (MAX_ENTITIES bytes)
ground_entity_count EQU #EA0B   ; Number of entries in ground_entity_list
anim_entity_list    EQU #EA0C   ; Active current-screen entities with Animation+Sprite (MAX_ENTITIES bytes)
anim_entity_count   EQU #EA2C   ; Number of entries in anim_entity_list

; Entity-entity collision optimized variables
coll_list           EQU #EA2D   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #EA4D   ; Number of entities in coll_list
coll_src_left       EQU #EA4E   ; Source AABB left edge (scratch)
coll_src_right      EQU #EA4F   ; Source AABB right edge (scratch)
coll_src_top        EQU #EA50   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #EA51   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #EA52   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #EA52   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #EA54   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #EA56   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #EA58   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #EA5A   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #EA5C   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #EA5E   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #EA60   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #EA62   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #EA63   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #EA68   ; Frame counter (16-bit)
task_exec_time          EQU #EA6A   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #EA6C   ; Set to 1 on each VBlank (1 byte)
RAM_INTERRUPT_END       EQU #EA6D   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #EA6D   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #EA6E   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #EA6F   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #EA70   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #EA71   ; 0=stopped, 1=track active
music_muted          EQU #EA72   ; 0=audible, 1=muted/pause
music_loop           EQU #EA73   ; 0=no loop, 1=loop enabled
music_track_index    EQU #EA74   ; Current ROM track index
music_row_frames     EQU #EA75   ; Frames per tracker row
music_row_countdown  EQU #EA76   ; Countdown to next row
music_order_pos      EQU #EA77   ; Current order position
music_pattern_index  EQU #EA78   ; Current pattern index
music_pattern_row    EQU #EA79   ; Current row inside pattern
music_pattern_rows   EQU #EA7A   ; Cached rows in current pattern
music_track_ptr_l    EQU #EA7B   ; Current track pointer low byte
music_track_ptr_h    EQU #EA7C   ; Current track pointer high byte
music_pattern_ptr_l  EQU #EA7D   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #EA7E   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #EA7F   ; PSG mixer shadow for music runtime
music_ch_note_base EQU #EA80   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #EA80   ; Channel A
music_ch_b_note EQU #EA81   ; Channel B
music_ch_c_note EQU #EA82   ; Channel C
music_ch_instrument_base EQU #EA83   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #EA83   ; Channel A
music_ch_b_instrument EQU #EA84   ; Channel B
music_ch_c_instrument EQU #EA85   ; Channel C
music_ch_ornament_base EQU #EA86   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #EA86   ; Channel A
music_ch_b_ornament EQU #EA87   ; Channel B
music_ch_c_ornament EQU #EA88   ; Channel C
music_ch_volume_base EQU #EA89   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #EA89   ; Channel A
music_ch_b_volume EQU #EA8A   ; Channel B
music_ch_c_volume EQU #EA8B   ; Channel C
music_ch_vol_step_base EQU #EA8C   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #EA8C   ; Channel A
music_ch_b_vol_step EQU #EA8D   ; Channel B
music_ch_c_vol_step EQU #EA8E   ; Channel C
music_ch_tone_step_base EQU #EA8F   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #EA8F   ; Channel A
music_ch_b_tone_step EQU #EA90   ; Channel B
music_ch_c_tone_step EQU #EA91   ; Channel C
music_ch_noise_step_base EQU #EA92   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #EA92   ; Channel A
music_ch_b_noise_step EQU #EA93   ; Channel B
music_ch_c_noise_step EQU #EA94   ; Channel C
music_ch_orn_step_base EQU #EA95   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #EA95   ; Channel A
music_ch_b_orn_step EQU #EA96   ; Channel B
music_ch_c_orn_step EQU #EA97   ; Channel C
...
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #EA98   ; End of project variables (10904 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#EA98: Project variables (10904 bytes)
;   #EA98-#F37F: Free RAM (~2280 bytes available)
