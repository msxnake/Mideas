...
const getPreferredScreen2AssignableCharRanges = (charsetRangeStart, charsetRangeEnd) => {
    ...
    const rangeStart = Math.max(exports.SCREEN2_MIN_ASSIGNABLE_TILE_CHAR_CODE, Math.min(exports.SCREEN2_MAX_ASSIGNABLE_TILE_CHAR_CODE, Number.isFinite(startValue) ? Math.floor(startValue) : exports.SCREEN2_MIN_ASSIGNABLE_TILE_CHAR_CODE));
    const rangeEnd = Math.max(rangeStart, Math.min(exports.SCREEN2_MAX_ASSIGNABLE_TILE_CHAR_CODE, Number.isFinite(endValue) ? Math.floor(endValue) : exports.SCREEN2_MAX_ASSIGNABLE_TILE_CHAR_CODE));
    const rangesToTry = [];
