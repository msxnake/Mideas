...
${transitionGameplayCacheInvalidateAsm}
    ; 2. Tile colors (chars 128+) — corrupted by color-table effects (#11 = black)
    call load_colors_to_vram
    ; 3. Font patterns + colors (chars 0-127) — also zeroed by color-table effects.
...
; in different patterns. Most legacy effects write tile 0 (blank/black);
; horizontal_lines and diagonal_clear write #FF, the project empty/SPC char.
...
; Write #FF (empty/SPC char) to all 32 columns of a single Name Table row.
; Input:  A = row (0-23)
; Preserves: BC, DE, HL
; ==================================================================
trans_clear_row_direct:
...
function generateNodeStructure(node, gameFlow, analysis, romMode = 'simple32k') {
    ...
    const hasData = ['Start', 'WorldLink', 'SubMenu', 'Text', 'IfThenElse', 'Globals', 'Transition', 'Music', 'PresentationScreen'].includes(node.type) ||
...
`;
        switch (node.type) {
            ...
            case 'Globals':
                ...
                break;
            case 'Transition': {
                // Effect IDs match execute_transition_effect dispatch (0-7)
