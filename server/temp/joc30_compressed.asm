...
; BANK 05 @#0000 : animtiles.asm (5718 bytes)
; BANK 05 @#1656 : statemachine.asm part 1/2 (2474 bytes)
; BANK 06 @#0000 : statemachine.asm part 2/2 (8192 bytes)
; BANK 07 @#0000 : statemachine.asm part 3/2 (4162 bytes)
; BANK 07 @#1042 : gameflow.asm (4030 bytes)
; BANK 08 @#0000 : gameflow.asm (1991 bytes)
...
sprite_asset_base_pattern_slot_runtime EQU #E1FB   ; Runtime base 16x16 slot per sprite asset (4 bytes)
sprite_placeholder_base_pattern_num EQU #E1FF   ; Runtime placeholder pattern number (base slot * 4)
current_sprite_pattern_pack_id EQU #E200   ; Active runtime sprite pattern pack id (#FF=none loaded)
sprite_attributes   EQU #E201   ; Interleaved sprite attributes (32 * 4 bytes)

; ==================================================================
; SCREEN SYSTEM VARIABLES (1 screens detected)
; ==================================================================
current_screen_id   EQU #E281   ; Currently displayed screen ID
screen_dirty_flag   EQU #E282   ; Screen needs redraw flag
screen_transition_cooldown EQU #E283   ; Cooldown frames after screen transition
current_world_id    EQU #E284   ; Current world ID (for multi-world support)
current_screen_index EQU #E285   ; Current screen index within world
current_screen_anim_group_count EQU #E286   ; Animated tile groups visible in current screen
current_screen_entity_count EQU #E287   ; Entity instances assigned to current screen
current_screen_sprite_pattern_slots EQU #E288   ; Sprite pattern slots needed by current screen
current_screen_summary_flags EQU #E289   ; Runtime screen summary flags (music/hud/effects/anim)

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #E28A   ; Player X position (16-bit)
player_y            EQU #E28C   ; Player Y position (16-bit)
player_runtime_enabled EQU #E28E   ; 1=player fast runtime bound to hero entity
player_entity_index EQU #E28F   ; Entity index used by player fast runtime (#FF=none)
player_vx_runtime   EQU #E290   ; Cached player X velocity (signed 8-bit)
player_vy_runtime   EQU #E291   ; Cached player Y velocity (signed 8-bit)
player_health       EQU #E292   ; Player health points
player_score        EQU #E293   ; Player score (16-bit)
gem_count           EQU #E295   ; Collectible tile counter (8-bit)
last_interaction_char EQU #E296   ; Char code of last interacted tile (for SM VARIABLE_COMPARE)
last_gem_char       EQU last_interaction_char   ; Backwards-compatible alias for collectible SM checks
last_interaction_pending EQU #E297   ; 1 when a new tile interaction is pending for State Machine logic
last_interaction_type EQU #E298   ; Interaction type id of last interacted tile
last_interaction_value EQU #E299   ; Interaction value byte of last interacted tile
last_interaction_target EQU #E29A   ; Interaction target id of last interacted tile
last_interaction_x  EQU #E29B   ; Tile X coordinate of last interaction
last_interaction_y  EQU #E29C   ; Tile Y coordinate of last interaction
last_interaction_entity EQU #E29D   ; Entity index that triggered the last interaction

; Persistent collectibles list (survives screen re-entry)
MAX_COLLECTIBLES     EQU 64              ; Max persistent collectible records
collected_count      EQU #E29E   ; Number of collected tiles recorded (8-bit)
collected_world      EQU #E29F   ; World IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_screen     EQU #E2DF   ; Screen IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_idx_l      EQU #E31F   ; Tile name-table index low byte (MAX_COLLECTIBLES bytes)
collected_idx_h      EQU #E35F   ; Tile name-table index high byte (MAX_COLLECTIBLES bytes)

; Timed bonus tile respawn slots (bonus gem regeneration)
MAX_BONUS_RESPAWNS   EQU 16              ; Max timed bonus tiles waiting to respawn
bonus_respawn_world  EQU #E39F   ; World IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_screen EQU #E3AF   ; Screen IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_l  EQU #E3BF   ; Tile index low byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_h  EQU #E3CF   ; Tile index high byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_secs   EQU #E3DF   ; Remaining seconds per timed respawn slot (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_frames EQU #E3EF   ; Frame countdown (60..1) per timed respawn slot (MAX_BONUS_RESPAWNS bytes)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #E3FF   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #E400   ; Temporary 16-bit storage
temp_word_2         EQU #E402   ; Temporary 16-bit storage
temp_byte_1         EQU #E404   ; Temporary 8-bit storage
temp_byte_2         EQU #E405   ; Temporary 8-bit storage
temp_byte_3         EQU #E406   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #E426   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #E446   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #E466   ; Temporary 8-bit storage (32 bytes)

; ==================================================================
; SOUND SYSTEM VARIABLES
; ==================================================================
sfx_active          EQU #E486   ; 0=no SFX active, 1=playing
sfx_timer           EQU #E487   ; Frames remaining for current SFX
sfx_fadeout         EQU #E488   ; Reserved fadeout flag/state
temp_byte_7         EQU #E489   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #E4A9   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #E4C9   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #E4E9   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #E509   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #E529   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #E549   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #E569   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #E589   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #E5A9   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #E5C9   ; Temporary 8-bit storage (32 bytes)
temp_byte_18        EQU #E5E9   ; Temporary 8-bit storage (32 bytes)
temp_byte_19        EQU #E609   ; Temporary 8-bit storage (32 bytes)
temp_byte_20        EQU #E629   ; Temporary 8-bit storage (32 bytes)
temp_byte_21        EQU #E649   ; Temporary 8-bit storage (32 bytes)
temp_byte_22        EQU #E669   ; Temporary 8-bit storage (32 bytes)
temp_byte_23        EQU #E689   ; Temporary 8-bit storage (32 bytes)
temp_byte_24        EQU #E6A9   ; Temporary 8-bit storage (32 bytes)
temp_byte_25        EQU #E6C9   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #E6E9   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #E729   ; Temporary 16-bit storage (64 bytes)
temp_byte_26        EQU #E769   ; Temporary 8-bit storage (32 bytes)
temp_byte_27        EQU #E789   ; Temporary 8-bit storage (32 bytes)
temp_byte_28        EQU #E7A9   ; Temporary 8-bit storage (32 bytes)
tileDead_dbg        EQU #E7C9   ; Debug byte: current hero deadly contact
tileDead_latched_dbg EQU #E7CA   ; Debug byte: latched hero deadly contact
tileDead_x_dbg      EQU #E7CB   ; Debug byte: last sampled deadly tile X
tileDead_y_dbg      EQU #E7CC   ; Debug byte: last sampled deadly tile Y
tileDead_value_dbg  EQU #E7CD   ; Debug byte: last raw deadly behavior value

; Wall collision temporary variables
wall_temp_x         EQU #E7CE   ; Cached entity X for wall checks
wall_temp_y         EQU #E7CF   ; Cached entity Y for wall checks
wall_hit_left       EQU #E7D0   ; Hitbox left edge cache
wall_hit_top        EQU #E7D1   ; Hitbox top edge cache
wall_hit_right      EQU #E7D2   ; Hitbox right edge cache
wall_hit_bottom     EQU #E7D3   ; Hitbox bottom edge cache
wall_hit_w          EQU #E7D4   ; Hitbox width cache (min 1)
wall_hit_h          EQU #E7D5   ; Hitbox height cache (min 1)
wall_probe_left     EQU #E7D6   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #E7D7   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #E7D8   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #E7D9   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #E7DA   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #E7FA   ; Number of entries in active_entity_list
hero_entity_id      EQU #E7FB   ; First current-screen entity flagged as player (#FF = none)
active_entity_list_dirty EQU #E7FC   ; 1=rebuild active_entity_list required
input_entity_list   EQU #E7FD   ; Active current-screen entities with Input component (MAX_ENTITIES bytes)
input_entity_count  EQU #E81D   ; Number of entries in input_entity_list
render_entity_list  EQU #E81E   ; Active current-screen entities with Sprite component (MAX_ENTITIES bytes)
render_entity_count EQU #E83E   ; Number of entries in render_entity_list
collision_entity_list EQU #E83F   ; Active current-screen entities with Collision component (MAX_ENTITIES bytes)
collision_entity_count EQU #E85F   ; Number of entries in collision_entity_list
ground_entity_list  EQU #E860   ; Active current-screen entities with Collision or Gravity (MAX_ENTITIES bytes)
ground_entity_count EQU #E880   ; Number of entries in ground_entity_list
anim_entity_list    EQU #E881   ; Active current-screen entities with Animation+Sprite (MAX_ENTITIES bytes)
anim_entity_count   EQU #E8A1   ; Number of entries in anim_entity_list

; Entity-entity collision optimized variables
coll_list           EQU #E8A2   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #E8C2   ; Number of entities in coll_list
coll_src_left       EQU #E8C3   ; Source AABB left edge (scratch)
coll_src_right      EQU #E8C4   ; Source AABB right edge (scratch)
coll_src_top        EQU #E8C5   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #E8C6   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #E8C7   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #E8C7   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #E8C9   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #E8CB   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #E8CD   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #E8CF   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #E8D1   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #E8D3   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #E8D5   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #E8D7   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #E8D8   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #E8DD   ; Frame counter (16-bit)
task_exec_time          EQU #E8DF   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #E8E1   ; Set to 1 on each VBlank (1 byte)
RAM_INTERRUPT_END       EQU #E8E2   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #E8E2   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #E8E3   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #E8E4   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #E8E5   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #E8E6   ; 0=stopped, 1=track active
music_muted          EQU #E8E7   ; 0=audible, 1=muted/pause
music_loop           EQU #E8E8   ; 0=no loop, 1=loop enabled
music_track_index    EQU #E8E9   ; Current ROM track index
music_row_frames     EQU #E8EA   ; Frames per tracker row
music_row_countdown  EQU #E8EB   ; Countdown to next row
music_order_pos      EQU #E8EC   ; Current order position
music_pattern_index  EQU #E8ED   ; Current pattern index
music_pattern_row    EQU #E8EE   ; Current row inside pattern
music_pattern_rows   EQU #E8EF   ; Cached rows in current pattern
music_track_ptr_l    EQU #E8F0   ; Current track pointer low byte
music_track_ptr_h    EQU #E8F1   ; Current track pointer high byte
music_pattern_ptr_l  EQU #E8F2   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #E8F3   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #E8F4   ; PSG mixer shadow for music runtime
music_ch_note_base EQU #E8F5   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #E8F5   ; Channel A
music_ch_b_note EQU #E8F6   ; Channel B
music_ch_c_note EQU #E8F7   ; Channel C
music_ch_instrument_base EQU #E8F8   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #E8F8   ; Channel A
music_ch_b_instrument EQU #E8F9   ; Channel B
music_ch_c_instrument EQU #E8FA   ; Channel C
music_ch_ornament_base EQU #E8FB   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #E8FB   ; Channel A
music_ch_b_ornament EQU #E8FC   ; Channel B
music_ch_c_ornament EQU #E8FD   ; Channel C
music_ch_volume_base EQU #E8FE   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #E8FE   ; Channel A
music_ch_b_volume EQU #E8FF   ; Channel B
music_ch_c_volume EQU #E900   ; Channel C
music_ch_vol_step_base EQU #E901   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #E901   ; Channel A
music_ch_b_vol_step EQU #E902   ; Channel B
music_ch_c_vol_step EQU #E903   ; Channel C
music_ch_tone_step_base EQU #E904   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #E904   ; Channel A
music_ch_b_tone_step EQU #E905   ; Channel B
music_ch_c_tone_step EQU #E906   ; Channel C
music_ch_noise_step_base EQU #E907   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #E907   ; Channel A
music_ch_b_noise_step EQU #E908   ; Channel B
music_ch_c_noise_step EQU #E909   ; Channel C
music_ch_orn_step_base EQU #E90A   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #E90A   ; Channel A
music_ch_b_orn_step EQU #E90B   ; Channel B
music_ch_c_orn_step EQU #E90C   ; Channel C
...
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #E90D   ; End of project variables (10509 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#E90D: Project variables (10509 bytes)
;   #E90D-#F37F: Free RAM (~2675 bytes available)
...
; ==================================================================
tile_pattern_bank0:
    ; ZX0 compressed tile_pattern (72 -> 55 bytes)
...
; ==================================================================
tile_color_bank0:
    ; ZX0 compressed tile_color (72 -> 23 bytes)
    DB #95,#61,#69,#A1,#68,#B1,#AA,#F1,#B1,#7E,#F7,#F2,#78,#FE,#89,#81
    DB #F1,#23,#C1,#F1,#55,#55,#80
load_color_bank0:
...
SCREEN_PAN1_0_BLOCK_CATALOG:
    ; ZX0 compressed screen_block_catalog (464 -> 126 bytes)
    DB #95,#FF,#8E,#80,#81,#FD,#FE,#82,#F0,#E0,#A5,#82,#F5,#C0,#EA,#EA
    DB #83,#78,#F0,#F7,#B0,#DF,#F0,#66,#6A,#84,#A8,#87,#84,#E4,#44,#A1
    DB #85,#E7,#78,#AE,#A8,#83,#CE,#5F,#DD,#93,#E6,#B9,#3C,#40,#69,#87
    DB #E9,#9D,#80,#F7,#40,#87,#78,#BA,#D5,#84,#6E,#8F,#84,#F8,#3D,#EE
    DB #FD,#C5,#DE,#FF,#FE,#7B,#C3,#18,#70,#80,#DC,#94,#3F,#9A,#97,#7A
    DB #E0,#9F,#DE,#E5,#60,#DD,#22,#7D,#5E,#FC,#2C,#97,#E0,#F6,#83,#FD
    DB #0C,#1F,#FD,#F8,#E0,#72,#F8,#B3,#E0,#3D,#50,#F7,#E0,#71,#40,#F2
    DB #FD,#C8,#5F,#E0,#FE,#E4,#E0,#5D,#97,#60,#F5,#E0,#55,#58
SCREEN_PAN1_0_BLOCK_MAP:
    ; ZX0 compressed screen_block_map (48 -> 42 bytes)
    DB #96,#00,#21,#01,#02,#94,#A0,#03,#04,#00,#05,#00,#06,#07,#08,#09
    DB #0A,#0B,#0C,#0D,#00,#4D,#0E,#0F,#04,#10,#11,#12,#13,#14,#00,#0F
    DB #15,#16,#17,#18,#19,#1A,#1B,#1C,#55,#56
...
SCREEN_PAN1_0_INTERACTION_TYPE_MAP:
    ...
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#05,#00,#00,#00,#00,#00,#07,#00,#00,#00,#00,#07,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    ...
    DB #00,#00,#00,#00,#00,#00,#00,#00,#07,#00,#00,#00,#00,#07,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#01,#00,#00,#00,#07,#00,#05,#00,#00,#07,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
...
SCREEN_PAN1_0_INTERACTION_VALUE_MAP:
    ...
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#01
    DB #01,#00,#01,#01,#01,#01,#00,#00,#01,#00,#00,#00,#00,#01,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#01,#01,#00,#00,#00,#00,#00,#01
    DB #01,#00,#00,#00,#01,#01,#01,#00,#01,#00,#00,#00,#00,#01,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#01,#01,#00,#00,#00,#00,#00,#01
    DB #01,#00,#00,#00,#00,#00,#01,#00,#01,#00,#00,#00,#00,#01,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#01
...
anim_tile_table:
    ...
    db 132, 1, 8, 8, 16    ; escala -> tile tile_1776514675238
    dw anim_transform_0_escala
    db 255                          ; End marker
...
anim_transform_0_escala:
    ; Group "escala" targetChar=132 chars=1
    ; Frame 0: transform_step_0
    db #7E, #42, #42, #42, #7E, #42, #42, #42, #A1, #A1, #A1, #A1, #A1, #A1, #A1, #A1
    ; Frame 1: transform_step_1
    db #42, #42, #42, #7E, #42, #42, #42, #7E, #A1, #A1, #A1, #A1, #A1, #A1, #A1, #A1
    ; Frame 2: transform_step_2
    db #42, #42, #7E, #42, #42, #42, #7E, #42, #A1, #A1, #A1, #A1, #A1, #A1, #A1, #A1
    ; Frame 3: transform_step_3
    db #42, #7E, #42, #42, #42, #7E, #42, #42, #A1, #A1, #A1, #A1, #A1, #A1, #A1, #A1
...
SM_New_Statemachine_state_1776707744092_Transitions_Actions_2: 
    DB 28; REPLACE_TILE 
    DB 136, 1        ; REPLACE_TILE tile=tile_1776787670830=>136, dir=down
    DB 0xFF; END
