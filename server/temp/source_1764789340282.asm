...
load_sprite_patterns:
    ...
    ld bc, 32
    call LDIRVM
    ; Load sprite 2: (reusing sprite 0 for entity)
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (2 * 32)
    ld bc, 32
    call LDIRVM
    ; Load sprite 3: (reusing sprite 0 for entity)
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (3 * 32)
    ld bc, 32
    call LDIRVM
    ; Load sprite 4: (reusing sprite 0 for entity)
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (4 * 32)
    ld bc, 32
    call LDIRVM
    ; Load sprite 5: (reusing sprite 0 for entity)
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (5 * 32)
    ld bc, 32
    call LDIRVM
    ; Load sprite 6: (reusing sprite 0 for entity)
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (6 * 32)
    ld bc, 32
    call LDIRVM
    ; Load sprite 7: (reusing sprite 0 for entity)
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (7 * 32)
    ld bc, 32
    call LDIRVM
    ; Load sprite 8: (reusing sprite 0 for entity)
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (8 * 32)
    ld bc, 32
    call LDIRVM
    ; Load sprite 9: (reusing sprite 0 for entity)
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (9 * 32)
    ld bc, 32
    call LDIRVM
    ; Load sprite 10: (reusing sprite 0 for entity)
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (10 * 32)
    ld bc, 32
    call LDIRVM
    ; Load sprite 11: (reusing sprite 0 for entity)
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (11 * 32)
    ld bc, 32
    call LDIRVM
...
; ==================================================================
; sprite_attributes: ds 48  ; Interleaved buffer: Y, X, Pattern, Color
; active_sprite_count: db 0
;
; Total: 49 bytes of RAM
...
init_nucleo_6:
    ...
    ; JSON position: (100, 11) tiles = (255, 88) pixels
    ; Template: tpl_1764698890994_fiwnc
    ; Components: Position
    ; Direction mask: #0F (1111b) = All directions
    ...
    ld d, 0
    add hl, de
    ld (hl), 255         ; Set real X position from JSON
...
.render_loop:
    ...
    inc hl
    ld b, (hl)                  ; B = Width (content)
    inc hl
    ld c, (hl)                  ; C = Height (content)
    inc hl
    ld a, (hl)                  ; A = Flags
    inc hl
    push af                     ; Save Flags
    
    ld a, (hl)                  ; Text pointer low
    inc hl
    ld h, (hl)                  ; Text pointer high
    ld l, a                     ; HL = Text pointer
    push hl                     ; Save Text pointer (stack: Flags, TextPtr)
    
    ; Move to Visible flag (HL was at TextPtr High)
    pop hl                      ; Restore Text pointer temporarily to advance? No.
    ; We need to advance the original HL (data pointer)
    ; Let's reload data pointer from stack to be safe? No, just use offsets.
    ; Current HL is TextPtr High from the read sequence.
    ; Wait, I messed up the register usage.
    ; Let's restart the read sequence cleanly.
    
    ; Stack: Counter, DataPtr
    pop hl                      ; Restore DataPtr
    push hl                     ; Put it back
    
    ; Offset to TextPtr (Type=0, X=1, Y=2, W=3, H=4, Flags=5, TextPtr=6, Visible=8)
    inc hl
    inc hl
    inc hl
    inc hl
    ...
    inc hl                      ; At TextPtr Low
    ld c, (hl)
    inc hl
    ld b, (hl)                  ; BC = Text Pointer
    ...
    ; Check if visible
    or a
    jr z, .skip_element_pop_flags

    ; ---------------------------------------------------------
    ; 1. Draw Frame (if enabled)
    ; ---------------------------------------------------------
    pop af                      ; Restore Flags
    push af                     ; Keep it for later
    bit 0, a                    ; Check Bit 0 (Border)
    jr z, .no_border

    ; Draw Border
    ; Inputs: D=X, E=Y (Pixels), B=Width, C=Height (Tiles/Chars)
    ; We need to convert Pixel X,Y to Tile X,Y for the frame routine
    ...
    pop hl                      ; Recover X/Y (into HL temporarily to swap)
    pop bc                      ; Recover Width/Height
    push bc                     ; Save Width/Height again
    push hl                     ; Save X/Y again
...
.no_border:
    ...
    ; Text Pointer is in BC
    ld d, b
    ld e, c                     ; DE = Text Pointer
...
.skip_element_pop_flags:
    pop af                      ; Pop unused flags

.skip_element:
    pop hl                      ; Restore data pointer
    ld de, 9                    ; Size of each element entry (updated size)
...
.top_edge_loop:
    ...
    ld a, c
    sub 2               ; Height - 2 rows
    jr z, .bottom_row   ; Skip if height is small (shouldn't happen)
