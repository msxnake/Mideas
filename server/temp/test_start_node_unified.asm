; ==================================================================
; TEST_START_NODE - MEGAROM UNIFIED FILE
...
; Bank 3 [#A000-#C000]: gameflow (5155/8192 bytes est.)
...
; Bank 7 [#6000-#8000]: sprites (2127/8192 bytes est.) [FAR — accessed via trampoline]
; Bank 8 [#8000-#A000]: screens_code (185/8192 bytes est.) [FAR — accessed via trampoline]
; Bank 9 [#A000-#C000]: worlds (152/8192 bytes est.) [FAR — accessed via trampoline]
...
; BANK 00 @#07F6 : worlds.asm (152 bytes)
; BANK 00 @#088E : screens.asm (185 bytes)
; BANK 00 @#0947 : sprites.asm (2127 bytes)
; BANK 00 @#1196 : font.asm (174 bytes)
; BANK 00 @#1244 : hud.asm (79 bytes)
; BANK 00 @#1293 : menus.asm (168 bytes)
; BANK 00 @#133B : sound.asm part 1/2 (3269 bytes)
; BANK 01 @#0000 : sound.asm part 2/2 (6264 bytes)
; BANK 01 @#1878 : scroll.asm (1928 bytes)
; BANK 02 @#0000 : scroll.asm (414 bytes)
; BANK 02 @#019E : animtiles.asm (4577 bytes)
; BANK 02 @#137F : statemachine.asm (5 bytes)
; BANK 02 @#1384 : gameflow.asm (3196 bytes)
; BANK 03 @#0000 : gameflow.asm (1960 bytes); ==================================================================
...
; SYSTEM VARIABLES
; ==================================================================
ROM_slot            EQU #C013   ; Expanded slot for normal page 1 ROM access
slot_primary_normal EQU #C014   ; Primary slot register snapshot for BIOS-ROM-ROM-RAM layout
page0_bios_slot     EQU #C015   ; Expanded slot for normal BIOS page 0
page2_normal_slot   EQU #C016   ; Expanded slot for normal page 2 layout
page3_normal_slot   EQU #C017   ; Expanded slot for normal RAM page 3
...
page0_transfer_buffer EQU #C038   ; Temporary RAM buffer for page0 -> VRAM copies

; ==================================================================
; SCREEN MAP POINTERS (Current active screen)
; ==================================================================
current_screen_layout   EQU #C138   ; Pointer to current screen layout data (16-bit)
current_screen_layout_bank EQU #C13A   ; Mapper bank for current screen layout data
current_behavior_map    EQU #C13B   ; Pointer to current behavior map data (16-bit)
current_behavior_map_bank EQU #C13D   ; Mapper bank for current behavior map data
behavior_cache_row     EQU #C13E   ; Cached behavior row (255=invalid)
behavior_cache_map_l   EQU #C13F   ; Cached behavior map pointer low byte
behavior_cache_map_h   EQU #C140   ; Cached behavior map pointer high byte
behavior_cache_row_base EQU #C141   ; Cached row base address in behavior map (16-bit)
RUNTIME_SCREEN_MAP_SIZE EQU 768
MAX_RUNTIME_EFFECT_ZONES EQU 64
runtime_background_layout EQU #C143   ; Immutable copy of current background layout (32x24)
runtime_screen_layout  EQU #C443   ; Mutable copy of current screen layout (32x24)
runtime_behavior_map   EQU #C743   ; Mutable copy of current behavior map (32x24)
runtime_effects_layout EQU #CA43   ; Alternate effects layout copy for secret zones (32x24)
runtime_effect_zone_table EQU #CD43   ; Current screen effect zone table (MAX_RUNTIME_EFFECT_ZONES * 8 bytes)
current_effect_zone_count EQU #CF43   ; Number of effect zones copied into runtime_effect_zone_table
secret_zone_active EQU #CF44   ; 1 if hero is currently inside an active secret zone
secret_zone_rect_x EQU #CF45   ; Active secret zone rect X in cells
secret_zone_rect_y EQU #CF46   ; Active secret zone rect Y in cells
secret_zone_rect_w EQU #CF47   ; Active secret zone rect width in cells
secret_zone_rect_h EQU #CF48   ; Active secret zone rect height in cells

; ==================================================================
; VIEWPORT/CAMERA VARIABLES (for scroll system)
; ==================================================================
camera_x            EQU #CF49   ; Camera X position in pixels (16-bit)
camera_y            EQU #CF4B   ; Camera Y position in pixels (16-bit)
camera_tile_x       EQU #CF4D   ; Camera tile X (column)
camera_tile_y       EQU #CF4E   ; Camera tile Y (row)
world_width_tiles   EQU #CF4F   ; World width in tiles
world_height_tiles  EQU #CF50   ; World height in tiles
scroll_dirty_flag   EQU #CF51   ; 1=viewport changed, needs redraw
hud_dirty_flag      EQU #CF52   ; 1=HUD needs redraw, 0=clean
time_second_frame_counter EQU #CF53   ; VBlank frames remaining until the next TimeRemaining decrement
time_last_interrupt_counter EQU #CF54   ; Last interrupt_counter snapshot used by TimeRemaining sync (16-bit)

; ==================================================================
; ANIMATED TILES VARIABLES
; ==================================================================
anim_tile_timer     EQU #CF56   ; Animation frame timer
anim_tile_frame     EQU #CF57   ; Current animation frame (0-3)
anim_tile_speed     EQU #CF58   ; Frames between animation updates
anim_tile_transform_flags EQU #CF59   ; Runtime flags for transform-mode tile animation (byte0=flags, byte1=opcode scratch)
anim_tile_row_buffer EQU #CF5B   ; Temp buffer (8 bytes) for row transforms

; ==================================================================
; ENTITY SYSTEM VARIABLES (Fixed 32 entities)
; ==================================================================
MAX_ENTITIES        EQU 32
entity_active       EQU #CF63   ; Entity active flags (32 bytes, 0=inactive, 1=active)
entity_is_player    EQU #CF83   ; Entity hero/player flag (32 bytes, 0=no, 1=yes)
entity_x_pos        EQU #CFA3   ; Entity X positions (32 bytes)
entity_y_pos        EQU #CFC3   ; Entity Y positions (32 bytes)
entity_vel_x        EQU #CFE3   ; Entity X velocity (32 bytes)
entity_vel_y        EQU #D003   ; Entity Y velocity (32 bytes)
entity_comp_masks   EQU #D023   ; Entity component masks (32 bytes)
entity_comp_masks_hi EQU #D043   ; Entity component masks high byte (32 bytes)
entity_screen_id    EQU #D063   ; Entity screen ID (32 bytes)
entity_job_period   EQU #D083   ; Entity job period in frames (32 bytes, 1=100%,2=50%,3=33%,4=25%)
entity_job_entry    EQU #D0A3   ; Entity job entry slot within period window (32 bytes)
entity_job_scheduler_active EQU #D0C3   ; 1 when any entity uses non-default job cadence
entity_dir_mask     EQU #D0C4   ; Entity direction mask (32 bytes)
entity_input_speed  EQU #D0E4   ; Entity input/cursor speed (32 bytes)
entity_health       EQU #D104   ; Entity health (32 bytes)
entity_anim_frame   EQU #D124   ; Entity animation frame (32 bytes)
entity_anim_tick    EQU #D144   ; Entity animation tick counter (32 bytes)
entity_anim_speed   EQU #D164   ; Entity animation speed (ticks per frame) (32 bytes)
entity_anim_flags   EQU #D184   ; Entity animation flags (32 bytes)
entity_sm_ptr_l     EQU #D1A4   ; Entity State Pointer Low (32 bytes)
entity_sm_ptr_h     EQU #D1C4   ; Entity State Pointer High (32 bytes)
entity_sm_timer_l   EQU #D1E4   ; Entity State Timer Low (32 bytes)
entity_sm_timer_h   EQU #D204   ; Entity State Timer High (32 bytes)
entity_sm_wait_timer EQU #D224   ; Entity State Wait Timer (32 bytes)
entity_lifetime     EQU #D244   ; Entity lifetime for auto-destroy (32 bytes, 0=infinite)
entity_carried_by   EQU #D264   ; Entity carrier ID (32 bytes, 255=not carried)
entity_template_token EQU #D284   ; Entity template token (32 bytes, 0=unknown)
entity_facing_dir   EQU #D2A4   ; Last facing direction (32 bytes, 0=none,1=left,2=right,3=up,4=down)
entity_sm_var_0     EQU #D2C4   ; Entity Variable 0 (32 bytes)
entity_sm_var_1     EQU #D2E4   ; Entity Variable 1 (32 bytes)
entity_sm_var_2     EQU #D304   ; Entity Variable 2 (32 bytes)
entity_sm_var_3     EQU #D324   ; Entity Variable 3 (32 bytes)
entity_sm_var_4     EQU #D344   ; Entity Variable 4 (32 bytes)
entity_sm_var_5     EQU #D364   ; Entity Variable 5 (32 bytes)
entity_sm_var_6     EQU #D384   ; Entity Variable 6 (32 bytes)
entity_sm_var_7     EQU #D3A4   ; Entity Variable 7 (32 bytes)

; ==================================================================
; SPRITE SYSTEM VARIABLES
; ==================================================================
entity_sprite_asset_index EQU #D3C4   ; Entity sprite asset index - RAM copy (32 bytes)
active_sprite_count EQU #D3E4   ; Number of sprites currently active
sprites_dirty      EQU #D3E5   ; 1=sprite_attributes changed, needs VRAM sync
sprite_pattern      EQU #D3E6   ; Sprite pattern IDs (32 bytes)
sprite_color        EQU #D406   ; Sprite colors (32 bytes)
sprite_layer_colors EQU #D426   ; HW sprite layer color cache - RAM copy (32 bytes, indexed by HW sprite index)
sprite_asset_base_pattern_slot_runtime EQU #D446   ; Runtime base 16x16 slot per sprite asset (1 bytes)
sprite_placeholder_base_pattern_num EQU #D447   ; Runtime placeholder pattern number (base slot * 4)
sprite_attributes   EQU #D448   ; Interleaved sprite attributes (32 * 4 bytes)

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #D4C8   ; Player X position (16-bit)
player_y            EQU #D4CA   ; Player Y position (16-bit)
player_runtime_enabled EQU #D4CC   ; 1=player fast runtime bound to hero entity
player_entity_index EQU #D4CD   ; Entity index used by player fast runtime (#FF=none)
player_vx_runtime   EQU #D4CE   ; Cached player X velocity (signed 8-bit)
player_vy_runtime   EQU #D4CF   ; Cached player Y velocity (signed 8-bit)
player_health       EQU #D4D0   ; Player health points
player_score        EQU #D4D1   ; Player score (16-bit)
gem_count           EQU #D4D3   ; Collectible tile counter (8-bit)
last_gem_char       EQU #D4D4   ; Char code of last collected gem tile (for SM VARIABLE_COMPARE)

; Persistent collectibles list (survives screen re-entry)
MAX_COLLECTIBLES     EQU 64              ; Max persistent collectible records
collected_count      EQU #D4D5   ; Number of collected tiles recorded (8-bit)
collected_world      EQU #D4D6   ; World IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_screen     EQU #D516   ; Screen IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_idx_l      EQU #D556   ; Tile name-table index low byte (MAX_COLLECTIBLES bytes)
collected_idx_h      EQU #D596   ; Tile name-table index high byte (MAX_COLLECTIBLES bytes)

; Timed bonus tile respawn slots (bonus gem regeneration)
MAX_BONUS_RESPAWNS   EQU 16              ; Max timed bonus tiles waiting to respawn
bonus_respawn_world  EQU #D5D6   ; World IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_screen EQU #D5E6   ; Screen IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_l  EQU #D5F6   ; Tile index low byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_h  EQU #D606   ; Tile index high byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_secs   EQU #D616   ; Remaining seconds per timed respawn slot (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_frames EQU #D626   ; Frame countdown (60..1) per timed respawn slot (MAX_BONUS_RESPAWNS bytes)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #D636   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #D637   ; Temporary 16-bit storage
temp_word_2         EQU #D639   ; Temporary 16-bit storage
temp_byte_1         EQU #D63B   ; Temporary 8-bit storage
temp_byte_2         EQU #D63C   ; Temporary 8-bit storage
temp_byte_3         EQU #D63D   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #D65D   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #D67D   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #D69D   ; Temporary 8-bit storage (32 bytes)

; ==================================================================
; SOUND SYSTEM VARIABLES
; ==================================================================
sfx_active          EQU #D6BD   ; 0=no SFX active, 1=playing
sfx_timer           EQU #D6BE   ; Frames remaining for current SFX
sfx_fadeout         EQU #D6BF   ; Reserved fadeout flag/state
temp_byte_7         EQU #D6C0   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #D6E0   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #D700   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #D720   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #D740   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #D760   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #D780   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #D7A0   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #D7C0   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #D7E0   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #D800   ; Temporary 8-bit storage (32 bytes)
temp_byte_18        EQU #D820   ; Temporary 8-bit storage (32 bytes)
temp_byte_19        EQU #D840   ; Temporary 8-bit storage (32 bytes)
temp_byte_20        EQU #D860   ; Temporary 8-bit storage (32 bytes)
temp_byte_21        EQU #D880   ; Temporary 8-bit storage (32 bytes)
temp_byte_22        EQU #D8A0   ; Temporary 8-bit storage (32 bytes)
temp_byte_23        EQU #D8C0   ; Temporary 8-bit storage (32 bytes)
temp_byte_24        EQU #D8E0   ; Temporary 8-bit storage (32 bytes)
temp_byte_25        EQU #D900   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #D920   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #D960   ; Temporary 16-bit storage (64 bytes)
temp_byte_26        EQU #D9A0   ; Temporary 8-bit storage (32 bytes)
temp_byte_27        EQU #D9C0   ; Temporary 8-bit storage (32 bytes)
temp_byte_28        EQU #D9E0   ; Temporary 8-bit storage (32 bytes)
tileDead_dbg        EQU #DA00   ; Debug byte: current hero deadly contact
tileDead_latched_dbg EQU #DA01   ; Debug byte: latched hero deadly contact
tileDead_x_dbg      EQU #DA02   ; Debug byte: last sampled deadly tile X
tileDead_y_dbg      EQU #DA03   ; Debug byte: last sampled deadly tile Y
tileDead_value_dbg  EQU #DA04   ; Debug byte: last raw deadly behavior value

; Wall collision temporary variables
wall_temp_x         EQU #DA05   ; Cached entity X for wall checks
wall_temp_y         EQU #DA06   ; Cached entity Y for wall checks
wall_hit_left       EQU #DA07   ; Hitbox left edge cache
wall_hit_top        EQU #DA08   ; Hitbox top edge cache
wall_hit_right      EQU #DA09   ; Hitbox right edge cache
wall_hit_bottom     EQU #DA0A   ; Hitbox bottom edge cache
wall_hit_w          EQU #DA0B   ; Hitbox width cache (min 1)
wall_hit_h          EQU #DA0C   ; Hitbox height cache (min 1)
wall_probe_left     EQU #DA0D   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #DA0E   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #DA0F   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #DA10   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #DA11   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #DA31   ; Number of entries in active_entity_list
hero_entity_id      EQU #DA32   ; First current-screen entity flagged as player (#FF = none)
active_entity_list_dirty EQU #DA33   ; 1=rebuild active_entity_list required
input_entity_list   EQU #DA34   ; Active current-screen entities with Input component (MAX_ENTITIES bytes)
input_entity_count  EQU #DA54   ; Number of entries in input_entity_list
render_entity_list  EQU #DA55   ; Active current-screen entities with Sprite component (MAX_ENTITIES bytes)
render_entity_count EQU #DA75   ; Number of entries in render_entity_list
collision_entity_list EQU #DA76   ; Active current-screen entities with Collision component (MAX_ENTITIES bytes)
collision_entity_count EQU #DA96   ; Number of entries in collision_entity_list
ground_entity_list  EQU #DA97   ; Active current-screen entities with Collision or Gravity (MAX_ENTITIES bytes)
ground_entity_count EQU #DAB7   ; Number of entries in ground_entity_list
anim_entity_list    EQU #DAB8   ; Active current-screen entities with Animation+Sprite (MAX_ENTITIES bytes)
anim_entity_count   EQU #DAD8   ; Number of entries in anim_entity_list

; Entity-entity collision optimized variables
coll_list           EQU #DAD9   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #DAF9   ; Number of entities in coll_list
coll_src_left       EQU #DAFA   ; Source AABB left edge (scratch)
coll_src_right      EQU #DAFB   ; Source AABB right edge (scratch)
coll_src_top        EQU #DAFC   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #DAFD   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #DAFE   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #DAFE   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #DB00   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #DB02   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #DB04   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #DB06   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #DB08   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #DB0A   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #DB0C   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #DB0E   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #DB0F   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #DB14   ; Frame counter (16-bit)
task_exec_time          EQU #DB16   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #DB18   ; Set to 1 on each VBlank (1 byte)
RAM_INTERRUPT_END       EQU #DB19   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #DB19   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #DB1A   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #DB1B   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #DB1C   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #DB1D   ; 0=stopped, 1=track active
music_muted          EQU #DB1E   ; 0=audible, 1=muted/pause
music_loop           EQU #DB1F   ; 0=no loop, 1=loop enabled
music_track_index    EQU #DB20   ; Current ROM track index
music_row_frames     EQU #DB21   ; Frames per tracker row
music_row_countdown  EQU #DB22   ; Countdown to next row
music_order_pos      EQU #DB23   ; Current order position
music_pattern_index  EQU #DB24   ; Current pattern index
music_pattern_row    EQU #DB25   ; Current row inside pattern
music_pattern_rows   EQU #DB26   ; Cached rows in current pattern
music_track_ptr_l    EQU #DB27   ; Current track pointer low byte
music_track_ptr_h    EQU #DB28   ; Current track pointer high byte
music_pattern_ptr_l  EQU #DB29   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #DB2A   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #DB2B   ; PSG mixer shadow for music runtime
music_ch_note_base EQU #DB2C   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #DB2C   ; Channel A
music_ch_b_note EQU #DB2D   ; Channel B
music_ch_c_note EQU #DB2E   ; Channel C
music_ch_instrument_base EQU #DB2F   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #DB2F   ; Channel A
music_ch_b_instrument EQU #DB30   ; Channel B
music_ch_c_instrument EQU #DB31   ; Channel C
music_ch_ornament_base EQU #DB32   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #DB32   ; Channel A
music_ch_b_ornament EQU #DB33   ; Channel B
music_ch_c_ornament EQU #DB34   ; Channel C
music_ch_volume_base EQU #DB35   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #DB35   ; Channel A
music_ch_b_volume EQU #DB36   ; Channel B
music_ch_c_volume EQU #DB37   ; Channel C
music_ch_vol_step_base EQU #DB38   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #DB38   ; Channel A
music_ch_b_vol_step EQU #DB39   ; Channel B
music_ch_c_vol_step EQU #DB3A   ; Channel C
music_ch_tone_step_base EQU #DB3B   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #DB3B   ; Channel A
music_ch_b_tone_step EQU #DB3C   ; Channel B
music_ch_c_tone_step EQU #DB3D   ; Channel C
music_ch_noise_step_base EQU #DB3E   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #DB3E   ; Channel A
music_ch_b_noise_step EQU #DB3F   ; Channel B
music_ch_c_noise_step EQU #DB40   ; Channel C
music_ch_orn_step_base EQU #DB41   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #DB41   ; Channel A
music_ch_b_orn_step EQU #DB42   ; Channel B
music_ch_c_orn_step EQU #DB43   ; Channel C
...
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #DB44   ; End of project variables (6980 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#DB44: Project variables (6980 bytes)
;   #DB44-#F37F: Free RAM (~6204 bytes available)
...
gameflow_node_start_001_data:
    ...
    db ((gameflow_node_start_001_init - #4000) / #2000)    ; Initialization routine bank
...
gameflow_node_end_001:
    db NODE_TYPE_END
    dw gameflow_no_data
    dw gameflow_node_end_001_conn

gameflow_node_end_001_conn:
    db CONNECTION_DEFAULT
    dw 0
    db CONNECTION_END
...
; Total data bytes (source blocks): 0
; Zones used: 0
; ------------------------------------------------------------------
; No banked data blocks emitted.
