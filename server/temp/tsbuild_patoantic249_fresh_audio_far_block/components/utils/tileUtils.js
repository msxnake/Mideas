        ...
        for (let cbX = 0; cbX < numCharBlocksX; cbX++) {
            ...
            for (let r = 0; r < EDITOR_BASE_TILE_DIM_S2; r++) {
                ...
                if (tile.lineAttributes[currentTileRow] && tile.lineAttributes[currentTileRow][cbX]) {
                    ...
                    const fgIdx = constants_1.MSX1_PALETTE_MAP.get(attrPair.fg)?.index ?? constants_1.DEFAULT_SCREEN2_FG_COLOR_INDEX;
                    const bgIdx = constants_1.MSX1_PALETTE_MAP.get(attrPair.bg)?.index ?? constants_1.DEFAULT_SCREEN2_BG_COLOR_INDEX;
                ...
                for (let pixelInBlock = 0; pixelInBlock < EDITOR_BASE_TILE_DIM_S2; pixelInBlock++) {
                    ...
                    if (pixelColor !== undefined) {
                        ...
                        if (isScreen2 && fgColorForSegment) {
                            ...
                            isPixelOn = (pixelColor !== constants_1.MSX_SCREEN5_PALETTE[0].hex && pixelColor !== tile.lineAttributes?.[0]?.[0]?.bg); // A very basic check
                        }
                        if (isPixelOn) {
                            patternByte |= (1 << (7 - pixelInBlock));
                        }
