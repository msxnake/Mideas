...
function formatManifestMapperName(targetFormat) {
    switch (targetFormat) {
        case 'konami':
            return 'KONAMI8K';
        case 'ascii8':
...
function formatManifestGroupName(groupKey) {
    return groupKey.trim().toLowerCase();
}
function inferManifestLifetime(resource) {
...
function inferManifestRuntimeTarget(resource) {
...
function buildManifestV2Id(packResult, mapperWindow, resources) {
...
function buildManifestV2Json(packResult, mapperWindow, resources) {
    ...
    return screens.map((screen, index) => {
        ...
        return {
            ...
            bosses: {
                ...
                bossIds: uniqueSorted(bossInstances.map((boss) => boss?.bossAssetId || boss?.bossId || boss?.id)),
            },
...
function collectPlacedBossAssetIdsForUsage(analysis) {
...
function collectUsedBossAttackRuntimeTypes(analysis) {
    const bosses = Array.isArray(analysis?.bosses) ? analysis?.bosses : [];
    const placedBossIds = collectPlacedBossAssetIdsForUsage(analysis);
    const usedTypes = new Set();
    const typeCounts = {};
    let referencedAttacks = 0;
    bosses
        .filter((boss) => placedBossIds.size === 0 || !boss?.id || placedBossIds.has(String(boss.id)))
        .forEach((boss) => {
        const attackById = new Map((Array.isArray(boss?.attacks) ? boss.attacks : [])
            .filter((attack) => attack?.id !== undefined && attack?.id !== null)
            .map((attack) => [String(attack.id), attack]));
        const usedAttackIds = new Set();
        (Array.isArray(boss?.phases) ? boss.phases : []).forEach((phase) => {
            (Array.isArray(phase?.attackSequence) ? phase.attackSequence : []).forEach((attackId) => {
                const normalized = asStringId(attackId);
                if (normalized) {
                    usedAttackIds.add(normalized);
                }
            });
            (Array.isArray(phase?.behaviorLoop) ? phase.behaviorLoop : []).forEach((action) => {
...
function collectUsedComponentRuntimeTypes(analysis) {
    if (!analysis) {
        return {
            usedComponents: [],
    ...
    const usage = (0, componentAnalyzer_1.analyzeComponentUsage)(analysis);
...
function addRuntimeType(type, ids, usedTypes, usedIds, counts, unknownTypes) {
    if (!type) {
        return;
    }
    const id = ids[type];
...
function collectUsedStateMachineRuntimeIds(analysis) {
    const stateMachines = Array.isArray(analysis?.stateMachines)
        ? (analysis?.stateMachines).map(unwrapAssetData)
        : [];
    ...
    const addAction = (action) => {
        addRuntimeType(String(action?.type || ''), stateMachineGenerator_1.ACTION_IDS, usedActionTypes, usedActionIds, actionTypeCounts, unknownActionTypes);
    };
    const addImplicitAlwaysTrueCondition = () => {
    ...
    const addCondition = (condition) => {
        ...
        addRuntimeType(type, stateMachineGenerator_1.CONDITION_IDS, usedConditionTypes, usedConditionIds, conditionTypeCounts, unknownConditionTypes);
...
function buildProjectUsageJson(analysis, resources, mapperWindow) {
    ...
    const gameFlow = unwrapAssetData(analysis?.gameFlow);
    const gameFlowNodes = Array.isArray(gameFlow?.nodes) ? gameFlow.nodes : [];
    const gameFlowSubMenuCount = gameFlowNodes.filter((node) => String(node?.type || '').toLowerCase() === 'submenu').length;
    const explicitMenuCount = Array.isArray(analysis?.menus)
