...
function generateNodeHandlers(nodeTypes, analysis, executionPlan, romMode = 'simple32k') {
    ...
    const hudRuntimeScreenIndexes = getHudRuntimeScreenIndexes(analysis);
    const hasHud = hudRuntimeScreenIndexes.length > 0;
    const visualNodeTypes = new Set(['WorldLink', 'SubMenu', 'Controls', 'Text', 'TextScroll', 'TextScrollColor', 'TextScroll2', 'End', 'Restart', 'PresentationScreen']);
...
`;
    ...
    const hasScreenColorRuntime = Array.isArray(analysis.screenMaps) && analysis.screenMaps.length > 0;
    const needsScreenColorRuntime = handlerNodeTypes.some((nodeType) => ['SubMenu', 'Controls', 'Text', 'TextScroll', 'TextScrollColor', 'Transition', 'End', 'Restart', 'PresentationScreen'].includes(nodeType));
...
`;
        switch (node.type) {
            ...
            case 'TextScrollColor': {
                ...
                const maxLineWidth = 28;
                const normalizeLine = (value) => sanitizeAsmText(value)
                    .toUpperCase()
                    .replace(/[^\x20-\x5F]/g, '?')
            ...
            case 'TextScroll2': {
                ...
                const normalizeLine = (value) => sanitizeAsmText(value)
                    ...
                    .replace(/[^\x20-\x5F]/g, '?')
                    .slice(0, maxLineWidth);
                const fixedLine = (value) => normalizeLine(value).padEnd(32, ' ').slice(0, 32);
                const lines = [];
                rawText.split(/\r?\n/).forEach((paragraph) => {
                    const words = paragraph.split(/\s+/).filter(Boolean);
                    if (words.length === 0) {
