...
gameflow_condition_result EQU #C00C   ; Result of last condition evaluation

; ==================================================================
; MIDEAS GLOBAL VARIABLES (DEFAULTS + CUSTOM)
; ==================================================================
global_var_goal     EQU #C00D   ; Goal status (0=Failure, 1=Completed)

; ==================================================================
; SYSTEM VARIABLES
; ==================================================================
ROM_slot            EQU #C00E   ; ROM slot number (for SETPAGES32K)
frame_counter       EQU #C00F   ; Frame counter (16-bit)

; ==================================================================
; SCREEN MAP POINTERS (Current active screen)
; ==================================================================
current_screen_layout   EQU #C011   ; Pointer to current screen layout data (16-bit)
current_behavior_map    EQU #C013   ; Pointer to current behavior map data (16-bit)

; ==================================================================
; VIEWPORT/CAMERA VARIABLES (for scroll system)
; ==================================================================
camera_x            EQU #C015   ; Camera X position in pixels (16-bit)
camera_y            EQU #C017   ; Camera Y position in pixels (16-bit)
camera_tile_x       EQU #C019   ; Camera tile X (column)
camera_tile_y       EQU #C01A   ; Camera tile Y (row)
world_width_tiles   EQU #C01B   ; World width in tiles
world_height_tiles  EQU #C01C   ; World height in tiles
scroll_dirty_flag   EQU #C01D   ; 1=viewport changed, needs redraw
hud_dirty_flag      EQU #C01E   ; 1=HUD needs redraw, 0=clean

; ==================================================================
; ANIMATED TILES VARIABLES
; ==================================================================
anim_tile_timer     EQU #C01F   ; Animation frame timer
anim_tile_frame     EQU #C020   ; Current animation frame (0-3)
anim_tile_speed     EQU #C021   ; Frames between animation updates

; ==================================================================
; PARTICLE SYSTEM VARIABLES
; ==================================================================
particle_pool       EQU #C022   ; Particle pool (8 particles * 8 bytes = 64 bytes)

; ==================================================================
; ENTITY SYSTEM VARIABLES (Fixed 32 entities)
; ==================================================================
MAX_ENTITIES        EQU 32
entity_active       EQU #C062   ; Entity active flags (32 bytes, 0=inactive, 1=active)
entity_x_pos        EQU #C082   ; Entity X positions (32 bytes)
entity_y_pos        EQU #C0A2   ; Entity Y positions (32 bytes)
entity_vel_x        EQU #C0C2   ; Entity X velocity (32 bytes)
entity_vel_y        EQU #C0E2   ; Entity Y velocity (32 bytes)
entity_comp_masks   EQU #C102   ; Entity component masks (32 bytes)
entity_comp_masks_hi EQU #C122   ; Entity component masks high byte (32 bytes)
entity_screen_id    EQU #C142   ; Entity screen ID (32 bytes)
entity_dir_mask     EQU #C162   ; Entity direction mask (32 bytes)
entity_health       EQU #C182   ; Entity health (32 bytes)
entity_anim_frame   EQU #C1A2   ; Entity animation frame (32 bytes)
entity_anim_tick    EQU #C1C2   ; Entity animation tick counter (32 bytes)
entity_anim_speed   EQU #C1E2   ; Entity animation speed (ticks per frame) (32 bytes)
entity_anim_flags   EQU #C202   ; Entity animation flags (32 bytes)
entity_sm_ptr_l     EQU #C222   ; Entity State Pointer Low (32 bytes)
entity_sm_ptr_h     EQU #C242   ; Entity State Pointer High (32 bytes)
entity_sm_timer_l   EQU #C262   ; Entity State Timer Low (32 bytes)
entity_sm_timer_h   EQU #C282   ; Entity State Timer High (32 bytes)
entity_sm_wait_timer EQU #C2A2   ; Entity State Wait Timer (32 bytes)
entity_lifetime     EQU #C2C2   ; Entity lifetime for auto-destroy (32 bytes, 0=infinite)
entity_carried_by   EQU #C2E2   ; Entity carrier ID (32 bytes, 255=not carried)
entity_sm_var_0     EQU #C302   ; Entity Variable 0 (32 bytes)
entity_sm_var_1     EQU #C322   ; Entity Variable 1 (32 bytes)
entity_sm_var_2     EQU #C342   ; Entity Variable 2 (32 bytes)
entity_sm_var_3     EQU #C362   ; Entity Variable 3 (32 bytes)
entity_sm_var_4     EQU #C382   ; Entity Variable 4 (32 bytes)
entity_sm_var_5     EQU #C3A2   ; Entity Variable 5 (32 bytes)
entity_sm_var_6     EQU #C3C2   ; Entity Variable 6 (32 bytes)
entity_sm_var_7     EQU #C3E2   ; Entity Variable 7 (32 bytes)

; ==================================================================
; SPRITE SYSTEM VARIABLES
; ==================================================================
entity_sprite_asset_index EQU #C402   ; Entity sprite asset index - RAM copy (32 bytes)
active_sprite_count EQU #C422   ; Number of sprites currently active
sprite_pattern      EQU #C423   ; Sprite pattern IDs (32 bytes)
sprite_color        EQU #C443   ; Sprite colors (32 bytes)
sprite_attributes   EQU #C463   ; Interleaved sprite attributes (32 * 4 bytes)

; ==================================================================
; SCREEN SYSTEM VARIABLES (2 screens detected)
; ==================================================================
current_screen_id   EQU #C4E3   ; Currently displayed screen ID
screen_dirty_flag   EQU #C4E4   ; Screen needs redraw flag
screen_transition_cooldown EQU #C4E5   ; Cooldown frames after screen transition
current_world_id    EQU #C4E6   ; Current world ID (for multi-world support)
current_screen_index EQU #C4E7   ; Current screen index within world

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #C4E8   ; Player X position (16-bit)
player_y            EQU #C4EA   ; Player Y position (16-bit)
player_health       EQU #C4EC   ; Player health points
player_score        EQU #C4ED   ; Player score (16-bit)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #C4EF   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #C4F0   ; Temporary 16-bit storage
temp_word_2         EQU #C4F2   ; Temporary 16-bit storage
temp_byte_1         EQU #C4F4   ; Temporary 8-bit storage
temp_byte_2         EQU #C4F5   ; Temporary 8-bit storage
temp_byte_3         EQU #C4F6   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #C516   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #C536   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #C556   ; Temporary 8-bit storage (32 bytes)
temp_byte_7         EQU #C576   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #C596   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #C5B6   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #C5D6   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #C5F6   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #C616   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #C636   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #C656   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #C676   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #C696   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #C6B6   ; Temporary 8-bit storage (32 bytes)
temp_byte_18        EQU #C6D6   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #C6F6   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #C736   ; Temporary 16-bit storage (64 bytes)

; Wall collision temporary variables
wall_temp_x         EQU #C776   ; Cached entity X for wall checks
wall_temp_y         EQU #C777   ; Cached entity Y for wall checks
wall_entity_idx     EQU #C778   ; Current entity index in wall loop

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #C779   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #C779   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #C77B   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #C77D   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #C77F   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #C781   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #C783   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #C785   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #C787   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #C789   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #C78A   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #C78F   ; Frame counter (16-bit)
task_exec_time          EQU #C791   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #C793   ; Set to 1 on each VBlank (1 byte)
RAM_INTERRUPT_END       EQU #C794   ; End of interrupt system

; ==================================================================
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #C794   ; End of project variables (1940 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#C794: Project variables (1940 bytes)
;   #C794-#F37F: Free RAM (~11244 bytes available)
...
;   Format: DB option_count, DB initial_selection, DW title_ptr, DW option_ptr[n]
