...
//
// DYNAMIC BANKING (Konami4/Konami 8K P3 window, bank 4+):
//   Banks 0, 1 and 2 are treated as the resident 24 KB kernel.
//   Bank 3 is the default A000-BFFF resident window after boot.
//   Banked data is placed in banks 4+ (ORG #C000, #E000, #10000...).
...
// Far code must not execute from the active data window. Konami4 builds use
// P3/A000h as the only dynamic data window, so overlay code is constrained to
// P1. This keeps A000h available for LoadAsset/resource-table access.
const FAR_CODE_SLOT_ORG = [0x6000];
const FAR_CODE_SLOT_END = [0x8000];
const FAR_CODE_SLOT_PAGE = [1];
...
function formatPackedBankLayoutComment(banks) {
    ...
    lines.push('; Bank 4+ (data) [#C000+]: DATA mapped through the configured data window');
    lines.push('; ------------------------------------------------------------------');
    return lines.join('\n');
}
// ==================================================================
...
function buildSegmentBudgetJson(packedBanks, dataPack) {
    const budget = {
        version: 1,
        scope: 'konami8k_segment_budget',
        segmentSize: MEGAROM_BANK_SIZE,
        codeBanks: packedBanks.map((bank) => ({
            ...
            freeBytes: Math.max(0, MEGAROM_BANK_SIZE - bank.usedBytes),
            overBudget: bank.usedBytes > MEGAROM_BANK_SIZE,
...
function generateMegaromUnifiedFile(files, projectName, analysis, executionPlan, config, options) {
    ...
    // Build bank4+ sections: sprite data + pattern data + color data + screen data + font data + presentation screen
    // assembled at org #C000+. Labels are accessed through mapperWindow (Konami: P3/A000).
    ...
    const segmentBudgetJson = buildSegmentBudgetJson(packedBanks, dataPack);
