...
show_sprite:
    ; Use alternate registers to safely preserve pattern/color
    ex af, af'                    ; Save sprite number to A'
    exx                           ; BC,DE,HL → BC',DE',HL'
    ; Now in main set: B=X, C=Y, D=pattern, E=color
    ; Save pattern and color to alternate set
    push bc                       ; BC' = X,Y
    push de                       ; DE' = pattern,color
    exx                           ; Switch back
    ex af, af'                    ; Restore sprite number to A

    ; Now safe to use all registers, pattern/color saved in alternate set
    ld l, a
    ld h, 0                       ; HL = sprite offset
    ...
    ; Retrieve pattern from alternate set and store
    exx                           ; Switch to alternates
    pop de                        ; DE' = pattern,color
    ld a, d                       ; A = pattern
    ...
    ; Retrieve color from alternate set and store
    exx                           ; Switch to alternates
    pop de                        ; DE' = pattern,color
    ld a, e                       ; A = color
    pop bc                        ; Restore BC'
