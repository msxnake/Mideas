...
; Estimated payload bytes: 125643
...
; BANK 08 @#0000 : sound.asm part 3/2 (82 bytes)
; BANK 08 @#0052 : scroll.asm (2353 bytes)
; BANK 08 @#0983 : animtiles.asm (4911 bytes)
; BANK 08 @#1CB2 : particles.asm (846 bytes)
; BANK 09 @#0000 : particles.asm (4117 bytes)
; BANK 09 @#1015 : statemachine.asm part 1/4 (4075 bytes)
; BANK 10 @#0000 : statemachine.asm part 2/4 (8192 bytes)
; BANK 11 @#0000 : statemachine.asm part 3/4 (8192 bytes)
; BANK 12 @#0000 : statemachine.asm part 4/4 (8192 bytes)
; BANK 13 @#0000 : statemachine.asm part 5/4 (1677 bytes)
; BANK 13 @#068D : gameflow.asm part 1/3 (6515 bytes)
; BANK 14 @#0000 : gameflow.asm part 2/3 (8192 bytes)
; BANK 15 @#0000 : gameflow.asm part 3/3 (2763 bytes)
...
; Reuses the ZX0 scratch region while PT3 playback is active.
; Suitable for this export as long as no other runtime decompression
; needs the D400-F356 range during music playback.
...
; PT3 TRACK TABLE
; ------------------------------------------------------------------
PT3_TRACK0_ADDR EQU #0100      ; Stored in linear 48K front page by external pack step
music_pt3_track_count:
    DB #01

music_pt3_track_table:
    DW PT3_TRACK0_ADDR - 99
...
;      la animaci??n avance aunque la entidad est?? quieta.
...
.acs_dir_done:
    ...
    ; D guardar?? el sprite ID para uso posterior (VRAM upload, loop flags).
    ...
    ;
    ; update_animation_component no se ejecuta hasta el pr??ximo frame.
    ...
    ; Validar que el sprite ID est?? dentro del rango conocido
    ld a, d                 ; A = sprite asset ID
    cp SM_SpriteAssetCount  ; ??fuera de rango?
    jr nc, .acs_upload_done ; s?? ??? saltar el upload (evitar acceso fuera de tabla)
    ...
    inc hl
    ld d, (hl)
    ex de, hl               ; HL = puntero al frame 0 (datos de patr??n en ROM)
    ...
    ; Leer configuraci??n HW del sprite: entity_sprite_config[entity * 2]
    ;   byte 0: base HW sprite index (slot en la OAM, 0-31)
    ;   byte 1: layer count (n??mero de capas HW del sprite, t??picamente 1-2)
    ...
    ; Calcular BC = layerCount * 32  (bytes totales de patr??n a copiar)
    ; Cada capa HW ocupa 32 bytes de patr??n (sprite 16x16 = 2 tiles * 16 bytes, comprimido como 32)
    ld a, c
    ld b, 0
    ...
    ld de, SPRPAT
    add hl, de              ; HL = SPRPAT + base * 32  (direcci??n VRAM del slot HW)
