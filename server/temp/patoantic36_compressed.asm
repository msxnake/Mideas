...
; BANK 00 @#06A1 : entities.asm (4994 bytes)
; BANK 00 @#1A23 : worlds.asm (1501 bytes)
; BANK 01 @#0000 : worlds.asm (1544 bytes)
; BANK 01 @#0608 : screens.asm part 1/2 (6648 bytes)
; BANK 02 @#0000 : screens.asm part 2/2 (7528 bytes)
; BANK 02 @#1D68 : sprites.asm (664 bytes)
; BANK 03 @#0000 : sprites.asm (6258 bytes)
; BANK 03 @#1872 : font.asm (1934 bytes)
; BANK 04 @#0000 : font.asm (1613 bytes)
; BANK 04 @#064D : hud.asm (3836 bytes)
; BANK 04 @#1549 : menus.asm (444 bytes)
; BANK 04 @#1705 : sound.asm (2299 bytes)
; BANK 05 @#0000 : sound.asm (885 bytes)
; BANK 05 @#0375 : scroll.asm (2353 bytes)
; BANK 05 @#0CA6 : animtiles.asm (3349 bytes)
; BANK 05 @#19BB : particles.asm (1605 bytes)
; BANK 06 @#0000 : particles.asm (3358 bytes)
; BANK 06 @#0D1E : statemachine.asm part 1/3 (4834 bytes)
; BANK 07 @#0000 : statemachine.asm part 2/3 (8192 bytes)
; BANK 08 @#0000 : statemachine.asm part 3/3 (8192 bytes)
; BANK 09 @#0000 : statemachine.asm part 4/3 (2703 bytes)
; BANK 09 @#0A8F : gameflow.asm part 1/3 (5489 bytes)
; BANK 10 @#0000 : gameflow.asm part 2/3 (8192 bytes)
; BANK 11 @#0000 : gameflow.asm part 3/3 (3050 bytes)
...
SPRITE_NINA_IDLE_LEFT_6_WIDTH     EQU 16
SPRITE_NINA_IDLE_LEFT_6_HEIGHT    EQU 16
SPRITE_NINA_IDLE_LEFT_6_FRAMES    EQU 1

;; ---- Sprite Frame: nina_idle_left_6_F0 ----
;; Size: 16x16
    ; ZX0 compressed sprite pattern moved to ZX0_SPRITE_FRAME_NINA_IDLE_LEFT_6_F0_DATA (32 bytes)
    ; ZX0 compressed sprite pattern moved to ZX0_SPRITE_FRAME_NINA_IDLE_LEFT_6_F0_DATA (32 bytes)
SPRITE_6_PATTERN EQU NINA_IDLE_LEFT_6_F0_LAYER1
SPRITE_6_PATTERN_BANK EQU ((SPRITE_6_PATTERN - #4000) / #2000)

; Sprite Asset 7: nina_jump_left
...
sprite_loop_flags:
    ...
    db 2 ; Sprite 1: bola
    db 2 ; Sprite 2: bossa
    db 0 ; Sprite 3: panell
    db 0 ; Sprite 4: bola_dead
    db 2 ; Sprite 5: nina_walk_left
    db 0 ; Sprite 6: nina_idle_left
    db 0 ; Sprite 7: nina_jump_left
    db 2 ; Sprite 8: anec_left
    db 2 ; Sprite 9: nina_walk_right
...
; Sprite 6: nina_idle_left frame pointers
SPRITE_6_FRAME_PTRS:
    dw NINA_IDLE_LEFT_6_F0_LAYER1
...
sprite_dir_left_table:
    db 8, 1, 2, 3, 4, 5, 6, 7, 8, 5, 6, 7

sprite_dir_right_table:
    db 0, 1, 2, 3, 4, 9, 10, 11, 0, 9, 10, 11
...
.acs_keep_only_moving:
    ...
    ; this same frame (without waiting for animation tick progression).
    push hl
    push bc
    push de

    ; Read current sprite asset index for this entity
    ld e, c
    ld d, 0
    ld hl, entity_sprite_asset_index
    add hl, de
    ld a, (hl)
    cp #FF
    jr z, .acs_upload_done
    ld b, a                    ; B = sprite asset index

    ; Get pointer to frame pointer list for this sprite asset
    ld l, b
    ld h, 0
    add hl, hl                 ; index * 2
    ld de, sprite_asset_frame_ptr_table
    ...
    ; Frame 0 pointer (already reset above)
    ld e, (hl)
    inc hl
    ld d, (hl)
    ex de, hl                  ; HL = source pattern data (frame 0)
    ...
    ld a, c
    or a
    jr z, .acs_upload_restore_source
...
SM_New_Statemachine_state_1771533526010_Transitions_Actions_0: 
    ...
    DB 4; KEY_PRESSED 
    DB 9          ; Key: space
    DW SM_New_Statemachine_state_1771966992839 
...
SM_New_Statemachine_state_1771966990568_OnEnter: 
    DB 5; CHANGE_SPRITE 
    DB 5; sprite: nina_walk 
    DB 0xFF; END

SM_New_Statemachine_state_1771966992839: 
    DB 0; ID(unused) 
    DW SM_New_Statemachine_state_1771966992839_OnEnter 
    DW 0 
    DW SM_New_Statemachine_state_1771966992839_Transitions 
SM_New_Statemachine_state_1771966992839_OnEnter: 
    DB 5; CHANGE_SPRITE 
    DB 7; sprite: nina_jump 
    DB 0xFF; END
SM_New_Statemachine_state_1771966992839_Transitions: 
    DB 1; Count
...
ZX0_SPRITE_FRAME_NINA_IDLE_LEFT_6_F0_DATA:
    ; ZX0 compressed sprite frame NINA_IDLE_LEFT_6_F0 (64 -> 48 bytes)
...
ZX0_SPRITE_FRAME_NINA_JUMP_LEFT_7_F0_DATA:
    ; ZX0 compressed sprite frame NINA_JUMP_LEFT_7_F0 (64 -> 50 bytes)
    DB #61,#03,#1F,#00,#92,#03,#16,#03,#87,#55,#00,#22,#C0,#E0,#9B,#95
    DB #2A,#01,#A8,#F1,#40,#39,#F0,#D6,#00,#02,#DF,#22,#00,#0F,#0B,#1B
    DB #1F,#0F,#EB,#29,#B5,#AA,#99,#FE,#BE,#40,#C0,#80,#F0,#ED,#28,#C2
    DB #55,#56
...
ZX0_SPRITE_FRAME_NINA_JUMP_RIGHT_11_F0_DATA:
    ; ZX0 compressed sprite frame NINA_JUMP_RIGHT_11_F0 (64 -> 51 bytes)
    DB #29,#03,#07,#A4,#A9,#54,#80,#21,#00,#03,#02,#07,#0F,#6B,#00,#40
    DB #00,#C0,#F8,#00,#92,#C0,#68,#C0,#E1,#AA,#00,#49,#BE,#02,#03,#01
    DB #F0,#3E,#14,#40,#C8,#02,#F0,#D0,#D8,#F8,#F0,#80,#00,#94,#29,#55
    DB #D5,#55,#60
...
; Frame group: NINA_IDLE_LEFT_6_F0
NINA_IDLE_LEFT_6_F0_LAYER1 EQU ZX0_SPRITE_FRAME_NINA_IDLE_LEFT_6_F0_DATA
