...
function buildResourceDescriptor(block, id, mapperWindow) {
    const resourceIdLabel = `RESOURCE_ID_${sanitizeAsmKey(block.label)}`;
    return {
        id,
        label: block.label,
        resourceIdLabel,
...
function renderCommentArtifactBlock(fileName, content) {
...
function renderMegaromGeneratedArtifactsAsCommentBlocks(packResult, mapperWindow) {
    return buildMegaromGeneratedArtifacts(packResult, mapperWindow)
        .map((artifact) => renderCommentArtifactBlock(artifact.fileName, artifact.content))
        .join('\n\n');
}
