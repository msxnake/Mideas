...
; BANK 02 @#0000 : entities.asm (167 bytes)
; BANK 02 @#00A7 : worlds.asm (1543 bytes)
; BANK 02 @#06AE : screens.asm part 1/2 (6482 bytes)
; BANK 03 @#0000 : screens.asm part 2/2 (4068 bytes)
; BANK 03 @#0FE4 : sprites.asm part 1/2 (4124 bytes)
; BANK 04 @#0000 : sprites.asm part 2/2 (6052 bytes)
; BANK 04 @#17A4 : font.asm (177 bytes)
; BANK 04 @#1855 : hud.asm (114 bytes)
; BANK 04 @#18C7 : menus.asm (219 bytes)
; BANK 04 @#19A2 : sound.asm (1630 bytes)
; BANK 05 @#0000 : sound.asm (3139 bytes)
; BANK 05 @#0C43 : scroll.asm (2389 bytes)
; BANK 05 @#1598 : animtiles.asm (2586 bytes)
; BANK 05 @#1FB2 : bosses.asm (78 bytes)
; BANK 06 @#0000 : bosses.asm (253 bytes)
; BANK 06 @#00FD : statemachine.asm part 1/2 (7939 bytes)
; BANK 07 @#0000 : statemachine.asm part 2/2 (7698 bytes)
; BANK 07 @#1E12 : gameflow.asm (494 bytes)
; BANK 08 @#0000 : gameflow.asm (5092 bytes)
...
sprite_asset_base_pattern_slot_runtime EQU #DE5A   ; Runtime base 16x16 slot per sprite asset (12 bytes)
sprite_placeholder_base_pattern_num EQU #DE66   ; Runtime placeholder pattern number (base slot * 4)
current_sprite_pattern_pack_id EQU #DE67   ; Active runtime sprite pattern pack id (#FF=none loaded)
sprite_attributes   EQU #DE68   ; Interleaved sprite attributes (32 * 4 bytes)

; ==================================================================
; SCREEN SYSTEM VARIABLES (1 screens detected)
; ==================================================================
current_screen_id   EQU #DEE8   ; Currently displayed screen ID
current_screen_engine EQU #DEE9   ; Runtime engine: 0=Player, 1=FakePlayer
screen_dirty_flag   EQU #DEEA   ; Screen needs redraw flag
screen_transition_cooldown EQU #DEEB   ; Cooldown frames after screen transition
current_world_id    EQU #DEEC   ; Current world ID (for multi-world support)
current_screen_index EQU #DEED   ; Current screen index within world
current_screen_anim_group_count EQU #DEEE   ; Animated tile groups visible in current screen
current_screen_entity_count EQU #DEEF   ; Entity instances assigned to current screen
current_screen_sprite_pattern_slots EQU #DEF0   ; Sprite pattern slots needed by current screen
current_screen_summary_flags EQU #DEF1   ; Runtime screen summary flags (music/hud/effects/anim)

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #DEF2   ; Player X position (16-bit)
player_y            EQU #DEF4   ; Player Y position (16-bit)
player_runtime_enabled EQU #DEF6   ; 1=player fast runtime bound to hero entity
player_entity_index EQU #DEF7   ; Entity index used by player fast runtime (#FF=none)
player_vx_runtime   EQU #DEF8   ; Cached player X velocity (signed 8-bit)
player_vy_runtime   EQU #DEF9   ; Cached player Y velocity (signed 8-bit)
player_dash_timer   EQU #DEFA   ; Frames remaining in current Player dash
player_dash_cooldown EQU #DEFB   ; Frames until Player can dash again
player_dash_dir     EQU #DEFC   ; Player dash direction (1=left,2=right,3=up,4=down)
player_dash_tile_x  EQU #DEFD   ; Dash front probe tile X scratch
player_dash_tile_y  EQU #DEFE   ; Dash front probe tile Y scratch
player_health       EQU #DEFF   ; Player health points
player_score        EQU #DF00   ; Player score (16-bit)
gem_count           EQU #DF02   ; Collectible tile counter (8-bit)
last_interaction_char EQU #DF03   ; Char code of last interacted tile (for SM VARIABLE_COMPARE)
last_gem_char       EQU last_interaction_char   ; Backwards-compatible alias for collectible SM checks
last_interaction_pending EQU #DF04   ; 1 when a new tile interaction is pending for State Machine logic
last_interaction_type EQU #DF05   ; Interaction type id of last interacted tile
last_interaction_value EQU #DF06   ; Interaction value byte of last interacted tile
last_interaction_target EQU #DF07   ; Interaction target id of last interacted tile
last_interaction_x  EQU #DF08   ; Tile X coordinate of last interaction
last_interaction_y  EQU #DF09   ; Tile Y coordinate of last interaction
last_interaction_entity EQU #DF0A   ; Entity index that triggered the last interaction

; Persistent collectibles list (survives screen re-entry)
MAX_COLLECTIBLES     EQU 64              ; Max persistent collectible records
collected_count      EQU #DF0B   ; Number of collected tiles recorded (8-bit)
collected_world      EQU #DF0C   ; World IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_screen     EQU #DF4C   ; Screen IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_idx_l      EQU #DF8C   ; Tile name-table index low byte (MAX_COLLECTIBLES bytes)
collected_idx_h      EQU #DFCC   ; Tile name-table index high byte (MAX_COLLECTIBLES bytes)

; Timed bonus tile respawn slots (bonus gem regeneration)
MAX_BONUS_RESPAWNS   EQU 16              ; Max timed bonus tiles waiting to respawn
bonus_respawn_world  EQU #E00C   ; World IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_screen EQU #E01C   ; Screen IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_l  EQU #E02C   ; Tile index low byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_h  EQU #E03C   ; Tile index high byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_secs   EQU #E04C   ; Remaining seconds per timed respawn slot (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_frames EQU #E05C   ; Frame countdown (60..1) per timed respawn slot (MAX_BONUS_RESPAWNS bytes)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #E06C   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #E06D   ; Temporary 16-bit storage
temp_word_2         EQU #E06F   ; Temporary 16-bit storage
temp_byte_1         EQU #E071   ; Temporary 8-bit storage
temp_byte_2         EQU #E072   ; Temporary 8-bit storage
temp_byte_3         EQU #E073   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #E093   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #E0B3   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #E0D3   ; Temporary 8-bit storage (32 bytes)

; ==================================================================
; SOUND SYSTEM VARIABLES
; ==================================================================
sfx_active          EQU #E0F3   ; 0=no SFX active, 1=playing
sfx_timer           EQU #E0F4   ; Frames remaining for current SFX
sfx_fadeout         EQU #E0F5   ; Reserved fadeout flag/state
temp_byte_7         EQU #E0F6   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #E116   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #E136   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #E156   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #E176   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #E196   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #E1B6   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #E1D6   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #E1F6   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #E216   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #E236   ; Temporary 8-bit storage (32 bytes)
temp_byte_18        EQU #E256   ; Temporary 8-bit storage (32 bytes)
temp_byte_19        EQU #E276   ; Temporary 8-bit storage (32 bytes)
temp_byte_20        EQU #E296   ; Temporary 8-bit storage (32 bytes)
temp_byte_21        EQU #E2B6   ; Temporary 8-bit storage (32 bytes)
temp_byte_22        EQU #E2D6   ; Temporary 8-bit storage (32 bytes)
temp_byte_23        EQU #E2F6   ; Temporary 8-bit storage (32 bytes)
temp_byte_24        EQU #E316   ; Temporary 8-bit storage (32 bytes)
temp_byte_25        EQU #E336   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #E356   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #E396   ; Temporary 16-bit storage (64 bytes)
temp_byte_26        EQU #E3D6   ; Temporary 8-bit storage (32 bytes)
temp_byte_27        EQU #E3F6   ; Temporary 8-bit storage (32 bytes)
temp_byte_28        EQU #E416   ; Temporary 8-bit storage (32 bytes)
temp_byte_29        EQU #E436   ; Temporary 8-bit storage (32 bytes)
temp_byte_30        EQU #E456   ; Temporary 8-bit storage (32 bytes)
temp_byte_31        EQU #E476   ; Temporary 8-bit storage (32 bytes)
tileDead_dbg        EQU #E496   ; Debug byte: current hero deadly contact
tileDead_latched_dbg EQU #E497   ; Debug byte: latched hero deadly contact
tileDead_x_dbg      EQU #E498   ; Debug byte: last sampled deadly tile X
tileDead_y_dbg      EQU #E499   ; Debug byte: last sampled deadly tile Y
tileDead_value_dbg  EQU #E49A   ; Debug byte: last raw deadly behavior value

; Wall collision temporary variables
wall_temp_x         EQU #E49B   ; Cached entity X for wall checks
wall_temp_y         EQU #E49C   ; Cached entity Y for wall checks
wall_hit_left       EQU #E49D   ; Hitbox left edge cache
wall_hit_top        EQU #E49E   ; Hitbox top edge cache
wall_hit_right      EQU #E49F   ; Hitbox right edge cache
wall_hit_bottom     EQU #E4A0   ; Hitbox bottom edge cache
wall_hit_w          EQU #E4A1   ; Hitbox width cache (min 1)
wall_hit_h          EQU #E4A2   ; Hitbox height cache (min 1)
wall_probe_left     EQU #E4A3   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #E4A4   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #E4A5   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #E4A6   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #E4A7   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #E4C7   ; Number of entries in active_entity_list
hero_entity_id      EQU #E4C8   ; First current-screen entity flagged as player (#FF = none)
active_entity_list_dirty EQU #E4C9   ; 1=rebuild active_entity_list required
input_entity_list   EQU #E4CA   ; Active current-screen entities with Input component (MAX_ENTITIES bytes)
input_entity_count  EQU #E4EA   ; Number of entries in input_entity_list
render_entity_list  EQU #E4EB   ; Active current-screen entities with Sprite component (MAX_ENTITIES bytes)
render_entity_count EQU #E50B   ; Number of entries in render_entity_list
collision_entity_list EQU #E50C   ; Active current-screen entities with Collision component (MAX_ENTITIES bytes)
collision_entity_count EQU #E52C   ; Number of entries in collision_entity_list
ground_entity_list  EQU #E52D   ; Active current-screen entities with Collision or Gravity (MAX_ENTITIES bytes)
ground_entity_count EQU #E54D   ; Number of entries in ground_entity_list
anim_entity_list    EQU #E54E   ; Active current-screen entities with Animation+Sprite (MAX_ENTITIES bytes)
anim_entity_count   EQU #E56E   ; Number of entries in anim_entity_list

; Entity-entity collision optimized variables
coll_list           EQU #E56F   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #E58F   ; Number of entities in coll_list
coll_src_left       EQU #E590   ; Source AABB left edge (scratch)
coll_src_right      EQU #E591   ; Source AABB right edge (scratch)
coll_src_top        EQU #E592   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #E593   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #E594   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #E594   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #E596   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #E598   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #E59A   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #E59C   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #E59E   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #E5A0   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #E5A2   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #E5A4   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #E5A5   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #E5AA   ; Frame counter (16-bit)
task_exec_time          EQU #E5AC   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #E5AE   ; Set to 1 on each VBlank (1 byte)
interrupt_in_progress   EQU #E5AF   ; 1 while the H.TIMI dispatcher is running
RAM_INTERRUPT_END       EQU #E5B0   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #E5B0   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #E5B1   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #E5B2   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #E5B3   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #E5B4   ; 0=stopped, 1=track active
music_muted          EQU #E5B5   ; 0=audible, 1=muted/pause
music_loop           EQU #E5B6   ; 0=no loop, 1=loop enabled
music_track_index    EQU #E5B7   ; Current ROM track index
music_row_frames     EQU #E5B8   ; Frames per tracker row
music_row_countdown  EQU #E5B9   ; Countdown to next row
music_order_pos      EQU #E5BA   ; Current order position
music_pattern_index  EQU #E5BB   ; Current pattern index
music_pattern_row    EQU #E5BC   ; Current row inside pattern
music_pattern_rows   EQU #E5BD   ; Cached rows in current pattern
music_track_ptr_l    EQU #E5BE   ; Current track pointer low byte
music_track_ptr_h    EQU #E5BF   ; Current track pointer high byte
music_pattern_ptr_l  EQU #E5C0   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #E5C1   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #E5C2   ; PSG mixer shadow for music runtime
music_pitch_note_work EQU #E5C3   ; Scratch note index while resolving tone/ornament macros
music_pitch_step_work EQU #E5C4   ; Scratch macro step while resolving tone/ornament macros
music_pitch_len_work  EQU #E5C5   ; Scratch macro length while resolving tone/ornament macros
music_ch_note_base EQU #E5C6   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #E5C6   ; Channel A
music_ch_b_note EQU #E5C7   ; Channel B
music_ch_c_note EQU #E5C8   ; Channel C
music_ch_instrument_base EQU #E5C9   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #E5C9   ; Channel A
music_ch_b_instrument EQU #E5CA   ; Channel B
music_ch_c_instrument EQU #E5CB   ; Channel C
music_ch_ornament_base EQU #E5CC   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #E5CC   ; Channel A
music_ch_b_ornament EQU #E5CD   ; Channel B
music_ch_c_ornament EQU #E5CE   ; Channel C
music_ch_volume_base EQU #E5CF   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #E5CF   ; Channel A
music_ch_b_volume EQU #E5D0   ; Channel B
music_ch_c_volume EQU #E5D1   ; Channel C
music_ch_vol_step_base EQU #E5D2   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #E5D2   ; Channel A
music_ch_b_vol_step EQU #E5D3   ; Channel B
music_ch_c_vol_step EQU #E5D4   ; Channel C
music_ch_tone_step_base EQU #E5D5   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #E5D5   ; Channel A
music_ch_b_tone_step EQU #E5D6   ; Channel B
music_ch_c_tone_step EQU #E5D7   ; Channel C
music_ch_noise_step_base EQU #E5D8   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #E5D8   ; Channel A
music_ch_b_noise_step EQU #E5D9   ; Channel B
music_ch_c_noise_step EQU #E5DA   ; Channel C
music_ch_orn_step_base EQU #E5DB   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #E5DB   ; Channel A
music_ch_b_orn_step EQU #E5DC   ; Channel B
music_ch_c_orn_step EQU #E5DD   ; Channel C
music_ch_hw_env_step_base EQU #E5DE   ; Software hardware-envelope divider step (3 bytes)
music_ch_a_hw_env_step EQU #E5DE   ; Channel A
music_ch_b_hw_env_step EQU #E5DF   ; Channel B
music_ch_c_hw_env_step EQU #E5E0   ; Channel C
...
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #E5E1   ; End of project variables (9697 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#E5E1: Project variables (9697 bytes)
;   #E5E1-#E600: Alignment padding/free RAM (31 bytes)
