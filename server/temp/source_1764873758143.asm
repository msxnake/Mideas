...
init_rom:
    ...
    ; DEBUG: Force sprite 0 to be visible at position (100, 100)
...
force_update_entity_sprite:
    ; Input: C = Entity Index
    ; Writes sprite attributes to RAM buffer for all layers
    push af
    push bc
    push de
    push hl
    
    ; E = Entity Index (preserved throughout)
    ld e, c
    ld d, 0
    ...
    ; Now: B=X, C=Y, E=EntityIndex, D=0
    
    ; Get sprite config
    ld hl, entity_sprite_config
    add hl, de
    add hl, de                     ; HL = config + entity*2
    ld a, (hl)                     ; A = Base HW sprite index
    inc hl
    ld h, (hl)                     ; H = Layer count
    ld l, a                        ; L = Current HW sprite
    
    ; Save X,Y for the loop
    push bc                        ; Stack: [X,Y]
    
.layer_loop:
    pop bc                         ; B=X, C=Y
    push bc                        ; Save again for next iteration
    
    push hl                        ; Save L=HW sprite, H=layer count
    
    ; Calculate sprite_attributes offset: HW_sprite * 4
    ld a, l
    sla a
    sla a                          ; A = HW_sprite * 4
    
    ; HL = sprite_attributes + offset
    ld h, 0
    ld l, a
    ld de, sprite_attributes
    add hl, de                     ; HL -> attribute entry
    ...
    ; Get HW sprite back for pattern calculation
    pop de                         ; D = layer count, E = HW sprite
    push de                        ; Save again
    
    ; Write Pattern = HW_sprite * 4
    ld a, e
    sla a
    sla a
    ld (hl), a                     ; Pattern
    ...
    ; Write Color from sprite_layer_colors[HW_sprite]
    push hl                        ; Save attr pointer
    ld hl, sprite_layer_colors
    ld d, 0
    add hl, de                     ; HL = &sprite_layer_colors[HW_sprite]
    ld a, (hl)                     ; A = Color
    pop hl
    ld (hl), a                     ; Color
    
    ; Next layer
    pop hl                         ; H=layer count, L=HW sprite
    inc l                          ; Next HW sprite
    dec h                          ; Decrement layer count
    jr nz, .layer_loop
    
    pop bc                         ; Pop saved X,Y
