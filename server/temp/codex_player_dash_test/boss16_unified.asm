...
player_dash_timer   EQU #DEE3   ; Frames remaining in current Player dash
player_dash_cooldown EQU #DEE4   ; Frames until Player can dash again
player_dash_dir     EQU #DEE5   ; Player dash direction (1=left,2=right,3=up,4=down)
player_dash_tile_x  EQU #DEE6   ; Dash front probe tile X scratch
player_dash_tile_y  EQU #DEE7   ; Dash front probe tile Y scratch
player_health       EQU #DEE8   ; Player health points
player_score        EQU #DEE9   ; Player score (16-bit)
gem_count           EQU #DEEB   ; Collectible tile counter (8-bit)
last_interaction_char EQU #DEEC   ; Char code of last interacted tile (for SM VARIABLE_COMPARE)
last_gem_char       EQU last_interaction_char   ; Backwards-compatible alias for collectible SM checks
last_interaction_pending EQU #DEED   ; 1 when a new tile interaction is pending for State Machine logic
last_interaction_type EQU #DEEE   ; Interaction type id of last interacted tile
last_interaction_value EQU #DEEF   ; Interaction value byte of last interacted tile
last_interaction_target EQU #DEF0   ; Interaction target id of last interacted tile
last_interaction_x  EQU #DEF1   ; Tile X coordinate of last interaction
last_interaction_y  EQU #DEF2   ; Tile Y coordinate of last interaction
last_interaction_entity EQU #DEF3   ; Entity index that triggered the last interaction

; Persistent collectibles list (survives screen re-entry)
MAX_COLLECTIBLES     EQU 64              ; Max persistent collectible records
collected_count      EQU #DEF4   ; Number of collected tiles recorded (8-bit)
collected_world      EQU #DEF5   ; World IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_screen     EQU #DF35   ; Screen IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_idx_l      EQU #DF75   ; Tile name-table index low byte (MAX_COLLECTIBLES bytes)
collected_idx_h      EQU #DFB5   ; Tile name-table index high byte (MAX_COLLECTIBLES bytes)

; Timed bonus tile respawn slots (bonus gem regeneration)
MAX_BONUS_RESPAWNS   EQU 16              ; Max timed bonus tiles waiting to respawn
bonus_respawn_world  EQU #DFF5   ; World IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_screen EQU #E005   ; Screen IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_l  EQU #E015   ; Tile index low byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_h  EQU #E025   ; Tile index high byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_secs   EQU #E035   ; Remaining seconds per timed respawn slot (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_frames EQU #E045   ; Frame countdown (60..1) per timed respawn slot (MAX_BONUS_RESPAWNS bytes)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #E055   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #E056   ; Temporary 16-bit storage
temp_word_2         EQU #E058   ; Temporary 16-bit storage
temp_byte_1         EQU #E05A   ; Temporary 8-bit storage
temp_byte_2         EQU #E05B   ; Temporary 8-bit storage
temp_byte_3         EQU #E05C   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #E07C   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #E09C   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #E0BC   ; Temporary 8-bit storage (32 bytes)

; ==================================================================
; SOUND SYSTEM VARIABLES
; ==================================================================
sfx_active          EQU #E0DC   ; 0=no SFX active, 1=playing
sfx_timer           EQU #E0DD   ; Frames remaining for current SFX
sfx_fadeout         EQU #E0DE   ; Reserved fadeout flag/state
temp_byte_7         EQU #E0DF   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #E0FF   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #E11F   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #E13F   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #E15F   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #E17F   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #E19F   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #E1BF   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #E1DF   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #E1FF   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #E21F   ; Temporary 8-bit storage (32 bytes)
temp_byte_18        EQU #E23F   ; Temporary 8-bit storage (32 bytes)
temp_byte_19        EQU #E25F   ; Temporary 8-bit storage (32 bytes)
temp_byte_20        EQU #E27F   ; Temporary 8-bit storage (32 bytes)
temp_byte_21        EQU #E29F   ; Temporary 8-bit storage (32 bytes)
temp_byte_22        EQU #E2BF   ; Temporary 8-bit storage (32 bytes)
temp_byte_23        EQU #E2DF   ; Temporary 8-bit storage (32 bytes)
temp_byte_24        EQU #E2FF   ; Temporary 8-bit storage (32 bytes)
temp_byte_25        EQU #E31F   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #E33F   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #E37F   ; Temporary 16-bit storage (64 bytes)
temp_byte_26        EQU #E3BF   ; Temporary 8-bit storage (32 bytes)
temp_byte_27        EQU #E3DF   ; Temporary 8-bit storage (32 bytes)
temp_byte_28        EQU #E3FF   ; Temporary 8-bit storage (32 bytes)
tileDead_dbg        EQU #E41F   ; Debug byte: current hero deadly contact
tileDead_latched_dbg EQU #E420   ; Debug byte: latched hero deadly contact
tileDead_x_dbg      EQU #E421   ; Debug byte: last sampled deadly tile X
tileDead_y_dbg      EQU #E422   ; Debug byte: last sampled deadly tile Y
tileDead_value_dbg  EQU #E423   ; Debug byte: last raw deadly behavior value

; Wall collision temporary variables
wall_temp_x         EQU #E424   ; Cached entity X for wall checks
wall_temp_y         EQU #E425   ; Cached entity Y for wall checks
wall_hit_left       EQU #E426   ; Hitbox left edge cache
wall_hit_top        EQU #E427   ; Hitbox top edge cache
wall_hit_right      EQU #E428   ; Hitbox right edge cache
wall_hit_bottom     EQU #E429   ; Hitbox bottom edge cache
wall_hit_w          EQU #E42A   ; Hitbox width cache (min 1)
wall_hit_h          EQU #E42B   ; Hitbox height cache (min 1)
wall_probe_left     EQU #E42C   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #E42D   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #E42E   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #E42F   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #E430   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #E450   ; Number of entries in active_entity_list
hero_entity_id      EQU #E451   ; First current-screen entity flagged as player (#FF = none)
active_entity_list_dirty EQU #E452   ; 1=rebuild active_entity_list required
input_entity_list   EQU #E453   ; Active current-screen entities with Input component (MAX_ENTITIES bytes)
input_entity_count  EQU #E473   ; Number of entries in input_entity_list
render_entity_list  EQU #E474   ; Active current-screen entities with Sprite component (MAX_ENTITIES bytes)
render_entity_count EQU #E494   ; Number of entries in render_entity_list
collision_entity_list EQU #E495   ; Active current-screen entities with Collision component (MAX_ENTITIES bytes)
collision_entity_count EQU #E4B5   ; Number of entries in collision_entity_list
ground_entity_list  EQU #E4B6   ; Active current-screen entities with Collision or Gravity (MAX_ENTITIES bytes)
ground_entity_count EQU #E4D6   ; Number of entries in ground_entity_list
anim_entity_list    EQU #E4D7   ; Active current-screen entities with Animation+Sprite (MAX_ENTITIES bytes)
anim_entity_count   EQU #E4F7   ; Number of entries in anim_entity_list

; Entity-entity collision optimized variables
coll_list           EQU #E4F8   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #E518   ; Number of entities in coll_list
coll_src_left       EQU #E519   ; Source AABB left edge (scratch)
coll_src_right      EQU #E51A   ; Source AABB right edge (scratch)
coll_src_top        EQU #E51B   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #E51C   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #E51D   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #E51D   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #E51F   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #E521   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #E523   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #E525   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #E527   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #E529   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #E52B   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #E52D   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #E52E   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #E533   ; Frame counter (16-bit)
task_exec_time          EQU #E535   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #E537   ; Set to 1 on each VBlank (1 byte)
interrupt_in_progress   EQU #E538   ; 1 while the H.TIMI dispatcher is running
RAM_INTERRUPT_END       EQU #E539   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #E539   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #E53A   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #E53B   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #E53C   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #E53D   ; 0=stopped, 1=track active
music_muted          EQU #E53E   ; 0=audible, 1=muted/pause
music_loop           EQU #E53F   ; 0=no loop, 1=loop enabled
music_track_index    EQU #E540   ; Current ROM track index
music_row_frames     EQU #E541   ; Frames per tracker row
music_row_countdown  EQU #E542   ; Countdown to next row
music_order_pos      EQU #E543   ; Current order position
music_pattern_index  EQU #E544   ; Current pattern index
music_pattern_row    EQU #E545   ; Current row inside pattern
music_pattern_rows   EQU #E546   ; Cached rows in current pattern
music_track_ptr_l    EQU #E547   ; Current track pointer low byte
music_track_ptr_h    EQU #E548   ; Current track pointer high byte
music_pattern_ptr_l  EQU #E549   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #E54A   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #E54B   ; PSG mixer shadow for music runtime
music_pitch_note_work EQU #E54C   ; Scratch note index while resolving tone/ornament macros
music_pitch_step_work EQU #E54D   ; Scratch macro step while resolving tone/ornament macros
music_pitch_len_work  EQU #E54E   ; Scratch macro length while resolving tone/ornament macros
music_ch_note_base EQU #E54F   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #E54F   ; Channel A
music_ch_b_note EQU #E550   ; Channel B
music_ch_c_note EQU #E551   ; Channel C
music_ch_instrument_base EQU #E552   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #E552   ; Channel A
music_ch_b_instrument EQU #E553   ; Channel B
music_ch_c_instrument EQU #E554   ; Channel C
music_ch_ornament_base EQU #E555   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #E555   ; Channel A
music_ch_b_ornament EQU #E556   ; Channel B
music_ch_c_ornament EQU #E557   ; Channel C
music_ch_volume_base EQU #E558   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #E558   ; Channel A
music_ch_b_volume EQU #E559   ; Channel B
music_ch_c_volume EQU #E55A   ; Channel C
music_ch_vol_step_base EQU #E55B   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #E55B   ; Channel A
music_ch_b_vol_step EQU #E55C   ; Channel B
music_ch_c_vol_step EQU #E55D   ; Channel C
music_ch_tone_step_base EQU #E55E   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #E55E   ; Channel A
music_ch_b_tone_step EQU #E55F   ; Channel B
music_ch_c_tone_step EQU #E560   ; Channel C
music_ch_noise_step_base EQU #E561   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #E561   ; Channel A
music_ch_b_noise_step EQU #E562   ; Channel B
music_ch_c_noise_step EQU #E563   ; Channel C
music_ch_orn_step_base EQU #E564   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #E564   ; Channel A
music_ch_b_orn_step EQU #E565   ; Channel B
music_ch_c_orn_step EQU #E566   ; Channel C
music_ch_hw_env_step_base EQU #E567   ; Software hardware-envelope divider step (3 bytes)
music_ch_a_hw_env_step EQU #E567   ; Channel A
music_ch_b_hw_env_step EQU #E568   ; Channel B
music_ch_c_hw_env_step EQU #E569   ; Channel C
...
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #E56A   ; End of project variables (9578 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#E56A: Project variables (9578 bytes)
;   #E56A-#E600: Alignment padding/free RAM (150 bytes)
...
; ==================================================================
tile_pattern_bank0:
    ; Tile 0: New Boss_part_4_5 (8x8px = 1x1 chars = 1 MSX characters)
    db #87, #00, #80, #E0, #F0, #FF, #FF, #FF
    ; Tile 1: New Boss_part_4_4 (8x8px = 1x1 chars = 1 MSX characters)
    db #7E, #FF, #FF, #FF, #FF, #FF, #FF, #7E
    ; Tile 2: New Boss_part_5_6 (8x8px = 1x1 chars = 1 MSX characters)
    db #3F, #3F, #2F, #37, #77, #7B, #7F, #3F
    ; Tile 3: New Boss_part_4_4 (8x8px = 1x1 chars = 1 MSX characters)
    db #FF, #BF, #FC, #F8, #F8, #FC, #BF, #FE
    ; Tile 4: New Boss_part_2_2 (8x8px = 1x1 chars = 1 MSX characters)
    db #00, #00, #1C, #7E, #FF, #FF, #FF, #FF
    ; Tile 5: New Boss_part_3_5 (8x8px = 1x1 chars = 1 MSX characters)
    db #FF, #F3, #E1, #D1, #D9, #E1, #F3, #FF
    ; Tile 6: New Boss_part_5_4 (8x8px = 1x1 chars = 1 MSX characters)
    db #FC, #FE, #FE, #BE, #FE, #7E, #DE, #FC
    ; Tile 7: New Boss_part_4_2 (8x8px = 1x1 chars = 1 MSX characters)
    db #F0, #F8, #FC, #7F, #7F, #3F, #1F, #07
    ; Tile 8: New Boss_part_4_2_mirror_h (8x8px = 1x1 chars = 1 MSX characters)
    db #0F, #1F, #7F, #FE, #FE, #FC, #F8, #E0
    ; Tile 9: New Boss_part_4_5_mirror_h (8x8px = 1x1 chars = 1 MSX characters)
    db #E1, #00, #01, #07, #0F, #FF, #FF, #FF
    ; Tile 10: New Boss_part_4_4_mirror_h (8x8px = 1x1 chars = 1 MSX characters)
    db #FF, #DD, #FF, #FF, #FF, #EF, #FD, #7F
    ; Tile 11: New Boss_part_4_4_mirror_h (8x8px = 1x1 chars = 1 MSX characters)
    db #FF, #FD, #3F, #1F, #1F, #3F, #FD, #7F
    ; Tile 12: New Boss_part_1_3 (8x8px = 1x1 chars = 1 MSX characters)
    db #FF, #E7, #C3, #E7, #E7, #E7, #E7, #E7
    ; Tile 13: rajola1 (16x16px = 2x2 chars = 4 MSX characters)
    ; Character layout: 2x2 grid
    ; Row 0: Char0 Char1 
    ; Row 1: Char2 Char3 
    db #81, #03, #06, #0F, #1E, #3F, #7E, #D4, #81, #C0, #60, #F0, #78, #FC, #7E, #2B, #D4, #7E, #3F, #1E, #0F, #06, #03, #81, #2B, #7E, #FC, #78, #F0, #60, #C0, #81
...
; ==================================================================
tile_color_bank0:
    ; Tile 0: New Boss_part_4_5 colors (fg/bg pairs)
    db #81, #81, #81, #81, #81, #81, #91, #91
    ; Tile 1: New Boss_part_4_4 colors (fg/bg pairs)
    db #E1, #E1, #E1, #E1, #F1, #F1, #F1, #F1
    ; Tile 2: New Boss_part_5_6 colors (fg/bg pairs)
    db #A1, #A1, #A1, #A1, #A1, #A1, #A1, #A1
    ; Tile 3: New Boss_part_4_4 colors (fg/bg pairs)
    db #E1, #E1, #E1, #E1, #E1, #E1, #E1, #E1
    ; Tile 4: New Boss_part_2_2 colors (fg/bg pairs)
    db #F1, #F1, #F1, #F1, #F1, #F1, #F1, #F1
    ; Tile 5: New Boss_part_3_5 colors (fg/bg pairs)
    db #F1, #F1, #F1, #F1, #F1, #F1, #F1, #F1
    ; Tile 6: New Boss_part_5_4 colors (fg/bg pairs)
    db #A1, #A1, #A1, #A1, #A1, #A1, #A1, #A1
    ; Tile 7: New Boss_part_4_2 colors (fg/bg pairs)
    db #F8, #F8, #F8, #F1, #F1, #F1, #F1, #F1
    ; Tile 8: New Boss_part_4_2_mirror_h colors (fg/bg pairs)
    db #F8, #F8, #F8, #F1, #F1, #F1, #F1, #F1
    ; Tile 9: New Boss_part_4_5_mirror_h colors (fg/bg pairs)
    db #81, #81, #81, #81, #81, #81, #91, #91
    ; Tile 10: New Boss_part_4_4_mirror_h colors (fg/bg pairs)
    db #E1, #E1, #E1, #E1, #E1, #E1, #E1, #E1
    ; Tile 11: New Boss_part_4_4_mirror_h colors (fg/bg pairs)
    db #F1, #F1, #F1, #F1, #F1, #F1, #F1, #F1
    ; Tile 12: New Boss_part_1_3 colors (fg/bg pairs)
    db #16, #16, #16, #16, #16, #16, #16, #16
    ; Tile 13: rajola1 colors (fg/bg pairs)
    db #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E, #6E
...
SCREEN_NEW_PLAYABLE_SCREEN_0_LAYOUT:
    ...
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF,#FF
    DB #0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E
    DB #0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E
    DB #0F,#10,#0F,#10,#0F,#10,#0F,#10,#0F,#10,#0F,#10,#0F,#10,#0F,#10
    DB #0F,#10,#0F,#10,#0F,#10,#0F,#10,#0F,#10,#0F,#10,#0F,#10,#0F,#10
    DB #0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D
    DB #0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D,#0E,#0D
