...
; Estimated payload bytes: 36953
; Estimated banks used: 5
; ------------------------------------------------------------------
; BANK 00 @#0000 : patterns.asm (836 bytes)
; BANK 00 @#0344 : colors.asm (785 bytes)
; BANK 00 @#0655 : components.asm (22 bytes)
; BANK 00 @#066B : entities.asm (303 bytes)
; BANK 00 @#079A : worlds.asm (1186 bytes)
; BANK 00 @#0C3C : screens.asm (5060 bytes)
; BANK 01 @#0000 : screens.asm (718 bytes)
; BANK 01 @#02CE : sprites.asm (1835 bytes)
; BANK 01 @#09F9 : font.asm (174 bytes)
; BANK 01 @#0AA7 : hud.asm (79 bytes)
; BANK 01 @#0AF6 : menus.asm (168 bytes)
; BANK 01 @#0B9E : sound.asm part 1/2 (5218 bytes)
; BANK 02 @#0000 : sound.asm part 2/2 (8145 bytes)
; BANK 02 @#1FD1 : scroll.asm (47 bytes)
; BANK 03 @#0000 : scroll.asm (2306 bytes)
; BANK 03 @#0902 : animtiles.asm (4605 bytes)
; BANK 03 @#1AFF : statemachine.asm (5 bytes)
; BANK 03 @#1B04 : gameflow.asm (1276 bytes)
; BANK 04 @#0000 : gameflow.asm (4185 bytes)
...
; Flow: Start → Music (gfn_1772519015895)
; ==================================================================
...
runtime_background_layout EQU #C03D   ; Immutable copy of current background layout (32x24)
runtime_screen_layout  EQU #C33D   ; Mutable copy of current screen layout (32x24)
runtime_behavior_map   EQU #C63D   ; Mutable copy of current behavior map (32x24)
runtime_effects_layout EQU #C93D   ; Alternate effects layout copy for secret zones (32x24)
runtime_effect_zone_table EQU #CC3D   ; Current screen effect zone table (MAX_RUNTIME_EFFECT_ZONES * 8 bytes)
current_effect_zone_count EQU #CE3D   ; Number of effect zones copied into runtime_effect_zone_table
secret_zone_active EQU #CE3E   ; 1 if hero is currently inside an active secret zone
secret_zone_rect_x EQU #CE3F   ; Active secret zone rect X in cells
secret_zone_rect_y EQU #CE40   ; Active secret zone rect Y in cells
secret_zone_rect_w EQU #CE41   ; Active secret zone rect width in cells
secret_zone_rect_h EQU #CE42   ; Active secret zone rect height in cells

; ==================================================================
; VIEWPORT/CAMERA VARIABLES (for scroll system)
; ==================================================================
camera_x            EQU #CE43   ; Camera X position in pixels (16-bit)
camera_y            EQU #CE45   ; Camera Y position in pixels (16-bit)
camera_tile_x       EQU #CE47   ; Camera tile X (column)
camera_tile_y       EQU #CE48   ; Camera tile Y (row)
world_width_tiles   EQU #CE49   ; World width in tiles
world_height_tiles  EQU #CE4A   ; World height in tiles
scroll_dirty_flag   EQU #CE4B   ; 1=viewport changed, needs redraw
hud_dirty_flag      EQU #CE4C   ; 1=HUD needs redraw, 0=clean

; ==================================================================
; ANIMATED TILES VARIABLES
; ==================================================================
anim_tile_timer     EQU #CE4D   ; Animation frame timer
anim_tile_frame     EQU #CE4E   ; Current animation frame (0-3)
anim_tile_speed     EQU #CE4F   ; Frames between animation updates
anim_tile_transform_flags EQU #CE50   ; Runtime flags for transform-mode tile animation (byte0=flags, byte1=opcode scratch)
anim_tile_row_buffer EQU #CE52   ; Temp buffer (8 bytes) for row transforms

; ==================================================================
; ENTITY SYSTEM VARIABLES (Fixed 32 entities)
; ==================================================================
MAX_ENTITIES        EQU 32
entity_active       EQU #CE5A   ; Entity active flags (32 bytes, 0=inactive, 1=active)
entity_is_player    EQU #CE7A   ; Entity hero/player flag (32 bytes, 0=no, 1=yes)
entity_x_pos        EQU #CE9A   ; Entity X positions (32 bytes)
entity_y_pos        EQU #CEBA   ; Entity Y positions (32 bytes)
entity_vel_x        EQU #CEDA   ; Entity X velocity (32 bytes)
entity_vel_y        EQU #CEFA   ; Entity Y velocity (32 bytes)
entity_comp_masks   EQU #CF1A   ; Entity component masks (32 bytes)
entity_comp_masks_hi EQU #CF3A   ; Entity component masks high byte (32 bytes)
entity_screen_id    EQU #CF5A   ; Entity screen ID (32 bytes)
entity_job_period   EQU #CF7A   ; Entity job period in frames (32 bytes, 1=100%,2=50%,3=33%,4=25%)
entity_job_entry    EQU #CF9A   ; Entity job entry slot within period window (32 bytes)
entity_job_scheduler_active EQU #CFBA   ; 1 when any entity uses non-default job cadence
entity_dir_mask     EQU #CFBB   ; Entity direction mask (32 bytes)
entity_input_speed  EQU #CFDB   ; Entity input/cursor speed (32 bytes)
entity_health       EQU #CFFB   ; Entity health (32 bytes)
entity_anim_frame   EQU #D01B   ; Entity animation frame (32 bytes)
entity_anim_tick    EQU #D03B   ; Entity animation tick counter (32 bytes)
entity_anim_speed   EQU #D05B   ; Entity animation speed (ticks per frame) (32 bytes)
entity_anim_flags   EQU #D07B   ; Entity animation flags (32 bytes)
entity_sm_ptr_l     EQU #D09B   ; Entity State Pointer Low (32 bytes)
entity_sm_ptr_h     EQU #D0BB   ; Entity State Pointer High (32 bytes)
entity_sm_timer_l   EQU #D0DB   ; Entity State Timer Low (32 bytes)
entity_sm_timer_h   EQU #D0FB   ; Entity State Timer High (32 bytes)
entity_sm_wait_timer EQU #D11B   ; Entity State Wait Timer (32 bytes)
entity_lifetime     EQU #D13B   ; Entity lifetime for auto-destroy (32 bytes, 0=infinite)
entity_carried_by   EQU #D15B   ; Entity carrier ID (32 bytes, 255=not carried)
entity_template_token EQU #D17B   ; Entity template token (32 bytes, 0=unknown)
entity_facing_dir   EQU #D19B   ; Last facing direction (32 bytes, 0=none,1=left,2=right,3=up,4=down)
entity_sm_var_0     EQU #D1BB   ; Entity Variable 0 (32 bytes)
entity_sm_var_1     EQU #D1DB   ; Entity Variable 1 (32 bytes)
entity_sm_var_2     EQU #D1FB   ; Entity Variable 2 (32 bytes)
entity_sm_var_3     EQU #D21B   ; Entity Variable 3 (32 bytes)
entity_sm_var_4     EQU #D23B   ; Entity Variable 4 (32 bytes)
entity_sm_var_5     EQU #D25B   ; Entity Variable 5 (32 bytes)
entity_sm_var_6     EQU #D27B   ; Entity Variable 6 (32 bytes)
entity_sm_var_7     EQU #D29B   ; Entity Variable 7 (32 bytes)

; ==================================================================
; SPRITE SYSTEM VARIABLES
; ==================================================================
entity_sprite_asset_index EQU #D2BB   ; Entity sprite asset index - RAM copy (32 bytes)
active_sprite_count EQU #D2DB   ; Number of sprites currently active
sprites_dirty      EQU #D2DC   ; 1=sprite_attributes changed, needs VRAM sync
sprite_pattern      EQU #D2DD   ; Sprite pattern IDs (32 bytes)
sprite_color        EQU #D2FD   ; Sprite colors (32 bytes)
sprite_layer_colors EQU #D31D   ; HW sprite layer color cache - RAM copy (32 bytes, indexed by HW sprite index)
sprite_attributes   EQU #D33D   ; Interleaved sprite attributes (32 * 4 bytes)

; ==================================================================
; SCREEN SYSTEM VARIABLES (1 screens detected)
; ==================================================================
current_screen_id   EQU #D3BD   ; Currently displayed screen ID
screen_dirty_flag   EQU #D3BE   ; Screen needs redraw flag
screen_transition_cooldown EQU #D3BF   ; Cooldown frames after screen transition
current_world_id    EQU #D3C0   ; Current world ID (for multi-world support)
current_screen_index EQU #D3C1   ; Current screen index within world

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #D3C2   ; Player X position (16-bit)
player_y            EQU #D3C4   ; Player Y position (16-bit)
player_health       EQU #D3C6   ; Player health points
player_score        EQU #D3C7   ; Player score (16-bit)
gem_count           EQU #D3C9   ; Collectible tile counter (8-bit)
last_gem_char       EQU #D3CA   ; Char code of last collected gem tile (for SM VARIABLE_COMPARE)

; Persistent collectibles list (survives screen re-entry)
MAX_COLLECTIBLES     EQU 64              ; Max persistent collectible records
collected_count      EQU #D3CB   ; Number of collected tiles recorded (8-bit)
collected_world      EQU #D3CC   ; World IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_screen     EQU #D40C   ; Screen IDs for each collected tile (MAX_COLLECTIBLES bytes)
collected_idx_l      EQU #D44C   ; Tile name-table index low byte (MAX_COLLECTIBLES bytes)
collected_idx_h      EQU #D48C   ; Tile name-table index high byte (MAX_COLLECTIBLES bytes)

; Timed bonus tile respawn slots (bonus gem regeneration)
MAX_BONUS_RESPAWNS   EQU 16              ; Max timed bonus tiles waiting to respawn
bonus_respawn_world  EQU #D4CC   ; World IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_screen EQU #D4DC   ; Screen IDs for timed bonus respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_l  EQU #D4EC   ; Tile index low byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_idx_h  EQU #D4FC   ; Tile index high byte for timed respawns (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_secs   EQU #D50C   ; Remaining seconds per timed respawn slot (MAX_BONUS_RESPAWNS bytes)
bonus_respawn_frames EQU #D51C   ; Frame countdown (60..1) per timed respawn slot (MAX_BONUS_RESPAWNS bytes)

; ==================================================================
; AUXILIARY VARIABLES 
; ==================================================================
deterministic        EQU #D52C   ; Deterministic mode flag

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #D52D   ; Temporary 16-bit storage
temp_word_2         EQU #D52F   ; Temporary 16-bit storage
temp_byte_1         EQU #D531   ; Temporary 8-bit storage
temp_byte_2         EQU #D532   ; Temporary 8-bit storage
temp_byte_3         EQU #D533   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #D553   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #D573   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #D593   ; Temporary 8-bit storage (32 bytes)

; ==================================================================
; SOUND SYSTEM VARIABLES
; ==================================================================
sfx_active          EQU #D5B3   ; 0=no SFX active, 1=playing
sfx_timer           EQU #D5B4   ; Frames remaining for current SFX
sfx_fadeout         EQU #D5B5   ; Reserved fadeout flag/state
temp_byte_7         EQU #D5B6   ; Temporary 8-bit storage (32 bytes)
temp_byte_8         EQU #D5D6   ; Temporary 8-bit storage (32 bytes)
temp_byte_9         EQU #D5F6   ; Temporary 8-bit storage (32 bytes)
temp_byte_10        EQU #D616   ; Temporary 8-bit storage (32 bytes)
temp_byte_11        EQU #D636   ; Temporary 8-bit storage (32 bytes)
temp_byte_12        EQU #D656   ; Temporary 8-bit storage (32 bytes)
temp_byte_13        EQU #D676   ; Temporary 8-bit storage (32 bytes)
temp_byte_14        EQU #D696   ; Temporary 8-bit storage (32 bytes)
temp_byte_15        EQU #D6B6   ; Temporary 8-bit storage (32 bytes)
temp_byte_16        EQU #D6D6   ; Temporary 8-bit storage (32 bytes)
temp_byte_17        EQU #D6F6   ; Temporary 8-bit storage (32 bytes)
temp_byte_18        EQU #D716   ; Temporary 8-bit storage (32 bytes)
temp_byte_19        EQU #D736   ; Temporary 8-bit storage (32 bytes)
temp_byte_20        EQU #D756   ; Temporary 8-bit storage (32 bytes)
temp_byte_21        EQU #D776   ; Temporary 8-bit storage (32 bytes)
temp_byte_22        EQU #D796   ; Temporary 8-bit storage (32 bytes)
temp_byte_23        EQU #D7B6   ; Temporary 8-bit storage (32 bytes)
temp_byte_24        EQU #D7D6   ; Temporary 8-bit storage (32 bytes)
temp_byte_25        EQU #D7F6   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #D816   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #D856   ; Temporary 16-bit storage (64 bytes)
temp_byte_26        EQU #D896   ; Temporary 8-bit storage (32 bytes)
temp_byte_27        EQU #D8B6   ; Temporary 8-bit storage (32 bytes)
tileDead_dbg        EQU #D8D6   ; Debug byte: current hero deadly contact
tileDead_latched_dbg EQU #D8D7   ; Debug byte: latched hero deadly contact
tileDead_x_dbg      EQU #D8D8   ; Debug byte: last sampled deadly tile X
tileDead_y_dbg      EQU #D8D9   ; Debug byte: last sampled deadly tile Y
tileDead_value_dbg  EQU #D8DA   ; Debug byte: last raw deadly behavior value

; Wall collision temporary variables
wall_temp_x         EQU #D8DB   ; Cached entity X for wall checks
wall_temp_y         EQU #D8DC   ; Cached entity Y for wall checks
wall_hit_left       EQU #D8DD   ; Hitbox left edge cache
wall_hit_top        EQU #D8DE   ; Hitbox top edge cache
wall_hit_right      EQU #D8DF   ; Hitbox right edge cache
wall_hit_bottom     EQU #D8E0   ; Hitbox bottom edge cache
wall_hit_w          EQU #D8E1   ; Hitbox width cache (min 1)
wall_hit_h          EQU #D8E2   ; Hitbox height cache (min 1)
wall_probe_left     EQU #D8E3   ; X probe near hitbox left (adaptive inset)
wall_probe_right    EQU #D8E4   ; X probe near hitbox right (adaptive inset)
wall_probe_top      EQU #D8E5   ; Y probe near hitbox top (adaptive inset)
wall_probe_bottom   EQU #D8E6   ; Y probe near hitbox bottom (adaptive inset)

; Unified update helpers
active_entity_list  EQU #D8E7   ; Entity indices with non-zero component masks (MAX_ENTITIES bytes)
active_entity_count EQU #D907   ; Number of entries in active_entity_list
hero_entity_id      EQU #D908   ; First current-screen entity flagged as player (#FF = none)
active_entity_list_dirty EQU #D909   ; 1=rebuild active_entity_list required
input_entity_list   EQU #D90A   ; Active current-screen entities with Input component (MAX_ENTITIES bytes)
input_entity_count  EQU #D92A   ; Number of entries in input_entity_list
render_entity_list  EQU #D92B   ; Active current-screen entities with Sprite component (MAX_ENTITIES bytes)
render_entity_count EQU #D94B   ; Number of entries in render_entity_list
collision_entity_list EQU #D94C   ; Active current-screen entities with Collision component (MAX_ENTITIES bytes)
collision_entity_count EQU #D96C   ; Number of entries in collision_entity_list
ground_entity_list  EQU #D96D   ; Active current-screen entities with Collision or Gravity (MAX_ENTITIES bytes)
ground_entity_count EQU #D98D   ; Number of entries in ground_entity_list
anim_entity_list    EQU #D98E   ; Active current-screen entities with Animation+Sprite (MAX_ENTITIES bytes)
anim_entity_count   EQU #D9AE   ; Number of entries in anim_entity_list

; Entity-entity collision optimized variables
coll_list           EQU #D9AF   ; Active collidable entity indices (MAX_ENTITIES bytes)
coll_list_count     EQU #D9CF   ; Number of entities in coll_list
coll_src_left       EQU #D9D0   ; Source AABB left edge (scratch)
coll_src_right      EQU #D9D1   ; Source AABB right edge (scratch)
coll_src_top        EQU #D9D2   ; Source AABB top edge (scratch)
coll_src_bottom     EQU #D9D3   ; Source AABB bottom edge (scratch)

; ==================================================================
; INTERRUPT SYSTEM VARIABLES (dynamically allocated)
; ==================================================================
task_table              EQU #D9D4   ; Task table base (8 slots x 2 bytes = 16 bytes)
task_0_ptr              EQU #D9D4   ; Slot 0 pointer (2 bytes)
task_1_ptr              EQU #D9D6   ; Slot 1 pointer (2 bytes)
task_2_ptr              EQU #D9D8   ; Slot 2 pointer (2 bytes)
task_3_ptr              EQU #D9DA   ; Slot 3 pointer (2 bytes)
task_4_ptr              EQU #D9DC   ; Slot 4 pointer (2 bytes)
task_5_ptr              EQU #D9DE   ; Slot 5 pointer (2 bytes)
task_6_ptr              EQU #D9E0   ; Slot 6 pointer (2 bytes)
task_7_ptr              EQU #D9E2   ; Slot 7 pointer (2 bytes)
interrupt_system_enabled EQU #D9E4   ; 0=disabled, 1=enabled (1 byte)
old_htimi_hook          EQU #D9E5   ; Original H.TIMI hook (5 bytes)
interrupt_counter       EQU #D9EA   ; Frame counter (16-bit)
task_exec_time          EQU #D9EC   ; Cycles used by tasks (16-bit, debug)
vblank_flag             EQU #D9EE   ; Set to 1 on each VBlank (1 byte)
RAM_INTERRUPT_END       EQU #D9EF   ; End of interrupt system

; ==================================================================
; STATE MACHINE SOUND RUNTIME (one active sound asset)
; ==================================================================
sm_sound_active       EQU #D9EF   ; 0=idle, 1=playing state-machine sound asset
sm_sound_frames_left  EQU #D9F0   ; Frames left for current state-machine sound asset
sm_sound_ptr_l        EQU #D9F1   ; Next sound frame pointer low byte
sm_sound_ptr_h        EQU #D9F2   ; Next sound frame pointer high byte

; ==================================================================
; TRACKER MUSIC RUNTIME
; ==================================================================
music_active         EQU #D9F3   ; 0=stopped, 1=track active
music_muted          EQU #D9F4   ; 0=audible, 1=muted/pause
music_loop           EQU #D9F5   ; 0=no loop, 1=loop enabled
music_track_index    EQU #D9F6   ; Current ROM track index
music_row_frames     EQU #D9F7   ; Frames per tracker row
music_row_countdown  EQU #D9F8   ; Countdown to next row
music_order_pos      EQU #D9F9   ; Current order position
music_pattern_index  EQU #D9FA   ; Current pattern index
music_pattern_row    EQU #D9FB   ; Current row inside pattern
music_pattern_rows   EQU #D9FC   ; Cached rows in current pattern
music_track_ptr_l    EQU #D9FD   ; Current track pointer low byte
music_track_ptr_h    EQU #D9FE   ; Current track pointer high byte
music_pattern_ptr_l  EQU #D9FF   ; Current pattern rows pointer low byte
music_pattern_ptr_h  EQU #DA00   ; Current pattern rows pointer high byte
music_mixer_shadow   EQU #DA01   ; PSG mixer shadow for music runtime
music_ch_note_base EQU #DA02   ; Current note index (255=silent) (3 bytes)
music_ch_a_note EQU #DA02   ; Channel A
music_ch_b_note EQU #DA03   ; Channel B
music_ch_c_note EQU #DA04   ; Channel C
music_ch_instrument_base EQU #DA05   ; Current instrument id (0=none) (3 bytes)
music_ch_a_instrument EQU #DA05   ; Channel A
music_ch_b_instrument EQU #DA06   ; Channel B
music_ch_c_instrument EQU #DA07   ; Channel C
music_ch_ornament_base EQU #DA08   ; Current ornament id (0=none) (3 bytes)
music_ch_a_ornament EQU #DA08   ; Channel A
music_ch_b_ornament EQU #DA09   ; Channel B
music_ch_c_ornament EQU #DA0A   ; Channel C
music_ch_volume_base EQU #DA0B   ; Current base volume (0-15) (3 bytes)
music_ch_a_volume EQU #DA0B   ; Channel A
music_ch_b_volume EQU #DA0C   ; Channel B
music_ch_c_volume EQU #DA0D   ; Channel C
music_ch_vol_step_base EQU #DA0E   ; Reserved software volume envelope step (3 bytes)
music_ch_a_vol_step EQU #DA0E   ; Channel A
music_ch_b_vol_step EQU #DA0F   ; Channel B
music_ch_c_vol_step EQU #DA10   ; Channel C
music_ch_tone_step_base EQU #DA11   ; Reserved software tone envelope step (3 bytes)
music_ch_a_tone_step EQU #DA11   ; Channel A
music_ch_b_tone_step EQU #DA12   ; Channel B
music_ch_c_tone_step EQU #DA13   ; Channel C
music_ch_noise_step_base EQU #DA14   ; Reserved software noise envelope step (3 bytes)
music_ch_a_noise_step EQU #DA14   ; Channel A
music_ch_b_noise_step EQU #DA15   ; Channel B
music_ch_c_noise_step EQU #DA16   ; Channel C
music_ch_orn_step_base EQU #DA17   ; Reserved ornament step (3 bytes)
music_ch_a_orn_step EQU #DA17   ; Channel A
music_ch_b_orn_step EQU #DA18   ; Channel B
music_ch_c_orn_step EQU #DA19   ; Channel C

; ==================================================================
; PT3 REPLAYER WORKSPACE (~448 bytes)
; Layout matches PT3-ROM-alltables-glass.asm expected labels
; ==================================================================
PT3_SETUP       EQU #DA1A   ; PT3 state flags (bit0=loop, bit7=song_ended)
PT3_MODADDR     EQU #DA1B   ; Module address pointer (2 bytes)
PT3_CrPsPtr     EQU #DA1D   ; Current position pointer
PT3_SAMPTRS     EQU #DA1F   ; Sample pointers base
PT3_OrnPtrs     EQU #DA21   ; Ornament pointers base
PT3_PDSP        EQU #DA23   ; Pattern data start pointer
PT3_CSP         EQU #DA25   ; Saved SP (CHREGS SP trick)
PT3_PSP         EQU #DA27   ; PT3 stack pointer save
PT3_PrNote      EQU #DA29   ; Previous note
PT3_PrSlide     EQU #DA2A   ; Previous slide (2 bytes)
PT3_AdInPtA     EQU #DA2C   ; Channel A inline pointer
PT3_AdInPtB     EQU #DA2E   ; Channel B inline pointer
PT3_AdInPtC     EQU #DA30   ; Channel C inline pointer
PT3_LPosPtr     EQU #DA32   ; Loop position pointer
PT3_PatsPtr     EQU #DA34   ; Patterns table pointer
PT3_Delay       EQU #DA36   ; Song speed/delay
PT3_AddToEn     EQU #DA37   ; Add to envelope
PT3_Env_Del     EQU #DA38   ; Envelope delay
PT3_ESldAdd     EQU #DA39   ; Envelope slide add (2 bytes)
PT3_NTL3        EQU #DA3B   ; Note table link 3
VARS            EQU #DA3D   ; Channel vars base
ChanA           EQU #DA3D   ; Channel A data (29 bytes)
ChanB           EQU #DA5A   ; Channel B data (29 bytes)
ChanC           EQU #DA77   ; Channel C data (29 bytes)
DelyCnt         EQU #DA94   ; Delay counter
CurESld         EQU #DA95   ; Current envelope slide (2 bytes)
CurEDel         EQU #DA97   ; Current envelope delay
Ns_Base_AddToNs EQU #DA98   ; Noise base + add to noise (combined)
Ns_Base         EQU #DA98   ; Noise base
AddToNs         EQU #DA99   ; Add to noise
NT_             EQU #DA9A   ; Note table (192 bytes)
AYREGS          EQU #DB5A  ; AY registers mirror (14 bytes)
VT_             EQU #DB5A  ; Volume table base (alias for AYREGS)
EnvBase         EQU #DB68  ; Envelope base
VAR0END         EQU #DB6A  ; End of fixed workspace
T1_             EQU #DB6A  ; Tone tables start (unpacked by PT3_INIT)
T_NEW_1         EQU #DB6A  ; Tone table new 1
T_OLD_1         EQU #DB6A  ; Tone table old 1
T_OLD_2         EQU #DB82  ; Tone table old 2
T_NEW_3         EQU #DB9A  ; Tone table new 3
T_OLD_3         EQU #DB9A  ; Tone table old 3
T_OLD_0         EQU #DB9C  ; Tone table old 0
T_NEW_0         EQU #DB9C  ; Tone table new 0
T_NEW_2         EQU #DBB4  ; Tone table new 2 (last, ends at +0x1B2)

; ==================================================================
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #DC5A   ; End of project variables (7258 bytes used)

; ==================================================================
; MEMORY LAYOUT INFO (Reference only - no code generated)
; ==================================================================
; RAM Layout:
;   #C000-#DC5A: Project variables (7258 bytes)
;   #DC5A-#F37F: Free RAM (~5926 bytes available)
...
; ==================================================================
tile_pattern_bank0:
    ; ZX0 compressed tile_pattern (32 -> 13 bytes)
    DB #A8,#FF,#C0,#6A,#FF,#87,#03,#C7,#E4,#D5,#F1,#55,#60
load_pattern_bank0:
...
; ==================================================================
tile_color_bank0:
    ; ZX0 compressed tile_color (32 -> 5 bytes)
    DB #95,#F1,#75,#55,#58
load_color_bank0:
...
SCREEN_NEW_SCREENMAP_0_LAYOUT:
    ; ZX0 compressed layout (768 -> 34 bytes)
    DB #90,#FF,#46,#38,#80,#81,#FC,#1E,#C0,#58,#E0,#82,#83,#FC,#7C,#80
    DB #7D,#5C,#7D,#80,#7D,#C0,#79,#80,#00,#4D,#75,#80,#5E,#FE,#41,#0D
    DB #55,#56
...
SCREEN_NEW_SCREENMAP_0_EFFECT_ZONE_TABLE:
    ; No effect zones exported for New Screenmap
    DB #00
...
load_screen_new_screenmap_772518805887:
    ...
    call mapper_push_p2
    ld a, SCREEN_NEW_SCREENMAP_0_LAYOUT_BANK
    call mapper_set_bank_p2
    ...
    call mapper_push_p2
    ld a, SCREEN_NEW_SCREENMAP_0_EFFECTS_LAYOUT_BANK
    call mapper_set_bank_p2
    ; Decompress ZX0 effects layout directly into runtime_effects_layout
    di
    ld hl, SCREEN_NEW_SCREENMAP_0_EFFECTS_LAYOUT
    ...
    call mapper_push_p2
    ld a, BEHAVIOR_NEW_SCREENMAP_0_DATA_BANK
    call mapper_set_bank_p2
    ...
    or a
    jr z, .load_new_screenmap_772518805887_zones_done
    call mapper_push_p2
    ld a, SCREEN_NEW_SCREENMAP_0_EFFECT_ZONE_TABLE_BANK
    call mapper_set_bank_p2
    ld hl, SCREEN_NEW_SCREENMAP_0_EFFECT_ZONE_TABLE
    ld de, runtime_effect_zone_table
    ld bc, 0
    ldir
    call mapper_pop_p2
.load_new_screenmap_772518805887_zones_done:
...
music_pt3_track_table:
    DW pt3_track_0_data - 99    ; pt3 (.99 stripped)

; --- PT3 Track 0: pt3 ---
pt3_track_0_data:
    DB #02,#05,#25,#00,#EF,#00,#00,#00,#81,#17,#D7,#17,#DD,#17,#03,#18
    DB #09,#18,#17,#18,#21,#18,#6B,#18,#75,#18,#97,#18,#C1,#18,#CF,#18
    DB #11,#19,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#33,#19,#36,#19,#3B,#19,#40,#19,#45,#19
    DB #49,#19,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#03,#06,#09,#00,#03,#06,#0C,#0F,#12
    DB #15,#18,#15,#18,#1B,#1E,#1B,#1E,#21,#24,#33,#36,#39,#3C,#3F,#42
    DB #45,#27,#2A,#2D,#30,#27,#2A,#2D,#30,#48,#4B,#FF,#8B,#01,#AE,#01
    DB #E9,#01,#F2,#01,#AE,#01,#15,#02,#1E,#02,#AE,#01,#41,#02,#4A,#02
    DB #AE,#01,#79,#02,#82,#02,#AF,#02,#EC,#02,#F8,#02,#AE,#01,#15,#03
    DB #BD,#03,#AE,#01,#E5,#03,#AE,#01,#8B,#04,#D8,#04,#AE,#01,#D5,#05
    DB #3D,#06,#AE,#01,#38,#07,#9C,#07,#AE,#01,#97,#08,#02,#09,#AE,#01
    DB #F7,#09,#25,#0A,#AE,#01,#D2,#0A,#08,#0B,#49,#14,#A4,#14,#8B,#15
    DB #95,#15,#B1,#15,#97,#16,#A1,#16,#B1,#15,#BD,#16,#C7,#16,#B1,#15
    DB #EE,#16,#CF,#0B,#0A,#0C,#7E,#0C,#CF,#0B,#7A,#0D,#DF,#0D,#CF,#0B
    DB #D9,#0E,#AE,#0F,#CF,#0B,#AB,#10,#21,#11,#AE,#01,#1D,#12,#7A,#12
    DB #AE,#01,#49,#13,#81,#13,#49,#14,#66,#14,#E9,#01,#F8,#16,#2B,#17
    DB #39,#17,#3D,#17,#7D,#17,#39,#17,#F0,#06,#C4,#B1,#08,#6C,#C5,#D0
    DB #C6,#D0,#C7,#D0,#C9,#B1,#01,#D0,#C7,#B1,#07,#D0,#C6,#B1,#08,#D0
    DB #C5,#D0,#C4,#B1,#07,#D0,#C3,#B1,#01,#D0,#00,#F1,#02,#CF,#B1,#02
    DB #7D,#7D,#B1,#04,#7D,#7D,#CE,#B1,#02,#7D,#42,#CF,#B1,#04,#7F,#7F
    DB #CE,#B1,#02,#7F,#41,#CF,#B1,#04,#7D,#7D,#43,#B1,#02,#7C,#7C,#B1
    DB #04,#7C,#7C,#CE,#B1,#02,#7C,#CF,#B1,#04,#7C,#7C,#CE,#B1,#02,#7C
    DB #CF,#B1,#04,#7C,#7C,#00,#1E,#00,#3F,#08,#40,#B1,#40,#71,#00,#F0
    DB #06,#C4,#B1,#08,#69,#C5,#D0,#C6,#D0,#C7,#D0,#C9,#B1,#01,#D0,#C7
    DB #B1,#07,#D0,#C6,#B1,#08,#D0,#C5,#D0,#C4,#B1,#07,#D0,#C3,#B1,#01
    DB #D0,#00,#1E,#00,#4B,#08,#40,#B1,#40,#6E,#00,#F0,#06,#C4,#B1,#08
    DB #71,#C5,#D0,#C6,#D0,#C7,#D0,#C9,#B1,#01,#D0,#C7,#B1,#07,#D0,#C6
    DB #B1,#08,#D0,#C5,#D0,#C4,#B1,#07,#D0,#C3,#B1,#01,#D0,#00,#1E,#00
    DB #5E,#08,#40,#B1,#40,#6A,#00,#F0,#06,#C4,#B1,#01,#71,#D9,#02,#B1
    DB #07,#73,#01,#36,#00,#FE,#FF,#C5,#B1,#08,#D0,#C6,#D0,#C7,#D0,#C8
    DB #B1,#01,#D0,#C7,#B1,#07,#D0,#C6,#B1,#08,#D0,#C5,#D0,#C4,#B1,#07
    DB #D0,#C3,#B1,#01,#D0,#00,#1E,#00,#54,#08,#40,#B1,#40,#6C,#00,#F0
    DB #06,#C4,#B1,#01,#71,#D9,#02,#B1,#07,#73,#01,#36,#00,#FE,#FF,#C5
    DB #B1,#08,#D0,#C6,#D0,#C7,#D0,#C8,#B1,#01,#D0,#C7,#B1,#07,#D0,#C6
    DB #B1,#08,#D0,#C5,#D0,#C2,#B1,#04,#D0,#C1,#D0,#00,#F1,#02,#CF,#B1
    DB #02,#7D,#7D,#B1,#04,#7D,#7D,#CE,#B1,#02,#7D,#42,#CF,#B1,#04,#7F
    DB #7F,#CE,#B1,#02,#7F,#41,#CF,#B1,#04,#7D,#7D,#43,#B1,#02,#7C,#7C
    DB #B1,#04,#7C,#7C,#CE,#B1,#02,#7C,#CF,#B1,#04,#7C,#C6,#7C,#C4,#B1
    DB #02,#7C,#C3,#B1,#04,#7C,#C2,#7C,#00,#1E,#00,#54,#08,#40,#B1,#38
    DB #6C,#B1,#08,#C0,#00,#F1,#02,#C4,#B1,#08,#7D,#B1,#06,#7D,#B1,#04
    DB #7D,#7D,#B1,#02,#7D,#42,#B1,#04,#7F,#7F,#41,#B1,#08,#7D,#7D,#7D
    DB #7D,#00,#1E,#00,#3F,#0A,#40,#CF,#B1,#02,#71,#D6,#CE,#B1,#01,#71
    DB #F1,#02,#C7,#7D,#1E,#00,#3F,#0A,#40,#CE,#B1,#02,#71,#D6,#71,#D7
    DB #B0,#CF,#76,#C8,#76,#D6,#BF,#00,#54,#CF,#6C,#CE,#6C,#D5,#BF,#00
    DB #3F,#CF,#71,#D6,#CE,#B1,#01,#71,#F2,#02,#C7,#7F,#1E,#00,#3F,#0C
    DB #40,#CF,#B1,#02,#71,#CE,#71,#D7,#B0,#CF,#76,#C8,#76,#D6,#BF,#00
    DB #54,#CF,#6C,#D5,#BF,#00,#4F,#CE,#6D,#BF,#00,#4B,#CF,#6E,#D6,#CE
    DB #B1,#01,#6E,#F3,#02,#C7,#7C,#1E,#00,#4B,#0A,#40,#CE,#B1,#02,#6E
    DB #D6,#6E,#D7,#CF,#76,#C8,#76,#D6,#BF,#00,#64,#CF,#69,#CE,#69,#D5
    DB #BF,#00,#4B,#CF,#6E,#D6,#CE,#B1,#01,#6E,#F3,#02,#C7,#7C,#1E,#00
    DB #4B,#0A,#40,#CE,#B1,#02,#6E,#D6,#6E,#D7,#CF,#76,#C8,#76,#D6,#BF
    DB #00,#70,#CF,#67,#BF,#00,#64,#CE,#69,#00,#F3,#02,#C4,#B1,#06,#7C
    DB #7C,#B1,#02,#7C,#41,#B1,#04,#7D,#7D,#B1,#02,#7D,#42,#B1,#04,#7F
    DB #7F,#41,#B1,#08,#7D,#7D,#B1,#0C,#7D,#D7,#40,#CE,#B1,#02,#76,#CA
    DB #76,#00,#1E,#00,#5E,#0A,#40,#CF,#B1,#02,#6A,#D6,#CE,#B1,#01,#6A
    DB #F1,#02,#C7,#7D,#1E,#00,#5E,#0A,#40,#CE,#B1,#02,#6A,#D6,#6A,#D7
    DB #CF,#76,#C8,#76,#D6,#BF,#00,#7E,#CF,#65,#CE,#65,#D5,#BF,#00,#5E
    DB #CF,#6A,#D6,#CE,#B1,#01,#6A,#F2,#02,#C6,#7F,#1E,#00,#5E,#0A,#40
    DB #CE,#B1,#02,#6A,#D6,#6A,#D7,#CF,#76,#C8,#76,#D6,#BF,#00,#5E,#CF
    DB #6A,#D5,#BF,#00,#59,#CE,#6B,#BF,#00,#54,#CF,#6C,#D6,#CE,#B1,#01
    DB #6C,#F3,#02,#C6,#7C,#1E,#00,#54,#0A,#40,#CE,#B1,#02,#6C,#D6,#6C
    DB #D7,#CF,#76,#C8,#76,#D6,#BF,#00,#70,#CF,#67,#CE,#67,#D5,#BF,#00
    DB #54,#CF,#6C,#D6,#CE,#B1,#01,#6C,#F3,#02,#C6,#7C,#1E,#00,#54,#0A
    DB #40,#CE,#B1,#02,#6C,#D6,#6C,#D7,#CF,#76,#C9,#76,#D8,#BF,#00,#4B
    DB #CF,#6E,#BF,#00,#43,#CE,#70,#00,#F4,#14,#CE,#B1,#02,#73,#75,#78
    DB #75,#78,#7A,#78,#7A,#7D,#7A,#7D,#7F,#7D,#B1,#01,#7F,#CD,#D0,#CE
    DB #B1,#03,#81,#CD,#B1,#01,#D0,#CE,#7F,#D2,#40,#CB,#02,#B1,#03,#81
    DB #01,#18,#00,#FF,#FF,#CA,#B1,#04,#D0,#C9,#D0,#D9,#B1,#07,#81,#C8
    DB #B1,#01,#D0,#DD,#C7,#81,#C6,#D0,#C5,#B1,#02,#D0,#C4,#B1,#06,#D0
    DB #C3,#B1,#02,#D0,#00,#1E,#00,#3F,#0A,#40,#CF,#B1,#02,#71,#D6,#CE
    DB #B1,#01,#71,#DA,#B0,#C9,#73,#D5,#BF,#00,#3F,#CE,#B1,#02,#71,#D6
    DB #B1,#01,#71,#DA,#B0,#C9,#78,#D7,#CF,#B1,#02,#76,#C8,#B1,#01,#76
    DB #DA,#C9,#78,#D6,#BF,#00,#54,#CF,#B1,#02,#6C,#CE,#B1,#01,#6C,#DA
    DB #B0,#C9,#78,#D5,#BF,#00,#3F,#CF,#B1,#02,#71,#D6,#CE,#B1,#01,#71
    DB #DA,#B0,#C9,#7D,#D6,#BF,#00,#3F,#CF,#B1,#02,#71,#CE,#B1,#01,#71
    DB #DA,#B0,#C9,#7D,#D7,#CF,#B1,#02,#76,#C8,#B1,#01,#76,#DA,#C9,#7D
    DB #D6,#BF,#00,#54,#CF,#B1,#02,#6C,#D5,#BF,#00,#4F,#CE,#B1,#01,#6D
    DB #DA,#B0,#C9,#81,#D5,#BF,#00,#4B,#CF,#B1,#02,#6E,#D6,#CE,#B1,#01
    DB #6E,#DA,#B0,#C9,#7F,#D5,#BF,#00,#4B,#CE,#B1,#02,#6E,#D6,#B1,#01
    DB #6E,#DA,#B0,#C9,#81,#D7,#CF,#B1,#02,#76,#C8,#B1,#01,#76,#DA,#C9
    DB #81,#D6,#BF,#00,#64,#CF,#B1,#02,#69,#CE,#B1,#01,#69,#DA,#B0,#C9
    DB #81,#D5,#BF,#00,#4B,#CF,#B1,#02,#6E,#D6,#CE,#B1,#01,#6E,#DA,#B0
    DB #C9,#81,#D5,#BF,#00,#4B,#CE,#B1,#02,#6E,#D6,#B1,#01,#6E,#DA,#B0
    DB #C6,#81,#D7,#CF,#B1,#02,#76,#C8,#B1,#01,#76,#DA,#C4,#7F,#D6,#BF
    DB #00,#70,#CF,#B1,#02,#67,#BF,#00,#64,#CE,#B1,#01,#69,#DA,#B0,#C3
    DB #7F,#00,#F4,#14,#CE,#B1,#08,#7F,#D9,#40,#CA,#B1,#01,#7F,#C9,#D0
    DB #C8,#D0,#C7,#D0,#DD,#C6,#B1,#02,#7F,#C5,#D0,#DA,#44,#CE,#B1,#08
    DB #81,#D9,#40,#CA,#B1,#01,#81,#C9,#D0,#C8,#D0,#C7,#D0,#DD,#C6,#B1
    DB #02,#81,#C5,#D0,#DA,#44,#CE,#B1,#04,#7D,#D9,#40,#CA,#B1,#01,#7D
    DB #C9,#D0,#C8,#D0,#C7,#D0,#DA,#44,#CE,#B1,#02,#7C,#40,#C8,#7D,#44
    DB #CE,#B1,#08,#78,#D9,#40,#CA,#B1,#01,#78,#C9,#D0,#C8,#D0,#C7,#D0
    DB #C6,#D0,#C5,#D0,#DD,#C4,#B1,#06,#78,#00,#1E,#00,#5E,#0A,#40,#CF
    DB #B1,#02,#6A,#D6,#CE,#B1,#01,#6A,#DA,#B0,#C9,#81,#D5,#BF,#00,#5E
    DB #CE,#B1,#02,#6A,#D6,#B1,#01,#6A,#DA,#B0,#C9,#7F,#D7,#CF,#B1,#02
    DB #76,#C8,#B1,#01,#76,#DA,#7F,#D6,#BF,#00,#7E,#CF,#B1,#02,#65,#CE
    DB #B1,#01,#65,#DA,#B0,#C6,#7F,#D5,#BF,#00,#5E,#CF,#B1,#02,#6A,#D6
    DB #CE,#B1,#01,#6A,#DA,#B0,#C9,#7F,#D5,#BF,#00,#5E,#CE,#B1,#02,#6A
    DB #D6,#B1,#01,#6A,#DA,#B0,#C9,#81,#D7,#CF,#B1,#02,#76,#C8,#B1,#01
    DB #76,#DA,#81,#D6,#BF,#00,#5E,#CF,#B1,#02,#6A,#D5,#BF,#00,#59,#CE
    DB #B1,#01,#6B,#DA,#B0,#C6,#81,#D5,#BF,#00,#54,#CF,#B1,#02,#6C,#D6
    DB #CE,#B1,#01,#6C,#DA,#B0,#C9,#81,#D5,#BF,#00,#54,#CE,#B1,#02,#6C
    DB #D6,#B1,#01,#6C,#DA,#B0,#C9,#7D,#D7,#CF,#B1,#02,#76,#C8,#B1,#01
    DB #76,#DA,#C9,#7C,#D6,#BF,#00,#70,#CF,#B1,#02,#67,#CE,#B1,#01,#67
    DB #DA,#B0,#C9,#7C,#D5,#BF,#00,#54,#CF,#B1,#02,#6C,#D6,#CE,#B1,#01
    DB #6C,#DA,#B0,#C9,#78,#D5,#BF,#00,#54,#CE,#B1,#02,#6C,#D6,#B1,#01
    DB #6C,#DA,#B0,#C9,#78,#D7,#CF,#B1,#02,#76,#C9,#B1,#01,#76,#DA,#C8
    DB #78,#D6,#BF,#00,#4B,#CF,#B1,#02,#6E,#BF,#00,#43,#CE,#B1,#01,#70
    DB #DA,#B0,#C6,#78,#00,#F4,#14,#CE,#B1,#02,#81,#84,#81,#7F,#B1,#01
    DB #7F,#D2,#40,#CC,#02,#81,#01,#18,#00,#FF,#FF,#CB,#B1,#02,#D0,#CA
    DB #B1,#08,#D0,#D9,#B1,#01,#81,#C9,#D0,#C8,#D0,#C7,#D0,#DD,#C6,#B1
    DB #02,#81,#C5,#D0,#DA,#44,#CE,#7D,#7F,#81,#84,#81,#7F,#B1,#01,#7F
    DB #D2,#40,#CC,#02,#81,#01,#18,#00,#FF,#FF,#CB,#B1,#02,#D0,#CA,#B1
    DB #08,#D0,#D9,#B1,#01,#81,#C9,#D0,#C8,#D0,#C7,#D0,#DD,#C6,#B1,#02
    DB #81,#C5,#D0,#DA,#44,#CE,#7D,#7F,#00,#1E,#00,#3F,#0A,#40,#CF,#B1
    DB #02,#71,#D6,#CE,#B1,#01,#71,#DA,#B0,#C9,#81,#D5,#BF,#00,#3F,#CE
    DB #B1,#02,#71,#D6,#B1,#01,#71,#DA,#B0,#C9,#81,#D7,#CF,#B1,#02,#76
    DB #C8,#B1,#01,#76,#DA,#C9,#7F,#D6,#BF,#00,#54,#CF,#B1,#02,#6C,#CE
    DB #B1,#01,#6C,#DA,#B0,#C9,#81,#D5,#BF,#00,#3F,#CF,#B1,#02,#71,#D6
    DB #CE,#B1,#01,#71,#DA,#B0,#C9,#81,#D6,#BF,#00,#3F,#CF,#B1,#02,#71
    DB #CE,#B1,#01,#71,#DA,#B0,#C9,#81,#D7,#CF,#B1,#02,#76,#C8,#B1,#01
    DB #76,#DA,#81,#D6,#BF,#00,#54,#CF,#B1,#02,#6C,#D5,#BF,#00,#4F,#CE
    DB #B1,#01,#6D,#DA,#B0,#C9,#7D,#D5,#BF,#00,#4B,#CF,#B1,#02,#6E,#D6
    DB #CE,#B1,#01,#6E,#DA,#B0,#C9,#81,#D5,#BF,#00,#4B,#CE,#B1,#02,#6E
    DB #D6,#B1,#01,#6E,#DA,#B0,#C9,#81,#D7,#CF,#B1,#02,#76,#C8,#B1,#01
    DB #76,#DA,#C9,#7F,#D6,#BF,#00,#64,#CF,#B1,#02,#69,#CE,#B1,#01,#69
    DB #DA,#B0,#C9,#81,#D5,#BF,#00,#4B,#CF,#B1,#02,#6E,#D6,#CE,#B1,#01
    DB #6E,#DA,#B0,#C9,#81,#D5,#BF,#00,#4B,#CE,#B1,#02,#6E,#D6,#B1,#01
    DB #6E,#DA,#B0,#C9,#81,#D7,#CF,#B1,#02,#76,#C8,#B1,#01,#76,#DA,#81
    DB #D6,#BF,#00,#70,#CF,#B1,#02,#67,#BF,#00,#64,#CE,#B1,#01,#69,#DA
    DB #B0,#C9,#7D,#00,#F4,#14,#CE,#B1,#08,#81,#D9,#40,#CA,#B1,#01,#81
    DB #C9,#D0,#C8,#D0,#C7,#D0,#DD,#C6,#B1,#02,#81,#C5,#D0,#DA,#44,#CE
    DB #B1,#08,#84,#D9,#40,#CA,#B1,#01,#84,#C9,#D0,#C8,#D0,#C7,#D0,#DD
    DB #C6,#B1,#02,#84,#C5,#D0,#DA,#44,#CE,#B1,#04,#7D,#D9,#40,#CA,#B1
    DB #01,#7D,#C9,#D0,#C8,#D0,#C7,#D0,#DA,#44,#CE,#B1,#02,#7C,#40,#C8
    DB #7D,#44,#CE,#B1,#08,#78,#D9,#40,#CA,#B1,#01,#78,#C9,#D0,#C8,#D0
    DB #C7,#D0,#C6,#B1,#02,#D0,#DD,#C5,#78,#D7,#CE,#76,#C9,#76,#00,#1E
    DB #00,#5E,#0A,#40,#CF,#B1,#02,#6A,#D6,#CE,#B1,#01,#6A,#DA,#B0,#C9
    DB #7F,#D5,#BF,#00,#5E,#CE,#B1,#02,#6A,#D6,#B1,#01,#6A,#DA,#B0,#C9
    DB #81,#D7,#CF,#B1,#02,#76,#C8,#B1,#01,#76,#DA,#81,#D6,#BF,#00,#7E
    DB #CF,#B1,#02,#65,#CE,#B1,#01,#65,#DA,#B0,#C6,#81,#D5,#BF,#00,#5E
    DB #CF,#B1,#02,#6A,#D6,#CE,#B1,#01,#6A,#DA,#B0,#C9,#81,#D5,#BF,#00
    DB #5E,#CE,#B1,#02,#6A,#D6,#B1,#01,#6A,#DA,#B0,#C9,#84,#D7,#CF,#B1
    DB #02,#76,#C8,#B1,#01,#76,#DA,#84,#D6,#BF,#00,#5E,#CF,#B1,#02,#6A
    DB #D5,#BF,#00,#59,#CE,#B1,#01,#6B,#DA,#B0,#C6,#84,#D5,#BF,#00,#54
    DB #CF,#B1,#02,#6C,#D6,#CE,#B1,#01,#6C,#DA,#B0,#C9,#84,#D5,#BF,#00
    DB #54,#CE,#B1,#02,#6C,#D6,#B1,#01,#6C,#DA,#B0,#C9,#7D,#D7,#CF,#B1
    DB #02,#76,#C8,#B1,#01,#76,#DA,#C9,#7C,#D6,#BF,#00,#70,#CF,#B1,#02
    DB #67,#CE,#B1,#01,#67,#DA,#B0,#C9,#7C,#D5,#BF,#00,#54,#CF,#B1,#02
    DB #6C,#D6,#CE,#B1,#01,#6C,#DA,#B0,#C9,#78,#D5,#BF,#00,#54,#CE,#B1
    DB #02,#6C,#D6,#B1,#01,#6C,#DA,#B0,#C9,#78,#D7,#CF,#B1,#02,#76,#CA
    DB #B1,#01,#76,#DA,#C9,#78,#D8,#BF,#00,#4B,#CF,#B1,#02,#6E,#BF,#00
    DB #43,#CE,#70,#00,#F0,#06,#C1,#B1,#04,#7D,#C2,#D0,#C3,#D0,#C4,#B1
    DB #01,#D0,#C5,#D0,#C6,#D0,#C8,#D0,#CA,#B1,#04,#7D,#C9,#B1,#0C,#D0
    DB #D9,#C8,#02,#B1,#10,#7C,#01,#0E,#00,#03,#00,#DD,#B1,#08,#7C,#C7
    DB #D0,#00,#1E,#00,#3F,#0A,#40,#CF,#B1,#02,#71,#D6,#CE,#71,#D5,#71
    DB #D6,#71,#DB,#B0,#CF,#74,#CE,#74,#CF,#74,#CE,#74,#DC,#CF,#74,#DB
    DB #CE,#B1,#01,#74,#DA,#C6,#7D,#DB,#CF,#B1,#02,#74,#CE,#B1,#01,#74
    DB #DA,#C6,#7D,#DB,#CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA,#C6,#7D,#DB
    DB #CF,#B1,#02,#74,#DC,#CE,#B1,#01,#74,#DA,#C6,#7D,#DC,#CF,#B1,#02
    DB #74,#DB,#CE,#B1,#01,#74,#DA,#C6,#7D,#DC,#CE,#B1,#02,#74,#DB,#B1
    DB #01,#74,#DA,#C6,#7C,#DB,#CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA,#C6
    DB #7C,#DB,#CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA,#C6,#7C,#DC,#CF,#B1
    DB #02,#74,#DB,#CE,#B1,#01,#74,#DA,#C6,#7C,#DC,#CE,#B1,#02,#74,#DB
    DB #B1,#01,#74,#DA,#C6,#7C,#DB,#CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA
    DB #C5,#7C,#DB,#CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA,#C4,#7C,#00,#F0
    DB #06,#CA,#B1,#10,#7D,#C9,#B1,#08,#D0,#CA,#B1,#02,#84,#B1,#04,#89
    DB #C9,#B1,#02,#D0,#CA,#86,#D9,#C9,#02,#B1,#0A,#88,#01,#10,#00,#FD
    DB #FF,#02,#B1,#0C,#84,#01,#22,#00,#03,#00,#DD,#B1,#04,#84,#C6,#B1
    DB #02,#D0,#C5,#D0,#00,#F0,#18,#CF,#B1,#02,#74,#DB,#CE,#B1,#01,#74
    DB #DA,#C6,#7C,#DC,#CE,#B1,#02,#74,#DB,#B1,#01,#74,#DA,#C6,#7D,#DB
    DB #CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA,#C6,#7D,#DB,#CF,#B1,#02,#74
    DB #CE,#B1,#01,#74,#DA,#C6,#7D,#DC,#CF,#B1,#02,#74,#DB,#CE,#B1,#01
    DB #74,#DA,#C6,#7D,#DB,#CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA,#C6,#7D
    DB #DB,#CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA,#C6,#84,#DB,#CF,#B1,#02
    DB #74,#DC,#CE,#B1,#01,#74,#DA,#C6,#89,#DC,#CF,#B1,#02,#74,#DB,#CE
    DB #B1,#01,#74,#DA,#C6,#86,#DC,#CE,#B1,#02,#74,#DB,#B1,#01,#74,#DA
    DB #C6,#88,#DB,#CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA,#C6,#88,#DB,#CF
    DB #B1,#02,#74,#CE,#B1,#01,#74,#DA,#C6,#88,#DC,#CF,#B1,#02,#74,#DB
    DB #CE,#B1,#01,#74,#DA,#C6,#84,#DC,#CE,#B1,#02,#74,#DB,#B1,#01,#74
    DB #DA,#C6,#84,#DB,#CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA,#C5,#84,#DB
    DB #CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA,#C4,#84,#00,#F1,#02,#CF,#B1
    DB #02,#80,#80,#B1,#04,#80,#80,#CE,#B1,#02,#80,#42,#CF,#B1,#04,#82
    DB #82,#CE,#B1,#02,#82,#41,#CF,#B1,#04,#80,#80,#43,#B1,#02,#7F,#7F
    DB #B1,#04,#7F,#7F,#CE,#B1,#02,#7F,#CF,#B1,#04,#7F,#7F,#CE,#B1,#02
    DB #7F,#CF,#B1,#04,#7F,#7F,#00,#F5,#14,#CE,#B1,#04,#7B,#D9,#40,#CA
    DB #7B,#DA,#45,#CE,#B1,#02,#78,#B1,#04,#76,#B1,#02,#76,#D2,#40,#CB
    DB #02,#B1,#04,#78,#01,#28,#00,#F9,#FF,#D9,#78,#DA,#45,#CE,#7B,#D9
    DB #40,#CA,#B1,#01,#7B,#C9,#D0,#C8,#B1,#02,#D0,#DA,#45,#CE,#B1,#04
    DB #7D,#D9,#40,#CA,#B1,#02,#7D,#C9,#D0,#DD,#C8,#7D,#C7,#D0,#DA,#45
    DB #CE,#7B,#B1,#04,#78,#D9,#40,#CA,#B1,#02,#78,#C9,#D0,#DD,#C8,#78
    DB #DA,#45,#CE,#76,#D9,#40,#CA,#B1,#01,#76,#C9,#D0,#DA,#45,#CE,#B1
    DB #02,#78,#D9,#40,#CA,#B1,#01,#78,#C9,#D0,#00,#1E,#00,#35,#0A,#40
    DB #CF,#B1,#02,#74,#D6,#CE,#B1,#01,#74,#DA,#B0,#C9,#7B,#D5,#BF,#00
    DB #35,#CE,#B1,#02,#74,#D6,#B1,#01,#74,#DA,#B0,#C9,#7B,#D7,#CF,#B1
    DB #02,#76,#C8,#B1,#01,#76,#DA,#C9,#78,#D6,#BF,#00,#47,#CF,#B1,#02
    DB #6F,#CE,#B1,#01,#6F,#DA,#B0,#C9,#76,#D5,#BF,#00,#35,#CF,#B1,#02
    DB #74,#D6,#CE,#B1,#01,#74,#DA,#B0,#C9,#76,#D6,#BF,#00,#35,#CF,#B1
    DB #02,#74,#CE,#B1,#01,#74,#DA,#B0,#C9,#78,#D7,#CF,#B1,#02,#76,#C8
    DB #B1,#01,#76,#DA,#C9,#78,#D6,#BF,#00,#47,#CF,#B1,#02,#6F,#D5,#BF
    DB #00,#43,#CE,#B1,#01,#70,#DA,#B0,#C9,#7B,#D5,#BF,#00,#3F,#CF,#B1
    DB #02,#71,#D6,#CE,#B1,#01,#71,#DA,#B0,#C9,#7B,#D5,#BF,#00,#3F,#CE
    DB #B1,#02,#71,#D6,#B1,#01,#71,#DA,#B0,#C8,#7D,#D7,#CF,#B1,#02,#76
    DB #C8,#B1,#01,#76,#DA,#7D,#D6,#BF,#00,#54,#CF,#B1,#02,#6C,#CE,#B1
    DB #01,#6C,#DA,#B0,#C9,#7B,#D5,#BF,#00,#3F,#CF,#B1,#02,#71,#D6,#CE
    DB #B1,#01,#71,#DA,#B0,#C9,#78,#D5,#BF,#00,#3F,#CE,#B1,#02,#71,#D6
    DB #B1,#01,#71,#DA,#B0,#C9,#78,#D7,#CF,#B1,#02,#76,#C8,#B1,#01,#76
    DB #DA,#C9,#78,#D6,#BF,#00,#5E,#CF,#B1,#02,#6A,#BF,#00,#54,#CE,#B1
    DB #01,#6C,#DA,#B0,#C9,#76,#00,#B1,#02,#D0,#F0,#1A,#CA,#78,#DA,#45
    DB #CE,#B1,#04,#79,#78,#B1,#06,#74,#D9,#40,#CA,#B1,#02,#74,#C9,#D0
    DB #C8,#D0,#DD,#C7,#74,#C6,#D0,#C5,#B1,#04,#D0,#DA,#45,#CE,#73,#D9
    DB #40,#CA,#73,#DD,#C9,#B1,#01,#73,#C8,#D0,#C7,#B1,#02,#D0,#DA,#45
    DB #CE,#B1,#04,#6F,#D9,#40,#CA,#B1,#01,#6F,#C9,#D0,#C8,#B1,#02,#D0
    DB #DA,#45,#CE,#B1,#04,#78,#D9,#40,#CA,#B1,#01,#78,#01,#D0,#01,#02
    DB #00,#01,#B1,#02,#D0,#01,#03,#00,#B1,#04,#76,#00,#1E,#00,#4F,#0A
    DB #40,#CF,#B1,#02,#6D,#D6,#CE,#B1,#01,#6D,#DA,#B0,#C9,#78,#D5,#BF
    DB #00,#4F,#CE,#B1,#02,#6D,#D6,#B1,#01,#6D,#DA,#B0,#C9,#79,#D7,#CF
    DB #B1,#02,#76,#C8,#B1,#01,#76,#DA,#C9,#79,#D6,#BF,#00,#6A,#CF,#B1
    DB #02,#68,#CE,#B1,#01,#68,#DA,#B0,#C9,#74,#D5,#BF,#00,#4F,#CF,#B1
    DB #02,#6D,#D6,#CE,#B1,#01,#6D,#DA,#B0,#C9,#74,#D5,#BF,#00,#4F,#CE
    DB #B1,#02,#6D,#D6,#B1,#01,#6D,#DA,#B0,#C9,#74,#D7,#CF,#B1,#02,#76
    DB #C8,#B1,#01,#76,#DA,#74,#D6,#BF,#00,#4F,#CF,#B1,#02,#6D,#D5,#BF
    DB #00,#4B,#CE,#B1,#01,#6E,#DA,#B0,#C6,#74,#D5,#BF,#00,#47,#CF,#B1
    DB #02,#6F,#D6,#CE,#B1,#01,#6F,#DA,#B0,#C9,#74,#D5,#BF,#00,#47,#CE
    DB #B1,#02,#6F,#D6,#B1,#01,#6F,#DA,#B0,#C9,#73,#D7,#CF,#B1,#02,#76
    DB #C8,#B1,#01,#76,#DA,#73,#D6,#BF,#00,#5E,#CF,#B1,#02,#6A,#CE,#B1
    DB #01,#6A,#DA,#B0,#C9,#73,#D5,#BF,#00,#47,#CF,#B1,#02,#6F,#D6,#CE
    DB #B1,#01,#6F,#DA,#B0,#C9,#6F,#D5,#BF,#00,#47,#CE,#B1,#02,#6F,#D6
    DB #B1,#01,#6F,#DA,#B0,#C9,#6F,#D7,#CF,#B1,#02,#76,#C9,#B1,#01,#76
    DB #DA,#78,#D6,#BF,#00,#3F,#CF,#B1,#02,#71,#BF,#00,#38,#CE,#B1,#01
    DB #73,#DA,#B0,#C9,#76,#00,#B1,#02,#D0,#F0,#1A,#C9,#76,#DA,#45,#CE
    DB #B1,#06,#78,#D9,#40,#CA,#B1,#02,#78,#C9,#B1,#01,#D0,#C8,#D0,#DD
    DB #C7,#B1,#02,#78,#C6,#D0,#DA,#45,#CE,#B1,#01,#7A,#D2,#40,#CC,#02
    DB #7B,#01,#11,#00,#FA,#FF,#CB,#D0,#DD,#CA,#7B,#DA,#45,#CE,#B1,#02
    DB #78,#B1,#01,#7A,#D2,#40,#CC,#02,#7B,#01,#11,#00,#FA,#FF,#CB,#D0
    DB #DD,#CA,#7B,#DA,#45,#CE,#B1,#02,#78,#B1,#01,#7A,#D2,#40,#CC,#02
    DB #7B,#01,#11,#00,#FA,#FF,#CB,#D0,#DD,#CA,#7B,#DA,#45,#CE,#B1,#02
    DB #78,#B1,#01,#7A,#D2,#40,#CC,#02,#7B,#01,#11,#00,#FA,#FF,#CB,#D0
    DB #DD,#CA,#7B,#DA,#45,#CE,#B1,#02,#78,#B1,#01,#7B,#01,#D0,#01,#FF
    DB #FF,#7D,#40,#C8,#7B,#45,#CE,#7F,#40,#C8,#7D,#45,#CE,#7F,#D9,#40
    DB #CC,#02,#80,#01,#0C,#00,#FA,#FF,#DA,#45,#CE,#02,#B1,#02,#7F,#01
    DB #06,#00,#06,#00,#B1,#01,#7B,#40,#C8,#7F,#45,#CE,#B1,#02,#7D,#78
    DB #B1,#01,#76,#D9,#40,#CC,#02,#77,#01,#14,#00,#F7,#FF,#DA,#45,#CE
    DB #02,#B1,#02,#76,#01,#09,#00,#09,#00,#74,#00,#1E,#00,#35,#0A,#40
    DB #CF,#B1,#02,#74,#D6,#CE,#B1,#01,#74,#DA,#B0,#C9,#76,#D5,#BF,#00
    DB #35,#CE,#B1,#02,#74,#D6,#B1,#01,#74,#DA,#B0,#C9,#78,#D7,#CF,#B1
    DB #02,#76,#C8,#B1,#01,#76,#DA,#C9,#78,#D6,#BF,#00,#47,#CF,#B1,#02
    DB #6F,#CE,#B1,#01,#6F,#DA,#B0,#C9,#78,#D5,#BF,#00,#35,#CF,#B1,#02
    DB #74,#D6,#CE,#B1,#01,#74,#DA,#B0,#C9,#78,#D6,#BF,#00,#35,#CF,#B1
    DB #02,#74,#CE,#B1,#01,#74,#DA,#B0,#C9,#7B,#D7,#CF,#B1,#02,#76,#C8
    DB #B1,#01,#76,#DA,#C9,#7B,#D6,#BF,#00,#47,#CF,#B1,#02,#6F,#D5,#BF
    DB #00,#43,#CE,#B1,#01,#70,#DA,#B0,#C9,#78,#D5,#BF,#00,#3F,#CF,#B1
    DB #02,#71,#D6,#CE,#B1,#01,#71,#DA,#B0,#C9,#7B,#D5,#BF,#00,#3F,#CE
    DB #B1,#02,#71,#D6,#B1,#01,#71,#DA,#B0,#C9,#7B,#D7,#CF,#B1,#02,#76
    DB #C8,#B1,#01,#76,#DA,#C9,#78,#D6,#BF,#00,#54,#CF,#B1,#02,#6C,#CE
    DB #B1,#01,#6C,#DA,#B0,#C9,#7D,#D5,#BF,#00,#3F,#CF,#B1,#02,#71,#D6
    DB #CE,#B1,#01,#71,#DA,#B0,#C9,#80,#D5,#BF,#00,#3F,#CE,#B1,#02,#71
    DB #D6,#B1,#01,#71,#DA,#B0,#C9,#7B,#D7,#CF,#B1,#02,#76,#C8,#B1,#01
    DB #76,#DA,#C9,#78,#D6,#BF,#00,#5E,#CF,#B1,#02,#6A,#BF,#00,#54,#CE
    DB #B1,#01,#6C,#DA,#B0,#C9,#76,#00,#B1,#04,#D0,#F0,#12,#CA,#B1,#02
    DB #74,#DD,#C9,#B1,#01,#74,#C8,#D0,#C7,#B1,#02,#D0,#C6,#D0,#DA,#45
    DB #CE,#B1,#04,#7B,#D9,#40,#CA,#B1,#01,#7B,#C9,#D0,#C8,#D0,#C7,#D0
    DB #DD,#C6,#B1,#02,#7B,#C5,#D0,#DA,#45,#CE,#B1,#01,#7C,#D9,#40,#CC
    DB #02,#7D,#01,#0E,#00,#FF,#FF,#CB,#D0,#CA,#D0,#7D,#02,#B1,#07,#7B
    DB #01,#1E,#00,#01,#00,#DD,#B1,#04,#7B,#C9,#B1,#06,#D0,#C8,#B1,#01
    DB #D0,#C7,#D0,#C6,#B1,#02,#D0,#C5,#B1,#06,#D0,#DA,#45,#CE,#B1,#02
    DB #74,#DD,#40,#CA,#74,#DA,#45,#CE,#73,#DD,#40,#CA,#73,#00,#1E,#00
    DB #4F,#0A,#40,#CF,#B1,#02,#6D,#D6,#CE,#B1,#01,#6D,#DA,#B0,#C9,#74
    DB #D5,#BF,#00,#4F,#CE,#B1,#02,#6D,#D6,#B1,#01,#6D,#DA,#B0,#C9,#74
    DB #D7,#CF,#B1,#02,#76,#C8,#B1,#01,#76,#DA,#C9,#74,#D6,#BF,#00,#6A
    DB #CF,#B1,#02,#68,#CE,#B1,#01,#68,#DA,#B0,#C9,#74,#D5,#BF,#00,#4F
    DB #CF,#B1,#02,#6D,#D6,#CE,#B1,#01,#6D,#DA,#B0,#C8,#7B,#D5,#BF,#00
    DB #4F,#CE,#B1,#02,#6D,#D6,#B1,#01,#6D,#DA,#B0,#C9,#7B,#D7,#CF,#B1
    DB #02,#76,#C8,#B1,#01,#76,#DA,#C9,#7D,#D6,#BF,#00,#4F,#CF,#B1,#02
    DB #6D,#D5,#BF,#00,#4B,#CE,#B1,#01,#6E,#DA,#B0,#C9,#7D,#D5,#BF,#00
    DB #47,#CF,#B1,#02,#6F,#D6,#CE,#B1,#01,#6F,#DA,#B0,#C9,#7B,#D5,#BF
    DB #00,#47,#CE,#B1,#02,#6F,#D6,#B1,#01,#6F,#DA,#B0,#C9,#7B,#D7,#CF
    DB #B1,#02,#76,#C8,#B1,#01,#76,#DA,#C9,#7B,#D6,#BF,#00,#5E,#CF,#B1
    DB #02,#6A,#CE,#B1,#01,#6A,#DA,#B0,#C8,#7B,#D5,#BF,#00,#47,#CF,#B1
    DB #02,#6F,#D6,#CE,#B1,#01,#6F,#DA,#B0,#C6,#7B,#D5,#BF,#00,#47,#CE
    DB #B1,#02,#6F,#D6,#B1,#01,#6F,#DA,#B0,#C6,#7B,#D7,#CF,#B1,#02,#76
    DB #C9,#B1,#01,#76,#DA,#74,#D6,#BF,#00,#3F,#CF,#B1,#02,#71,#BF,#00
    DB #38,#CE,#B1,#01,#73,#DA,#B0,#C9,#73,#00,#B1,#02,#D0,#C9,#D0,#C8
    DB #D0,#DD,#C7,#B1,#04,#73,#C6,#B1,#02,#D0,#01,#D0,#01,#02,#00,#C7
    DB #B1,#01,#D0,#C8,#D0,#D3,#CA,#B1,#08,#71,#C9,#D0,#CA,#02,#7C,#01
    DB #EA,#00,#E8,#FF,#C9,#B1,#04,#D0,#CA,#02,#B1,#06,#78,#01,#45,#00
    DB #20,#00,#C9,#B1,#04,#D0,#D9,#02,#7A,#01,#24,#00,#FD,#FF,#C8,#B1
    DB #01,#D0,#01,#D0,#01,#03,#00,#CA,#B1,#02,#75,#02,#B1,#01,#78,#01
    DB #3F,#00,#E0,#FF,#C9,#D0,#00,#1E,#00,#3F,#0A,#40,#CF,#B1,#02,#71
    DB #D6,#CE,#B1,#01,#71,#DA,#B0,#C7,#73,#D5,#BF,#00,#3F,#CE,#B1,#02
    DB #71,#D6,#B1,#01,#71,#DA,#C6,#73,#DB,#B0,#CF,#B1,#02,#74,#CE,#B1
    DB #01,#74,#DA,#C5,#73,#DB,#CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA,#C6
    DB #73,#DC,#CF,#B1,#02,#74,#DB,#CE,#B1,#01,#74,#DA,#C8,#73,#DB,#CF
    DB #B1,#02,#74,#CE,#B1,#01,#74,#DA,#C8,#71,#DB,#CF,#B1,#02,#74,#CE
    DB #B1,#01,#74,#DA,#C8,#71,#DB,#CF,#B1,#02,#74,#DC,#CE,#B1,#01,#74
    DB #DA,#C8,#71,#DC,#CF,#B1,#02,#74,#DB,#CE,#B1,#01,#74,#DA,#C8,#71
    DB #DC,#CE,#B1,#02,#74,#DB,#B1,#01,#74,#DA,#C8,#7C,#DB,#CF,#B1,#02
    DB #74,#CE,#B1,#01,#74,#DA,#C8,#7C,#DB,#CF,#B1,#02,#74,#CE,#B1,#01
    DB #74,#DA,#C8,#7C,#DC,#CF,#B1,#02,#74,#DB,#CE,#B1,#01,#74,#DA,#C8
    DB #78,#DC,#CE,#B1,#02,#74,#DB,#B1,#01,#74,#DA,#C8,#78,#DB,#CF,#B1
    DB #02,#74,#CE,#B1,#01,#74,#DA,#C8,#7A,#DB,#CF,#B1,#02,#74,#CE,#B1
    DB #01,#74,#DA,#C8,#75,#00,#F0,#12,#C9,#B1,#08,#78,#C8,#D0,#DD,#C7
    DB #78,#C6,#B1,#07,#D0,#C7,#B1,#01,#D0,#D9,#C9,#02,#83,#01,#9C,#00
    DB #D0,#FF,#CA,#02,#B1,#07,#84,#01,#76,#00,#FE,#FF,#C9,#B1,#08,#D0
    DB #DD,#C8,#84,#C7,#B1,#04,#D0,#C6,#B1,#02,#D0,#C5,#D0,#00,#F0,#18
    DB #CF,#B1,#02,#74,#D6,#CE,#B1,#01,#71,#DA,#C7,#75,#DC,#CE,#B1,#02
    DB #74,#DB,#B1,#01,#71,#DA,#C7,#78,#DB,#CF,#B1,#02,#74,#CE,#B1,#01
    DB #74,#DA,#C6,#78,#DB,#CF,#B1,#02,#74,#CE,#B1,#01,#74,#DA,#C6,#78
    DB #DC,#CF,#B1,#02,#74,#DB,#CE,#B1,#01,#74,#DA,#C5,#78,#DB,#CF,#B1
    DB #02,#74,#CE,#B1,#01,#74,#DA,#C5,#78,#DB,#CF,#B1,#02,#74,#CE,#B1
    DB #01,#74,#DA,#C5,#78,#DB,#CF,#B1,#02,#74,#DC,#CE,#B1,#01,#74,#DA
    DB #C5,#78,#DC,#CF,#B1,#02,#74,#DB,#CD,#B1,#01,#74,#DA,#C8,#78,#DC
    DB #CD,#B1,#02,#74,#DB,#CC,#B1,#01,#74,#DA,#C8,#78,#DB,#CC,#B1,#02
    DB #74,#CB,#B1,#01,#74,#DA,#C7,#78,#DB,#CA,#B1,#02,#74,#C9,#B1,#01
    DB #74,#DA,#C7,#78,#DC,#C8,#B1,#02,#74,#DB,#C7,#B1,#01,#74,#DA,#C6
    DB #78,#DC,#C7,#B1,#02,#74,#DB,#C6,#B1,#01,#74,#DA,#78,#DB,#C5,#B1
    DB #02,#74,#C4,#B1,#01,#74,#DA,#C5,#78,#DB,#C3,#B1,#02,#74,#C2,#B1
    DB #01,#74,#DA,#C4,#78,#00,#F0,#06,#C4,#B1,#08,#6C,#C5,#D0,#C6,#D0
    DB #C7,#D0,#C9,#B1,#01,#D0,#C7,#B1,#07,#D0,#C6,#B1,#08,#D0,#C5,#D0
    DB #C4,#D0,#00,#F1,#02,#CF,#B1,#02,#7D,#42,#CE,#B1,#04,#7F,#41,#CC
    DB #B1,#02,#7D,#42,#CB,#B1,#04,#7F,#41,#C9,#B1,#02,#7D,#42,#C8,#B1
    DB #04,#7F,#41,#C7,#B1,#02,#7D,#42,#C6,#B1,#04,#7F,#41,#C5,#B1,#02
    DB #7D,#42,#C4,#B1,#04,#7F,#41,#C3,#B1,#02,#7D,#42,#C2,#B1,#20,#7F
    DB #00,#F0,#12,#C7,#B1,#02,#81,#02,#82,#01,#0B,#00,#F8,#FF,#02,#81
    DB #01,#0B,#00,#08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#C8,#02,#81,#01
    DB #0B,#00,#08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#02,#81,#01,#0B,#00
    DB #08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#C9,#02,#81,#01,#0B,#00,#08
    DB #00,#02,#82,#01,#0B,#00,#F8,#FF,#02,#81,#01,#0B,#00,#08,#00,#02
    DB #82,#01,#0B,#00,#F8,#FF,#CA,#02,#81,#01,#0B,#00,#08,#00,#02,#82
    DB #01,#0B,#00,#F8,#FF,#02,#81,#01,#0B,#00,#08,#00,#02,#82,#01,#0B
    DB #00,#F8,#FF,#C9,#02,#81,#01,#0B,#00,#08,#00,#02,#82,#01,#0B,#00
    DB #F8,#FF,#02,#81,#01,#0B,#00,#08,#00,#02,#82,#01,#0B,#00,#F8,#FF
    DB #C8,#02,#81,#01,#0B,#00,#08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#02
    DB #81,#01,#0B,#00,#08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#C7,#02,#81
    DB #01,#0B,#00,#08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#02,#81,#01,#0B
    DB #00,#08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#C6,#02,#81,#01,#0B,#00
    DB #08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#02,#81,#01,#0B,#00,#08,#00
    DB #02,#82,#01,#0B,#00,#F8,#FF,#00,#1E,#00,#38,#08,#40,#CF,#B1,#40
    DB #73,#00,#D3,#C4,#B1,#08,#67,#C5,#D0,#C6,#D0,#C7,#D0,#C9,#B1,#01
    DB #D0,#C7,#B1,#07,#D0,#C6,#B1,#08,#D0,#C5,#D0,#C4,#D0,#00,#D9,#C7
    DB #B1,#02,#81,#02,#82,#01,#0B,#00,#F8,#FF,#02,#81,#01,#0B,#00,#08
    DB #00,#02,#82,#01,#0B,#00,#F8,#FF,#C8,#02,#81,#01,#0B,#00,#08,#00
    DB #02,#82,#01,#0B,#00,#F8,#FF,#02,#81,#01,#0B,#00,#08,#00,#02,#82
    DB #01,#0B,#00,#F8,#FF,#C9,#02,#81,#01,#0B,#00,#08,#00,#02,#82,#01
    DB #0B,#00,#F8,#FF,#02,#81,#01,#0B,#00,#08,#00,#02,#82,#01,#0B,#00
    DB #F8,#FF,#CA,#02,#81,#01,#0B,#00,#08,#00,#02,#82,#01,#0B,#00,#F8
    DB #FF,#02,#81,#01,#0B,#00,#08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#C9
    DB #02,#81,#01,#0B,#00,#08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#02,#81
    DB #01,#0B,#00,#08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#C8,#02,#81,#01
    DB #0B,#00,#08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#02,#81,#01,#0B,#00
    DB #08,#00,#02,#82,#01,#0B,#00,#F8,#FF,#C7,#02,#81,#01,#0B,#00,#08
    DB #00,#02,#82,#01,#0B,#00,#F8,#FF,#02,#81,#01,#0B,#00,#08,#00,#02
    DB #82,#01,#0B,#00,#F8,#FF,#C6,#02,#81,#01,#0B,#00,#08,#00,#02,#82
    DB #01,#0B,#00,#F8,#FF,#02,#81,#01,#0B,#00,#08,#00,#02,#82,#01,#0B
    DB #00,#F8,#FF,#00,#1E,#00,#4B,#08,#40,#CF,#B1,#40,#6E,#00,#D3,#C4
    DB #B1,#08,#71,#C5,#D0,#C6,#D0,#C7,#D0,#C9,#B1,#01,#D0,#C7,#B1,#07
    DB #D0,#C6,#B1,#08,#D0,#C5,#D0,#C4,#D0,#00,#1E,#00,#3F,#08,#40,#CF
    DB #B1,#40,#71,#00,#D3,#C4,#B1,#01,#71,#02,#B1,#07,#73,#01,#36,#00
    DB #FE,#FF,#C5,#B1,#08,#D0,#C6,#D0,#C7,#D0,#C9,#B1,#01,#D0,#C7,#B1
    DB #07,#D0,#C6,#B1,#08,#D0,#C5,#D0,#C4,#D0,#00,#1E,#00,#54,#08,#40
    DB #CF,#B1,#40,#6C,#00,#F1,#02,#C6,#B1,#02,#7D,#7D,#B1,#04,#7D,#7D
    DB #B1,#02,#7D,#42,#B1,#04,#7F,#7F,#B1,#02,#7F,#41,#B1,#04,#7D,#7D
    DB #43,#B1,#02,#7C,#7C,#B1,#04,#7C,#7C,#B1,#02,#7C,#B1,#04,#7C,#7C
    DB #B1,#02,#7C,#B1,#04,#7C,#7C,#00,#C3,#B1,#08,#D0,#C2,#D0,#C1,#B1
    DB #0A,#D0,#B1,#26,#C0,#00,#B1,#40,#D0,#00,#F1,#02,#C7,#B1,#02,#7D
    DB #7D,#B1,#04,#7D,#C8,#7D,#C7,#B1,#02,#7D,#42,#C8,#B1,#04,#7F,#C9
    DB #7F,#C8,#B1,#02,#7F,#41,#CA,#B1,#04,#7D,#7D,#43,#CB,#B1,#02,#7C
    DB #7C,#B1,#04,#7C,#CC,#7C,#CB,#B1,#02,#7C,#CC,#B1,#04,#7C,#CD,#7C
    DB #CC,#B1,#02,#7C,#CE,#B1,#04,#7C,#7C,#00,#B1,#40,#C0,#00,#10,#15
    DB #01,#8E,#00,#00,#01,#8D,#00,#00,#01,#8C,#00,#00,#01,#8B,#00,#00
    DB #01,#8B,#00,#00,#01,#8A,#00,#00,#01,#8A,#00,#00,#01,#8A,#00,#00
    DB #01,#89,#00,#00,#01,#89,#00,#00,#01,#8A,#00,#00,#01,#89,#00,#00
    DB #01,#89,#00,#00,#01,#88,#00,#00,#01,#88,#00,#00,#01,#88,#00,#00
    DB #01,#87,#00,#00,#01,#87,#00,#00,#01,#87,#00,#00,#01,#87,#00,#00
    DB #81,#87,#00,#00,#00,#01,#01,#8F,#00,#00,#01,#09,#01,#8F,#00,#00
    DB #01,#8E,#00,#00,#01,#8E,#FF,#FF,#01,#8E,#FE,#FF,#01,#8E,#FF,#FF
    DB #01,#8E,#00,#00,#01,#8E,#01,#00,#01,#8E,#02,#00,#01,#8E,#01,#00
    DB #00,#01,#00,#9F,#00,#00,#02,#03,#0F,#0F,#C0,#01,#01,#8E,#C0,#02
    DB #00,#90,#00,#00,#01,#02,#01,#1A,#00,#00,#00,#90,#00,#00,#11,#12
    DB #17,#0F,#A0,#00,#11,#0E,#C0,#00,#0D,#0D,#80,#01,#09,#0C,#00,#02
    DB #05,#0B,#80,#02,#03,#0A,#00,#03,#05,#09,#C0,#03,#03,#08,#00,#04
    DB #01,#07,#80,#04,#05,#07,#C0,#04,#03,#06,#40,#05,#01,#06,#80,#05
    DB #03,#05,#00,#06,#01,#04,#40,#06,#01,#03,#C0,#06,#01,#02,#00,#07
    DB #01,#01,#40,#07,#01,#80,#00,#00,#01,#02,#17,#1A,#00,#00,#00,#90
    DB #00,#00,#00,#08,#01,#8F,#00,#00,#01,#8F,#FF,#FF,#01,#8F,#FE,#FF
    DB #01,#8F,#FF,#FF,#01,#8F,#00,#00,#01,#8F,#01,#00,#01,#8F,#02,#00
    DB #01,#8F,#01,#00,#09,#0A,#01,#8F,#00,#00,#01,#8E,#00,#00,#01,#8E
    DB #00,#00,#01,#8D,#00,#00,#01,#8D,#00,#00,#01,#8D,#00,#00,#01,#8C
    DB #00,#00,#01,#8C,#00,#00,#01,#8C,#00,#00,#01,#8B,#00,#00,#02,#03
    DB #01,#1A,#00,#00,#01,#15,#00,#00,#01,#11,#00,#00,#0F,#10,#0F,#0F
    DB #C0,#01,#01,#8E,#C0,#02,#01,#8C,#80,#03,#01,#8A,#40,#04,#01,#89
    DB #C0,#04,#01,#88,#80,#05,#01,#88,#00,#06,#01,#87,#80,#06,#01,#87
    DB #00,#07,#01,#86,#80,#07,#01,#85,#40,#08,#01,#84,#00,#09,#01,#83
    DB #80,#09,#01,#82,#00,#0A,#01,#81,#C0,#0A,#01,#80,#00,#00,#00,#08
    DB #01,#8F,#00,#00,#01,#8F,#FE,#FF,#01,#8F,#FC,#FF,#01,#8F,#FE,#FF
    DB #01,#8F,#00,#00,#01,#8F,#02,#00,#01,#8F,#04,#00,#01,#8F,#02,#00
    DB #00,#01,#00,#00,#03,#00,#04,#07,#00,#03,#00,#02,#05,#00,#03,#00
    DB #05,#08,#01,#02,#0C,#00,#03,#06,#0C,#18,#00,#00,#00,#00
...
; Total Nodes: 4
; Total Connections: 3
; Start Node: gf_start_1772519005654
;
; ARCHITECTURE:
...
gameflow_start:
    ; Load the Start node
    ld hl, gameflow_node_gf_start_1772519005654
...
; Node: Start - "gf_start_1772519005654"
gameflow_node_gf_start_1772519005654:
    db NODE_TYPE_START
    dw gameflow_node_gf_start_1772519005654_data
    dw gameflow_node_gf_start_1772519005654_conn

gameflow_node_gf_start_1772519005654_data:
    dw gameflow_node_gf_start_1772519005654_init    ; Initialization routine address
    db ((gameflow_node_gf_start_1772519005654_init - #4000) / #2000)    ; Initialization routine bank

gameflow_node_gf_start_1772519005654_conn:
    db CONNECTION_DEFAULT
    dw gameflow_node_gfn_1772519015895
    db CONNECTION_END

; ------------------------------------------------------------------
; gameflow_node_gf_start_1772519005654_init
...
; Node: Music - "gfn_1772519015895"
gameflow_node_gfn_1772519015895:
    db NODE_TYPE_MUSIC
    dw gameflow_node_gfn_1772519015895_data
    dw gameflow_node_gfn_1772519015895_conn

gameflow_node_gfn_1772519015895_data:
    db 1, 1, 1    ; command, track index, loop flag

gameflow_node_gfn_1772519015895_conn:
    db CONNECTION_DEFAULT
    dw gameflow_node_gfn_1772519023718
    db CONNECTION_END

; Node: WorldLink - "gfn_1772519023718"
gameflow_node_gfn_1772519023718:
    db NODE_TYPE_WORLD_LINK
    dw gameflow_node_gfn_1772519023718_data
    dw gameflow_node_gfn_1772519023718_conn

gameflow_node_gfn_1772519023718_data:
    dw load_world_worldmap_1772518931791
    db ((load_world_worldmap_1772518931791 - #4000) / #2000)

gameflow_node_gfn_1772519023718_conn:
    db CONNECTION_DEFAULT
    dw gameflow_node_gfn_1772519030191
    db CONNECTION_END

; Node: End - "gfn_1772519030191"
gameflow_node_gfn_1772519030191:
    db NODE_TYPE_END
    dw gameflow_no_data
    dw gameflow_node_gfn_1772519030191_conn

gameflow_node_gfn_1772519030191_conn:
    db CONNECTION_DEFAULT
    dw 0
    db CONNECTION_END
...
; World: New Worldmap (worldmap_1772518931791)
...
; World ID: worldmap_1772518931791
; Screens: 1
; Start Screen Node: wmnode_1772518935884
; ------------------------------------------------------------------
load_world_worldmap_1772518931791:
    ; Load start screen: New Screenmap (screenmap_1772518805887)
    ld a, ((load_screen_new_screenmap_772518805887 - #4000) / #2000)
    ld hl, load_screen_new_screenmap_772518805887
    call mapper_call_hl_auto
...
.dispatch_world:
    ...
    jp z, check_transition_world_worldmap_1772518931791
    ret

check_transition_world_worldmap_1772518931791:
    ret
...
; Free RAM buffer for tile pattern data decompression (32 bytes)
; ==================================================================
ZX0_TILE_PATTERN_BUFFER EQU #E300

; ==================================================================
; ZX0 TILE COLOR BUFFER (AUTO-INJECTED)
; Free RAM buffer for tile color data decompression (32 bytes)
; ==================================================================
