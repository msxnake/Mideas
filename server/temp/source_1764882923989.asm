...
sprite_pattern      EQU #C2C8   ; Sprite pattern IDs (2 bytes)
sprite_color        EQU #C2CA   ; Sprite colors (2 bytes)
sprite_attributes   EQU #C2CC   ; Interleaved sprite attributes (2 * 4 bytes)

; ==================================================================
; SCREEN SYSTEM VARIABLES (1 screens detected)
; ==================================================================
current_screen_id   EQU #C2D4   ; Currently displayed screen ID
screen_dirty_flag   EQU #C2D5   ; Screen needs redraw flag

; ==================================================================
; PLAYER SYSTEM VARIABLES (player entity detected)
; ==================================================================
player_x            EQU #C2D6   ; Player X position (16-bit)
player_y            EQU #C2D8   ; Player Y position (16-bit)
player_health       EQU #C2DA   ; Player health points
player_score        EQU #C2DB   ; Player score (16-bit)

; ==================================================================
; TEMPORARY VARIABLES (ALWAYS NEEDED)
; ==================================================================
temp_word_1         EQU #C2DD   ; Temporary 16-bit storage
temp_word_2         EQU #C2DF   ; Temporary 16-bit storage
temp_byte_1         EQU #C2E1   ; Temporary 8-bit storage
temp_byte_2         EQU #C2E2   ; Temporary 8-bit storage
temp_byte_3         EQU #C2E3   ; Temporary 8-bit storage (32 bytes)
temp_byte_4         EQU #C303   ; Temporary 8-bit storage (32 bytes)
temp_byte_5         EQU #C323   ; Temporary 8-bit storage (32 bytes)
temp_byte_6         EQU #C343   ; Temporary 8-bit storage (32 bytes)
temp_byte_7         EQU #C363   ; Temporary 8-bit storage (32 bytes)
temp_word_3         EQU #C383   ; Temporary 16-bit storage (64 bytes)
temp_word_4         EQU #C3C3   ; Temporary 16-bit storage (64 bytes)

; ==================================================================
; END OF VARIABLES
; ==================================================================
RAM_USAGE_END       EQU #C403   ; End of project variables (1027 bytes used)

; ==================================================================
; MEMORY SAFETY CHECK
; ==================================================================
; RAM Layout:
;   #C000-#C403: Project variables (1027 bytes)
;   #C403-#F37F: Free RAM (~12157 bytes available)
...
; 2 sprite attribute slots reserved (2 usable on MSX1)
; ==================================================================
...
SPRITE_0_PATTERN_LAYER0 EQU NEW_SPRITE_0_F0_LAYER1

SPRITE_0_PATTERN_LAYER1 EQU NEW_SPRITE_0_F0_LAYER2
...
SPRITE_1_PATTERN_LAYER0 EQU COIN_1_F0_LAYER1
...
load_sprite_patterns:
    ; Load sprite 0 layer 0
    ld hl, SPRITE_0_PATTERN_LAYER0
    ld de, SPRPAT + (0 * 32)
    ld bc, 32
    call LDIRVM
    ; Load sprite 0 layer 1
    ld hl, SPRITE_0_PATTERN_LAYER1
    ld de, SPRPAT + (1 * 32)
    ld bc, 32
    call LDIRVM
    ; Load sprite 1 layer 0
    ld hl, SPRITE_1_PATTERN_LAYER0
    ld de, SPRPAT + (2 * 32)
    ld bc, 32
    call LDIRVM
    ; Padding blank layer 1 for sprite 1
    ld hl, SPRITE_0_PATTERN_LAYER0
    ld de, SPRPAT + (3 * 32)
    ld bc, 32
    call LDIRVM
...
update_sprites_to_vram:
    ld hl, sprite_attributes
    ld de, SPRATR
    ld bc, 8  ; 4 bytes per sprite slot
    call LDIRVM
    ret
...
SPRITE_LAYER_MAX    EQU 2
SPRITE_SLOT_COUNT   EQU 2

; Sprite pattern base index per sprite (points to first layer pattern)
sprite_pattern_base_table:
    db 0
    db 2

; Layers used per sprite (colors excluding background)
sprite_layer_count_table:
    db 2
    db 1

; Layer colors per sprite (flattened, padded to SPRITE_LAYER_MAX)
; Index = spriteIndex * SPRITE_LAYER_MAX + layerIndex
sprite_layer_color_table:
    db #FFFFFF, #D4524D
    db #D4C154, 0
...
; Sprite asset index per entity (used by sprite renderer)
entity_sprite_asset_index_table:
    db 0
    db 0
    db 0
    db 0
...
init_player_1:
    ...
    ; Use multi-layer renderer for this entity
    ld c, 0             ; Entity Index
...
init_nucleo_1:
    ...
    ; Non-renderable entity (no Render) - skip sprite init
.skip_show_1:
    ret
