...
function setAttribInfoBufferFromArray(gl2, attribInfo, array, offset) {
  array = makeTypedArray(array);
  if (offset !== void 0) {
    gl2.bindBuffer(ARRAY_BUFFER$1, attribInfo.buffer);
    gl2.bufferSubData(ARRAY_BUFFER$1, offset, array);
  } else {
    setBufferFromTypedArray(gl2, ARRAY_BUFFER$1, attribInfo.buffer, array, attribInfo.drawType);
  }
}
...
function createBufferInfoFromArrays(gl2, arrays, srcBufferInfo) {
  ...
  if (indices) {
    ...
    bufferInfo.indices = createBufferFromTypedArray(gl2, newIndices, ELEMENT_ARRAY_BUFFER$2);
...
function getTextureInternalFormatInfo(internalFormat) {
  if (!s_textureInternalFormatInfo) {
    ...
    Object.keys(t).forEach(function(internalFormat2) {
      const info2 = t[internalFormat2];
      info2.bytesPerElementMap = {};
      info2.bytesPerElement.forEach(function(bytesPerElement, ndx) {
        const type = info2.type[ndx];
        info2.bytesPerElementMap[type] = bytesPerElement;
      });
    });
...
function getFormatAndTypeForInternalFormat(internalFormat) {
  ...
  return {
    ...
    type: ((_a = info.type) == null ? void 0 : _a[0]) ?? UNSIGNED_BYTE$1
  };
}
...
function canGenerateMipmap(gl2, width, height, internalFormat) {
  if (!isWebGL2(gl2)) {
...
function getTextureTypeForArrayType(gl2, src, defaultType) {
  if (isArrayBuffer2(src)) {
    return getGLTypeForTypedArray(src);
  }
  return defaultType || UNSIGNED_BYTE$1;
}
function guessDimensions(gl2, target, width, height, numElements) {
  if (numElements % 1 !== 0) {
...
function scopedSetPackState(gl2, options, fn) {
  let colorspaceConversion;
  let premultiplyAlpha;
  let flipY;
  if (options.colorspaceConversion !== void 0) {
    colorspaceConversion = gl2.getParameter(UNPACK_COLORSPACE_CONVERSION_WEBGL);
    gl2.pixelStorei(UNPACK_COLORSPACE_CONVERSION_WEBGL, options.colorspaceConversion);
  }
  if (options.premultiplyAlpha !== void 0) {
    premultiplyAlpha = gl2.getParameter(UNPACK_PREMULTIPLY_ALPHA_WEBGL);
    gl2.pixelStorei(UNPACK_PREMULTIPLY_ALPHA_WEBGL, options.premultiplyAlpha);
  }
  if (options.flipY !== void 0) {
    flipY = gl2.getParameter(UNPACK_FLIP_Y_WEBGL);
    gl2.pixelStorei(UNPACK_FLIP_Y_WEBGL, options.flipY);
  }
  fn();
  if (colorspaceConversion !== void 0) {
    gl2.pixelStorei(UNPACK_COLORSPACE_CONVERSION_WEBGL, colorspaceConversion);
  }
  if (premultiplyAlpha !== void 0) {
    gl2.pixelStorei(UNPACK_PREMULTIPLY_ALPHA_WEBGL, premultiplyAlpha);
  }
  if (flipY !== void 0) {
    gl2.pixelStorei(UNPACK_FLIP_Y_WEBGL, flipY);
  }
}
function getPackStateOption(gl2, options, property, pname) {
  const v = options[property];
  return {
    [property]: v === void 0 ? gl2.getParameter(pname) : v
  };
}
function copyOptionsAndApplyPackState(gl2, options) {
  return {
    ...options,
    ...getPackStateOption(gl2, options, "flipY", UNPACK_FLIP_Y_WEBGL),
    ...getPackStateOption(gl2, options, "premultiplyAlpha", UNPACK_PREMULTIPLY_ALPHA_WEBGL),
    ...getPackStateOption(gl2, options, "colorspaceConversion", UNPACK_COLORSPACE_CONVERSION_WEBGL)
  };
}
function setSkipStateToDefault(gl2) {
  gl2.pixelStorei(UNPACK_ALIGNMENT, 4);
  if (isWebGL2(gl2)) {
    gl2.pixelStorei(UNPACK_ROW_LENGTH, 0);
    gl2.pixelStorei(UNPACK_IMAGE_HEIGHT, 0);
    gl2.pixelStorei(UNPACK_SKIP_PIXELS, 0);
    gl2.pixelStorei(UNPACK_SKIP_ROWS, 0);
    gl2.pixelStorei(UNPACK_SKIP_IMAGES, 0);
  }
}
function setTextureSamplerParameters(gl2, target, parameteriFn, options) {
  if (options.minMag) {
    parameteriFn.call(gl2, target, TEXTURE_MIN_FILTER, options.minMag);
    parameteriFn.call(gl2, target, TEXTURE_MAG_FILTER, options.minMag);
  }
  if (options.min) {
    parameteriFn.call(gl2, target, TEXTURE_MIN_FILTER, options.min);
  }
  if (options.mag) {
    parameteriFn.call(gl2, target, TEXTURE_MAG_FILTER, options.mag);
  }
  if (options.wrap) {
    parameteriFn.call(gl2, target, TEXTURE_WRAP_S, options.wrap);
    parameteriFn.call(gl2, target, TEXTURE_WRAP_T, options.wrap);
    if (target === TEXTURE_3D$1 || isSampler(gl2, target)) {
      parameteriFn.call(gl2, target, TEXTURE_WRAP_R, options.wrap);
    }
  }
  if (options.wrapR) {
    parameteriFn.call(gl2, target, TEXTURE_WRAP_R, options.wrapR);
  }
  if (options.wrapS) {
    parameteriFn.call(gl2, target, TEXTURE_WRAP_S, options.wrapS);
  }
  if (options.wrapT) {
    parameteriFn.call(gl2, target, TEXTURE_WRAP_T, options.wrapT);
  }
  if (options.minLod !== void 0) {
    parameteriFn.call(gl2, target, TEXTURE_MIN_LOD, options.minLod);
  }
  if (options.maxLod !== void 0) {
    parameteriFn.call(gl2, target, TEXTURE_MAX_LOD, options.maxLod);
  }
  if (options.baseLevel !== void 0) {
    parameteriFn.call(gl2, target, TEXTURE_BASE_LEVEL, options.baseLevel);
  }
  if (options.maxLevel !== void 0) {
    parameteriFn.call(gl2, target, TEXTURE_MAX_LEVEL, options.maxLevel);
  }
  if (options.compareFunc !== void 0) {
    parameteriFn.call(gl2, target, TEXTURE_COMPARE_FUNC, options.compareFunc);
  }
  if (options.compareMode !== void 0) {
    parameteriFn.call(gl2, target, TEXTURE_COMPARE_MODE, options.compareMode);
  }
}
function setTextureParameters(gl2, tex, options) {
  const target = options.target || TEXTURE_2D$2;
  gl2.bindTexture(target, tex);
  setTextureSamplerParameters(gl2, target, gl2.texParameteri, options);
}
function setSamplerParameters(gl2, sampler, options) {
  setTextureSamplerParameters(gl2, sampler, gl2.samplerParameteri, options);
}
function createSampler(gl2, options) {
  const sampler = gl2.createSampler();
  setSamplerParameters(gl2, sampler, options);
  return sampler;
}
function createSamplers(gl2, samplerOptions) {
  const samplers = {};
  Object.keys(samplerOptions).forEach(function(name) {
    samplers[name] = createSampler(gl2, samplerOptions[name]);
  });
  return samplers;
}
...
function setTextureFilteringForSizeInternal(gl2, tex, { options, width, height, internalFormat, lastMipLevelUploaded }) {
  ...
  if (lastMipLevelUploaded > 1) {
    ...
    const minFiltering = canFilter(internalFormat) ? canUseMips ? NEAREST_MIPMAP_LINEAR : LINEAR$1 : NEAREST;
    gl2.texParameteri(target, TEXTURE_MAG_FILTER, magFiltering);
    gl2.texParameteri(target, TEXTURE_MIN_FILTER, minFiltering);
  } else if (canGenerateMipmap(gl2, width, height, internalFormat)) {
    gl2.generateMipmap(target);
  } else {
    const filtering = canFilter(internalFormat) ? LINEAR$1 : NEAREST;
    gl2.texParameteri(target, TEXTURE_MIN_FILTER, filtering);
    gl2.texParameteri(target, TEXTURE_MAG_FILTER, filtering);
    gl2.texParameteri(target, TEXTURE_WRAP_S, CLAMP_TO_EDGE$1);
    gl2.texParameteri(target, TEXTURE_WRAP_T, CLAMP_TO_EDGE$1);
  }
}
function setTextureFilteringForSize(gl2, tex, options, width, height, internalFormat) {
  setTextureFilteringForSizeInternal(gl2, tex, { options, width, height, internalFormat });
}
function shouldAutomaticallySetTextureFilteringForSize(options) {
  return options.auto === true || options.auto === void 0 && options.level === void 0;
}
function getCubeFaceOrder(gl2, options) {
  options = options || {};
...
function getCubeFacesWithNdx(gl2, options) {
  const faces = getCubeFaceOrder(gl2, options);
...
function setTextureFromElement(gl2, tex, element, options) {
  options = options || defaults$1.textureOptions;
  const target = options.target || TEXTURE_2D$2;
  ...
  if (target === TEXTURE_CUBE_MAP$1) {
    ...
    if (ctx) {
      ...
      width = size;
      height = size;
      scopedSetPackState(gl2, options, () => {
        getCubeFacesWithNdx(gl2, options).forEach(function(f) {
          const xOffset = slices[f.ndx * 2 + 0] * size;
          const yOffset = slices[f.ndx * 2 + 1] * size;
          ctx.drawImage(element, xOffset, yOffset, size, size, 0, 0, size, size);
          gl2.texImage2D(f.face, level, internalFormat, format, type, ctx.canvas);
        });
        ctx.canvas.width = 1;
        ctx.canvas.height = 1;
      });
      ...
      getCubeFacesWithNdx(gl2, options).forEach(function(f) {
        const xOffset = slices[f.ndx * 2 + 0] * size;
        const yOffset = slices[f.ndx * 2 + 1] * size;
        gl2.texImage2D(f.face, level, internalFormat, size, size, 0, format, type, null);
        createImageBitmap(element, xOffset, yOffset, size, size, {
          ...
          scopedSetPackState(gl2, options, () => {
            gl2.bindTexture(target, tex);
            gl2.texImage2D(f.face, level, internalFormat, format, type, imageBitmap);
            if (shouldAutomaticallySetTextureFilteringForSize(options)) {
              setTextureFilteringForSize(gl2, tex, options, width, height, internalFormat);
            }
          });
        });
      });
    ...
    scopedSetPackState(gl2, options, () => {
      ...
      setSkipStateToDefault(gl2);
    });
  } else {
    scopedSetPackState(gl2, options, () => {
      gl2.texImage2D(target, level, internalFormat, format, type, element);
    });
  }
  if (shouldAutomaticallySetTextureFilteringForSize(options)) {
    setTextureFilteringForSize(gl2, tex, options, width, height, internalFormat);
  }
  setTextureParameters(gl2, tex, options);
}
function noop() {
}
...
function setTextureTo1PixelColor(gl2, tex, options) {
  options = options || defaults$1.textureOptions;
  const target = options.target || TEXTURE_2D$2;
  gl2.bindTexture(target, tex);
  ...
  if (target === TEXTURE_CUBE_MAP$1) {
    for (let ii = 0; ii < 6; ++ii) {
      gl2.texImage2D(TEXTURE_CUBE_MAP_POSITIVE_X + ii, 0, RGBA$1, 1, 1, 0, RGBA$1, UNSIGNED_BYTE$1, color);
    }
  } else if (target === TEXTURE_3D$1 || target === TEXTURE_2D_ARRAY$1) {
    gl2.texImage3D(target, 0, RGBA$1, 1, 1, 1, 0, RGBA$1, UNSIGNED_BYTE$1, color);
  } else {
    gl2.texImage2D(target, 0, RGBA$1, 1, 1, 0, RGBA$1, UNSIGNED_BYTE$1, color);
  }
}
function loadTextureFromUrl(gl2, tex, options, callback) {
  callback = callback || noop;
  options = options || defaults$1.textureOptions;
  setTextureTo1PixelColor(gl2, tex, options);
  options = copyOptionsAndApplyPackState(gl2, options);
  const img = loadAndUseImage(options.src, options.crossOrigin, function(err, img2) {
    if (err) {
      callback(err, tex, img2);
    } else {
      setTextureFromElement(gl2, tex, img2, options);
      callback(null, tex, img2);
    }
  });
  return img;
}
function loadCubemapFromUrls(gl2, tex, options, callback) {
  callback = callback || noop;
          ...
          scopedSetPackState(gl2, options, () => {
            ...
            if (numToLoad === 5) {
              getCubeFaceOrder().forEach(function(otherTarget) {
                gl2.texImage2D(otherTarget, level, internalFormat, format, type, img);
              });
            } else {
              gl2.texImage2D(faceTarget, level, internalFormat, format, type, img);
            }
            if (shouldAutomaticallySetTextureFilteringForSize(options)) {
              gl2.generateMipmap(target);
            }
...
function loadSlicesFromUrls(gl2, tex, options, callback) {
  callback = callback || noop;
  const urls = options.src;
  const internalFormat = options.internalFormat || options.format || RGBA$1;
  const formatType = getFormatAndTypeForInternalFormat(internalFormat);
        ...
        scopedSetPackState(gl2, options, () => {
          ...
          if (firstImage) {
            ...
            for (let s = 0; s < depth; ++s) {
              gl2.texSubImage3D(target, level, 0, 0, s, width, height, 1, format, type, img);
            }
          } else {
            let src = img;
            let ctx;
            ...
            gl2.texSubImage3D(target, level, 0, 0, slice, width, height, 1, format, type, src);
...
function setTextureFromArray(gl2, tex, src, options) {
  options = options || defaults$1.textureOptions;
  const target = options.target || TEXTURE_2D$2;
  gl2.bindTexture(target, tex);
  const level = options.level || 0;
  ...
  const formatType = getFormatAndTypeForInternalFormat(internalFormat);
  const format = options.format || formatType.format;
  const type = options.type || getTextureTypeForArrayType(gl2, src, formatType.type);
  if (!isArrayBuffer2(src)) {
  ...
  setSkipStateToDefault(gl2);
  gl2.pixelStorei(UNPACK_ALIGNMENT, options.unpackAlignment || 1);
  let mipLevelOffset = 0;
  scopedSetPackState(gl2, options, () => {
    ...
    while (mipLevelByteOffset < src.byteLength) {
      ...
      if (target === TEXTURE_CUBE_MAP$1) {
        ...
        getCubeFacesWithNdx(gl2, options).forEach((f) => {
          const offset = faceSize * f.ndx;
          const data = mipSource.subarray(offset, offset + faceSize);
          if (compressed) {
            gl2.compressedTexImage2D(f.face, mipLevel, internalFormat, mipWidth, mipHeight, 0, data);
          } else {
            gl2.texImage2D(f.face, mipLevel, internalFormat, mipWidth, mipHeight, 0, format, type, data);
          }
        });
      } else if (target === TEXTURE_3D$1 || target === TEXTURE_2D_ARRAY$1) {
        gl2.texImage3D(target, mipLevel, internalFormat, mipWidth, mipHeight, mipDepth, 0, format, type, mipSource);
      } else {
        if (compressed) {
          gl2.compressedTexImage2D(target, mipLevel, internalFormat, mipWidth, mipHeight, 0, mipSource);
        } else {
          gl2.texImage2D(target, mipLevel, internalFormat, mipWidth, mipHeight, 0, format, type, mipSource);
        }
      }
      ++mipLevelOffset;
...
function setEmptyTexture(gl2, tex, options) {
  const target = options.target || TEXTURE_2D$2;
  gl2.bindTexture(target, tex);
  const level = options.level || 0;
  const internalFormat = options.internalFormat || options.format || RGBA$1;
  ...
  scopedSetPackState(gl2, options, () => {
    if (target === TEXTURE_CUBE_MAP$1) {
      for (let ii = 0; ii < 6; ++ii) {
        gl2.texImage2D(TEXTURE_CUBE_MAP_POSITIVE_X + ii, level, internalFormat, options.width, options.height, 0, format, type, null);
      }
    } else if (target === TEXTURE_3D$1 || target === TEXTURE_2D_ARRAY$1) {
      gl2.texImage3D(target, level, internalFormat, options.width, options.height, options.depth, 0, format, type, null);
    } else {
      gl2.texImage2D(target, level, internalFormat, options.width, options.height, 0, format, type, null);
    }
  });
}
function createTexture(gl2, options, callback) {
  callback = callback || noop;
  options = options || defaults$1.textureOptions;
  ...
  if (src) {
    ...
    if (typeof src === "string") {
      loadTextureFromUrl(gl2, tex, options, callback);
    } else if (isArrayBuffer2(src) || Array.isArray(src) && (typeof src[0] === "number" || Array.isArray(src[0]) || isArrayBuffer2(src[0]))) {
      const dimensions = setTextureFromArray(gl2, tex, src, options);
      width = dimensions.width;
      height = dimensions.height;
      lastMipLevelUploaded = dimensions.lastMipLevelUploaded;
    } else if (Array.isArray(src) && (typeof src[0] === "string" || isTexImageSource(src[0]))) {
      if (target === TEXTURE_CUBE_MAP$1) {
        loadCubemapFromUrls(gl2, tex, options, callback);
      } else {
        loadSlicesFromUrls(gl2, tex, options, callback);
      }
    } else {
      setTextureFromElement(gl2, tex, src, options);
      width = src.width;
      height = src.height;
    }
  } else {
    setEmptyTexture(gl2, tex, options);
  }
  if (shouldAutomaticallySetTextureFilteringForSize(options)) {
    setTextureFilteringForSizeInternal(gl2, tex, { options, width, height, internalFormat, lastMipLevelUploaded });
  }
  setTextureParameters(gl2, tex, options);
  return tex;
}
function createTextureAsync(gl2, options) {
  return new Promise((resolve, reject) => {
    createTexture(gl2, options, (err, texture, source) => {
      if (err) {
        reject(err);
      } else {
...
function resizeTexture(gl2, tex, options, width, height, depth) {
  width = width || options.width;
  height = height || options.height;
  ...
  if (!src) {
    ...
    type = options.type || getTextureTypeForArrayType(gl2, src, formatType.type);
  } else {
    type = options.type || formatType.type;
  }
  if (target === TEXTURE_CUBE_MAP$1) {
    for (let ii = 0; ii < 6; ++ii) {
      gl2.texImage2D(TEXTURE_CUBE_MAP_POSITIVE_X + ii, level, internalFormat, width, height, 0, format, type, null);
    }
  } else if (target === TEXTURE_3D$1 || target === TEXTURE_2D_ARRAY$1) {
    gl2.texImage3D(target, level, internalFormat, width, height, depth, 0, format, type, null);
  } else {
    gl2.texImage2D(target, level, internalFormat, width, height, 0, format, type, null);
  }
}
function isAsyncSrc(src) {
  return typeof src === "string" || Array.isArray(src) && typeof src[0] === "string";
}
function createTextures(gl2, textureOptions, callback) {
  callback = callback || noop;
  ...
  Object.keys(textureOptions).forEach(function(name) {
    ...
    textures2[name] = createTexture(gl2, options, onLoadFn);
  });
  callCallbackIfReady();
  return textures2;
}
function createTexturesAsync(gl2, options) {
  return new Promise((resolve, reject) => {
    createTexture(gl2, options, (err, textures2, sources) => {
      if (err) {
        reject(err);
      } else {
...
function samplerArraySetter(gl2, type, unit, location2, size) {
  ...
  return isWebGL2(gl2) ? function(textures2) {
...
async function waitForAllProgramsLinkCompletionAsync(gl2, programs2) {
...
function createProgramFromScripts(gl2, shaderScriptIds, opt_attribs, opt_locations, opt_errorCallback) {
  const progOptions = getProgramOptions(opt_attribs, opt_locations, opt_errorCallback);
  const shaders = [];
...
function bindTransformFeedbackInfo(gl2, transformFeedbackInfo, bufferInfo) {
  ...
  for (const name in bufferInfo) {
    ...
    if (varying) {
      ...
      if (buf.offset) {
        gl2.bindBufferRange(TRANSFORM_FEEDBACK_BUFFER, varying.index, buf.buffer, buf.offset, buf.size);
      } else {
        gl2.bindBufferBase(TRANSFORM_FEEDBACK_BUFFER, varying.index, buf.buffer);
      }
    }
  }
}
function createTransformFeedback(gl2, programInfo, bufferInfo) {
  const tf = gl2.createTransformFeedback();
  gl2.bindTransformFeedback(TRANSFORM_FEEDBACK, tf);
  gl2.useProgram(programInfo.program);
  bindTransformFeedbackInfo(gl2, programInfo, bufferInfo);
  gl2.bindTransformFeedback(TRANSFORM_FEEDBACK, null);
  return tf;
}
...
function createUniformBlockInfoFromProgram(gl2, program, uniformBlockSpec, blockName, options = {}) {
  ...
  const buffer = options.buffer ?? gl2.createBuffer();
  const uniformBufferIndex = blockSpec.index;
  gl2.bindBuffer(UNIFORM_BUFFER, buffer);
  if (!options.buffer) {
    gl2.bufferData(UNIFORM_BUFFER, array.byteLength, DYNAMIC_DRAW);
  }
  gl2.uniformBlockBinding(program, blockSpec.index, uniformBufferIndex);
...
function createUniformBlockInfo(gl2, programInfo, blockName, options = {}) {
  return createUniformBlockInfoFromProgram(gl2, programInfo.program, programInfo.uniformBlockSpec, blockName, options);
}
function bindUniformBlock(gl2, programInfo, uniformBlockInfo) {
  const uniformBlockSpec = programInfo.uniformBlockSpec || programInfo;
  const blockSpec = uniformBlockSpec.blockSpecs[uniformBlockInfo.name];
  if (blockSpec) {
    const bufferBindIndex = blockSpec.index;
    gl2.bindBufferRange(UNIFORM_BUFFER, bufferBindIndex, uniformBlockInfo.buffer, uniformBlockInfo.offset || 0, uniformBlockInfo.size ?? uniformBlockInfo.array.byteLength);
    return true;
  }
  return false;
}
function setUniformBlock(gl2, programInfo, uniformBlockInfo) {
  if (bindUniformBlock(gl2, programInfo, uniformBlockInfo)) {
    gl2.bufferSubData(UNIFORM_BUFFER, 0, uniformBlockInfo.asUint8, uniformBlockInfo.offset || 0, uniformBlockInfo.size || 0);
  }
}
...
function setBuffersAndAttributes(gl2, programInfo, buffers) {
...
function createProgramInfoFromProgram(gl2, program) {
  ...
  if (isWebGL2(gl2)) {
    programInfo.uniformBlockSpec = createUniformBlockSpecFromProgram(gl2, program);
    programInfo.transformFeedbackInfo = createTransformFeedbackInfo(gl2, program);
  }
  return programInfo;
}
var notIdRE = /\s|{|}|;/;
...
function checkAllPrograms(gl2, programs2, programSpecs, noDeleteShadersSet, programOptions) {
...
function createPrograms(gl2, programSpecs, programOptions = {}) {
  ...
  if (programOptions.callback) {
    waitForAllProgramsLinkCompletionAsync(gl2, programs2).then(() => {
      const errors2 = checkAllPrograms(gl2, programs2, programSpecs, noDeleteShadersSet, programOptions);
      programOptions.callback(errors2, errors2 ? void 0 : programs2);
    });
    return void 0;
  }
  const errors = checkAllPrograms(gl2, programs2, programSpecs, noDeleteShadersSet, programOptions);
...
function createProgramInfos(gl2, programSpecs, programOptions) {
  ...
  function createProgramInfosForPrograms(gl3, programs3) {
  ...
  if (origCallback) {
    programOptions.callback = (err, programs3) => {
      origCallback(err, err ? void 0 : createProgramInfosForPrograms(gl2, programs3));
    };
  }
  const programs2 = createPrograms(gl2, programSpecs, programOptions);
  if (origCallback || !programs2) {
    return void 0;
  }
  return createProgramInfosForPrograms(gl2, programs2);
}
...
function drawObjectList(gl2, objectsToDraw) {
  ...
  objectsToDraw.forEach(function(object) {
    ...
    if (bindBuffers || bufferInfo !== lastUsedBufferInfo) {
      ...
      setBuffersAndAttributes(gl2, programInfo, bufferInfo);
    }
...
function createFramebufferInfo(gl2, attachments, width, height) {
  const target = FRAMEBUFFER;
  const fb = gl2.createFramebuffer();
  gl2.bindFramebuffer(target, fb);
  width = width || gl2.drawingBufferWidth;
  height = height || gl2.drawingBufferHeight;
  ...
  attachments.forEach(function(attachmentOptions, i) {
    ...
    if (!attachment) {
      if (samples !== void 0 || isRenderbufferFormat(format)) {
        attachment = gl2.createRenderbuffer();
        gl2.bindRenderbuffer(RENDERBUFFER, attachment);
        if (samples > 1) {
          gl2.renderbufferStorageMultisample(RENDERBUFFER, samples, format, width, height);
        } else {
          gl2.renderbufferStorage(RENDERBUFFER, format, width, height);
        }
      } else {
        ...
        attachment = createTexture(gl2, textureOptions);
      }
    }
    if (isRenderbuffer(gl2, attachment)) {
      gl2.framebufferRenderbuffer(target, attachmentPoint, RENDERBUFFER, attachment);
    } else if (isTexture(gl2, attachment)) {
  ...
  if (gl2.drawBuffers) {
    gl2.drawBuffers(usedColorAttachmentsPoints);
  }
  return framebufferInfo;
}
function resizeFramebufferInfo(gl2, framebufferInfo, attachments, width, height) {
  width = width || gl2.drawingBufferWidth;
  height = height || gl2.drawingBufferHeight;
  ...
  attachments.forEach(function(attachmentOptions, ndx) {
    ...
    const format = attachmentOptions.format;
    const samples = attachmentOptions.samples;
    if (samples !== void 0 || isRenderbuffer(gl2, attachment)) {
      gl2.bindRenderbuffer(RENDERBUFFER, attachment);
      if (samples > 1) {
        gl2.renderbufferStorageMultisample(RENDERBUFFER, samples, format, width, height);
      } else {
      ...
      resizeTexture(gl2, attachment, attachmentOptions, width, height);
    } else {
      throw new Error("unknown attachment type");
    }
  });
}
function bindFramebufferInfo(gl2, framebufferInfo, target) {
  target = target || FRAMEBUFFER;
  if (framebufferInfo) {
    gl2.bindFramebuffer(target, framebufferInfo.framebuffer);
    gl2.viewport(0, 0, framebufferInfo.width, framebufferInfo.height);
  } else {
    gl2.bindFramebuffer(target, null);
...
var prefixRE = /^(.*?)_/;
function addExtensionToContext(gl2, extensionName) {
  glEnumToString(gl2, 0);
  ...
  if (ext) {
    ...
    for (const key in ext) {
      ...
      if (gl2[name] !== void 0) {
        if (!isFunc && gl2[name] !== value) {
          warn$1(name, gl2[name], value, key);
        }
      } else {
        if (isFunc) {
          gl2[name] = /* @__PURE__ */ (function(origFn) {
            return function() {
...
function addExtensionsToContext(gl2) {
  for (let ii = 0; ii < supportedExtensions.length; ++ii) {
    addExtensionToContext(gl2, supportedExtensions[ii]);
  }
}
...
function getWebGLContext(canvas, opt_attribs) {
  const gl2 = create3DContext(canvas, opt_attribs);
  return gl2;
}
...
function getContext(canvas, opt_attribs) {
  const gl2 = createContext(canvas, opt_attribs);
  return gl2;
}
...
twgl.js/dist/7.x/twgl-full.module.js:
