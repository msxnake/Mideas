...
import { 
  EditorType, ProjectAsset, Tile, Sprite, Msx2Sprite, Msx2Bitmap, ScreenMap, MSXColorValue, SpriteFrame, PixelData,
...
export const AppUI: React.FC<AppUIProps> = (props) => {
  ...
  */
  ...
  const dataAssets = assets.filter(a =>
    ['tile', 'sprite', 'msx2sprite', 'msx2bitmap', 'screenmap', 'sound', 'track', 'worldmap'].includes(a.type)
  );
  ...
  const renderRightPanelContent = () => {
    ...
    const paletteEditors = [EditorType.Tile, EditorType.Sprite, EditorType.Msx2Sprite, EditorType.Msx2Bitmap, EditorType.Screen, EditorType.Font, EditorType.Boss];
    if (paletteEditors.includes(currentEditor) || (currentEditor === EditorType.None && selectedAssetId === null) ) {
      ...
      const usesScreen2Palette = currentScreenMode === "SCREEN 2 (Graphics I)" && !isMsx2SpriteEditor && !isMsx2BitmapEditor;
