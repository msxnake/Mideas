...
export function normalizeGlobalVariableName(rawName) {
    const trimmedName = typeof rawName === 'string' ? rawName.trim() : '';
    if (!trimmedName)
        return '';
    const builtInVariable = MIDEAS_GLOBAL_VARIABLES.find(variable => variable.name.toLowerCase() === trimmedName.toLowerCase());
    return builtInVariable ? builtInVariable.name : trimmedName;
}
export function buildGlobalVariableAsmName(variableName) {
...
export function buildGlobalVariableConstantPrefix(variableName) {
    const normalizedName = normalizeGlobalVariableName(variableName);
