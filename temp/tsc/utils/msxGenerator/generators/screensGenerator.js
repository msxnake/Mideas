...
import { DEFAULT_TILE_BANK_DEFINITIONS, EDITOR_BASE_TILE_DIM_S2 } from '../../../constants';
import { normalizeEffectZoneParams, resolveEffectZoneType } from '../../../types';
...
import { buildReferencedScreen2TileBanks, getScreen2TileBankIdLabel, getScreen2TileBankColorLoaderLabel, getScreen2TileBankPatternLoaderLabel, resolveRuntimeScreen2TileBankCharCode, resolveRuntimeScreen2TileBankDefinitions, } from '../utils/screen2TileBanks';
...
import { buildMapperBankEqu, buildMapperWindowedAddress, getMapperWindowConfig, } from './mapperWindowUtils';
...
function buildLayerLayoutBytes(screen, layerName, analysis, tileBankDefinitions) {
    ...
    return Array.from(generateScreenMapLayoutBytes(exportScreen, analysis.tiles || [], tileBankDefinitions, 'SCREEN 2 (Graphics I)'));
}
...
`;
    ...
    const hlExpr = (label) => useBank4DataGroup ? buildMapperWindowedAddress(label, mapperWindow) : label;
 ...
 */
export function getPresentationScreenBank4Data(analysis) {
    if (!hasPresentationScreenData(analysis))
        return '';
    const config = analysis.presentationScreen;
    let asm = '';
...
export function buildScreenResourcePolicyManifest(analysis) {
    const screens = Array.isArray(analysis.screenMaps) ? analysis.screenMaps : [];
    const screenSpriteUsage = new Map(buildScreenSpritePatternUsageSummaries(analysis).map((summary) => [summary.screenId, summary.totalSlotsRequired]));
 ...
 */
export function generateScreensFile(analysis, romMode = 'simple32k', dataInBank4 = false, targetFormat = 'konami') {
    const usesMapper = usesMapperBanking(romMode);
    const useResourceManager = romMode === 'megarom';
    const mapperWindow = getMapperWindowConfig(romMode, targetFormat);
    const mapperAddr = (label) => usesMapper ? buildMapperWindowedAddress(label, mapperWindow) : label;
            ...
            if (screen.layers && screen.layers.background) {
                if (dataInBank4) {
                    ...
                    if (backgroundBlockMap) {
                        ...
                        const asmCode = generateScreenLayoutASMCode(screenNameWithIndex, SCREEN_WIDTH, SCREEN_HEIGHT, backgroundLayoutBytes, referenceComments, 'hex');
                        code += asmCode;
                        code += `\n`;
                    }
                    ...
                    if (false) {
                        ...
                        if (analysis.tiles && analysis.tiles.length > 0) {
                            ...
                            const baseDef = DEFAULT_TILE_BANK_DEFINITIONS[1];
                            const globalBankDef = {
                                ...baseDef,
                                assignedTiles: {},
                        ...
                        const asmCode = generateScreenLayoutASMCode(screenNameWithIndex, SCREEN_WIDTH, SCREEN_HEIGHT, mapIndices, referenceComments, 'hex');
                    ...
                    if (screen.layers.collision && analysis.tiles) {
                        ...
                        const behaviorASM = generateBehaviorMapASMCode(screenNameWithIndex, SCREEN_WIDTH, SCREEN_HEIGHT, behaviorMapData, 'hex');
                        code += `\n${behaviorASM}`;
                    }
...
`
            ...
            const importedHudFrameCells = (screen.hudConfiguration?.importedFrame?.cells || [])
                ...
                y: cell.y | 0,
                charCode: (() => {
                    const runtimeCharCode = resolveRuntimeScreen2TileBankCharCode(analysis, screen.hudConfiguration?.importedFrame?.sourceTileBankAssetId, cell.tileId, cell.y | 0, cell.subTileX | 0, cell.subTileY | 0);
 ...
 */
export function getScreensBank4Data(analysis, romMode = 'simple32k') {
    if (!analysis.screenMaps || analysis.screenMaps.length === 0) {
    ...
    screenExports.forEach((screenExport) => {
        ...
        if (screen.layers && screen.layers.background) {
            if (backgroundBlockMap) {
                ...
                const asmCode = generateScreenLayoutASMCode(screenNameWithIndex, SCREEN_WIDTH, SCREEN_HEIGHT, backgroundLayoutBytes, [], 'hex');
                asm += asmCode;
                asm += `\n`;
            }
            // Effects layout
            ...
            if (screen.layers.collision && analysis.tiles) {
                ...
                asm += generateBehaviorMapASMCode(screenNameWithIndex, SCREEN_WIDTH, SCREEN_HEIGHT, behaviorMapData, 'hex');
                asm += `\n`;
            }
            else {
