 ...
 */
import { ActionTypes, ConditionTypes } from '../../../statemachine.types';
...
function applyConditionalHandlers(asm, usedActions, usedConditions) {
    ...
    const needsVarHelpers = hasAction(ActionTypes.SET_VARIABLE, ActionTypes.INCREMENT_VARIABLE, ActionTypes.DECREMENT_VARIABLE, ActionTypes.SET_COMPONENT_PROPERTY, ActionTypes.ADD_VARIABLES, ActionTypes.SUBTRACT_VARIABLES, ActionTypes.MULTIPLY_VARIABLES, ActionTypes.DIVIDE_VARIABLES, ActionTypes.MODULO_VARIABLES, ActionTypes.ASSIGN_VARIABLE);
    if (!needsVarHelpers) {
    ...
    const needsMatchDir = hasCond(ConditionTypes.KEY_PRESSED, ConditionTypes.KEY_RELEASED, ConditionTypes.CAN_MOVE_DIRECTION, ConditionTypes.KEY_AND_MOVEMENT);
    if (!needsMatchDir) {
 ...
 */
export function generateStateMachineSystem(stateMachines, globalVariables, sprites, tiles, templates, sounds, trackIndexByAssetId, romMode = 'simple32k') {
...
`);
    ...
    const spriteCatalog = buildMSXDirectionalSpriteCatalog((sprites || []));
