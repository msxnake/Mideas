...
import { buildMapperBankEqu, buildMapperDataPopAsm, buildMapperWindowedAddress, getMapperWindowConfig, } from './mapperWindowUtils';
...
export const buildRuntimeSpritePatternPacks = (analysis) => {
...
export const buildScreenSpritePatternUsageSummaries = (analysis) => {
    const buildPack = createRuntimeSpritePatternPackBuilder(analysis);
...
export function buildWorldSpritePatternPolicyManifest(analysis) {
    const spriteCatalog = buildMSXDirectionalSpriteCatalog(analysis.sprites || []);
 ...
 */
export function generateSpritesFile(analysis, romMode = 'simple32k', dataInBank4 = false, targetFormat = 'konami') {
...
export function getSpritesBank4Data(analysis) {
    const sourceSprites = analysis.sprites || [];
    const spriteCatalog = buildMSXDirectionalSpriteCatalog(sourceSprites);
