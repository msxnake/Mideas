 ...
 */
export const generateTileASMCode = (tile, tileName, dataFormat) => {
 ...
 */
export const createDefaultLineAttributes = (tileWidth, tileHeight, fgColor, bgColor) => {
 ...
 */
export const generateTilePatternBytes = (tile, currentScreenMode) => {
    const bytes = [];
    const numCharBlocksX = tile.width / EDITOR_BASE_TILE_DIM_S2;
    const numCharBlocksY = tile.height / EDITOR_BASE_TILE_DIM_S2;
 ...
 */
export const shiftTileDataUp = (tileData, lineAttributes, screenMode) => {
 ...
 * @returns The shifted pixel data.
 */
export const shiftTileDataDown = (tileData, lineAttributes, screenMode) => {
    const height = tileData.length;
    if (height < 2)
        return tileData;
 ...
 * @returns The shifted pixel data.
 */
export const shiftTileDataLeft = (tileData, lineAttributes, screenMode) => {
 ...
 * @returns The shifted pixel data.
 */
export const shiftTileDataRight = (tileData, lineAttributes, screenMode) => {
    if (tileData.length === 0)
        return [];
    const rolledData = tileData.map(row => {
 ...
 */
export const mirrorTileDataHorizontal = (tileData, lineAttributes, screenMode) => {
    if (tileData.length === 0)
        return [];
 ...
 * @returns The mirrored pixel data.
 */
export const mirrorTileDataVertical = (tileData, lineAttributes, screenMode) => {
    if (tileData.length === 0)
        return [];
 ...
 */
export const generateTileColorBytes = (tile) => {
    if (!tile.lineAttributes)
        return null;
    const bytes = [];
    const numCharBlocksX = tile.width / EDITOR_BASE_TILE_DIM_S2;
