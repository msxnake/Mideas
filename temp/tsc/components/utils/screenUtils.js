import { EDITOR_BASE_TILE_DIM_S2, EMPTY_CELL_CHAR_CODE as CONST_EMPTY_CELL_CHAR_CODE, SCREEN2_PIXELS_PER_COLOR_SEGMENT } from '../../constants';
 ...
 */
export const generateScreenMapLayoutBytes = (screenMap, tileset, tileBanks, currentScreenMode) => {
                    ...
                    for (const bank of tileBanks) {
                        ...
                        if ((bank.enabled ?? true) && bank.assignedTiles[screenTile.tileId]) {
                            ...
                            if (tileAsset) {
                                ...
                                actualCharCodeForCell = fontChar?.bankCharCode ?? CONST_EMPTY_CELL_CHAR_CODE;
                            }
                            else {
 ...
 */
export const generateScreenLayoutASMCode = (mapName, mapWidth, mapHeight, mapIndices, referenceComments, dataFormat = 'hex') => {
...
export const generateScreenLayoutExportASMCode = ({ mapName, mapWidth, mapHeight, mapIndices, referenceComments, dataFormat, exportMode = 'raw', blockData, }) => {
...
export const generateScreenLayoutExportBinary = ({ mapIndices, exportMode = 'raw', blockData, }) => {
    if (exportMode === 'raw' || !blockData) {
        return new Uint8Array(mapIndices);
    }
    const header = [
 ...
 * @returns A string containing the generated assembly code.
 */
export const generateBehaviorMapASMCode = (mapName, mapWidth, mapHeight, behaviorMapData, dataFormat = 'hex') => {
    const ASM_BYTES_PER_LINE = 16;
    const safeMapName = mapName.replace(/[^a-zA-Z0-9_]/g, '_').toUpperCase();
 ...
 */
export const generateBehaviorMapData = (screenMapData, tileset) => {
 ...
 */
export const generateOptimizedRLEData = (uncompressedBytes) => {
 ...
 */
export const generateSuperRLEData = (backgroundLayer, tileset, baseTileDim, tileBanks, compressionType = 'superRLE') => {
    ...
    const getByteForTilePart = (tileId, subTileX, subTileY) => {
        ...
        if (tileBanks && baseTileDim === EDITOR_BASE_TILE_DIM_S2) {
            let foundInBank = false;
            for (const bank of tileBanks) {
                ...
                if (isBankEffectivelyEnabled && bank.assignedTiles[tileId]) {
                    ...
                    const calculatedCharCode = tileAsset
                        ...
                        : (Array.isArray(assignment.fontCharacters) ? (assignment.fontCharacters[sX]?.bankCharCode ?? CONST_EMPTY_CELL_CHAR_CODE) : CONST_EMPTY_CELL_CHAR_CODE);
 ...
 */
export const deepCompareTiles = (tile1, tile2) => {
 ...
 */
export function createTileDataURL(fullTileAsset, subTileXCoord, subTileYCoord, outputCellPixelWidth, outputCellPixelHeight, baseSliceDim, currentScreenMode) {
 ...
 */
export function createSpriteDataURL(spriteFrameData, spriteWidth, spriteHeight) {
 ...
 */
export const renderScreenToCanvas = (canvas, screenMap, tileset, currentScreenMode, baseSliceDim, tileBanks, allAssets) => {
