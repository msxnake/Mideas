...
; perception skill (SCREEN 5 bitmap): 1 hidden_obj marker(s), radius 40px.
bitmap_flag_near EQU #C0E7
    org #4000
...
init_rom:
    ...
    ; perception: no hidden object near at boot.
    xor a
...
.skip_player_movement:
    call bitmap_update_perception    ; perception: hidden_obj proximity -> flag_near
...
; ------------------------------------------------------------
; PURPOSE:
;   Resolve the current room's hidden_obj record table.
;
; OUTPUT:
;   HL = first record (centerX, centerY pairs), B = record count.
;   Z set (and B=0) when the room has no hidden objects.
;
...
; ------------------------------------------------------------
; PURPOSE:
;   Per-frame Perception scan: measure the player center against every
;   hidden_obj center of the current room. Within the 40px square
;   radius -> bitmap_flag_near = 1 and (when no action skill owns the
;   animation state) player_anim_state = 1 (perceiving).
;
; INPUT:
...
;
; OUTPUT:
;   bitmap_flag_near = 1 while any hidden_obj is within radius, else 0
;   (recomputed every frame; also 0 during room transitions).
;
; DESTROYS: AF, BC, DE, HL.  PRESERVES: IX, IY.
;
; NOTES:
;   |dx|/|dy| are exact 8-bit absolute differences (sub + conditional neg),
;   so the test is wrap-safe across the whole 0..255 pixel range.
; ------------------------------------------------------------
...
.percept_dy_abs:
    cp 41
    jp nc, .percept_next
    ; Near: latch the flag for this frame and stop scanning.
    ld a, 1
    ld (bitmap_flag_near), a
    ld a, (player_anim_state)
    or a
    ret nz                   ; an action skill owns the animation this frame
    ld a, 1
    ld (player_anim_state), a
    ret
.percept_next:
...
; Room 0 hidden_obj records: centerX, centerY
bitmap_percept_room_0:
    DB #88,#98
...
; Sprite 0 line color table (mode 2): configured player sprite "Smoke Player Sprite" + 1 state clip(s)
...
; Sprite 0 pattern (16x16, mode 2 quadrants): configured player sprite "Smoke Player Sprite" + 1 state clip(s)
