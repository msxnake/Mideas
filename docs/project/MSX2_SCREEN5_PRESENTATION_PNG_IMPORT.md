...
- `--with-msx2-gameflow` makes the generated project open in the MSX2 GameFlow editor and validates through `MSX2_GAMEFLOW_*` ASM markers in smoke tests. The current SCREEN 5 backend supports the linear intro path `Start -> optional Waypoint nodes -> Screen5Presentation -> optional Waypoint nodes -> optional terminal Transition -> End/Restart`; broader GameFlow branching remains outside this backend.
