...
- `--with-msx2-gameflow` makes the generated project open in the MSX2 GameFlow editor and validates through `MSX2_GAMEFLOW_*` ASM markers in smoke tests. The current SCREEN 5 backend supports the linear intro path `Start -> optional Waypoint/Globals nodes -> Screen5Presentation -> optional Waypoint/Globals nodes -> optional terminal Transition -> optional Waypoint/Globals nodes -> End/Restart`; terminal transitions support `cls`, `fade_to_black`, `screen5_vertical_pixel_wipe`, `screen5_horizontal_pixel_wipe`, `screen5_diagonal_pixel_wipe`, and `screen5_mirror_pixel_wipe`. Broader GameFlow branching remains outside this backend.
