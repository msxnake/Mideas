...
- `*_project.json`: Mideas project with one `msx2presentation` asset. With `--with-msx2-gameflow`, it also includes a `Main MSX2` flow wired as `Start -> Screen5Presentation -> End`.
...
- `--with-msx2-gameflow` makes the generated project open in the MSX2 GameFlow editor and validates through `MSX2_GAMEFLOW_*` ASM markers in smoke tests.
