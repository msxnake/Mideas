 ...
 * Run with: node test_generator_fixes.mjs
 */
...
// Import the generator (using dynamic import for TypeScript)
async function testGenerator() {
  ...
  try {
    // Import the generator (TypeScript module)
    const generatorModule = await import('./utils/msxGenerator/index.ts');
    const { generateModularASM } = generatorModule;
    ...
    const spriteCountMatch = unifiedASM.match(/ld bc, (\d+)\s+; 4 bytes per sprite/);
    const spriteCount = spriteCountMatch ? parseInt(spriteCountMatch[1]) / 4 : 0;
    ...
    const hasGlobalsCase = unifiedASM.includes('GameFlow: Start → Globals') ||
                           unifiedASM.includes('case \'Globals\'');
    ...
    console.log(`\n${allPassed ? '✅ ALL CRITICAL FIXES VERIFIED!' : '⚠️  SOME FIXES NEED ATTENTION'}`);

  } catch (err) {
    console.error('❌ Import error:', err.message);
    console.error('\nThis might be because TypeScript needs to be transpiled.');
    console.error('Please regenerate from Mideas UI after restarting the dev server.');
  }
}
