...
exports.generateMsx2Screen5PresentationFiles = generateMsx2Screen5PresentationFiles;
...
const clampLevel = (value, fallback = 0) => {
    const numeric = Number(value);
    if (!Number.isFinite(numeric))
        return fallback;
...
function getNextFlowNode(flow, node, sourceId) {
    if (!node)
        return undefined;
...
function getNextExportNode(flow, node) {
...
function collectNextExportStep(flow, node, sourceId) {
    let nextNode = getNextFlowNode(flow, node, sourceId);
    const visited = new Set();
    const globals = [];
    while ((nextNode?.type === 'Waypoint' || nextNode?.type === 'Globals') && !visited.has(nextNode.id)) {
        visited.add(nextNode.id);
        if (nextNode.type === 'Globals')
            globals.push(nextNode);
        nextNode = getNextFlowNode(flow, nextNode);
    }
    return { node: nextNode, globals };
}
function resolveTerminalStep(flow, node, sourceId, visited = new Set()) {
    ...
    if (nextNode.type === 'End')
        return { terminalAction: 'loop', globalsBeforeText: [], globalsAfterPreTextTransition: [], globals: step.globals };
    if (nextNode.type === 'Restart')
        return { terminalAction: 'restart', globalsBeforeText: [], globalsAfterPreTextTransition: [], globals: step.globals };
    ...
    if (nodeAfterTransition?.type === 'Text') {
        ...
        return {
            ...afterTextStep,
            text: nodeAfterTransition,
            preTextTransition: nextNode,
            globalsBeforeText: step.globals,
...
function collectInitialGlobalsNodes(flow, presentationNode) {
    if (!flow || !presentationNode)
        return [];
...
function resolveMsx2GameFlowPresentationNode(flow) {
    if (!flow || !Array.isArray(flow.nodes))
        return undefined;
    const startId = flow.startNodeId || flow.nodes.find(node => node.type === 'Start')?.id;
    const startNode = getNodeById(flow, startId);
    const nextNode = getNextExportNode(flow, startNode);
    return nextNode?.type === 'Screen5Presentation' ? nextNode : undefined;
}
function getScreen4RuntimeGameFlow(analysis) {
    return (analysis.msx2GameFlows || [])
        .find(candidate => candidate?.purpose === 'screen4-runtime');
}
function resolvePresentationChain(analysis, minimumScenes = 2) {
    ...
    while (current && !visited.has(current.id)) {
        ...
        const presentation = requestedPresentationAssetId
            ? presentations.find(item => item.id === requestedPresentationAssetId)
            : presentations[0];
        if (!presentation) {
        ...
        if (afterTransition?.type !== 'Screen5Presentation')
            return undefined;
        scene.transitionToNext = nextNode;
        scenes.push(scene);
...
function resolveNextExportStep(flow, node) {
...
function resolvePresentationFlow(analysis) {
    const presentations = (analysis.msx2Presentations || []);
    const flows = (analysis.msx2GameFlows || []);
    ...
    if (requestedPresentationAssetId && !presentation) {
        throw new Error(`MSX2 GameFlow Screen5Presentation node "${node?.id || 'unknown'}" references missing msx2presentation asset "${requestedPresentationAssetId}".`);
    }
...
function resolvePaletteSlot(slot) {
    ...
    const fromHex = parseHexColor(slot?.hex);
    if (fromHex)
        return fromHex.map(value => clampLevel(value));
    return [0, 0, 0];
}
...
function resolveTextPaletteIndex(palette) {
    const source = Array.isArray(palette) ? palette : [];
    let bestSlot = 15;
...
function buildBitmapBytes(presentation) {
...
function buildGlobalAsmName(variableName) {
    return `global_var_${variableName.replace(/([A-Z])/g, '_$1').toLowerCase().replace(/^_/, '').replace(/[^a-z0-9_]/g, '_')}`;
}
function resolveGlobalVariableInfo(variableName, analysis) {
...
function parseGlobalAssignmentValue(rawValue, values, isWord) {
...
function resolveInitialGlobalAssignments(globalsNodes, analysis) {
...
function resolveConditionAsGlobalAssignment(node, analysis) {
    if (!node)
        return undefined;
...
function generateGlobalVariableEquates(assignments) {
    const seen = new Map();
    assignments.forEach(assignment => {
        if (!seen.has(assignment.asmName))
            seen.set(assignment.asmName, assignment);
    });
    let address = 0xC000;
...
function generateGlobalsRoutine(label, assignments, comment) {
    const lines = [
        `${label}:`,
    ];
...
function generateAfterPresentationRoutine(nextLabel, hasAssignments, shouldEmit) {
    if (!shouldEmit)
        return '';
...
function generateIfThenElseRoutine(node, condition) {
    if (!node || !condition)
        return '';
...
function generateCompareRoutine(needed) {
    if (!needed)
        return '';
    return `
...
function sanitizeScreen5TextLine(value, maxLength) {
    return String(value || '')
        .toUpperCase()
        .replace(/[^\x20-\x7E]/g, ' ')
        .replace(/"/g, "'")
...
function wrapScreen5Text(value, maxLineLength = 28, maxLines = 8) {
...
function chunkBitmapBytes(bytes, chunkLines) {
...
function generateWaitStep(presentation, nextLabel) {
...
function generateWaitLoop(presentation, nextLabel) {
    if (nextLabel) {
...
function generateSceneWaitStep(label, presentation, nextLabel) {
    const runtime = presentation.runtime;
    if (runtime.waitForKey !== false) {
...
function renderScreen5TextBlock(text, colorIndex, width = 160, height = 9) {
...
function screen5TextBlockDb(label, text, colorIndex) {
...
function screen5TextBlockCall(label, x, y, vramBase, width = 160, height = 9) {
...
function generateScreen5TextRoutine(label, node, nextActionAsm, vramBase, textColorIndex) {
    if (!node)
        return '';
...
function generateScreen5TextHelpers(needed) {
    if (!needed)
        return '';
    return '';
}
function generateTransitionRuntime(label, transition, vramBase, afterTransitionAction, beforeActionAsm = '') {
...
function getScreen5TransitionEffectBlock(transition, _vramBase) {
...
function generatePreTextTransitionRuntime(label, transition, vramBase, textLabel, beforeActionAsm = '', afterActionAsm = '') {
    const effectBlock = getScreen5TransitionEffectBlock(transition, vramBase);
    const durationFrames = Math.max(0, Math.min(255, Math.trunc(Number(transition.durationFrames) || 0)));
    const waitBlock = durationFrames > 0
        ? `    ld b, ${hexByte(durationFrames)}
...
function generateTransitionToLabelRuntime(label, transition, vramBase, nextLabel) {
    const effectBlock = getScreen5TransitionEffectBlock(transition, vramBase);
    const durationFrames = Math.max(0, Math.min(255, Math.trunc(Number(transition.durationFrames) || 0)));
    const waitBlock = durationFrames > 0
        ? `    ld b, ${hexByte(durationFrames)}
...
function generateTerminalActionRoutine(label, step, globalsLabel, globalsBeforeTextLabel, vramBase, textColorIndex) {
    if (!step)
        return '';
...
`
        ...
        return `${preTextRoutine}${generateScreen5TextRoutine(textLabel, step.text, textAction, vramBaseAddress, textColorIndex)}${step.transition
...
function generateTerminalTransitionRoutine(transition, vramBase, afterTransitionAction, hasAfterTransitionGlobals) {
    if (!transition)
        return '';
    return generateTransitionRuntime('msx2_gameflow_run_transition', transition, vramBase, afterTransitionAction, hasAfterTransitionGlobals ? '    call msx2_gameflow_apply_after_transition_globals\n' : '');
}
function generateScreen5DiagonalPixelWipeTable() {
    const entries = [];
    ...
    for (let diagonal = 0; diagonal <= rowBlocks + columnBlocks - 2; diagonal++) {
        for (let rowBlock = 0; rowBlock < rowBlocks; rowBlock++) {
            ...
            if (columnBlock < 0 || columnBlock >= columnBlocks)
                continue;
...
function generateTransitionHelpers(needed, vramBase, effects) {
    if (!needed)
        return '';
    const wants = (effect) => !effects || effects.has(effect);
...
function applyGameFlowRuntimeOverrides(presentation, node) {
    if (!node)
        return presentation;
    return {
        ...presentation,
...
function generatePresentationChainUnitedFiles(projectName, flow, scenes, config, options = {}) {
    ...
    const chunkBankByLabel = new Map();
...
function renameScreen4EntryForMixedAsm(screen4Asm) {
...
function generateMixedScreen5ToScreen4UnitedFiles(projectName, flow, scenes, analysis, config) {
    const screen4Asm = renameScreen4EntryForMixedAsm((0, msx2Screen4Generator_1.generateMsx2Screen4UnitedFiles)(projectName, analysis, {
        ...config,
    ...
    const introCodeAsm = chunkDataMatch
        ...
        : introAsm;
    const bankedIntroCodeAsm = introCodeAsm.replace(/    call map_page2_to_cart_primary\n    call init_konami8k_fixed_bank0_banks\n/, '');
...
function generateMsx2Screen5PresentationFiles(projectName, analysis, config) {
