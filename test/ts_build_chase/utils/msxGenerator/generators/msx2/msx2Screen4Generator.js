...
const MSX2_ENEMY_RUNTIME_BYTES = msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN * 7;
...
function getEnemyHardwareSpriteSource(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const entity = screen?.layers?.entities?.find(candidate => (candidate.kind === 'enemy' || candidate.kind === 'hazard') &&
