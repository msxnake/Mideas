...
exports.MSX2_BITMAP_SCREEN_SHAKE_RAM_BYTES = exports.MSX2_BITMAP_POWER_STOMP_RAM_BYTES = void 0;
exports.bitmapPowerStompEnabled = bitmapPowerStompEnabled;
exports.bitmapPowerStompWantsShake = bitmapPowerStompWantsShake;
exports.buildBitmapPowerStompEquates = buildBitmapPowerStompEquates;
exports.buildBitmapPowerStompInitClearAsm = buildBitmapPowerStompInitClearAsm;
exports.buildBitmapPowerStompInputHookAsm = buildBitmapPowerStompInputHookAsm;
exports.buildBitmapPowerStompGravityHookAsm = buildBitmapPowerStompGravityHookAsm;
exports.buildBitmapPowerStompLandClearAsm = buildBitmapPowerStompLandClearAsm;
exports.buildBitmapPowerStompMainLoopCallAsm = buildBitmapPowerStompMainLoopCallAsm;
exports.buildBitmapPowerStompRuntimeAsm = buildBitmapPowerStompRuntimeAsm;
/**
 ...
 */
exports.MSX2_BITMAP_POWER_STOMP_RAM_BYTES = 2;
exports.MSX2_BITMAP_SCREEN_SHAKE_RAM_BYTES = 1;
...
function bitmapPowerStompEnabled(config) {
    return Boolean(config?.enabled);
}
function bitmapPowerStompWantsShake(config) {
    return bitmapPowerStompEnabled(config) && config?.screenShake !== false;
}
function buildBitmapPowerStompEquates(config, ramBase, shakeRamBase) {
    if (!bitmapPowerStompEnabled(config))
        return '';
...
function buildBitmapPowerStompInitClearAsm(config) {
    if (!bitmapPowerStompEnabled(config))
        return '';
...
function buildBitmapPowerStompInputHookAsm(config) {
    if (!bitmapPowerStompEnabled(config))
        return '';
    return `    push bc
...
function buildBitmapPowerStompGravityHookAsm(config) {
    if (!bitmapPowerStompEnabled(config))
        return '';
    return `    call bitmap_step_stomp_fall
`;
}
function buildBitmapPowerStompLandClearAsm(config) {
    if (!bitmapPowerStompEnabled(config))
        return '';
    return `    call bitmap_stomp_on_land
`;
}
function buildBitmapPowerStompMainLoopCallAsm(config) {
    if (!bitmapPowerStompWantsShake(config))
        return '';
    return `    call bitmap_screen_shake_update
`;
}
function buildBitmapPowerStompRuntimeAsm(config) {
    if (!config || !bitmapPowerStompEnabled(config))
        return '';
