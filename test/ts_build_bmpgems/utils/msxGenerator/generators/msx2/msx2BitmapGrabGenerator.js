...
exports.MSX2_BITMAP_GRAB_RAM_BYTES = void 0;
exports.bitmapGrabEnabled = bitmapGrabEnabled;
exports.buildBitmapGrabEquates = buildBitmapGrabEquates;
exports.buildBitmapGrabInitClearAsm = buildBitmapGrabInitClearAsm;
exports.buildBitmapGrabGravityHookAsm = buildBitmapGrabGravityHookAsm;
exports.buildBitmapGrabRuntimeAsm = buildBitmapGrabRuntimeAsm;
exports.buildBitmapGrabGateAsm = buildBitmapGrabGateAsm;
/**
 ...
 */
exports.MSX2_BITMAP_GRAB_RAM_BYTES = 1;
...
function bitmapGrabEnabled(c) { return Boolean(c?.enabled); }
function buildBitmapGrabEquates(c, ramBase) {
    if (!bitmapGrabEnabled(c))
        return '';
    return `; --- GRAB skill runtime state (1 byte) ---
bitmap_grab_active EQU ${asmWord(ramBase)}
`;
}
function buildBitmapGrabInitClearAsm(c) {
    if (!bitmapGrabEnabled(c))
        return '';
    return `    xor a
...
function buildBitmapGrabGravityHookAsm(c) {
    if (!bitmapGrabEnabled(c))
        return '';
...
function buildBitmapGrabRuntimeAsm(c) {
    if (!c || !bitmapGrabEnabled(c))
        return '';
    const slideSpeed = asmByte(Math.max(0, Math.min(4, c.slideSpeed ?? 1)));
    return `
...
function buildBitmapGrabGateAsm(c) {
    if (!bitmapGrabEnabled(c))
        return '';
