...
exports.bitmapShootEnabled = bitmapShootEnabled;
exports.bitmapShootRamBytes = bitmapShootRamBytes;
exports.buildBitmapShootEquates = buildBitmapShootEquates;
exports.buildBitmapShootInitClearAsm = buildBitmapShootInitClearAsm;
exports.buildBitmapShootGateAsm = buildBitmapShootGateAsm;
exports.buildBitmapBulletSatCallAsm = buildBitmapBulletSatCallAsm;
exports.buildBitmapBulletInitUploadAsm = buildBitmapBulletInitUploadAsm;
exports.buildBitmapBulletDataTables = buildBitmapBulletDataTables;
exports.buildBitmapShootRuntimeAsm = buildBitmapShootRuntimeAsm;
/**
...
function bitmapShootEnabled(config) {
    return Boolean(config?.enabled);
}
/** Number of bytes the bullet pool + shared state occupies. */
function bitmapShootRamBytes(config) {
    if (!bitmapShootEnabled(config))
        return 0;
...
function buildBitmapShootEquates(config, ramBase) {
    if (!bitmapShootEnabled(config))
        return '';
    const maxBullets = Math.max(1, Math.min(8, Math.floor(config.maxBullets) || 3));
...
function buildBitmapShootInitClearAsm(config) {
    if (!bitmapShootEnabled(config))
        return '';
    const maxBullets = Math.max(1, Math.min(8, Math.floor(config.maxBullets) || 3));
...
function buildBitmapShootGateAsm(config) {
    if (!bitmapShootEnabled(config))
        return '';
...
function buildBitmapBulletDataTables(config, sprite) {
    if (!bitmapShootEnabled(config))
        return '';
    ...
    const colors = sprite?.colorBytes && sprite.colorBytes.length === 16
        ? sprite.colorBytes
        : Array(16).fill(15);
    const emit = (label, bytes, comment) => {
        const l = [`; ${comment}`, `${label}:`];
...
function buildBitmapShootRuntimeAsm(config, opts) {
    if (!config || !bitmapShootEnabled(config))
        return '';
    const maxBullets = Math.max(1, Math.min(8, Math.floor(config.maxBullets) || 3));
