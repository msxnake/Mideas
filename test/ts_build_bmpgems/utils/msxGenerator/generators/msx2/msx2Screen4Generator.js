...
exports.resolveMsx2SpriteById = resolveMsx2SpriteById;
exports.getMsx2PlayerAssetRecords = getMsx2PlayerAssetRecords;
exports.getFirstReferencedMsx2Sprite = getFirstReferencedMsx2Sprite;
exports.buildHardwareSpriteLayersForFrame = buildHardwareSpriteLayersForFrame;
exports.generateMsx2Screen4UnitedFiles = generateMsx2Screen4UnitedFiles;
exports.generateMsx2Screen4Files = generateMsx2Screen4Files;
...
const msx2EntityRuntimeGenerator_1 = require("./msx2EntityRuntimeGenerator");
const msx2Box2ComponentGenerator_1 = require("./msx2Box2ComponentGenerator");
const msx2Box2RuntimeGenerator_1 = require("./msx2Box2RuntimeGenerator");
const msx2PlayerBulletCharGenerator_1 = require("./msx2PlayerBulletCharGenerator");
const msx2GridSnapComponentGenerator_1 = require("./msx2GridSnapComponentGenerator");
const msx2Screen4TileConstraints_1 = require("../../../msx2Screen4TileConstraints");
const msx2Screen4TileBehavior_1 = require("../../../msx2Screen4TileBehavior");
const msx2CellFlags_1 = require("../../../msx2CellFlags");
const msx2PlatformPhysics_1 = require("../../../msx2PlatformPhysics");
const msx2DashGenerator_1 = require("./msx2DashGenerator");
const msx2AirDashGenerator_1 = require("./msx2AirDashGenerator");
const msx2CollectorGemsGenerator_1 = require("./msx2CollectorGemsGenerator");
...
const MSX2_MAX_PLAYER_HARDWARE_LAYERS = 32 - msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN - MSX2_PLAYER_BULLET_HARDWARE_SLOTS - MSX2_ENEMY_BULLET_HARDWARE_SLOTS - 1;
const MSX2_ENEMY_ANIM_RUNTIME_BYTES = 2;
const MSX2_ENEMY_RUNTIME_BYTES = (msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN * 7) + MSX2_ENEMY_ANIM_RUNTIME_BYTES;
...
/**
 * Effect runtime base = snake-body end (32 cells * 2 bytes) + worst-case skill
 * chain reserved bytes + defensive padding to keep skills from ever touching
 * effect buffers. The reserve must keep MSX2_EFFECT_RUNTIME_BASE at or above
 * MSX2_SKILL_RAM_LIMIT (msx2SkillRamLayout.ts), which is what
...
const formatHexWord = (value) => `#${Math.max(0, Math.min(0xFFFF, Math.floor(value))).toString(16).toUpperCase().padStart(4, '0')}`;
const formatHexByte = (value) => `#${Math.max(0, Math.min(0xFF, Math.floor(value))).toString(16).toUpperCase().padStart(2, '0')}`;
function validateMsx2Screen4RomConfig(config) {
    if (isMsx2Screen4MegaRomRequested(config) && config.targetFormat !== 'konami') {
        throw new Error(`MSX2 SCREEN 4 MegaROM currently supports only Konami 8K fixed-bank0 compatibility; requested ${config.targetFormat}.`);
    }
}
function isMsx2Screen4MegaRomRequested(config) {
    return config.romMode === 'megarom' || (config.romMode === 'auto' && config.autoMegaROM === true);
}
function usesMsx2Screen4KonamiDataBank(config) {
...
function resolveScreen4Palette(analysis) {
...
function buildTileScreenLayerBytes(screen, layerName) {
    ...
    for (let y = 0; y < MSX2_TILE_SCREEN_HEIGHT; y++) {
        for (let x = 0; x < MSX2_TILE_SCREEN_WIDTH; x++) {
            ...
            const behaviorKind = (0, msx2Screen4TileBehavior_1.getMsx2TileBehaviorKind)(tile);
    ...
    if (layerName === 'effects') {
        for (const entity of screen?.layers?.entities || []) {
            ...
            const isPushBox = (0, msx2Box2RuntimeGenerator_1.entityHasMsx2Box2)(entity);
            ...
            if (effect)
                bytes[(y * MSX2_TILE_SCREEN_WIDTH) + x] = effect;
        }
    }
    return bytes;
}
function countTileScreenEffectCode(screen, code) {
    return buildTileScreenLayerBytes(screen, 'effects').filter(value => value === code).length;
}
function buildTileScreenCellFlagBytes(screen) {
    return (0, msx2CellFlags_1.buildMsx2CellFlagBytes)(screen, {
...
function getTileScreenRequiredCollectibles(screen) {
...
function getTileScreenInitialAir(screen) {
...
function getTileScreenHudStyle(screen) {
    const runtime = screen?.runtime;
    return runtime?.hudStyle === 'statusBars' || runtime?.hudStyle === 'vampire' ? 1 : 0;
}
function getTileScreenRuntimeByte(screen, key, fallback, min = 0, max = 255) {
    const runtime = screen?.runtime;
...
const MSX2_HUD_WIDGET_KIND_IDS = {
    bar: 1,
    counter: 2,
    icon: 3,
    text: 4,
};
const MSX2_HUD_WIDGET_BINDING_IDS = {
    playerEnergy: 1,
    bossEnergy: 2,
    air: 3,
    score: 4,
...
function getTileScreenHudWidgets(screen) {
    const widgets = screen?.runtime?.hudWidgets;
    return Array.isArray(widgets) ? widgets.slice(0, 16) : [];
}
function hudWidgetByte(widget, key, fallback, min = 0, max = 255) {
...
function appendHudStringPoolEntry(pool, bytes) {
    if (bytes.length <= 0)
        return 0;
...
function buildTilePatternBytes(tile) {
...
function buildScreen4CharacterBytes(tile, subTileX, subTileY) {
    const pattern = [];
    const color = [];
    ...
    for (let y = 0; y < CELL_SIZE; y++) {
        const row = Array.from({ length: CELL_SIZE }, (_unused, x) => Math.max(0, Math.min(15, Number(pixels[baseY + y]?.[baseX + x]) || 0)));
        const { fg, bg } = (0, msx2Screen4TileConstraints_1.chooseScreen4RowColors)(row);
        let bits = 0;
...
function buildScreen4TilePairBytes(tile) {
    const pattern = [];
    const color = [];
    const tileWidth = getMsx2TilePixelWidth(tile);
    const tileHeight = getMsx2TilePixelHeight(tile);
    ...
    while (pattern.length < 32)
        pattern.push(0);
    while (color.length < 32)
        color.push(0);
    return { pattern: pattern.slice(0, 32), color: color.slice(0, 32) };
}
function getScreen4TileBytesForEntity(screen, entity) {
    ...
    const inferredTileIndex = Number.isFinite(explicitTileIndex)
        ? explicitTileIndex
        : (screen?.tiles || []).findIndex((tile) => {
            const tileToken = String(`${tile?.id || ''} ${tile?.name || ''}`).replace(/[\s_-]+/g, '').toLowerCase();
            if (!tileToken)
                return false;
            if (entityToken.includes('food'))
                return tileToken.includes('food') || tileToken.includes('apple');
...
function buildScreen4ScreenData(screen) {
    ...
    const bankCharByKey = Array.from({ length: 3 }, () => new Map());
    const bankDefs = Array.from({ length: 3 }, () => []);
    const names = Array(SCREEN4_NAME_BYTES).fill(0);
    const claimedCharBlocks = [];
    const reserveBaseForCharBlock = (requestedBase) => {
        ...
        while (claimedCharBlocks.some(existing => existing !== base && Math.abs(existing - base) < 4)) {
            ...
            if (base > 252)
                base = 1;
            if (claimedCharBlocks.length >= 63)
                break;
        }
        if (!claimedCharBlocks.includes(base))
            claimedCharBlocks.push(base);
        return base;
    };
    const reserveCharForAllBanks = (charCode, pattern, color) => {
    ...
    const reserveCharBlockForAllBanks = (baseCharCode, pattern, color) => {
        if (baseCharCode < 0 || baseCharCode > 252)
            return;
        reserveCharBlockForBanks([0, 1, 2], baseCharCode, pattern, color);
    };
    const reserveCharBlockForBanks = (banks, baseCharCode, pattern, color) => {
        if (baseCharCode < 0 || baseCharCode > 252)
            return;
    ...
    const charForBytes = (bank, pattern, color) => {
    ...
    const mapBoxTileIndexes = new Set();
    ...
    for (let tileY = 0; tileY < MSX2_TILE_SCREEN_HEIGHT; tileY++) {
        ...
        for (let tileX = 0; tileX < MSX2_TILE_SCREEN_WIDTH; tileX++) {
            ...
            if (reservedBase !== undefined) {
                ...
                charCodes = Array.from({ length: 4 }, (_unused, quadrant) => charForBytes(bank, bytes.pattern.slice(quadrant * 8, quadrant * 8 + 8), bytes.color.slice(quadrant * 8, quadrant * 8 + 8)));
            }
            const nameX = tileX * 2;
            const nameY = tileY * 2;
    ...
    for (const entity of screen?.layers?.entities || []) {
        if ((0, msx2Box2RuntimeGenerator_1.entityHasMsx2Box2)(entity)) {
        ...
        const { charWidth, charHeight } = (0, msx2GridSnapComponentGenerator_1.getMsx2CharStampDimensions)(entity, getMsx2TilePixelWidth(stampTile), getMsx2TilePixelHeight(stampTile));
...
function buildMsx2Box2NameLayout(screen) {
    const painted = buildScreen4ScreenData(screen);
    const readNameQuad = (names, tileX, tileY) => {
        const nameX = tileX * 2;
        const nameY = tileY * 2;
        return [
...
function getCollectibleErasePaletteIndex(screens) {
    ...
    screens.forEach(screen => {
        ...
        effects.forEach((effect, offset) => {
            ...
            const pixels = tile?.pixels || tile?.data || [];
...
function buildTileScreenTileBlockParts(label, screen) {
...
function buildTileScreenTileBlocks(label, screen) {
    return buildTileScreenTileBlockParts(label, screen).map(block => block.asm).join('\n');
}
function buildTileScreenLoadRoutine(label, screen, screenIndex, loadRuntimeLayerPointers, afterPatternLoad = '', afterNameLoad = '', useKonamiDataBank = false, dataBankConstant = 'MSX2_SCREEN4_DATA_BANK', payloadDataBankConstants = undefined) {
    const data = buildScreen4ScreenData(screen);
    const payloadCopy = (payloadLabel, body) => {
        if (!useKonamiDataBank)
            return body;
...
function buildTileScreenCollectedVisualsRoutine(label, screen, screenIndex, effectRuntimeBase) {
    const effects = buildTileScreenLayerBytes(screen, 'effects');
...
function buildInitEffectBuffersRoutine(tileScreenLoadLabels, effectRuntimeBase, useKonamiDataBank = false, payloadDataBankConstants = undefined) {
...
function estimateMsx2RuntimeRamEnd(tileScreenCount, hazardHitboxCacheSize = 256) {
    const layerSize = MSX2_TILE_SCREEN_WIDTH * MSX2_TILE_SCREEN_HEIGHT;
...
function maxPersistentMsx2ScreenCount() {
    let count = 0;
...
function buildMsx2RamBudget(tileScreens, runtimeRamEnd, shooterBudget, hazardHitboxCacheSize = 256) {
    const layerSize = MSX2_TILE_SCREEN_WIDTH * MSX2_TILE_SCREEN_HEIGHT;
    ...
    const sections = [
        ...
        },
        {
            ...
            mutable: true,
            slots: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN,
            ...(shooterBudget ? {
...
function formatWords(label, words, comment) {
...
function getMsx2HudFontAsset(analysis) {
    const assets = analysis.assets;
    return assets?.find(asset => asset?.type === 'msx2hudfont')?.data;
}
function getMsx2HudFontCharacters(analysis) {
...
function getMsx2HudFontBaseChar(analysis) {
    const asset = getMsx2HudFontAsset(analysis);
...
function isMsx2HudFontContiguousAscii(analysis) {
    const characters = getMsx2HudFontCharacters(analysis);
    return characters.length > 0 && Array.from(characters).every((char, index) => char.charCodeAt(0) === MSX2_ZX_FONT_ASCII_FIRST + index);
}
function buildMsx2HudFontPatternBytes(analysis) {
    const asset = getMsx2HudFontAsset(analysis);
    const characters = getMsx2HudFontCharacters(analysis);
...
function buildMsx2HudFontColorBytes(analysis) {
    const asset = getMsx2HudFontAsset(analysis);
...
function normalizeMsx2HudText(value, maxLength = 31, allowedCharacters = MSX2_HUD_FONT_CHARS) {
...
function getMsx2HudCounterText(widget) {
...
function getMsx2HudWidgetText(widget, allowedCharacters = MSX2_HUD_FONT_CHARS) {
    const kind = String(widget?.kind || '').trim();
    if (kind === 'text')
        return normalizeMsx2HudText(String(widget?.text || widget?.name || ''), Math.max(1, Math.ceil((Number(widget?.width) || 64) / 8)), allowedCharacters);
    if (kind === 'counter')
        return normalizeMsx2HudText(getMsx2HudCounterText(widget), 31, allowedCharacters);
    return '';
}
function buildMsx2HudTextRoutines(label, screen, analysis) {
    ...
    call draw_msx2_hud_string`).join('\n')
        : '    ret';
    const strings = drawable.map(entry => formatBytes(`${label}_HUD_TEXT_${entry.index}`, [...Array.from(entry.text).map(char => char.charCodeAt(0) & 0xff), 0], `${screen?.name || label} HUD text "${entry.text}"`)).join('');
...
function buildMsx2SubMenuTextData(label, node, analysis) {
    const allowedCharacters = getMsx2HudFontCharacters(analysis);
    const title = normalizeMsx2HudText(String(node?.title || 'MENU'), 20, allowedCharacters);
    const options = (Array.isArray(node?.options) ? node.options : [])
        .slice(0, 6)
        .map((option) => normalizeMsx2HudText(String(option?.text || ''), 18, allowedCharacters));
    const rows = [title, ...options].map((text, index) => formatBytes(`${label}_TEXT_${index}`, [...Array.from(text).map(char => String(char).charCodeAt(0) & 0xff), 0], `MSX2 SCREEN 4 SubMenu text "${text}"`));
...
function wrapMsx2Screen4Text(value, maxChars = 24, maxLines = 5) {
...
function buildMsx2TextNodeData(label, node, analysis) {
    const allowedCharacters = getMsx2HudFontCharacters(analysis);
    ...
    const rows = [title, ...messageLines, ...(prompt ? [prompt] : [])].map((text, index) => formatBytes(`${label}_TEXT_${index}`, [...Array.from(text).map(char => String(char).charCodeAt(0) & 0xff), 0], `MSX2 SCREEN 4 Text node text "${text}"`));
...
function buildMsx2TextScrollNodeData(label, node, analysis) {
    const allowedCharacters = getMsx2HudFontCharacters(analysis);
    ...
    const prompt = node?.waitForKey === false ? '' : normalizeMsx2HudText('PRESS KEY', 12, allowedCharacters);
    const rows = [title, ...textLines, ...(prompt ? [prompt] : [])].map((text, index) => formatBytes(`${label}_TEXT_${index}`, [...Array.from(text).map(char => String(char).charCodeAt(0) & 0xff), 0], `MSX2 SCREEN 4 TextScroll panel text "${text}"`));
...
function getMsx2Screen4ColorNibble(value, fallback) {
    return Math.max(0, Math.min(15, Math.trunc(Number(value) || fallback)));
}
function buildMsx2TextScrollColorNodeData(label, node, analysis) {
    const allowedCharacters = getMsx2HudFontCharacters(analysis);
    const title = normalizeMsx2HudText(String(node?.title || 'TEXT SCROLL COLOR'), 22, allowedCharacters);
    const textLines = wrapMsx2Screen4Text(node?.text || '', 26, 8)
        .map(line => normalizeMsx2HudText(line, 26, allowedCharacters));
    const prompt = node?.waitForKey === false ? '' : normalizeMsx2HudText('PRESS KEY', 12, allowedCharacters);
    const rows = [title, ...textLines, ...(prompt ? [prompt] : [])].map((text, index) => formatBytes(`${label}_TEXT_${index}`, [...Array.from(text).map(char => String(char).charCodeAt(0) & 0xff), 0], `MSX2 SCREEN 4 TextScrollColor panel text "${text}"`));
    const textDraws = textLines.map((_line, index) => {
        const vram = 0x1943 + ((index + 1) * SCREEN4_CHAR_COLUMNS);
        return `    ld hl, #${vram.toString(16).toUpperCase().padStart(4, '0')}
    ld de, ${label}_TEXT_${index + 1}
...
function buildMsx2ControlsTextData(label, node, analysis) {
    const allowedCharacters = getMsx2HudFontCharacters(analysis);
    const keyForButton = (button) => String(button || '').toLowerCase() === 'button2'
    ...
    const prompt = node?.waitForKey === false ? '' : normalizeMsx2HudText('PRESS KEY', 12, allowedCharacters);
    const rows = [title, button1, button2, action1, action2, ...(prompt ? [prompt] : [])].map((text, index) => formatBytes(`${label}_TEXT_${index}`, [...Array.from(text).map(char => String(char).charCodeAt(0) & 0xff), 0], `MSX2 SCREEN 4 Controls text "${text}"`));
...
function getMsx2PlayerAssetPushBox(player) {
    const pushBox = player?.components?.msx2_push_box;
    if (!pushBox || pushBox.enabled === false)
        return undefined;
    return pushBox;
}
function getScreenPlayerReferenceIds(screen) {
    const ids = new Set();
...
function getPushBoxComponentsForScreen(analysis, screen) {
    const components = [];
    for (const entity of screen?.layers?.entities || []) {
        if (!(0, msx2Box2RuntimeGenerator_1.playerHasMsx2PushBox)(entity))
            continue;
        const pushBox = entity.components?.msx2_push_box;
        if (pushBox && pushBox.enabled !== false)
            components.push(pushBox);
    }
...
function getMsx2PlayerAssetForScreen(analysis, screen) {
    const playerRecords = getMsx2PlayerAssetRecords(analysis);
    if (!playerRecords.length)
        return undefined;
    const referenceIds = getScreenPlayerReferenceIds(screen);
    if (referenceIds.size) {
        const referenced = playerRecords.find(record => referenceIds.has(record.assetId) || referenceIds.has(record.playerId) || referenceIds.has(record.name));
        if (referenced)
            return referenced.player;
    }
    return playerRecords.length === 1 ? playerRecords[0].player : undefined;
}
function isBlankPlayerComponentValue(value) {
    return value === undefined || value === null || value === '';
}
function mergePlayerComponentBag(assetComponents, entityComponents) {
    const result = {};
    ...
    componentIds.forEach(componentId => {
        ...
        Object.entries(override).forEach(([key, value]) => {
            if (isBlankPlayerComponentValue(value) && !isBlankPlayerComponentValue(merged[key]))
                return;
            merged[key] = value;
        });
...
function mergePlayerAssetIntoRuntimeEntity(entity, playerAsset) {
    if (!entity && !playerAsset)
        return undefined;
    ...
    const mergedComponents = mergePlayerComponentBag(assetComponents, entity?.components);
    return {
        ...(playerAsset || {}),
        ...(entity || {}),
...
function collectReferencedMsx2SpriteIds(analysis) {
    ...
    for (const screen of collectReferencedTileScreens(analysis)) {
        for (const entity of screen.layers?.entities || []) {
            ...
            if ((0, msx2Box2RuntimeGenerator_1.playerHasMsx2PushBox)(entity)) {
                ...
                if (pushSpriteId)
                    spriteIds.add(pushSpriteId);
            }
            if ((0, msx2Box2RuntimeGenerator_1.entityHasMsx2Box2)(entity)) {
                const boxSpriteId = String(entity?.components?.msx2_hardware_sprite?.msx2SpriteAssetId
                    ?? entity?.params?.msx2SpriteAssetId
                    ?? '').trim();
                if (boxSpriteId)
                    spriteIds.add(boxSpriteId);
            }
        }
...
function getFirstReferencedMsx2Sprite(analysis) {
...
function normalizeEntityMovementMode(entity) {
    return String(entity?.components?.msx2_movement?.mode
        ?? entity?.params?.movement
        ?? entity?.params?.movementMode
        ?? entity?.params?.engine
        ?? '').replace(/[\s_-]+/g, '').toLowerCase();
}
function isBallBounceEntity(entity) {
...
function getEnemyHardwareSpriteEntity(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    return screen?.layers?.entities?.find(candidate => (candidate.kind === 'enemy' || candidate.kind === 'hazard') &&
        !(0, msx2Box2RuntimeGenerator_1.entityHasMsx2Box2)(candidate) &&
        !(0, msx2CarryObjectGenerator_1.isMsx2CarryableEntity)(candidate) &&
        getEntityRenderSpriteId(candidate));
}
function getEnemyHardwareSpriteSource(analysis) {
...
function parseEnemyFrameList(value) {
...
function getEnemyHardwareSpriteAnimationSettings(analysis, sprite) {
...
/**
 * Resolves the unique carryable sprite assets referenced by the carryable
 * entities across all tile screens. The order is deterministic
 * (first appearance, screens in order) and MUST be shared between the
 * pattern data emission, the per-screen sprite-index tables and the color
 * row pool — they all index into this set.
 */
function resolveMsx2CarryableSpriteSet(analysis, tileScreens) {
    const uniqueIds = [];
    const patternById = new Map();
    const colorsById = new Map();
    for (const screen of tileScreens) {
        for (const slot of (0, msx2CarryObjectGenerator_1.getMsx2CarryableRuntimeSlots)(screen)) {
            const id = slot.spriteId || '';
            if (patternById.has(id))
                continue;
            if (uniqueIds.length >= 4)
                continue;
            const sprite = id ? resolveMsx2SpriteById(analysis, id) : undefined;
            const layer = sprite
                ? buildHardwareSpriteLayersForFrame(sprite, msx2CarryObjectGenerator_1.MSX2_CARRY_DEFAULT_COLOR, 0)[0]
                : undefined;
            uniqueIds.push(id);
            patternById.set(id, layer?.pattern || msx2CarryObjectGenerator_1.MSX2_CARRY_DEFAULT_PATTERN);
            colorsById.set(id, layer?.colors || Array(16).fill(msx2CarryObjectGenerator_1.MSX2_CARRY_DEFAULT_COLOR));
        }
    }
    if (uniqueIds.length === 0) {
        uniqueIds.push('');
        patternById.set('', msx2CarryObjectGenerator_1.MSX2_CARRY_DEFAULT_PATTERN);
        colorsById.set('', Array(16).fill(msx2CarryObjectGenerator_1.MSX2_CARRY_DEFAULT_COLOR));
    }
    return { uniqueIds, patternById, colorsById };
}
function getPongBallHardwareSpriteSource(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const entity = screen?.layers?.entities?.find(candidate => (candidate.components?.msx2_ball || isBallBounceEntity(candidate)) &&
        getEntityRenderSpriteId(candidate));
    const spriteAssetId = getEntityRenderSpriteId(entity);
    if (!spriteAssetId)
        return undefined;
...
function getPrimaryPlayerEntity(screen) {
    return screen?.layers?.entities?.find(entity => entity.kind === 'player');
}
function getPrimaryPlayerEntry(screen) {
    return Array.isArray(screen?.playerEntries) && screen.playerEntries.length > 0
        ? screen.playerEntries[0]
        : undefined;
}
function getPlayerEntryById(screen, id) {
    if (!id || !Array.isArray(screen?.playerEntries))
        return undefined;
    return screen.playerEntries.find(entry => entry.id === id);
}
function getRuntimeMovementMode(screen) {
    const runtime = (screen?.runtime || {});
    return String(runtime.movementMode
        ?? runtime.controlMode
        ?? runtime.playerMode
...
function getPlayerRuntimeSource(screen, analysis) {
    ...
    if (entity)
        return mergePlayerAssetIntoRuntimeEntity(entity, playerAsset);
    const entry = getPrimaryPlayerEntry(screen);
    if (!entry)
        return mergePlayerAssetIntoRuntimeEntity(undefined, playerAsset);
    ...
    return mergePlayerAssetIntoRuntimeEntity({
        ...
        components: {
            msx2_player_control: {
                ...
                air: screen?.runtime?.initialAir ?? 255,
                disableAirTimer: screen?.runtime?.disableAirTimer ?? (screen?.runtime?.initialAir === 0),
            },
            msx2_animation: {
...
function getPlayerStartFromTileScreen(screen, preferredEntryId = 'default') {
...
function getPlayerEdgeEntryId(direction) {
    if (direction === 'west')
        return 'from_right';
    if (direction === 'east')
        return 'from_left';
...
function getPlayerEdgeFallbackStart(screen, direction) {
    const runtime = screen?.runtime;
    ...
    if (direction === 'west')
        return { x: Math.max(minX, maxX - MSX2_PLAYER_EDGE_ENTRY_MARGIN), y: 144 };
    if (direction === 'east')
        return { x: Math.min(maxX, minX + MSX2_PLAYER_EDGE_ENTRY_MARGIN), y: 144 };
    if (direction === 'north')
        return { x: 96, y: Math.max(minY, maxY - MSX2_PLAYER_EDGE_ENTRY_MARGIN) };
    return { x: 96, y: Math.min(maxY, minY + MSX2_PLAYER_EDGE_ENTRY_MARGIN) };
}
function getPlayerTransitionEntryStartFromTileScreen(screen, direction) {
...
function getHardwareSpriteRuntimeSettings(analysis, sprite) {
    ...
    const initialFrame = Math.max(0, Math.min(frameCount - 1, Math.floor(Number(hardware.frame ?? sprite.currentFrameIndex ?? 0) || 0)));
    return {
        ...settings,
...
function getPaddleCollisionSettings(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const player = getPlayerRuntimeSource(screen, analysis);
    const width = Math.max(8, Math.min(64, Math.floor(Number(player?.components?.msx2_collision?.hitboxW ?? player?.params?.hitboxW ?? 32) || 32)));
...
function getMsx2PlatformPlayerEntity(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    return getPlayerRuntimeSource(screen, analysis);
}
function getMsx2PlayerHealthSettings(analysis) {
...
function getMsx2PlayerAnimationSettings(player) {
...
function getMsx2PlayerAnimateOnlyWhenMoving(analysis) {
    return getMsx2PlayerAnimationSettings(getMsx2PlatformPlayerEntity(analysis)).animateOnlyWhenMoving;
}
function buildMsx2ClearPlayerWalkingFlagAsm(enabled) {
    return enabled
        ? `    xor a
...
function buildMsx2SetPlayerWalkingFlagAsm(enabled) {
    return enabled
        ? `    ld a, 1
    ld (msx2_player_walking_flag), a
`
...
function usesMsx2PlatformVerticalPhysics(analysis) {
    ...
    const physics = (0, msx2PlatformPhysics_1.getMsx2PlatformPhysicsFromPlayerEntity)(getMsx2PlatformPlayerEntity(analysis));
    return physics.jumpEnabled || physics.gravityEnabled;
}
function clampMsx2PlatformHitboxByte(value, fallback) {
    return Math.max(0, Math.min(255, Math.floor(Number(value) || fallback)));
}
function clampMsx2PlatformHitboxSigned(value, fallback) {
    return Math.max(-128, Math.min(127, Math.floor(Number(value) || fallback)));
}
function getMsx2PlatformPlayerBodyHitbox(player) {
...
function formatSignedPixelAdd(target, baseRamLabel, offset) {
    const load = `    ld a, (${baseRamLabel})\n`;
    const store = `    ld ${target}, a\n`;
    if (offset > 0)
        return `${load}    add a, ${(0, msx2PlatformPhysics_1.formatAsmByte)(offset)}\n${store}`;
    if (offset < 0)
        return `${load}    sub ${(0, msx2PlatformPhysics_1.formatAsmByte)(Math.abs(offset))}\n${store}`;
    return `${load}${store}`;
}
function buildMsx2PlatformVerticalPhysicsAsm(analysis, options) {
...
`}`;
    ...
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const player = getMsx2PlatformPlayerEntity(analysis);
    const physics = (0, msx2PlatformPhysics_1.getMsx2PlatformPhysicsFromScreen)(screen, player);
    const glideConfig = (0, msx2PlatformPhysics_1.getMsx2GlideConfigFromPlayerEntity)(player);
    const wallJumpConfig = (0, msx2PlatformPhysics_1.getMsx2WallJumpConfigFromPlayerEntity)(player);
    const powerStompConfig = (0, msx2PlatformPhysics_1.getMsx2PowerStompConfigFromPlayerEntity)(player);
    const screenShakeEnabled = (0, msx2PlatformPhysics_1.msx2PlayerWantsScreenShake)(player);
    const glideGravityHookAsm = (0, msx2GlideGenerator_1.buildMsx2GlideGravityHookAsm)(glideConfig);
    const glideLandRefillAsm = (0, msx2GlideGenerator_1.buildMsx2GlideLandRefillAsm)(glideConfig);
    const wallJumpGravityHookAsm = (0, msx2WallJumpGenerator_1.buildMsx2WallJumpGravityHookAsm)(wallJumpConfig);
    const wallJumpLandClearAsm = (0, msx2WallJumpGenerator_1.buildMsx2WallJumpLandClearAsm)(wallJumpConfig);
    const powerStompLandHookAsm = (0, msx2PowerStompGenerator_1.buildMsx2PowerStompLandHookAsm)(powerStompConfig, { screenShakeEnabled });
    ...
    const jumpImpulseHi = (0, msx2PlatformPhysics_1.formatAsmByte)(physics.jumpImpulse88 >> 8);
    const jumpImpulseLo = (0, msx2PlatformPhysics_1.formatAsmByte)(physics.jumpImpulse88);
    const airJumpImpulseHi = (0, msx2PlatformPhysics_1.formatAsmByte)(physics.airJumpImpulse88 >> 8);
    const airJumpImpulseLo = (0, msx2PlatformPhysics_1.formatAsmByte)(physics.airJumpImpulse88);
    const gravityStrength = (0, msx2PlatformPhysics_1.formatAsmByte)(physics.gravityStrength88);
    const terminalHigh = (0, msx2PlatformPhysics_1.formatAsmByte)((0, msx2PlatformPhysics_1.getTerminalVelocityHighByte)(physics.terminalVelocity88));
    const terminalWord = (0, msx2PlatformPhysics_1.formatAsmWord)(physics.terminalVelocity88);
    const maxJumps = (0, msx2PlatformPhysics_1.formatAsmByte)(physics.maxJumps);
    const coyoteFrames = (0, msx2PlatformPhysics_1.formatAsmByte)(physics.coyoteTime);
    const jumpBufferFrames = (0, msx2PlatformPhysics_1.formatAsmByte)(physics.jumpBuffer);
...
${glideGravityHookAsm}${wallJumpGravityHookAsm}`
        : '';
...
.platform_cancel_jump:
    ...
    and #F0
    add a, #10
    sub ${(0, msx2PlatformPhysics_1.formatAsmByte)(Math.max(0, hitboxTopOffset))}
    ld (msx2_player_sprite_y), a
    ld hl, msx2_player_gravity_vel
    xor a
    ld (hl), a
...
.platform_land:
    ld a, e
    and #F0
    sub ${(0, msx2PlatformPhysics_1.formatAsmByte)(Math.max(0, hitboxTopOffset + hitboxHeight))}
    ld (msx2_player_sprite_y), a
    ld hl, msx2_player_gravity_vel
    xor a
    ld (hl), a
...
` : ''}${glideLandRefillAsm}${wallJumpLandClearAsm}${powerStompLandHookAsm}    ld a, #1
    ld (msx2_player_flags), a
...
function getPaddleHorizontalSpeed(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const player = getPlayerRuntimeSource(screen, analysis);
    const speed = Number(player?.components?.msx2_paddle?.speed
...
function getPlatformHorizontalSpeed(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const player = getPlayerRuntimeSource(screen, analysis);
    const speed = Number(player?.components?.msx2_movement?.speed
        ?? player?.components?.msx2_player_control?.speed
        ?? player?.params?.speed
        ?? 2);
    return Math.max(1, Math.min(16, Math.floor(speed) || 2));
}
function getPrimaryBallEntity(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    return screen?.layers?.entities?.find(entity => (entity.kind === 'enemy' || entity.kind === 'hazard') && isBallBounceEntity(entity));
}
function getPrimaryBallSpeedByte(analysis, axis) {
    const ball = getPrimaryBallEntity(analysis);
    const key = axis === 'x' ? 'speedX' : 'speedY';
    const speedY = Number(ball?.components?.msx2_ball?.[key]
...
function getPrimaryBallLaunchDy(analysis) {
    return 0x100 - getPrimaryBallSpeedByte(analysis, 'y');
}
function usesMazeMovement(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const runtime = (screen?.runtime || {});
    const mode = String(runtime.movementMode
        ?? runtime.movement
        ?? runtime.controlMode
        ?? runtime.playerMode
        ?? '').toLowerCase();
...
function usesShooterHorizontalMovement(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const runtime = (screen?.runtime || {});
    const player = getPlayerRuntimeSource(screen, analysis);
    const mode = String(runtime.movementMode
...
function usesMsx2Screen4BackgroundScroll(analysis) {
    return collectReferencedTileScreens(analysis).some(screen => Boolean(screen?.runtime?.scrollMode)
        || Boolean(screen?.runtime?.scroll)
        || ['tileVertical', 'spaceLoop'].includes(String(screen?.runtime?.shooter?.scrollMode || ''))
        || Boolean(screen?.layers?.entities?.some(entity => Boolean(entity?.components?.msx2_scroll))));
}
function usesShooterVerticalMovement(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const runtime = (screen?.runtime || {});
    const player = getPlayerRuntimeSource(screen, analysis);
    const mode = String(runtime.movementMode
...
function isRuntimeHudHidden(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const runtime = (screen?.runtime || {});
    return runtime.hideHud === true;
}
function getMsx2Shooter60HzBudgetFromAnalysis(analysis) {
    const tileScreens = collectReferencedTileScreens(analysis);
    const shooterScreens = tileScreens.filter(screen => screen.runtime?.screenEngine === 'shooter' || Boolean(screen.runtime?.shooter));
    const primary = shooterScreens[0] || getPrimaryRuntimeTileScreen(analysis);
    if (!primary)
        return null;
    const hasShooterRuntime = primary.runtime?.screenEngine === 'shooter' || Boolean(primary.runtime?.shooter);
    if (!hasShooterRuntime && !usesShooterHorizontalMovement(analysis) && !usesShooterVerticalMovement(analysis)) {
        return null;
    }
    return (0, msx2ShooterRuntime_1.resolveMsx2Shooter60HzBudgetForGeneration)(primary.runtime?.shooter || {
...
function getPlayerBulletSlotCount(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const shooterBudget = getMsx2Shooter60HzBudgetFromAnalysis(analysis);
    const player = getPlayerRuntimeSource(screen, analysis);
    const configured = Number(shooterBudget?.budget.maxPlayerShots
...
function getEnemyBulletSlotCount(analysis) {
    const shooterBudget = getMsx2Shooter60HzBudgetFromAnalysis(analysis);
    const primary = getPrimaryRuntimeTileScreen(analysis);
    const explicitMaxEnemyShots = primary?.runtime?.shooter?.budget?.maxEnemyShots;
    const configured = Number(explicitMaxEnemyShots !== undefined && explicitMaxEnemyShots !== null
...
function isMsx2ShooterEnabled(entity) {
...
function getPlayerProjectileEntity(analysis) {
    ...
    if (projectilePresetId) {
        ...
        const byId = entities.find(entity => {
            const entityToken = String(`${entity?.id || ''} ${entity.templateId || ''} ${entity?.name || ''}`)
                .replace(/[\s_-]+/g, '')
                .toLowerCase();
            return entity?.id === projectilePresetId || entity.templateId === projectilePresetId || entityToken.includes(presetToken);
        });
        if (byId)
            return byId;
    }
    return entities.find(entity => entity?.components?.msx2_projectile?.owner === 'player'
        || entity?.params?.owner === 'player');
}
function getPlayerBulletCooldownFrames(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const player = getPlayerRuntimeSource(screen, analysis);
    const configured = Number(player?.components?.msx2_shooter?.cooldownFrames
        ?? player?.params?.cooldownFrames
        ?? 8);
    return Math.max(1, Math.min(255, Math.floor(configured) || 8));
}
function getPlayerShooterDirection(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const player = getPlayerRuntimeSource(screen, analysis);
    const configured = String(player?.components?.msx2_shooter?.direction
        ?? player?.params?.shotDirection
        ?? '').replace(/[\s_-]+/g, '').toLowerCase();
    if (configured === 'up' || configured === 'vertical')
        return 'up';
...
function getPlayerBulletSpeedX(analysis) {
    const projectile = getPlayerProjectileEntity(analysis);
    const configured = Number(projectile?.components?.msx2_projectile?.velocityX
        ?? projectile?.params?.velocityX
        ?? 4);
    return Math.max(1, Math.min(16, Math.floor(Math.abs(configured)) || 4));
}
function getPlayerBulletSpeedY(analysis) {
    const projectile = getPlayerProjectileEntity(analysis);
    const configured = Number(projectile?.components?.msx2_projectile?.velocityY
        ?? projectile?.params?.velocityY
        ?? -6);
    return Math.max(1, Math.min(16, Math.floor(Math.abs(configured)) || 6));
}
function getGalaxianAttackWaveSettingsForScreen(screen) {
    const controller = screen?.layers?.entities?.find(entity => Boolean(entity?.components?.msx2_attack_wave)
    ...
    const intervalFrames = Math.max(1, Math.min(255, Math.floor(Number(component.intervalFrames ?? controller?.params?.attackIntervalFrames ?? 180) || 180)));
    const minAttackers = Math.max(1, Math.min(3, Math.floor(Number(component.minAttackers ?? controller?.params?.minAttackers ?? 1) || 1)));
    const maxAttackers = Math.max(minAttackers, Math.min(3, Math.floor(Number(component.maxAttackers ?? controller?.params?.maxAttackers ?? 3) || 3)));
    const randomSeed = Math.max(1, Math.min(255, Math.floor(Number(component.randomSeed ?? controller?.params?.randomSeed ?? 73) || 73)));
    return { intervalFrames, minAttackers, maxAttackers, randomSeed };
}
function getGalaxianAttackWaveSettings(analysis) {
    return getGalaxianAttackWaveSettingsForScreen(getPrimaryRuntimeTileScreen(analysis));
}
function normalizeGalaxianAttackPattern(value, slot) {
    const token = String(value || '').replace(/[\s_-]+/g, '').toLowerCase();
    if (token === 'circle' || token === 'loop' || token === 'arc')
        return 'circle';
    if (token === 'zigzag' || token === 'zig' || token === 'zag')
        return 'zigzag';
    if (token === 'diagonal' || token === 'diag')
        return 'diagonal';
    return slot % 3 === 0 ? 'circle' : slot % 3 === 1 ? 'zigzag' : 'diagonal';
}
function getGalaxianAttackPatterns(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const enemies = (screen?.layers?.entities || [])
        .filter(entity => (entity.kind === 'enemy' || entity.kind === 'hazard') && entity.position)
        .slice(0, msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN);
    return Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, slot) => normalizeGalaxianAttackPattern(enemies[slot]?.components?.msx2_attack_pattern?.pattern ?? enemies[slot]?.params?.attackPattern, slot));
}
function usesPaddleHorizontalMovement(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const runtime = (screen?.runtime || {});
    const player = getPlayerRuntimeSource(screen, analysis);
    const mode = String(runtime.movementMode
...
function usesControl2Players(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const runtime = (screen?.runtime || {});
    const player = getPlayerRuntimeSource(screen, analysis);
    const mode = String(runtime.movementMode
...
function getControl2PlayersComponent(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
...
function getControl2PlayersSpeed(analysis) {
    const component = getControl2PlayersComponent(analysis);
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const player = getPlayerRuntimeSource(screen, analysis);
    const speed = Number(component.speed
...
function getControl2PlayersVerticalBounds(analysis) {
    const component = getControl2PlayersComponent(analysis);
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const runtime = (screen?.runtime || {});
...
function usesSnakeCharMovement(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const runtime = (screen?.runtime || {});
    const player = getPlayerRuntimeSource(screen, analysis);
    const mode = String(runtime.movementMode
...
function getComponentNumber(entity, componentId, key, fallback) {
    const value = Number(entity?.components?.[componentId]?.[key] ?? entity?.params?.[key]);
    return Number.isFinite(value) ? value : fallback;
}
function clampScreen4CharBlockBase(value) {
    return Math.max(1, Math.min(252, Math.floor(value)));
}
function screen4CharBlocksOverlap(a, b) {
    return Math.abs(a - b) < 4;
}
function findFreeScreen4CharBlockBase(requestedBase, reservedBases) {
    let base = clampScreen4CharBlockBase(requestedBase);
    for (let attempts = 0; attempts < 64; attempts++) {
        if (!reservedBases.some(existing => screen4CharBlocksOverlap(existing, base)))
            return base;
        base += 4;
        if (base > 252)
            base = 1;
...
function buildSnakeWallMapBytes(screen) {
    const tiles = screen?.tiles || [];
    const solidTileIndexes = new Set();
    tiles.forEach((tile, index) => {
        const token = String(`${tile?.id || ''} ${tile?.name || ''}`)
            .replace(/[\s_-]+/g, '')
            .toLowerCase();
        if (token.includes('wall') ||
            token.includes('muro') ||
...
function getSnakeCharRuntimeSettings(analysis) {
    const screen = getPrimaryRuntimeTileScreen(analysis);
    const entities = screen?.layers?.entities || [];
    ...
    const food = entities.find(entity => entity?.components?.msx2_collectible ||
        entity?.params?.food ||
        String(entity?.params?.engine || '').toLowerCase() === 'snakefood');
    const body = entities.find(entity => entity?.components?.msx2_snake_segment ||
    ...
    const bodySegments = entities
        .filter(entity => entity?.components?.msx2_snake_segment ||
        entity?.params?.snakeSegment ||
        String(entity?.params?.role || '').toLowerCase() === 'body')
...
function usesSnakeGrowth(analysis) {
    return collectReferencedTileScreens(analysis).some(screen => {
        const runtime = (screen?.runtime || {});
...
function getSnakeGrowthBodyTileBytes(analysis) {
...
function reverseSpritePatternByte(value) {
    let result = 0;
    for (let bit = 0; bit < 8; bit++) {
        if (value & (1 << bit))
            result |= 0x80 >> bit;
...
function mirrorHardwareSpritePatternHorizontally(pattern) {
...
function getHorizontalFacingDirection(sprite) {
...
function buildHardwareSpriteLayersForFrame(sprite, fallbackColor, frameIndex) {
...
function getHardwareSpriteAnimationFrameCount(sprite, layerCount) {
...
function getHardwareSpriteAnimationDelayFrames(sprite) {
 ...
 */
function getPlayerHardwareSpriteColorAnimation(analysis) {
    const sprite = getHardwareSpriteSource(analysis);
    if (!sprite)
        return { enabled: false, frameCount: 1, layerCount: 0, stride: 0 };
...
function multiplyABySmallConstant(multiplier) {
    if (multiplier <= 1)
        return '';
    if (multiplier === 2)
        return '    add a, a\n';
...
function buildEnemyScreenSlotOffsetAsm(slot = 0) {
    const addSlot = slot ? `    add a, ${slot}\n` : '';
    return `    ld a, (msx2_current_screen_index)
${multiplyABySmallConstant(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN)}${addSlot}    ld e, a
    ld d, 0
`;
}
function buildHardwareSpritePatternIndexAsm(basePatternIndex, framePatternStride, layerPatternOffset, frameCount, mirrorPatternOffset = 0, authoredFacing, labelSuffix = '0') {
...
function buildHardwareSpriteInitAsm(analysis, useKonamiDataBank = false) {
    const sprite = getHardwareSpriteSource(analysis);
    if (!sprite)
        return '';
    const settings = getHardwareSpriteRuntimeSettings(analysis, sprite);
    ...
    const dashConfig = (0, msx2PlatformPhysics_1.getMsx2DashConfigFromPlayerEntity)(platformPlayer);
    const airDashConfig = (0, msx2PlatformPhysics_1.getMsx2AirDashConfigFromPlayerEntity)(platformPlayer);
    const teleportConfig = (0, msx2PlatformPhysics_1.getMsx2TeleportABConfigFromPlayerEntity)(platformPlayer);
    const glideConfig = (0, msx2PlatformPhysics_1.getMsx2GlideConfigFromPlayerEntity)(platformPlayer);
    const wallJumpConfig = (0, msx2PlatformPhysics_1.getMsx2WallJumpConfigFromPlayerEntity)(platformPlayer);
    const powerStompConfig = (0, msx2PlatformPhysics_1.getMsx2PowerStompConfigFromPlayerEntity)(platformPlayer);
    const screenShakeEnabled = (0, msx2PlatformPhysics_1.msx2PlayerWantsScreenShake)(platformPlayer);
    const carryObjectConfig = (0, msx2PlatformPhysics_1.getMsx2CarryObjectConfigFromPlayerEntity)(platformPlayer);
    const dashInitClearAsm = dashConfig.enabled ? (0, msx2DashGenerator_1.buildMsx2DashInitClearAsm)() : '';
    const airDashInitClearAsm = airDashConfig.enabled ? (0, msx2AirDashGenerator_1.buildMsx2AirDashInitClearAsm)() : '';
    const carryObjectInitClearAsm = carryObjectConfig.enabled ? (0, msx2CarryObjectGenerator_1.buildMsx2CarryObjectInitClearAsm)() : '';
    const teleportInitClearAsm = teleportConfig.enabled ? (0, msx2TeleportABGenerator_1.buildMsx2TeleportABInitClearAsm)() : '';
    const glideInitClearAsm = glideConfig.enabled ? (0, msx2GlideGenerator_1.buildMsx2GlideInitClearAsm)() : '';
    const wallJumpInitClearAsm = wallJumpConfig.enabled ? (0, msx2WallJumpGenerator_1.buildMsx2WallJumpInitClearAsm)() : '';
    const powerStompInitClearAsm = powerStompConfig.enabled ? (0, msx2PowerStompGenerator_1.buildMsx2PowerStompInitClearAsm)() : '';
    const screenShakeInitClearAsm = screenShakeEnabled ? (0, msx2ScreenShakeGenerator_1.buildMsx2ScreenShakeInitClearAsm)() : '';
    const glideInitAsm = (0, msx2GlideGenerator_1.buildMsx2GlideInitAsm)(glideConfig);
    const initialLivesByte = (0, msx2PlatformPhysics_1.formatAsmByte)(playerHealth.lives);
...
function buildMsx2VramByteWriteAsm() {
...
function buildMsx2HudTextRuntimeAsm(analysis, useKonamiDataBank = false) {
...
function buildMsx2GameFlowTransitionHelpersAsm(needed) {
    if (!needed)
        return '';
    return `
clear_screen4_names:
...
function buildMsx2GameFlowMusicHelpersAsm(needed) {
    if (!needed)
        return '';
    return `
...
function buildHardwareSpriteRuntimeAsm(analysis, requiredCollectibles, restartScreenLabel, restartScreenIndex, tileScreenCount = 1, options = {}) {
    const sprite = getHardwareSpriteSource(analysis);
    if (!sprite)
        return '';
    const settings = getHardwareSpriteRuntimeSettings(analysis, sprite);
    ...
    const playerHealth = getMsx2PlayerHealthSettings(analysis);
    const initialLivesByte = (0, msx2PlatformPhysics_1.formatAsmByte)(playerHealth.lives);
    const invincibleFramesByte = (0, msx2PlatformPhysics_1.formatAsmByte)(playerHealth.invincibleFrames);
    ...
    const carryConfig = (0, msx2PlatformPhysics_1.getMsx2CarryObjectConfigFromPlayerEntity)(getMsx2PlatformPlayerEntity(analysis));
    ...
    const runtimeEnemyHazards = (options.tileScreens?.length ? options.tileScreens : collectReferencedTileScreens(analysis)).map(screen => (0, msx2EntityRuntimeGenerator_1.getMsx2EnemyHazardRuntimeSlots)(screen));
    ...
    const dashRuntimeAsm = (0, msx2DashGenerator_1.buildMsx2DashRuntimeAsm)(dashConfig, patrolBounds, {
        setPlayerWalkingFlagAsm,
    });
    const dashInputGateAsm = (0, msx2DashGenerator_1.buildMsx2DashInputGateAsm)(dashConfig);
    const dashActiveFrameAsm = (0, msx2DashGenerator_1.buildMsx2DashActiveFrameAsm)(dashConfig);
    const dashHazardSkipAsm = (0, msx2DashGenerator_1.buildMsx2DashHazardSkipAsm)(dashConfig);
    const dashDamageSkipAsm = (0, msx2DashGenerator_1.buildMsx2DashDamageSkipAsm)(dashConfig);
    const dashInitClearAsm = dashConfig.enabled ? (0, msx2DashGenerator_1.buildMsx2DashInitClearAsm)() : '';
    const airDashRuntimeAsm = (0, msx2AirDashGenerator_1.buildMsx2AirDashRuntimeAsm)(airDashConfig, patrolBounds, {
        lockGroundDashOnStart: dashConfig.enabled,
    });
    const airDashInputGateAsm = (0, msx2AirDashGenerator_1.buildMsx2AirDashInputGateAsm)(airDashConfig);
    const airDashActiveFrameAsm = (0, msx2AirDashGenerator_1.buildMsx2AirDashActiveFrameAsm)(airDashConfig);
    const airDashHazardSkipAsm = (0, msx2AirDashGenerator_1.buildMsx2AirDashHazardSkipAsm)(airDashConfig);
    const airDashDamageSkipAsm = (0, msx2AirDashGenerator_1.buildMsx2AirDashDamageSkipAsm)(airDashConfig);
    const airDashInitClearAsm = airDashConfig.enabled ? (0, msx2AirDashGenerator_1.buildMsx2AirDashInitClearAsm)() : '';
    const collectorGemsConfig = (0, msx2PlatformPhysics_1.getMsx2CollectorGemsConfigFromPlayerEntity)(platformPlayer);
    const collectorGemsCollectHookAsm = (0, msx2CollectorGemsGenerator_1.buildMsx2CollectorGemsCollectHookAsm)(collectorGemsConfig);
    const collectorGemsSfxAsm = (0, msx2CollectorGemsGenerator_1.buildMsx2CollectorGemsSfxAsm)(collectorGemsConfig);
    const teleportRuntimeAsm = (0, msx2TeleportABGenerator_1.buildMsx2TeleportABRuntimeAsm)(teleportConfig);
    const teleportInputGateAsm = (0, msx2TeleportABGenerator_1.buildMsx2TeleportABInputGateAsm)(teleportConfig);
    const teleportHazardSkipAsm = (0, msx2TeleportABGenerator_1.buildMsx2TeleportABHazardSkipAsm)(teleportConfig);
    const teleportDamageSkipAsm = (0, msx2TeleportABGenerator_1.buildMsx2TeleportABDamageSkipAsm)(teleportConfig);
    const teleportInitClearAsm = teleportConfig.enabled ? (0, msx2TeleportABGenerator_1.buildMsx2TeleportABInitClearAsm)() : '';
    const glideRuntimeAsm = (0, msx2GlideGenerator_1.buildMsx2GlideRuntimeAsm)(glideConfig);
    const glideInitClearAsm = glideConfig.enabled ? (0, msx2GlideGenerator_1.buildMsx2GlideInitClearAsm)() : '';
    const wallJumpRuntimeAsm = (0, msx2WallJumpGenerator_1.buildMsx2WallJumpRuntimeAsm)(wallJumpConfig, getPlatformHorizontalSpeed(analysis), patrolBounds);
    const wallJumpInputGateAsm = (0, msx2WallJumpGenerator_1.buildMsx2WallJumpInputGateAsm)(wallJumpConfig);
    const wallJumpGravityHookAsm = (0, msx2WallJumpGenerator_1.buildMsx2WallJumpGravityHookAsm)(wallJumpConfig);
    const wallJumpLandClearAsm = (0, msx2WallJumpGenerator_1.buildMsx2WallJumpLandClearAsm)(wallJumpConfig);
    const wallJumpDamageSkipAsm = (0, msx2WallJumpGenerator_1.buildMsx2WallJumpDamageSkipAsm)(wallJumpConfig);
    const wallJumpHazardSkipAsm = (0, msx2WallJumpGenerator_1.buildMsx2WallJumpHazardSkipAsm)(wallJumpConfig);
    const wallJumpResetAsm = wallJumpConfig.enabled ? (0, msx2WallJumpGenerator_1.buildMsx2WallJumpResetAsm)() : '';
    const wallJumpInitClearAsm = wallJumpConfig.enabled ? (0, msx2WallJumpGenerator_1.buildMsx2WallJumpInitClearAsm)() : '';
    const powerStompConfig = (0, msx2PlatformPhysics_1.getMsx2PowerStompConfigFromPlayerEntity)(platformPlayer);
    const screenShakeEnabled = (0, msx2PlatformPhysics_1.msx2PlayerWantsScreenShake)(platformPlayer);
    const powerStompRuntimeAsm = (0, msx2PowerStompGenerator_1.buildMsx2PowerStompRuntimeAsm)(powerStompConfig, { screenShakeEnabled });
    const powerStompInputGateAsm = (0, msx2PowerStompGenerator_1.buildMsx2PowerStompInputGateAsm)(powerStompConfig);
    const powerStompInitClearAsm = powerStompConfig.enabled ? (0, msx2PowerStompGenerator_1.buildMsx2PowerStompInitClearAsm)() : '';
    const screenShakeRuntimeAsm = screenShakeEnabled ? (0, msx2ScreenShakeGenerator_1.buildMsx2ScreenShakeRuntimeAsm)() : '';
    const screenShakeInitClearAsm = screenShakeEnabled ? (0, msx2ScreenShakeGenerator_1.buildMsx2ScreenShakeInitClearAsm)() : '';
    const screenShakeMainLoopCallAsm = screenShakeEnabled ? (0, msx2ScreenShakeGenerator_1.buildMsx2ScreenShakeMainLoopCallAsm)() : '';
    ...
    const shooterActiveIrqProfile = shooter60HzBudget?.budget.irqProfiles.find(profile => profile.id === shooter60HzBudget.budget.activeIrqProfile);
    ...
    const tileScreenLoadLabelsForBullets = tileScreensForBullets.map((screen, index) => sanitizeLabel(screen?.name || `msx2_screen4_screen_${index}`, `MSX2_SCREEN4_SCREEN_${index}`));
    const playerBulletCharSettings = (0, msx2PlayerBulletCharGenerator_1.getPlayerBulletCharSettings)(analysis, getPlayerProjectileEntity, getScreen4TileBytesForEntity, getPrimaryRuntimeTileScreen);
    const playerBulletCharCoreAsm = playerBulletCharMode
        ? (0, msx2PlayerBulletCharGenerator_1.buildMsx2PlayerBulletCharCoreAsm)(playerBulletCharSettings, tileScreenLoadLabelsForBullets, false)
        : '';
    ...
    const playerBulletSlot0UpdateAsm = playerBulletCharMode
        ? (0, msx2PlayerBulletCharGenerator_1.buildPlayerBulletCharSlotUpdateAsm)({ slot: 0, horizontal: playerBulletHorizontal, speedX: playerBulletSpeedX, speedY: playerBulletSpeedY })
        : '';
    const playerBulletSlot1UpdateAsm = secondPlayerBullet && playerBulletCharMode
        ? (0, msx2PlayerBulletCharGenerator_1.buildPlayerBulletCharSlotUpdateAsm)({ slot: 1, horizontal: playerBulletHorizontal, speedX: playerBulletSpeedX, speedY: playerBulletSpeedY })
        : '';
    const playerBulletSpawn0Asm = playerBulletCharMode
        ? (0, msx2PlayerBulletCharGenerator_1.buildPlayerBulletCharSpawnAsm)({ slot: 0, horizontal: playerBulletHorizontal, cooldownFrames: playerBulletCooldownFrames })
        : '';
    const playerBulletSpawn1Asm = secondPlayerBullet && playerBulletCharMode
        ? (0, msx2PlayerBulletCharGenerator_1.buildPlayerBulletCharSpawnAsm)({ slot: 1, horizontal: playerBulletHorizontal, cooldownFrames: playerBulletCooldownFrames })
        : '';
...
`;
    ...
    const enemyAttrSlotAddress = (base, slot) => slot
        ? `    ld hl, ${base}
    ld de, ${slot}
    add hl, de
`
        : `    ld hl, ${base}
`;
    const buildEnemyPatternIndexAsm = (slot) => {
...
.enemy_sprite_${slot}_base_pattern:`;
    ...
    const buildRegularEnemyAttrWrite = (slot) => {
...
`;
    ...
    const buildControl2PlayersAttrWrite = (slot) => {
        const attrAddress = 0x1E00 + ((layers.length + slot) * 4);
        const slotAddress = (base) => slot
            ? `    ld hl, ${base}
    ld de, ${slot}
    add hl, de
`
...
`;
    ...
    const enemyAttrWrites = Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, slot) => control2Players ? buildControl2PlayersAttrWrite(slot) : buildRegularEnemyAttrWrite(slot)).join('\n');
    const playerBulletAttrAddress = 0x1E00 + ((layers.length + msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN) * 4);
...
` : ''}`;
    const enemyBulletAttrAddress = 0x1E00 + ((layers.length + msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN + playerBulletSlotCount) * 4);
...
`;
    const hudLivesAttrBase = 0x1E00 + ((layers.length + msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN + playerBulletSlotCount + (secondEnemyBullet ? 2 : 1)) * 4);
...
`;
    ...
    const pushBoxAttrWrite = pushBoxHardwareSpriteActive
        ? (0, msx2Box2ComponentGenerator_1.buildMsx2Box2HardwareSpriteAttrWrite)({ attrAddress: pushBoxAttrAddress, patternIndex: pushBoxPatternIndex })
        : '';
    const pushBoxSatRefresh = pushBoxHardwareSpriteActive
        ? (0, msx2Box2ComponentGenerator_1.buildMsx2Box2HardwareSpriteSatRefreshAsm)({ attrAddress: pushBoxAttrAddress, patternIndex: pushBoxPatternIndex })
        : '';
    const carryAttrBase = pushBoxAttrAddress + (pushBoxHardwareSpriteActive ? 4 : 0);
    const carrySatSlotBase = (carryAttrBase - 0x1E00) / 4;
    const carryAttrWrites = carryConfig.enabled
        ? (0, msx2CarryObjectGenerator_1.buildMsx2CarryObjectAttrWritesAsm)(carryAttrBase, carryPatternIndexBase)
        : '';
    const carryObjectRuntimeAsm = carryConfig.enabled
        ? (0, msx2CarryObjectGenerator_1.buildMsx2CarryObjectRuntimeAsm)(carryConfig, { satSlotBase: carrySatSlotBase })
        : '';
    const carryInitClearAsm = carryConfig.enabled ? (0, msx2CarryObjectGenerator_1.buildMsx2CarryObjectInitClearAsm)() : '';
    const carryUpdateCallAsm = carryConfig.enabled ? '    call update_msx2_carry_objects\n' : '';
    const carryResetCallAsm = carryConfig.enabled ? '    call msx2_reset_carry_runtime_for_current_screen\n' : '';
    const terminatorAttrAddress = carryAttrBase + (carryConfig.enabled ? msx2CarryObjectGenerator_1.MSX2_MAX_CARRYABLES_PER_SCREEN * 4 : 0);
...
.control_2_players_p2_down_store:
    ...
    ret
` : '';
    const enemySlotAddress = (base, slot) => slot
        ? `    ld hl, ${base}
    ld de, ${slot}
    add hl, de
`
        : `    ld hl, ${base}
`;
    const enemySlotCollisionChecks = Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, slot) => {
        const addSlot = slot ? `    add a, ${slot}\n` : '';
        return `    ld a, (msx2_current_screen_index)
    ld e, a
    ld d, 0
...
${paddleHorizontal ? `${buildEnemyScreenSlotOffsetAsm(slot)}    ld hl, msx2_screen_enemy_mode
    add hl, de
    ld a, (hl)
    cp ${msx2EntityRuntimeGenerator_1.MSX2_ENEMY_MOVEMENT_BALL_BOUNCE}
...
`;
    ...
    const buildBulletEnemyCollisionChecks = (label, bulletX, bulletY, bulletActive) => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, slot) => {
        const addSlot = slot ? `    add a, ${slot}\n` : '';
        return `    ld a, (msx2_current_screen_index)
    ld e, a
    ld d, 0
...
` : ''}
` : '';
    const enemyWaveCompleteChecks = Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, slot) => `    ld a, (msx2_current_screen_index)
    ld e, a
    ld d, 0
    ld hl, msx2_screen_enemy_count
    add hl, de
...
`).join('');
    const enemyBulletSpawnChecks = Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, slot) => {
...
`;
    ...
    const enemySlotMovementRoutines = Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, slot) => {
        const addSlot = slot ? `    add a, ${slot}\n` : '';
        return `    call update_msx2_enemy_position_slot_${slot}
`;
    }).join('');
    const enemyBehaviorStateSlotRoutines = enemyBehaviorStateSwitchEnabled ? Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, slot) => `    call update_msx2_enemy_behavior_state_slot_${slot}
`).join('') : '';
    const enemyBehaviorStateSlotHandlers = enemyBehaviorStateSwitchEnabled ? Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, slot) => `update_msx2_enemy_behavior_state_slot_${slot}:
    ld a, (msx2_current_screen_index)
    ld e, a
    ld d, 0
    ld hl, msx2_screen_enemy_count
...
${enemyBehaviorStateSlotHandlers}` : '';
    const enemySlotMovementHandlers = Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, slot) => {
        const addSlot = slot ? `    add a, ${slot}\n` : '';
        if (control2Players) {
            if (slot !== 1) {
...
${enemySlotAddress('msx2_enemy_runtime_mode', slot)}
    ...
    cp ${msx2EntityRuntimeGenerator_1.MSX2_ENEMY_MOVEMENT_DIVE}
    jp z, .enemy_slot_${slot}_dive
    cp ${msx2EntityRuntimeGenerator_1.MSX2_ENEMY_MOVEMENT_GHOST_MAZE}
    jp z, .enemy_slot_${slot}_ghost_maze
    cp ${msx2EntityRuntimeGenerator_1.MSX2_ENEMY_MOVEMENT_FLYER_SINE}
    jp z, .enemy_slot_${slot}_flyer_sine
    cp ${msx2EntityRuntimeGenerator_1.MSX2_ENEMY_MOVEMENT_JUMPER}
    jp z, .enemy_slot_${slot}_jumper
    cp ${msx2EntityRuntimeGenerator_1.MSX2_ENEMY_MOVEMENT_WALKER_EDGE}
    jp z, .enemy_slot_${slot}_walker_edge
    cp ${msx2EntityRuntimeGenerator_1.MSX2_ENEMY_MOVEMENT_CHASE_H}
...
msx2_enemy_screen_slot_offset_from_b:
    ...
    push bc
    ld a, (msx2_current_screen_index)
${multiplyABySmallConstant(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN)}    pop bc
    add a, b
    ld e, a
    ld d, 0
    ret
...
${collectorGemsSfxAsm}msx2_check_enemy_wave_complete:
...
move_hardware_sprite_right:
    ...
    jp nc, msx2_try_world_edge_transition_right
${pushBoxEnabled ? `${(0, msx2Box2ComponentGenerator_1.buildMsx2Box2PlayerHookAsm)('right')}    ; msx2_try_box2_from_player clobbers A: rebuild the probe X from RAM.
    ld a, (msx2_player_sprite_x)
    add a, ${platformMoveSpeed}
` : ''}    add a, 15
    ld b, a
...
move_hardware_sprite_left:
    ...
    jp z, msx2_try_world_edge_transition_left
    jp c, msx2_try_world_edge_transition_left
${pushBoxEnabled ? (0, msx2Box2ComponentGenerator_1.buildMsx2Box2PlayerHookAsm)('left') : ''}    ld a, (msx2_player_sprite_x)
...
${buildEnemyScreenSlotOffsetAsm()}
    ...
    ld bc, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN}
    ldir
${buildEnemyScreenSlotOffsetAsm()}
    ld hl, msx2_screen_enemy_y
    add hl, de
...
${shooterHorizontal ? `msx2_init_galaxian_attack_runtime:
    ...
    ld hl, msx2_enemy_runtime_tick
    ld b, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN}
    ld a, #FF
...
msx2_respawn_current_screen:
    ...
    ld a, #01
    ld (msx2_player_flags), a
${dashInitClearAsm}${teleportInitClearAsm}${glideInitClearAsm}${wallJumpInitClearAsm}${powerStompInitClearAsm}${screenShakeInitClearAsm}${airDashInitClearAsm}${carryInitClearAsm}${mazeMovement ? `    ld a, #01
    ld (msx2_player_sprite_dx), a
    ld (msx2_player_sprite_frame), a
` : ''}
    ret
...
${dashRuntimeAsm}
${airDashRuntimeAsm}
${carryObjectRuntimeAsm}
${teleportRuntimeAsm}
${glideRuntimeAsm}
${wallJumpRuntimeAsm}
${powerStompRuntimeAsm}
${screenShakeRuntimeAsm}
${playerBulletCharCoreAsm}
${playerBulletCharPatternAsm}
`;
}
function buildScreen4TileHardwareSpritePattern(tilePattern) {
...
function buildScreen4TileHardwareSpriteColors(tileColors, fallbackColor) {
...
function entityHasPushBoxTileRender(entity) {
    const charRender = entity?.components?.msx2_char_render;
    const tileId = String(charRender?.tileId ?? entity?.params?.tileId ?? '').trim();
    const tileIndex = Number(charRender?.tileIndex ?? entity?.params?.tileIndex);
    return Boolean(tileId) || (Number.isFinite(tileIndex) && tileIndex >= 0);
}
function resolvePushBoxPaletteFromScreens(analysis, tileScreens) {
    for (const screen of tileScreens) {
...
function getPushBoxMovingSpriteLayerFromPlayerPushBox(analysis, tileScreens) {
    for (const screen of tileScreens) {
        if (!screen)
            continue;
        for (const pushBox of getPushBoxComponentsForScreen(analysis, screen)) {
            ...
            if (layer)
                return { pattern: layer.pattern, colors: layer.colors };
        }
    }
    return undefined;
}
function resolvePushBoxHardwareSpriteLayer(analysis, tileScreens) {
    const fromPlayerSprite = getPushBoxMovingSpriteLayerFromPlayerPushBox(analysis, tileScreens);
    if (fromPlayerSprite)
        return fromPlayerSprite;
...
function getPushBoxMovingSpriteLayer(analysis, tileScreens, mapBoxPaletteSlot = 6) {
    const entity = (0, msx2Box2RuntimeGenerator_1.getFirstBox2Entity)(tileScreens);
    if (!entity) {
        for (const screen of tileScreens) {
            const tiles = screen?.tiles || [];
            const map = screen?.map || [];
    ...
    const paletteSlot = Math.max(1, Math.min(15, Number(entity?.components?.msx2_hardware_sprite?.paletteSlot
        ?? entity?.components?.msx2_char_render?.paletteSlot
        ?? entity?.params?.paletteSlot
        ?? 6) || 6));
    const screen = tileScreens.find(candidate => (candidate?.layers?.entities || []).some(item => (0, msx2Box2RuntimeGenerator_1.entityHasMsx2Box2)(item)));
...
function buildHardwareSpriteDataAsm(analysis, options = {}) {
    const sprite = getHardwareSpriteSource(analysis);
    if (!sprite)
        return '';
    const settings = getHardwareSpriteRuntimeSettings(analysis, sprite);
    ...
    const control2Players = usesControl2Players(analysis);
    const enemyHorizontalFacing = !control2Players && enemySprite ? getHorizontalFacingDirection(enemySprite) : undefined;
    const fallbackEnemySpriteLayer = {
    ...
    const enemyAnimationSettings = getEnemyHardwareSpriteAnimationSettings(analysis, enemySprite);
    const enemyAnimationFrameCount = enemyAnimationSettings.frameIndices.length;
    const enemyFrameLayers = enemyAnimationSettings.frameIndices.map(sourceFrameIndex => (enemySprite
...
` : '';
    ...
    const pushBoxLayer = pushBoxEnabled
        ? resolvePushBoxHardwareSpriteLayer(analysis, options.tileScreens || [])
        : undefined;
    // carry_object pattern groups: MUST mirror buildHardwareSpriteRuntimeAsm
    // (same sprite set + same group count) so basePatternIndex clamps equally.
    const carryConfig = (0, msx2PlatformPhysics_1.getMsx2CarryObjectConfigFromPlayerEntity)(getMsx2PlatformPlayerEntity(analysis));
    const carryTileScreens = options.tileScreens?.length ? options.tileScreens : collectReferencedTileScreens(analysis);
    const carrySpriteSet = carryConfig.enabled ? resolveMsx2CarryableSpriteSet(analysis, carryTileScreens) : undefined;
    const carryPatternGroupCount = carrySpriteSet ? carrySpriteSet.uniqueIds.length : 0;
    ...
    const enemyAttributes = Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, () => [
        208,
        0,
        enemyPatternIndex,
        0,
...
function sourceTargetNodeId(connections, nodeId, sourceIds) {
...
function validateMsx2GameFlowSubMenuEdges(graph) {
    ...
    for (const node of nodes) {
        ...
        for (const option of options) {
            const optionConnections = connections.filter((connection) => {
                const fromNodeId = connection.from?.nodeId || connection.fromNodeId;
...
function validateMsx2GameFlowMusicRuntime(graph) {
    const nodes = Array.isArray(graph?.nodes) ? graph.nodes : [];
    for (const node of nodes) {
        if (node?.type !== 'Music')
            continue;
...
function getScreen4RuntimeGameFlow(analysis) {
    const msx2Flow = (analysis.msx2GameFlows || []).find((flow) => flow?.purpose === 'screen4-runtime');
    return msx2Flow || analysis.gameFlow;
}
function getFlowBackgroundScreenAssetId(node) {
    return node?.appearance?.backgroundScreenAssetId || node?.backgroundScreenAssetId || node?.screenAssetId;
}
function isPlayableMsx2TileScreenRuntime(screen) {
    if (!screen?.runtime)
        return false;
    const engine = String(screen.runtime.screenEngine || '').trim();
    if (engine === 'fakePlayer')
        return false;
    if (engine === 'player' || engine === 'shooter')
        return true;
    if (screen.runtime.screenKind === 'playable')
        return true;
...
function buildMsx2EnterGameplayLoopLines(options) {
    const lines = [];
    if (options.refreshSnakeChars)
        lines.push('    call init_msx2_snake_char');
...
function resolveWorldByAssetId(analysis, worldAssetId) {
    if (!worldAssetId)
        return undefined;
    return (analysis.worldmaps || []).find((candidate) => candidate?.id === worldAssetId);
}
...
function getGameFlowWorldAssetId(node) {
...
function getAssetIdFromData(asset) {
    return asset?.id || asset?.data?.id;
}
function addIncludedAsset(included, asset, reason, extra = {}) {
    ...
    if (included.has(key)) {
        ...
        if (ownerWorldIds.length > 0)
            existing.ownerWorldIds = ownerWorldIds;
        if (extra.ownerScreenId && !existing.ownerScreenId)
            existing.ownerScreenId = extra.ownerScreenId;
        return;
    }
    included.set(key, {
        type: asset.type,
...
function estimateSerializedByteSize(value) {
    try {
...
function getStorageDecisionForReadonlyData(rawBytes, accessPattern) {
    if (accessPattern === 'load_to_vram') {
        return rawBytes > 512 ? 'ROM_ZX0_CANDIDATE_TO_VRAM' : 'ROM_RAW_TO_VRAM';
    }
    if (accessPattern === 'runtime_read')
        return 'ROM_RAW';
    if (accessPattern === 'manifest_read' || accessPattern === 'stream_or_runtime_read')
        return 'ROM_RAW';
    if (rawBytes < 64)
        return 'ROM_RAW';
    if (rawBytes <= 512)
        return 'ROM_RAW_UNLESS_ZX0_SAVES_25_PERCENT';
    return 'ROM_ZX0_CANDIDATE';
}
function estimateMsx2ScreenStoragePolicy(screen) {
...
function estimateMsx2SpriteStoragePolicy(analysis, sprite) {
    const settings = getHardwareSpriteRuntimeSettings(analysis, sprite);
    const color = Math.max(1, Math.min(15, settings.color));
    ...
    const authoredParts = Array.isArray(sprite.superSpriteParts) && sprite.superSpriteParts.length
        ...
        : Array.from({ length: cellRows }, (_unusedRow, cellY) => Array.from({ length: cellColumns }, (_unusedColumn, cellX) => ({
    ...
    const hardwareLayerCountsByRow = Array.from({ length: cellRows }, (_unused, cellY) => authoredLayers.filter(layer => layer.yOffset === cellY * 16).length);
...
function buildMsx2AssetStoragePolicy(analysis, includedAssets, tileScreens) {
    ...
    return includedAssets.map(entry => {
        ...
        if (entry.type === 'msx2screen' && screenById.has(entry.id)) {
            ...
            const payloadParts = [
                ...
                { label: `${screenLabel}_VISUAL_MAP`, kind: 'screen4_visual_map', rawBytes: (0, msx2Screen4TileBehavior_1.buildMsx2TileVisualMapBytes)(screenById.get(entry.id)).length, loadOrder: 34 },
                { label: `${screenLabel}_TILE_HAZ_HIT`, kind: 'screen4_hazard_hitbox', rawBytes: (0, msx2Screen4TileBehavior_1.buildMsx2TileHazardHitboxBytes)(screenById.get(entry.id)).length, loadOrder: 35 },
...
function buildMsx2LogicalBankBudget(assetStoragePolicy) {
    const bankSizeBytes = 8192;
    const warningThresholdBytes = Math.floor(bankSizeBytes * 0.9);
    const packPayloadPartsIntoBankChunks = (payloadParts) => {
        ...
        return chunks.map(chunk => [...chunk].sort((left, right) => Number(left.loadOrder || 0) - Number(right.loadOrder || 0)));
    };
    ...
    const packages = originalPackages.flatMap(entry => {
        if (!entry.canSplit || entry.usedBytes <= bankSizeBytes)
            return [entry];
    ...
    const splitPackages = packages.filter(entry => Boolean(entry.splitFrom));
    const isScreen4ChunkPayloadLoaderCovered = (label) => /_BANK_\d+_PATTERNS$/.test(label)
    ...
    const splitChunkManifest = splitPackages.map(entry => {
        const splitIndex = Number(entry.splitIndex || 0);
        const splitCount = Number(entry.splitCount || 1);
        const payloadParts = Array.isArray(entry.payloadParts) ? entry.payloadParts : [];
        const chunkPayloadParts = Array.isArray(entry.chunkPayloadParts) && entry.chunkPayloadParts.length > 0
            ? entry.chunkPayloadParts
            : payloadParts.filter((_part, index) => Math.floor((index * splitCount) / Math.max(1, payloadParts.length)) === splitIndex);
        const payloadLabels = chunkPayloadParts.map((part) => String(part?.label || '')).filter(Boolean);
        const payloadKinds = Array.from(new Set(chunkPayloadParts.map((part) => String(part?.kind || '')).filter(Boolean)));
        const payloadBytes = chunkPayloadParts.reduce((sum, part) => sum + (Number(part?.rawBytes) || 0), 0);
        ...
        const labelStem = sanitizeLabel(`${entry.type}_${entry.sourceId}_chunk_${String(splitIndex).padStart(2, '0')}`, `WORLD_CHUNK_${String(splitIndex).padStart(2, '0')}`);
        return {
            chunkId: entry.id,
            ...
            splitIndex,
            splitCount,
            splitStrategy: entry.splitStrategy || 'auto_world_package_chunk',
            usedBytes: entry.usedBytes,
            recommendedBankClass: entry.recommendedBankClass,
    ...
    const splitSourcePackages = originalPackages.filter(entry => entry.canSplit && entry.usedBytes > bankSizeBytes);
    ...
    const graphicsPressurePackages = packages.filter(entry => (entry.type === 'msx2screen' || entry.type === 'msx2sprite') &&
        (entry.warning || entry.overBudgetBytes > 0));
    const unsplittablePressurePackages = packages.filter(entry => !entry.canSplit && (entry.warning || entry.overBudgetBytes > 0));
...
function buildMsx2WorldPackageSummary(includedAssets, assetStoragePolicy) {
    const includedByKey = new Map(includedAssets.map(asset => [assetKey(asset.type, asset.id), asset]));
    const summaries = new Map();
    const ensureSummary = (worldId) => {
    ...
    for (const policy of assetStoragePolicy) {
        if (policy.decision === 'INHERIT_OWNER_SCREEN_POLICY')
            continue;
...
function buildMsx2WorldBankManifest(includedAssets, assetStoragePolicy, logicalBankBudget, worldPackageSummary, useKonamiDataBank) {
    const bankSizeBytes = Number(logicalBankBudget.bankSizeBytes || 8192);
    const dataWindowAddress = '#8000';
    const includedByKey = new Map(includedAssets.map(asset => [assetKey(asset.type, asset.id), asset]));
    const physicalBankByPackage = new Map();
    for (const bank of (Array.isArray(logicalBankBudget.estimatedPackedBanks) ? logicalBankBudget.estimatedPackedBanks : [])) {
        for (const pkg of (Array.isArray(bank?.packages) ? bank.packages : [])) {
            if (pkg?.id)
                physicalBankByPackage.set(String(pkg.id), Number(bank.bankIndex || 0));
        }
    }
    const logicalPackagesBySource = new Map();
    for (const pkg of (Array.isArray(logicalBankBudget.packages) ? logicalBankBudget.packages : [])) {
        const sourceKey = assetKey(pkg?.type, pkg?.sourceId);
        if (!sourceKey)
            continue;
        if (!logicalPackagesBySource.has(sourceKey))
            logicalPackagesBySource.set(sourceKey, []);
        logicalPackagesBySource.get(sourceKey).push(pkg);
    }
    const logicalSectionForPolicy = (policy) => policy.type === 'msx2screen'
        ? 'world screens'
        : policy.type === 'msx2sprite'
            ? 'world graphics'
            : policy.type === 'track'
                ...
                : policy.type === 'worldmap'
                    ? 'world manifest'
                    : 'world manifest';
    const bankClassForPolicy = (policy) => policy.type === 'msx2screen'
        ? 'world.screen'
        : policy.type === 'msx2sprite'
            ? 'world.graphics.sprite'
            : 'world.manifest';
    ...
    for (const policy of assetStoragePolicy) {
        if (!policy || policy.decision === 'INHERIT_OWNER_SCREEN_POLICY')
            continue;
        if ((Number(policy.storedBytesEstimate) || 0) <= 0)
            continue;
        const included = includedByKey.get(assetKey(policy.type, policy.id));
        const ownerWorldIds = policy.type === 'worldmap'
            ? [policy.id]
...
function buildMsx2Screen4DataBankPlan(tileScreens, tileScreenLoadLabels, logicalBankBudget) {
    const estimatedPackedBanks = Array.isArray(logicalBankBudget.estimatedPackedBanks)
        ? logicalBankBudget.estimatedPackedBanks
        : [];
    const splitPackages = Array.isArray(logicalBankBudget.splitPackages)
        ? logicalBankBudget.splitPackages
        : [];
    const splitChunkManifest = Array.isArray(logicalBankBudget.splitChunkManifest)
        ? logicalBankBudget.splitChunkManifest
        : [];
    const packageBankById = new Map();
    const packageIdsByBank = new Map();
    for (const bank of estimatedPackedBanks) {
        ...
        for (const pkg of (Array.isArray(bank?.packages) ? bank.packages : [])) {
            ...
            if (!packageIdsByBank.has(bankIndex))
                packageIdsByBank.set(bankIndex, []);
            packageIdsByBank.get(bankIndex).push(id);
        }
    }
    const screenPackageIds = new Set();
    const screenBankIndexByLabel = new Map();
    const screenPackageIdByLabel = new Map();
    tileScreens.forEach((screen, index) => {
...
function buildMsx2ProjectSliceJson(projectName, analysis, config, tileScreens, runtimeRamEnd, useKonamiDataBank) {
    const assets = (analysis.assets || []);
    ...
    const attackProfiles = new Set();
    const screenIds = new Set(tileScreens.map(screen => screen.id).filter(Boolean));
    const screenOwnerWorldIds = new Map();
    const spriteOwnerWorldIds = new Map();
    const playerRecords = getMsx2PlayerAssetRecords(analysis);
    const shooter60HzScreens = tileScreens
        .filter(screen => screen.runtime?.screenEngine === 'shooter' || Boolean(screen.runtime?.shooter))
        .map(screen => {
        const shooter = (0, msx2ShooterRuntime_1.normalizeMsx2ShooterRuntimeConfig)(screen.runtime?.shooter);
        ...
        return {
            ...
            validation: (0, msx2ShooterRuntime_1.validateMsx2Shooter60HzBudget)(shooter),
        };
    });
    const includeByTypeAndId = (type, id, reason, extra = {}) => {
        if (!id)
            return;
        const asset = assets.find(candidate => candidate?.type === type && (candidate.id === id || candidate.data?.id === id));
        if (asset)
            addIncludedAsset(included, asset, reason, extra);
    };
    const addReferencedSpriteId = (spriteAssetId, ownerWorldIds) => {
        const id = String(spriteAssetId || '').trim();
        if (!id)
            return;
        spriteIds.add(id);
        if (ownerWorldIds.length > 0) {
            if (!spriteOwnerWorldIds.has(id))
                spriteOwnerWorldIds.set(id, new Set());
            for (const worldId of ownerWorldIds)
                spriteOwnerWorldIds.get(id).add(worldId);
        }
    };
    ...
    for (const worldId of worldIds) {
        ...
        for (const worldNode of world?.nodes || []) {
            ...
            if (screenId) {
                ...
                if (!screenOwnerWorldIds.has(screenId))
                    screenOwnerWorldIds.set(screenId, new Set());
    ...
    for (const screen of tileScreens) {
        ...
        for (const record of referencedPlayers) {
            ...
            for (const componentId of Object.keys(record.player.components || {}))
                componentIds.add(componentId);
            addReferencedSpriteId(String(record.player.components?.msx2_hardware_sprite?.msx2SpriteAssetId
        ...
        for (const entity of screen.layers?.entities || []) {
            const referencedSpriteIds = [
                ...
                (0, msx2Box2RuntimeGenerator_1.playerHasMsx2PushBox)(entity)
                    ? String(entity?.components?.msx2_push_box?.msx2SpriteAssetId ?? entity?.components?.msx2_push_box?.spriteAssetId ?? '').trim()
                    : '',
                (0, msx2Box2RuntimeGenerator_1.entityHasMsx2Box2)(entity)
            ...
            const movement = String(entity.components?.msx2_movement?.mode
                ?? entity.params?.movement
                ?? entity.params?.movementMode
                ...
                ?? '').trim();
            if (movement)
                movementModes.add(movement);
            const attack = String(entity.components?.msx2_attack_pattern?.pattern
                ?? entity.params?.attackPattern
                ?? '').trim();
            if (attack)
                attackProfiles.add(attack);
        }
    }
    ...
    const runtimeModuleCandidates = [
        ...
        { id: 'runtime.msx2.box2', enabled: (0, msx2Box2ComponentGenerator_1.usesMsx2Box2)(analysis, tileScreens), placement: 'resident', reason: 'Enabled when reachable screens contain msx2_box2 or legacy msx2_push_box entities' },
    ...
    const worldBankManifest = buildMsx2WorldBankManifest(includedAssetList, assetStoragePolicy, logicalBankBudget, worldPackageSummary, useKonamiDataBank);
    const ramBudget = buildMsx2RamBudget(tileScreens, runtimeRamEnd, shooter60HzScreens[0]?.pools
        ? {
    ...
    const artifact = {
        ...
        shooter60Hz: {
            ...
            frameBudget: (() => {
                ...
                if (!resolvedShooterBudget)
                    return null;
                return (0, msx2ShooterRuntime_1.buildMsx2Shooter60HzFrameBudgetSummary)(resolvedShooterBudget, {
                    scrollRowRoutine: (0, msx2ShooterRuntime_1.resolveMsx2ShooterScrollRowRoutine)(resolvedShooterBudget, {
            ...
            warnings: shooter60HzScreens.flatMap(screen => (screen.validation || [])
                .filter(issue => issue.severity === 'warning')
                .map(issue => ({ screenId: screen.screenId, ...issue }))),
            errors: shooter60HzScreens.flatMap(screen => (screen.validation || [])
...
function buildSnakeCharRuntimeAsm(analysis) {
    if (!usesSnakeCharMovement(analysis))
        return '';
    ...
    const runtimeCharCopy = (label, charCode, tableBase, byteCount = 8) => [0x0000, 0x0800, 0x1000].map(bankOffset => `    ld hl, ${label}
...
function buildMsx2TileScreenLoadLines(label, tileScreenIndexByLabel, refreshHardwareSprites = false) {
    if (!label)
        return '';
...
function buildLoadCurrentTileScreenDispatcher(tileScreenLoadLabels) {
...
function buildScreen4BackgroundScrollAsm(tileScreenLoadLabels, useKonamiDataBank, options = {}) {
    ...
    if (!includeHorizontalR23 && !includeVerticalScrollRow)
        return '';
...
function normalizeMsx2GameFlowGlobalName(name) {
    return typeof name === 'string' ? name.trim() : '';
}
function buildMsx2GameFlowGlobalAsmName(variableName) {
    return `global_var_${variableName.replace(/([A-Z])/g, '_$1').toLowerCase().replace(/^_/, '').replace(/[^a-z0-9_]/g, '_')}`;
}
function resolveMsx2GameFlowGlobalInfo(variableName, analysis) {
    const normalizedName = normalizeMsx2GameFlowGlobalName(variableName);
    const variables = Array.isArray(analysis.globalVariables) ? analysis.globalVariables : [];
    const found = variables.find((variable) => String(variable?.name || '').toLowerCase() === normalizedName.toLowerCase());
...
function parseMsx2GameFlowGlobalValue(rawValue, values, isWord) {
...
function getMsx2GameFlowOperatorId(operator) {
    switch (operator) {
        case '!=': return 1;
        case '>': return 2;
        case '<': return 3;
...
function buildMsx2GameFlowGlobalMap(graph, analysis, baseAddress) {
    const globals = new Map();
    let address = baseAddress;
    const includeName = (rawName) => {
        const variableName = normalizeMsx2GameFlowGlobalName(rawName);
        if (!variableName || globals.has(variableName.toLowerCase()))
            return;
    ...
    (graph?.nodes || []).forEach((node) => {
        if (node?.type === 'Globals') {
            (Array.isArray(node.variables) ? node.variables : []).forEach((variable) => includeName(variable?.name));
        }
...
function buildMsx2GameFlowGlobalEquates(globals) {
    if (globals.size === 0)
        return '';
    return Array.from(globals.values()).map(global => `${global.asmName} EQU ${formatHexWord(global.address)}    ; MSX2 SCREEN 4 GameFlow global: ${global.variableName}`).join('\n') + '\n';
}
function estimateMsx2GameFlowRuntimeRamEnd(analysis, baseAddress) {
    const graph = getScreen4RuntimeGameFlow(analysis);
    if (!graph?.nodes?.length)
        return baseAddress;
    const globals = buildMsx2GameFlowGlobalMap(graph, analysis, baseAddress);
    return Array.from(globals.values()).reduce((end, global) => Math.max(end, global.address + (global.isWord ? 2 : 1)), baseAddress);
}
function buildMsx2GameFlowGlobalInitLines(globals) {
    const lines = [];
...
function buildMsx2GameFlowGlobalsApplyLines(node, globals) {
    const lines = [];
...
function buildMsx2GameFlowCompareRoutine(needed) {
    if (!needed)
        return '';
    return `
...
function buildMsx2GameFlowTransitionWaitLines(label, durationFrames) {
...
function buildMsx2GameFlowTransitionLines(effect, label, durationFrames) {
    const lines = ['    call load_msx2_hud_font'];
    const nameAddr = (offset) => formatHexWord(0x1800 + offset);
...
function buildMsx2GameFlowProgram(analysis, screenLabels, tileScreenLabels, tileScreenIndexByLabel, globalBaseAddress) {
    const graph = getScreen4RuntimeGameFlow(analysis);
    ...
    const hasIfThenElse = graph.nodes.some((node) => node?.type === 'IfThenElse');
    const labelForNodeId = (nodeId) => nodeId ? nodeLabels.get(nodeId) : undefined;
    const jumpToNodeOrMain = (nodeId) => {
    ...
    const emitNode = (current) => {
        if (!current || emitted.has(current.id))
            return;
        ...
        switch (current.type) {
            ...
            case 'SubMenu': {
                ...
                if (backgroundLabel)
                    lines.push(buildMsx2TileScreenLoadLines(backgroundLabel, tileScreenIndexByLabel, refreshHardwareSprites).trimEnd());
        ...
        const nextNodeIds = [
            ...
            ...(Array.isArray(current.options) ? current.options.map((option) => sourceTargetNodeId(graph.connections, current.id, option?.id)) : []),
        ].filter(Boolean);
...
function buildMsx2WorldTransitionAsm(analysis, tileScreens, tileScreenLoadLabels) {
    const mazeMovement = usesMazeMovement(analysis);
    const screenIndexById = new Map();
    tileScreens.forEach((screen, index) => {
        if (screen.id)
            screenIndexById.set(screen.id, index);
    });
    const transitions = new Map();
    const setTransition = (fromIndex, direction, toIndex) => {
    ...
    for (const node of graph?.nodes || []) {
        ...
        if (!world?.nodes?.length)
            continue;
        const worldNodeById = new Map((world.nodes || []).map((worldNode) => [String(worldNode?.id || ''), worldNode]));
        for (const connection of world.connections || []) {
            ...
            if (fromIndex === undefined || toIndex === undefined)
                continue;
            if (connection?.fromDirection === 'west'
            ...
            if (connection?.toDirection === 'west'
    ...
    const buildDirectionRoutine = (direction) => {
...
function generateMsx2Screen4UnitedFiles(projectName, analysis, config) {
    ...
    const tileScreenIndexByLabel = new Map();
    ...
    const tileScreenRuntimeBlocks = tileScreens.map((screen, index) => {
        ...
        return [
            ...
            useKonamiDataBank ? '' : formatBytes(`${label}_VISUAL_MAP`, (0, msx2Screen4TileBehavior_1.buildMsx2TileVisualMapBytes)(screen), `${screen?.name || `MSX2 Tile Screen ${index}`} visual tile index map, 16x12 bytes`),
            useKonamiDataBank ? '' : formatBytes(`${label}_TILE_HAZ_HIT`, (0, msx2Screen4TileBehavior_1.buildMsx2TileHazardHitboxBytes)(screen, maxTileCount), `${screen?.name || `MSX2 Tile Screen ${index}`} per-tile hazard hitboxes (ox, oy, w, h)`),
        ].filter(Boolean).join('\n');
    });
    ...
    const pushBoxMovement = (0, msx2Box2ComponentGenerator_1.usesMsx2Box2)(analysis, tileScreens);
    const platformPlayer = getMsx2PlatformPlayerEntity(analysis);
    const dashConfig = (0, msx2PlatformPhysics_1.getMsx2DashConfigFromPlayerEntity)(platformPlayer);
    const airDashConfig = (0, msx2PlatformPhysics_1.getMsx2AirDashConfigFromPlayerEntity)(platformPlayer);
    const teleportConfig = (0, msx2PlatformPhysics_1.getMsx2TeleportABConfigFromPlayerEntity)(platformPlayer);
    ...
    const screenShakeEnabled = (0, msx2PlatformPhysics_1.msx2PlayerWantsScreenShake)(platformPlayer);
    const carryObjectConfig = (0, msx2PlatformPhysics_1.getMsx2CarryObjectConfigFromPlayerEntity)(platformPlayer);
    const skillRamOptions = (0, msx2SkillRamLayout_1.buildMsx2SkillRamOptions)(pushBoxMovement, dashConfig.enabled, teleportConfig.enabled, glideConfig.enabled, wallJumpConfig.enabled, powerStompConfig.enabled, screenShakeEnabled, airDashConfig.enabled, carryObjectConfig.enabled);
    ...
    const playerTimersRamBase = (0, msx2SkillRamLayout_1.resolveMsx2PlayerTimersRamBase)(pushBoxMovement);
...
`;
    const dashEquatesAsm = dashConfig.enabled
        ? (0, msx2DashGenerator_1.buildMsx2DashEquates)((0, msx2DashGenerator_1.resolveMsx2DashRamBase)(pushBoxMovement))
        : '';
    const teleportEquatesAsm = teleportConfig.enabled
        ? (0, msx2TeleportABGenerator_1.buildMsx2TeleportABEquates)((0, msx2TeleportABGenerator_1.resolveMsx2TeleportABRamBase)(pushBoxMovement, dashConfig.enabled))
        : '';
    const glideEquatesAsm = glideConfig.enabled
        ? (0, msx2GlideGenerator_1.buildMsx2GlideEquates)((0, msx2GlideGenerator_1.resolveMsx2GlideRamBase)((0, msx2SkillRamLayout_1.buildMsx2SkillRamOptions)(pushBoxMovement, dashConfig.enabled, teleportConfig.enabled, false, false)))
        : '';
    const wallJumpEquatesAsm = wallJumpConfig.enabled
        ? (0, msx2WallJumpGenerator_1.buildMsx2WallJumpEquates)((0, msx2WallJumpGenerator_1.resolveMsx2WallJumpRamBase)((0, msx2SkillRamLayout_1.buildMsx2SkillRamOptions)(pushBoxMovement, dashConfig.enabled, teleportConfig.enabled, glideConfig.enabled, false)))
        : '';
    // power_stomp comes after wall_jump in the chain: pass wallJumpEnabled but
    // NOT powerStompEnabled (the base is "where power_stomp starts").
    const powerStompEquatesAsm = powerStompConfig.enabled
        ? (0, msx2PowerStompGenerator_1.buildMsx2PowerStompEquates)((0, msx2PowerStompGenerator_1.resolveMsx2PowerStompRamBase)((0, msx2SkillRamLayout_1.buildMsx2SkillRamOptions)(pushBoxMovement, dashConfig.enabled, teleportConfig.enabled, glideConfig.enabled, wallJumpConfig.enabled, false, false)))
        : '';
    // screen_shake comes after power_stomp: pass powerStompEnabled but NOT
    // screenShakeEnabled (the base is "where screen_shake starts").
    const screenShakeEquatesAsm = screenShakeEnabled
        ? (0, msx2ScreenShakeGenerator_1.buildMsx2ScreenShakeEquates)((0, msx2ScreenShakeGenerator_1.resolveMsx2ScreenShakeRamBase)((0, msx2SkillRamLayout_1.buildMsx2SkillRamOptions)(pushBoxMovement, dashConfig.enabled, teleportConfig.enabled, glideConfig.enabled, wallJumpConfig.enabled, powerStompConfig.enabled, false)))
        : '';
    ...
    const airDashEquatesAsm = airDashConfig.enabled
        ? (0, msx2AirDashGenerator_1.buildMsx2AirDashEquates)((0, msx2AirDashGenerator_1.resolveMsx2AirDashRamBase)((0, msx2SkillRamLayout_1.buildMsx2SkillRamOptions)(pushBoxMovement, dashConfig.enabled, teleportConfig.enabled, glideConfig.enabled, wallJumpConfig.enabled, powerStompConfig.enabled, screenShakeEnabled, false)))
        : '';
    // carry_object comes after air_dash: pass airDashEnabled but NOT
    // carryObjectEnabled (the base is "where carry_object starts").
    const carryObjectEquatesAsm = carryObjectConfig.enabled
        ? (0, msx2CarryObjectGenerator_1.buildMsx2CarryObjectEquates)((0, msx2CarryObjectGenerator_1.resolveMsx2CarryObjectRamBase)((0, msx2SkillRamLayout_1.buildMsx2SkillRamOptions)(pushBoxMovement, dashConfig.enabled, teleportConfig.enabled, glideConfig.enabled, wallJumpConfig.enabled, powerStompConfig.enabled, screenShakeEnabled, airDashConfig.enabled, false)))
        : '';
    (0, msx2SkillRamLayout_1.assertMsx2SkillRamWithinLimit)((0, msx2SkillRamLayout_1.resolveMsx2SkillExtensionRamBase)(skillRamOptions), 'player timers + dash/teleport/glide/wall_jump/power_stomp/screen_shake/air_dash/carry_object skill RAM chain');
    const pushBoxVerticalPush = (0, msx2Box2RuntimeGenerator_1.usesMsx2Box2VerticalPush)(tileScreens);
    ...
    const hudPlayerEnergyInitialByScreen = tileScreens.map((screen, index) => Math.min(hudPlayerEnergyMaxByScreen[index] ?? 16, getTileScreenRuntimeByte(screen, 'playerEnergyInitial', hudPlayerEnergyMaxByScreen[index] ?? 16, 1)));
    const hudBossEnergyMaxByScreen = tileScreens.map(screen => getTileScreenRuntimeByte(screen, 'bossEnergyMax', 16, 1));
    const hudBossEnergyInitialByScreen = tileScreens.map((screen, index) => Math.min(hudBossEnergyMaxByScreen[index] ?? 16, getTileScreenRuntimeByte(screen, 'bossEnergyInitial', hudBossEnergyMaxByScreen[index] ?? 16, 1)));
    ...
    const hudWidgetIconTileByWidget = hudWidgetsFlat.map(widget => hudWidgetKindId(widget) === MSX2_HUD_WIDGET_KIND_IDS.icon
        ? hudWidgetByte(widget, 'iconTileIndex', 0)
        : 0xff);
    const hudWidgetTextBytesByWidget = hudWidgetsFlat.map(widget => hudWidgetKindId(widget) === MSX2_HUD_WIDGET_KIND_IDS.text
        ? encodeHudAscii(widget?.text || widget?.name || '', 31)
        : []);
    const hudWidgetTextPool = [0];
    const hudWidgetTextOffsetByWidget = hudWidgetTextBytesByWidget.map(bytes => appendHudStringPoolEntry(hudWidgetTextPool, bytes));
    const hudWidgetTextLengthByWidget = hudWidgetTextBytesByWidget.map(bytes => bytes.length);
    const hudWidgetVariableNameBytesByWidget = hudWidgetsFlat.map(widget => hudWidgetBindingId(widget) === MSX2_HUD_WIDGET_BINDING_IDS.custom
        ? encodeHudAscii(widget?.variableName || widget?.name || '', 31, true)
        : []);
    const hudWidgetVariableNamePool = [0];
    const hudWidgetVariableNameOffsetByWidget = hudWidgetVariableNameBytesByWidget.map(bytes => appendHudStringPoolEntry(hudWidgetVariableNamePool, bytes));
    ...
    if (runtimeRamEnd > MSX2_RUNTIME_RAM_LIMIT) {
        ...
        throw new Error(`MSX2 SCREEN 4 runtime RAM overflow: ${tileScreens.length} native msx2screen rooms require ` +
    ...
    const enemyHazards = tileScreens.map(screen => (0, msx2EntityRuntimeGenerator_1.getMsx2EnemyHazardRuntimeSlots)(screen));
    const enemyCountBytes = enemyHazards.map(enemies => Math.min(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN, enemies.length));
    const enemyXBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => clampHardwareSpriteX(enemies[index]?.x ?? 0)));
    const enemyYBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => clampHardwareSpriteY(enemies[index]?.y ?? 0)));
    const enemyMinXBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => clampHardwareSpriteX(enemies[index]?.minX ?? 0)));
    const enemyMaxXBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => clampHardwareSpriteX(enemies[index]?.maxX ?? 0)));
    const enemyMinYBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => clampHardwareSpriteY(enemies[index]?.minY ?? 0)));
    const enemyMaxYBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => clampHardwareSpriteY(enemies[index]?.maxY ?? 0)));
    const signedRuntimeByte = (value) => {
    ...
    const enemyDxBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => signedRuntimeByte(enemies[index]?.dx)));
    const enemyDyBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => signedRuntimeByte(enemies[index]?.dy)));
    const enemyModeBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => Math.max(0, Math.min(255, enemies[index]?.mode ?? 0))));
    const enemySpeedBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => Math.max(1, Math.min(240, enemies[index]?.speed ?? 2))));
    const enemyScoreBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => Math.max(1, Math.min(255, enemies[index]?.score ?? 1))));
    const enemyDropBombBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => enemies[index]?.dropBombOnPlayerX ? 1 : 0));
    const enemyDropBombTableAsm = enemyDropBombBytes.some(byte => byte !== 0)
        ? formatBytes('msx2_screen_enemy_drop_bomb', enemyDropBombBytes, `Per-msx2screen "drop bomb when aligned with player X" flag, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)
        : '';
    const enemyStateSwitchBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => enemies[index]?.stateSwitch ? 1 : 0));
    const enemyStateNearModeBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => Math.max(0, Math.min(255, enemies[index]?.stateNearMode ?? 0))));
    const enemyStateFarModeBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => Math.max(0, Math.min(255, enemies[index]?.stateFarMode ?? enemies[index]?.mode ?? 0))));
    const enemyStateRangeXBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => Math.max(0, Math.min(255, enemies[index]?.stateRangeX ?? 0))));
    const enemyStateRangeYBytes = enemyHazards.flatMap(enemies => Array.from({ length: msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN }, (_unused, index) => Math.max(0, Math.min(191, enemies[index]?.stateRangeY ?? 0))));
    const enemyStateSwitchTablesAsm = enemyStateSwitchBytes.some(byte => byte !== 0) ? `${formatBytes('msx2_screen_enemy_state_switch', enemyStateSwitchBytes, `Per-msx2screen enemy behavior state-switch flag, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_state_near_mode', enemyStateNearModeBytes, `Per-msx2screen enemy PlayerNear target movement mode, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_state_far_mode', enemyStateFarModeBytes, `Per-msx2screen enemy return movement mode, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_state_range_x', enemyStateRangeXBytes, `Per-msx2screen enemy PlayerNear horizontal range in pixels, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_state_range_y', enemyStateRangeYBytes, `Per-msx2screen enemy PlayerNear vertical range in pixels, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
` : '';
    ...
    const carrySpriteSetForTables = carryObjectConfig.enabled
        ? resolveMsx2CarryableSpriteSet(analysis, tileScreens)
        : undefined;
    const carrySlotsByScreen = tileScreens.map(screen => (0, msx2CarryObjectGenerator_1.getMsx2CarryableRuntimeSlots)(screen));
    const carryObjectDataTablesAsm = carryObjectConfig.enabled && carrySpriteSetForTables
        ? (0, msx2CarryObjectGenerator_1.buildMsx2CarryObjectDataTablesAsm)({
            countByScreen: carrySlotsByScreen.map(slots => Math.min(msx2CarryObjectGenerator_1.MSX2_MAX_CARRYABLES_PER_SCREEN, slots.length)),
            xBytes: carrySlotsByScreen.flatMap(slots => Array.from({ length: msx2CarryObjectGenerator_1.MSX2_MAX_CARRYABLES_PER_SCREEN }, (_unused, index) => clampHardwareSpriteX(slots[index]?.x ?? 0))),
            yBytes: carrySlotsByScreen.flatMap(slots => Array.from({ length: msx2CarryObjectGenerator_1.MSX2_MAX_CARRYABLES_PER_SCREEN }, (_unused, index) => clampHardwareSpriteY(slots[index]?.y ?? 0))),
            spriteIdxBytes: carrySlotsByScreen.flatMap(slots => Array.from({ length: msx2CarryObjectGenerator_1.MSX2_MAX_CARRYABLES_PER_SCREEN }, (_unused, index) => {
                const id = slots[index]?.spriteId || '';
                const uniqueIndex = carrySpriteSetForTables.uniqueIds.indexOf(id);
                return uniqueIndex >= 0 ? uniqueIndex : 0;
            })),
            colorRows: carrySpriteSetForTables.uniqueIds.flatMap(id => carrySpriteSetForTables.colorsById.get(id) || Array(16).fill(msx2CarryObjectGenerator_1.MSX2_CARRY_DEFAULT_COLOR)),
        })
        : '';
    ...
    const includeGameFlowTransitionHelpers = Boolean(screen4GameFlow?.nodes?.some((node) => node?.type === 'Transition'));
    const includeGameFlowMusicHelpers = Boolean(screen4GameFlow?.nodes?.some((node) => node?.type === 'Music'));
    ...
    const shooter60HzBudget = getMsx2Shooter60HzBudgetFromAnalysis(analysis);
    const shooterActiveIrqProfile = shooter60HzBudget?.budget.irqProfiles.find(profile => profile.id === shooter60HzBudget.budget.activeIrqProfile);
    const shooterSatUploadInFrameDispatch = Boolean(shooter60HzBudget
    ...
    const hardwareSpriteRuntimeAsm = hasHardwareSprite(analysis)
        ? buildHardwareSpriteRuntimeAsm(analysis, requiredCollectibles, firstScreenLabel, firstScreenIndex ?? 0, tileScreens.length, { deferSatUploadToShooterFrameDispatch: shooterSatUploadInFrameDispatch, pushBoxEnabled: pushBoxMovement, tileScreens })
        : `upload_hardware_sprite_attrs:
write_hardware_sprite_attrs:
update_hardware_sprite_input:
update_msx2_air_timer:
...
`;
    ...
    const shooterVerticalTileScroll = Boolean(shooter60HzBudget
    ...
    const shooter60HzFrameDispatchAsm = shooter60HzBudget
        ? (0, msx2ShooterRuntime_1.buildMsx2Shooter60HzFrameDispatchAsm)({
    ...
    const shooter60HzHelperAsm = shooter60HzBudget
        ? [
            (0, msx2ShooterRuntime_1.buildMsx2ShooterMusicTickAsm)({ snakeMusic: snakeCharMovement }),
            (0, msx2ShooterRuntime_1.buildMsx2ShooterHudDirtyTaskAsm)({ enabled: shooterActiveIrqProfile?.tasks.includes('hud_dirty') }),
            (0, msx2ShooterRuntime_1.buildMsx2ShooterPaletteSmallTaskAsm)({ enabled: shooterActiveIrqProfile?.tasks.includes('palette_small') }),
        ].join('')
        : '';
    ...
    const pushBoxRuntimeAsm = (0, msx2Box2ComponentGenerator_1.buildMsx2Box2RuntimeAsm)({
    ...
    const box2SlotsByScreen = pushBoxMovement
        ? tileScreens.map(screen => (0, msx2Box2RuntimeGenerator_1.getMsx2Box2RuntimeSlotsForScreen)(screen, getScreen4TileBytesForEntity, buildMsx2Box2NameLayout(screen)))
        : undefined;
    const pushBoxDataTablesAsm = (0, msx2Box2ComponentGenerator_1.buildMsx2Box2DataTables)(tileScreens, getScreen4TileBytesForEntity, box2SlotsByScreen);
...
`;
    ...
    const screen4DataBankPlan = buildMsx2Screen4DataBankPlan(tileScreens, tileScreenLoadLabels, projectSliceData.logicalBankBudget || {});
    ...
    const tileScreenVisualMapBlocks = tileScreens.map((screen, index) => {
        ...
        return {
            ...
            asm: formatBytes(`${label}_VISUAL_MAP`, (0, msx2Screen4TileBehavior_1.buildMsx2TileVisualMapBytes)(screen), `${screen?.name || `MSX2 Tile Screen ${index}`} visual tile index map copied to RAM on screen load`),
        };
    });
    const tileScreenHazardHitboxBlocks = tileScreens.map((screen, index) => {
        ...
        return {
            ...
            asm: formatBytes(`${label}_TILE_HAZ_HIT`, (0, msx2Screen4TileBehavior_1.buildMsx2TileHazardHitboxBytes)(screen, maxTileCount), `${screen?.name || `MSX2 Tile Screen ${index}`} per-tile hazard hitboxes copied to RAM on screen load`),
        };
    });
    const projectSliceArtifact = (0, megaromResourceArtifacts_1.renderNamedArtifactAsCommentBlock)('project_slice.json', JSON.stringify(projectSliceData, null, 2) + '\n');
    const assetStoragePolicyArtifact = (0, megaromResourceArtifacts_1.renderNamedArtifactAsCommentBlock)('asset_storage_policy.json', JSON.stringify(projectSliceData.assetStoragePolicy, null, 2) + '\n');
    const logicalBankBudgetArtifact = (0, megaromResourceArtifacts_1.renderNamedArtifactAsCommentBlock)('logical_bank_budget.json', JSON.stringify(projectSliceData.logicalBankBudget, null, 2) + '\n');
    const worldBankManifestArtifact = (0, megaromResourceArtifacts_1.renderNamedArtifactAsCommentBlock)('msx2_world_bank_manifest.json', JSON.stringify(projectSliceData.worldBankManifest, null, 2) + '\n');
    const ramBudgetArtifact = (0, megaromResourceArtifacts_1.renderNamedArtifactAsCommentBlock)('ram_budget.json', JSON.stringify(projectSliceData.ramBudget, null, 2) + '\n');
    const shooter60HzConstantsAsm = shooter60HzBudget
        ? (0, msx2ShooterRuntime_1.buildMsx2Shooter60HzConstantsAsm)(shooter60HzBudget)
        : '';
    const chunkDataBankByPayloadLabel = new Map();
    screen4DataBankPlan.splitChunkManifest.forEach(chunk => {
        ...
        (Array.isArray(chunk?.payloadLabels) ? chunk.payloadLabels : [])
            .map((payloadLabel) => String(payloadLabel || ''))
            .filter(Boolean)
            .forEach(payloadLabel => chunkDataBankByPayloadLabel.set(payloadLabel, dataBankSymbol));
    });
    const getScreenPayloadDataBankConstants = (label) => {
    ...
    const loadRuntimeLayerPointers = (label, screenIndex, payloadDataBankConstants) => {
        ...
        const copyRuntimeLayerFromBank = (payloadLabel, destinationLabel) => {
...
`
    ...
    ld (msx2_current_behavior_ptr), hl
`;
        const copyRuntimeBytesFromBank = (payloadLabel, destinationLabel, bytes) => {
            const bankConstant = payloadDataBankConstants?.get(payloadLabel) || `${label}_DATA_BANK`;
...
`;
    ...
    const tileScreenLoadRoutines = tileScreens.map((screen, index) => buildTileScreenLoadRoutine(tileScreenLoadLabels[index], screen, index, (label, screenIndex) => loadRuntimeLayerPointers(label, screenIndex, getScreenPayloadDataBankConstants(label)), tileScreenAfterPatternLoad, `    call load_msx2_hud_font\n    call draw_${tileScreenLoadLabels[index]}_hud_text\n`, useKonamiDataBank, `${tileScreenLoadLabels[index]}_DATA_BANK`, getScreenPayloadDataBankConstants(tileScreenLoadLabels[index])));
    const screen4DataBankEquates = useKonamiDataBank
        ? [
            'MSX2_SCREEN4_DATA_BANK EQU 4',
            ...screen4DataBankPlan.bankIndexes.map(bankIndex => `MSX2_SCREEN4_DATA_BANK_${bankIndex} EQU ${4 + bankIndex}`),
            ...tileScreenLoadLabels.map(label => `${label}_DATA_BANK EQU MSX2_SCREEN4_DATA_BANK_${screen4DataBankPlan.screenBankIndexByLabel.get(label) ?? 0}`),
    ...
    const tileScreenHudTextRoutines = tileScreens.map((screen, index) => buildMsx2HudTextRoutines(tileScreenLoadLabels[index], screen, analysis));
    const splitChunkLoaderRoutines = useKonamiDataBank
        ? screen4DataBankPlan.splitChunkManifest.map(chunk => {
            ...
            const dataBankSymbol = String(chunk?.dataBankSymbol || '');
            if (!loaderSymbol || !dataBankSymbol)
                return '';
            return `${loaderSymbol}:
    ld a, ${dataBankSymbol}
    call msx2_screen4_data_bank_enter_selected
...
`;
    ...
    const tileScreenCollectedVisualRoutines = tileScreens.map((screen, index) => buildTileScreenCollectedVisualsRoutine(tileScreenLoadLabels[index], screen, index, effectRuntimeBase));
    const chunkPayloadLabelSet = new Set(screen4DataBankPlan.splitChunkManifest.flatMap(chunk => Array.isArray(chunk?.payloadLabels)
        ? chunk.payloadLabels.map((payloadLabel) => String(payloadLabel || '')).filter(Boolean)
        : []));
    const coldPayloadBlocks = new Map();
    ...
    const splitChunkSectionsByBank = new Map();
    screen4DataBankPlan.splitChunkManifest.forEach(chunk => {
        const bankIndex = Number(chunk?.bankIndex || 0);
        const dataLabel = String(chunk?.dataLabel || '');
        const dataEndLabel = String(chunk?.dataEndLabel || '');
        if (!dataLabel || !dataEndLabel)
            return;
        const payloadAsms = (Array.isArray(chunk?.payloadLabels) ? chunk.payloadLabels : [])
            .map((payloadLabel) => coldPayloadBlocks.get(String(payloadLabel || ''))?.asm || '')
            .filter(Boolean);
        const section = `${dataLabel}:
${payloadAsms.join('\n')}
${dataEndLabel}:`;
        if (!splitChunkSectionsByBank.has(bankIndex))
            splitChunkSectionsByBank.set(bankIndex, []);
...
${playerTimerEquatesAsm}${dashEquatesAsm}${teleportEquatesAsm}${glideEquatesAsm}${wallJumpEquatesAsm}${powerStompEquatesAsm}${screenShakeEquatesAsm}${airDashEquatesAsm}${carryObjectEquatesAsm}msx2_player_gravity_vel EQU #C008
...
${pushBoxMovement ? (0, msx2Box2ComponentGenerator_1.buildMsx2Box2Equates)(MSX2_SNAKE_BODY_BASE) : ''}msx2_attack_timer EQU #C03C
...
${snakeCharMovement ? `msx2_snake_body_cells EQU ${formatHexWord(MSX2_SNAKE_BODY_BASE)}\n` : pushBoxMovement ? `; msx2_snake_body_cells region reused by box2 runtime (#${MSX2_SNAKE_BODY_BASE.toString(16).toUpperCase()}-#${(MSX2_SNAKE_BODY_BASE + msx2Box2ComponentGenerator_1.MSX2_BOX2_RUNTIME_BYTES - 1).toString(16).toUpperCase()})\n` : ''}msx2_effects_runtime_buffers EQU ${formatHexWord(effectRuntimeBase)}
...
msx2_enemy_runtime_y EQU ${formatHexWord(enemyRuntimeBase + msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN)}
msx2_enemy_runtime_dx EQU ${formatHexWord(enemyRuntimeBase + (msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN * 2))}
msx2_enemy_runtime_dy EQU ${formatHexWord(enemyRuntimeBase + (msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN * 3))}
msx2_enemy_runtime_mode EQU ${formatHexWord(enemyRuntimeBase + (msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN * 4))}
msx2_enemy_runtime_speed EQU ${formatHexWord(enemyRuntimeBase + (msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN * 5))}
msx2_enemy_runtime_tick EQU ${formatHexWord(enemyRuntimeBase + (msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN * 6))}
msx2_enemy_anim_counter EQU ${formatHexWord(enemyRuntimeBase + (msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN * 7))}
msx2_enemy_anim_frame EQU ${formatHexWord(enemyRuntimeBase + (msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN * 7) + 1)}
...
${screenShakeEnabled && hasHardwareSprite(analysis) ? (0, msx2ScreenShakeGenerator_1.buildMsx2ScreenShakeMainLoopCallAsm)() : ''}${pushBoxMovement && hasHardwareSprite(analysis) ? '    call update_msx2_box2_boxes\n' : ''}
...
` : ''}
${formatBytes('msx2_screen_enemy_count', enemyCountBytes.length ? enemyCountBytes : [0], `Per-msx2screen active enemy/hazard entity count, capped at ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN}`)}
${formatBytes('msx2_screen_enemy_x', enemyXBytes.length ? enemyXBytes : Array(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN).fill(0), `Per-msx2screen enemy/hazard entity X coordinates, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_y', enemyYBytes.length ? enemyYBytes : Array(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN).fill(0), `Per-msx2screen enemy/hazard entity Y coordinates, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_min_x', enemyMinXBytes.length ? enemyMinXBytes : Array(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN).fill(0), `Per-msx2screen enemy/hazard patrol minimum X, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_max_x', enemyMaxXBytes.length ? enemyMaxXBytes : Array(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN).fill(0), `Per-msx2screen enemy/hazard patrol maximum X, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_min_y', enemyMinYBytes.length ? enemyMinYBytes : Array(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN).fill(0), `Per-msx2screen enemy/hazard patrol minimum Y, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_max_y', enemyMaxYBytes.length ? enemyMaxYBytes : Array(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN).fill(0), `Per-msx2screen enemy/hazard patrol maximum Y, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_dx', enemyDxBytes.length ? enemyDxBytes : Array(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN).fill(0), `Per-msx2screen enemy/hazard initial movement direction, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_dy', enemyDyBytes.length ? enemyDyBytes : Array(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN).fill(0), `Per-msx2screen enemy/hazard initial vertical movement direction, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_mode', enemyModeBytes.length ? enemyModeBytes : Array(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN).fill(0), `Per-msx2screen enemy/hazard movement component mode, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_speed', enemySpeedBytes.length ? enemySpeedBytes : Array(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN).fill(2), `Per-msx2screen enemy/hazard movement component frame delay, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
${formatBytes('msx2_screen_enemy_score', enemyScoreBytes.length ? enemyScoreBytes : Array(msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN).fill(1), `Per-msx2screen enemy/hazard score value, ${msx2EntityRuntimeGenerator_1.MSX2_MAX_ENTITY_HAZARDS_PER_SCREEN} slots per screen`)}
...
function generateMsx2Screen4Files(projectName, analysis, config) {
