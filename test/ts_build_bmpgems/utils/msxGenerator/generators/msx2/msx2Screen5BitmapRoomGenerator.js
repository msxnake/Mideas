...
exports.generateMsx2Screen5BitmapRoomFiles = generateMsx2Screen5BitmapRoomFiles;
const msx2PlatformPhysics_1 = require("../../../msx2PlatformPhysics");
const msx2BitmapAirDashGenerator_1 = require("./msx2BitmapAirDashGenerator");
const msx2BitmapDashGenerator_1 = require("./msx2BitmapDashGenerator");
const msx2BitmapDoubleJumpGenerator_1 = require("./msx2BitmapDoubleJumpGenerator");
const msx2BitmapWallClimbGenerator_1 = require("./msx2BitmapWallClimbGenerator");
const msx2BitmapGlideGenerator_1 = require("./msx2BitmapGlideGenerator");
const msx2BitmapWallJumpGenerator_1 = require("./msx2BitmapWallJumpGenerator");
const msx2BitmapPowerStompGenerator_1 = require("./msx2BitmapPowerStompGenerator");
const msx2BitmapShootGenerator_1 = require("./msx2BitmapShootGenerator");
const msx2BitmapTeleportABGenerator_1 = require("./msx2BitmapTeleportABGenerator");
const msx2BitmapSlashGenerator_1 = require("./msx2BitmapSlashGenerator");
const msx2BitmapGrabGenerator_1 = require("./msx2BitmapGrabGenerator");
const msx2BitmapHighJumpGenerator_1 = require("./msx2BitmapHighJumpGenerator");
const msx2BitmapWallBreakGenerator_1 = require("./msx2BitmapWallBreakGenerator");
const msx2BitmapSpinAttackGenerator_1 = require("./msx2BitmapSpinAttackGenerator");
const msx2BitmapIceSlideGenerator_1 = require("./msx2BitmapIceSlideGenerator");
const msx2BitmapCrouchGenerator_1 = require("./msx2BitmapCrouchGenerator");
const msx2Screen4Generator_1 = require("./msx2Screen4Generator");
...
const hexVram = (value) => `#${Math.max(0, Math.trunc(value)).toString(16).toUpperCase().padStart(5, '0')}`;
 ...
 */
function collectBitmapWorldRooms(analysis) {
    const allRooms = (analysis.msx2BitmapRooms || []).filter(Boolean);
    if (allRooms.length === 0)
        return { rooms: [], startIndex: 0, transitions: new Map() };
    const roomById = new Map(allRooms.map(room => [room.id, room]));
    const worldmaps = (analysis.worldmaps || []);
    const graph = worldmaps.find(wm => (wm?.nodes || []).some((node) => roomById.has(node?.screenAssetId)));
    if (!graph)
        return { rooms: [allRooms[0]], startIndex: 0, transitions: new Map() };
    // Order the rooms by the worldmap nodes that resolve to a bitmap room.
    const orderedNodes = (graph.nodes || []).filter((node) => roomById.has(node?.screenAssetId));
    const rooms = orderedNodes.map((node) => roomById.get(node.screenAssetId));
    const indexByScreenId = new Map(orderedNodes.map((node, index) => [node.screenAssetId, index]));
    ...
    const setTransition = (from, dir, to) => {
...
const BITMAP_INTRO_SUPPORTED_EFFECTS = new Set([
    'cls',
    'fade_to_black',
...
function parseIntroHexColor(hex) {
...
function buildIntroPaletteBytes(palette) {
    ...
    return Array.from({ length: 16 }, (_unused, slotIndex) => {
        ...
        const rgb = parseIntroHexColor(slot?.hex);
        if (rgb)
            return [(rgb[0] << 4) | rgb[2], rgb[1]];
        return [0, 0];
    }).flat();
}
...
function buildIntroBitmapBytes(presentation) {
    const nested = presentation?.data;
    const packed = (Array.isArray(presentation.packedBitmap) && presentation.packedBitmap.length ? presentation.packedBitmap : undefined)
        || (Array.isArray(nested?.packedBitmap) ? nested.packedBitmap : undefined)
        || (Array.isArray(nested?.packedPixels) ? nested.packedPixels : undefined)
        || [];
    const declaredHeight = presentation.height ?? nested?.height;
...
function resolveBitmapIntroScenes(analysis) {
    const flows = (analysis.msx2GameFlows || [])
        .filter(flow => flow?.purpose === 'screen4-bitmap-runtime');
    const flow = flows.find(candidate => candidate?.name === 'Main MSX2') || flows[0];
    if (!flow || !Array.isArray(flow.nodes))
        return [];
    const presentations = (analysis.msx2Presentations || []);
    ...
    while (current && current.type === 'Screen5Presentation' && !visited.has(current.id)) {
        ...
        const presentation = node.presentationAssetId
            ? presentations.find(item => item.id === node.presentationAssetId)
            : presentations[0];
        if (!presentation) {
        ...
        const scene = {
            name: String(presentation.name || `scene ${scenes.length}`),
            paletteBytes: buildIntroPaletteBytes(presentation.palette || presentation.data?.palette),
...
const BITMAP_INTRO_EFFECT_CALLS = {
 ...
 */
function buildBitmapIntroAsm(blobs, banked) {
    if (!blobs.length)
        return { initCallAsm: '', routinesAsm: '', dataAsm: '' };
    const usedEffects = new Set(blobs.map(blob => blob.scene.transition?.effect).filter(Boolean));
...
` : ''}
${uploadRoutines}`;
    const paletteData = blobs.map(({ scene, index }) => formatBytes(`bitmap_intro_scene${index}_palette`, scene.paletteBytes, `GameFlow intro scene ${index} palette: byte1=(R<<4)|B, byte2=G`)).join('');
    ...
    const chunkData = banked
        ...
        : blobs.map(({ scene, index, rleChunks }) => formatRleChunks(rleChunks, scene.bitmapBytes.length, `GameFlow intro scene ${index} SCREEN 5 bitmap, packed 4bpp RLE, destination VRAM #00000`)).join('');
    return {
...
function normalizeRoom(room) {
    ...
    const entryBottom = Math.max(0, ...((room?.atlas?.entries || []).map(entry => Math.trunc(Number(entry?.sy) || 0) + Math.max(1, Math.trunc(Number(entry?.h) || TILE_GRID_SIZE)))));
    const authoredAtlasHeight = Math.max(Number(room?.atlas?.height) || 0, Array.isArray(room?.atlas?.pixels) ? room.atlas.pixels.length : 0, entryBottom);
...
function extractAtlasEntryPixels(room, entry) {
    ...
    return Array.from({ length: height }, (_row, y) => Array.from({ length: width }, (_col, x) => pixels[sy + y]?.[sx + x] ?? 0));
}
function atlasEntryFingerprint(pixels) {
...
function buildSharedWorldAtlasRooms(rooms) {
    ...
    const sharedHeight = Math.max(TILE_GRID_SIZE, Math.min(BITMAP_ROOM_ATLAS_MAX_HEIGHT, requiredHeight));
    ...
    const placedByFingerprint = new Map();
    let cursorX = 0;
    let cursorY = 0;
    let shelfHeight = TILE_GRID_SIZE;
    const placePixels = (pixels) => {
...
function findBitmapHudFontAsset(assets, fontAssetId) {
    if (!fontAssetId)
        return undefined;
    return assets.find(asset => asset.type === 'msx2hudfont' && asset.id === fontAssetId)?.data;
}
function getBitmapHudFontAsset(analysis, room, linkedHudAsset) {
 ...
 * the Msx2HudEditor).
 */
function resolveLinkedHudAsset(analysis, hudAssetId) {
    if (!hudAssetId)
        return undefined;
    const assets = (analysis.assets || []);
    const asset = assets.find(item => item.id === hudAssetId && item.type === 'msx2hud')?.data;
    return asset && Array.isArray(asset.layers) ? asset : undefined;
}
function normalizeHudText(value, maxLength, allowedCharacters) {
...
function getBitmapHudWidgetText(widget, room, allowedCharacters) {
    const maxChars = Math.max(1, Math.min(31, Math.floor((Number(widget.width) || 64) / 8)));
    if (widget.kind === 'text')
        return normalizeHudText(widget.text || widget.name || 'TEXT', maxChars, allowedCharacters);
    if (widget.kind !== 'counter')
        return '';
    const binding = widget.binding || 'custom';
    const fallbackValue = binding === 'air' ? room.runtime?.initialAir ?? 255 :
        binding === 'lives' ? 3 :
...
function normalizeScreen5HudFontGlyph(font, char, fallbackColor, fallbackBg = BITMAP_HUD_HEART_COLOR_BG) {
    if (font?.vdpMode !== 'SCREEN5')
        return undefined;
    ...
    if (Array.isArray(bitmap)) {
        return Array.from({ length: 8 }, (_unused, y) => Array.from({ length: 8 }, (_unused2, x) => clampByte(bitmap[y]?.[x], backgroundSlot) & 0x0f));
    }
    const pattern = font.patterns?.[char] || DEFAULT_HUD_PATTERNS[char] || DEFAULT_HUD_PATTERNS[' '];
    return Array.from({ length: 8 }, (_unused, y) => Array.from({ length: 8 }, (_unused2, x) => ((Number(pattern[y]) || 0) & (0x80 >> x)) ? (fallbackColor & 0x0f) : backgroundSlot));
}
function drawBitmapHudText(pixels, text, x, y, font, color) {
    ...
    for (const [charIndex, char] of Array.from(text).entries()) {
        ...
        if (bitmapGlyph) {
            ...
            for (let row = 0; row < 8; row++) {
                for (let col = 0; col < 8; col++) {
                    ...
                    if (glyphColor === backgroundSlot)
                        continue;
                    const px = x + (charIndex * 8) + col;
                    const py = y + row;
                    if (px >= 0 && px < SCREEN_WIDTH && py >= 0 && py < pixels.length)
                        pixels[py][px] = glyphColor;
                }
            }
            continue;
        }
        ...
        for (let row = 0; row < 8; row++) {
            ...
            for (let col = 0; col < 8; col++) {
                ...
                const py = y + row;
                if (px >= 0 && px < SCREEN_WIDTH && py >= 0 && py < pixels.length)
                    pixels[py][px] = color & 0x0f;
            }
        }
    }
}
function drawBitmapHudAtlasIcon(pixels, room, atlasPixels, widget) {
 ...
 */
function drawHudAssetIcon(pixels, icons, element, x0, y0, width, height) {
 ...
 */
function buildBitmapHeartTilePixels() {
    ...
    const grid = Array.from({ length: 16 }, () => Array.from({ length: 32 }, () => bg));
    for (let y = 0; y < 16; y++) {
        for (let x = 0; x < 16; x++) {
            if (mask[y][x])
                grid[y][x] = full;
            // outline: a filled cell with at least one empty 4-neighbour
            const neighbourEmpty = (x === 0) || (x === 15) || (y === 0) || (y === 15) ||
                !mask[y][x - 1] || !mask[y][x + 1] || !mask[y - 1]?.[x] || !mask[y + 1]?.[x];
            if (mask[y][x] && neighbourEmpty)
                grid[y][x + 16] = empty;
        }
    }
    return grid;
}
function buildBitmapHudSeedPixels(room, atlasPixels, analysis, linkedHudAsset, playerVitals) {
...
function isFullScreenFillCommand(command) {
 ...
 */
function buildRoomTileIndexGrid(room) {
    const entries = room.atlas?.entries || [];
    ...
    for (const command of room.composition?.commands || []) {
        ...
        if (cx >= 0 && cx < COLLISION_COLS && cy >= 0 && cy < COLLISION_ROWS)
            grid[cy][cx] = index + 1;
    }
    return grid;
}
 ...
 */
function buildRoomRenderBlocks(room, pageBaseY = BITMAP_ROOM_PAGE0_BASE_Y) {
    ...
    for (const command of room.composition?.commands || []) {
        if (command.op === 'fill') {
            if (isFullScreenFillCommand(command))
                continue;
    ...
    for (const entity of room.entities || []) {
        if (entity.kind !== 'npc')
            continue;
        const npcParams = entity.params?.npcDialogue;
        if (!npcParams?.atlasEntryId)
            continue;
...
function formatDbExpressions(label, values, comment) {
    const lines = [];
    if (comment)
        lines.push(`; ${comment}`);
    lines.push(`${label}:`);
...
function buildRleChunksForVram(bytes, vramBaseOffset, labelPrefix) {
    const chunks = [];
    let offset = 0;
...
function buildRleUploadAsm(rleChunks, banked) {
    const lines = [];
...
function buildBankedRleDataBlocks(chunks, description) {
...
function packBitmapRoomDataBanks(blocks) {
    const banks = [];
    let current;
...
function assignDataBankConstants(banks, chunks) {
    ...
    for (const bank of banks) {
        for (const block of bank.blocks) {
            ...
            if (chunk)
                chunk.dataBank = bank.bank;
        }
    }
}
function formatBankedDataEquates(banks) {
    const lines = [];
    for (const bank of banks) {
        for (const block of bank.blocks) {
...
function formatBankedDataBanks(banks) {
    if (!banks.length)
        return '';
    const lines = ['; --- SCREEN 5 bitmap-room Konami MegaROM data banks ---'];
    for (const bank of banks) {
...
function formatRleChunks(chunks, rawByteCount, description) {
...
function resolveWorldPalette(analysis, paletteAssetId, fallback) {
    const assets = (analysis.assets || []);
    const paletteAsset = assets.find(asset => asset.id === paletteAssetId && asset.type === 'palette')?.data;
...
function multiplyABySmallConstantAsm(multiplier) {
    if (multiplier <= 1)
        return '';
    if (multiplier === 2)
        return '    add a, a\n';
...
function buildRuntimeAsm(room, rleChunks, hudSeedRleChunks, playerAnimation, options, playerPhysics, playerHitbox, skillHooks = {}, foreground = null, enableBlink = false, enableKeyDoorTransitions = false, doorVisualPendingPageCallAsm = '') {
    ...
    const probeRowOffsets = [];
    for (let row = hbTop; row < hbBottom; row += 16)
        probeRowOffsets.push(row);
    probeRowOffsets.push(hbBottom);
    const probeColOffsets = [];
    for (let col = hbLeft; col < hbRight; col += 16)
        probeColOffsets.push(col);
    ...
    const addAImmediate = (offset) => (offset > 0 ? `    add a, ${offset}\n` : '');
...
.check_west_edge:
    ...
    pop bc
    ret c
${(0, msx2BitmapDoubleJumpGenerator_1.buildBitmapJumpBlockAsm)(playerPhysics)}
.apply_gravity:
...
function resolveBitmapRoomPlayer(analysis, room) {
    const playerRecords = (0, msx2Screen4Generator_1.getMsx2PlayerAssetRecords)(analysis);
    const referenceIds = new Set();
    for (const entry of room.playerEntries || []) {
        const playerId = String(entry?.playerId || '').trim();
        if (playerId)
            referenceIds.add(playerId);
    }
...
function getBitmapPlayerBodyHitbox(player, spriteSize) {
    const body = player?.hitboxes?.body || player?.components?.msx2_collision || player?.params || {};
    const inset = (value, fallback) => Math.max(0, Math.min(31, Math.floor(Number(value) || fallback)));
    const span = (value, fallback) => Math.max(1, Math.min(32, Math.floor(Number(value) || fallback)));
...
function resolveBitmapPlayerPhysics(player) {
    const physics = (0, msx2PlatformPhysics_1.getMsx2PlatformPhysicsFromPlayerEntity)(player);
    const toSigned16 = (value) => ((value & 0x8000) ? value - 0x10000 : value);
    ...
    return {
        ...
        jumpKeyboard: player?.inputEnabled?.jump === false ? null : ((0, msx2PlatformPhysics_1.resolveMsx2BitmapKeyboardBinding)(player, 'jump') ?? null),
        gravityFrac: Math.max(16, Math.min(128, Math.floor(physics.gravityStrength88) || 0x40)) & 0xff,
    };
}
function resolveBitmapPlayerVitals(player) {
    const health = player?.health || {};
    const clampByte01 = (value, fallback) => Math.max(1, Math.min(255, Math.floor(Number(value) || fallback)));
    return {
...
function buildBitmapDeadlySystemAsm(vitals, hitbox) {
    ...
    const addA = (n) => (n > 0 ? `    add a, ${n}\n` : '');
    const hexB = (n) => `#${(n & 0xff).toString(16).toUpperCase().padStart(2, '0')}`;
...
function normalizeDoorConfig(value) {
...
function resolveDoorTargetEntry(room, entryId) {
...
function buildDoorVisualCommand(room, atlasEntryId, dx, dy) {
...
function collectBitmapKeyDoorRecords(rooms) {
    const keyBits = new Map();
    for (const room of rooms) {
        for (const item of room.keyItems || []) {
            if (!item?.id || keyBits.has(item.id))
                continue;
    ...
    for (const [roomIndex, room] of rooms.entries()) {
        for (const entity of room.entities || []) {
            ...
            if (!door.enabled || !door.targetRoomId)
                continue;
...
function buildBitmapKeyDoorSystemAsm(rooms, hitbox, ramBase, bankedRoomData) {
    ...
    const pickupDataAsm = pickupTables.map((items, roomIndex) => formatBytes(`bitmap_key_pickups_room_${roomIndex}`, items.flatMap(item => [item.x, item.y, item.keyMask, item.flagOffset]), `Room ${roomIndex} key pickup records: x,y,keyMask,pickupFlagOffset`)).join('');
    const doorDataAsm = doorTables.map((items, roomIndex) => formatBytes(`bitmap_key_doors_room_${roomIndex}`, items.flatMap(item => [item.x, item.y, item.requiredMask, item.targetRoomIndex, item.targetX, item.targetY, item.flags, item.openOffset]), `Room ${roomIndex} locked door records: x,y,requiredMask,targetRoom,targetX,targetY,flags,doorOpenOffset`)).join('');
    const visualDataAsm = visualTables.map((items, roomIndex) => formatBytes(`bitmap_key_door_visuals_room_${roomIndex}`, items.flatMap(item => [item.openOffset, item.flags, ...item.closedCommand, ...item.openCommand]), `Room ${roomIndex} door visual records: openFlagOffset,flags,closedHMMM(15),openHMMM(15)`)).join('');
...
function buildGemAtlasCopyCommand(sxRaw, syRaw, dx, dy) {
 ...
 */
function buildGemEraseCommand(room, dx, dy) {
    const grid = buildRoomTileIndexGrid(room);
...
function collectBitmapGemRecords(rooms) {
    const records = [];
    for (const [roomIndex, room] of rooms.entries()) {
        for (const entity of room.entities || []) {
            if (entity.kind !== 'collectible')
                continue;
            if (typeof entity.params?.keyPickupId === 'string' && entity.params.keyPickupId)
                continue;
...
function buildBitmapGemSystemAsm(rooms, hitbox, ramBase, config, hudCounter) {
    ...
    const dataAsm = gemTables.map((items, roomIndex) => formatBytes(`bitmap_gems_room_${roomIndex}`, items.flatMap(item => [item.x, item.y, item.flagOffset, ...item.drawCommand, ...item.eraseCommand]), `Room ${roomIndex} gem records: x,y,flagOffset,drawCmd(15),eraseCmd(15)`)).join('') +
...
const BITMAP_DLG_TALK_KEY_MASKS = { up: 0x20, space: 0x01 };
...
function bitmapDialogueSupportedChars(font) {
    const supported = new Set(Object.keys(DEFAULT_HUD_PATTERNS));
    for (const key of Object.keys(font?.patterns || {}))
        supported.add(key);
    for (const key of Object.keys(font?.bitmapPatterns || {}))
        supported.add(key);
    supported.add(' ');
    return supported;
}
/** 8x8 palette-slot glyph for one char: font bitmap glyph or 1bpp pattern. */
function buildBitmapDialogueGlyph(font, char, textColor, bgColor) {
    ...
    const pattern = font?.patterns?.[char] || DEFAULT_HUD_PATTERNS[char] || DEFAULT_HUD_PATTERNS[' '];
    return Array.from({ length: 8 }, (_unused, y) => Array.from({ length: 8 }, (_unused2, x) => ((Number(pattern[y]) || 0) & (0x80 >> x)) ? (textColor & 0x0f) : (bgColor & 0x0f)));
}
/** Word-wrap normalized dialogue text into at most maxRows rows of maxCols chars. */
function wrapBitmapDialogueText(text, maxCols, maxRows) {
    const rows = [];
 ...
 */
function collectBitmapDialogueData(analysis, rooms, fallbackFont) {
    const assets = (analysis.assets || []);
    const dialogueById = new Map();
    for (const asset of assets) {
        if (asset.type === 'msx2dialogue' && asset.id && asset.data)
            dialogueById.set(asset.id, asset.data);
    }
    const npcs = [];
    ...
    usedDialogues.forEach((dialogue, dialogueIndex) => {
        ...
        const assetPortraits = (Array.isArray(dialogue.portraits) ? dialogue.portraits : []);
        const portraitIndexById = new Map();
        let porMaxW = 0;
        let porMaxH = 0;
        for (const portrait of assetPortraits) {
            ...
            const normalizeFrame = (pixels) => Array.from({ length: height }, (_unused, y) => Array.from({ length: width }, (_unused2, x) => clampByte(pixels?.[y]?.[x], bgColor) & 0x0f));
        ...
        const resolvePortraitIndex = (portraitId) => {
        ...
        const anyPortrait = (Array.isArray(dialogue.lines) ? dialogue.lines : [])
            .some((line) => resolvePortraitIndex(line?.portraitId) !== 0xff);
        if (!anyPortrait) {
            porMaxW = 0;
            porMaxH = 0;
        }
        ...
        for (const line of dialogueLines) {
            ...
            rows.forEach((row, rowIndex) => {
                if (rowIndex > 0)
                    encoded.push(BITMAP_DLG_NEWLINE);
...
function buildBitmapDialogueBlobPixels(data) {
    const rows = Array.from({ length: data.blobRows }, () => Array.from({ length: SCREEN_WIDTH }, () => 0));
...
function buildBitmapDialogueSystemAsm(data, rooms, hitbox, ramBase, vramBaseRow, uploadAsm, keyDoorVisibleDrawCall, bankedRoomData) {
    if (!data) {
...
`;
    ...
    const npcDataAsm = npcTables.map((items, roomIndex) => items.length
    ...
    const configDataAsm = data.configs.map(config => formatBytes(config.label, [
    ...
    ], `Dialogue config: boxX,boxY,boxW,boxH,borderClr,bgClr,delay,mouthInt,textX,textY,textW,textH,stripSY(w),porX,porY,porMaxW,porMaxH,lineBase,lineCount`)).join('');
    const lineTextDataAsm = data.lines.map((line, index) => formatBytes(`bitmap_dlg_text_${index}`, line.encoded, `Dialogue line ${index} glyph indices (#FE newline, #FF end)`)).join('');
    const lineRecordsAsm = `; 4 bytes/line: text ptr (word), flags (bit0 = waitForInput), portrait index (#FF none)
bitmap_dlg_line_records:\n${data.lines.map((line, index) => `    DW bitmap_dlg_text_${index}\n    DB ${line.waitForInput ? 1 : 0}, ${hexByte(line.portraitIndex)}`).join('\n')}\n`;
...
function buildBitmapHeartsHudAsm(maxHealthRaw, heartUploadAsm) {
...
function buildBitmapHudSeparatorRestoreAsm(enabled) {
    if (!enabled)
        return { mainLoopCall: '', routinesAsm: '' };
    return {
        mainLoopCall: '',
        routinesAsm: `; ------------------------------------------------------------
...
function collectLinkedHudDynamicSources(hudAsset) {
    const sources = [];
    ...
    for (const layer of [...(hudAsset.layers || [])].reverse()) {
        if (layer.kind !== 'widget' || !layer.visible || !layer.element.visible)
            continue;
        const kind = layer.element.kind;
        if (kind === 'iconRow')
            sources.push({ kind: 'iconRow', element: layer.element });
        else if (kind === 'counter' || kind === 'iconCounter')
            sources.push({ kind: 'counter', element: layer.element });
        else if (kind === 'bar')
            sources.push({ kind: 'bar', element: layer.element });
    }
    return sources;
}
/**
 ...
 */
function resolveHudElementBindingRamLabel(element, instanceIndex) {
    if (element.binding === 'playerEnergy')
        return 'player_health';
    if (element.binding === 'lives')
        return 'player_lives';
    if (element.binding === 'air')
        return 'air_timer';
    if (element.binding === 'experience')
        return 'player_xp';
    if (element.binding === 'level')
        return 'player_level';
    if (element.binding === 'skillPoints')
        return 'player_skill_points';
    return `hud_linked_${instanceIndex}_value`;
}
function resolveHudElementMaxValue(element, fallback, playerVitals) {
...
function resolveHudElementInitialValue(element, maxValue, playerVitals) {
    if (element.binding === 'playerEnergy' && playerVitals) {
 ...
 */
function buildIconRowTilePixels(hudAsset, element) {
    ...
    const paintIcon = (icon, colOffset, fallbackColor) => {
        if (icon) {
            ...
            for (let y = 4; y < 12; y++) {
                for (let x = 4; x < 12; x++)
                    grid[y][colOffset + x] = fallbackColor & 0x0f;
            }
        }
    };
...
function buildCounterGlyphPixels(font, color) {
    const patterns = font?.patterns || DEFAULT_HUD_PATTERNS;
    const bg = BITMAP_HUD_HEART_COLOR_BG;
    const grid = Array.from({ length: 8 }, () => Array.from({ length: 80 }, () => bg));
    for (let digit = 0; digit <= 9; digit++) {
        ...
        for (let row = 0; row < 8; row++) {
            ...
            for (let col = 0; col < 8; col++) {
                if (bits & (0x80 >> col))
                    grid[row][(digit * 8) + col] = color & 0x0f;
            }
        }
    }
    return grid;
 ...
 */
function linkedCounterSpec(element) {
 ...
 */
function buildBitmapHudLinkedIconRowAsm(source, tileVramY, ram, bindingRamLabel, uploadAsm, instanceIndex, playerVitals) {
 ...
 */
function buildBitmapHudLinkedCounterAsm(source, glyphVramY, ram, bindingRamLabel, uploadAsm, instanceIndex) {
    const element = source.element;
...
const sanitizeAsmHookLabel = (value) => {
 ...
 * linked widgets.
 */
function buildBitmapHudLinkedBarAsm(source, ram, bindingRamLabel, instanceIndex, playerVitals) {
    const element = source.element;
...
function resolveBitmapRoomPlayerSprite(analysis, room) {
    const playerRecords = (0, msx2Screen4Generator_1.getMsx2PlayerAssetRecords)(analysis);
    const referenceIds = new Set();
    for (const entry of room.playerEntries || []) {
        const playerId = String(entry?.playerId || '').trim();
    ...
    const resolveFromPlayer = (player) => {
        const spriteAssetId = String(player?.render?.spriteAssetId || '').trim();
        return spriteAssetId ? (0, msx2Screen4Generator_1.resolveMsx2SpriteById)(analysis, spriteAssetId) : undefined;
    };
    ...
    return (0, msx2Screen4Generator_1.getFirstReferencedMsx2Sprite)(analysis);
}
/**
 ...
 */
function resolveBitmapBulletSprite(analysis, player) {
    ...
    if (Array.isArray(weapons)) {
        for (const weapon of weapons) {
            ...
            if (!assetId)
                continue;
            const sprite = (0, msx2Screen4Generator_1.resolveMsx2SpriteById)(analysis, assetId);
            if (!sprite)
                continue;
            const layers = (0, msx2Screen4Generator_1.buildHardwareSpriteLayersForFrame)(sprite, BITMAP_ROOM_DEFAULT_SPRITE_COLOR, 0)
            ...
            if (primary && Array.isArray(primary.pattern) && primary.pattern.length === 32) {
                ...
                while (colors.length < 16)
                    colors.push(BITMAP_ROOM_DEFAULT_SPRITE_COLOR);
                return {
...
function getBitmapRoomSpriteFrameIndices(sprite) {
...
function getBitmapRoomSpriteAnimationDelayFrames(sprite) {
...
function buildSpriteTables(sprite) {
    ...
    if (sprite) {
        ...
        const rawFrameLayerSets = frameIndices
            .map(frameIndex => (0, msx2Screen4Generator_1.buildHardwareSpriteLayersForFrame)(sprite, BITMAP_ROOM_DEFAULT_SPRITE_COLOR, frameIndex)
            .filter(layer => Array.isArray(layer.pattern) && layer.pattern.length === 32));
        ...
        const cellKey = (layer) => `${layer.yOffset}:${layer.xOffset}`;
        const cellMap = new Map();
        for (const layers of rawFrameLayerSets) {
            for (const layer of layers) {
                if (!cellMap.has(cellKey(layer)))
                    cellMap.set(cellKey(layer), { x: layer.xOffset, y: layer.yOffset });
            }
        }
        const cells = Array.from(cellMap.values()).sort((a, b) => (a.y - b.y) || (a.x - b.x));
        const cellCount = Math.max(1, cells.length);
        const layersForCell = (layers, cell) => layers.filter(layer => layer.xOffset === cell.x && layer.yOffset === cell.y);
        ...
        const maxAuthoredColorLayers = Math.max(1, ...rawFrameLayerSets.flatMap(layers => cells.map(cell => layersForCell(layers, cell).length || 1)));
        const maxColorLayersByBudget = Math.max(1, Math.floor(64 / (frameCount * cellCount * mirrorFactor)));
        const colorLayerCount = Math.max(1, Math.min(BITMAP_ROOM_MAX_PLAYER_SPRITE_LAYERS, maxAuthoredColorLayers, maxColorLayersByBudget));
        ...
        const spriteOffsets = cells.flatMap(cell => Array.from({ length: colorLayerCount }, () => ({ x: cell.x, y: cell.y })));
        ...
        if (rawFrameLayerSets.some(layers => layers.length)) {
            ...
            const patternSetForFrame = (layers, transform) => cells.flatMap(cell => {
            ...
            const colors = rawFrameLayerSets.flatMap(layers => cells.flatMap(cell => {
                const cellLayers = layersForCell(layers, cell);
                return Array.from({ length: colorLayerCount }, (_unused, layerIndex) => {
                    const layerColors = (cellLayers[layerIndex]?.colors || []).slice(0, 16);
                    while (layerColors.length < 16)
                        layerColors.push(BITMAP_ROOM_DEFAULT_SPRITE_COLOR);
                    return layerColors;
                }).flat();
            }));
            return {
 ...
 */
function extractStateSpriteBank(sprite, cells, colorLayerCount, authoredFacing) {
 ...
 */
function resolveBitmapRoomStateAnimations(analysis, room, base) {
    ...
    if (animations && typeof animations === 'object') {
        const order = Array.isArray(player?.animationOrder) && player.animationOrder.length
        ...
        for (const key of order) {
            ...
            if (!state || !spriteAssetId)
                continue;
            if (BITMAP_BASE_ANIM_STATES.has(state.toLowerCase()))
                continue;
            if (!mapping.has(state))
                mapping.set(state, spriteAssetId);
        }
    }
    const stateSprites = player?.render?.stateSprites;
    ...
    for (const [state, spriteAssetId] of mapping) {
        const sprite = (0, msx2Screen4Generator_1.resolveMsx2SpriteById)(analysis, spriteAssetId);
        if (!sprite)
            continue;
...
function buildBehaviorTableBytes(room) {
    const bytes = [];
    for (let y = 0; y < COLLISION_ROWS; y++) {
        for (let x = 0; x < COLLISION_COLS; x++) {
...
function resolvePlayerSpawnPixels(room) {
    ...
    if (!entry)
        return { x: 0, y: 0xD8, visible: false };
 ...
 */
function buildForegroundMaskPattern(pixels, _backgroundColor) {
 ...
 */
function resolveForegroundTileColor(pixels, backgroundColor, explicit) {
 ...
 */
function buildBitmapRoomForegroundTables(rooms, foregroundCount) {
    const patternBytes = [];
    const colorBytes = [];
    const roomTables = [];
    let globalOffset = 0;
    for (const room of rooms) {
        ...
        for (let slot = 0; slot < foregroundCount; slot++) {
            ...
            for (const b of mask)
                patternBytes.push(b & 0xff);
            for (let i = 0; i < 16; i++)
                colorBytes.push(color & 0xff);
 ...
 */
function buildBitmapLoadForegroundSpritesAsm(fg) {
    const slotBlocks = [];
...
function generateUnitedFiles(projectName, analysis, config) {
    ...
    const linkedHudSlotYs = [212, 228, 468];
    ...
    linkedHudDynamicSources.forEach((source, index) => {
        if (source.kind === 'bar')
            return; // bars use HMMV, no offscreen tile
    ...
    const roomDataBlocks = roomTables.flatMap(table => [
        {
    ...
    const transitionTableBytes = rooms.flatMap((_roomData, index) => {
        ...
        const target = (dir) => (rails[dir] === undefined ? 0xff : rails[dir]);
    ...
    const introSceneBlobs = introScenes.map((scene, index) => ({
        scene,
        index,
    ...
    if (isKonamiMegaRom)
        assignDataBankConstants(bankedDataBanks, allRleChunks);
    ...
    const linkedHudDataAsm = isKonamiMegaRom
        ...
        : linkedHudTileData.map(entry => formatRleChunks(entry.rleChunks, entry.bytes.length, `Linked HUD dynamic widget #${entry.index} (${entry.kind}) tile/glyph data, packed 4bpp RLE, destination VRAM ${hexVram(entry.tileVramY * ROW_BYTES)}`)).join('');
    ...
    const stateAnimIds = {};
    for (const bank of stateAnimations)
        stateAnimIds[bank.state] = bank.animId;
    ...
    const dashConfig = (0, msx2PlatformPhysics_1.getMsx2DashConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const dashEquates = (0, msx2BitmapDashGenerator_1.buildBitmapDashEquates)(dashConfig);
    const dashInitClear = (0, msx2BitmapDashGenerator_1.buildBitmapDashInitClearAsm)(dashConfig);
    const dashGate = (0, msx2BitmapDashGenerator_1.buildBitmapDashGateAsm)(dashConfig);
    const dashRuntime = (0, msx2BitmapDashGenerator_1.buildBitmapDashRuntimeAsm)(dashConfig, { dashing: stateAnimIds['dashing'] });
    ...
    const airDashConfig = (0, msx2PlatformPhysics_1.getMsx2AirDashConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const airDashRamBase = msx2BitmapDashGenerator_1.MSX2_BITMAP_DASH_RAM_BASE + ((0, msx2BitmapDashGenerator_1.bitmapDashEnabled)(dashConfig) ? msx2BitmapDashGenerator_1.MSX2_BITMAP_DASH_RAM_BYTES : 0);
    const airDashEquates = (0, msx2BitmapAirDashGenerator_1.buildBitmapAirDashEquates)(airDashConfig, airDashRamBase);
    const airDashInitClear = (0, msx2BitmapAirDashGenerator_1.buildBitmapAirDashInitClearAsm)(airDashConfig);
    const airDashGate = (0, msx2BitmapAirDashGenerator_1.buildBitmapAirDashGateAsm)(airDashConfig);
    const airDashRuntime = (0, msx2BitmapAirDashGenerator_1.buildBitmapAirDashRuntimeAsm)(airDashConfig, { lockGroundDashOnStart: (0, msx2BitmapDashGenerator_1.bitmapDashEnabled)(dashConfig) });
    ...
    const glideConfig = (0, msx2PlatformPhysics_1.getMsx2GlideConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const glideRamBase = airDashRamBase + ((0, msx2BitmapAirDashGenerator_1.bitmapAirDashEnabled)(airDashConfig) ? msx2BitmapAirDashGenerator_1.MSX2_BITMAP_AIR_DASH_RAM_BYTES : 0);
    const glideEquates = (0, msx2BitmapGlideGenerator_1.buildBitmapGlideEquates)(glideConfig, glideRamBase);
    const glideInitClear = (0, msx2BitmapGlideGenerator_1.buildBitmapGlideInitClearAsm)(glideConfig);
    const glideGravityHook = (0, msx2BitmapGlideGenerator_1.buildBitmapGlideGravityHookAsm)(glideConfig);
    const glideRuntime = (0, msx2BitmapGlideGenerator_1.buildBitmapGlideRuntimeAsm)(glideConfig);
    ...
    const wallJumpConfig = (0, msx2PlatformPhysics_1.getMsx2WallJumpConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const wallJumpRamBase = glideRamBase + (glideConfig.enabled ? msx2BitmapGlideGenerator_1.MSX2_BITMAP_GLIDE_RAM_BYTES : 0);
    const wallJumpEquates = (0, msx2BitmapWallJumpGenerator_1.buildBitmapWallJumpEquates)(wallJumpConfig, wallJumpRamBase);
    const wallJumpInitClear = (0, msx2BitmapWallJumpGenerator_1.buildBitmapWallJumpInitClearAsm)(wallJumpConfig);
    const wallJumpInputHook = (0, msx2BitmapWallJumpGenerator_1.buildBitmapWallJumpInputHookAsm)(wallJumpConfig);
    const wallJumpGravityHook = (0, msx2BitmapWallJumpGenerator_1.buildBitmapWallJumpGravityHookAsm)(wallJumpConfig);
    const wallJumpLandClear = (0, msx2BitmapWallJumpGenerator_1.buildBitmapWallJumpLandClearAsm)(wallJumpConfig);
    const wallJumpRuntime = (0, msx2BitmapWallJumpGenerator_1.buildBitmapWallJumpRuntimeAsm)(wallJumpConfig, {
    ...
    const powerStompConfig = (0, msx2PlatformPhysics_1.getMsx2PowerStompConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const powerStompRamBase = wallJumpRamBase + (wallJumpConfig.enabled ? msx2BitmapWallJumpGenerator_1.MSX2_BITMAP_WALL_JUMP_RAM_BYTES : 0);
    const powerStompShakeRamBase = powerStompRamBase + ((0, msx2BitmapPowerStompGenerator_1.bitmapPowerStompEnabled)(powerStompConfig) ? msx2BitmapPowerStompGenerator_1.MSX2_BITMAP_POWER_STOMP_RAM_BYTES : 0);
    const powerStompEquates = (0, msx2BitmapPowerStompGenerator_1.buildBitmapPowerStompEquates)(powerStompConfig, powerStompRamBase, powerStompShakeRamBase);
    const powerStompInitClear = (0, msx2BitmapPowerStompGenerator_1.buildBitmapPowerStompInitClearAsm)(powerStompConfig);
    const powerStompInputHook = (0, msx2BitmapPowerStompGenerator_1.buildBitmapPowerStompInputHookAsm)(powerStompConfig);
    const powerStompGravityHook = (0, msx2BitmapPowerStompGenerator_1.buildBitmapPowerStompGravityHookAsm)(powerStompConfig);
    const powerStompLandClear = (0, msx2BitmapPowerStompGenerator_1.buildBitmapPowerStompLandClearAsm)(powerStompConfig);
    const powerStompMainLoopCall = (0, msx2BitmapPowerStompGenerator_1.buildBitmapPowerStompMainLoopCallAsm)(powerStompConfig);
    const powerStompRuntime = (0, msx2BitmapPowerStompGenerator_1.buildBitmapPowerStompRuntimeAsm)(powerStompConfig);
    // SHOOT skill: fires bullets in the facing direction. RAM follows power_stomp shake.
    const shootConfig = (0, msx2PlatformPhysics_1.getMsx2ShootConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const shootRamBase = powerStompShakeRamBase + ((0, msx2BitmapPowerStompGenerator_1.bitmapPowerStompEnabled)(powerStompConfig) ? msx2BitmapPowerStompGenerator_1.MSX2_BITMAP_SCREEN_SHAKE_RAM_BYTES : 0);
    const shootEquates = (0, msx2BitmapShootGenerator_1.buildBitmapShootEquates)(shootConfig, shootRamBase);
    const shootInitClear = (0, msx2BitmapShootGenerator_1.buildBitmapShootInitClearAsm)(shootConfig);
    const shootGate = (0, msx2BitmapShootGenerator_1.buildBitmapShootGateAsm)(shootConfig);
    ...
    if (foregroundCount > 0 && foregroundPatternGroupBase + foregroundCount > 64) {
        throw new Error(`SCREEN 5 bitmap-room foreground sprites need pattern groups ${foregroundPatternGroupBase}..${foregroundPatternGroupBase + foregroundCount - 1}, ` +
    ...
    const shootBulletInitUpload = (0, msx2BitmapShootGenerator_1.buildBitmapBulletInitUploadAsm)(shootConfig, shootRuntimeOptions);
    const shootBulletSatCall = (0, msx2BitmapShootGenerator_1.buildBitmapBulletSatCallAsm)(shootConfig);
    const shootBulletDataTables = (0, msx2BitmapShootGenerator_1.buildBitmapBulletDataTables)(shootConfig, bulletSprite);
    const shootRuntime = (0, msx2BitmapShootGenerator_1.buildBitmapShootRuntimeAsm)(shootConfig, shootRuntimeOptions);
    // TELEPORT A-B skill: saves/restores position between two points. Pure RAM.
    const teleportConfig = (0, msx2PlatformPhysics_1.getMsx2TeleportABConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const teleportRamBase = shootRamBase + (0, msx2BitmapShootGenerator_1.bitmapShootRamBytes)(shootConfig);
    const teleportEquates = (0, msx2BitmapTeleportABGenerator_1.buildBitmapTeleportABEquates)(teleportConfig, teleportRamBase);
    const teleportInitClear = (0, msx2BitmapTeleportABGenerator_1.buildBitmapTeleportABInitClearAsm)(teleportConfig);
    const teleportGate = (0, msx2BitmapTeleportABGenerator_1.buildBitmapTeleportABGateAsm)(teleportConfig);
    const teleportRuntime = (0, msx2BitmapTeleportABGenerator_1.buildBitmapTeleportABRuntimeAsm)(teleportConfig);
    // SLASH skill: melee attack in facing direction. RAM follows teleport.
    const slashConfig = (0, msx2PlatformPhysics_1.getMsx2SlashConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const slashRamBase = teleportRamBase + ((0, msx2BitmapTeleportABGenerator_1.bitmapTeleportABEnabled)(teleportConfig) ? msx2BitmapTeleportABGenerator_1.MSX2_BITMAP_TELEPORT_AB_RAM_BYTES : 0);
    const slashEquates = (0, msx2BitmapSlashGenerator_1.buildBitmapSlashEquates)(slashConfig, slashRamBase);
    const slashInitClear = (0, msx2BitmapSlashGenerator_1.buildBitmapSlashInitClearAsm)(slashConfig);
    const slashGate = (0, msx2BitmapSlashGenerator_1.buildBitmapSlashGateAsm)(slashConfig);
    const slashRuntime = (0, msx2BitmapSlashGenerator_1.buildBitmapSlashRuntimeAsm)(slashConfig);
    // GRAB skill: wall cling with slow slide. RAM follows slash.
    const grabConfig = (0, msx2PlatformPhysics_1.getMsx2GrabConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const grabRamBase = slashRamBase + ((0, msx2BitmapSlashGenerator_1.bitmapSlashEnabled)(slashConfig) ? msx2BitmapSlashGenerator_1.MSX2_BITMAP_SLASH_RAM_BYTES : 0);
    const grabEquates = (0, msx2BitmapGrabGenerator_1.buildBitmapGrabEquates)(grabConfig, grabRamBase);
    const grabInitClear = (0, msx2BitmapGrabGenerator_1.buildBitmapGrabInitClearAsm)(grabConfig);
    const grabGate = (0, msx2BitmapGrabGenerator_1.buildBitmapGrabGateAsm)(grabConfig);
    const grabGravityHook = (0, msx2BitmapGrabGenerator_1.buildBitmapGrabGravityHookAsm)(grabConfig);
    const grabRuntime = (0, msx2BitmapGrabGenerator_1.buildBitmapGrabRuntimeAsm)(grabConfig);
    // HIGH JUMP skill: variable jump height. RAM follows grab.
    const highJumpConfig = (0, msx2PlatformPhysics_1.getMsx2HighJumpConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const highJumpRamBase = grabRamBase + ((0, msx2BitmapGrabGenerator_1.bitmapGrabEnabled)(grabConfig) ? msx2BitmapGrabGenerator_1.MSX2_BITMAP_GRAB_RAM_BYTES : 0);
    const highJumpEquates = (0, msx2BitmapHighJumpGenerator_1.buildBitmapHighJumpEquates)(highJumpConfig, highJumpRamBase);
    const highJumpInitClear = (0, msx2BitmapHighJumpGenerator_1.buildBitmapHighJumpInitClearAsm)(highJumpConfig);
    const highJumpInputHook = (0, msx2BitmapHighJumpGenerator_1.buildBitmapHighJumpInputHookAsm)(highJumpConfig);
    const highJumpGravityHook = (0, msx2BitmapHighJumpGenerator_1.buildBitmapHighJumpGravityHookAsm)(highJumpConfig);
    const highJumpLandClear = (0, msx2BitmapHighJumpGenerator_1.buildBitmapHighJumpLandClearAsm)(highJumpConfig);
    const highJumpRuntime = (0, msx2BitmapHighJumpGenerator_1.buildBitmapHighJumpRuntimeAsm)(highJumpConfig);
    // WALL BREAK skill: break solid tiles ahead. RAM follows high_jump.
    const wallBreakConfig = (0, msx2PlatformPhysics_1.getMsx2WallBreakConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const wallBreakRamBase = highJumpRamBase + ((0, msx2BitmapHighJumpGenerator_1.bitmapHighJumpEnabled)(highJumpConfig) ? msx2BitmapHighJumpGenerator_1.MSX2_BITMAP_HIGH_JUMP_RAM_BYTES : 0);
    const wallBreakEquates = (0, msx2BitmapWallBreakGenerator_1.buildBitmapWallBreakEquates)(wallBreakConfig, wallBreakRamBase);
    const wallBreakInitClear = (0, msx2BitmapWallBreakGenerator_1.buildBitmapWallBreakInitClearAsm)(wallBreakConfig);
    const wallBreakGate = (0, msx2BitmapWallBreakGenerator_1.buildBitmapWallBreakGateAsm)(wallBreakConfig);
    const wallBreakRuntime = (0, msx2BitmapWallBreakGenerator_1.buildBitmapWallBreakRuntimeAsm)(wallBreakConfig);
    // SPIN ATTACK skill: radial melee. RAM follows wall_break.
    const spinAttackConfig = (0, msx2PlatformPhysics_1.getMsx2SpinAttackConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const spinAttackRamBase = wallBreakRamBase + ((0, msx2BitmapWallBreakGenerator_1.bitmapWallBreakEnabled)(wallBreakConfig) ? msx2BitmapWallBreakGenerator_1.MSX2_BITMAP_WALL_BREAK_RAM_BYTES : 0);
    const spinAttackEquates = (0, msx2BitmapSpinAttackGenerator_1.buildBitmapSpinAttackEquates)(spinAttackConfig, spinAttackRamBase);
    const spinAttackInitClear = (0, msx2BitmapSpinAttackGenerator_1.buildBitmapSpinAttackInitClearAsm)(spinAttackConfig);
    const spinAttackGate = (0, msx2BitmapSpinAttackGenerator_1.buildBitmapSpinAttackGateAsm)(spinAttackConfig);
    const spinAttackRuntime = (0, msx2BitmapSpinAttackGenerator_1.buildBitmapSpinAttackRuntimeAsm)(spinAttackConfig);
    // ICE SLIDE skill: grounded inertia on behavior-layer ice cells. RAM follows spin_attack.
    const iceSlideConfig = {
        ...(0, msx2PlatformPhysics_1.getMsx2IceSlideConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room)),
    ...
    const iceSlideRamBase = spinAttackRamBase + ((0, msx2BitmapSpinAttackGenerator_1.bitmapSpinAttackEnabled)(spinAttackConfig) ? msx2BitmapSpinAttackGenerator_1.MSX2_BITMAP_SPIN_ATTACK_RAM_BYTES : 0);
    const iceSlideEquates = (0, msx2BitmapIceSlideGenerator_1.buildBitmapIceSlideEquates)(iceSlideConfig, iceSlideRamBase);
    const iceSlideInitClear = (0, msx2BitmapIceSlideGenerator_1.buildBitmapIceSlideInitClearAsm)(iceSlideConfig);
    const iceSlideHorizontalHook = (0, msx2BitmapIceSlideGenerator_1.buildBitmapIceSlideHorizontalHookAsm)(iceSlideConfig);
    const iceSlideRuntime = (0, msx2BitmapIceSlideGenerator_1.buildBitmapIceSlideRuntimeAsm)(iceSlideConfig);
    ...
    const crouchConfig = (0, msx2PlatformPhysics_1.getMsx2CrouchConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    const crouchRamBase = iceSlideRamBase + ((0, msx2BitmapIceSlideGenerator_1.bitmapIceSlideEnabled)(iceSlideConfig) ? msx2BitmapIceSlideGenerator_1.MSX2_BITMAP_ICE_SLIDE_RAM_BYTES : 0);
    const crouchEquates = (0, msx2BitmapCrouchGenerator_1.buildBitmapCrouchEquates)(crouchConfig, crouchRamBase);
    const crouchInitClear = (0, msx2BitmapCrouchGenerator_1.buildBitmapCrouchInitClearAsm)(crouchConfig);
    const crouchHorizontalHook = (0, msx2BitmapCrouchGenerator_1.buildBitmapCrouchHorizontalHookAsm)(crouchConfig);
    const crouchRuntime = (0, msx2BitmapCrouchGenerator_1.buildBitmapCrouchRuntimeAsm)(crouchConfig, {
    ...
    const linkedHudRamBase = crouchRamBase + (crouchConfig.enabled ? msx2BitmapCrouchGenerator_1.MSX2_BITMAP_CROUCH_RAM_BYTES : 0);
    ...
    const anyExperienceHud = linkedHudDynamicSources.some(source => source.element.binding === 'experience'
    ...
    const linkedHudRamAllocations = linkedHudDynamicSources.map(source => {
        ...
        const needsOwnValue = !(source.element.binding === 'playerEnergy'
    ...
    if (hudDec3BufferAddress !== undefined)
        hudLinkedRamCursor += 3;
    const hudDec5BufferAddress = anyWideCounter ? hudLinkedRamCursor : undefined;
    if (hudDec5BufferAddress !== undefined)
        hudLinkedRamCursor += 5;
    ...
    if (airTimerAllocated)
        hudLinkedRamCursor += 1;
    ...
    const collectorGemsConfig = (0, msx2PlatformPhysics_1.getMsx2CollectorGemsConfigFromPlayerEntity)(resolveBitmapRoomPlayer(analysis, room));
    ...
    const gemSystem = buildBitmapGemSystemAsm(rooms, playerHitbox, hudLinkedRamCursor, collectorGemsConfig, gemCounterEntry
    ...
    const dialogueSystem = buildBitmapDialogueSystemAsm(dialogueData, rooms, playerHitbox, hudLinkedRamCursor, dialogueVramBaseRow, buildRleUploadAsm(dialogueRleChunks, isKonamiMegaRom), `${keyDoorSystem.initialDrawCall}${gemSystem.initialDrawCall}`, isKonamiMegaRom);
    hudLinkedRamCursor += dialogueSystem.ramBytes;
    if ((linkedHudDynamicSources.length || keyDoorSystem.enabled || gemSystem.enabled || dialogueSystem.enabled) && hudLinkedRamCursor > HUD_LINKED_RAM_CEILING) {
        throw new Error(`MSX2 SCREEN 5 bitmap room "${room.name}" needs too much RAM for dynamic HUD/key-door/gem/dialogue systems: chain (${hexWord(hudLinkedRamCursor)}) would overflow the reserved player-animation block at ${hexWord(HUD_LINKED_RAM_CEILING)}. Reduce dynamic HUD widgets, disable air timer, or reduce key pickups/locked doors/gems.`);
    }
...
${hudDec3BufferAddress !== undefined ? `hud_dec3_buffer EQU ${hexWord(hudDec3BufferAddress)}\n` : ''}${hudDec5BufferAddress !== undefined ? `hud_dec5_buffer EQU ${hexWord(hudDec5BufferAddress)}\n` : ''}`
    ...
    const doubleJumpEquates = (0, msx2BitmapDoubleJumpGenerator_1.buildBitmapDoubleJumpEquates)(playerPhysics);
    const doubleJumpInitClear = (0, msx2BitmapDoubleJumpGenerator_1.buildBitmapDoubleJumpInitClearAsm)(playerPhysics);
    ...
    const coyoteBufferEquates = (0, msx2BitmapDoubleJumpGenerator_1.buildBitmapCoyoteBufferEquates)(playerPhysics);
    const coyoteBufferInitClear = (0, msx2BitmapDoubleJumpGenerator_1.buildBitmapCoyoteBufferInitClearAsm)(playerPhysics);
    const coyoteBufferLandHook = (0, msx2BitmapDoubleJumpGenerator_1.buildBitmapCoyoteBufferLandHookAsm)(playerPhysics);
    const coyoteBufferLeaveGroundHook = (0, msx2BitmapDoubleJumpGenerator_1.buildBitmapCoyoteBufferLeaveGroundHookAsm)(playerPhysics);
    ...
    const wallClimbConfig = (0, msx2BitmapWallClimbGenerator_1.resolveBitmapWallClimbConfig)(resolveBitmapRoomPlayer(analysis, room));
    const wallClimbGravityHook = (0, msx2BitmapWallClimbGenerator_1.buildBitmapWallClimbGravityHookAsm)(wallClimbConfig);
    const wallClimbRuntime = (0, msx2BitmapWallClimbGenerator_1.buildBitmapWallClimbRuntimeAsm)(wallClimbConfig);
    ...
    }, foregroundContext, true /* enableBlink: bitmap backend always renders i-frame flicker */, keyDoorSystem.enabled, `${keyDoorSystem.pendingPageDrawCall}${gemSystem.pendingPageDrawCall}`);
    ...
    const roomDataAsm = isKonamiMegaRom
        ...
        : roomTables.map(table => `${formatBytes(table.renderLabelPage0, table.renderBytesPage0, `Room ${table.index} page 0 render program: ${table.blockCount} V9938 command blocks (clear + 16x16 tile copies)`)}` +
            ...
            `${formatBytes(table.behaviorLabel, table.behaviorBytes, `Room ${table.index} ${COLLISION_COLS}x${COLLISION_ROWS} behavior grid (16x16 px cells), row-major, 0=empty, 3=ice_slide default`)}`).join('\n');
    ...
    const roomSpawnTableAsm = `bitmap_room_spawn_x_table:\n    DB ${roomSpawnBytes.map(b => `${b.x}`).join(',')}\n` +
...
function generateMsx2Screen5BitmapRoomFiles(projectName, analysis, config) {
