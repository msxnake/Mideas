...
; --- MULTI-SCREEN SHAFT state (6 bytes): 1 cabin(s) x 4
; (slot, y, dir, rider) + 1 shared forced-room byte ---
bitmap_shaft_state EQU #C0E7
...
bitmap_shaft_forced_room EQU #C0EB
...
bitmap_shaft_handover EQU #C0EC
...
.shaft_have_dir:
    ; start_room_transition destroys IX (and AF/BC/DE/HL). Everything below --
    ; and the caller's cabin loop -- addresses the cabin through IX, so it MUST
    ; be saved across the call; without this the writes below land on garbage
    ; and the ROM walks off into the BIOS.
    push ix
    call start_room_transition
    pop ix
    pop bc
...
; ------------------------------------------------------------
; PURPOSE: Write the 1 fixed shaft SAT slot(s) at VRAM #F608.
