...
exports.MSX2_BOX2_RUNTIME_BYTES = exports.MSX2_BOX2_RAM_BASE = void 0;
exports.usesMsx2Box2 = usesMsx2Box2;
exports.buildMsx2Box2Equates = buildMsx2Box2Equates;
exports.buildMsx2Box2DataTables = buildMsx2Box2DataTables;
exports.buildMsx2Box2PlayerHookAsm = buildMsx2Box2PlayerHookAsm;
exports.buildMsx2Box2HardwareSpriteAttrWrite = buildMsx2Box2HardwareSpriteAttrWrite;
exports.buildMsx2Box2HardwareSpriteSatRefreshAsm = buildMsx2Box2HardwareSpriteSatRefreshAsm;
exports.buildMsx2Box2RuntimeAsm = buildMsx2Box2RuntimeAsm;
...
const formatHexByte = (value) => `#${Math.max(0, Math.min(255, value)).toString(16).toUpperCase().padStart(2, '0')}`;
const formatHexWord = (value) => `#${Math.max(0, Math.min(0xFFFF, value)).toString(16).toUpperCase().padStart(4, '0')}`;
...
exports.MSX2_BOX2_RAM_BASE = 0xC047;
exports.MSX2_BOX2_RUNTIME_BYTES = 8 + msx2Box2RuntimeGenerator_1.MSX2_MAX_BOX2_PER_SCREEN * 5;
function usesMsx2Box2(analysis, tileScreens = []) {
    if ((0, msx2Box2RuntimeGenerator_1.usesMsx2Box2FromScreens)(tileScreens))
        return true;
    const screens = analysis?.msx2Screen4TileScreens;
    return (0, msx2Box2RuntimeGenerator_1.usesMsx2Box2FromScreens)(screens || []);
}
function buildMsx2Box2Equates(ramBase = exports.MSX2_BOX2_RAM_BASE) {
    const slotBytes = msx2Box2RuntimeGenerator_1.MSX2_MAX_BOX2_PER_SCREEN;
...
msx2_box2_runtime_end EQU ${formatHexWord(ramBase + exports.MSX2_BOX2_RUNTIME_BYTES)}
MSX2_MAX_BOX2_PER_SCREEN EQU ${msx2Box2RuntimeGenerator_1.MSX2_MAX_BOX2_PER_SCREEN}
`;
}
function buildBox2RuntimeTables(slotsByScreen) {
    const countBytes = slotsByScreen.map(items => Math.min(msx2Box2RuntimeGenerator_1.MSX2_MAX_BOX2_PER_SCREEN, items.length));
    const pickNumber = (getter, fallback) => slotsByScreen.flatMap(items => Array.from({ length: msx2Box2RuntimeGenerator_1.MSX2_MAX_BOX2_PER_SCREEN }, (_unused, index) => {
        const slot = items[index];
        return slot ? getter(slot) : fallback;
    }));
    const restoreNameBytes = slotsByScreen.flatMap(items => Array.from({ length: msx2Box2RuntimeGenerator_1.MSX2_MAX_BOX2_PER_SCREEN }, (_unused, index) => {
        const slot = items[index];
...
function buildMsx2Box2DataTables(tileScreens, resolveTileBytes, slotsByScreen) {
    if (!(0, msx2Box2RuntimeGenerator_1.usesMsx2Box2FromScreens)(tileScreens))
        return '';
    const resolvedSlots = slotsByScreen
        ?? tileScreens.map(screen => (0, msx2Box2RuntimeGenerator_1.getMsx2Box2RuntimeSlotsForScreen)(screen, resolveTileBytes));
    const tables = buildBox2RuntimeTables(resolvedSlots);
    const slotCount = msx2Box2RuntimeGenerator_1.MSX2_MAX_BOX2_PER_SCREEN;
...
function buildMsx2Box2PlayerHookAsm(direction) {
...
function buildMsx2Box2HardwareSpriteAttrWrite(options) {
...
function buildMsx2Box2HardwareSpriteSatRefreshAsm(options) {
...
function buildMsx2Box2RuntimeAsm(options) {
    if (!options.enabled)
        return '';
...
`
        : `    ret
`;
    return `${(0, msx2GridSnapComponentGenerator_1.buildMsx2GridSnapCharDrawAsm)({ drawLabel: 'msx2_box2_grid_draw_char_block_16', scratchCharVar: 'msx2_box2_draw_char', includeErase: false })}
