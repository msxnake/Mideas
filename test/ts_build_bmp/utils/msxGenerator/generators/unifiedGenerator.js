...
const bossesGenerator_1 = require("./bossesGenerator");
...
function buildFarIrqLockEnter() {
    return `    ld a, (far_call_irq_lock_depth)
    inc a
    ld (far_call_irq_lock_depth), a
`;
}
function buildFarIrqLockLeave() {
    return `    ld a, (far_call_irq_lock_depth)
    dec a
    ld (far_call_irq_lock_depth), a
`;
}
function buildFarIrqRestoreGuard(doneLabel, requireP1BankZero = false) {
...
function splitBossAttackRuntimeModule(content) {
...
function splitScreenLoaderRuntimeModule(content) {
...
function splitComponentsResidentTailModule(content, enabled) {
    if (!enabled) {
...
function splitGameflowAuxModule(content) {
    let core = content;
    const auxBlocks = [];
    const aux2Blocks = [];
    const extractRange = (start, end, replacement) => {
    ...
    if (displayStart >= 0 && printHelperStart > displayStart) {
        const block = extractRange(displayStart, printHelperStart, '\n; [End screen renderer moved to gameflow_aux far module]\n');
        if (block) {
    ...
    if (endStringsStart >= 0 && endBlockMarker > endStringsStart) {
        const block = extractRange(endStringsStart, endBlockMarker, '\n; [End screen strings moved to gameflow_aux far module]\n');
        if (block) {
            auxBlocks.push(block);
        }
    }
    ...
    if (transitionHelpersMatch && transitionHelpersStart >= 0 && transitionHelpersEnd > transitionHelpersStart) {
        const block = extractRange(transitionHelpersStart, transitionHelpersEnd, '\n; [Transition row/VRAM helpers moved to gameflow_aux far module]\n');
        if (block) {
            auxBlocks.push(block);
            core = core
    ...
    if (textScrollHelpersMatch && textScrollHelpersStart >= 0 && textScrollHelpersEnd > textScrollHelpersStart) {
        const block = extractRange(textScrollHelpersStart, textScrollHelpersEnd, '\n; [TextScroll VRAM/color helpers moved to gameflow_aux far module]\n');
        if (block) {
            auxBlocks.push(block);
            core = core
    ...
    if (textScroll2HelpersMatch && textScroll2HelpersStart >= 0 && textScroll2HelpersEnd > textScroll2HelpersStart) {
        const block = extractRange(textScroll2HelpersStart, textScroll2HelpersEnd, '\n; [TextScroll2 pattern-scroll helpers moved to gameflow_aux far module]\n');
        if (block) {
            auxBlocks.push(block);
            core = core
    ...
    if (defaultConnectionMatch && defaultConnectionStart >= 0 && defaultConnectionEnd > defaultConnectionStart) {
        const block = extractRange(defaultConnectionStart, defaultConnectionEnd, '\n; [Default connection helper moved to gameflow_aux far module]\n');
        if (block) {
            auxBlocks.push(block);
    ...
    if (confirmInputMatch && confirmInputStart >= 0 && confirmInputEnd > confirmInputStart) {
        const block = extractRange(confirmInputStart, confirmInputEnd, '\n; [Confirm input helper moved to gameflow_aux far module]\n');
        if (block) {
            auxBlocks.push(block);
    ...
    if (textScreenStart >= 0 && textScreenEnd > textScreenStart) {
        const block = extractRange(textScreenStart, textScreenEnd, '\n; [Text screen renderer moved to gameflow_aux far module]\n');
        if (block) {
            auxBlocks.push(block);
    ...
    if (submenuHelpersStart >= 0 && submenuEndMarkerStart > submenuHelpersStart) {
        const block = extractRange(submenuHelpersStart, submenuEndMarkerStart, '\n; [SubMenu render/cursor helpers moved to gameflow_aux2 far module]\n');
        if (block) {
    ...
    if (printStringHelperMatch && printStringHelperStart >= 0 && printStringHelperEnd > printStringHelperStart) {
        ...
        const auxCallsPrintStringHelper = auxBlocks.some((block) => /\b(?:call|jp)\s+print_string_vram\b/i.test(block));
        if (auxCallsPrintStringHelper && !coreCallsPrintStringHelper) {
            const block = extractRange(printStringHelperStart, printStringHelperEnd, '\n; [Print string VRAM helper moved to gameflow_aux far module]\n');
            if (block) {
                auxBlocks.push(block);
            }
        }
...
function rewriteDynamicFarCallSites(files, farModuleKeys) {
    ...
    if (farModuleKeys.has('patterns_code')) {
        Object.keys(rewritten).forEach((fileKey) => {
            ...
            rewritten[fileKey] = rewritten[fileKey].replace(/\bDW\s+(load_tilebank_[A-Za-z0-9_]+_patterns_to_vram)\b/g, 'DW $1_far');
        });
    }
    if (farModuleKeys.has('colors_code')) {
        Object.keys(rewritten).forEach((fileKey) => {
            ...
            rewritten[fileKey] = rewritten[fileKey].replace(/\bDW\s+(load_tilebank_[A-Za-z0-9_]+_colors_to_vram)\b/g, 'DW $1_far');
        });
    }
    if (farModuleKeys.has('components_autocontrol')) {
        Object.keys(rewritten).forEach((fileKey) => {
            rewritten[fileKey] = replaceCallInstruction(rewritten[fileKey], 'init_auto_control_script_system', 'init_auto_control_script_system_far');
            rewritten[fileKey] = replaceCallInstruction(rewritten[fileKey], 'update_auto_control_script_component', 'update_auto_control_script_component_far');
            rewritten[fileKey] = replaceCallInstruction(rewritten[fileKey], 'update_auto_event_string_component', 'update_auto_event_string_component_far');
        });
    }
    return rewritten;
}
...
function generateResidentCallWrappers(farModuleKeys, availableLabels, targetFormat, includeBossWrappers = true) {
...
function buildSegmentBudgetJson(packedBanks, dataPack, mapperWindow, bossDataBanks = { asm: '', bankCount: 0, banks: [] }, ascii16HiddenCallReport = { count: 0, samples: [] }, ascii16ResidentBridgeReport = { count: 0, samples: [] }, ascii16ResidentWindowOverflowReport = {
    count: 0,
    samples: [],
    estimatedOutOfWindowLabelCount: 0,
    estimatedOutOfWindowLabelSamples: [],
...
function generateMegaromUnifiedFile(files, projectName, analysis, executionPlan, config, options) {
    ...
    const componentsRuntimeModules = splitComponentsResidentTailModule(files['components.asm'], config.targetFormat !== 'ascii16');
    ...
    const bossDataBanks = (0, bossesGenerator_1.generateBossDataBankSections)(analysis, firstBossDataPhysicalBank, dataZoneSize, {
    ...
    const emittedFiles = rewriteResidentCallSites({ ...files, 'gameflow.asm': gameflowRuntimeModules.core }, farModuleKeySet, config.targetFormat, farCodeBanks);
    ...
    const emittedComponentsRuntimeModules = splitComponentsResidentTailModule(emittedFiles['components.asm'], config.targetFormat !== 'ascii16');
...
`
    ...
    const residentCallWrappers = generateResidentCallWrappers(farModuleKeySet, availableWrapperTargets, config.targetFormat, hasBossRuntime);
...
`;
    ...
    const segmentBudgetJson = buildSegmentBudgetJson(packedBanks, dataPack, mapperWindow, bossDataBanks, ascii16HiddenCallReport, ascii16ResidentBridgeReport, ascii16ResidentWindowOverflowReport, ascii16FarToFarCallReport);
