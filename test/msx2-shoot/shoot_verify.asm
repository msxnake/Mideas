...
; --- SHOOT skill runtime state (10 bytes) ---
bitmap_bullet_pool     EQU #C0E7
bitmap_shoot_cooldown  EQU #C0EF
bitmap_shoot_lock      EQU #C0F0

; --- TELEPORT A-B skill runtime state (8 bytes) ---
bitmap_teleport_cooldown EQU #C0F1
bitmap_teleport_delay    EQU #C0F2
bitmap_teleport_lock     EQU #C0F3
bitmap_teleport_flags    EQU #C0F4
bitmap_teleport_ax       EQU #C0F5
bitmap_teleport_ay       EQU #C0F6
bitmap_teleport_bx       EQU #C0F7
bitmap_teleport_by       EQU #C0F8
BITMAP_TELEPORT_FLAG_HAS_A    EQU #01
BITMAP_TELEPORT_FLAG_HAS_B    EQU #02
BITMAP_TELEPORT_FLAG_TARGET_B EQU #04

; --- SLASH skill runtime state (3 bytes) ---
bitmap_slash_timer    EQU #C0F9
bitmap_slash_cooldown EQU #C0FA
bitmap_slash_lock     EQU #C0FB

; --- GRAB skill runtime state (1 byte) ---
bitmap_grab_active EQU #C0FC

; --- HIGH JUMP skill runtime state (1 byte) ---
bitmap_highjump_hold EQU #C0FD

; --- WALL BREAK skill runtime state (2 bytes) ---
bitmap_wallbreak_cooldown EQU #C0FE
bitmap_wallbreak_lock     EQU #C0FF

; --- SPIN ATTACK skill runtime state (3 bytes) ---
bitmap_spin_timer    EQU #C100
bitmap_spin_cooldown EQU #C101
bitmap_spin_lock     EQU #C102
...
bitmap_enemy_count EQU #C103
bitmap_enemy_pool  EQU #C104
bitmap_enemy_update_lane EQU #C132
; --- SCREEN 5 CARRY & THROW runtime (32 bytes) ---
bitmap_carry_slot     EQU #C134
bitmap_carry_lock     EQU #C135
bitmap_carry_cooldown EQU #C136
bitmap_carry_count    EQU #C137
bitmap_carry_draw_page EQU #C138
bitmap_carry_x        EQU #C139
bitmap_carry_y        EQU #C13A
bitmap_carry_vx       EQU #C13B
bitmap_carry_vy       EQU #C13C
bitmap_carry_state    EQU #C13D
bitmap_carry_pattern  EQU #C13E
bitmap_carry_render   EQU #C13F
bitmap_carry_source_x EQU #C140
bitmap_carry_source_row EQU #C141
bitmap_carry_drawn    EQU #C142
bitmap_carry_old_x    EQU #C143
bitmap_carry_old_y    EQU #C144
bitmap_carry_cmd_buffer EQU #C145
...
init_rom:
    ...
    call vdp_write_register
    xor a
    ; Clear SHOOT pool (10 bytes at bitmap_bullet_pool)
...
.bitmap_main_loop:
    ...
    call bitmap_try_teleport_ab
    call bitmap_try_slash
    call bitmap_grab_detect
    call bitmap_try_wall_break
    call bitmap_try_spin_attack
...
update_player_movement:
    ...
    pop bc
    jp c, .apply_gravity
    ; HIGH JUMP: arm hold counter when player just left the ground ascending.
    ld a, (player_vy)
    or a
    jp p, .hj_input_done
...
.after_gravity_tick:
    ...
    ; GRAB gravity hook: clamp fall to slideSpeed when clinging to wall.
    ...
    ld (player_vy), a
.grab_hook_done:
    ; HIGH JUMP gravity hook: reduce fall while jump held and counter active.
    ld a, (bitmap_highjump_hold)
    or a
    jp z, .hj_hook_done
...
; ------------------------------------------------------------
; PURPOSE: Reads the teleport key ('O', keyboard matrix row 4 bit 4) via PPI.
; INPUT: none. OUTPUT: A = 1 when pressed, A = 0 otherwise (Z when not pressed).
; DESTROYS: AF. PRESERVES: BC, DE, HL, IX, IY.
; ------------------------------------------------------------
...
; FUNCTION: bitmap_slash_pressed — reads 'P' key (row 4 bit 5).
...
;   player_vy to #01 (slide) when grabbing.
...
; FUNCTION: bitmap_wallbreak_pressed — reads 'R' key (row 4 bit 7).
...
; FUNCTION: bitmap_spin_pressed — reads 'Q' key (row 4 bit 6).
bitmap_spin_pressed:
    in a, (PPI_C)
    and #F0
    or 4
...
.benemy_slot_0:
    ...
    ; --- upload frameCount*2 pattern groups -> VRAM #F920 (group 9+) ---
    ld a, (ix+8)
    ...
    ld de, #F920
    call copy_to_vram_ext
    ; --- upload 16-byte colour table -> VRAM #F420 (slot 0) ---
    ld a, (ix+9)
...
.benemy_slot_1:
    ...
    ; --- upload frameCount*2 pattern groups -> VRAM #F9A0 (group 13+) ---
    ld a, (ix+8)
    call bitmap_enemy_patterns_offset
    ld a, (ix+10)             ; frameCount
    add a, a                  ; *2 variants
    ...
    ld de, #F9A0
    call copy_to_vram_ext
    ; --- upload 16-byte colour table -> VRAM #F430 (slot 1) ---
    ld a, (ix+9)
    call bitmap_enemy_colors_offset
    ld de, #F430
    ld bc, 16
...
; ------------------------------------------------------------
; PURPOSE: Writes the 2 fixed enemy SAT slot(s) at VRAM #F608
...
bitmap_carry_sprite_colors:
    DB #0E,#0E,#0F,#0F,#0F,#08,#08,#08,#08,#0F,#0F,#0F,#08,#08,#08,#08
...
bitmap_room_enemy_table_0:
    DB #02,#90,#90,#00,#00,#90,#90,#90,#90,#00,#00,#02,#07,#00,#00,#00
    DB #00,#00,#00,#10,#10,#02,#00,#90,#90,#00,#00,#90,#90,#90,#90,#04
    DB #02,#02,#07,#00,#00,#00,#00,#00,#00,#10,#10,#02,#00
