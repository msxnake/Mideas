...
;           },
;           {
;             "id": "msx2sprite.msx2sprite_1781331326246",
;             "usedBytes": 112,
;             "recommendedBankClass": "world.graphics.sprite"
;           },
;           {
;             "id": "msx2sprite.msx2sprite_1781266153662",
;             "usedBytes": 80,
;             "recommendedBankClass": "world.graphics.sprite"
;           },
;           {
...
;           },
;           {
;             "packageId": "msx2sprite.msx2sprite_1781266153662",
;             "type": "msx2sprite",
;             "sourceId": "msx2sprite_1781266153662",
;             "logicalSection": "world graphics",
;             "recommendedBankClass": "world.graphics.sprite",
;             "physicalBankIndex": 0,
;             "windowAddress": "#8000",
...
;           },
;           {
;             "packageId": "msx2sprite.msx2sprite_1781331326246",
;             "type": "msx2sprite",
;             "sourceId": "msx2sprite_1781331326246",
;             "logicalSection": "world graphics",
;             "recommendedBankClass": "world.graphics.sprite",
;             "physicalBankIndex": 0,
;             "windowAddress": "#8000",
...
;     "reachableMsx2SpriteCount": 4,
;     "reachableWorldCount": 1,
;     "usesKonamiDataBank": true,
;     "romPayloadBytesEstimate": 5145,
...
msx2_carry_carried_slot EQU #C079
msx2_carry_lock EQU #C07A
msx2_carry_cooldown EQU #C07B
msx2_carry_fly_dir EQU #C07C
msx2_carry_count EQU #C07D
msx2_carry_runtime_x EQU #C07F
msx2_carry_runtime_y EQU #C081
msx2_carry_runtime_state EQU #C083
msx2_carry_runtime_pattern EQU #C085
MSX2_MAX_CARRYABLES_PER_SCREEN EQU 2
...
msx2_hazard_probe_ox EQU #C600
msx2_hazard_probe_oy EQU #C601
msx2_hazard_probe_w EQU #C602
msx2_hazard_probe_h EQU #C603
msx2_enemy_runtime_x EQU #C610
msx2_enemy_runtime_y EQU #C61C
msx2_enemy_runtime_dx EQU #C628
msx2_enemy_runtime_dy EQU #C634
msx2_enemy_runtime_mode EQU #C640
msx2_enemy_runtime_speed EQU #C64C
msx2_enemy_runtime_tick EQU #C658
msx2_enemy_anim_counter EQU #C664
msx2_enemy_anim_frame EQU #C665
msx2_runtime_ram_end EQU #C666
...
.enemy_bullet_try_spawn:
    ...
    ; Bat bomb gate: only flagged enemies, and only when this enemy's X is
    ...
    ld e, a
    ld d, 0
    ld hl, msx2_screen_enemy_drop_bomb
    add hl, de
    ld a, (hl)
    or a
    jp z, .enemy_bullet_no_spawn_0
    ld hl, msx2_enemy_runtime_x
    ld b, (hl)
    ld a, (msx2_player_sprite_x)
    sub b
    jp nc, .enemy_bomb_aligned_pos_0
    neg
.enemy_bomb_aligned_pos_0:
    cp 9
...
.enemy_bullet_no_spawn_0:
    ...
    ld a, (msx2_player_sprite_x)
    sub b
    jp nc, .enemy_bomb_aligned_pos_1
    neg
.enemy_bomb_aligned_pos_1:
    cp 9
...
.enemy_bullet_no_spawn_1:
    ...
    ld a, (msx2_player_sprite_x)
    sub b
    jp nc, .enemy_bomb_aligned_pos_2
    neg
.enemy_bomb_aligned_pos_2:
    cp 9
...
.enemy_bullet_no_spawn_2:
    ...
    ld a, (msx2_player_sprite_x)
    sub b
    jp nc, .enemy_bomb_aligned_pos_3
    neg
.enemy_bomb_aligned_pos_3:
    cp 9
...
.enemy_bullet_no_spawn_3:
    ...
    ld a, (msx2_player_sprite_x)
    sub b
    jp nc, .enemy_bomb_aligned_pos_4
    neg
.enemy_bomb_aligned_pos_4:
    cp 9
...
.enemy_bullet_no_spawn_4:
    ...
    ld a, (msx2_player_sprite_x)
    sub b
    jp nc, .enemy_bomb_aligned_pos_5
    neg
.enemy_bomb_aligned_pos_5:
    cp 9
...
.enemy_bullet_no_spawn_5:
    ...
    ld a, (msx2_player_sprite_x)
    sub b
    jp nc, .enemy_bomb_aligned_pos_6
    neg
.enemy_bomb_aligned_pos_6:
    cp 9
...
.enemy_bullet_no_spawn_6:
    ...
    ld a, (msx2_player_sprite_x)
    sub b
    jp nc, .enemy_bomb_aligned_pos_7
    neg
.enemy_bomb_aligned_pos_7:
    cp 9
...
.enemy_bullet_no_spawn_7:
    ...
    ld a, (msx2_player_sprite_x)
    sub b
    jp nc, .enemy_bomb_aligned_pos_8
    neg
.enemy_bomb_aligned_pos_8:
    cp 9
...
.enemy_bullet_no_spawn_8:
    ...
    ld a, (msx2_player_sprite_x)
    sub b
    jp nc, .enemy_bomb_aligned_pos_9
    neg
.enemy_bomb_aligned_pos_9:
    cp 9
...
.enemy_bullet_no_spawn_9:
    ...
    ld a, (msx2_player_sprite_x)
    sub b
    jp nc, .enemy_bomb_aligned_pos_10
    neg
.enemy_bomb_aligned_pos_10:
    cp 9
...
.enemy_bullet_no_spawn_10:
    ...
    ld a, (msx2_player_sprite_x)
    sub b
    jp nc, .enemy_bomb_aligned_pos_11
    neg
.enemy_bomb_aligned_pos_11:
    cp 9
...
update_hardware_sprite_vertical:
    ...
    ; Jump accepts the cached grounded flag when the player is already settled.
...
.box2_sprite_hide:
    ...
    call write_vram_byte_ext
.box2_sprite_done:
    ; Carryable object hardware sprite slot 0.
    ld a, (msx2_carry_runtime_state + 0)
    cp #FF
    jp nz, .carry_sprite_0_visible
    ld a, 208
    ld hl, #1E48
    call write_vram_byte_ext
    jp .carry_sprite_0_done
.carry_sprite_0_visible:
    ld a, (msx2_carry_runtime_y + 0)
    ld hl, #1E48
    call write_vram_byte_ext
    ld a, (msx2_carry_runtime_x + 0)
    ld hl, #1E49
    call write_vram_byte_ext
    ld a, (msx2_carry_runtime_pattern + 0)
    add a, a
    add a, a
    add a, #38
    ld hl, #1E4A
...
.carry_sprite_0_done:
    ...
    ; Carryable object hardware sprite slot 1.
    ld a, (msx2_carry_runtime_state + 1)
    cp #FF
    jp nz, .carry_sprite_1_visible
    ld a, 208
    ld hl, #1E4C
    call write_vram_byte_ext
    jp .carry_sprite_1_done
.carry_sprite_1_visible:
    ld a, (msx2_carry_runtime_y + 1)
    ld hl, #1E4C
    call write_vram_byte_ext
    ld a, (msx2_carry_runtime_x + 1)
    ld hl, #1E4D
    call write_vram_byte_ext
    ld a, (msx2_carry_runtime_pattern + 1)
    add a, a
    add a, a
    add a, #38
    ld hl, #1E4E
...
upload_hardware_sprite_attrs:
    ...
    call update_msx2_enemy_sprite_animation
...
msx2_control_carry_pressed:
    ; ------------------------------------------------------------
    ; FUNCTION: msx2_control_carry_pressed
    ; PURPOSE: A=1 when the configured carry object skill input combo is held.
...
.carry_try_pickup:
    ...
    jp .carry_pickup_hit
.carry_pickup_miss_0:
    ld ix, msx2_carry_runtime_x + 1
    call msx2_carry_pickup_probe
    or a
    jp z, .carry_pickup_miss_1
    ld a, 1
    jp .carry_pickup_hit
.carry_pickup_miss_1:
    jp .carry_update_phases
...
apply_PANTALLA1_collected_visuals:
    ...
    srl a
    cp 3
    jp z, keep_PANTALLA1_collectible_0
    ld hl, #19D0
    call clear_screen4_name_cell_16
keep_PANTALLA1_collectible_0:
    ret
...
; Per-msx2screen "drop bomb when aligned with player X" flag, 12 slots per screen
msx2_screen_enemy_drop_bomb:
    DB #01,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00
...
msx2_screen_box2_x:
    DB #60,#B0,#50,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00

; Per-screen box2 initial Y (8-aligned px), 8 slots/screen
msx2_screen_box2_y:
    DB #70,#90,#A0,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00

; Per-screen box2 2x2 char block base, 8 slots/screen
msx2_screen_box2_char_base:
    DB #09,#09,#09,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
...
msx2_screen_box2_restore_names:
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00


; Per-msx2screen carryable entity count, capped at 2
msx2_screen_carry_count:
    DB #01,#00
; Per-msx2screen carryable X coordinates, 2 slots per screen
msx2_screen_carry_x:
    DB #80,#00,#00,#00
; Per-msx2screen carryable Y coordinates, 2 slots per screen
msx2_screen_carry_y:
    DB #70,#00,#00,#00
; Per-msx2screen carryable unique sprite indices, 2 slots per screen
msx2_screen_carry_sprite:
    DB #00,#00,#00,#00
; Carryable sprite color rows, 16 bytes per unique carryable sprite
msx2_carry_color_rows:
    DB #0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0F,#0F,#0F,#0F,#0F,#0E
...
msx2_hw_enemy_sprite_pattern:
    DB #00,#00,#00,#0C,#0A,#06,#7E,#C3,#81,#01,#01,#00,#00,#00,#00,#00
    DB #00,#00,#00,#60,#A0,#C0,#FE,#83,#01,#00,#00,#00,#00,#00,#00,#00
; Shared enemy/hazard hardware sprite frame 1 from MSX2 entity sprite asset
msx2_hw_enemy_sprite_frame_1_pattern:
    DB #00,#00,#00,#0C,#0A,#06,#1E,#23,#41,#81,#01,#00,#00,#00,#00,#00
    DB #00,#00,#00,#60,#A0,#C0,#F0,#88,#04,#02,#00,#00,#00,#00,#00,#00
; Shared enemy/hazard hardware sprite frame 2 from MSX2 entity sprite asset
msx2_hw_enemy_sprite_frame_2_pattern:
    DB #00,#00,#0C,#0A,#06,#0E,#13,#11,#11,#11,#10,#08,#00,#00,#00,#00
    DB #00,#00,#60,#A0,#C0,#E0,#90,#10,#10,#10,#10,#20,#00,#00,#00,#00
...
; Carryable object hardware sprite pattern 0 from msx2sprite msx2sprite_1781266153662
msx2_hw_carry_sprite_pattern_0:
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#38,#54,#6F,#54,#38,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#FE,#3A,#08,#00
...
msx2_hw_enemy_sprite_colors_0:
    DB #0D,#0D,#0D,#0B,#0B,#0B,#05,#05,#05,#05,#05,#0D,#0D,#0D,#0D,#0D
...
msx2_hw_sprite_attrs:
    ...
    DB #D0,#00,#20,#00,#D0,#00,#20,#00,#D0,#00,#20,#00,#D0,#00,#20,#00
    DB #D0,#00,#20,#00,#D0,#00,#20,#00,#D0,#00,#2C,#00,#D0,#00,#2C,#00
    DB #D0,#00,#30,#00,#D0,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
...
PANTALLA1_NAMES:
    ...
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0E,#0E,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0E,#0E,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0E,#0E,#0D,#0D,#0D,#0D,#0E,#0E,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0E,#0E,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0E,#0E,#0D,#0D,#0D,#0D,#0E,#0E,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D
    ...
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D
    DB #0E,#0E,#0D,#0D,#0D,#0D,#0E,#0E,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0E,#0E,#0D,#0D,#0D,#0D,#0E,#0E,#0D,#0D
    DB #0F,#0F,#10,#11,#0D,#0D,#0F,#0F,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D
    DB #0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0D,#0D,#0D,#0D,#09,#0A,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0E,#0E,#0D,#0D
    DB #0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#0D,#0D,#0D,#0D,#0B,#0C,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0D,#0F,#0F,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E
    DB #0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0E,#0D,#0D
    DB #0D,#0D,#0D,#0D,#0D,#0D,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F
    DB #0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#0D,#0D
...
PANTALLA1_COLLISION:
    ...
    DB #00,#00,#00,#01,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#01,#00
    DB #00,#00,#00,#01,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#01,#00
    DB #00,#00,#00,#01,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#01,#00
    DB #00,#00,#00,#01,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#01,#00
    DB #00,#00,#00,#01,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#01,#00
    DB #01,#00,#00,#01,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#01,#00
    DB #01,#01,#01,#01,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#01,#00
...
PANTALLA1_CELL_FLAGS:
    ...
    DB #00,#00,#00,#01,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#01,#00
    DB #00,#00,#00,#01,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#01,#00
    DB #01,#02,#00,#01,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#01,#00
    DB #01,#01,#01,#01,#00,#00,#29,#00,#06,#00,#00,#00,#00,#00,#01,#00
    DB #00,#00,#00,#01,#01,#01,#01,#01,#01,#01,#01,#01,#01,#01,#01,#00
...
PANTALLA1_VISUAL_MAP:
    ...
    DB #00,#00,#00,#01,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#01,#00
    DB #00,#00,#00,#01,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#01,#00
    DB #01,#03,#00,#01,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#01,#00
    DB #01,#01,#01,#01,#00,#00,#02,#00,#00,#00,#00,#00,#00,#00,#01,#00
    DB #00,#00,#00,#01,#01,#01,#01,#01,#01,#01,#01,#01,#01,#01,#01,#00
