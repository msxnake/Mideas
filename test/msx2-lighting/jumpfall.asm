...
bitmap_sm_state EQU #D023
bitmap_light_x                 EQU #D024
bitmap_light_y                 EQU #D025
bitmap_light_tx                EQU #D026
bitmap_light_ty                EQU #D027
bitmap_light_active            EQU #D028
bitmap_light_page              EQU #D029
bitmap_light_op_clr            EQU #D02A
bitmap_light_op_cmd            EQU #D02B
bitmap_light_d                 EQU #D02C
bitmap_light_xsign             EQU #D02D
bitmap_light_xadj              EQU #D02E
bitmap_light_ybias             EQU #D02F
bitmap_light_rx                EQU #D030
bitmap_light_ry                EQU #D031
bitmap_light_rw                EQU #D032
bitmap_light_rh                EQU #D034
bitmap_light_band_y            EQU #D035
bitmap_light_band_h            EQU #D036
bitmap_light_band_hw           EQU #D037
; --- MOVING PLATFORM runtime state (26 bytes): count + rider + 2 slot(s) x 11
; (x,y,dx,dy,minX,maxX,minY,maxY,widthCells,movedX,movedY) ---
bitmap_platform_count EQU #D009
bitmap_platform_rider EQU #D00A
bitmap_platform_pool  EQU #D00B
; Per-slot colour state: 0 = authored (normal room), 1 = dim, 2 = inside halo.
bitmap_platform_light_state EQU #D021
...
; ------------------------------------------------------------
; PURPOSE:
;   Assert the authored jump / fall animation clip while the player is off
...
; Sprite 0 line color table (mode 2): configured player sprite "player_jump" + 6 state clip(s)
...
; Sprite 0 pattern (16x16, mode 2 quadrants): configured player sprite "player_jump" + 6 state clip(s)
...
; Player animation clip table: id 0 = base idle/walk, ids 1..6 = state
; clips. 3 bytes/entry: frameBase, frameCount, delayFrames. Indexed by player_anim_state.
; 1=IDLE(base 0,2f), 2=WALK(base 2,2f), 3=perceiving(base 4,1f), 4=ATTACK(base 5,1f), 5=jumping(base 0,2f), 6=falling(base 0,2f)
