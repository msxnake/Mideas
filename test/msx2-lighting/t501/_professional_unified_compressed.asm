...
; Project: test501_bats1_professional
; Room: caverna1
...
bitmap_sm_state EQU #D0D9
bitmap_light_x                 EQU #D0DA
bitmap_light_y                 EQU #D0DB
bitmap_light_tx                EQU #D0DC
bitmap_light_ty                EQU #D0DD
bitmap_light_active            EQU #D0DE
bitmap_light_page              EQU #D0DF
bitmap_light_op_clr            EQU #D0E0
bitmap_light_op_cmd            EQU #D0E1
bitmap_light_d                 EQU #D0E2
bitmap_light_xsign             EQU #D0E3
bitmap_light_xadj              EQU #D0E4
bitmap_light_ybias             EQU #D0E5
bitmap_light_rx                EQU #D0E6
bitmap_light_ry                EQU #D0E7
bitmap_light_rw                EQU #D0E8
bitmap_light_rh                EQU #D0EA
bitmap_light_band_y            EQU #D0EB
bitmap_light_band_h            EQU #D0EC
bitmap_light_band_hw           EQU #D0ED
bitmap_light_on                EQU #D0EE
bitmap_light_stage             EQU #D0EF
bitmap_light_timer             EQU #D0F0
bitmap_light_bands_ptr         EQU #D0F2
bitmap_light_sdown_ptr         EQU #D0F4
bitmap_light_sdown_n           EQU #D0F6
bitmap_light_sup_ptr           EQU #D0F7
bitmap_light_sup_n             EQU #D0F9
bitmap_light_cxmin             EQU #D0FA
bitmap_light_cxmax             EQU #D0FB
bitmap_mush_cx                 EQU #D0FC
bitmap_mush_cy                 EQU #D0FD
bitmap_light_srx               EQU #D0FE
bitmap_light_sry               EQU #D0FF
bitmap_light_srw               EQU #D100
bitmap_light_srh               EQU #D101
bitmap_light_tx0               EQU #D102
bitmap_light_ty0               EQU #D103
bitmap_light_tw                EQU #D104
bitmap_light_th                EQU #D105
bitmap_mush_ex                 EQU #D106
bitmap_mush_ey                 EQU #D107
bitmap_mush_flag               EQU #D108
bitmap_mush_sx                 EQU #D109
bitmap_mush_sy                 EQU #D10A
bitmap_light_protect           EQU #D10C
bitmap_mush_flags              EQU #D10D
bitmap_bl_on                   EQU #D10E
bitmap_bl_x                    EQU #D10F
bitmap_bl_y                    EQU #D110
; --- ENEMY runtime state (106 bytes): count + 4 slot(s) x 26 + PRNG seed
; (x,y,dx,dy,minX,maxX,minY,maxY,animTick,animFrame,frameCount,animDelay,colorOff,mode,xOff,yOff,damage,hitX,hitY,hitW,hitH,speed,logicInterval,logicCountdown,flyLeft,flyTurnPx) ---
bitmap_enemy_count EQU #D055
bitmap_enemy_pool  EQU #D056
bitmap_enemy_rand_seed EQU #D0BE
; --- MOVING PLATFORM runtime state (26 bytes): count + rider + 2 slot(s) x 11
; (x,y,dx,dy,minX,maxX,minY,maxY,widthCells,movedX,movedY) ---
bitmap_platform_count EQU #D0BF
bitmap_platform_rider EQU #D0C0
bitmap_platform_pool  EQU #D0C1
; Per-slot colour state: 0 = authored (normal room), 1 = dim, 2 = inside halo.
bitmap_platform_light_state EQU #D0D7
...
.benemy_slot_1:
    ...
    ld a, (ix+21)             ; logic update interval in video frames (1..255)
    or a
    jp nz, .benemy_slot_1_interval_valid
    inc a                     ; legacy/corrupt zero falls back to every frame
.benemy_slot_1_interval_valid:
    ld (bitmap_enemy_pool + 26 + 22), a
    ld a, 1
    ld (bitmap_enemy_pool + 26 + 23), a  ; first gameplay frame executes logic
    ; Bat flight: fly the authored distance before the first random turn.
    ld a, (ix+22)          ; turnPx (0 on non-bat slots)
    ld (bitmap_enemy_pool + 26 + 25), a
    ld (bitmap_enemy_pool + 26 + 24), a
...
.benemy_slot_2:
    ...
    ld a, (ix+21)             ; logic update interval in video frames (1..255)
    or a
    jp nz, .benemy_slot_2_interval_valid
    inc a                     ; legacy/corrupt zero falls back to every frame
.benemy_slot_2_interval_valid:
    ld (bitmap_enemy_pool + 52 + 22), a
    ld a, 1
    ld (bitmap_enemy_pool + 52 + 23), a  ; first gameplay frame executes logic
    ; Bat flight: fly the authored distance before the first random turn.
    ld a, (ix+22)          ; turnPx (0 on non-bat slots)
    ld (bitmap_enemy_pool + 52 + 25), a
    ld (bitmap_enemy_pool + 52 + 24), a
...
.benemy_slot_3:
    ...
    ld a, (ix+21)             ; logic update interval in video frames (1..255)
    or a
    jp nz, .benemy_slot_3_interval_valid
    inc a                     ; legacy/corrupt zero falls back to every frame
.benemy_slot_3_interval_valid:
    ld (bitmap_enemy_pool + 78 + 22), a
    ld a, 1
    ld (bitmap_enemy_pool + 78 + 23), a  ; first gameplay frame executes logic
    ; Bat flight: fly the authored distance before the first random turn.
    ld a, (ix+22)          ; turnPx (0 on non-bat slots)
    ld (bitmap_enemy_pool + 78 + 25), a
    ld (bitmap_enemy_pool + 78 + 24), a
...
.fly8_turn_down:
    ld (ix+3), #01
.fly8_step_done:
    dec (ix+24)         ; one pixel of this heading spent
    jp nz, .fly8_next_px
    call .fly8_reroll
.fly8_next_px:
    dec d
...
.fly8_reroll:
    ...
    ld a, (hl)
    ld (ix+3), a               ; dy sign
    ld a, (ix+25)         ; rearm with the authored distance
    ld (ix+24), a
    pop hl
    pop de
    pop bc
...
; OUTPUT: enemy colour tables refreshed at VRAM #F440.
; DESTROYS: AF, DE. PRESERVES: BC, HL, IX, IY.
...
.color_slot_1:
    ...
    ld a, (bitmap_enemy_pool + 26 + 0)   ; logical X
    ld b, a
    ld a, (bitmap_enemy_pool + 26 + 1)   ; logical Y
    ld c, a
    call bitmap_enemy_light_reaches
    ld d, a                   ; D = 0 while no light reaches this slot
    ...
    ld e, a                   ; in the dark: the eyes-only bank sits one whole
    ld a, (bitmap_enemy_pool + 26 + 10)  ; set of frames later
    add a, e
.color_slot_1_lit:
    call bitmap_enemy_colors_offset
    ld de, #F450
...
.color_slot_2:
    ...
    ld a, (bitmap_enemy_pool + 52 + 0)   ; logical X
    ld b, a
    ld a, (bitmap_enemy_pool + 52 + 1)   ; logical Y
    ld c, a
    call bitmap_enemy_light_reaches
    ld d, a                   ; D = 0 while no light reaches this slot
    ...
    ld e, a                   ; in the dark: the eyes-only bank sits one whole
    ld a, (bitmap_enemy_pool + 52 + 10)  ; set of frames later
    add a, e
.color_slot_2_lit:
    call bitmap_enemy_colors_offset
    ld de, #F460
...
.color_slot_3:
    ...
    ld a, (bitmap_enemy_pool + 78 + 0)   ; logical X
    ld b, a
    ld a, (bitmap_enemy_pool + 78 + 1)   ; logical Y
    ld c, a
    call bitmap_enemy_light_reaches
    ld d, a                   ; D = 0 while no light reaches this slot
    ...
    ld e, a                   ; in the dark: the eyes-only bank sits one whole
    ld a, (bitmap_enemy_pool + 78 + 10)  ; set of frames later
    add a, e
.color_slot_3_lit:
    call bitmap_enemy_colors_offset
    ld de, #F470
...
; Room 0 enemies: count + 4 slot(s) x 23 (x,y,dx,dy,minX,maxX,minY,maxY,patOff,colOff,frames,delay,mode,xOff,yOff,damage,hitX,hitY,hitW,hitH,speed,logicInterval,turnPx)
bitmap_room_enemy_table_0:
    DB #04,#30,#20,#01,#01,#00,#F0,#00,#B0,#00,#00,#02,#06,#0D,#00,#00
    DB #01,#02,#03,#0C,#0A,#01,#03,#64,#30,#20,#01,#01,#00,#F0,#00,#B0
    DB #04,#04,#02,#06,#0D,#00,#00,#01,#02,#03,#0C,#0A,#01,#03,#64,#B0
    DB #40,#FF,#01,#00,#F0,#00,#B0,#00,#00,#02,#06,#0D,#00,#00,#01,#02
    DB #03,#0C,#0A,#01,#03,#64,#B0,#40,#FF,#01,#00,#F0,#00,#B0,#04,#04
    DB #02,#06,#0D,#00,#00,#01,#02,#03,#0C,#0A,#01,#03,#64
; Room 1 enemies: count + 4 slot(s) x 23 (x,y,dx,dy,minX,maxX,minY,maxY,patOff,colOff,frames,delay,mode,xOff,yOff,damage,hitX,hitY,hitW,hitH,speed,logicInterval,turnPx)
bitmap_room_enemy_table_1:
    ...
    DB #03,#0C,#0A,#01,#03,#64,#B0,#40,#FF,#01,#00,#F0,#00,#B0,#04,#04
    DB #02,#06,#0D,#00,#00,#01,#02,#03,#0C,#0A,#01,#03,#64
; Room 2 enemies: count + 4 slot(s) x 23 (x,y,dx,dy,minX,maxX,minY,maxY,patOff,colOff,frames,delay,mode,xOff,yOff,damage,hitX,hitY,hitW,hitH,speed,logicInterval,turnPx)
bitmap_room_enemy_table_2:
    ...
    DB #03,#0C,#0A,#01,#03,#64,#B0,#40,#FF,#01,#00,#F0,#00,#B0,#04,#04
    DB #02,#06,#0D,#00,#00,#01,#02,#03,#0C,#0A,#01,#03,#64
; Room 3 enemies: count + 4 slot(s) x 23 (x,y,dx,dy,minX,maxX,minY,maxY,patOff,colOff,frames,delay,mode,xOff,yOff,damage,hitX,hitY,hitW,hitH,speed,logicInterval,turnPx)
bitmap_room_enemy_table_3:
    ...
    DB #03,#0C,#0A,#01,#03,#64,#B0,#40,#FF,#01,#00,#F0,#00,#B0,#04,#04
    DB #02,#06,#0D,#00,#00,#01,#02,#03,#0C,#0A,#01,#03,#64
; Room 4 enemies: count + 4 slot(s) x 23 (x,y,dx,dy,minX,maxX,minY,maxY,patOff,colOff,frames,delay,mode,xOff,yOff,damage,hitX,hitY,hitW,hitH,speed,logicInterval,turnPx)
bitmap_room_enemy_table_4:
    ...
    DB #03,#0C,#0A,#01,#03,#64,#B0,#40,#FF,#01,#00,#F0,#00,#B0,#04,#04
    DB #02,#06,#0D,#00,#00,#01,#02,#03,#0C,#0A,#01,#03,#64
; Room 5 enemies: count + 4 slot(s) x 23 (x,y,dx,dy,minX,maxX,minY,maxY,patOff,colOff,frames,delay,mode,xOff,yOff,damage,hitX,hitY,hitW,hitH,speed,logicInterval,turnPx)
bitmap_room_enemy_table_5:
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#00
    DB #00,#00,#00,#10,#10,#01,#01,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#01,#00,#00,#00,#00,#00,#00,#10,#10,#01,#01,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#00,#00,#00
    DB #00,#10,#10,#01,#01,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#01,#00,#00,#00,#00,#00,#00,#10,#10,#01,#01,#00
; Room 6 enemies: count + 4 slot(s) x 23 (x,y,dx,dy,minX,maxX,minY,maxY,patOff,colOff,frames,delay,mode,xOff,yOff,damage,hitX,hitY,hitW,hitH,speed,logicInterval,turnPx)
bitmap_room_enemy_table_6:
    DB #01,#40,#10,#00,#00,#40,#40,#10,#10,#08,#08,#01,#08,#00,#00,#00
    DB #01,#00,#00,#10,#10,#02,#01,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#01,#00,#00,#00,#00,#00,#00,#10,#10,#01,#01,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#01,#00,#00,#00,#00,#00
    DB #00,#10,#10,#01,#01,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#01,#00,#00,#00,#00,#00,#00,#10,#10,#01,#01,#00
...
bitmap_enemy_sprite_patterns:
    DB #00,#00,#60,#30,#38,#3A,#3A,#1B,#1D,#0F,#03,#03,#01,#01,#00,#00
    DB #00,#00,#06,#0C,#1C,#5C,#5C,#D8,#B8,#F0,#C0,#C0,#80,#80,#00,#00
    DB #00,#00,#60,#30,#38,#3A,#3A,#1B,#1D,#0F,#03,#03,#01,#01,#00,#00
    DB #00,#00,#06,#0C,#1C,#5C,#5C,#D8,#B8,#F0,#C0,#C0,#80,#80,#00,#00
    DB #00,#00,#00,#00,#00,#02,#02,#03,#3D,#7F,#7B,#7B,#41,#01,#00,#00
    DB #00,#00,#00,#00,#00,#40,#40,#C0,#BC,#FE,#DE,#DE,#82,#80,#00,#00
    DB #00,#00,#00,#00,#00,#02,#02,#03,#3D,#7F,#7B,#7B,#41,#01,#00,#00
    DB #00,#00,#00,#00,#00,#40,#40,#C0,#BC,#FE,#DE,#DE,#82,#80,#00,#00
...
bitmap_enemy_sprite_colors:
    DB #0F,#0F,#02,#02,#02,#02,#02,#02,#02,#02,#02,#02,#02,#02,#0F,#0F
    DB #0F,#0F,#0F,#0F,#0F,#02,#02,#02,#02,#02,#02,#02,#02,#02,#0F,#0F
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00,#00
    DB #0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#05,#0F,#0F,#0F,#0F,#0F,#0F,#0F
    DB #0F,#0F,#0F,#0F,#0F,#0F,#0F,#0F,#05,#0F,#0F,#0F,#0F,#0F,#0F,#0F
