...
;
; OUTPUT:
;   bitmap_light_x advanced by at most 4 px.
;
; DESTROYS:
;   AF, BC, DE, HL.
; ------------------------------------------------------------
...
; ------------------------------------------------------------
; PURPOSE:
;   Move the halo vertically. Every band vacates rows at one end and gains rows
;   at the other, and the rows a band vacates belong to its neighbour after the
;   move -- so ALL the dim passes must run before ANY of the lit passes, or a
;   band would erase the strip its neighbour just drew.
;
; INPUT:
...
;
; OUTPUT:
;   bitmap_light_y advanced by at most 4 px (never more than the
;   shortest band, or the erase would overrun a neighbour).
;
; DESTROYS:
;   AF, BC, DE, HL.
; ------------------------------------------------------------
...
.down_step_ok:
    ld (bitmap_light_d), a
    xor a
    ld (bitmap_light_ymode), a        ; dim the top d rows of each band
    call bitmap_light_op_dim
    call bitmap_light_row_pass
    ld a, 1
    ld (bitmap_light_ymode), a        ; light the d rows below each band
    call bitmap_light_op_lit
    call bitmap_light_row_pass
    ld a, (bitmap_light_d)
    ld c, a
...
.up_step_ok:
    ld (bitmap_light_d), a
    ld a, 2
    ld (bitmap_light_ymode), a        ; dim the bottom d rows of each band
    call bitmap_light_op_dim
    call bitmap_light_row_pass
    ld a, 3
    ld (bitmap_light_ymode), a        ; light the d rows above each band
    call bitmap_light_op_lit
    call bitmap_light_row_pass
    ld a, (bitmap_light_y)
    ld c, a
...
; ------------------------------------------------------------
; FUNCTION: bitmap_light_row_pass
; ------------------------------------------------------------
; PURPOSE:
;   One horizontal-strip pass over every band: full band width, d rows tall, at
;   the edge selected by bitmap_light_ymode
;   (0 = band top, 1 = below band, 2 = band bottom, 3 = above band).
;
; DESTROYS:
;   AF, BC, DE, HL.
; ------------------------------------------------------------
bitmap_light_row_pass:
    ld hl, bitmap_light_bands
    ld b, 5
.row_loop:
    push bc
    ...
    dec a
    jp z, .row_at_bottom
    ; mode 3: the d rows immediately above the band
    ld a, (bitmap_light_band_y)
    ld c, a
    ld a, (bitmap_light_d)
    ld b, a
