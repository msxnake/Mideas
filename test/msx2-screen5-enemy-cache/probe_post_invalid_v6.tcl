# Enemy colour cache probe v6 (CONTRATO B) — build post.
...
set log_path "C:/Users/salam/Documents/Programacion/Mideas/test/msx2-screen5-enemy-cache/probe_post_invalid_v6.txt"
...
proc done_slot {s} {
    ...
    if {[catch {
        ...
        if {!$injected} { debug write memory [expr {53476 + $s}] 0 ; set injected 1 ; logline "INJECT valid=0 slot=$s" }
...
proc tick {} {
    ...
    if {[catch {
        if {$n >= $DURATION && !$done} {
            ...
            logline [expr {($boot_hits == 1 && $stale == 0 && $verified > 0 && $keyErrors == 0 && $vramFails == 0 && $tickerr == 0 && $upTotal > 0 && [llength $anomalies] == 0) ? "VERDICT: PASS" : "VERDICT: FAIL"}]
...
logline "probe v6 start (build=post, INIT bp=0x4010, slots=4, colorBase=#F420, oracle=pool-derived expected colour block)"
after frame tick
