...
; ------------------------------------------------------------
; PURPOSE:
;   Wall-climb gravity hook. When UP is held together with a horizontal key that
;   points into an adjacent solid wall, override player_vy with an upward climb
;   velocity (-2 px/frame) so the vertical movement loop carries the
;   player up the wall (and stops at solids). No-op otherwise.
;
; INPUT:
;   player_x, player_y; PPI keyboard row 8 (UP bit 5, RIGHT bit 7, LEFT bit 4).
;
; OUTPUT:
;   player_vy = -2 and player_flags grounded bit cleared while climbing.
;
; DESTROYS:
;   AF, BC, DE, HL.
;
...
;   frame). Writes player_vy and player_flags only while climbing.
;
; NOTES:
...
;   single probe one pixel past the player edge at mid-height (player_y + 8).
; ------------------------------------------------------------
...
.wc_climb:
    ld a, (player_flags)
    and #FE                 ; leaving the ground while climbing
    ld (player_flags), a
    ld a, #FE    ; -2 px/frame (upward)
    ld (player_vy), a
    ret
