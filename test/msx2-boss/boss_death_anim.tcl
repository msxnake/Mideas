...
set log_path "C:/Users/salam/Documents/Programacion/Mideas/test/msx2-boss/boss_death_anim.txt"
...
after time 12.0 { sample "alive" ; screenshot -prefix death_anim_alive_ }

# Start the death presentation exactly as bitmap_boss_kill does.
after time 12.5 {
    debug write memory 0xD0F1 10      ;# boss_death_left = authored blast count
    debug write memory 0xD0F2 1       ;# boss_death_tick = first blast next frame
    debug write memory 0xD0F3 0xA5    ;# boss_death_seed (must not be zero)
    debug write memory 0xD0F4 0
    debug write memory 0xD0F9 0
    debug write memory 0xD0FE 0
...
after time 12.8 { screenshot -prefix death_anim_blast_ }
after time 13.6 { screenshot -prefix death_anim_mid_ }
after time 17.5 {
    sample "after"
    screenshot -prefix death_anim_done_
    after time 1 { exit }
}
