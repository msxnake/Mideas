...
exports.DEFAULT_MSX2_ENTITY_CREATE_PRESETS = exports.MSX2_ENTITY_REPERTOIRE = exports.MSX2_ENTITY_MOVEMENT_OPTIONS = exports.MSX2_ENTITY_KIND_OPTIONS = exports.MSX2_COMPONENT_REPERTOIRE = exports.MSX2_COMPONENT_FIELD_EDITORS = void 0;
exports.mapEnemyBehaviorToMovementMode = mapEnemyBehaviorToMovementMode;
exports.buildMsx2EnemyEntityFromAsset = buildMsx2EnemyEntityFromAsset;
exports.buildEntityTemplateFromMsx2Entity = buildEntityTemplateFromMsx2Entity;
exports.buildMsx2PresetFromEntityTemplate = buildMsx2PresetFromEntityTemplate;
exports.buildMsx2EntityComponents = buildMsx2EntityComponents;
exports.MSX2_COMPONENT_FIELD_EDITORS = {
    msx2_transform: {
...
exports.MSX2_COMPONENT_REPERTOIRE = [
    {
        id: 'msx2_transform',
        label: 'Position',
...
exports.MSX2_ENTITY_KIND_OPTIONS = [
...
exports.MSX2_ENTITY_MOVEMENT_OPTIONS = [
...
exports.MSX2_ENTITY_REPERTOIRE = [
    {
        id: 'player',
...
exports.DEFAULT_MSX2_ENTITY_CREATE_PRESETS = exports.MSX2_ENTITY_REPERTOIRE;
/**
 ...
 */
function mapEnemyBehaviorToMovementMode(behavior) {
    switch (behavior) {
...
function selectEnemyRenderRole(def) {
    ...
    const scored = roles.map((role, index) => {
        let score = 0;
        if (role.behavior === behavior)
            score += 4;
        else if (!role.behavior || role.behavior === 'Any')
            score += 1;
        if (role.attack === attack)
            score += 3;
        else if (!role.attack || role.attack === 'Any')
            score += 1;
        if (role.state && String(role.state).toLowerCase().includes(String(behavior).toLowerCase()))
            score += 1;
        if (role.spriteId)
            score += 1;
...
function selectEnemyBehaviorStateSwitch(def) {
    ...
    if (!near.implemented || !far.implemented || near.movementName === far.movementName)
        return undefined;
 ...
 */
function buildMsx2EnemyEntityFromAsset(asset, x, y) {
    const def = (asset.data || {});
...
const MSX2_VALID_ENTITY_KINDS = new Set(exports.MSX2_ENTITY_KIND_OPTIONS.map(option => option.value));
/**
 ...
 */
function buildEntityTemplateFromMsx2Entity(entity, templateId, name) {
    return {
        id: templateId,
        name,
        target: 'MSX2',
 ...
 */
function buildMsx2PresetFromEntityTemplate(template) {
    if (!template || template.target !== 'MSX2' || !Array.isArray(template.components))
        return null;
    const components = {};
    for (const component of template.components) {
        if (!component?.definitionId)
            continue;
        components[component.definitionId] = { ...(component.defaultValues || {}) };
    }
...
const defaultsByComponent = new Map(exports.MSX2_COMPONENT_REPERTOIRE.map(component => [component.id, component.defaults]));
function buildMsx2EntityComponents(preset, tileX, tileY) {
    const components = {};
    Object.entries(preset.components).forEach(([componentId, values]) => {
        const defaults = defaultsByComponent.get(componentId) || {};
