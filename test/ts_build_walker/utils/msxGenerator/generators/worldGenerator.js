...
exports.buildWorldMusicPolicyManifest = buildWorldMusicPolicyManifest;
...
function getExportableTrackIndex(trackAssetId, analysis) {
...
function buildWorldMusicPolicyMap(analysis) {
    ...
    const initialPolicy = { mode: 'stop' };
    const queue = [{ nodeId: startNodeId, policy: initialPolicy }];
    ...
    const samePolicy = (left, right) => {
        if (left.mode !== right.mode)
            return false;
        if (left.mode !== 'play' || right.mode !== 'play')
            return true;
...
function buildWorldMusicPolicyManifest(analysis) {
    ...
    worldMaps.forEach((world, index) => {
        ...
        const policy = policies.get(worldId) || { mode: 'preserve' };
...
function getConnectionTransitionMode(conn, direction) {
 ...
 */
function emitDirectionalTransitionCode(worldLabel, screenIndex, direction, targetScreenIndex, targetGlobalScreenId, targetLoadRoutine, currentBounds, targetBounds, connection, useFarCall = false) {
...
`;
    ...
    if (fixedEntry && (transitionMode === 'fixed_entry' ||
 ...
 */
function generateWorldsFile(analysis, romMode = 'simple32k') {
