...
exports.generateResourceManagerFile = generateResourceManagerFile;
function buildMapperPushSet(page) {
    return `    push af
...
function buildMapperPop(page) {
    return `    call mapper_pop_${page}\n`;
}
function buildIrqRestoreGuard(doneLabel) {
...
function generateResourceManagerFile(mapperWindow, options = {}) {
...
${keepRuntimeBackgroundLayout ? `    ld (resource_ram_cache_screen_layout_id), a\n` : ''}
...
.resource_copy_ram_has_size:
    ...
    di
${mapperPushSet}    ldir
${mapperPop}${buildIrqRestoreGuard('.resource_copy_ram_irq_done')}
...
resource_decompress_from_bank_to_ram:
    di
${mapperPushSet}    call dzx0_standard
${mapperPop}${buildIrqRestoreGuard('.resource_decompress_ram_irq_done')}
...
${mapperPop}${buildIrqRestoreGuard('.resource_copy_vram_irq_done')}
...
.resource_decompress_vram_direct:
    pop af
    di
${mapperPushSet}    call resource_dzx0_to_vram
${mapperPop}${buildIrqRestoreGuard('.resource_decompress_vram_irq_done')}
...
;   carry set if resource id is missing
; Notes:
${keepRuntimeBackgroundLayout ? `;   Keeps the immutable layout in runtime_background_layout and rebuilds
;   runtime_screen_layout from that clean RAM copy on every screen load.` : `;   Loads the current screen layout directly into runtime_screen_layout.
;   No immutable background copy is kept when secret zones are not generated.`}
; Clobbers:
;   AF, BC, DE, HL
; ------------------------------------------------------------------
resource_load_screen_layout_cached:
${keepRuntimeBackgroundLayout ? `    ld c, a
...
.resource_layout_cache_hit:
    ...
    ldir
    xor a
    ret` : `    ld de, runtime_screen_layout
    call resource_load_to_ram_by_id
    ret`}
