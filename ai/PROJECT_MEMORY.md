...
- NUEVO editor "Tile Map" para bitmap SCREEN 5 (commits `95e49418`, `524fcd35`): `components/editors/Msx2BitmapScreenEditor.tsx`, en PARALELO al clasico `Msx2Screen4BitmapRoomEditor` (intacto), accesible por un toggle beta en `AppUI.tsx` para assets `msx2bitmaproom`.
  - Modelo de datos: nuevo campo opcional `tileGrid?: number[][]` en `Msx2Screen4BitmapRoom` (matriz 16x(alto/16), valor = indice de atlas entry +1, 0=vacio). Es la FUENTE DE VERDAD de los tiles (un tile por celda, ultimo gana); los comandos `copy` del render se DERIVAN de ella (preservando fills/lineas de color). Tilemap compacto, optimo para export MSX2 (pendiente: que el generador lo consuma directo).
...
- Editor unico: el clasico `Msx2Screen4BitmapRoomEditor` se borro; `msx2bitmaproom` abre siempre el Tile Map (`Msx2BitmapScreenEditor`).
...
- Para reservar el byte fijo, el skill chain se desplazo +1: `MSX2_BITMAP_DASH_RAM_BASE` `#C0D9 -> #C0DA` (en `msx2BitmapDashGenerator.ts`), y todo el chain (air_dash, glide, wall_jump, power_stomp, shake, shoot, teleport, slash, grab, high_jump, wall_break, spin_attack) corre +1. Actualizado `scripts/build_msx2_screen4_bitmap_room_smoke.py` (EQUs + probes + pokes del rango #C0D9-#C0E5 desplazados +1).
