...
- G3.1 (timer air, HECHO y verificado): contador ligado a 'air' ahora lee `air_timer` (byte compartido, como playerEnergy→player_health). Nuevo sistema `buildBitmapAirTimerAsm`: EQUs air_timer+air_frame_divider, init (seed room.runtime.initialAir), rutina `update_air_timer` (frame divider 60 → decremento ~1/seg a 60Hz, para a 0). `resolveHudElementBindingRamLabel` mapea air→air_timer; `linkedCounterSpec` trata air como byteBinding (narrow); `needsOwnValue` excluye air. El byte se asigna+siempra siempre que hay un counter air (simbolo valido); la rutina de decremento solo se emite si initialAir>0 y !disableAirTimer.
- RAM: +2 bytes (air_timer+air_frame_divider) solo si hay counter air. En el smoke (bar+narrow+wide+air): air_timer #C0F7, air_frame_divider #C0F8. Cursor #C0F9 < #C1F0. CPU: ~6 ciclos/frame (inc divider, cp, ret nz / dec).
- Verificacion OpenMSX: timer arranca en 180 (initialAir), decrementa ~1/seg (Run A ~7.5s → 0xB2=178, Run B ~12s → 0xAD=173; el arranque "lento" es el init ROM antes del main loop, no bug). El dirty del counter air sigue a air_timer (B2/B2, AD/AD). Sin crash.
- Pendiente: G3.2 coleccionables+score (sistema nuevo, mas grande) y "time up" event (muerte/fin al llegar air_timer a 0) — este es logica de juego futura, no HUD. El score 'custom' sigue sin fuente (solo se muestra el initialValue sembrado).
