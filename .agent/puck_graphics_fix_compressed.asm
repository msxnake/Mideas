...
.draw_collectible_hud_2:
    ...
    ld a, (msx2_collectible_count)
    cp 3
    jp nc, .collectible_hud_3_on
    ld a, #11
    jp .draw_collectible_hud_3
.collectible_hud_3_on:
    ld a, #AA
.draw_collectible_hud_3:
    ld hl, #0125
    call draw_msx2_life_pip
    ld a, (msx2_collectible_count)
    cp 4
    jp nc, .collectible_hud_4_on
    ld a, #11
    jp .draw_collectible_hud_4
.collectible_hud_4_on:
    ld a, #AA
.draw_collectible_hud_4:
    ld hl, #012B
    call draw_msx2_life_pip
    ret
...
msx2_hw_sprite_pattern_0:
    DB #00,#0F,#1F,#3F,#7F,#7F,#7F,#7F,#00,#F0,#F8,#FC,#F8,#E0,#C0,#00
    DB #7F,#7F,#7F,#7F,#3F,#1F,#0F,#00,#00,#C0,#E0,#F8,#FC,#F8,#F0,#00
