...
.cursor_loop:
    ...
    ; Only process entities with INPUT component to avoid moving all active entities.
    ld hl, entity_comp_masks
    ld e, c
    ld d, 0
    add hl, de
    ...
    ; Use the interrupt-polled input snapshot to avoid BIOS calls inside
    ; the per-entity loop (safer and deterministic).
    ld a, (input_state)           ; A = stick direction (0-8)
    or a
...
update_all_entities:
    ...
    ; Keep cursors update enabled for projects that drive movement
    ; directly from cursor-style input.
    ...
    ret
; Total systems called: 13 (optimized from 15)
...
SPRITE_0_PATTERN EQU HERO_RUNNING_0_F0_LAYER1
...
SPRITE_0_PATTERN_MIRRORED EQU HERO_RUNNING_MIRRORED_0_F0_LAYER1
...
SPRITE_2_PATTERN EQU HERO_IDLE_2_F0_LAYER1
...
SPRITE_2_PATTERN_MIRRORED EQU HERO_IDLE_MIRRORED_2_F0_LAYER1
...
SPRITE_0_FRAME_PTRS:
    dw HERO_RUNNING_0_F0_LAYER1
    dw HERO_RUNNING_0_F1_LAYER1
    dw HERO_RUNNING_0_F2_LAYER1
...
entity_sprite_config: 
    db 0, 2 ; Entity 0 (hero_idle)
    db 2, 2 ; Entity 1 (hero_running)
    db 4, 2 ; Entity 2 (hero_running)
    db 6, 2 ; Entity 3 (hero_running)
    db 8, 2 ; Entity 4 (hero_running)
    db 10, 2 ; Entity 5 (hero_running)
    db 12, 2 ; Entity 6 (hero_running)
    db 14, 2 ; Entity 7 (hero_running)
    db 16, 2 ; Entity 8 (hero_running)
    db 18, 2 ; Entity 9 (hero_running)
    db 20, 2 ; Entity 10 (hero_running)
...
load_sprite_patterns:
    ...
    ; Entity 0: hero_idle (2 layers)
    ; Base HW Sprite: 0
    ld hl, SPRITE_2_PATTERN
    ld de, SPRPAT + (0 * 32)
    ld bc, 64 ; Load 2 layers (32 bytes each)
    call FAST_LDIRVM
    ; Entity 1: hero_running (2 layers)
    ; Base HW Sprite: 2
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (2 * 32)
    ld bc, 64 ; Load 2 layers (32 bytes each)
    call FAST_LDIRVM
    ; Entity 2: hero_running (2 layers)
    ; Base HW Sprite: 4
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (4 * 32)
    ld bc, 64 ; Load 2 layers (32 bytes each)
    call FAST_LDIRVM
    ; Entity 3: hero_running (2 layers)
    ; Base HW Sprite: 6
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (6 * 32)
    ld bc, 64 ; Load 2 layers (32 bytes each)
    call FAST_LDIRVM
    ; Entity 4: hero_running (2 layers)
    ; Base HW Sprite: 8
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (8 * 32)
    ld bc, 64 ; Load 2 layers (32 bytes each)
    call FAST_LDIRVM
    ; Entity 5: hero_running (2 layers)
    ; Base HW Sprite: 10
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (10 * 32)
    ld bc, 64 ; Load 2 layers (32 bytes each)
    call FAST_LDIRVM
    ; Entity 6: hero_running (2 layers)
    ; Base HW Sprite: 12
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (12 * 32)
    ld bc, 64 ; Load 2 layers (32 bytes each)
    call FAST_LDIRVM
    ; Entity 7: hero_running (2 layers)
    ; Base HW Sprite: 14
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (14 * 32)
    ld bc, 64 ; Load 2 layers (32 bytes each)
    call FAST_LDIRVM
    ; Entity 8: hero_running (2 layers)
    ; Base HW Sprite: 16
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (16 * 32)
    ld bc, 64 ; Load 2 layers (32 bytes each)
    call FAST_LDIRVM
    ; Entity 9: hero_running (2 layers)
    ; Base HW Sprite: 18
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (18 * 32)
    ld bc, 64 ; Load 2 layers (32 bytes each)
    call FAST_LDIRVM
    ; Entity 10: hero_running (2 layers)
    ; Base HW Sprite: 20
    ld hl, SPRITE_0_PATTERN
    ld de, SPRPAT + (20 * 32)
    ld bc, 64 ; Load 2 layers (32 bytes each)
...
load_screen_pantalla1_764187751187:
    ...
    ; HUD occupies rows 0-2 (3 rows = 96 bytes)
    ; Game area: rows 3-23 (21 rows = 672 bytes)
...
; Pad ROM to 8KB boundary for cartridge/emulator compatibility.
ROM_SIZE_BYTES   EQU ($ - #4000)
ROM_PAD_BYTES    EQU (((ROM_SIZE_BYTES + #1FFF) / #2000) * #2000) - ROM_SIZE_BYTES
    ds ROM_PAD_BYTES, #FF
