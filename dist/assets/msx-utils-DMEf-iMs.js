const Mo=[16,24,32];var J=(t=>(t.Score="Score",t.HighScore="HighScore",t.Lives="Lives",t.EnergyBar="EnergyBar",t.ItemDisplay="ItemDisplay",t.SceneName="SceneName",t.MiniMap="MiniMap",t.CoinCounter="CoinCounter",t.BossEnergyBar="BossEnergyBar",t.PhaseIndicator="PhaseIndicator",t.AttackAlert="AttackAlert",t.TextBox="TextBox",t.NumericField="NumericField",t.CustomCounter="CustomCounter",t))(J||{});const Xt={water:{bit:0,label:"Water Effect",maskValue:1,color:"rgba(50, 100, 200, 0.4)"},customGravity:{bit:1,label:"Custom Gravity",maskValue:2,color:"rgba(150, 50, 200, 0.4)"},icePhysics:{bit:2,label:"Ice Physics",maskValue:4,color:"rgba(100, 200, 255, 0.4)"},spriteConceal:{bit:3,label:"Sprite Concealment",maskValue:8,color:"rgba(100, 100, 100, 0.4)"}};var Kt=(t=>(t.None="None",t.Tile="Tile",t.Sprite="Sprite",t.Screen="Screen",t.Code="Code",t.Attributes="Attributes",t.Sound="Sound",t.Platformer="Platformer",t.WorldMap="WorldMap",t.Track="Track",t.HUD="HUD",t.TileBanks="TileBanks",t.Font="Font",t.HelpDocs="HelpDocs",t.BehaviorEditor="BehaviorEditor",t.ComponentDefinitionEditor="ComponentDefinitionEditor",t.EntityTemplateEditor="EntityTemplateEditor",t.Boss="Boss",t.WorldView="WorldView",t.GameFlow="GameFlow",t.MainMenu="MainMenu",t.StateMachine="StateMachine",t.GlobalVariables="GlobalVariables",t.Palette="Palette",t))(Kt||{});const Po=[1,3,5,7],Oo=[{id:0,name:"NoSolid (Passable)",isSolid:!1},{id:1,name:"Solid (Wall/Ground)",isSolid:!0},{id:2,name:"Platform (Top-Solid)",isSolid:!0},{id:3,name:"Slope (Solid)",isSolid:!0}],ko={isBreakable:{bit:0,label:"Breakable"},isMovable:{bit:1,label:"Movable"},causesDamage:{bit:2,label:"Deadly"},isInteractiveSwitch:{bit:3,label:"Interactable"}},Uo="0.267",Se=[{name:"Transparent",hex:"rgba(0,0,0,0)"},{name:"Black",hex:"#000000"},{name:"Medium Green",hex:"#3EB847"},{name:"Light Green",hex:"#74D07D"},{name:"Dark Blue",hex:"#2F2FC1"},{name:"Light Blue",hex:"#5858FC"},{name:"Dark Red",hex:"#B63125"},{name:"Cyan",hex:"#68D2DA"},{name:"Medium Red",hex:"#FC584A"},{name:"Light Red",hex:"#FF8E81"},{name:"Dark Yellow",hex:"#C0BF3B"},{name:"Light Yellow",hex:"#E7E474"},{name:"Dark Green",hex:"#309337"},{name:"Magenta",hex:"#B640C8"},{name:"Gray",hex:"#999999"},{name:"White",hex:"#FFFFFF"}],X=[{name:"Transparent (Backdrop)",hex:"rgba(0,0,0,0)",index:0},{name:"Black",hex:"#000000",index:1},{name:"Medium Green",hex:"#21C842",index:2},{name:"Light Green",hex:"#5EDC78",index:3},{name:"Dark Blue",hex:"#5455ED",index:4},{name:"Light Blue",hex:"#7D76FC",index:5},{name:"Dark Red",hex:"#D4524D",index:6},{name:"Cyan",hex:"#42EBF5",index:7},{name:"Medium Red",hex:"#FC5554",index:8},{name:"Light Red",hex:"#FF7978",index:9},{name:"Dark Yellow",hex:"#D4C154",index:10},{name:"Light Yellow",hex:"#E6CE80",index:11},{name:"Dark Green",hex:"#21B03B",index:12},{name:"Magenta",hex:"#C95BBA",index:13},{name:"Gray",hex:"#CCCCCC",index:14},{name:"White",hex:"#FFFFFF",index:15}],de=[0,36,73,109,146,182,219,255],ye=t=>t.toString(16).padStart(2,"0").toUpperCase(),Fo=(()=>{const t=[];for(let e=0;e<de.length;e++)for(let a=0;a<de.length;a++)for(let l=0;l<de.length;l++){const n=e<<6|a<<3|l;t.push({index:n,hex:`#${ye(de[e])}${ye(de[a])}${ye(de[l])}`,rLevel:e,gLevel:a,bLevel:l})}return t})(),Be=t=>{let e=0,a=1/0;return de.forEach((l,n)=>{const o=Math.abs(l-t);o<a&&(a=o,e=n)}),e},Zt=t=>!t||!t.startsWith("#")||t.length!==7?"#000000":t.toUpperCase(),Jt=t=>{const e=Zt(t),a=parseInt(e.slice(1,3),16),l=parseInt(e.slice(3,5),16),n=parseInt(e.slice(5,7),16),o=Be(a),s=Be(l),r=Be(n),d=`#${ye(de[o])}${ye(de[s])}${ye(de[r])}`,i=o<<6|s<<3|r;return{hex:d,masterIndex:i}},Bo=Se.map((t,e)=>{if(e===0)return{slotIndex:0,masterIndex:-1,hex:"rgba(0,0,0,0)"};const a=Jt(t.hex);return{slotIndex:e,masterIndex:a.masterIndex,hex:a.hex}}),$o=[8,16,24,32],jo=16,Ho=16,Vo=16,ue=32,we=24,be=8,me=255,zo="SCREEN 2 (Graphics I)",Go=["ADC","ADD","AND","BIT","CALL","CCF","CP","CPD","CPDR","CPI","CPIR","CPL","DAA","DEC","DI","DJNZ","EI","EX","EXX","HALT","IM","IN","INC","IND","INDR","INI","INIR","JP","JR","LD","LDD","LDDR","LDI","LDIR","NEG","NOP","OR","OTDR","OTIR","OUT","OUTD","OUTI","POP","PUSH","RES","RET","RETI","RETN","RL","RLA","RLC","RLCA","RLD","RR","RRA","RRC","RRCA","RRD","RST","SBC","SCF","SET","SLA","SLL","SRA","SRL","SUB","XOR"],Wo=["A","F","B","C","D","E","H","L","AF","BC","DE","HL","IXH","IXL","IYH","IYL","IX","IY","SP","PC","I","R","AF'"],Yo=["NZ","Z","NC","C","PO","PE","P","M"],Qo=[".ORG","ORG","END",".END",".EQU","EQU",".DB","DB",".BYTE","BYTE","DEFB",".DW","DW",".WORD","WORD","DEFW",".DS","DS",".BLOCK","BLOCK","DEFS",".DEFINE","DEFINE",".MACRO","MACRO",".ENDM","ENDM",".IF","IF",".ENDIF","ENDIF",".ELSE","ELSE",".INCLUDE","INCLUDE",".DEFM","DEFM",".ZILOG",".PHASE",".REPT",".ENDR",".SEARCH",".RANDOM",".ROM",".MEGAROM",".BASIC",".CAS",".WAV",".MSXDOS"],Xo=[{id:"pac_man_collection",name:"Pac-Man Tile Collection",code:`; Pac-Man Style Tile Collection System for MSX
...
`;const o=t.width/Y,s=t.height/Y,r=m=>a==="hex"?`$${ea(m)}`:m.toString(10),d=[],i=[];for(let m=0;m<s;m++)for(let f=0;f<o;f++){const b=`;; Character Block (${f}, ${m}) for ${l}`,p=[];for(let v=0;v<Y;v++){const E=m*Y+v;let c=0;if(t.lineAttributes[E]&&t.lineAttributes[E][f]){const u=t.lineAttributes[E][f].fg;for(let y=0;y<Y;y++){const A=f*Y+y;t.data[E]&&t.data[E][A]!==void 0&&t.data[E][A]===u&&(c|=1<<7-y)}}p.push(c)}const g=p.map(r).join(",");d.push({comment:`${b} - PATTERN Data (8 bytes):`,dataString:`DB ${g}`});const S=[];for(let v=0;v<Y;v++){const E=m*Y+v;let c=Ae<<4|Te;if(t.lineAttributes[E]&&t.lineAttributes[E][f]){const u=t.lineAttributes[E][f],y=((h=Ne.get(u.fg))==null?void 0:h.index)??Ae,A=((_=Ne.get(u.bg))==null?void 0:_.index)??Te;c=y<<4|A}S.push(c)}const T=S.map(r).join(",");i.push({comment:`${b} - COLOR Attribute Data (8 bytes - FG|BG):`,dataString:`DB ${T}`})}return n+=`;; --- PATTERN DATA ---
`,d.length>0?(n+=`${l}_PATTERN_DATA:
`,d.forEach(m=>{n+=`${m.comment}
`,n+=`    ${m.dataString}
...
`,n},gn=(t,e,a,l)=>{const n=Math.max(1,t/Pe);return Array(e).fill(null).map(()=>Array(n).fill(null).map(()=>({fg:a,bg:l})))},Ot=(t,e)=>{var s,r,d,i;const a=[],l=t.width/Y,n=t.height/Y,o=e==="SCREEN 2 (Graphics I)";for(let h=0;h<n;h++)for(let _=0;_<l;_++)for(let m=0;m<Y;m++){const f=h*Y+m;let b=0,p;o&&t.lineAttributes&&t.lineAttributes[f]&&t.lineAttributes[f][_]&&(p=t.lineAttributes[f][_].fg);for(let g=0;g<Y;g++){const S=_*Y+g,T=(s=t.data[f])==null?void 0:s[S];if(T!==void 0){let v=!1;o&&p?v=T===p:o||(v=T!==Se[0].hex&&T!==((i=(d=(r=t.lineAttributes)==null?void 0:r[0])==null?void 0:d[0])==null?void 0:i.bg)),v&&(b|=1<<7-g)}}a.push(b)}return new Uint8Array(a)},Ee=(t,e)=>{var o,s;const a=t.length;if(a===0)return[];const l=((o=t[0])==null?void 0:o.length)||0;if(l===0)return[[]];const n=t.map(r=>[...r]);for(let r=0;r<a;r++)for(let d=0;d<l;d++){const i=Math.floor(d/Pe),h=(s=e[r])==null?void 0:s[i],_=n[r][d];h&&_!==h.fg&&_!==h.bg&&(n[r][d]=h.fg)}return n},Sn=(t,e,a)=>{if(t.length<2)return t;const n=t.slice(1);return n.push([...t[0]]),a==="SCREEN 2 (Graphics I)"&&e?Ee(n,e):n},An=(t,e,a)=>{const l=t.length;if(l<2)return t;const n=t.slice(0,l-1);return n.unshift([...t[l-1]]),a==="SCREEN 2 (Graphics I)"&&e?Ee(n,e):n},Tn=(t,e,a)=>{if(t.length===0)return[];const l=t.map(n=>{if(n.length<2)return[...n];const o=n.slice(1);return o.push(n[0]),o});return a==="SCREEN 2 (Graphics I)"&&e?Ee(l,e):l},In=(t,e,a)=>{if(t.length===0)return[];const l=t.map(n=>{const o=n.length;if(o<2)return[...n];const s=n.slice(0,o-1);return s.unshift(n[o-1]),s});return a==="SCREEN 2 (Graphics I)"&&e?Ee(l,e):l},Cn=(t,e,a)=>{if(t.length===0)return[];const l=t.map(n=>[...n].reverse());return a==="SCREEN 2 (Graphics I)"&&e?Ee(l,e):l},vn=(t,e,a)=>{if(t.length===0)return[];const l=[...t].reverse();return a==="SCREEN 2 (Graphics I)"&&e?Ee(l,e):l},kt=t=>{var n,o,s;if(!t.lineAttributes)return null;const e=[],a=t.width/Y,l=t.height/Y;for(let r=0;r<l;r++)for(let d=0;d<a;d++)for(let i=0;i<Y;i++){const h=r*Y+i;let _=Ae<<4|Te;const m=(n=t.lineAttributes[h])==null?void 0:n[d];if(m){const f=((o=Ne.get(m.fg))==null?void 0:o.index)??Ae,b=((s=Ne.get(m.bg))==null?void 0:s.index)??Te;_=f<<4|b}e.push(_)}return new Uint8Array(e)},Dn=t=>{const e=[];t.frames.forEach(l=>{var n,o,s,r,d;for(let i=0;i<t.spritePalette.length;i++){const h=t.spritePalette[i];if(h===t.backgroundColor)continue;let _=!1;const m=[],f=t.size.width,b=t.size.height;if(f===16&&b===16){for(let p=0;p<8;p++){let g=0;for(let S=0;S<8;S++)((n=l.data[p])==null?void 0:n[S])===h&&(g|=1<<7-S,_=!0);m.push(g)}for(let p=8;p<16;p++){let g=0;for(let S=0;S<8;S++)((o=l.data[p])==null?void 0:o[S])===h&&(g|=1<<7-S,_=!0);m.push(g)}for(let p=0;p<8;p++){let g=0;for(let S=0;S<8;S++)((s=l.data[p])==null?void 0:s[8+S])===h&&(g|=1<<7-S,_=!0);m.push(g)}for(let p=8;p<16;p++){let g=0;for(let S=0;S<8;S++)((r=l.data[p])==null?void 0:r[8+S])===h&&(g|=1<<7-S,_=!0);m.push(g)}}else for(let p=0;p<b;p++)for(let g=0;g<Math.ceil(f/8);g++){let S=0;for(let T=0;T<8;T++){const v=g*8+T;v<f&&((d=l.data[p])==null?void 0:d[v])===h&&(S|=1<<7-T,_=!0)}m.push(S)}_&&e.push(m)}});const a=e.flat();return new Uint8Array(a)},ut=t=>t.map(e=>[...e].reverse()),mt=t=>[...t].reverse(),ta=/_(left|right|up|down)$/i,Ut=t=>{if(!t)return;const e=t.trim().toLowerCase();if(e==="left"||e==="right"||e==="up"||e==="down")return e},aa=t=>{const e=t.match(ta);return e?{baseName:t.slice(0,-e[0].length),suffixDirection:Ut(e[1])}:{baseName:t}},la=(t,e,a,l)=>({...t,id:`${t.id}__auto_${a}`,name:e,facingDirection:a,frames:t.frames.map((n,o)=>({...n,id:`${n.id||`f${o}`}_${a}_auto`,data:l(n.data)}))}),je=(t,e,a,l)=>{if(!e)return;(e===e.toLowerCase()?[e]:[e,e.toLowerCase()]).forEach(o=>{const s=t[o];if(s===void 0){t[o]=a;return}s!==a&&l.push(`Name alias collision for "${o}" between indexes ${s} and ${a}. Keeping first mapping.`)})},Qe=t=>{const e=[],a=new Set,l=[],n=new Map,o=(_,m,f)=>{if(!a.has(_))return _;if(!a.has(m))return e.push(`Name "${_}" already exists. Using fallback "${m}" for ${f}.`),m;let b=1,p=`${_}_${b}`;for(;a.has(p);)b+=1,p=`${_}_${b}`;return e.push(`Name "${_}" already exists. Using "${p}" for ${f}.`),p};t.forEach((_,m)=>{const f=_.name||`sprite_${m}`,{baseName:b,suffixDirection:p}=aa(f),g=Ut(_.facingDirection);g&&p&&g!==p&&e.push(`Sprite "${f}" has suffix "${p}" but facing "${g}". Using facing direction.`);const S=g||p,T=p?b:f,v=S?`${T}_${S}`:f,E=o(v,f,`sprite "${f}"`),c=new Set;f!==E&&c.add(f);const y={sprite:{..._,name:E,facingDirection:S||_.facingDirection},baseName:T,direction:S,aliases:c};if(l.push(y),a.add(E),S){const A=n.get(T)||{};A[S]===void 0?(A[S]=l.length-1,n.set(T,A)):e.push(`Duplicate directional sprite for "${T}_${S}". Keeping first occurrence.`)}}),n.forEach((_,m)=>{const f=(b,p,g,S)=>{if(p===void 0||_[b]!==void 0)return;const T=`${m}_${b}`;if(a.has(T)){e.push(`Cannot auto-generate "${T}" because the name already exists.`);return}const v=l[p],c={sprite:la(v.sprite,T,b,g),baseName:m,direction:b,aliases:new Set};l.push(c),_[b]=l.length-1,a.add(T),e.push(`Auto-generated "${T}" from "${v.sprite.name}" using ${S}.`)};_.right!==void 0&&_.left===void 0?f("left",_.right,ut,"horizontal mirror"):_.left!==void 0&&_.right===void 0&&f("right",_.left,ut,"horizontal mirror"),_.up!==void 0&&_.down===void 0?f("down",_.up,mt,"vertical mirror"):_.down!==void 0&&_.up===void 0&&f("up",_.down,mt,"vertical mirror")});const s={};l.forEach((_,m)=>{je(s,_.sprite.name,m,e),je(s,_.sprite.id,m,e)}),l.forEach((_,m)=>{_.aliases.forEach(f=>je(s,f,m,e))});const r=l.map((_,m)=>m),d=l.map((_,m)=>m),i=l.map((_,m)=>m),h=l.map((_,m)=>m);return l.forEach((_,m)=>{const f=n.get(_.baseName);f&&(f.left!==void 0&&(r[m]=f.left),f.right!==void 0&&(d[m]=f.right),f.up!==void 0&&(i[m]=f.up),f.down!==void 0&&(h[m]=f.down))}),{sprites:l.map(_=>_.sprite),nameToIndex:s,directionalLookupTables:{left:r,right:d,up:i,down:h},warnings:e}},oa=t=>{let e=t.toString(16).toUpperCase();return e.length===1&&(e="0"+e),e},na=(t,e,a,l,n,o,s="hex",r)=>{var f,b,p,g,S;const i=t.replace(/[^a-zA-Z0-9_]/g,"_").toUpperCase();let h=`;; ---- Sprite Frame: ${t} ----
`;h+=`;; Size: ${n}x${o}
`;let _=0;const m=Array.isArray(r)&&r.length>0?r:a.map((T,v)=>v).filter(T=>{const v=a[T];return!!v&&v!==l});for(const T of m){const v=a[T];if(!v||v===l)continue;const E=[];if(n===16&&o===16){for(let c=0;c<8;c++){let u=0;for(let y=0;y<8;y++){const A=y;((f=e[c])==null?void 0:f[A])===v&&(u|=1<<7-y)}E.push(u)}for(let c=8;c<16;c++){let u=0;for(let y=0;y<8;y++){const A=y;((b=e[c])==null?void 0:b[A])===v&&(u|=1<<7-y)}E.push(u)}for(let c=0;c<8;c++){let u=0;for(let y=0;y<8;y++){const A=8+y;((p=e[c])==null?void 0:p[A])===v&&(u|=1<<7-y)}E.push(u)}for(let c=8;c<16;c++){let u=0;for(let y=0;y<8;y++){const A=8+y;((g=e[c])==null?void 0:g[A])===v&&(u|=1<<7-y)}E.push(u)}}else for(let c=0;c<o;c++)for(let u=0;u<Math.ceil(n/8);u++){let y=0;for(let A=0;A<8;A++){const I=u*8+A;I<n&&((S=e[c])==null?void 0:S[I])===v&&(y|=1<<7-A)}E.push(y)}_+=1,h+=`${i}_LAYER${T}: ; Brush Color Index ${T} (Actual Color: ${v})
`,n%8!==0&&(h+=`;; WARNING: Sprite width ${n} is not a multiple of 8. Bitmask generation might be problematic for standard VDP.
`);for(let c=0;c<E.length;c+=16){const y=E.slice(c,c+16).map(A=>s==="hex"?`#${oa(A)}`:A.toString());h+=`    DB ${y.join(",")}
`}h+=`
`}return _===0&&(h+=`;; NO DRAWABLE LAYERS EXPORTED for ${t} - Palette may match background color.
`),h+=`;; ---- End of Frame: ${t} ----

`,h},ra=(t,e="hex",a)=>{let l=`;; Sprite: ${t.name}
...
`;const n=a!==void 0?`_${a}`:"",o=t.name+n,s=o.replace(/[^a-zA-Z0-9_]/g,"_").toUpperCase();l+=`SPRITE_${s}_WIDTH     EQU ${t.size.width}
`,l+=`SPRITE_${s}_HEIGHT    EQU ${t.size.height}
`,l+=`SPRITE_${s}_FRAMES    EQU ${t.frames.length}

`;const r=t.spritePalette.map((d,i)=>i).filter(d=>{const i=t.spritePalette[d];return!i||i===t.backgroundColor?!1:t.frames.some(h=>{var _;return(_=h==null?void 0:h.data)==null?void 0:_.some(m=>m==null?void 0:m.some(f=>f===i))})});return t.frames.forEach((d,i)=>{l+=na(`${o}_F${i}`,d.data,t.spritePalette,t.backgroundColor,t.size.width,t.size.height,e,r)}),l},He=16,Ft="SCREEN 2 (Graphics I)",ia="SCREEN 5 (Graphics III)",he=8,sa={pixelWidth:ue*He,pixelHeight:we*He,widthTiles:ue,heightTiles:we,baseTileSize:He},ft={[Ft]:{pixelWidth:ue*be,pixelHeight:we*be,widthTiles:ue,heightTiles:we,baseTileSize:be},[ia]:{pixelWidth:256,pixelHeight:212,widthTiles:32,heightTiles:27,baseTileSize:be},"SCREEN 0 (Text 40)":{pixelWidth:240,pixelHeight:192,widthTiles:40,heightTiles:24,baseTileSize:he},"SCREEN 1 (Text 32)":{pixelWidth:256,pixelHeight:192,widthTiles:32,heightTiles:24,baseTileSize:he},"SCREEN 3 (Multicolor)":{pixelWidth:256,pixelHeight:192,widthTiles:32,heightTiles:24,baseTileSize:he},"SCREEN 4 (Graphics II)":{pixelWidth:256,pixelHeight:192,widthTiles:32,heightTiles:24,baseTileSize:he},"SCREEN 6 (Graphics IV)":{pixelWidth:512,pixelHeight:212,widthTiles:64,heightTiles:27,baseTileSize:he},"SCREEN 7 (Graphics V)":{pixelWidth:512,pixelHeight:212,widthTiles:64,heightTiles:27,baseTileSize:he},"SCREEN 8 (Graphics VI)":{pixelWidth:256,pixelHeight:212,widthTiles:32,heightTiles:27,baseTileSize:he}};function Ln(t){const e=typeof t=="string"?t.trim():"";return e&&ft[e]?ft[e]:sa}const xe=t=>t===Ft,da=t=>xe(t)?X:Se,ca=(t,e)=>{const a=da(e);if(t===void 0||t<0||t>=a.length)return xe(e)?X[1].hex:Se[4].hex;const l=a[t];return(l==null?void 0:l.hex)??(xe(e)?X[1].hex:Se[4].hex)},Rn=(t,e,a,l)=>{var m;const n=t.layers.background,o=t.activeAreaX??0,s=t.activeAreaY??0,r=t.activeAreaWidth??t.width,d=t.activeAreaHeight??t.height,i=[];let h=0;const _=new Map;for(let f=0;f<d;f++){const b=s+f;for(let p=0;p<r;p++){const g=o+p;if(b>=n.length||g>=((m=n[b])==null?void 0:m.length)){i.push(me);continue}const S=n[b][g];if(!S||!S.tileId)i.push(me);else{let T=me;const v=e.find(E=>E.id===S.tileId);if(l==="SCREEN 2 (Graphics I)"&&a&&v){let E=!1,c={tileId:S.tileId,position:{x:g,y:b},attempts:[],banksReceived:a.length};typeof globalThis.screenUtils_firstTileLogged>"u"&&(console.log("🔍 First tile structure check:",{tileId:S.tileId,position:{x:g,y:b},banksCount:a.length,banks:a.map(u=>({name:u.name,assignedTileIds:Object.keys(u.assignedTiles||{}),hasThisTile:!!(u.assignedTiles&&u.assignedTiles[S.tileId]),assignedTilesType:typeof u.assignedTiles,assignedTilesSample:u.assignedTiles?Object.entries(u.assignedTiles).slice(0,2):[]}))}),globalThis.screenUtils_firstTileLogged=!0);for(const u of a)if((u.enabled??!0)&&u.assignedTiles[S.tileId]){const y=u.assignedTiles[S.tileId].charCode,A=Math.ceil(v.width/be),I=S.subTileX||0,L=S.subTileY||0;T=y+L*A+I;const w=T>=u.charsetRangeStart&&T<=u.charsetRangeEnd;if(c.attempts.push({bankName:u.name,baseCharCode:y,calculated:T,range:`${u.charsetRangeStart}-${u.charsetRangeEnd}`,inRange:w}),w){E=!0;break}else T=me}else c.attempts.push({bankName:u.name,reason:"Tile not assigned to this bank"});E||(console.warn("⚠️ Tile not found in valid range:",c),T=me)}else if(l!=="SCREEN 2 (Graphics I)"){const E=`${S.tileId}_${S.subTileX??0}_${S.subTileY??0}`;_.has(E)?T=_.get(E):h>255?T=me:(_.set(E,h),T=h++)}i.push(T)}}}return new Uint8Array(i)},pa=(t,e,a,l,n,o="hex")=>{const r=t.replace(/[^a-zA-Z0-9_]/g,"_").toUpperCase();let d=`;; MAP: ${t} (${e}x${a} tiles)
...
`),d+=`SCREEN_${r}_WIDTH     EQU ${e}
`,d+=`SCREEN_${r}_HEIGHT    EQU ${a}
`,d+=`SCREEN_${r}_SIZE      EQU ${l.length}

`,d+=`SCREEN_${r}_LAYOUT:
`;for(let i=0;i<l.length;i+=16){const _=l.slice(i,i+16).map(m=>o==="hex"?`#${m.toString(16).padStart(2,"0").toUpperCase()}`:m.toString());d+=`    DB ${_.join(",")}
`}return d},_a=(t,e,a,l,n="hex")=>{const s=t.replace(/[^a-zA-Z0-9_]/g,"_").toUpperCase();let r=`;; BEHAVIOR MAP: ${t} (${e}x${a} tiles)
`;r+=`;; Total size: ${l.length} bytes (Map IDs 0-255)
`,r+=`;; Data format: ${n.toUpperCase()}

`,r+=`BEHAVIOR_${s}_WIDTH     EQU ${e}
...
`,r},wn=(t,e)=>{if(t.width!==e.width||t.height!==e.height||t.data.length!==e.data.length)return!1;for(let a=0;a<t.height;a++){if(t.data[a].length!==e.data[a].length)return!1;for(let l=0;l<t.width;l++)if(t.data[a][l]!==e.data[a][l])return!1}if(t.lineAttributes&&e.lineAttributes){if(t.lineAttributes.length!==e.lineAttributes.length)return!1;for(let a=0;a<t.lineAttributes.length;a++){if(t.lineAttributes[a].length!==e.lineAttributes[a].length)return!1;for(let l=0;l<t.lineAttributes[a].length;l++)if(t.lineAttributes[a][l].fg!==e.lineAttributes[a][l].fg||t.lineAttributes[a][l].bg!==e.lineAttributes[a][l].bg)return!1}}else if(t.lineAttributes!==e.lineAttributes)return!1;return JSON.stringify(t.logicalProperties)===JSON.stringify(e.logicalProperties)};function Nn(t,e,a,l,n,o,s){const{data:r,width:d,height:i,lineAttributes:h}=t;if(!r||i===0||d===0)return"";const _=document.createElement("canvas");_.width=o,_.height=o;const m=_.getContext("2d");if(!m)return"";m.imageSmoothingEnabled=!1;const f=(e??0)*o,b=(a??0)*o;for(let S=0;S<o;S++)for(let T=0;T<o;T++){const v=f+T,E=b+S;if(E>=0&&E<i&&v>=0&&v<d){let c=r[E][v];if(s==="SCREEN 2 (Graphics I)"&&h&&h[E]){const u=Math.floor(v/Pe),y=h[E][u];y&&c!==y.fg&&c!==y.bg&&(c=y.fg)}m.fillStyle=c,m.fillRect(T,S,1,1)}}if(_.width===l&&_.height===n)return _.toDataURL();const p=document.createElement("canvas");p.width=l,p.height=n;const g=p.getContext("2d");return g?(g.imageSmoothingEnabled=!1,g.drawImage(_,0,0,l,n),p.toDataURL()):_.toDataURL()}function xn(t,e,a){var o;if(!t||a===0||e===0)return"";const l=document.createElement("canvas");l.width=e,l.height=a;const n=l.getContext("2d");if(!n)return"";n.imageSmoothingEnabled=!1;for(let s=0;s<a;s++)for(let r=0;r<e;r++){const d=(o=t[s])==null?void 0:o[r];d&&d!=="rgba(0,0,0,0)"&&(n.fillStyle=d,n.fillRect(r,s,1,1))}return l.toDataURL()}const Mn=(t,e,a,l,n,o,s)=>{var _,m;const r=xe(l);t.width=e.width*n,t.height=e.height*n;const d=t.getContext("2d");if(!d)return;d.imageSmoothingEnabled=!1;const i=ca(e.backgroundColor,l);d.fillStyle=i,d.fillRect(0,0,t.width,t.height);const h=e.layers.background;for(let f=0;f<e.height;f++)for(let b=0;b<e.width;b++){const p=(_=h[f])==null?void 0:_[b];if(!(p!=null&&p.tileId))continue;const g=a.find(I=>I.id===p.tileId);if(!g)continue;const{data:S,width:T,height:v,lineAttributes:E}=g;if(!S)continue;const c=p.subTileX??0,u=p.subTileY??0,y=c*n,A=u*n;for(let I=0;I<n;I++)for(let L=0;L<n;L++){const w=y+L,N=A+I;if(N<v&&w<T){let C=(m=S[N])==null?void 0:m[w];if(C===void 0)continue;if(r&&E&&E[N]){const O=Math.floor(w/Pe),H=E[N][O];H&&C!==H.fg&&C!==H.bg&&(C=H.fg)}d.fillStyle=C,d.fillRect(b*n+L,f*n+I,1,1)}}}};function ha(t){const e=t.find(s=>s.type==="globalvariables");if(!e||!e.data)return[...$e];const a=e.data.customVariables||[],l=new Map;$e.forEach(s=>{l.set(s.name,s)}),a.forEach(s=>{l.set(s.name,s)});const n=$e.map(s=>s.name),o=[];return n.forEach(s=>{const r=l.get(s);r&&(o.push(r),l.delete(s))}),l.forEach(s=>{o.push(s)}),o}function Pn(t){const e=t.find(l=>l.type==="globalvariables");return!e||!e.data?[]:e.data.customVariables||[]}function ua(t){const e=ha(t);if(e.length===0)return[];const a=[];t.filter(h=>h.type==="screenmap").forEach(h=>{var m,f;(((f=(m=h.data)==null?void 0:m.layers)==null?void 0:f.entities)||[]).forEach(b=>{var p,g;(g=(p=b.components)==null?void 0:p.Behavior)!=null&&g.behaviorCode&&a.push(b.components.Behavior.behaviorCode)})});const n=t.find(h=>h.type==="gameflow"),o=new Set,s=new Set;if(n!=null&&n.data){const h=n.data;h.nodes&&Array.isArray(h.nodes)&&h.nodes.forEach(_=>{var m;_.type==="StateMachine"&&((m=_.data)!=null&&m.customCode)&&a.push(_.data.customCode),_.type==="IfThenElse"&&_.variableName&&o.add(_.variableName),_.type==="Globals"&&_.variables&&Array.isArray(_.variables)&&_.variables.forEach(f=>{f.variableName&&s.add(f.variableName)})})}t.filter(h=>h.type==="componentdefinition").forEach(h=>{const _=h.data;_.customCode&&a.push(_.customCode)});const d=[],i=new Set;return e.forEach(h=>{const _=a.some(b=>new RegExp(`\\b${h.asmName}\\b`,"i").test(b)),m=o.has(h.name),f=s.has(h.name);(_||m||f)&&!i.has(h.name)&&(d.push(h),i.add(h.name))}),s.forEach(h=>{if(!i.has(h)){const _=`global_var_${h.replace(/([A-Z])/g,"_$1").toLowerCase().replace(/^_/,"")}`;d.push({name:h,asmName:_,constantPrefix:`${h.replace(/[^A-Za-z0-9]/g,"_").toUpperCase()}_`,type:"8bit",description:"Auto-generated variable from Globals node",values:[{label:"0",value:0}],category:"special"}),i.add(h)}}),o.forEach(h=>{if(!i.has(h)){const _=`global_var_${h.replace(/([A-Z])/g,"_$1").toLowerCase().replace(/^_/,"")}`;d.push({name:h,asmName:_,constantPrefix:`${h.replace(/[^A-Za-z0-9]/g,"_").toUpperCase()}_`,type:"8bit",description:"Auto-generated variable from IfThenElse node",values:[{label:"0",value:0}],category:"special"}),i.add(h)}}),d}const F={AND:"AND",OR:"OR",XOR:"XOR",NOT:"NOT",KEY_PRESSED:"KEY_PRESSED",KEY_RELEASED:"KEY_RELEASED",TIME_OUT:"TIME_OUT",CAN_MOVE_DIRECTION:"CAN_MOVE_DIRECTION",HAS_COLLISION:"HAS_COLLISION",PATH_CLEAR:"PATH_CLEAR",ON_WALL_COLLISION:"ON_WALL_COLLISION",HAS_DEADLY_TILE_COLLISION:"HAS_DEADLY_TILE_COLLISION",ANIMATION_COMPLETE:"ANIMATION_COMPLETE",KEY_AND_MOVEMENT:"KEY_AND_MOVEMENT",VARIABLE_COMPARE:"VARIABLE_COMPARE"},R={NONE:"NONE",SET_POSITION:"SET_POSITION",MOVE_BY:"MOVE_BY",SET_VELOCITY:"SET_VELOCITY",APPLY_FORCE:"APPLY_FORCE",CHANGE_SPRITE:"CHANGE_SPRITE",PLAY_ANIMATION:"PLAY_ANIMATION",SET_ANIMATION_SPEED:"SET_ANIMATION_SPEED",TOGGLE_ANIMATION:"TOGGLE_ANIMATION",PLAY_SOUND:"PLAY_SOUND",PLAY_MUSIC:"PLAY_MUSIC",MUTE_MUSIC:"MUTE_MUSIC",STOP_MUSIC:"STOP_MUSIC",SET_VARIABLE:"SET_VARIABLE",INCREMENT_VARIABLE:"INCREMENT_VARIABLE",DECREMENT_VARIABLE:"DECREMENT_VARIABLE",SET_COMPONENT_PROPERTY:"SET_COMPONENT_PROPERTY",WAIT:"WAIT",GOTO_STATE:"GOTO_STATE",DESTROY_ENTITY:"DESTROY_ENTITY",SPAWN_ENTITY:"SPAWN_ENTITY",GET_RANDOM_ENTITY_POSITION:"GET_RANDOM_ENTITY_POSITION",CHANGE_GAME_FLOW_NODE:"CHANGE_GAME_FLOW_NODE",DECREASE_LIVES:"DECREASE_LIVES",INCREASE_LIVES:"INCREASE_LIVES",RESPAWN_PLAYER:"RESPAWN_PLAYER",BREAK_TILE:"BREAK_TILE",REPLACE_TILE:"REPLACE_TILE",RND:"RND",POINT_AT:"POINT_AT",ADD_VARIABLES:"ADD_VARIABLES",SUBTRACT_VARIABLES:"SUBTRACT_VARIABLES",MULTIPLY_VARIABLES:"MULTIPLY_VARIABLES",DIVIDE_VARIABLES:"DIVIDE_VARIABLES",MODULO_VARIABLES:"MODULO_VARIABLES",ASSIGN_VARIABLE:"ASSIGN_VARIABLE",DISABLE_INPUT:"DISABLE_INPUT",ENABLE_INPUT:"ENABLE_INPUT"};function Xe(t,e){const a=e.filter(C=>C.type==="componentdefinition").map(C=>C.data),l=e.filter(C=>C.type==="entitytemplate").map(C=>C.data),n=e.filter(C=>C.type==="sprite").map(C=>C.data),o=e.filter(C=>C.type==="tile").map(C=>C.data),s=e.filter(C=>C.type==="screenmap").map(C=>C.data),r=e.filter(C=>C.type==="worldmap").map(C=>C.data),d=e.filter(C=>C.type==="statemachine").map(C=>C.data),i=[],h=new Set,_=(C,O,H)=>{var V,oe;if(C!=null&&C.id)return String(C.id);const x=((V=C==null?void 0:C.position)==null?void 0:V.x)??"",B=((oe=C==null?void 0:C.position)==null?void 0:oe.y)??"",W=(C==null?void 0:C.entityTemplateId)??"",k=(C==null?void 0:C.name)??"";return`${(O==null?void 0:O.id)??`screen_${H}`}|${W}|${k}|${x}|${B}`},m=(C,O,H)=>{if(!C||typeof C!="object")return;const x=_(C,O,H);h.has(x)||(h.add(x),i.push({...C,screenAssetId:C.screenAssetId||(O==null?void 0:O.id),screenIndex:typeof C.screenIndex=="number"?C.screenIndex:H}))};s.forEach((C,O)=>{var H;(H=C.layers)!=null&&H.entities&&Array.isArray(C.layers.entities)&&C.layers.entities.forEach(x=>m(x,C,O)),C.entities&&Array.isArray(C.entities)&&C.entities.forEach(x=>m(x,C,O))});const f=e.find(C=>C.type==="gameflow"),b=f==null?void 0:f.data,p=i.length>0,g=a.length>0||p,S=s.length>1,T=n.length>0,v=o.length>0,E=s.length>0,c=a.length>0,u=!!b,y=e.some(C=>C.type==="font"),A=n.some(C=>C.frames.length>1),I=s.some(C=>C.layers.collision.some(O=>O.some(H=>H!==null))),L=l.some(C=>C.name.toLowerCase().includes("menu")),w=[];a.forEach(C=>{C.name.toLowerCase().includes("state")&&w.push(C.name.replace(/[^a-zA-Z0-9]/g,"").toUpperCase())});const N=ua(e);return{projectName:t,components:a,templates:l,sprites:n,tiles:o,screenMaps:s,screens:s,worldmaps:r,entities:i,fonts:e.filter(C=>C.type==="font"),gameFlow:b,stateMachines:d,hasECS:g,hasMultipleScreens:S,hasSprites:T,hasTiles:v,hasScreens:E,hasEntities:p,hasComponents:c,hasGameFlow:u,hasMenus:L,hasFonts:y,hasAnimations:A,hasCollisions:I,hasMenuSystem:L,customStates:w,globalVariables:N}}const ma=t=>{if(!t.hasECS)return`    ; No ECS system - basic entity updates
...
input_no_pause:`),e+=`
    RET`,e},Ea=t=>t.hasMenuSystem?`    ; Update menu graphics and cursor
...
menu_draw_cursor:
    ...
    RET`:`    ; No menu system
    RET`,ga=t=>{if(t.customStates.length===0)return"; No custom states detected";let e=`; Custom state handlers for project-specific logic
`;return t.customStates.forEach(a=>{e+=`
logic_${a.toLowerCase()}:
    ; Custom logic for ${a} state
    ; TODO: Implement ${a} specific logic
    RET
`}),e},Sa=[{marker:"{{ENTITY_UPDATES}}",generator:ma,description:"Entity update system based on ECS components"},{marker:"{{SPRITE_UPDATES}}",generator:fa,description:"Sprite animation and movement updates"},{marker:"{{COLLISION_CHECK}}",generator:ba,description:"Collision detection system"},{marker:"{{INPUT_HANDLING}}",generator:ya,description:"Input handling with project-specific controls"},{marker:"{{MENU_SYSTEM}}",generator:Ea,description:"Menu system updates and rendering"},{marker:"{{CUSTOM_STATES}}",generator:ga,description:"Custom state handlers detected from project"}];function Aa(t,e,a,l=Sa){const n=Xe(e,a);let o=t;return o=o.replace(/{{PROJECT_NAME}}/g,e.toUpperCase()),o=o.replace(/{{PROJECT_NAME_LOWER}}/g,e.toLowerCase()),o=o.replace(/{{GENERATION_DATE}}/g,new Date().toISOString()),l.forEach(s=>{if(o.includes(s.marker)){const r=s.generator(n);o=o.replace(new RegExp(Ia(s.marker),"g"),r)}}),o}function Ta(){return`;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
`}function Ia(t){return t.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}function On(t,e){const a=Ta(),l=Aa(a,t,e),o=`${t.toLowerCase().replace(/[^a-z0-9]/g,"_")}_dynamic_system.asm`,s=Xe(t,e);return{filename:o,content:l,analysis:s}}function fe(t,e,a){if(!(!a||a.length===0)){t.push(`;   ${e}:`);for(const l of a)t.push(`;     - ${l}`)}}function j(t){const e=[];return e.push("; Register Contract:"),t.purpose&&e.push(`;   Purpose: ${t.purpose}`),fe(e,"Inputs",t.inputs),fe(e,"Outputs",t.outputs),fe(e,"Clobbers",t.clobbers),fe(e,"Preserved",t.preserved),fe(e,"Register roles",t.usage),fe(e,"Notes",t.notes),`${e.join(`
`)}
`}function Ca(t={mode:"hybrid"}){const{mode:e,optimizeLevel:a="safe",includeDebug:l=!1}=t;let n=`; ==================================================================
...
`;return n+=va(),n+=La(),n+=Ra(),n+=wa(),n+=Na(),n+=xa(),n+=Ma(),a==="aggressive"&&(n+=Da(),n+=Pa()),l&&(n+=Oa()),n+=`
; ==================================================================
; END OF DIRECT HARDWARE ROUTINES
; ==================================================================
`,n}function va(){return`
; ==================================================================
...
`+Ca(e):a}function Ua(t){let e="";if(!t.globalVariables||t.globalVariables.length===0)return e+=`; Goal Variable Values (default)
...
`,l.values.forEach(n=>{const o=(n.asmConstant||"UNKNOWN").trim(),s=typeof n.value=="number"?n.value:0;a.has(o)||(e+=`${o.padEnd(24)}EQU ${s}    ; ${l.name} = "${n.label}"
`,a.add(o))}))}),e}function Fa(t){var a,l;const e=Qe(t.sprites||[]).sprites.length;return`; ==================================================================
...
; ==================================================================
`,t.globalVariables&&t.globalVariables.length>0?t.globalVariables.forEach(l=>{const n=l.type==="16bit"?2:1,o=l.type==="16bit"?" (16-bit)":" (8-bit)",s=l.description||l.name;e+=`${l.asmName.padEnd(20)} EQU #${a.toString(16).toUpperCase().padStart(4,"0")}   ; ${s}${o}
...
`,a++,e+=`anim_tile_transform_flags EQU #${a.toString(16).toUpperCase().padStart(4,"0")}   ; Runtime flags for transform-mode tile animation
`,a++,e+=`anim_tile_row_buffer EQU #${a.toString(16).toUpperCase().padStart(4,"0")}   ; Temp buffer (8 bytes) for row transforms
`,a+=8,e+=`
; ==================================================================
...
; ==================================================================
`,e}function $a(t){if(!t)return"";let e="";return e+=`    ld a, 1
...
; END OF HEADER
; ==================================================================
`}function se(t){return t.replace(/[^a-zA-Z0-9]/g,"_")}function bt(t){return String(t||"").replace(/"/g,"").replace(/\r?\n/g," ").trim()}function yt(t){const e=String(t||"").trim();if(!e||e.toLowerCase().startsWith("rgba(0,0,0,0"))return null;const a=e.replace("#","");if(a.length!==6)return null;const l=parseInt(a.substring(0,2),16),n=parseInt(a.substring(2,4),16),o=parseInt(a.substring(4,6),16);return[l,n,o].some(s=>Number.isNaN(s))?null:{r:l,g:n,b:o}}function Bt(t,e=!0){const a=String(t||"").trim();if(!a||a.toLowerCase().startsWith("rgba(0,0,0,0"))return e?0:1;const l=a.toUpperCase(),n=X.find(d=>d.hex.toUpperCase()===l);if(n)return n.index;const o=yt(a);if(!o)return e?0:1;let s=e?0:1,r=1/0;for(const d of X){if(!e&&d.index===0)continue;const i=yt(d.hex);if(!i)continue;const h=(o.r-i.r)**2+(o.g-i.g)**2+(o.b-i.b)**2;h<r&&(r=h,s=d.index)}return s}function Et(t){const e=Bt(t,!1);return e===0?1:e}function Ha(t,e){const a=String(e||"").trim();return a?(Array.isArray(t.sprites)?t.sprites:[]).findIndex(n=>String((n==null?void 0:n.id)||"").trim()===a):-1}function Va(t){var o;const e=(t==null?void 0:t.spritePalette)||[],a=t==null?void 0:t.backgroundColor,l=(t==null?void 0:t.frames)||[];if(!e.length||!l.length)return[];const n=[];for(let s=0;s<e.length;s++){const r=e[s];if(!r||r===a)continue;let d=!1;for(const i of l)if(i!=null&&i.data){for(let h=0;h<(i.data.length||0)&&!d;h++)for(let _=0;_<(((o=i.data[h])==null?void 0:o.length)||0)&&!d;_++)i.data[h][_]===r&&(d=!0);if(d)break}d&&n.push(s)}return n}function za(t){const e=(t==null?void 0:t.spritePalette)||[],a=t==null?void 0:t.backgroundColor,l=Va(t);if(l.length===0)return{layerOffsets:[0],layerColors:[15]};const n=l.slice(0,4);if(n.length===0)return{layerOffsets:[0],layerColors:[15]};const o=n.map((r,d)=>d),s=n.map(r=>{const d=e[r];return!d||a&&d===a?0:Bt(d,!0)});return{layerOffsets:o,layerColors:s}}function Ga(t){var l,n,o;const e=((l=t==null?void 0:t.appearance)==null?void 0:l.selectorType)??((n=t==null?void 0:t.appearance)==null?void 0:n.cursorType)??((o=t==null?void 0:t.appearance)==null?void 0:o.cursorMode)??(t==null?void 0:t.selectorType)??(t==null?void 0:t.cursorType)??(t==null?void 0:t.cursorMode),a=String(e||"").trim().toLowerCase();return a==="char"||a==="character"||a==="text"||a==="glyph"?"char":a==="sprite"||a==="image"?"sprite":"auto"}function Wa(t){var n;const e=Array.isArray(t==null?void 0:t.options)?t.options:[];if(e.length===0)return 0;const a=(t==null?void 0:t.initialSelection)??(t==null?void 0:t.initialSelectedOption)??((n=t==null?void 0:t.appearance)==null?void 0:n.initialSelection)??0,l=Number(a);return!Number.isFinite(l)||l<0||l>=e.length?0:Math.floor(l)}function $t(t){return`NODE_TYPE_${t.replace(/([a-z])([A-Z])/g,"$1_$2").toUpperCase()}`}function Ya(t){const e=(t.name||"DEFAULT").toUpperCase().replace(/[^A-Z0-9]/g,"_"),a=t.id?`_${t.id.replace(/[^a-zA-Z0-9]/g,"_").slice(-12)}`:"";return`load_screen_${e.toLowerCase()}${a.toLowerCase()}`}function gt(t,e){const a=String(t||"").trim();if(!a)return null;const l=r=>`global_var_${r.replace(/([A-Z])/g,"_$1").toLowerCase().replace(/^_/,"").replace(/[^a-z0-9_]/g,"_")}`,n=a.toLowerCase(),o=l(a),s=Array.isArray(e.globalVariables)?e.globalVariables:[];for(const r of s){const d=String((r==null?void 0:r.name)||"").trim(),i=String((r==null?void 0:r.asmName)||"").trim();if(d&&d.toLowerCase()===n)return i||l(d);if(i&&i.toLowerCase()===n)return i;if(d&&l(d)===o)return i||l(d)}return null}function Qa(t){var n,o;const e=(o=(n=t==null?void 0:t.hudConfiguration)==null?void 0:n.importedFrame)==null?void 0:o.cells;if(!Array.isArray(e)||e.length===0)return null;const a=(t.name||"DEFAULT").toUpperCase().replace(/[^A-Z0-9]/g,"_"),l=t.id?`_${t.id.replace(/[^a-zA-Z0-9]/g,"_").slice(-12)}`:"";return`hud_imported_frame_${a.toLowerCase()}${l.toLowerCase()}_draw`}function Xa(t){var o,s,r,d;if(!t.gameFlow)return qa(t);const e=t.gameFlow;let a=`; ==================================================================
...
; Total Connections: ${((s=e.connections)==null?void 0:s.length)||0}
...
`;const l=Array.from(new Set(((r=e.nodes)==null?void 0:r.map(i=>i.type))||[]));l.forEach(i=>{const h=`gameflow_handle_${i.toLowerCase()}`;a+=`    cp ${$t(i)}
    jp z, ${h}
`}),a+=`    
...
`;const n=(d=t.screenMaps)==null?void 0:d.some(i=>{var h;return((h=i.hudConfiguration)==null?void 0:h.elements)&&i.hudConfiguration.elements.length>0});return a+=`; ==================================================================
...
`,e.nodes&&e.nodes.length>0&&e.nodes.forEach(i=>{a+=Za(i,e,t)}),a+=`
; ==================================================================
...
`,t.globalVariables.forEach(i=>{const h=i.name,_=i.asmName||`global_var_${h.replace(/([A-Z])/g,"_$1").toLowerCase().replace(/^_/,"")}`,m=i.values&&i.values.length>0?i.values[0].value:0;a+=`    ld a, ${typeof m=="boolean"?m?1:0:m}
`,a+=`    ld (${_}), a    ; ${h} = ${m}
`})),a+=`    ret
...
; END OF GAMEFLOW
; ==================================================================
`,a}function Ka(t,e){var s;let a="";const l=(s=e.screenMaps)==null?void 0:s.some(r=>{var d;return((d=r.hudConfiguration)==null?void 0:d.elements)&&r.hudConfiguration.elements.length>0});t.forEach(r=>{var d;switch(r){case"Start":a+=`gameflow_handle_start:
...
`;break;case"SubMenu":{const i=Math.max(((d=e.sprites)==null?void 0:d.length)||0,1);let h="";for(let _=0;_<i;_++)h+=`    dw SPRITE_${_}_PATTERN
...
`),a}function Za(t,e,a){var i,h,_,m,f,b,p,g,S,T,v,E;const l=`gameflow_node_${se(t.id)}`,n=`${l}_conn`,o=["Start","WorldLink","SubMenu","Text","IfThenElse","Globals","Transition"].includes(t.type)||t.type==="Globals"&&t.variables&&t.variables.length>0,s=o?`${l}_data`:"gameflow_no_data";let r=`; Node: ${t.type} - "${t.title||t.name||t.id}"
${l}:
    db ${$t(t.type)}
    dw ${s}
    dw ${n}
...
`;break;case"WorldLink":const c=t.worldAssetId||"default";r+=`    dw load_world_${se(c)}
`,r+=`    db ((load_world_${se(c)} - #4000) / #2000)
`;break;case"SubMenu":{const I=se(t.id),L=(Array.isArray(t.options)?t.options:[]).slice(0,6),w=L.length,N=Wa(t),C=w>0?Math.min(N,w-1):0,O=bt(t.title||t.name||"MENU").toUpperCase(),H=((h=(i=t==null?void 0:t.appearance)==null?void 0:i.colors)==null?void 0:h.background)||"#000000",x=Et(H),B=Ga(t),W=(_=t==null?void 0:t.appearance)==null?void 0:_.cursorSpriteAssetId,k=Ha(a,W),K=k>=0?(m=a.sprites)==null?void 0:m[k]:null,V=B==="char"?!1:k>=0,oe=V?k:255,z=V&&K?za(K):{layerOffsets:[],layerColors:[]},G=z.layerOffsets.slice(0,4),q=z.layerColors.slice(0,4),Ie=Math.min(q.length,4);for(;G.length<4;)G.push(0);for(;q.length<4;)q.push(0);const ne=(f=t==null?void 0:t.appearance)==null?void 0:f.backgroundScreenAssetId;let ae="0";if(ne&&a.screenMaps){const $=a.screenMaps.find(Z=>Z.id===ne);if($){const Z=$.name.toUpperCase().replace(/[^A-Z0-9]/g,"_"),re=$.id?`_${$.id.replace(/[^a-zA-Z0-9]/g,"_").slice(-12)}`:"";ae=`load_screen_${Z.toLowerCase()}${re.toLowerCase()}`}}const ce=ae==="0"?"0":`((${ae} - #4000) / #2000)`;r+=`    db ${x}    ; Background color (MSX index)
`,r+=`    db ${oe}    ; Cursor sprite asset index (#FF = use text marker)
`,r+=`    db ${Ie}    ; Cursor sprite layer count (max 4)
...
`,r+=`    db ${w}    ; Number of options (max 6)
`,r+=`    db ${C}    ; Initial selected option
`,r+=`    dw submenu_${I}_title
`,L.forEach(($,Z)=>{r+=`    dw submenu_${I}_opt${Z}
`}),r+=`
submenu_${I}_title:
`,r+=`    db "${O}", 0
`,L.forEach(($,Z)=>{const re=bt(($==null?void 0:$.text)||($==null?void 0:$.label)||($==null?void 0:$.name)||($==null?void 0:$.id)||`OPTION ${Z+1}`).toUpperCase();r+=`submenu_${I}_opt${Z}:
`,r+=`    db "${re}", 0
`})}break;case"Text":{const I=se(t.id),L=(t.title||t.name||"").replace(/"/g,"").replace(/\r?\n/g," ").trim().toUpperCase()||"TEXT",w=(t.message||"").replace(/"/g,"").replace(/\r?\n/g," "),N=((p=(b=t.appearance)==null?void 0:b.colors)==null?void 0:p.background)||"#000000",C=Et(N),O=28,H=w.split(" "),x=[];let B="";for(const z of H){const G=z.toUpperCase(),q=B?B+" "+G:G;q.length>O&&B?(x.push(B),B=G):B=q}B.trim()&&x.push(B);const W="PRESS FIRE TO CONTINUE",k=[];k.push({row:3,text:L,label:`text_${I}_title`}),x.forEach((z,G)=>{k.push({row:7+G,text:z,label:`text_${I}_msg${G}`})}),k.push({row:20,text:W,label:`text_${I}_prompt`});const K=(g=t.appearance)==null?void 0:g.backgroundScreenAssetId;let V="0";if(K&&a.screenMaps){const z=a.screenMaps.find(G=>G.id===K);if(z){const G=z.name.toUpperCase().replace(/[^A-Z0-9]/g,"_"),q=z.id?`_${z.id.replace(/[^a-zA-Z0-9]/g,"_").slice(-12)}`:"";V=`load_screen_${G.toLowerCase()}${q.toLowerCase()}`}}const oe=V==="0"?"0":`((${V} - #4000) / #2000)`;r+=`    DB ${C}                  ; Background color (MSX index from ${N})
`,r+=`    DW ${V}            ; Background screen load function (0=none)
`,r+=`    DB ${oe}         ; Background screen load bank
`,r+=`    DB ${k.length}                  ; Number of lines
`;for(const z of k){const G=Math.max(0,Math.floor((32-z.text.length)/2));r+=`    DB ${z.row}, ${G}              ; Row ${z.row}, Col ${G}
`,r+=`    DW ${z.label}          ; -> "${z.text}"
`}r+=`
`;for(const z of k)r+=`${z.label}:
`,r+=`    DB "${z.text}", 0
`;break}case"IfThenElse":const u=t.variableName||"unknown",y=gt(u,a),A=t.compareValue||0;y?r+=`    dw ${y}    ; Variable to check
`:r+=`    dw 0                 ; WARNING: Missing global variable "${u}"
`,r+=`    db ${A}   ; Compare value
`,r+=`    db 0                 ; Operator (0=equals)
`;break;case"Globals":if(t.variables&&t.variables.length>0){const I=t.variables.map(w=>{const N=w.variableName||w.name||"unknown",C=gt(N,a),O=w.value||0;return{vName:N,vAsmName:C,vValue:O}}).filter(w=>!!w.vAsmName);r+=`    db ${I.length}    ; Number of assignments
`,I.forEach(w=>{r+=`    dw ${w.vAsmName}
`,r+=`    db ${w.vValue}
`});const L=t.variables.length-I.length;L>0&&(r+=`    ; WARNING: ${L} Globals assignment(s) skipped (undefined global variable)
`),I.length===0&&(r+=`    ; No valid global assignments found
`)}else r+=`    db 0    ; No assignments
`;break;case"Transition":{const I={cls:0,dissolve_pixels:1,dissolve_chars:2,vertical_lines:3,horizontal_lines:4,spiral:5,fill_white_squares:6},L={cls:1,dissolve_pixels:8,dissolve_chars:8,vertical_lines:16,horizontal_lines:24,spiral:96,fill_white_squares:4},w=I[t.effect]??0,N=L[t.effect]??8,C=t.duration??1e3,O=Math.max(1,Math.min(255,Math.round(C/N/20)));r+=`    db ${w}              ; Effect: ${t.effect||"cls"}
`,r+=`    db ${O}              ; Frames per step (duration ${C}ms / ${N} steps / 20ms)
`;break}}r+=`
`}r+=`${n}:
`;const d=((S=e.connections)==null?void 0:S.filter(c=>{var u;return(((u=c.from)==null?void 0:u.nodeId)||c.from)===t.id}))||[];if(t.type==="IfThenElse"){const c=d.find(y=>{var A,I;return((A=y.from)==null?void 0:A.sourceId)==="then"||!((I=y.from)!=null&&I.sourceId)}),u=d.find(y=>{var A;return((A=y.from)==null?void 0:A.sourceId)==="else"});r+=`    db CONNECTION_THEN
`,r+=`    dw ${c?`gameflow_node_${se(((T=c.to)==null?void 0:T.nodeId)||c.to)}`:"0"}
`,r+=`    db CONNECTION_ELSE
`,r+=`    dw ${u?`gameflow_node_${se(((v=u.to)==null?void 0:v.nodeId)||u.to)}`:"0"}
`}else if(t.type==="SubMenu")(Array.isArray(t.options)?t.options:[]).slice(0,6).forEach((u,y)=>{var I;const A=d.find(L=>{var w;return((w=L.from)==null?void 0:w.sourceId)===u.id});r+=`    db CONNECTION_OPTION_${y}
`,r+=`    dw ${A?`gameflow_node_${se(((I=A.to)==null?void 0:I.nodeId)||A.to)}`:"0"}
`});else{const c=d[0];r+=`    db CONNECTION_DEFAULT
`,r+=`    dw ${c?`gameflow_node_${se(((E=c.to)==null?void 0:E.nodeId)||c.to)}`:"0"}
...
`,l}function qa(t){var o;const e=(o=t.screenMaps)==null?void 0:o.some(s=>{var r;return((r=s.hudConfiguration)==null?void 0:r.elements)&&s.hudConfiguration.elements.length>0}),a=t.screenMaps&&t.screenMaps.length>0?t.screenMaps[0]:null,l=a?Qa(a):null;return`; ==================================================================
...
mapper_call_hl_auto:
    jp mapper_call_hl_p1
`;const o=tl(e),s=n?"; Mapper register writes are enabled for this build configuration.":"; Mapper register writes are disabled (simple32k mode).";return`; ==================================================================
; MAPPER RUNTIME API
; File: mapper.asm
...
`}function ll(t){var e;return!t.tiles||t.tiles.length===0?`; ==================================================================
...
; ==================================================================
tile_pattern_bank0:
${t.tiles.map((a,l)=>{const n=Ot(a,"SCREEN 2 (Graphics I)"),o=Math.ceil(a.width/8),s=Math.ceil(a.height/8),r=o*s;(a.width%8!==0||a.height%8!==0)&&console.warn(`⚠️  Tile ${a.name} size ${a.width}x${a.height} is not multiple of 8px - may cause visual artifacts`);const d=Array.from(n).map(h=>`#${h.toString(16).padStart(2,"0").toUpperCase()}`);let i="";if(r>1){i=`
    ; Character layout: ${o}×${s} grid`;for(let h=0;h<s;h++){i+=`
    ; Row ${h}: `;for(let _=0;_<o;_++){const m=h*o+_;i+=`Char${m} `}}}return`    ; Tile ${l}: ${a.name} (${a.width}x${a.height}px = ${o}×${s} chars = ${r} MSX characters)${i}
    db ${d.join(", ")}
`}).join("")}
...
; ==================================================================
tile_color_bank0:
${t.tiles.map((a,l)=>{const n=kt(a),o=n?Array.from(n).map(s=>`#${s.toString(16).padStart(2,"0").toUpperCase()}`):["#F0","#F0","#F0","#F0","#F0","#F0","#F0","#F0"];return`    ; Tile ${l}: ${a.name} colors (fg/bg pairs)
    db ${o.join(", ")}
`}).join("")}
...
; END OF COLOR DATA
; ==================================================================
`}const ve=8192,nl=new Set(["header.asm","bios.asm","constants.asm","variables.asm","mapper.asm","interrupt.asm","main.asm","unitedFiles.asm"]);function rl(t){const e=t.trim().toLowerCase();return e?/^\d+$/.test(e)?parseInt(e,10):/^#([0-9a-f]+)$/.test(e)?parseInt(e.slice(1),16):/^0x([0-9a-f]+)$/.test(e)?parseInt(e.slice(2),16):/^([0-9a-f]+)h$/.test(e)?parseInt(e.slice(0,-1),16):null:null}function il(t){let e=0;const a=t.split(/\r?\n/);for(const l of a){const n=l.split(";")[0].trim();if(!n)continue;const o=n.match(/^db\s+(.+)$/i);if(o){e+=o[1].split(",").filter(d=>d.trim().length>0).length;continue}const s=n.match(/^dw\s+(.+)$/i);if(s){e+=s[1].split(",").filter(d=>d.trim().length>0).length*2;continue}const r=n.match(/^ds\s+(.+)$/i);if(r){const d=rl(r[1]);d!==null&&d>0&&(e+=d)}}return e}function sl(t){if(!t)return 0;const e=il(t),a=new TextEncoder().encode(t).length,l=Math.floor(a*.28);return Math.max(e,l)}function dl(t){const e=Object.entries(t).filter(([r,d])=>!!d&&!nl.has(r)).map(([r,d])=>({moduleName:r,estimatedBytes:sl(d)})).filter(r=>r.estimatedBytes>0),a=[];let l=0,n=0,o=0;for(const r of e){let d=r.estimatedBytes,i=0;const h=Math.max(1,Math.ceil(r.estimatedBytes/ve));for(;d>0;){const _=ve-n,m=Math.min(d,_);a.push({moduleName:r.moduleName,chunkBytes:m,bankIndex:l,bankOffset:n,segmentIndex:i,totalSegments:h}),d-=m,o+=m,n+=m,i++,n>=ve&&(l++,n=0)}}const s=o===0?0:n===0?l:l+1;return{bankSize:ve,totalEstimatedBytes:o,banksUsed:s,entries:a}}function cl(t){const e=[];if(e.push("; ------------------------------------------------------------------"),e.push("; 8KB BANK PACKER ESTIMATE (diagnostic placement view)"),e.push("; Runtime bank constants are derived from label addresses at assemble time."),e.push(`; Estimated payload bytes: ${t.totalEstimatedBytes}`),e.push(`; Estimated banks used: ${t.banksUsed}`),e.push("; ------------------------------------------------------------------"),t.entries.length===0)return e.push("; No banked payload candidates detected."),e.join(`
...
`)}function pl(t,e,a,l={romMode:"simple32k",targetFormat:"konami",autoMegaROM:!1}){var h,_,m,f,b,p,g,S,T,v;const n=(_=(h=a.gameFlow)==null?void 0:h.nodes)==null?void 0:_.some(E=>E.type==="SubMenu"),o=(m=a.screenMaps)==null?void 0:m.some(E=>{var c,u;return((c=E.layers)==null?void 0:c.text)||((u=E.textElements)==null?void 0:u.length)>0}),s=(f=a.screenMaps)==null?void 0:f.some(E=>{var c;return((c=E.hudConfiguration)==null?void 0:c.elements)&&E.hudConfiguration.elements.length>0}),r=n||o||s,d=dl(t),i=cl(d);return`; ==================================================================
...
`}const Me={comp_pos:"Position",comp_position:"Position",comp_render:"Sprite",comp_sprite:"Sprite",comp_movement:"Movement",comp_velocity:"Movement",comp_collision:"Collision",comp_wall_collision:"WallCollision",comp_player_input:"Input",comp_input:"Input",comp_ai_behavior:"Behavior",comp_behavior:"Behavior",comp_health:"Health",comp_animation:"Animation",comp_gravity:"Gravity",comp_jump:"Jump",comp_damage:"Damage",comp_statemachine:"StateMachine",comp_cursors:"Cursors",comp_carry:"Carry",comp_collectible:"Collectible",comp_patrol:"Patrol"};function _l(t,e){var s,r,d;const a=(s=e==null?void 0:e.components)==null?void 0:s.find(i=>i.definitionId==="comp_sprite"||i.definitionId==="comp_render");if(!a)return;const l=a.defaultValues||{},n=((r=t.componentOverrides)==null?void 0:r.comp_sprite)||((d=t.componentOverrides)==null?void 0:d.comp_render)||{},o={...l,...n};return o.spriteId||o.spriteAssetId||o.sprite||o.spriteName}function Oe(t){var o;const e=new Set,a=new Set,l=[],n=new Map;return console.log("🔍 Analyzing component usage..."),console.log(`📊 Total entities in project: ${((o=t.entities)==null?void 0:o.length)||0}`),t.entities&&t.entities.length>0&&t.entities.forEach(s=>{console.log(`  - Entity: ${s.name} (template: ${s.entityTemplateId})`),l.push(s),s.entityTemplateId&&a.add(s.entityTemplateId)}),console.log(`✅ Active entities: ${l.length}`),console.log(`✅ Used templates: ${Array.from(a).join(", ")}`),l.forEach(s=>{var i;const r=s.name||s.id,d=(i=t.templates)==null?void 0:i.find(h=>h.id===s.entityTemplateId);d?(console.log(`  📦 Analyzing template "${d.name}" for entity "${r}"`),d.components&&Array.isArray(d.components)&&d.components.forEach(h=>{const _=h.definitionId||h.componentDefinitionId;if(_){const m=Me[_]||_;console.log(`    - Component: ${_} → ${m}`),e.add(m),n.has(m)||n.set(m,new Set),n.get(m).add(r)}}),s.componentOverrides&&Object.keys(s.componentOverrides).forEach(h=>{const _=Me[h]||h;console.log(`    - Override: ${h} → ${_}`),e.add(_),n.has(_)||n.set(_,new Set),n.get(_).add(r)})):console.warn(`  ⚠️  Template "${s.entityTemplateId}" not found for entity "${r}"`)}),console.log("📊 Component usage summary:"),console.log(`  - Total used components: ${e.size}`),e.forEach(s=>{const r=n.get(s);console.log(`    • ${s}: ${(r==null?void 0:r.size)||0} entities`)}),{usedComponents:e,usedTemplates:a,activeEntities:l,componentToEntitiesMap:n}}function St(t,e,a){var s;let l=0;const n={Position:0,Sprite:1,Movement:2,Collision:3,Input:4,Behavior:5,Health:6,Animation:7,Jump:8,Gravity:9};let o=!1;if(e&&e.components&&e.components.forEach(r=>{const d=r.definitionId||r.componentDefinitionId,i=Me[d];i&&n[i]!==void 0&&(l|=1<<n[i],i==="Sprite"&&(o=!0)),i==="Patrol"&&(l|=1<<n.Movement)}),t.componentOverrides&&Object.keys(t.componentOverrides).forEach(r=>{const d=Me[r];d&&n[d]!==void 0&&(l|=1<<n[d],d==="Sprite"&&(o=!0))}),l|=1<<n.Position,o)l|=1<<n.Sprite;else{const r=_l(t,e);r&&((s=a.sprites)==null?void 0:s.some(i=>i.id===r||i.name===r))&&(l|=1<<n.Sprite)}return l}const hl=224,ul="hex",jt=t=>{var o;const e=(t==null?void 0:t.spritePalette)||[],a=t==null?void 0:t.backgroundColor,l=(t==null?void 0:t.frames)||[];if(!e.length||!l.length)return[];const n=[];for(let s=0;s<e.length;s++){const r=e[s];if(!r||r===a)continue;let d=!1;for(const i of l)if(i!=null&&i.data){for(let h=0;h<(i.data.length||0)&&!d;h++)for(let _=0;_<(((o=i.data[h])==null?void 0:o.length)||0)&&!d;_++)i.data[h][_]===r&&(d=!0);if(d)break}d&&n.push(s)}return n},At=t=>{const e=jt(t);return e.length>0?e[0]:-1};function ml(t){var v,E;const e=t.sprites||[],a=Qe(e),l=a.sprites,n=a.nameToIndex,o=a.directionalLookupTables;a.warnings.forEach(c=>{console.warn(`[Sprites Generator] ${c}`)}),console.log("🎨 generateSpritesFile() called:"),console.log(`  - analysis.sprites.length: ${e.length}`),console.log(`  - expandedSprites.length: ${l.length}`),console.log(`  - analysis.entities.length: ${((v=t.entities)==null?void 0:v.length)||0}`),console.log(`  - analysis.templates.length: ${((E=t.templates)==null?void 0:E.length)||0}`);const{activeEntities:s}=Oe(t);console.log(`  - activeEntities.length: ${s.length}`);const r=c=>{if(!c||c.startsWith("rgba"))return null;const u=c.replace("#","");return u.length!==6?null:{r:parseInt(u.substring(0,2),16),g:parseInt(u.substring(2,4),16),b:parseInt(u.substring(4,6),16)}},d=c=>{if(!c)return 0;const u=X.find(L=>L.hex.toUpperCase()===c.toUpperCase());if(u)return u.index;const y=r(c);if(!y)return 15;let A=15,I=1/0;for(const L of X){if(L.index===0)continue;const w=r(L.hex);if(!w)continue;const N=(y.r-w.r)**2+(y.g-w.g)**2+(y.b-w.b)**2;N<I&&(I=N,A=L.index)}return A},i=c=>{if(!c)return[15];const u=c.spritePalette||[],y=c.backgroundColor,A=jt(c);if(A.length===0)return[15];const I=A.map(L=>{const w=u[L];return!w||y&&w===y?0:d(w)});return I.length>0?I:[15]},h=(c,u)=>{let y=`${c}:
`;if(u.length===0)return y+=`    db 0
`,y;const A=16;for(let I=0;I<u.length;I+=A){const L=u.slice(I,I+A);y+=`    db ${L.join(", ")}
`}return y},_=c=>{var L,w,N,C,O,H;console.log(`
🔍 getEntitySpriteInfo for entity: "${c.name}" (template: ${c.entityTemplateId})`),console.log(`   Available sprites: ${l.map(x=>`"${x.name}" (${x.id})`).join(", ")||"NONE"}`);const u=(L=t.templates)==null?void 0:L.find(x=>x.id===c.entityTemplateId);if(!u)return console.log("   ❌ Template not found!"),null;console.log(`   Template found: "${u.name}"`),console.log(`   Template components: ${((w=u.components)==null?void 0:w.map(x=>x.definitionId).join(", "))||"NONE"}`);const y=t.components||[];let A;if(c.componentOverrides)for(const x in c.componentOverrides){const B=y.find(k=>k.id===x),W=(N=B==null?void 0:B.properties)==null?void 0:N.find(k=>k.type==="sprite_ref");if(W&&((C=c.componentOverrides[x])!=null&&C[W.name])){A=c.componentOverrides[x][W.name],console.log(`   ✅ Found spriteAssetId in overrides: "${A}"`);break}}if(!A)for(const x of u.components||[]){const B=y.find(k=>k.id===x.definitionId),W=(O=B==null?void 0:B.properties)==null?void 0:O.find(k=>k.type==="sprite_ref");if(W&&((H=x.defaultValues)!=null&&H[W.name])){A=x.defaultValues[W.name],console.log(`   ✅ Found spriteAssetId in template defaults: "${A}"`);break}}if(console.log(`   Resolved spriteAssetId: "${A||"undefined"}"`),!A)return console.log("   ⚠️ No sprite_ref property found in any component"),l.length>0?(console.log(`   ⚠️ Defaulting to first sprite "${l[0].name}"`),{spriteAssetIndex:0,spriteName:l[0].name,colors:i(l[0])}):null;let I=n[A];if(I===void 0&&(I=n[A.toLowerCase()]),I===void 0){const x=A.toLowerCase();I=l.findIndex(B=>{var W,k;return((W=B.name)==null?void 0:W.toLowerCase().includes(x))||x.includes(((k=B.name)==null?void 0:k.toLowerCase())||"")})}return I!==void 0&&I>=0?(console.log(`   ✅ Found sprite "${l[I].name}" at index ${I}`),{spriteAssetIndex:I,spriteName:l[I].name,colors:i(l[I])}):(console.log(`   ❌ Sprite "${A}" not found in project assets`),{spriteAssetIndex:-1,spriteName:`MISSING_${A}`,colors:[15]})},m=[];let f=0;s.forEach((c,u)=>{const y=_(c);if(!y){m.push({entityIndex:u,spriteName:"PLACEHOLDER",spriteAssetIndex:-1,baseHwSpriteIndex:f,layerCount:1,colors:[15]}),f+=1;return}m.push({entityIndex:u,spriteName:y.spriteName,spriteAssetIndex:y.spriteAssetIndex,baseHwSpriteIndex:f,layerCount:y.colors.length,colors:y.colors}),f+=y.colors.length});const b=32;let p=`; ==================================================================
; SPRITE DATA
; File: sprites.asm
...
; SPRITE PATTERN DATA
; ==================================================================
`;l.forEach((c,u)=>{const y=`_${u}`,I=(c.name+y).replace(/[^a-zA-Z0-9_]/g,"_").toUpperCase(),L=ra(c,ul,u),w=At(c);p+=`
; Sprite Asset ${u}: ${c.name}
${L}`,w>=0?p+=`
; Unified pattern label for sprite ${u}
SPRITE_${u}_PATTERN EQU ${I}_F0_LAYER${w}
...
`,l.forEach((c,u)=>{var A;const y=((A=c.frames)==null?void 0:A.length)||1;p+=`    db ${y} ; Sprite ${u}: ${c.name}
`}),l.length===0&&(p+=`    db 1 ; Placeholder
`),p+=`
; Table: Sprite Asset Loop Flags
; Format: db flags (bit 1: 1=loop, 0=once)
sprite_loop_flags:
`,l.forEach((c,u)=>{const A=c.loops!==!1?"2":"0";p+=`    db ${A} ; Sprite ${u}: ${c.name}
...
`,l.forEach((c,u)=>{p+=`    dw SPRITE_${u}_FRAME_PTRS
`}),l.length===0&&(p+=`    dw SPRITE_0_FRAME_PTRS
`),l.forEach((c,u)=>{var N;const y=`_${u}`,I=(c.name+y).replace(/[^a-zA-Z0-9_]/g,"_").toUpperCase(),L=At(c),w=((N=c.frames)==null?void 0:N.length)||1;p+=`
; Sprite ${u}: ${c.name} frame pointers
SPRITE_${u}_FRAME_PTRS:
`;for(let C=0;C<w;C++)L>=0?p+=`    dw ${I}_F${C}_LAYER${L}
...
`,m.forEach(c=>{const u=c.baseHwSpriteIndex>=0?c.baseHwSpriteIndex:0;p+=`    db ${u}, ${c.layerCount} ; Entity ${c.entityIndex} (${c.spriteName})
`}),m.length<32&&(p+=`    ds ${(32-m.length)*2}, 0 ; Padding
`),p+=`
...
`,m.forEach(c=>{const u=c.spriteAssetIndex>=0?c.spriteAssetIndex:255;p+=`    db #${u.toString(16).toUpperCase().padStart(2,"0")} ; Entity ${c.entityIndex} (${c.spriteName})
`}),m.length<32&&(p+=`    ds ${32-m.length}, #FF ; Padding
`),p+=` 
; Table: Hardware Sprite Layer Colors 
; Format: db color_index 
sprite_layer_colors: 
`;let g=0;m.forEach(c=>{c.layerCount>0&&(p+=`    ; Entity ${c.entityIndex} (${c.spriteName}) layers:
`,c.colors.forEach((u,y)=>{p+=`    db ${u} ; Layer ${y}
`,g+=1}))});const S=b-g;S>0&&(p+=`    ds ${S}, 0 ; Padding
`),p+=`
; ==================================================================
...
load_sprite_patterns:
    ...
    call mapper_push_p2
`;let T=!1;if(m.forEach(c=>{if(c.layerCount===0)return;const u=c.spriteAssetIndex<0?"SPRITE_PLACEHOLDER_PATTERN":`SPRITE_${c.spriteAssetIndex}_PATTERN`;p+=`    ; Entity ${c.entityIndex}: ${c.spriteName} (${c.layerCount} layers)
    ...
    call FAST_LDIRVM
`,T=!0}),!T)if(l.length===0)p+=`    ; No sprites to load
`;else{p+=`    ; No active entities detected, load all sprite assets sequentially
`;let c=0;l.forEach((u,y)=>{var w;const A=i(u).length||1,I=((w=u.frames)==null?void 0:w.length)||1,L=A*I*32;p+=`    ; Sprite Asset ${y}: ${u.name} (${I} frames, ${A} layers)
    ...
    ld bc, ${L}
    call FAST_LDIRVM
`,c+=A*I})}return p+=`    call mapper_pop_p2
    ret
...
`}function El(){return`
    ...
    ret
    `}function gl(t){const o=Array.from({length:3},(d,i)=>`    srl a                      ; A = X / ${Math.pow(2,i+1)}`).join(`
...
; Set component mask for entity
    ...
    ret
    `}function Ul(t){const e=t.usedComponents;let a=`init_components: 
...
; Initialize current screen ID(multi - screen support) 
    ...
    `),a+=`
    ret
    `,a}function Ht(t){if(!t.entities||t.entities.length===0)return`; ==================================================================
...
; END OF COMPONENTS(MINIMAL VERSION)
    ; ==================================================================
        `;const e=Oe(t),a=e.usedComponents,l=(i,h)=>{if(!i||typeof i!="object")return!1;const _=String(i.type||"").toUpperCase();if(h.has(_))return!0;const m=Array.isArray(i.conditions)?i.conditions:[];for(const f of m)if(l(f,h))return!0;return!1},n=Array.isArray(t.stateMachines)?t.stateMachines:[],o=new Set(["HAS_COLLISION","HAS_DEADLY_TILE_COLLISION"]);n.some(i=>(Array.isArray(i==null?void 0:i.transitions)?i.transitions:[]).some(_=>l(_==null?void 0:_.conditions,o)))&&!a.has("Collision")&&(console.log("  - Forcing Collision system: required by state machine conditions"),a.add("Collision")),console.log("🎯 Generating optimized components.asm..."),console.log(`  - Active entities: ${e.activeEntities.length} `),console.log(`  - Used components: ${Array.from(a).join(", ")} `),console.log(`  - Filtered out: ${8-a.size} unused components`);let r=`; ==================================================================
...
; CORE ECS SYSTEM FUNCTIONS
    ; ==================================================================
        ...
        ${Ul(e)}
`;r+=bl();const d=t.sprites&&t.sprites.length>0;return a.has("Sprite")||d?r+=yl():r+=`
...
update_collision_component:
    ret
    `,(a.has("Collision")||a.has("WallCollision"))&&(r+=Sl()),a.has("Input")?r+=Al():r+=`
...
update_shoot_component:
    ret
    `,r+=Ll(),a.has("WallCollision")?r+=Pl():r+=`
...
tile_behavior_table:
    ...
    ${Array(127).fill(0).map((i,h)=>`db TILE_PASSABLE              ; ${h+1}: Passable`).join(`
    `)}

    ; Index 128-255: Project tile characters (solid by default)
    ; MSX Screen 2 assigns character IDs >= 128 to project tiles
    ${Array(128).fill(0).map((i,h)=>`db TILE_SOLID                 ; ${128+h}: Solid`).join(`
    `)}
...
; END OF COMPONENT SYSTEMS
    ; ==================================================================
        `,r}function Fl(t){var g,S,T,v;const e=(E,c)=>{if(typeof E=="boolean")return E;if(typeof E=="number")return E!==0;if(typeof E=="string"){const u=E.trim().toLowerCase();if(u==="true")return!0;if(u==="false")return!1;const y=parseInt(u,10);if(!Number.isNaN(y))return y!==0}return c},a=(E,c)=>{const u=typeof E=="number"?E:parseInt(String(E??""),10);return Number.isNaN(u)?c:Math.max(0,Math.min(255,u|0))},l=(E,c)=>{const u=typeof E=="number"?E:parseInt(String(E??""),10);return Number.isNaN(u)?c&255:u<0?256+Math.max(-128,Math.min(-1,u|0))&255:Math.max(0,Math.min(255,u|0))},n=E=>(E&255).toString(16).toUpperCase().padStart(2,"0"),o=E=>{const c=(E==null?void 0:E.screenAssetId)||(E==null?void 0:E.screenId)||(E==null?void 0:E.screenMapId);if(c){const I=t.worldmaps||[];for(const L of I){const N=((L==null?void 0:L.nodes)||[]).findIndex(C=>(C==null?void 0:C.screenAssetId)===c);if(N>=0)return N}}if(typeof(E==null?void 0:E.screenIndex)=="number"&&E.screenIndex>=0)return E.screenIndex;let u=0,y=null;if(t.screenMaps&&t.screenMaps.forEach((I,L)=>{var N;(((N=I==null?void 0:I.layers)==null?void 0:N.entities)||[]).some(C=>C.id===E.id)&&(u=L,y=I.id||null)}),!y)return u;const A=t.worldmaps||[];for(const I of A){const w=((I==null?void 0:I.nodes)||[]).findIndex(N=>(N==null?void 0:N.screenAssetId)===y);if(w>=0)return w}return u},s=E=>{const c={};if(!E||E.length===0)return c;let u=1;return E.forEach(y=>{!y||!y.id||c[y.id]===void 0&&(c[y.id]=u,y.name&&(c[String(y.name)]=u,c[String(y.name).toLowerCase()]=u),u<255&&(u+=1))}),c},d=Oe(t).activeEntities,i=2,h=16,_=s(t.templates),m=E=>String(E??"entity").toUpperCase().replace(/[^A-Z0-9]/g,"_").replace(/^_+|_+$/g,"")||"ENTITY",f=new Map,b=d.map((E,c)=>{const u=m((E==null?void 0:E.name)||`ENTITY_${c}`),y=(f.get(u)||0)+1;return f.set(u,y),y===1?u:`${u}_${y}`});console.log("🎯 Generating optimized entities.asm..."),console.log(`  - Total entity templates in JSON: ${((g=t.templates)==null?void 0:g.length)||0}`),console.log(`  - Actually instantiated entities: ${d.length}`),console.log(`  - Filtered out: ${(((S=t.templates)==null?void 0:S.length)||0)-d.length} unused templates`);let p=`; ==================================================================
; GAME ENTITIES
; File: entities.asm
...
;   Filtered out: ${(((v=t.templates)==null?void 0:v.length)||0)-d.length} unused templates
;
; ==================================================================
...
`,d.forEach((c,u)=>{var L;const y=b[u],A=(L=t.templates)==null?void 0:L.find(w=>w.id===c.entityTemplateId),I=St(c,A,t);p+=`; Entity: ${c.name} (instance from template: ${c.entityTemplateId})
ENTITY_${y}_ID EQU ${u}
ENTITY_${y}_COMP_MASK EQU #${I.toString(16).toUpperCase().padStart(2,"0")}  ; Component mask: ${I.toString(2).padStart(8,"0")}b
`,c.entityTemplateId&&(p+=`; Template: ${c.entityTemplateId}
`),c.position&&(p+=`ENTITY_${y}_X EQU ${c.position.x}
...
`,d.length>0?d.forEach((c,u)=>{const y=b[u];p+=`    call init_${y.toLowerCase()}
`}):p+=`    ; No entities to initialize
`,p+=`    ret

update_entities:
    ; Update all active entities (${d.length} entities)
`,d.length>0?d.forEach((c,u)=>{const y=b[u];p+=`    ; Skip entity update if entity belongs to another screen
...
`;let E=!1;d.forEach((c,u)=>{var et,tt,at,lt,ot,nt,rt,it,st,dt,ct,pt,_t;const y=b[u],A=(et=t.templates)==null?void 0:et.find(P=>P.id===c.entityTemplateId),I=St(c,A,t),L=(I&i)!==0,w=(I&h)!==0;L&&w&&(E=!0);const N=((tt=c.position)==null?void 0:tt.x)||100,C=((at=c.position)==null?void 0:at.y)||100,O=8,H=8,x=N*O,B=C*H,W=Math.min(x,240),k=Math.min(B,191);(x!==W||B!==k)&&console.warn(`Entity ${c.name} position clamped: (${x},${B}) → (${W},${k})`);const K=[];I&1&&K.push("Position"),I&2&&K.push("Sprite"),I&4&&K.push("Movement"),I&8&&K.push("Collision"),I&16&&K.push("Input"),I&32&&K.push("Behavior"),I&64&&K.push("Health"),I&128&&K.push("Animation"),I&256&&K.push("Jump"),I&512&&K.push("Gravity");let V=15;if(I&16){const P=A==null?void 0:A.components.find(Q=>Q.definitionId==="comp_cursors"||Q.definitionId==="comp_input"||Q.definitionId==="comp_player_input");if(P){const Q=P.defaultValues||{},ee=((lt=c.componentOverrides)==null?void 0:lt.comp_cursors)||{},U={...Q,...ee};V=0,U.allowUp!==!1&&(V|=1),U.allowDown!==!1&&(V|=2),U.allowLeft!==!1&&(V|=4),U.allowRight!==!1&&(V|=8)}}const oe=[];V&1&&oe.push("UP"),V&2&&oe.push("DOWN"),V&4&&oe.push("LEFT"),V&8&&oe.push("RIGHT");const z=oe.length===4?"All directions":oe.join("+");let G="";if(I&128){const P=(ot=A==null?void 0:A.components)==null?void 0:ot.find(Ce=>Ce.definitionId==="comp_animation"||Ce.definitionName==="Animation"),Q=(P==null?void 0:P.defaultValues)||(P==null?void 0:P.values)||{},ee=((nt=c.componentOverrides)==null?void 0:nt.comp_animation)||{},U={...Q,...ee},te=a(U.currentFrameIndex??U.currentFrame??0,0),ie=Math.max(1,a(U.animationSpeed??6,6)),le=e(U.loops,!0),pe=e(U.isPlaying,!0),ge=e(U.animateOnlyWhenMoving,!1),Fe=(pe?1:0)|(le?2:0)|(ge?4:0);G=`
    ...
    ld hl, entity_anim_speed
    add hl, de
    ld (hl), #${ie.toString(16).toUpperCase().padStart(2,"0")}           ; animationSpeed

    ld hl, entity_anim_flags
    add hl, de
    ld (hl), #${Fe.toString(16).toUpperCase().padStart(2,"0")}           ; flags (playing/loop/onlyWhenMoving)
`}let q="",Ie=!1,ne=0,ae=0,ce=0,$=0,Z=0,re=0;const Ke=(rt=A==null?void 0:A.components)==null?void 0:rt.find(P=>P.definitionId==="comp_patrol");if(Ke){Ie=!0;const P=Ke.defaultValues||{},Q=((it=c.componentOverrides)==null?void 0:it.comp_patrol)||{},ee={...P,...Q};ne=Math.max(0,Math.min(255,Number(ee.waypoint1_x)||0)),ae=Math.max(0,Math.min(191,Number(ee.waypoint1_y)||0)),ce=Math.max(0,Math.min(255,Number(ee.waypoint2_x??ne))),$=Math.max(0,Math.min(191,Number(ee.waypoint2_y??ae)));const U=ce-ne,te=$-ae,ie=Math.sqrt(U*U+te*te),le=Number(ee.speed)||1;ie>0&&(Z=Math.round(U/ie*le),re=Math.round(te/ie*le),U!==0&&Z===0&&(Z=U>0?1:-1),te!==0&&re===0&&(re=te>0?1:-1));const pe=Z>=0?Z:256+Z,ge=re>=0?re:256+re;q=`
    ...
    ld hl, entity_vel_y
    add hl, de
    ld (hl), ${ge}           ; VelY = ${re>=0?"+":""}${re}
`}let Ze="";if(I&8){const P=(st=A==null?void 0:A.components)==null?void 0:st.find(ht=>ht.definitionId==="comp_collision"||ht.definitionName==="Collision"),Q=(P==null?void 0:P.defaultValues)||{},ee=((dt=c.componentOverrides)==null?void 0:dt.comp_collision)||{},U={...Q,...ee},te=a(U.hitboxWidth,16),ie=a(U.hitboxHeight,16),le=l(U.offsetX,0),pe=l(U.offsetY,0),ge=le>=128?le-256:le,Fe=pe>=128?pe-256:pe,Ce=a(U.collisionLayer,1),Qt=a(U.collidesWith,255);Ze=`
    ...
    ld hl, entity_collision_hitbox_h
    add hl, de
    ld (hl), #${n(ie)}      ; hitboxHeight

    ld hl, entity_collision_offset_x
    add hl, de
    ld (hl), #${n(le)}      ; offsetX (${ge})

    ld hl, entity_collision_offset_y
    add hl, de
    ld (hl), #${n(pe)}      ; offsetY (${Fe})

    ld hl, entity_collision_layer
    add hl, de
    ld (hl), #${n(Ce)}      ; collisionLayer

    ld hl, entity_collides_with
    add hl, de
    ld (hl), #${n(Qt)}      ; collidesWith
`}let Je="";const ke=(ct=c.componentOverrides)==null?void 0:ct.comp_statemachine,Ue=(pt=A==null?void 0:A.components)==null?void 0:pt.find(P=>P.definitionId==="comp_statemachine"),qe=(ke==null?void 0:ke.stateMachineAssetId)||((_t=Ue==null?void 0:Ue.defaultValues)==null?void 0:_t.stateMachineAssetId);if(qe&&t.stateMachines){const P=t.stateMachines.find(Q=>Q.id===qe);if(P&&P.states&&P.states.length>0){let Q=P.states[0];if(P.initialStateId){const te=P.states.find(ie=>ie.id===P.initialStateId);te&&(Q=te)}const U=`SM_${P.name.replace(/[^a-zA-Z0-9]/g,"_")}_${Q.id.replace(/[^a-zA-Z0-9]/g,"_")}`;Je=`
...
`}}let D="";if(Ie){L&&(E=!0);const P=Math.min(ne,ce),Q=Math.max(ne,ce),ee=Math.min(ae,$),U=Math.max(ae,$),te=ne!==ce,ie=ae!==$,le=ie?`.patrol_check_y_${u}`:`.patrol_end_${u}`;D=`update_${y.toLowerCase()}:
`,D+=`    ; Update ${c.name} - Patrol bounce
`,D+=`    ; Waypoints: (${ne}, ${ae}) -> (${ce}, ${$})
`,D+=`    ld e, ${u}             ; Entity index
`,D+=`    ld d, 0
`,te&&(D+=`
    ; --- X axis bounce ---
...
`,D+=`    ld a, (hl)
`,D+=`    or a
`,D+=`    jp z, .patrol_end_${u}
`,D+=`    bit 7, a
`,D+=`    jp nz, .patrol_chk_min_y_${u}
`,D+=`
    ; Moving down: y >= ${U}?
...
`,D+=`    ret
`;const Wt=o(c),Yt=_[c.entityTemplateId]??0;p+=`init_${y.toLowerCase()}:
    ; Initialize ${c.name} at real position from JSON
    ; JSON position: (${N}, ${C}) tiles = (${W}, ${k}) pixels
    ; Template: ${c.entityTemplateId}
    ; Components: ${K.join(", ")}
    ; Direction mask: #${V.toString(16).toUpperCase().padStart(2,"0")} (${V.toString(2).padStart(4,"0")}b) = ${z}
    ...
    ld hl, entity_screen_id
    add hl, de
    ld (hl), ${Wt}                 ; Screen ID (world node index / fallback screen index)
...
`}
    ...
    ld hl, entity_dir_mask
    add hl, de
    ld (hl), #${V.toString(16).toUpperCase().padStart(2,"0")}            ; Direction restrictions: ${z}
...
; END OF ENTITIES
; ==================================================================
`,p}function Bl(t){const e=!!t.sprites&&t.sprites.length>0;if(!t.screenMaps||t.screenMaps.length===0)return`; ==================================================================
...
`,t.screenMaps.forEach(l=>{var n,o;if(l.layers&&l.layers.background){const s=[];if(t.tiles&&t.tiles.length>0){const g={...qt[1],assignedTiles:{},charsetRangeStart:128,charsetRangeEnd:255,enabled:!0};let S=128;t.tiles.forEach(v=>{if(v&&v.id){const E=Math.ceil(v.width/8),c=Math.ceil(v.height/8);g.assignedTiles[v.id]={charCode:S,assignedAt:Date.now()},S+=E*c}});const T={id:"global_auto_bank",name:"Global Auto Bank",banks:[g,g,g]};s.push(T),console.log(`✅ Created GLOBAL tile bank with ${Object.keys(g.assignedTiles).length} assigned tiles`)}const r=[];l.activeAreaX,l.activeAreaY,l.activeAreaWidth??l.width,l.activeAreaHeight??l.height;const d=32,i=24;for(let p=0;p<i;p++)for(let g=0;g<d;g++){const S=(n=l.layers.background[p])==null?void 0:n[g];if(!S||!S.tileId)r.push(0);else{let T=0;const v=(o=t.tiles)==null?void 0:o.find(c=>c.id===S.tileId),E=s.length>0?s[0].banks:void 0;if(E&&v){let c=!1;for(const u of E)if((u.enabled??!0)&&u.assignedTiles[S.tileId]){const y=u.assignedTiles[S.tileId].charCode,A=Math.ceil(v.width/be),I=S.subTileX||0,L=S.subTileY||0;if(T=y+L*A+I,T>=u.charsetRangeStart&&T<=u.charsetRangeEnd){c=!0;break}else T=0}c||(T=0)}else T=0;r.push(T)}}const h=r.filter(p=>p!==255).length,_=new Set(r);console.log(`📊 Generated ${r.length} bytes: ${h} non-FF (${(h/r.length*100).toFixed(1)}%)`),console.log(`🎯 Unique byte values: [${Array.from(_).sort((p,g)=>p-g).join(", ")}]`);const m=[];m.push('; Generated using exact Screen Editor "Download ASM" logic'),m.push("; Byte values represent actual character codes in VRAM");const f=`${l.name}_${t.screenMaps.indexOf(l)}`,b=pa(f,d,i,r,m,"hex");if(a+=b,l.layers.collision&&t.tiles){const p=l.layers.collision,g=[];p.forEach(T=>{T.forEach(v=>{v.tileId?g.push(1):g.push(0)})});const S=_a(f,l.width,l.height,g,"hex");a+=`
${S}`}}else{const s=t.screenMaps.indexOf(l),r=l.name.toUpperCase().replace(/[^A-Z0-9]/g,"_");a+=`SCREEN_${r}_${s}_LAYOUT:
...
`,t.screenMaps.forEach((l,n)=>{var A,I;const o=l.name.toUpperCase().replace(/[^A-Z0-9]/g,"_"),s=l.backgroundColor!==void 0?l.backgroundColor:1,r=l.borderColor!==void 0?l.borderColor:1,d=l.id?`_${l.id.replace(/[^a-zA-Z0-9]/g,"_").slice(-12)}`:"",i=l.activeAreaX??0,h=l.activeAreaY??0,_=l.activeAreaWidth??l.width??32,m=l.activeAreaHeight??l.height??24,f=Math.max(0,Math.min(31,i)),b=Math.max(0,Math.min(23,h)),p=Math.max(0,Math.min(32-f,_)),g=Math.max(0,Math.min(24-b,m)),T=(f>0||b>0||p<32||g<24)&&p>0&&g>0,v=b*32+f,E=p*g,c=(((I=(A=l.hudConfiguration)==null?void 0:A.importedFrame)==null?void 0:I.cells)||[]).filter(L=>typeof(L==null?void 0:L.x)=="number"&&typeof(L==null?void 0:L.y)=="number"&&typeof(L==null?void 0:L.charCode)=="number"&&L.x>=0&&L.x<32&&L.y>=0&&L.y<24).map(L=>({x:L.x|0,y:L.y|0,charCode:L.charCode&255})),u=c.length>0,y=`hud_imported_frame_${o.toLowerCase()}${d.toLowerCase()}`;u&&(a+=`${y}_data:
    ; Imported HUD frame snapshot for ${l.name} (${c.length} cells)
`,c.forEach(L=>{const w=L.y*32+L.x,N=w&255,C=w>>8&255,O=L.charCode&255;a+=`    DB #${N.toString(16).padStart(2,"0").toUpperCase()},#${C.toString(16).padStart(2,"0").toUpperCase()},#${O.toString(16).padStart(2,"0").toUpperCase()}
`}),a+=`
${y}_draw:
...
`),T?(a+=`load_screen_${o.toLowerCase()}${d.toLowerCase()}:
    ; Load ${l.name} screen (fast direct port access)
    ; Active Area: X=${f}, Y=${b}, W=${p}, H=${g}
...
; END OF SCREENS
; ==================================================================
`,a}function $l(t){var _,m,f,b;const e=(m=(_=t.gameFlow)==null?void 0:_.nodes)==null?void 0:m.some(p=>p.type==="SubMenu"),a=(f=t.screenMaps)==null?void 0:f.some(p=>{var g,S;return((g=p.layers)==null?void 0:g.text)||((S=p.textElements)==null?void 0:S.length)>0}),l=(b=t.screenMaps)==null?void 0:b.some(p=>{var g;return((g=p.hudConfiguration)==null?void 0:g.elements)&&p.hudConfiguration.elements.length>0});if(!e&&!a&&!l)return`; ==================================================================
...
; ==================================================================
`;const n=new Map,o=new Map,s=[{code:32,pattern:[0,0,0,0,0,0,0,0]},{code:43,pattern:[0,16,16,124,16,16,0,0]},{code:45,pattern:[0,0,0,126,0,0,0,0]},{code:62,pattern:[0,48,24,12,24,48,0,0]},{code:124,pattern:[24,24,24,24,24,24,24,24]}];if(s.forEach(p=>{n.set(p.code,p.pattern),o.set(p.code,[240,240,240,240,240,240,240,240])}),t.fonts&&t.fonts.length>0){const p=t.fonts[0],g=p.data.fontData||{},S=p.data.fontColorAttributes||{},T=v=>{if(v.startsWith("rgba(0,0,0,0)"))return 0;const E=v.toUpperCase();return{"RGBA(0,0,0,0)":0,"#000000":1,"#21C842":2,"#5EDC78":3,"#5455ED":4,"#7D76FC":5,"#D4524D":6,"#42EBF5":7,"#FC5554":8,"#FF7978":9,"#D4C154":10,"#E6CE80":11,"#21B03B":12,"#C95BBA":13,"#CCCCCC":14,"#FFFFFF":15}[E]??15};Object.keys(g).forEach(v=>{const E=parseInt(v,10),c=g[E];if(Array.isArray(c)&&c.length===8)if(n.set(E,c),S[E]&&Array.isArray(S[E])){const u=S[E],y=[];for(let A=0;A<8;A++)if(u[A]&&typeof u[A]=="object"){const I=u[A].fg,L=u[A].bg,w=T(I),N=T(L);y.push(w<<4|N)}else y.push(240);o.set(E,y)}else o.set(E,[240,240,240,240,240,240,240,240])})}else{for(let p=48;p<=57;p++)n.set(p,[62,127,115,115,115,127,62,0]);for(let p=65;p<=90;p++)n.set(p,[62,127,99,127,127,99,99,0]);s.forEach(p=>n.set(p.code,p.pattern))}let r=`FONT_PATTERN_DATA:
`,d=`FONT_COLOR_DATA:
`,i=`FONT_CHAR_INDEX:
    DB `;const h=Array.from(n.keys()).filter(p=>p<128).sort((p,g)=>p-g);return h.forEach((p,g)=>{const S=n.get(p),T=o.get(p)||[240,240,240,240,240,240,240,240];r+=`    ; Char ${p} ('${String.fromCharCode(p)}')
`,r+=`    DB ${S.map(v=>"#"+v.toString(16).padStart(2,"0").toUpperCase()).join(", ")}
`,d+=`    ; Char ${p}
`,d+=`    DB ${T.map(v=>"#"+v.toString(16).padStart(2,"0").toUpperCase()).join(", ")}
`,i+=`${p}${g<h.length-1?", ":""}`}),i+=`
...
; END OF FONT DATA
; ==================================================================
`}function jl(t){var n,o,s;const e=[],a=new Map;if(console.log(`🎯 [HUD Generator] Total screens: ${((n=t.screenMaps)==null?void 0:n.length)||0}`),(o=t.screenMaps)==null||o.forEach(r=>{var h;const d=!!r.hudConfiguration,i=((h=r.hudConfiguration)==null?void 0:h.elements)||[];console.log(`  📺 Screen "${r.name}" (${r.id}): hudConfiguration=${d}, elements=${i.length}`),i.length>0&&(i.forEach((_,m)=>console.log(`    📝 Element[${m}]: type=${_.type}, name="${_.name}", text="${_.text||""}" pos=(${_.position.x},${_.position.y}) visible=${_.visible}`)),e.push(...i),a.set(r.id,i))}),console.log(`🎯 [HUD Generator] Total HUD elements found: ${e.length}`),e.length===0)return`; ==================================================================
...
`;return l+=Hl(e),(s=t.screenMaps)==null||s.forEach(r=>{r.activeAreaY}),l+=Vl(),l+=zl(),l+=Gl(e),l}function Hl(t){let e=`; ------------------------------------------------------------------
; HUD DATA STRUCTURES
; ------------------------------------------------------------------
...
`,e+=`hud_element_data:
`,t.forEach((a,l)=>{const n=Wl(a.type),o=a.position.x,s=a.position.y,r=a.visible?1:0,d=`hud_text_${l}`;let i=0,h=1,_=0;const m=a.details||{};(m.border||m.borderColor||m.overallBorderColor)&&(_|=1),a.text?i=a.text.length:m.width?i=Math.ceil(m.width/8):i=10,e+=`    DB ${n}, ${o}, ${s}    ; Element ${l}: ${a.type} at (${o},${s})
`,e+=`    DB ${i}, ${h}, ${_} ; W, H, Flags
`,e+=`    DW ${d}             ; Text pointer
`,e+=`    DB ${r}                ; Visible
`}),e+=`
`,e+=`; HUD Text Strings
...
`}function Gl(t){const e=t.findIndex(o=>o.type===J.Score),a=t.findIndex(o=>o.type===J.Lives),l=e>=0?`hud_text_${e}`:null,n=a>=0?`hud_text_${a}`:null;return`; ------------------------------------------------------------------
; hud_print_string
...
`}function Wl(t){return{[J.Score]:1,[J.HighScore]:2,[J.Lives]:3,[J.EnergyBar]:4,[J.ItemDisplay]:5,[J.SceneName]:6,[J.MiniMap]:7,[J.CoinCounter]:8,[J.BossEnergyBar]:9,[J.PhaseIndicator]:10,[J.AttackAlert]:11,[J.TextBox]:12,[J.NumericField]:13,[J.CustomCounter]:14}[t]||0}function De(t){return t.toLowerCase().replace(/[^a-z0-9]/g,"_")}function Le(t){return t.toUpperCase().replace(/[^A-Z0-9]/g,"_")}function Tt(t){switch(String(t??"").trim().toLowerCase()){case"north":case"up":return"north";case"south":case"down":return"south";case"east":case"right":return"east";case"west":case"left":return"west";default:return null}}function Re(t,e){const a=e==="from"?"fromNodeId":"toNodeId",l=t==null?void 0:t[a];if(typeof l=="string"&&l.length>0)return l;const n=t==null?void 0:t[e];return typeof n=="string"&&n.length>0?n:n&&typeof n.nodeId=="string"&&n.nodeId.length>0?n.nodeId:null}function It(t,e){const a=e==="from"?"fromDirection":"toDirection",l=t==null?void 0:t[a],n=Tt(l);if(n)return n;const o=t==null?void 0:t[e];return Tt(o==null?void 0:o.direction)}function Ve(t,e){var o,s;const a=(o=e.screens)==null?void 0:o.find(r=>r.id===t),l=((s=a==null?void 0:a.name)==null?void 0:s.toUpperCase().replace(/[^A-Z0-9]/g,"_"))||"UNKNOWN",n=t?`_${t.replace(/[^a-zA-Z0-9]/g,"_").slice(-12)}`:"";return`load_screen_${l.toLowerCase()}${n.toLowerCase()}`}function Yl(t,e){var s,r,d,i;const a=(s=e.screens)==null?void 0:s.find(h=>h.id===t),l=(d=(r=a==null?void 0:a.hudConfiguration)==null?void 0:r.importedFrame)==null?void 0:d.cells;if(!Array.isArray(l)||l.length===0)return null;const n=((i=a==null?void 0:a.name)==null?void 0:i.toUpperCase().replace(/[^A-Z0-9]/g,"_"))||"UNKNOWN",o=t?`_${t.replace(/[^a-zA-Z0-9]/g,"_").slice(-12)}`:"";return`hud_imported_frame_${n.toLowerCase()}${o.toLowerCase()}_draw`}function Ql(t,e){const a=Array.isArray(t==null?void 0:t.nodes)?t.nodes:[];if(a.length===0)return null;const l=[],n=t==null?void 0:t.startScreenNodeId,o=a.find(s=>(s==null?void 0:s.id)===n);o&&l.push(o),a.forEach(s=>{(!o||(s==null?void 0:s.id)!==o.id)&&l.push(s)});for(const s of l){const r=s==null?void 0:s.screenAssetId;if(!r)continue;const d=Yl(r,e);if(d)return d}return null}function Xl(t,e,a,l,n){const o=`check_transition_${t}_s${e}_skip_${a}`,s=`check_transition_${t}_s${e}_apply_${a}`;let r="",d="";return a==="east"?(r=`    ; East exit: X near right edge and rightward input
    ld a, (input_state)
    cp STICK_RIGHT
    jr z, .dir_ok_${o}
    cp STICK_UPRIGHT
...
${o}:
`}function Kl(t){var n;const e=t.worldmaps||[],a=!!((n=t.screenMaps)!=null&&n.some(o=>{var s;return Array.isArray((s=o==null?void 0:o.hudConfiguration)==null?void 0:s.elements)&&o.hudConfiguration.elements.length>0}));if(e.length===0)return`; ==================================================================
...
`,e.forEach((o,s)=>{var i;const r=Le(o.name||`world_${s}`),d=o.id||`world_${s}`;if(l+=`; World: ${o.name||"Unnamed"} (${d})
WORLD_${r}_ID EQU ${s}
WORLD_${r}_SCREEN_COUNT EQU ${((i=o.nodes)==null?void 0:i.length)||0}
`,o.nodes&&o.nodes.length>0){const h=new Map;o.nodes.forEach((_,m)=>{const f=Le(_.name||`screen_${m}`),b=h.get(f)||0,p=b===0?f:`${f}_${b+1}`;h.set(f,b+1),l+=`WORLD_${r}_SCREEN_${p}_ID EQU ${m}
`})}l+=`
`}),l+=`; ==================================================================
; WORLD LOADING FUNCTIONS
; ==================================================================

`,e.forEach(o=>{const s=o.id||"unknown",r=o.startScreenNodeId,d=o.nodes||[];if(l+=`; ------------------------------------------------------------------
...
`;return}const i=d.find(b=>b.id===r)||d[0],h=Math.max(0,d.findIndex(b=>b.id===i.id)),_=i.screenAssetId;if(!_){l+=`    ; No valid start screen found
    ret

`;return}const m=Ve(_,t),f=Ql(o,t);l+=`    ; Load start screen: ${i.name||"unknown"} (${_})
    ld a, ((${m} - #4000) / #2000)
    ld hl, ${m}
    call mapper_call_hl_auto
...
`),l+=`    ; Initialize world state
    ld a, WORLD_${Le(o.name||"unnamed")}_ID
...
`,e.forEach(o=>{const s=o.id||"unknown",r=o.nodes||[],d=o.connections||[];if(d.length===0){l+=`; World ${o.name||"Unnamed"} has no screen connections
...
`,d.forEach((i,h)=>{const _=Re(i,"from"),m=Re(i,"to");if(!_||!m){l+=`; Invalid connection ${h}: missing endpoint IDs

`;return}const f=r.find(T=>T.id===_),b=r.find(T=>T.id===m);if(!f||!b){l+=`; Invalid connection ${h}: missing nodes

`;return}const p=b.screenAssetId,g=r.findIndex(T=>T.id===b.id),S=Ve(p,t);l+=`; Transition: ${f.name||"screen"} -> ${b.name||"screen"}
transition_${De(s)}_${h}:
    ld a, ((${S} - #4000) / #2000)
    ld hl, ${S}
    call mapper_call_hl_auto
...
`,e.forEach((o,s)=>{const r=Le(o.name||`world_${s}`),d=o.id||`world_${s}`,i=`check_transition_world_${De(d)}`;l+=`    cp WORLD_${r}_ID
    jp z, ${i}
`}),l+=`    ret

`,e.forEach((o,s)=>{const r=o.id||`world_${s}`,d=De(r),i=o.nodes||[],h=o.connections||[];if(l+=`check_transition_world_${d}:
`,i.length===0||h.length===0){l+=`    ret

`;return}const _=new Map;i.forEach((f,b)=>_.set(f.id,b));const m=new Map;i.forEach((f,b)=>m.set(b,{})),h.forEach(f=>{const b=Re(f,"from"),p=Re(f,"to"),g=It(f,"from"),S=It(f,"to");if(!b||!p)return;const T=_.get(b),v=_.get(p);if(!(T===void 0||v===void 0)){if(g){const E=m.get(T);E&&E[g]===void 0&&(E[g]=v)}if(S){const E=m.get(v);E&&E[S]===void 0&&(E[S]=T)}}}),l+=`    ld a, (current_screen_index)
`,i.forEach((f,b)=>{const p=`check_transition_${d}_screen_${b}`;l+=`    cp ${b}
    jp z, ${p}
`}),l+=`    ret

`,i.forEach((f,b)=>{const p=m.get(b)||{},g=`check_transition_${d}_screen_${b}`;l+=`${g}:
`;const S=["east","west","south","north"];let T=!1;S.forEach(v=>{const E=p[v];if(E===void 0)return;const c=i[E];if(!(c!=null&&c.screenAssetId))return;const u=Ve(c.screenAssetId,t);l+=Xl(d,b,v,E,u),T=!0}),l+=`    ret
...
; END OF WORLDS
; ==================================================================
`,l}function Ct(t){t=t.replace("#","");const e=parseInt(t.substring(0,2),16),a=parseInt(t.substring(2,4),16),l=parseInt(t.substring(4,6),16);if(e<50&&a<50&&l<50)return 1;if(e>200&&a>200&&l>200)return 15;if(e>200&&a<100&&l<100)return 8;if(e<100&&a>200&&l<100)return 3;if(e<100&&a<100&&l>200)return 5;if(e>200&&a>200&&l<100)return 10;if(e>150&&a<100&&l>150)return 13;if(e<100&&a>150&&l>150)return 7;const n=(e+a+l)/3;return n<64?1:n<128?14:15}function Zl(t){const e=t.gameFlow&&t.gameFlow.nodes&&t.gameFlow.nodes.some(l=>l.type==="SubMenu");if(!e)return`; ==================================================================
...
`,t.gameFlow.nodes.filter(o=>o.type==="SubMenu").forEach((o,s)=>{const r=(o.title||o.id).toUpperCase().replace(/[^A-Z0-9]/g,"_");a+=`MENU_${r}_ID EQU ${s}
`}),a+=`
; ==================================================================
; MENU FUNCTIONS
; ==================================================================

`,t.gameFlow.nodes.filter(o=>o.type==="SubMenu").forEach(o=>{var _,m,f,b;(o.title||o.id).toUpperCase().replace(/[^A-Z0-9]/g,"_");const s=o.id.replace(/[^a-zA-Z0-9]/g,"_"),r=((m=(_=o.appearance)==null?void 0:_.colors)==null?void 0:m.background)||"#000000",d=((b=(f=o.appearance)==null?void 0:f.colors)==null?void 0:b.border)||"#FFFFFF",i=Ct(r),h=Ct(d);a+=`show_menu_${s}:
    ; Display ${o.title||o.id} menu
    ; Set background color using VDP
    ld b, ${i*16+h} ; Background (high) | Border (low)
    ld c, 7                     ; VDP Register 7
    call FAST_WRTVDP
...
; END OF MENUS
; ==================================================================
`,a}const vt={[R.NONE]:0,[R.SET_POSITION]:1,[R.MOVE_BY]:2,[R.SET_VELOCITY]:3,[R.APPLY_FORCE]:4,[R.CHANGE_SPRITE]:5,[R.PLAY_ANIMATION]:6,[R.SET_ANIMATION_SPEED]:7,[R.TOGGLE_ANIMATION]:8,[R.PLAY_SOUND]:9,[R.PLAY_MUSIC]:10,[R.MUTE_MUSIC]:11,[R.STOP_MUSIC]:12,[R.SET_VARIABLE]:13,[R.INCREMENT_VARIABLE]:14,[R.DECREMENT_VARIABLE]:15,[R.SET_COMPONENT_PROPERTY]:16,[R.WAIT]:17,[R.GOTO_STATE]:18,[R.DESTROY_ENTITY]:19,[R.SPAWN_ENTITY]:20,[R.GET_RANDOM_ENTITY_POSITION]:21,[R.CHANGE_GAME_FLOW_NODE]:22,[R.DECREASE_LIVES]:23,[R.INCREASE_LIVES]:24,[R.RESPAWN_PLAYER]:25,[R.BREAK_TILE]:26,[R.REPLACE_TILE]:27,[R.RND]:28,[R.POINT_AT]:29,[R.ADD_VARIABLES]:30,[R.SUBTRACT_VARIABLES]:31,[R.MULTIPLY_VARIABLES]:32,[R.DIVIDE_VARIABLES]:33,[R.MODULO_VARIABLES]:34,[R.ASSIGN_VARIABLE]:35,[R.DISABLE_INPUT]:36,[R.ENABLE_INPUT]:37,END:255},Jl={[F.AND]:1,[F.OR]:2,[F.NOT]:3,[F.KEY_PRESSED]:4,[F.KEY_RELEASED]:5,[F.TIME_OUT]:6,[F.CAN_MOVE_DIRECTION]:7,[F.HAS_COLLISION]:8,[F.PATH_CLEAR]:9,[F.ON_WALL_COLLISION]:10,[F.HAS_DEADLY_TILE_COLLISION]:11,[F.ANIMATION_COMPLETE]:12,[F.KEY_AND_MOVEMENT]:13,[F.VARIABLE_COMPARE]:14,[F.XOR]:15},ql={x:0,y:1,vx:2,vy:3,isOnGround:4,health:5},Dt={"==":0,"!=":1,">":2,"<":3,">=":4,"<=":5},Lt={up:1,arrowup:1,down:5,arrowdown:5,left:7,arrowleft:7,right:3,arrowright:3,fire:9,space:9},ze={up:1,down:5,left:7,right:3},Rt={any:0,up:1,down:5,left:7,right:3},wt={any:0,wall:1,enemy:2,item:3,entity:4},Nt={up:0,down:1,left:2,right:3,"up-right":4,"up-left":5,"down-right":6,"down-left":7},eo={comp_pos:1,position:1,comp_physics:2,physics:2,comp_render:3,render:3,comp_animation:4,animation:4,comp_health:5,health:5},to={x:1,y:2,vx:3,velocityx:3,vy:4,velocityy:4,sprite:5,spriteassetid:5,isvisible:6,frame:7,currentframeindex:7,animationspeed:8,speed:8,isplaying:9,current:10,max:11};function ao(t){const e={...ql};return t&&t.length>0&&t.forEach((a,l)=>{const n=6+l;e[a.name]=n,a.asmName&&(e[a.asmName]=n)}),e}function lo(t){const e={};if(!t||t.length===0)return e;let a=128;return t.forEach(l=>{if(!l||!l.id)return;e[l.id]=a,l.name&&(e[String(l.name)]=a,e[String(l.name).toLowerCase()]=a);const n=Math.max(1,Math.ceil((Number(l.width)||8)/8)),o=Math.max(1,Math.ceil((Number(l.height)||8)/8));a+=n*o}),e}function oo(t){if(typeof t=="string"){const e=t.toLowerCase(),a=eo[e];if(a!==void 0)return a}return parseInt(M(t),10)||0}function no(t){if(typeof t=="string"){const e=t.toLowerCase(),a=to[e];if(a!==void 0)return a}return parseInt(M(t),10)||0}function ro(t,e){if(typeof t=="string"&&e){if(e[t]!==void 0)return e[t];const l=t.toLowerCase();if(e[l]!==void 0)return e[l]}const a=parseInt(M(t),10);return Number.isNaN(a)?0:a}function Vt(t){const e={};if(!t||t.length===0)return e;let a=1;return t.forEach(l=>{!l||!l.id||e[l.id]===void 0&&(e[l.id]=a,l.name&&(e[String(l.name)]=a,e[String(l.name).toLowerCase()]=a),a<255&&(a+=1))}),e}function io(t,e,a){const l=a||Vt(t);let n=0;Object.values(l).forEach(h=>{h>n&&(n=h)});const o=new Array(n+1).fill(0),s=new Array(n+1).fill(6),r=new Array(n+1).fill(1),d=new Array(n+1).fill(1),i=(h,_)=>{const m=Number(h);return Number.isFinite(m)?Math.max(0,Math.min(255,m|0)):_};return t==null||t.forEach(h=>{if(!(h!=null&&h.id))return;const _=l[h.id];if(!_)return;const m=Array.isArray(h.components)?h.components:[],f=m.find(u=>(u==null?void 0:u.definitionId)==="comp_render"),b=(f==null?void 0:f.defaultValues)||{},p=b.spriteAssetId??b.sprite??b.spriteId;if(typeof p=="string"&&e){const u=e[p],y=e[p.toLowerCase()];u!==void 0?o[_]=u&255:y!==void 0&&(o[_]=y&255)}const g=m.find(u=>(u==null?void 0:u.definitionId)==="comp_animation"),S=(g==null?void 0:g.defaultValues)||{};s[_]=i(S.animationSpeed??S.speed??6,6);const T=m.find(u=>(u==null?void 0:u.definitionId)==="comp_health"),v=(T==null?void 0:T.defaultValues)||{},E=i(v.current??1,1),c=i(v.max??E,E);r[_]=E,d[_]=c>=E?c:E}),{maxToken:n,spriteByToken:o,animSpeedByToken:s,healthCurByToken:r,healthMaxByToken:d}}const so=`
    ; ------------------------------------------------------------------
    ; SM_Update
...
.return_false:
    ld a, 0
    ret
    `;function po(t,e,a,l,n){let o=so+`
`+co+`

`;const s=Qe(a||[]),r=s.nameToIndex;s.warnings.forEach(f=>{console.warn(`[State Machine Generator] ${f}`)}),o+=`; ==================================================================
`,o+=`; GLOBAL VARIABLES TABLE
`,o+=`; ==================================================================
`,e&&e.length>0?(o+=`; Maps variable IDs (6+) to their RAM addresses
`,o+=`SM_GlobalVarTable:
`,e.forEach((f,b)=>{const p=6+b;o+=`    DW ${f.asmName}            ; ID ${p}: ${f.name}
`}),o+=`
...
`;const d=ao(e),i=lo(l),h=Vt(n),_=io(n,r,h),m=(f,b)=>{const p=b.map(g=>Math.max(0,Math.min(255,g|0)));return`${f}:
    DB ${p.join(", ")}
`};o+=`; ==================================================================
`,o+=`; TEMPLATE PROFILE TABLES
`,o+=`; ==================================================================
`,o+=`SM_TemplateProfileCount EQU ${_.maxToken}
`,o+=m("SM_TemplateSpriteTable",_.spriteByToken),o+=m("SM_TemplateAnimSpeedTable",_.animSpeedByToken),o+=m("SM_TemplateHealthCurrentTable",_.healthCurByToken),o+=m("SM_TemplateHealthMaxTable",_.healthMaxByToken),o+=`
`,o+=`; ==================================================================
`,o+=`; STATE MACHINE SPRITE RUNTIME TABLES
`,o+=`; ==================================================================
`,o+=`SM_SpriteAssetCount EQU ${s.sprites.length}
`,o+=`SM_SpritePatternPtrTable:
`,s.sprites.length>0?s.sprites.forEach((f,b)=>{o+=`    DW SPRITE_${b}_PATTERN
`}):o+=`    ; Empty table (no sprites)
`,o+=`SM_SpritePatternBankTable:
`,s.sprites.length>0?s.sprites.forEach((f,b)=>{o+=`    DB SPRITE_${b}_PATTERN_BANK
`}):o+=`    ; Empty table (no sprites)
`,o+=`
`;for(const f of t)o+=_o(f,d,r,i,h);return o}function _o(t,e,a,l,n){let o=`; State Machine: ${t.name} (${t.id}) 
`;const s=t.name.replace(/[^a-zA-Z0-9]/g,"_"),r=d=>{if(!d)return!1;const i=d.trim().toLowerCase();return i==="any"||i==="__any_state__"||i==="any state (*)"};for(const d of t.states){const i=`SM_${s}_${d.id.replace(/[^a-zA-Z0-9]/g,"_")}`,h=`${i}_OnEnter`,_=`${i}_OnExit`,m=`${i}_Transitions`;o+=`${i}: 
`,o+=`    DB 0; ID(unused) 
`,o+=`    DW ${d.onEnter&&d.onEnter.length>0?h:0} 
`,o+=`    DW ${d.onExit&&d.onExit.length>0?_:0} 
`;const f=t.transitions.filter(b=>b.fromStateId===d.id?!0:r(b.fromStateId)?b.toStateId!==d.id:!1);if(o+=`    DW ${f.length>0?m:0} 
`,d.onEnter&&d.onEnter.length>0){o+=`${h}: 
`;for(const b of d.onEnter)o+=Ge(b,t.name,e,a,l,n);o+=`    DB 0xFF; END
`}if(d.onExit&&d.onExit.length>0){o+=`${_}: 
`;for(const b of d.onExit)o+=Ge(b,t.name,e,a,l,n);o+=`    DB 0xFF; END
`}f.length>0&&(o+=`${m}: 
`,o+=`    DB ${f.length}; Count
`,f.forEach((b,p)=>{const S=r(b.fromStateId)&&r(b.toStateId)?"0":`SM_${s}_${b.toStateId.replace(/[^a-zA-Z0-9]/g,"_")}`,T=b.actions&&b.actions.length>0?`${m}_Actions_${p}`:"0";if(b.conditions?o+=Ye(b.conditions,e):o+=`    DB 0; Empty Condition(Always True) 
`,o+=`    DW ${S} 
`,o+=`    DW ${T} 
`,T!=="0"){o+=`${T}: 
`;for(const v of b.actions||[])o+=Ge(v,t.name,e,a,l,n);o+=`    DB 0xFF; END
`}})),o+=`
`}return o}function M(t){if(typeof t=="number")return t.toString();if(typeof t=="boolean")return t?"1":"0";if(typeof t=="string"){if(t==="true")return"1";if(t==="false")return"0";const e=parseInt(t,10);return isNaN(e)?"0":e.toString()}return"0"}function Ge(t,e="",a,l,n,o){var d;const s=vt[t.type];if(s===void 0)return`; Unknown Action: ${t.type} 
`;let r=`    DB ${s}; ${t.type} 
`;switch(t.type){case R.NONE:break;case R.SET_POSITION:case R.MOVE_BY:case R.SET_VELOCITY:case R.APPLY_FORCE:r+=`    DB ${M(t.params.x)}, ${M(t.params.y)} 
`;break;case R.CHANGE_SPRITE:{const i=t.params.sprite||t.params.spriteId||"";let h=0;if(l&&typeof i=="string"){const _=l[i],m=l[i.toLowerCase()];_!==void 0?h=_:m!==void 0?h=m:h=M(i)==="0"?0:parseInt(M(i),10)||0}else h=M(i)==="0"?0:parseInt(M(i),10)||0;r+=`    DB ${h}; sprite: ${i} 
`;break}case R.PLAY_ANIMATION:r+=`    DB ${M(t.params.animationName)} 
`;break;case R.SET_ANIMATION_SPEED:r+=`    DB ${M(t.params.speed)} 
`;break;case R.TOGGLE_ANIMATION:r+=`    DB ${M(t.params.playing)} 
`;break;case R.PLAY_SOUND:{const i=t.params.soundId??t.params.sound??t.params.soundAssetId??0;r+=`    DB ${M(i)} 
`;break}case R.PLAY_MUSIC:{const i=t.params.trackId??t.params.musicId??t.params.music??0;r+=`    DB ${M(i)} 
`;break}case R.SET_VARIABLE:case R.INCREMENT_VARIABLE:case R.DECREMENT_VARIABLE:{const i=t.params.variable||t.params.variableName||t.params.name,h=(a==null?void 0:a[i])??0,_=t.params.value??t.params.amount??0;r+=`    DB ${h}, ${M(_)}        ; ${i} (ID ${h})
`;break}case R.WAIT:r+=`    DB ${M(t.params.duration)} 
`;break;case R.GOTO_STATE:if(e&&t.params.stateId){const i=`SM_${e.replace(/[^a-zA-Z0-9]/g,"_")}_${t.params.stateId.replace(/[^a-zA-Z0-9]/g,"_")} `;r+=`    DW ${i} 
`}else r+=`    DW 0; Invalid GOTO target
`;break;case R.SPAWN_ENTITY:{const i=t.params.templateId??t.params.entityTemplateId??t.params.entityId??0,h=typeof i=="string"?(o==null?void 0:o[i])??(o==null?void 0:o[i.toLowerCase()])??0:parseInt(M(i),10)||0,_=t.params.x??0,m=t.params.y??0;r+=`    DB ${h}, ${M(_)}, ${M(m)}        ; template=${i}=>${h}
`;break}case R.DESTROY_ENTITY:{const i=((d=t.params)==null?void 0:d.target)||"self";r+=`    DB ${i==="other"?1:0}          ; Target: ${i}
`;break}case R.GET_RANDOM_ENTITY_POSITION:{const i=t.params.templateId??t.params.entityTemplateId??0,h=typeof i=="string"?(o==null?void 0:o[i])??(o==null?void 0:o[i.toLowerCase()])??0:parseInt(M(i),10)||0,_=t.params.targetVariableX??t.params.variableX,m=t.params.targetVariableY??t.params.variableY,f=(a==null?void 0:a[_])??0,b=(a==null?void 0:a[m])??0;r+=`    DB ${h}, ${f}, ${b}        ; template=${i}, x->${_}(${f}), y->${m}(${b})
`;break}case R.SET_COMPONENT_PROPERTY:{const i=t.params.componentId??t.params.component??t.params.compId??0,h=t.params.propertyName??t.params.prop??t.params.name??0,_=t.params.value??0,m=oo(i),f=no(h);let b=M(_);if(f===5&&typeof _=="string"&&l){const p=l[_],g=l[_.toLowerCase()];p!==void 0?b=String(p):g!==void 0&&(b=String(g))}r+=`    DB ${m}, ${f}, ${b}        ; comp=${i}=>${m}, prop=${h}=>${f}, value=${_}
`;break}case R.CHANGE_GAME_FLOW_NODE:{const i=t.params.nodeId??t.params.targetNodeId??0,h=typeof i=="string"&&i.toUpperCase()==="START"?255:M(i);r+=`    DB ${h}        ; node=${i}
`;break}case R.BREAK_TILE:{const i=String(t.params.direction||"up").toLowerCase(),h=Nt[i]??0;r+=`    DB 0, ${h}        ; BREAK_TILE dir=${i}
`;break}case R.REPLACE_TILE:{const i=String(t.params.direction||"up").toLowerCase(),h=Nt[i]??0,_=t.params.replacementTileId??t.params.tileId??0,m=ro(_,n);r+=`    DB ${m}, ${h}        ; REPLACE_TILE tile=${_}=>${m}, dir=${i}
`;break}case R.RND:{const i=t.params.variable??t.params.variableName??t.params.targetVariable??t.params.name,h=(a==null?void 0:a[i])??M(t.params.varId??0),_=M(t.params.dataType??t.params.type??0);r+=`    DB ${h}, ${_}        ; RND var=${i??t.params.varId??0}, type=${t.params.dataType??t.params.type??0}
`;break}case R.POINT_AT:{const i=M(t.params.x1??0),h=M(t.params.y1??0),_=M(t.params.x2??0),m=M(t.params.y2??0),f=M(t.params.speed??1);r+=`    DB ${i}, ${h}, ${_}, ${m}, ${f}
`;break}case R.DECREASE_LIVES:case R.INCREASE_LIVES:{const i=t.params.amount??1;r+=`    DB ${M(i)} 
`;break}case R.RESPAWN_PLAYER:{const i=t.params.x??255,h=t.params.y??255;r+=`    DB ${M(i)}, ${M(h)} 
`;break}case R.ADD_VARIABLES:case R.SUBTRACT_VARIABLES:case R.MULTIPLY_VARIABLES:case R.DIVIDE_VARIABLES:case R.MODULO_VARIABLES:{const i=t.params.destination||t.params.dest||t.params.result,h=t.params.source1||t.params.src1||t.params.operand1,_=t.params.source2||t.params.src2||t.params.operand2,m=(a==null?void 0:a[i])??0,f=(a==null?void 0:a[h])??0,b=(a==null?void 0:a[_])??0,p=t.type===R.ADD_VARIABLES?"ADD":t.type===R.SUBTRACT_VARIABLES?"SUB":t.type===R.MULTIPLY_VARIABLES?"MUL":t.type===R.DIVIDE_VARIABLES?"DIV":"MOD";r+=`    DB ${m}, ${f}, ${b}        ; ${i} = ${h} ${p} ${_}
`;break}case R.ASSIGN_VARIABLE:{const i=t.params.targetVariable||t.params.destination||t.params.dest||t.params.result,h=(a==null?void 0:a[i])??0;if((t.params.sourceType||(t.params.sourceVariable?"variable":"constant"))!=="variable"){const b=t.params.sourceValue??t.params.value??0;r=`    DB ${vt[R.SET_VARIABLE]}; ${R.SET_VARIABLE} (from ${R.ASSIGN_VARIABLE})
`,r+=`    DB ${h}, ${M(b)}        ; ${i} = ${b}
`;break}const m=t.params.sourceVariable||t.params.source||t.params.src||t.params.operand||t.params.source1,f=(a==null?void 0:a[m])??0;r+=`    DB ${h}, ${f}        ; ${i} = ${m}
`;break}default:r+=`    ; Params not implemented for ${t.type}
`;break}return r}function Ye(t,e){var n,o,s,r,d,i,h,_,m,f,b,p,g,S,T,v,E;const a=Jl[t.type];if(!a)return console.warn(`[State Machine Generator] Unknown condition "${t.type}". Falling back to NOP condition.`),`    DB 0; FALLBACK NOP for unknown condition ${t.type}
`;let l=`    DB ${a}; ${t.type} 
`;switch(t.type){case F.KEY_PRESSED:case F.KEY_RELEASED:{const c=(o=(n=t.params)==null?void 0:n.key)==null?void 0:o.toLowerCase(),u=Lt[c]??0;l+=`    DB ${u}          ; Key: ${c||"unknown"}
`;break}case F.TIME_OUT:l+=`    DB ${M((s=t.params)==null?void 0:s.duration)} 
`;break;case F.CAN_MOVE_DIRECTION:{const c=String(((r=t.params)==null?void 0:r.direction)||"").toLowerCase(),u=ze[c]??0;c&&u===0&&console.warn(`[State Machine Generator] Unknown direction "${c}" in CAN_MOVE_DIRECTION. Using 0 (no direction).`),l+=`    DB ${u}          ; Direction: ${c||"none"}
`;break}case F.ON_WALL_COLLISION:{const c=String(((d=t.params)==null?void 0:d.direction)||"any").toLowerCase(),u=Rt[c]??0;c in Rt||console.warn(`[State Machine Generator] Unknown direction "${c}" in ON_WALL_COLLISION. Using any.`),l+=`    DB ${u}          ; Wall direction: ${c}
`;break}case F.HAS_COLLISION:{const c=String(((i=t.params)==null?void 0:i.collisionType)||"any").toLowerCase();let u=wt[c];u===void 0&&(console.warn(`[State Machine Generator] Unknown collisionType "${c}" in HAS_COLLISION. Using any.`),u=wt.any),l+=`    DB ${u}          ; collisionType: ${c}
`;break}case F.PATH_CLEAR:{const c=String(((h=t.params)==null?void 0:h.direction)||"").toLowerCase(),u=ze[c]??0;c&&u===0&&console.warn(`[State Machine Generator] Unknown direction "${c}" in PATH_CLEAR. Using auto-deduce (0).`),l+=`    DB ${u}          ; Direction (0=auto): ${c||"auto"}
`;break}case F.ANIMATION_COMPLETE:break;case F.KEY_AND_MOVEMENT:{const c=String(((_=t.params)==null?void 0:_.key)||"").toLowerCase(),u=Lt[c]??0,y=String(((m=t.params)==null?void 0:m.direction)||"").toLowerCase();let A=ze[y]??0;!y&&u!==9&&(A=u),y&&A===0&&console.warn(`[State Machine Generator] Unknown direction "${y}" in KEY_AND_MOVEMENT. Using 0.`),l+=`    DB ${u}, ${A}          ; key=${c||"unknown"}, dir=${y||"auto"}
`;break}case F.AND:case F.OR:case F.XOR:if(t.conditions){l+=`    DB ${t.conditions.length} 
`;for(const c of t.conditions)l+=Ye(c,e)}else l+=`    DB 0
`;break;case F.NOT:t.conditions&&t.conditions.length>0?(l+=`    DB 1 
`,l+=Ye(t.conditions[0],e)):(l+=`    DB 1 
`,l+=`    DB 0; Fallback NOP subcondition for NOT
`);break;case F.VARIABLE_COMPARE:{const c=((f=t.params)==null?void 0:f.variable)||"x",u=e==null?void 0:e[c];if(u===void 0)console.warn(`[State Machine Generator] Unknown variable "${c}" in VARIABLE_COMPARE. Using x (ID 0) as fallback.`),l+=`    DB 0, ${Dt[((b=t.params)==null?void 0:b.operator)||"=="]||0}, ${M(((p=t.params)==null?void 0:p.value)||0)}; FALLBACK: unknown var "${c}" -> x ${((g=t.params)==null?void 0:g.operator)||"=="} ${((S=t.params)==null?void 0:S.value)||0}
`;else{const y=Dt[((T=t.params)==null?void 0:T.operator)||"=="]||0,A=((v=t.params)==null?void 0:v.value)||0;l+=`    DB ${u}, ${y}, ${M(A)}; ${c} (ID ${u}) ${((E=t.params)==null?void 0:E.operator)||"=="} ${A}
`}break}}return l}function ho(t,e={}){console.log("ÐYZî [INTERRUPT GENERATOR] Generating interrupt.asm...");let a="";return a+=`; ==================================================================
`,a+=`; INTERRUPT TASK SYSTEM - File: interrupt.asm
`,a+=`; Konami-style technique: Hook H.TIMI for 50/60Hz task execution
`,a+=`; ==================================================================

`,a+=uo(),a+=mo(),a+=fo(),a+=bo(),a+=yo(),a+=Eo(t),e.interruptDrivenComponents&&(a+=`
; ==================================================================
...
`),console.log(`ƒo. [INTERRUPT GENERATOR] Generated interrupt.asm (${a.length} chars)`),a}function uo(){return`; ==================================================================
...
`,t.hasEntities){const l=Oe(t).usedComponents,n=l.has("Jump"),o=l.has("Movement")||l.has("Cursors"),s=l.has("Gravity");n||o||s?(e+=`; ==================================================================
...
; ==================================================================
`}function So(t){var l,n,o,s;const e=((n=(l=t.tiles)==null?void 0:l[0])==null?void 0:n.width)||8,a=((s=(o=t.tiles)==null?void 0:o[0])==null?void 0:s.height)||8;return`; ==================================================================
; SCROLL SYSTEM
; File: scroll.asm
...
; ==================================================================
`}const Ao="SCREEN 2 (Graphics I)";function _e(t,e,a=0,l=255){return Number.isFinite(t)?t<a?a:t>l?l:Math.floor(t):e}function We(t){return String(t||"").trim().toLowerCase()}function To(t){const e=String(t||"").trim();if(!e)return null;const a=e.match(/^(.*?)(?:[_\-\s](?:f|frame))(\d+)$/i);if(!a)return null;const l=String(a[1]||"").trim(),n=parseInt(a[2],10);return!l||Number.isNaN(n)?null:{groupId:l,frameOrder:n}}const zt={rotate_left:1,rotate_right:2,shift_left:3,shift_right:4,shift_up:5,shift_down:6,swap_top_bottom:7};function Io(t){return String(t||"").trim().toLowerCase()==="transform"?"transform":"frames"}function Gt(t){const e=String(t||"").trim().toLowerCase();return e&&Object.prototype.hasOwnProperty.call(zt,e)?e:null}function Co(t){const e=(t==null?void 0:t.animation)??(t==null?void 0:t.animatedTile)??(t==null?void 0:t.tileAnimation)??null;if((typeof(e==null?void 0:e.enabled)=="boolean"?e.enabled:typeof(t==null?void 0:t.isAnimated)=="boolean"?t.isAnimated:void 0)===!1||e===!1)return{enabled:!1,mode:"frames",groupId:null,frameOrder:null,speed:null,baseTileId:null,transformEffect:null,transformIncludeColors:!0};const l=(t==null?void 0:t.animationGroup)??(e==null?void 0:e.groupId)??(e==null?void 0:e.group)??(e==null?void 0:e.name)??(e==null?void 0:e.id)??null,n=typeof l=="string"&&l.trim()?l.trim():null,o=(t==null?void 0:t.animationFrameIndex)??(t==null?void 0:t.frameIndex)??(e==null?void 0:e.frameIndex)??(e==null?void 0:e.frame)??null,s=Number.isFinite(Number(o))?_e(Number(o),0):null,r=(t==null?void 0:t.animationSpeed)??(t==null?void 0:t.animationSpeedFrames)??(e==null?void 0:e.speed)??(e==null?void 0:e.speedFrames)??(e==null?void 0:e.ticksPerFrame)??null,d=Number.isFinite(Number(r))?_e(Number(r),8,1,255):null,i=(t==null?void 0:t.animationBaseTileId)??(e==null?void 0:e.baseTileId)??(e==null?void 0:e.targetTileId)??null,h=typeof i=="string"&&i.trim()?i.trim():null,_=(e==null?void 0:e.transform)??null,m=Gt((t==null?void 0:t.animationTransformEffect)??(e==null?void 0:e.transformEffect)??(_==null?void 0:_.effect)??(e==null?void 0:e.effect)),f=(t==null?void 0:t.animationMode)??(e==null?void 0:e.mode)??(e==null?void 0:e.animationMode)??(m?"transform":null),b=Io(f),p=m||(b==="transform"?"rotate_left":null),g=(t==null?void 0:t.animationTransformIncludeColors)??(e==null?void 0:e.animationTransformIncludeColors)??(_==null?void 0:_.includeColors);return{enabled:!0,mode:b,groupId:n,frameOrder:s,speed:d,baseTileId:h,transformEffect:p,transformIncludeColors:typeof g=="boolean"?g:!0}}function vo(t){const e=new Map,a=Array.isArray(t.tiles)?t.tiles:[];let l=128;return a.forEach((n,o)=>{if(!(n!=null&&n.id))return;const s=Math.max(1,Math.ceil((n.width||8)/8)),r=Math.max(1,Math.ceil((n.height||8)/8)),d=s*r;e.set(n.id,{charCode:l,charsPerTile:d,tileIndex:o}),l+=d}),e}function Do(t){return`#${_e(t,0).toString(16).toUpperCase().padStart(2,"0")}`}function xt(t,e){return String(t||"").toLowerCase().replace(/[^a-z0-9]+/g,"_").replace(/^_+|_+$/g,"")||e}function Lo(t,e=16){if(!t.length)return"    db #00";const a=[];for(let l=0;l<t.length;l+=e){const n=t.slice(l,l+e).map(Do).join(", ");a.push(`    db ${n}`)}return a.join(`
`)}function Ro(t){var s,r;const e=Array.isArray(t.tiles)?t.tiles:[];if(!e.length)return{frameGroups:[],transformGroups:[]};const a=vo(t),l=new Map,n=[];e.forEach((d,i)=>{const h=String((d==null?void 0:d.id)||"").trim();if(!h)return;const _=a.get(h);if(!_)return;const m=Co(d);if(!m.enabled)return;const f=To((d==null?void 0:d.name)||""),b=(m.groupId||(f==null?void 0:f.groupId)||(d==null?void 0:d.name)||h||"").trim();if(m.mode==="transform"){const v=Gt(m.transformEffect);if(!v)return;const c=(m.baseTileId&&a.has(m.baseTileId)?m.baseTileId:null)||h,u=a.get(c);if(!u)return;const y=u.charCode,A=u.charsPerTile;if(y<0||y+A-1>255)return;const I=zt[v];if(!I)return;const L=m.transformIncludeColors?1:0,w=`anim_transform_${n.length}_${xt(b,`t${n.length}`)}`;n.push({label:w,groupId:b,speed:_e(m.speed??8,8,1,255),targetTileId:c,targetCharCode:y,charsPerTile:A,operationCode:I,flags:L});return}if(!b)return;let p=m.frameOrder;p===null&&f&&(!m.groupId||We(m.groupId)===We(f.groupId))&&(p=f.frameOrder),p===null&&(p=_.tileIndex);const g=_e(m.speed??8,8,1,255),S=We(b),T=l.get(S)||[];T.push({tile:d,tileIndex:i,tileId:h,groupId:b,frameOrder:p,speed:g,baseTileId:m.baseTileId}),l.set(S,T)});const o=[];for(const d of l.values()){if(d.length<2)continue;const i=[...d].sort((c,u)=>c.frameOrder!==u.frameOrder?c.frameOrder-u.frameOrder:c.tileIndex-u.tileIndex),_=((s=i.find(c=>!!c.baseTileId&&a.has(c.baseTileId)))==null?void 0:s.baseTileId)||null||i[0].tileId,m=a.get(_);if(!m)continue;const f=i.filter(c=>{const u=a.get(c.tileId);return!!u&&u.charsPerTile===m.charsPerTile});if(f.length<2)continue;const b=m.charCode,p=m.charsPerTile;if(b<0||b+p-1>255)continue;const g=[],S=p*8;let T=f[0].speed;for(const c of f){const u=Array.from(Ot(c.tile,Ao)||[]);if(u.length!==S)continue;const y=kt(c.tile),A=241,I=y?Array.from(y).slice(0,S):new Array(S).fill(A);for(;I.length<S;)I.push(A);const L=[];for(let w=0;w<p;w++){const N=w*8;L.push(...u.slice(N,N+8)),L.push(...I.slice(N,N+8))}g.push({tileName:String(((r=c.tile)==null?void 0:r.name)||c.tileId),bytes:L}),T=Math.min(T,c.speed)}if(g.length<2)continue;const v=f[0].groupId,E=`anim_group_${o.length}_${xt(v,`g${o.length}`)}`;o.push({label:E,groupId:v,speed:_e(T,8,1,255),targetTileId:_,targetCharCode:b,charsPerTile:p,frameCount:_e(g.length,g.length,2,255),bytesPerFrame:p*16,frames:g})}return{frameGroups:o,transformGroups:n}}function wo(t){var f,b;const{frameGroups:e,transformGroups:a}=Ro(t),l=e.length>0,n=a.length>0,o=_e(((f=e[0])==null?void 0:f.speed)??((b=a[0])==null?void 0:b.speed)??8,8,1,255),s=Math.max(1,e.length+a.length),r=l?e.map(p=>`    db ${p.targetCharCode}, ${p.charsPerTile}, ${p.frameCount}, ${p.speed}, ${p.bytesPerFrame}    ; ${p.groupId} -> tile ${p.targetTileId}
    dw ${p.label}`).join(`
`):"    ; No animated tile groups detected in project data",d=n?a.map(p=>`    db ${p.targetCharCode}, ${p.charsPerTile}, ${p.operationCode}, ${p.flags}    ; ${p.groupId} -> tile ${p.targetTileId}`).join(`
`):"    ; No transform tile groups detected in project data",i=l?e.map(p=>{const g=p.frames.slice(0,p.frameCount).map((S,T)=>`    ; Frame ${T}: ${S.tileName}
${Lo(S.bytes)}`).join(`
`);return`${p.label}:
...
; ==================================================================
`}function No(t){var e,a,l,n;return(a=(e=t.tiles)==null?void 0:e[0])!=null&&a.width,(n=(l=t.tiles)==null?void 0:l[0])!=null&&n.height,`; ==================================================================
...
`}function xo(t,e,a={}){var _;if(console.log("🔧 Generating modular ASM files..."),!t)throw console.error("❌ projectName is required"),new Error("projectName is required");if(!e)throw console.error("❌ assets is undefined or null"),new Error("assets array is required");if(!Array.isArray(e))throw console.error("❌ assets is not an array"),new Error("assets must be an array");console.log(`📊 Project: ${t}, Assets: ${e.length}, Config:`,a);let l;try{l=Xe(t,e),console.log(`🔍 Analysis complete: ${l.sprites.length} sprites, ${l.tiles.length} tiles`)}catch(m){console.error("❌ Error analyzing project:",m),l={hasSprites:!1,hasTiles:!1,hasScreens:!1,hasEntities:!1,hasComponents:!1,hasGameFlow:!1,hasMenus:!1,hasFonts:!1,hasECS:!1,hasMultipleScreens:!1,hasAnimations:!1,hasCollisions:!1,hasMenuSystem:!1,components:[],templates:[],entities:[],sprites:[],tiles:[],screens:[],screenMaps:[],projectName:t,customStates:[],stateMachines:[],globalVariables:[]},console.log("🔄 Using fallback empty analysis")}const n=a.interruptDrivenComponents??!0,o=a.hardwareMode||"hybrid",s=a.optimizeLevel||"safe",r=a.targetFormat||"konami",d=a.romMode||"simple32k",i=a.autoMegaROM??!1;console.log("📝 [MSX GENERATOR] Generating all ASM files..."),console.log(`🔧 Hardware Mode: ${o.toUpperCase()}, Optimize: ${s}`),console.log(`[MSX GENERATOR] ROM config: mode=${d}, mapper=${r}, autoMegaROM=${i}`);const h={"bios.asm":ka({hardwareMode:{mode:o,optimizeLevel:s}}),"constants.asm":Fa(l),"variables.asm":Ba(l),"mapper.asm":al({targetFormat:r,romMode:d,autoMegaROM:i}),"interrupt.asm":ho(l,{interruptDrivenComponents:n}),"header.asm":ja(t,l),"patterns.asm":ll(l),"colors.asm":ol(l),"components.asm":n?`; Components are generated inside interrupt.asm (interruptDrivenComponents=true)
`:Ht(l),"entities.asm":Fl(l),"worlds.asm":Kl(l),"screens.asm":Bl(l),"sprites.asm":ml(l),"font.asm":$l(l),"hud.asm":jl(l),"menus.asm":Zl(l),"sound.asm":go(),"scroll.asm":So(l),"animtiles.asm":wo(l),"particles.asm":No(l),"statemachine.asm":l.stateMachines?po(l.stateMachines,l.globalVariables,l.sprites,l.tiles,l.templates):`; No State Machines
`,"gameflow.asm":Xa(l),"main.asm":el(t,l),"unitedFiles.asm":""};return a.generateUnified&&(h["unitedFiles.asm"]=pl(h,t,l,{romMode:d,targetFormat:r,autoMegaROM:i})),console.log("✅ Modular ASM files generated successfully!"),console.log(`📊 Generated ${Object.keys(h).filter(m=>h[m]).length} files`),console.log("📋 [DEBUG] Files generated:",Object.keys(h)),console.log("🎯 [DEBUG] interrupt.asm length:",((_=h["interrupt.asm"])==null?void 0:_.length)||"MISSING!"),h}const kn=Object.freeze(Object.defineProperty({__proto__:null,generateModularASM:xo},Symbol.toStringTag,{value:"Module"}));export{pn as $,ut as A,be as B,mt as C,Bo as D,$o as E,pa as F,_a as G,J as H,Nn as I,en as J,qo as K,ca as L,Fo as M,mn as N,xe as O,ko as P,Ln as Q,wn as R,Pe as S,Rn as T,Kt as U,rn as V,tn as W,ln as X,an as Y,Wo as Z,dn as _,Yo as a,on as a0,nn as a1,cn as a2,sn as a3,_n as a4,ue as a5,we as a6,Ko as a7,Mn as a8,Ho as a9,jo as aa,$e as ab,Pn as ac,un as ad,ha as ae,F as af,R as ag,Uo as ah,Xe as ai,On as aj,fn as ak,zo as al,qt as am,Xo as an,bn as ao,hn as ap,Vo as aq,yn as ar,kn as as,Go as b,Qo as c,Se as d,gn as e,Zo as f,En as g,Jo as h,Ot as i,kt as j,X as k,Po as l,Oo as m,Ne as n,An as o,Tn as p,In as q,Cn as r,Sn as s,vn as t,Ae as u,Te as v,ra as w,Dn as x,Mo as y,xn as z};
