...
const editorTypeBlock = typesSource.match(/export enum EditorType \{([\s\S]*?)\n\}/)?.[1] || '';
const editorTypeNames = Array.from(editorTypeBlock.matchAll(/\b([A-Za-z0-9_]+)\s*=/g), match => match[1]);
const missingEditorHelp = editorTypeNames.filter(editorTypeName => !propertiesPanelSource.includes(`[EditorType.${editorTypeName}]`));
...
const checks = [
  ...
  ['MSX2 SCREEN 4 Bitmap Room can be added to WorldMap and WorldView', appUiSource.includes("a.type === 'msx2bitmaproom'") && worldMapEditorSource.includes('isMsx2Screen4BitmapRoom') && worldMapEditorSource.includes('[MSX2 SCREEN 4 Bitmap]') && worldMapEditorSource.includes('drawMsx2BitmapRoomPreview') && worldMapEditorSource.includes("command.op === 'lineV'") && worldViewEditorSource.includes('isMsx2Screen4BitmapRoom') && worldViewEditorSource.includes('renderMsx2BitmapRoomToCanvas') && worldViewEditorSource.includes("command.op === 'lineV'")],
  ...
  ['MSX2 SCREEN 4 multibank smoke is exposed in npm scripts', packageScripts['smoke:msx2-screen4-multibank'] === 'python scripts/build_msx2screen4_multibank_loader_smoke.py' && packageScripts['smoke:msx2-static']?.includes('npm run smoke:msx2-screen4-multibank -- --skip-openmsx')],
  ...
  ['movement options include patrol X/Y, paddle/ball, shooter vertical, and snake char modes', source.includes('MSX2_ENTITY_MOVEMENT_OPTIONS') && source.includes("value: 'patrolX'") && source.includes("value: 'patrolY'") && source.includes("value: 'paddleHorizontal'") && source.includes("value: 'shooterVertical'") && source.includes("value: 'ballBounce'") && source.includes("value: 'snakeChar'")],
  ['SCREEN 4 editor exposes shooter 60Hz IRQ budget controls', source.includes('Shooter 60Hz budget') && source.includes('MSX2_SHOOTER_IRQ_PROFILES_60HZ') && source.includes('estimatedCycles') && source.includes('worstCaseCycles')],
  ['SCREEN 4 export contract includes shooter 60Hz budget validation', source.includes('shooter60Hz') && source.includes('validateMsx2Shooter60HzBudget') && readFileSync(join(repoRoot, 'utils', 'msx2ShooterRuntime.ts'), 'utf8').includes('sustained_irq_over_budget')],
  ['Asset Properties summarizes SCREEN 4 shooter IRQ budget', source.includes('Shooter:</strong>') && source.includes('cyc worst') && source.includes('normalizeMsx2ShooterRuntimeConfig')],
  ['SCREEN 4 project slice carries shooter 60Hz IRQ metadata', generatorSource.includes('shooter60HzScreens') && generatorSource.includes('runtime.msx2.shooter60hz.contract') && generatorSource.includes('validateMsx2Shooter60HzBudget(shooter)')],
  ...
  ['SCREEN 4 Pong angle labels are Glass-safe globals', generatorSource.includes('control_2_players_ball_angle_store_up') && generatorSource.includes('control_2_players_ball_angle_store_down') && !generatorSource.includes('jp c, .up\n') && !generatorSource.includes('\n.up:\n    ld a, #FE')],
  ...
  ['Asset Properties panel doubles as contextual editor help', propertiesPanelSource.includes('Properties / Help') && propertiesPanelSource.includes('renderEditorHelp') && propertiesPanelSource.includes('MSX2 Sprite Editor Help') && propertiesPanelSource.includes('MSX2 GameFlow Help') && propertiesPanelSource.includes('MSX2 SCREEN 5 Presentation Help')],
  ['Asset Properties help covers secondary editor surfaces', propertiesPanelSource.includes('Boss Editor Help') && propertiesPanelSource.includes('Portrait Editor Help') && propertiesPanelSource.includes('MSX1 Presentation Screen Help') && propertiesPanelSource.includes('Tile Banks Help') && propertiesPanelSource.includes('PNG to MSX Chars Help') && propertiesPanelSource.includes('Behavior Script Help') && propertiesPanelSource.includes('Editor Help')],
  ['Asset Properties help covers every EditorType enum value', editorTypeNames.length > 0 && missingEditorHelp.length === 0, missingEditorHelp.join(', ')],
  ['Asset Properties panel covers MSX2 asset families', propertiesPanelSource.includes("case 'msx2bitmap'") && propertiesPanelSource.includes("case 'msx2hudfont'") && propertiesPanelSource.includes("case 'msx2presentation'") && propertiesPanelSource.includes("case 'msx2gameflow'") && propertiesPanelSource.includes('activeEditorType === EditorType.Msx2HudFont') && propertiesPanelSource.includes('activeEditorType === EditorType.Msx2Presentation') && propertiesPanelSource.includes('activeEditorType === EditorType.Msx2GameFlow')],
  ...
  ['SCREEN 4 MegaROM keeps HUD font patterns in cold data bank', generatorSource.includes('buildMsx2HudTextRuntimeAsm(analysis, useKonamiDataBank)') && generatorSource.includes('call msx2_screen4_data_bank_enter') && generatorSource.includes("${useKonamiDataBank ? '' : hudFontPatternDataAsm}") && generatorSource.includes('const screen4ColdDataAsm = useKonamiDataBank') && generatorSource.includes('hudFontPatternDataAsm,') && generatorSource.includes('hardwareSpriteDataAsm,') && generatorSource.includes("bankIndex === 0 ? 'MSX2_SCREEN4_DATA_BANK_ROM_START:' : ''")],
  ...
  ['SCREEN 4 MegaROM copies effect templates from cold data banks to RAM', generatorSource.includes('function buildInitEffectBuffersRoutine') && generatorSource.includes('ld a, ${label}_DATA_BANK') && generatorSource.includes('call msx2_screen4_data_bank_enter_selected') && generatorSource.includes('call msx2_screen4_data_bank_leave') && generatorSource.includes('tileScreenEffectBlocks') && generatorSource.includes("useKonamiDataBank ? '' : formatBytes(`${label}_EFFECTS`") && generatorSource.includes('effects layer, copied from cold ROM to RAM on screen reset')],
  ...
  ['GameFlow backgrounds resolve native SCREEN 4 rooms', generatorSource.includes('function getFlowBackgroundScreenAssetId') && generatorSource.includes('node?.appearance?.backgroundScreenAssetId || node?.backgroundScreenAssetId') && (generatorSource.match(/screenLoadLabelForAssetId\(analysis, screenLabels, tileScreenLabels, getFlowBackgroundScreenAssetId\(current\)\)/g) || []).length >= 2 && generatorSource.includes("node.type === 'Restart'")],
  ...
  ['summary extraction keeps SCREEN 4 bitmap room world references', summaryExtractorSource.includes('msx2BitmapRooms') && summaryExtractorSource.includes('node.screenAssetId || node.screenId') && summaryExtractorSource.includes("getAsset(assets, 'msx2bitmaproom', screenId)") && summaryExtractorSource.includes("screenAsset.type === 'msx2bitmaproom'") && summaryExtractorSource.includes('usedAssets.msx2BitmapRooms.push')],
  ...
  ['MSX2 project slice emits world bank manifest artifact', generatorSource.includes('function buildMsx2WorldBankManifest') && generatorSource.includes('worldBankManifest') && generatorSource.includes('msx2_world_bank_manifest.json') && generatorSource.includes("scope: 'msx2_screen4_world_bank_manifest'") && generatorSource.includes("dataWindowAddress = '#8000'")],
  ['MSX2 SCREEN 4 loader can select per-screen cold data banks', generatorSource.includes('function buildMsx2Screen4DataBankPlan') && generatorSource.includes('screen4DataBankPlan') && generatorSource.includes('msx2_screen4_data_bank_enter_selected') && generatorSource.includes('MSX2_SCREEN4_MULTI_BANK_LOADER_READY') && generatorSource.includes('_DATA_BANK EQU MSX2_SCREEN4_DATA_BANK_') && generatorSource.includes('_PHYS_START:') && generatorSource.includes('_USED_END:') && generatorSource.includes('_PHYS_START + #2000')],
  ['MSX2 SCREEN 4 collision and behavior definitions are cold in MegaROM', generatorSource.includes('tileScreenCollisionBlocks') && generatorSource.includes('tileScreenBehaviorBlocks') && generatorSource.includes("useKonamiDataBank ? '' : formatBytes(`${label}_COLLISION`") && generatorSource.includes("useKonamiDataBank ? '' : formatBytes(`${label}_BEHAVIOR`")],
  ['MSX2 SCREEN 4 collision and behavior load through current-screen RAM caches', generatorSource.includes('msx2_collision_runtime_cache') && generatorSource.includes('msx2_behavior_runtime_cache') && generatorSource.includes('ld de, msx2_collision_runtime_cache') && generatorSource.includes('ld de, msx2_behavior_runtime_cache') && generatorSource.includes('ld (msx2_current_collision_ptr), hl') && generatorSource.includes('ld (msx2_current_behavior_ptr), hl')],
  ['MSX2 SCREEN 4 runtime layer policy is emitted and preflight-validated', generatorSource.includes('screen4RuntimeLayerPolicy') && generatorSource.includes("runtimePlacement: useKonamiDataBank ? 'ram_cache' : 'resident_rom'") && buildScriptSource.includes('screen4RuntimeLayerPolicy') && buildScriptSource.includes('runtime.collision_current_cache') && buildScriptSource.includes('runtime.behavior_current_cache')],
  ['MSX2 budget feedback exposes SCREEN 4 runtime layer policy', msx2BudgetFeedbackSource.includes('screen4RuntimeLayerPolicy') && serverSource.includes('screen4RuntimeLayerPolicy: projectSlice.screen4RuntimeLayerPolicy')],
  ['MSX2 logical bank budget auto-splits large splittable world packages', generatorSource.includes("splitStrategy: 'auto_world_package_chunk'") && generatorSource.includes('splitSourcePackages') && generatorSource.includes("status: splitSourcePackages.length > 0 ? 'applied'") && generatorSource.includes('logicalPackagesBySource')],
  ['MSX2 split package chunks expose future loader label contract', generatorSource.includes('splitChunkManifest') && generatorSource.includes('packPayloadPartsIntoBankChunks') && generatorSource.includes('chunkPayloadParts') && generatorSource.includes('payloadLabels') && generatorSource.includes('payloadLabelCount') && generatorSource.includes('payloadBytes') && generatorSource.includes('loaderCoverageStatus') && generatorSource.includes('loaderCoveredPayloadLabels') && generatorSource.includes('loaderUncoveredPayloadLabels') && generatorSource.includes('payloadKind') && generatorSource.includes('dataBankSymbol') && generatorSource.includes('loaderSymbol') && generatorSource.includes('`${dataBankSymbol} EQU MSX2_SCREEN4_DATA_BANK_${bankIndex}`') && generatorSource.includes('`${payloadLabel}_DATA_BANK EQU ${dataBankSymbol') && generatorSource.includes('splitChunkSectionsByBank') && generatorSource.includes('`${dataLabel}:') && generatorSource.includes('${dataEndLabel}:`') && generatorSource.includes('splitChunkLoaderRoutines') && generatorSource.includes('`${loaderSymbol}:') && generatorSource.includes('payloadDataBankConstants') && generatorSource.includes('copyRuntimeLayerFromBank') && generatorSource.includes('chunkDataBankByPayloadLabel') && buildScriptSource.includes('splitChunkManifest entry is missing') && buildScriptSource.includes('payloadLabels must be a non-empty list') && buildScriptSource.includes('payloadLabelCount does not match payloadLabels') && buildScriptSource.includes('payloadBytes exceeds one 8KB bank') && buildScriptSource.includes('loader coverage is incomplete')],
  ...
  ['MSX2 failure reports expose structured resolver candidates', buildScriptSource.includes('build_msx2_preflight_resolver_candidates') && buildScriptSource.includes('"resolverCandidates"') && buildScriptSource.includes('"enable_zx0_preprocess"') && buildScriptSource.includes('"relax_strict_warning_gate"') && buildScriptSource.includes('"emit_multi_bank_world_data_loader"') && buildScriptSource.includes('"move_cold_readonly_data_to_world_bank"')],
  ['MSX2 failure reports expose automatic resolution plans', buildScriptSource.includes('build_msx2_automatic_resolution_plan') && buildScriptSource.includes('"automaticResolutionPlan"') && buildScriptSource.includes('"missingPart"') && serverSource.includes('buildMsx2AutomaticResolutionPlan') && serverSource.includes('automaticResolutionPlan') && msx2BudgetFeedbackSource.includes('buildMsx2AutomaticResolutionPlan') && msx2BudgetFeedbackSource.includes('automaticResolutionPlan') && codeExportModalSource.includes('Automatic plan:') && codeExportModalSource.includes('Blocked detail:')],
  ['MSX2 unresolved auto-resolve records blocked resolver attempts', buildScriptSource.includes('append_msx2_blocked_resolution_attempts') && buildScriptSource.includes('"status": "blocked"') && serverSource.includes('appendMsx2BlockedResolutionAttempts') && serverSource.includes("status: 'blocked'")],
  ...
  ['MSX2 CLI enables safe budget auto-resolve by default for Konami MegaROM', buildScriptSource.includes('--no-auto-resolve-msx2-budget') && buildScriptSource.includes('auto_resolve_msx2_budget = bool') && buildScriptSource.includes('args.rom_mode == "megarom"') && buildScriptSource.includes('args.target_format == "konami"')],
  ['MSX2 CLI compile resolver regenerates resident cold data from project JSON before Post-ASM fallback', buildScriptSource.includes('move_cold_readonly_data_to_world_bank') && buildScriptSource.includes('resident-regenerated.asm') && buildScriptSource.includes('Regenerating ASM from project JSON with the current SCREEN 4 runtime layer policy') && buildScriptSource.includes('run_post_asm_dead_block_optimizer')],
  ['compile endpoint records project-JSON regeneration as blocked for ASM-only requests', serverSource.includes('The /compile endpoint received ASM only') && serverSource.includes('regenerate_from_project_json_with_current_screen4_runtime_layer_policy') && serverSource.includes("status: 'blocked'")],
  ['MSX2 build summary records Post-ASM attempts', buildScriptSource.includes('"postAsmAttempts"') && buildScriptSource.includes('post_asm_attempts.append') && buildScriptSource.includes('"accepted"') && buildScriptSource.includes('"check_only_passed"') && buildScriptSource.includes('"rejected_validation_failed"') && buildScriptSource.includes('"fallback_to_baseline"') && buildScriptSource.includes('"ruleClasses"')],
  ...
  ['compile endpoint budget resolver exposes candidate IDs', serverSource.includes('buildMsx2BudgetResolverCandidates') && serverSource.includes('resolverCandidateIds') && serverSource.includes('candidateId') && serverSource.includes('emit_multi_bank_world_data_loader') && serverSource.includes('chunkLabels') && msx2BudgetFeedbackSource.includes('resolverCandidates') && msx2BudgetFeedbackSource.includes('emit_multi_bank_world_data_loader') && msx2BudgetFeedbackSource.includes('chunkLabels')],
  ...
  ['ROM build result displays MSX2 resident contributors', serverSource.includes('analyzeMsx2ResidentAsmContributors') && serverSource.includes('residentBankAnalysis') && serverSource.includes('residentContributors') && codeExportModalSource.includes('msx2ResidentContributors') && codeExportModalSource.includes('Resident contributors:')],
  ['MSX2 resident overflow reports regeneration readiness', buildScriptSource.includes('build_msx2_resident_regeneration_readiness') && buildScriptSource.includes('residentRegenerationReadiness') && buildScriptSource.includes('readinessStatus') && serverSource.includes('buildMsx2ResidentRegenerationReadiness') && serverSource.includes('nextAutomaticAction') && serverSource.includes('readinessStatus') && codeExportModalSource.includes('Regeneration readiness:') && codeExportModalSource.includes('Move targets:') && codeExportModalSource.includes('Candidate action:')],
  ['compile endpoint exposes automatic resident overflow post-ASM resolver', serverSource.includes('run_post_asm_dead_block_optimizer') && serverSource.includes("postAsmRules: 'dead-blocks'") && serverSource.includes('move_cold_readonly_data_to_world_bank')],
  ['compile endpoint records MSX2 compile resolver attempts', serverSource.includes('msx2CompileResolution') && serverSource.includes('msx2CompileResolverAttempts') && serverSource.includes("status: 'resolved'") && serverSource.includes("status: 'failed'") && codeExportModalSource.includes('msx2CompileResolverAttempts') && codeExportModalSource.includes('Compile resolver attempts:')],
