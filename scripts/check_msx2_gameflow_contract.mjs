...
addCheck('SCREEN 4 generator keeps heavy transitions compact for resident bank', screen4Generator.includes('clear_screen4_checkerboard_phase0:') && screen4Generator.includes('clear_screen4_checkerboard_phase1:') && screen4Generator.includes("lines.push('    call clear_screen4_checkerboard_phase0')") && screen4Generator.includes('for (let radius = 0; radius <= 12; radius += 2)') && screen4Generator.includes('for (let row = 0; row < 24; row += 4)') && !screen4Generator.includes('Math.abs(column - 15) + Math.abs(row - 11)'));
