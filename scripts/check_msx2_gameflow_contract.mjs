...
addCheck('MSX2 GameFlow editor exposes low-risk control nodes', editor.includes("addNode('Waypoint')") && editor.includes("addNode('Restart')") && editor.includes("addNode('Globals')") && editor.includes("addNode('SubMenu')") && editor.includes("addNode('Text')") && editor.includes("addNode('WorldLink')") && editor.includes("addNode('IfThenElse')") && editor.includes("sourceId === 'then'") && editor.includes("sourceId === 'else'") && editor.includes('getNextExportNode') && editor.includes('nodes do not have outgoing connections'));
...
addCheck(
  ...
  types.includes("export type Msx2GameFlowNodeType = 'Start' | 'Globals' | 'Screen5Presentation' | 'SubMenu' | 'Text' | 'WorldLink' | 'Waypoint' | 'IfThenElse' | 'Transition' | 'Restart' | 'End';")
);
addCheck(
...
addCheck(
  ...
  !/export type Msx2GameFlowNode[\s\S]*(TextScroll|PresentationScreen|Controls|Music|Group)/.test(types.split('// --- End Game Flow Types ---')[0].split('// --- MSX2 Game Flow Types ---')[1] || '')
);
...
addCheck('SCREEN 4 generator reads MSX2 SubMenu background ids', screen4Generator.includes('getFlowBackgroundScreenAssetId') && screen4Generator.includes('node?.backgroundScreenAssetId') && screen4Generator.includes('getFlowBackgroundScreenAssetId(current)'));
