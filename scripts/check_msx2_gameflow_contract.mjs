...
addCheck('MSX2 GameFlow editor validates exportable SCREEN 5 flow shape', editor.includes('flowIssues') && editor.includes('Flow status') && editor.includes('Export path:') && editor.includes('flowPath.map((item, index)') && editor.includes('Start should connect to a SCREEN 5 Presentation node') && editor.includes('SCREEN 5 node should continue to Transition or End') && editor.includes('SCREEN 5 node can only continue to Transition or End') && editor.includes("afterTransition?.type !== 'End'") && editor.includes('Terminal Transition should continue to End') && editor.includes('Export path contains a cycle') && editor.includes('flowHasAnyCycle') && editor.includes('Flow contains a cycle') && editor.includes('Orphaned node') && editor.includes('End nodes do not support outgoing connections'));
