...
addCheck('MSX2 GameFlow editor validates exportable SCREEN 5 flow shape', editor.includes('flowIssues') && editor.includes('Flow status') && editor.includes('Export path:') && editor.includes('flowPath.map((item, index)') && editor.includes("presentationNode?.type !== 'Screen5Presentation'") && editor.includes('Start should connect to a SCREEN 5 Presentation node') && editor.includes('Missing SCREEN 5 Presentation node') && editor.includes('SCREEN 5 node has no valid presentation asset') && editor.includes('SCREEN 5 node should continue to Transition or End') && editor.includes('SCREEN 5 node can only continue to Transition or End') && editor.includes("afterTransition?.type !== 'End'") && editor.includes('Terminal Transition should continue to End') && editor.includes('Export path contains a cycle') && editor.includes('flowHasAnyCycle') && editor.includes('Flow contains a cycle') && editor.includes('Flow contains a connection to a missing node') && editor.includes('Orphaned node') && editor.includes('End nodes do not support outgoing connections') && editor.includes('has more than one outgoing connection'));
...
addCheck('SCREEN 5 generator resolves MSX2 gameflow presentation node', screen5PresentationGenerator.includes('resolveMsx2GameFlowPresentationNode') && screen5PresentationGenerator.includes("nextNode?.type === 'Screen5Presentation'"));
...
addCheck('SCREEN 5 generator rejects invalid MSX2 gameflow shape', screen5PresentationGenerator.includes('must start with Start -> Screen5Presentation') && screen5PresentationGenerator.includes('must continue to End or terminal Transition') && screen5PresentationGenerator.includes('nodeAfterTransition') && screen5PresentationGenerator.includes('must continue to End'));
