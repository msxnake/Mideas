...
addCheck('MSX2 GameFlow editor exposes low-risk control nodes', editor.includes("addNode('Waypoint')") && editor.includes("addNode('Restart')") && editor.includes('getNextExportNode') && editor.includes('nodes do not have outgoing connections'));
...
addCheck(
  ...
  types.includes("export type Msx2GameFlowNodeType = 'Start' | 'Screen5Presentation' | 'Waypoint' | 'Transition' | 'Restart' | 'End';")
);
addCheck(
...
addCheck('SCREEN 5 generator rejects invalid MSX2 gameflow shape', screen5PresentationGenerator.includes('must reach Screen5Presentation from Start through optional Waypoint nodes') && screen5PresentationGenerator.includes('must continue to End, Restart, or terminal Transition') && screen5PresentationGenerator.includes('nodeAfterTransition') && screen5PresentationGenerator.includes('must continue to End or Restart'));
