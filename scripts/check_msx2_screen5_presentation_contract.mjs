...
check('PNG import CLI can create a ready MSX2 GameFlow', pngImportScript.includes('--with-msx2-gameflow') && pngImportScript.includes("type\": \"msx2gameflow\"") && pngImportScript.includes("type\": \"Screen5Presentation\"") && pngImportScript.includes("presentationAssetId") && pngImportScript.includes("type\": \"Transition\"") && pngImportScript.includes("effect\": \"fade_to_black\"") && pngImportScript.includes("durationFrames\": 30"));
...
check('code export displays selected MSX2 GameFlow and SCREEN 5 presentation', codeExportModal.includes('getMsx2Screen5ExportInfo') && codeExportModal.includes('SCREEN 5 export: GameFlow=') && codeExportModal.includes('Terminal transition=') && codeExportModal.includes('invalidFlowShape') && codeExportModal.includes("startNextNode?.type !== 'Screen5Presentation'") && codeExportModal.includes('missingPresentation') && codeExportModal.includes("nodeAfterTransition?.type === 'End'") && codeExportModal.includes('missing SCREEN 5 presentation asset') && codeExportModal.includes('reachable Screen5Presentation node') && codeExportModal.includes('optional terminal Transition'));
...
check('CLI smoke validates MSX2 GameFlow markers when fixture has one', smokeScript.includes('get_msx2_gameflow_contract(fixture)') && smokeScript.includes('assert_msx2_gameflow_asm_contract') && smokeScript.includes('MSX2_GAMEFLOW_PRESENTATION_ASSET_ID'));
...
check('presentation generator emits terminal MSX2 GameFlow transitions', presentationGenerator.includes('resolveNextTransition') && presentationGenerator.includes('nodeAfterTransition') && presentationGenerator.includes('must continue to End') && presentationGenerator.includes('MSX2_GAMEFLOW_NEXT_TRANSITION') && presentationGenerator.includes('msx2_gameflow_run_transition') && presentationGenerator.includes('screen5_black_palette_data') && presentationGenerator.includes('clear_screen5_visible_vram') && presentationGenerator.includes('FILVRM'));
